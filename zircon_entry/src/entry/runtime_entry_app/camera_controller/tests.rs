...
use zircon_scene::{NodeKind, World};
...
fn right_drag_orbits_active_camera() {
    let (mut scene, _) = test_scene_with_selected_cube();
    let initial = scene
        .find_node(scene.active_camera())
        .expect("camera")
        .transform;
    let mut controller = RuntimeCameraController::new(UVec2::new(1280, 720));
    controller.set_orbit_target(test_selection_translation(&scene));
...
fn test_scene_with_selected_cube() -> (World, u64) {
    ...
    scene.set_selected(Some(selected));
    (scene, selected)
}

fn test_selection_translation(scene: &World) -> zircon_math::Vec3 {
    let selected = scene.selected_node().expect("selected node");
    scene
        .find_node(selected)
