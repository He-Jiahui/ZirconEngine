...
impl UiTree {
    ...
    pub(crate) fn focusable_nodes_in_navigation_order(&self) -> Result<Vec<UiNodeId>, UiTreeError> {
        let mut focusable = Vec::new();
        for root_id in &self.roots {
            self.collect_focusable_nodes(*root_id, &mut focusable)?;
        }
        Ok(focusable)
    }

    pub(crate) fn next_focusable_target(
        &self,
        ...
        match kind {
            ...
            | UiNavigationEventKind::Right => {
                ...
                self.nearest_focusable_in_direction(current, kind, &focusable)
            }
    ...
    fn collect_focusable_nodes(
        ...
        for child_id in &node.children {
            self.collect_focusable_nodes(*child_id, focusable)?;
        }
        Ok(())
    }
