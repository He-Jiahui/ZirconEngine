...
- `render_product_advanced.rs::render_product_hzb_occlusion_respects_storage_buffer_limit_fallback` isolates the per-stage storage-buffer capacity gate. The fixture leaves storage-buffer support and GPU-driven submission enabled, drops only `max_storage_buffers_per_shader_stage` below `HZB_OCCLUSION_REQUIRED_STORAGE_BUFFERS_PER_SHADER_STAGE`, and proves the renderer falls back to CPU visibility without reporting HZB occlusion, compact replay, or visibility occlusion culls.
...
The 2026-06-21 storage-buffer capacity product guard adds `render_product_hzb_occlusion_respects_storage_buffer_limit_fallback`. The test reuses the wall scene but keeps only the HZB storage-buffer binding capacity below the required threshold, proving the product path reports no HZB occlusion execution, no visibility occlusion culls, and no compact replay counts while the captured frame matches the CPU fallback. `rustfmt --edition 2021 zircon_runtime\src\graphics\tests\render_product_advanced.rs` passed. The Cargo wrapper for the exact filter timed out twice during shared lib-test compilation, but the residual compile naturally produced `target\codex-runtime-hzb-storage-limit-0620\debug\deps\zircon_runtime-c339c28ec98a5de7.exe`; direct exact execution passed the new storage-limit test 1/1 and re-ran `render_product_hzb_occlusion_wall_scene` 1/1.

The same warmed binary also closes the current non-RenderDoc diagnostics focused sweep for Plan 04. Direct exact runs passed `render_product_diagnostics_record_hzb_stats`, `render_product_diagnostics_record_visibility_stats`, and `runtime_diagnostics_combines_core_render_contract_and_missing_externalized_plugins` 3/3. That confirms the HZB occlusion stat surface, visibility stat surface, and runtime `DiagnosticStore` aggregate still publish the planned rows after the product fallback guard landed. RenderDoc capture validation remains open.
