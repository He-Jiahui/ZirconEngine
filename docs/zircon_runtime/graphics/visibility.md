...
tests:
  ...
  - zircon_runtime/src/graphics/scene/scene_renderer/hzb/hzb_occlusion_culler.rs::tests::hzb_occlusion_dispatch_groups_cover_indirect_args
...
`FrameVisibility` and `ViewVisibilityContext` now provide the per-view container. The current implementation emits the main camera, directional `ShadowCascade { light, cascade }` views, point-light `ShadowPointFace { light, face }` views, and spot-light `ShadowSpot { light }` views. Shadow-casting directional lights emit four cascade keys to match Plan 05 CSM atlas slots; non-shadow directional lights keep one conservative cascade-0 view for the legacy/default-shadow path. Directional cascade cameras now use the same Plan 05 camera frustum slice bounds that drive `ShadowFramePlan`, while point/spot cameras are built from authored light position, direction, range, and cone/face data. It already uses the final shape: a stable frame primitive index space (`entities`, `bounds`, `relevance`) plus view-local visible indices and `ViewCullingStats`. `VisibilityContext` keeps `frame_visibility` beside the legacy flat fields so later custom render-target camera slices can migrate consumers without breaking existing batching and history code.
...
The visibility module itself remains WGPU-free for VC-M3. Its new contract is `HzbOcclusionCullReport`, with `HzbOcclusionPhase::SingleFrameReproject` as the implemented phase and `TwoPhaseRetest` reserved for a later two-stage retest/redraw path. The report carries candidate arg count, candidate instance count, dispatch group count, dispatched phase count, whether previous HZB history was available, optional `HzbOcclusionCullReadbackStats` copied from the WGPU stats buffer, and optional `HzbOcclusionIndirectArgsReadbackSummary` copied from the phase-local indirect args buffers after the occlusion pass.
...
The compute shader includes `zr_gpu_scene.wgsl` and `zr_hzb.wgsl`. It dispatches one thread per indirect args record, walks the record's instance range, projects GPUScene primitive bounds through `SceneUniform.previous_view_proj`, chooses a previous-HZB mip from screen radius, and uses a conservative nearest-depth <= furthest-depth comparison. Each tested args record atomically increments tested arg/instance counters. If no instance in a batch is conservatively visible, it writes `instance_count = 0` into that indirect args record and atomically increments culled arg/instance counters.

The built-in HZB feature now declares two passes:

- `hzb-occlusion-cull` in `DepthPrepass`, executor `visibility.hzb-occlusion-cull`, async-compute declared queue, previous-HZB read, execution-owned external reads/writes for compaction metadata, indirect args, visible-instance remap, draw-count, and HZB stats, plus `RenderGraphComputeWorkload::indirect_args(...)`.
...
`RenderGraphComputeWorkloadDispatchContext` has an `IndirectArgs` extent so graph execution audit can compare planned occlusion work against the mesh indirect args count. `MeshIndirectDrawExecution` now creates args buffers with `STORAGE` usage and exposes `args_count()` / `total_instances()`, while `RenderPassMeshCommandLists` can report the total occlusion cull candidate arg and instance counts across all mesh phases. `MeshIndirectArgsReadback` and `MeshIndirectArgsSnapshot` provide the post-cull readback parser for real WGPU indirect args buffers, including `zero_instance_arg_count()` for args word1 validation and `remaining_instance_count()` for product diagnostics.

`mesh_pass::IndirectCompactionPlan` is now the Rust-side ABI for the later UE-style clear + atomic compact path. It is built from the same phase-local `IndexedIndirectArgs` rows that back the WGPU indirect args buffer. Each metadata row preserves the source args record index, source `first_instance/count`, and the prefix-assigned visible-instance remap base. The plan also exposes metadata, visible-instance-index, and draw-count buffer sizes. `MeshIndirectCompactionResources` now allocates the matching WGPU buffers per `MeshIndirectDrawExecution`: metadata storage initialized from the plan, a visible-instance-index storage/copy buffer, and a zeroed storage/copy/indirect draw-count buffer. Before each phase HZB occlusion dispatch, `HzbOcclusionCuller` now encodes a clear for the visible-instance remap allocation and draw-count buffer, leaving metadata immutable. This makes the next shader step explicit: atomically append visible source instance indices into the remap buffer, rewrite indirect args to the visible remap base/count, then teach group3 mesh shaders to translate `@builtin(instance_index)` through that remap. That atomic shader/replay migration has not been done yet.

Runtime graph execution injects `HzbOcclusionCuller` only into stages that have mesh draw lists. `record_hzb_occlusion_cull_to_indirect_args(...)` uses `history.previous.hzb-furthest` when present and falls back to the post-process white texture view on the first frame, then records compute storage writes against `"mesh.indirect-args"`, `"mesh.visible-instance-index"`, `"mesh.indirect-draw-count"`, and `"visibility.hzb-occlusion-stats"` for graph audit. Compaction metadata is declared as an external read in the feature descriptor because it is execution-owned and immutable for the pass.

The returned `HzbOcclusionCullReport` is preserved through `RenderPassGpuExecutionContext`, `RenderGraphExecutionRecord`, and `SceneRenderer::last_hzb_occlusion_cull_report()`. After graph execution but before encoder submission, `SceneRendererCore` encodes readbacks for the same phase-local indirect args buffers that the HZB shader may rewrite. After submission, it maps the HZB stats readback buffer and those args readback buffers, attaching exact stats plus the indirect args summary to the report when dispatches executed; zero-dispatch reports get exact zero rows. `update_base_stats(...)` copies the report into `RenderStats`, while frames without the pass explicitly reset the HZB occlusion report fields to zero/false. Product diagnostics expose this surface as `render.hzb.occlusion.reported`, `candidate_arg_count`, `candidate_instance_count`, `dispatch_group_count`, `dispatched_phase_count`, `history_available`, `readback_available`, `tested_*`, `culled_*`, `indirect_args_readback_available`, `readback_arg_count`, `zero_instance_arg_count`, and `remaining_instance_count`. `graphics::visibility` re-exports `HzbOcclusionCullReport`, `HzbOcclusionCullReadbackStats`, and `HzbOcclusionIndirectArgsReadbackSummary` from the root module so scene-renderer and render-stats consumers do not import through the private occlusion subtree.
...
- The visible-instance remap/compact ABI and per-execution WGPU buffers are present in mesh pass execution, but VC-M3 still uses the single-frame zero-instance V1 shader until clear pass wiring, atomic compact WGSL, graph declarations, replay count-buffer use, and group3 remap consumption are wired.
...
`VisibilityContext` still exposes the pre-existing single-view fields for compatibility. The per-view `FrameVisibility` / `ViewVisibilityContext` shape is now present for the main camera, directional cascades, point faces, and spot shadows, and the legacy `visible_entities` / `visible_batches` compatibility fields are derived from the explicit main-view result. Directional cascade views now use Plan 05 CSM frustum-slice bounds instead of sharing one frame-bounds light camera. Custom render-target cameras are not yet populated because the current `RenderFrameExtract` only carries one concrete camera snapshot; `scene_camera_order_report` has ordering metadata but not the per-camera projection/transform payload needed to cull `CustomTarget` views. That dependency belongs to plan 09 camera descriptor work.
