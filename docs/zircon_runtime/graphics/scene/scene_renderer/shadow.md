...
plan_sources:
  ...
  - user: 2026-07-10 continue PBR/HDRI viewer DX12 and RenderDoc validation
...
- `shadow/shaders/zr_shadow.wgsl` owns the shader-side atlas sampling helper. It reads `GpuLightData.shadow_slot_layer`, chooses directional cascades or point faces, projects through `ZrShadowSlot.view_proj`, and selects Low/Medium/High comparison-sampler PCF kernels from the slot quality flags. Each PCF tap uses explicit-level `textureSampleCompareLevel(...)`; this preserves depth comparison and the existing kernels while keeping dynamic forward/deferred light loops derivative-free for DX12/FXC. High keeps nine samples but uses a wider 8-texel radius so product captures can distinguish it from Low on receiver edges.
...
The 2026-07-10 DX12 portability slice locks the shared PCF helper to explicit-level comparison sampling. The viewer rebuild passed, direct DX12 execution exited 0 after graphics-debugger capture start/stop, and RenderDoc DX12 launch exited 0 with a 13,378,644-byte capture under `docs/tests/runtime/shader`. This proves the assembled shadow receiver shaders no longer trigger FXC's dynamic-loop implicit-gradient unroll failure. It does not replace the dedicated shadow-atlas visual/product captures or their wider scene sweeps.
