---
related_code:
  - zircon_runtime/src/graphics/resource_limits.rs
  - zircon_runtime/src/graphics/backend/render_backend/request_device.rs
  - zircon_runtime/src/core/framework/render/mod.rs
  - zircon_runtime/src/core/framework/render/post_process/color_space.rs
  - zircon_runtime/src/core/framework/render/post_process/chain.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component/params.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component/tests.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_registry.rs
  - zircon_runtime/src/core/framework/render/post_process/resolved_stack.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_profile.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_extract.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_evaluator.rs
  - zircon_runtime/src/core/framework/render/post_process/exposure_settings.rs
  - zircon_runtime/src/core/framework/render/post_process/exposure_readback.rs
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs
  - zircon_runtime/src/core/framework/render/anti_alias/fallback.rs
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs
  - zircon_runtime/src/core/framework/render/frame_extract.rs
  - zircon_runtime/src/core/framework/render/backend_types.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/anti_alias.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/post_process.rs
  - zircon_runtime/src/graphics/backend/mod.rs
  - zircon_runtime/src/graphics/runtime/render_framework/submit_frame_extract/build_frame_submission_context/build.rs
  - zircon_runtime/src/graphics/runtime/render_framework/submit_frame_extract/update_stats/base_stats.rs
  - zircon_runtime/src/scene/world/render_post_process.rs
  - zircon_runtime/src/graphics/backend/render_backend/read_buffer_f32x4.rs
  - zircon_runtime/src/graphics/backend/render_backend/read_texture_rgba16float_3d.rs
  - zircon_runtime/src/core/framework/render/post_process/effect.rs
  - zircon_runtime/src/core/framework/render/post_process/stack.rs
  - zircon_runtime/src/core/framework/render/post_process/graph_resource_names.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/effect_stack.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/exposure.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/screen_space_reflection.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/temporal_history.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/terminal_chain.rs
  - zircon_runtime/src/rhi/descriptors.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/anti_alias.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/color_grading.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/post_process.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/descriptor_filtering.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/plugin_render_features.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/anti_alias/smaa.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_record.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_resources/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/transient_resource_pool.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/exposure.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/texture_formats.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/params/exposure_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/params/color_lut_bake_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/post_process.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/exposure_histogram.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/exposure_resolve.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/color_lut_bake.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/upscale.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/output_transfer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/smaa.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_exposure/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_color_lut_bake/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_depth_of_field/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_motion_blur/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_scene_composite/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_blur/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/build_post_process_params/build.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/pass_params_buffer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_output_transfer/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_fxaa/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_smaa/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/render_region.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_upscale/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_buffer_bundle/color_lut_bake_params_buffer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/taa_resolve_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/output_transfer_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/fxaa_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/smaa_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/upscale_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/exposure_histogram_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/exposure_resolve_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/color_lut_bake_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/build.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/bloom_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_bloom/execute_bloom.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/depth_of_field_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/motion_blur_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/scene_composite_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/blur_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/full_scene_post_process_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/profiled_scene_post_process_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/scene_output_transfer_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/pass_graph/execute.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_executor_registry.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_execution_context/gpu/post_process.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_execution_context/gpu.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/render/render.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/history/scene_frame_history_textures/construct.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/history/copy_history_textures.rs
  - zircon_runtime/src/graphics/types/viewport_render_region.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pass/processors/taa_reactive_mask.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline/create_taa_reactive_mask_mesh_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline_cache/ensure_taa_reactive_mask_pipeline.rs
  - zircon_runtime/src/core/framework/render/material/standard_material.rs
  - zircon_runtime/src/asset/assets/material/material_control.rs
  - zircon_runtime/src/graphics/scene/resources/gpu_material_uniform/gpu_material_uniform_resource.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/execute_taa_resolve.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/taa_resolve_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/temporal_history_store.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/post_process.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/color_lut_bake.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/exposure_histogram.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/exposure_resolve.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/post_process_screen_space_reflection.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/output_transfer.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/fxaa.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/smaa.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/upscale.wgsl
  - zircon_runtime/src/graphics/tests/render_product_post_process.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process/motion_blur.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_terminal.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_volume.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_full_chain.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_full_chain/fixture.rs
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/production_file_budget/render_product_post_process_motion_blur_tests.rs
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/render_post_process_stack.rs
implementation_files:
  - zircon_runtime/src/graphics/resource_limits.rs
  - zircon_runtime/src/graphics/backend/render_backend/request_device.rs
  - zircon_runtime/src/core/framework/render/mod.rs
  - zircon_runtime/src/core/framework/render/post_process/color_space.rs
  - zircon_runtime/src/core/framework/render/post_process/chain.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component/params.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_component/tests.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_registry.rs
  - zircon_runtime/src/core/framework/render/post_process/resolved_stack.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_profile.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_extract.rs
  - zircon_runtime/src/core/framework/render/post_process/volume_evaluator.rs
  - zircon_runtime/src/core/framework/render/post_process/exposure_settings.rs
  - zircon_runtime/src/core/framework/render/post_process/exposure_readback.rs
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs
  - zircon_runtime/src/core/framework/render/anti_alias/fallback.rs
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs
  - zircon_runtime/src/core/framework/render/frame_extract.rs
  - zircon_runtime/src/core/framework/render/backend_types.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/anti_alias.rs
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/post_process.rs
  - zircon_runtime/src/graphics/backend/mod.rs
  - zircon_runtime/src/graphics/runtime/render_framework/submit_frame_extract/build_frame_submission_context/build.rs
  - zircon_runtime/src/graphics/runtime/render_framework/submit_frame_extract/update_stats/base_stats.rs
  - zircon_runtime/src/scene/world/render_post_process.rs
  - zircon_runtime/src/graphics/backend/render_backend/read_buffer_f32x4.rs
  - zircon_runtime/src/graphics/backend/render_backend/read_texture_rgba16float_3d.rs
  - zircon_runtime/src/core/framework/render/post_process/effect.rs
  - zircon_runtime/src/core/framework/render/post_process/stack.rs
  - zircon_runtime/src/core/framework/render/post_process/graph_resource_names.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/effect_stack.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/exposure.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/screen_space_reflection.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/temporal_history.rs
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/terminal_chain.rs
  - zircon_runtime/src/rhi/descriptors.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/anti_alias.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/color_grading.rs
  - zircon_runtime/src/graphics/feature/builtin_render_feature_descriptor/feature_descriptors/post_process.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/descriptor_filtering.rs
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/plugin_render_features.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/anti_alias/smaa.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_record.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_resources/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/transient_resource_pool.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/exposure.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/constants/texture_formats.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/params/exposure_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/params/color_lut_bake_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/post_process.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/exposure_histogram.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/exposure_resolve.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/color_lut_bake.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/upscale.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/output_transfer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/smaa.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_exposure/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_color_lut_bake/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_depth_of_field/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_motion_blur/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_scene_composite/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_blur/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/build_post_process_params/build.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/pass_params_buffer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_output_transfer/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_fxaa/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_smaa/mod.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/render_region.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_upscale/mod.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_full_chain.rs
  - zircon_runtime/src/graphics/tests/render_product_post_process_full_chain/fixture.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_buffer_bundle/color_lut_bake_params_buffer.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/taa_resolve_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/output_transfer_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/fxaa_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/smaa_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/upscale_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/exposure_histogram_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/exposure_resolve_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/color_lut_bake_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/build.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/bloom_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_bloom/execute_bloom.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/depth_of_field_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/motion_blur_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/scene_composite_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/blur_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/full_scene_post_process_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/profiled_scene_post_process_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/scene_post_process_resources/scene_output_transfer_resources.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/pass_graph/execute.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_executor_registry.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_execution_context/gpu/post_process.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_execution_context/gpu.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/render/render.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/history/scene_frame_history_textures/construct.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pass/processors/taa_reactive_mask.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline/create_taa_reactive_mask_mesh_pipeline.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline_cache/ensure_taa_reactive_mask_pipeline.rs
  - zircon_runtime/src/core/framework/render/material/standard_material.rs
  - zircon_runtime/src/asset/assets/material/material_control.rs
  - zircon_runtime/src/graphics/scene/resources/gpu_material_uniform/gpu_material_uniform_resource.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/execute_taa_resolve.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/taa_resolve_params.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/temporal/taa/temporal_history_store.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/post_process.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/color_lut_bake.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/exposure_histogram.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/exposure_resolve.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/post_process_screen_space_reflection.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/output_transfer.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/fxaa.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/smaa.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/shaders/upscale.wgsl
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/history/copy_history_textures.rs
  - zircon_runtime/src/graphics/types/viewport_render_region.rs
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/render_post_process_stack.rs
plan_sources:
  - docs/plans/zircon_runtime/render/06-temporal-pipeline.md
  - docs/plans/zircon_runtime/render/07-postprocess-color-pipeline.md
  - docs/plans/zircon_runtime/render/09-camera-render-ordering.md
tests:
  - zircon_runtime/src/core/framework/render/post_process/chain.rs::tests::render_post_chain_backbone_order_is_stable
  - zircon_runtime/src/core/framework/render/post_process/chain.rs::tests::render_post_chain_current_effect_kinds_have_migration_slots
  - zircon_runtime/src/core/framework/render/post_process/chain.rs::tests::render_post_chain_planned_executor_ids_are_stable
  - zircon_runtime/src/graphics/backend/render_backend/request_device.rs::tests::offscreen_device_limits_cover_renderer_layout_requirements
  - zircon_runtime/src/graphics/backend/render_backend/request_device.rs::tests::offscreen_device_limits_keep_hzb_optional_when_adapter_limit_is_lower
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/post_process.rs::tests::post_process_layout_sampled_texture_count_matches_device_request_limit
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/materialization/tests.rs::non_storage_texture_formats_do_not_request_storage_binding
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/materialization/tests.rs::storage_texture_formats_request_storage_binding
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/build_post_process_params/build.rs::tests::post_process_projection_params_ignore_temporal_jitter
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_post_process/execute/build_post_process_params/build.rs::tests::post_process_params_pack_physical_viewport_and_local_scene_source_origins_separately
  - zircon_runtime/src/graphics/types/viewport_render_region.rs::tests::viewport_region_maps_local_postprocess_coords_to_physical_target_coords
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/history/copy_history_textures.rs::tests::history_region_copy_targets_selected_camera_region
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/history/copy_history_textures.rs::tests::history_region_copy_clamps_dynamic_resolution_to_viewport_region
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/history/copy_history_textures.rs::tests::history_region_copy_uses_local_extent_and_physical_destination
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/temporal_history.rs::taa_resolve_declares_history_velocity_and_output_transfer_input
  - zircon_runtime/src/graphics/tests/pipeline_compile/temporal_and_ops.rs::taa_resolve_compiles_temporal_history_pass_when_taa_stack_is_effective
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_executor_registry/tests.rs::taa_reactive_mask_mesh_executor_requires_graph_resources_instead_of_nooping
  - zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pass/processors/mod.rs::tests::taa_reactive_mask_processor_draws_visible_transparent_batches_only
  - zircon_runtime/src/asset/tests/assets/material.rs::material_owned_taa_reactive_mask_strength_drives_standard_descriptor_without_shader_override
  - zircon_runtime/src/asset/tests/assets/material.rs::material_owned_taa_reactive_mask_strength_reports_invalid_override
  - zircon_runtime/src/core/framework/render/post_process/color_space.rs::tests::render_post_color_space_intermediate_hdr_defaults_to_rg11b10ufloat
  - zircon_runtime/src/core/framework/render/post_process/color_space.rs::tests::render_post_color_lut_contract_keeps_power_of_two_sizes
  - zircon_runtime/src/core/framework/render/post_process/color_space.rs::tests::render_post_output_transfer_defaults_to_srgb
  - zircon_runtime/src/graphics/backend/render_backend/request_device.rs::tests::offscreen_device_features_require_rg11b10_render_target_for_post_process
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs::tests::compile_describes_hzb_and_ssr_reflection_pyramids_as_mip_chain_transients
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/effect_stack.rs::enabled_effect_stack_declares_tonemapped_for_uber_descriptor
  - zircon_runtime/src/graphics/tests/pipeline_compile/plugin_features.rs::rendering_plugin_post_process_routes_output_transfer_through_terminal_anti_alias_input
  - zircon_runtime/src/graphics/tests/project_render/render_quality.rs::ssao_product_default_is_fail_closed_and_preserves_scene_output
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/taa_resolve_pipeline.rs::tests::taa_resolve_shader_parses_and_declares_history_outputs
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs::tests::output_transfer_shader_source_declares_single_texture_input
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/output_transfer_pipeline.rs::tests::output_transfer_shader_parses
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_executor_registry/tests.rs::builtin_registry_covers_product_postprocess_executor_ids
  - zircon_runtime/src/core/framework/render/post_process/volume_component/tests.rs::render_volume_param_interp_blends_float_vec3_and_discrete_values
  - zircon_runtime/src/core/framework/render/post_process/volume_component/tests.rs::render_volume_component_descriptor_applies_defaults_to_resolved_stack
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/render_post_process_volume_component.rs::runtime_15_post_process_volume_component_is_folder_backed
  - zircon_runtime/src/core/framework/render/post_process/volume_registry.rs::tests::render_volume_registry_exposes_builtin_post_process_components
  - zircon_runtime/src/core/framework/render/post_process/volume_registry.rs::tests::render_volume_registry_rejects_duplicate_component_id
  - zircon_runtime/src/core/framework/render/post_process/volume_extract.rs::tests::render_volume_extract_maps_profile_to_component_overrides
  - zircon_runtime/src/core/framework/render/post_process/volume_extract.rs::tests::render_volume_extract_stores_global_box_and_sphere_shapes
  - zircon_runtime/src/core/framework/render/post_process/volume_evaluator.rs::tests::render_volume_evaluator_blends_global_volumes_by_priority_order
  - zircon_runtime/src/core/framework/render/post_process/volume_evaluator.rs::tests::render_volume_evaluator_box_blend_distance_weight
  - zircon_runtime/src/core/framework/render/post_process/volume_evaluator.rs::tests::render_volume_evaluator_keeps_unset_component_params_from_current_stack
  - zircon_runtime/src/core/framework/render/post_process/exposure_settings.rs::tests
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/exposure.rs::manual_exposure_declares_resolve_without_histogram
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/exposure.rs::histogram_exposure_declares_histogram_before_resolve
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/exposure.rs::default_stack_declares_light_list_for_uber_cluster_bind_group
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs::tests::compile_describes_color_lut_as_rgba16float_3d_transient_when_enabled
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_color_lut_bake/mod.rs::tests::color_lut_bake_dispatch_covers_default_lut_volume
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_color_lut_bake/mod.rs::tests::color_lut_bake_is_required_for_all_color_transform_sources
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_color_lut_bake/mod.rs::tests::color_lookup_layout_binding_modes_match_shader_contract
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/color_lut_bake_pipeline.rs::tests::color_lut_bake_shader_declares_compute_entry_and_3d_storage_output
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/color_lut_bake_pipeline.rs::tests::color_lut_bake_shader_bakes_tonemap_grading_and_user_lut
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs::tests::color_lut_bake_shader_source_declares_internal_bake_outputs
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_record.rs::tests::execution_record_tracks_compute_dispatch_metadata
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_record.rs::tests::execution_record_preserves_color_lut_readback_report
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_record.rs::tests::execution_record_preserves_exposure_readback_report
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_execution_context/gpu/post_process.rs::tests::color_lut_bake_dispatch_reports_exposure_read_and_lut_write
  - zircon_runtime/src/core/framework/render/post_process/exposure_readback.rs::tests::exposure_readback_report_accepts_valid_history_words
  - zircon_runtime/src/core/framework/render/post_process/exposure_readback.rs::tests::exposure_readback_report_rejects_invalid_length_and_nan_words
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs::tests::color_lut_readback_report_accepts_identity_rgba16float_bytes
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs::tests::color_lut_readback_report_accepts_color_transform_reference_rgba16float_bytes
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs::tests::color_lut_readback_report_rejects_non_identity_rgba16float_bytes
  - zircon_runtime/src/core/framework/render/post_process/color_lut_readback.rs::tests::color_lut_readback_report_tracks_invalid_byte_length
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/post_process.rs::tests::post_process_diagnostics_record_color_lut_readback_identity_report
  - zircon_runtime/src/core/runtime/diagnostics/render_stats_store/post_process.rs::tests::post_process_diagnostics_record_exposure_readback_report
  - zircon_runtime/src/graphics/backend/render_backend/read_buffer_f32x4.rs::tests::f32x4_readback_decodes_little_endian_words
  - zircon_runtime/src/graphics/backend/render_backend/read_texture_rgba16float_3d.rs::tests::rgba16float_3d_readback_strips_row_padding_per_slice
  - zircon_runtime/src/graphics/tests/render_framework_bridge/stats.rs::render_framework_stats_report_neutral_color_lut_readback_identity
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/terminal_chain.rs::fxaa_terminal_anti_alias_routes_output_transfer_through_terminal_input
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs::tests::compile_routes_output_transfer_through_fxaa_terminal_input
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile_tests.rs::compile_routes_output_transfer_through_fxaa_terminal_input
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::output_transfer_executor_targets_terminal_input_when_declared
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::output_transfer_executor_defaults_to_final_color
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/fxaa_pipeline.rs::tests::fxaa_shader_parses
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs::tests::fxaa_shader_source_declares_terminal_input_and_neighbor_filter
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/smaa_pipeline.rs::tests::smaa_shader_parses
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/smaa_pipeline.rs::tests::smaa_internal_stage_format_is_sdr_weight_texture
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_smaa/mod.rs::tests::smaa_stage_textures_store_edge_and_blend_weights
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs::tests::smaa_shader_source_declares_terminal_input_and_edge_filter
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs::tests::compile_routes_output_transfer_through_smaa_terminal_input
  - zircon_runtime/src/graphics/tests/plugin_feature_compile.rs::builtin_smaa_terminal_aa_pass_compiles_after_output_transfer_when_requested
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_pass_executor_registry/tests.rs::builtin_registry_covers_product_postprocess_executor_ids
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/materialization/tests.rs::materialization_overrides_preimported_terminal_aa_input_with_owned_transient
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/terminal_chain.rs::dynamic_resolution_declares_upscale_before_output_transfer
  - zircon_runtime/src/graphics/tests/pipeline_compile/dynamic_resolution.rs::dynamic_resolution_scales_internal_graph_resources_without_resizing_viewport_output
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::output_transfer_executor_reads_upscaled_input_when_declared
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/shader_sources.rs::tests::upscale_shader_source_declares_filtered_source_sampling
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/upscale_pipeline.rs::tests::upscale_shader_parses
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/transient_resource_pool.rs::tests::render_post_dynamic_resolution_scale_swap_releases_pool
  - zircon_runtime/src/graphics/pipeline/render_pipeline_asset/compile.rs::tests::compile_routes_bloom_extract_after_split_scene_color_passes
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::bloom_executor_reads_motion_blurred_source_when_declared
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::bloom_executor_falls_back_to_scene_color_input
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/effect_stack.rs::effect_stack_blur_feeds_uber_from_dedicated_intermediate
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/execute_blur/mod.rs::tests::blur_pass_writes_dedicated_intermediate_resource
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/blur_pipeline.rs::tests::blur_shader_entry_is_split_from_uber
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/builtin_postprocess_executors.rs::tests::uber_executor_reads_blurred_source_when_declared
  - zircon_runtime/src/graphics/tests/render_product_post_process.rs::render_product_post_uber_light_effects_change_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process.rs::render_product_post_non_neutral_tonemap_grading_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process.rs::render_product_post_user_lut_texture_changes_final_frame_and_matches_readback_reference
  - zircon_runtime/src/graphics/tests/render_product_post_process/motion_blur.rs::render_product_post_motion_blur_split_uses_velocity_and_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_blur.rs::render_product_post_blur_split_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_depth_of_field.rs::render_product_post_depth_of_field_split_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_scene_composite.rs::render_product_post_scene_composite_fog_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_scene_composite.rs::render_product_post_scene_composite_ssr_changes_final_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_terminal.rs::render_product_post_terminal_fxaa_changes_final_frame_after_output_transfer
  - zircon_runtime/src/graphics/tests/render_product_post_process_terminal.rs::render_product_post_dynamic_resolution_upscale_feeds_smaa_terminal_frame
  - zircon_runtime/src/graphics/tests/render_product_post_process_volume.rs::render_product_post_volume_camera_transition
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/pass_graph/execute.rs::tests::post_process_pass_graph_records_nodes_from_executed_executor_ids
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/pass_graph/execute.rs::tests::post_process_pass_graph_records_bloom_extract_executor_as_bloom_node
  - zircon_runtime/src/graphics/runtime/render_framework/submit_frame_extract/update_stats/base_stats.rs::tests::effect_stack_resource_status_detects_split_motion_blur_node_motion_vectors
  - zircon_runtime/src/core/framework/tests.rs::render_product_post_process_graph_elides_disabled_effects
  - zircon_runtime/src/core/framework/tests.rs::render_product_post_process_stack_elides_history_until_history_is_available
  - zircon_runtime/src/core/framework/tests.rs::render_product_post_process_stack_can_drop_history_from_validated_graph
  - zircon_runtime/src/core/framework/tests.rs::render_product_post_process_effect_stack_runs_before_output_transfer_when_authored
  - zircon_runtime/src/core/framework/tests.rs::render_product_post_process_extended_effect_stack_settings_enable_product_node
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs::tests::taa_resolution_reports_camera_msaa_sample_count_normalization
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs::tests::unsupported_terminal_aa_reports_slot_normalization
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs::tests::smaa_resolution_keeps_terminal_mode_when_supported
  - zircon_runtime/src/core/framework/render/anti_alias/settings.rs::tests::auto_resolution_uses_smaa_when_fxaa_is_unavailable
  - zircon_runtime/src/core/framework/render/backend_types.rs::tests::screen_space_anti_alias_capability_accepts_smaa
  - zircon_runtime/src/core/framework/render/post_process/stack/tests/terminal_chain.rs::smaa_terminal_anti_alias_routes_output_transfer_through_terminal_input
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/render_post_process_stack.rs::runtime_15_post_process_stack_is_folder_backed
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/bind_group_layouts/post_process.rs::tests::post_process_layout_binds_resolved_exposure_storage_buffer
  - zircon_runtime/src/graphics/scene/scene_renderer/post_process/resources/construct/create_pipeline_bundle/post_process_pipeline.rs::tests::post_process_shader_multiplies_color_grading_by_resolved_exposure
  - zircon_runtime/src/scene/tests/render_post_process_extract.rs::local_sphere_post_process_volume_uses_camera_distance_for_full_influence
  - zircon_runtime/src/scene/tests/render_post_process_extract.rs::local_box_post_process_volume_uses_camera_distance_for_blend
  - source scan for retired PP-M1-S3 names: PostProcessEffectKind::ColorGrading, PostProcessEffectKind::EffectStack, post.color-grade, post.effect-stack, post.stack
  - source scan for Plan 07 PP-M2-S2 extract/shape/override symbols
  - source scan for Plan 07 PP-M2-S3 evaluator/read hook/submit cutover symbols
  - source scan for Plan 07 PP-M3-S1b exposure graph/resource/executor symbols
  - source scan for Plan 07 PP-M3-S2 COLOR_LUT graph resource, WGPU compute bake pipeline, dispatch record resource accesses, and internal baked-LUT uber mode
  - cargo test -p zircon_runtime --lib render_product_post_uber_light_effects_change_final_frame --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_non_neutral_tonemap_grading_changes_final_frame --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_non_neutral_tonemap_grading_changes_final_frame --locked --target-dir target\codex-runtime-postprocess-0621 -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_user_lut_texture_changes_final_frame_and_matches_readback_reference --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib compile_routes_blur_split_through_uber_and_output_transfer --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_blur_split_changes_final_frame --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_motion_blur_split_uses_velocity_and_changes_final_frame --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib effect_stack_resource_status_detects_split_motion_blur_node_motion_vectors --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_depth_of_field_split_changes_final_frame --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --color never -- --test-threads=1 --nocapture
  - target\codex-runtime-postprocess-0621\debug\deps\zircon_runtime-c339c28ec98a5de7.exe render_product_post_scene_composite_fog_changes_final_frame --test-threads=1 --nocapture
  - target\codex-runtime-postprocess-0621\debug\deps\zircon_runtime-c339c28ec98a5de7.exe render_product_post_ --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_scene_composite_ssr_changes_final_frame --locked --target-dir target\codex-runtime-postprocess-0621 -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib post_process_pass_graph_records_nodes_from_executed_executor_ids --locked --target-dir target\codex-runtime-postprocess-0621 -- --test-threads=1
  - docs/tests/runtime/render/plan08_current_render_product_post_process_group_after_all_fixes_20260704.out.log
  - docs/tests/runtime/render/plan08_current_bloom_extract_node_mapping_binary_after_effect_guard_20260704.out.log
  - docs/tests/runtime/render/plan08_current_bloom_order_binary_after_all_fixes_20260704.out.log
  - docs/tests/runtime/render/plan08_post_process_params_direct_after_camera_target_fixes_20260704.out.log
  - docs/tests/runtime/render/plan08_history_region_copy_direct_after_camera_target_fixes_20260704.out.log
  - cargo test -p zircon_runtime --lib render_product_post_ --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-runtime-postprocess-0621 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_runtime --lib render_product_post_ --locked --target-dir target\codex-runtime-postprocess-0621 -- --test-threads=1 --nocapture
  - cargo check -p zircon_runtime --lib --locked --target-dir target\codex-runtime-postprocess-0621
  - cargo test -p zircon_runtime --lib render_product_post_process_terminal --locked (2026-06-22: passed 2/2)
  - cargo test -p zircon_runtime --lib render_product_post_volume_camera_transition --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-plan07-volume-transition-0623 --message-format short --color never -- --test-threads=1 --nocapture (2026-06-23: timed out after 245s with no result; not counted as passed)
  - cargo test -p zircon_runtime --lib smaa --locked (2026-06-22: passed 11/11)
  - cargo test -p zircon_runtime --lib dynamic_resolution --locked (2026-06-22: passed 8/8)
  - cargo test -p zircon_runtime --lib render_product_post_process --locked (2026-06-22: passed 22/22)
  - cargo check -p zircon_runtime --lib --locked (2026-06-22: passed with existing warnings)
  - cargo test -p zircon_runtime --lib enabled_effect_stack_declares_tonemapped_for_uber_descriptor --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-hzb-ssao-product-check --message-format short --color never -- --test-threads=1 --nocapture (2026-06-22: passed 1/1)
  - cargo test -p zircon_runtime --lib rendering_plugin_post_process_routes_output_transfer_through_terminal_anti_alias_input --no-default-features --features core-min --locked --jobs 1 --target-dir target\codex-hzb-ssao-product-check --message-format short --color never -- --test-threads=1 --nocapture (2026-06-22: passed 1/1)
  - `ssao_product_default_is_fail_closed_and_preserves_scene_output` source regression added on 2026-08-30; managed Cargo/WGPU execution is pending and the superseded 2026-06-22 luma-darkening result is not accepted as current AO correctness evidence.
  - cargo test -q -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-wgpu-exposure-0615 exposure_ -- --nocapture
  - cargo test -q -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-wgpu-exposure-0615 runtime_diagnostics -- --nocapture
  - cargo fmt --package zircon_runtime -- --check
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-chain-0615 --message-format short --color never
  - cargo test -p zircon_runtime --lib render_post_chain --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-chain-0615 --message-format short --color never -- --test-threads=1 --nocapture (timed out after 604s while compiling the shared lib-test target)
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-color-s2-0615 --message-format short --color never
  - cargo test -p zircon_runtime --lib render_post_color_space --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-color-s2-0615 --message-format short --color never -- --test-threads=1 --nocapture (timed out after 904s while compiling the shared lib-test target)
  - cargo test -p zircon_runtime --lib output_transfer --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-color-s2-0615 --message-format short --color never -- --test-threads=1 --nocapture (timed out after 304s while compiling the shared lib-test target)
  - cargo fmt --package zircon_runtime
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-color-s2-0615 --message-format short --color never (after PP-M1-S3)
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-post-color-s2-0615 --message-format short --color never (after PP-M2-S1)
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-temporal-s2-0614 --message-format short --color never
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-temporal-m3-s1-0614 --message-format short --color never
  - cargo test -p zircon_runtime --lib vm_lifecycle_fallback --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-06-fallback-lifecycle-0616 -- --nocapture --test-threads=1 (2026-06-16: passed 5/5 after restoring compile.rs test-module imports for post-process pipeline descriptors)
  - cargo check -p zircon_editor --lib --locked --jobs 1 --message-format short --color never (2026-06-17 editor UI validation rerun: passed after restoring the `SMAA_STAGE_FORMAT` re-export from post-process constants)
doc_type: module-overview
---

# Scene Renderer Post-Process

This directory documents post-process renderer behavior that is too specific for the broad render-product submit document.

- `projection.md` owns camera projection, SSR reconstruction, reflection-probe projection, and temporal jitter audit rules for post-process screen-space consumers.
- `temporal/taa.md` owns the TAA resolve contract, WGPU fullscreen resolve pass, quality/responsive uniforms, shader, `TemporalHistoryStore` history behavior, TAA reactive-mask clear/mesh writer ordering, material-authored reactive strength packing, and opaque/alpha-mask material flag writes.
- TP-M3 adds `TAA_OUTPUT` as an optional post-process graph input. The pipeline compiler strips it from default no-stack compatibility compiles and retains it only when an effective stack enables `PostProcessEffectKind::TaaResolve`, so final composite consumes the resolved TAA color without making TAA resources live in Off/Fxaa paths.
- TP-M4-S1 removes the legacy scene-color history resolve path from post-process ownership. TAA scene-color history now flows only through `TemporalHistoryStore` and `temporal.taa-resolve`; SSR keeps its dedicated temporal history texture.
- TP-M4-S2 adds `TAA_REACTIVE_MASK = "taa.reactive-mask"` as a TAA-only graph texture. `temporal.taa-reactive-mask-clear` clear-stores the `R8Unorm` default-zero mask, and the compiler culls the mask resource when the effective stack is not using TAA.
- TP-M4-S3 inserts `temporal.taa-reactive-mask-mesh` between the clear pass and `temporal.taa-resolve`. The mesh writer replays visible transparent mesh commands, reads scene depth, load/stores the mask, and writes sampled base alpha through `fs_taa_reactive_mask`.
- TP-M4-S3b adds `StandardMaterialDescriptor::taa_reactive_mask_strength` as a material-owned override and packs it into `MaterialPropertyUniform.data8.x`; `fs_taa_reactive_mask` writes the maximum of sampled alpha and explicit strength.
- TP-M4-S3c adds `MeshPassPipelineKind::TaaReactiveMaterialMask` and `fs_taa_reactive_material_mask`; opaque/alpha-mask batches only emit material-mask commands when their CPU-side material strength is nonzero, while transparent batches still use the sampled-alpha writer.
- TP-M4-S4a tightens the WGPU resource envelope needed by the full post-process layout: device request limits now reserve 21 sampled textures per fragment stage, a layout test counts the same `zircon-post-process-bind-group-layout` entries passed to WGPU, and transient `R8Unorm`/`R16Float`/`Rg16Float` textures no longer request unsupported storage binding usage.
- PP-M1-S1 adds `PostProcessChainSlot` as the fixed Plan 07 backbone from `taa-resolve` through `output-transfer`. Current legacy `PostProcessEffectKind` nodes now carry `chain_slot` and `planned_chain_executor_id`, and validated graphs expose both `planned_backbone_slots` and `active_chain_slots` so later executor hard cuts can compare graph order against the target post-process chain.
- PP-M1-S2a adds the framework-level color-space contract: `INTERMEDIATE_HDR_FORMAT_DEFAULT` is `rg11b10ufloat`, WGPU devices request `RG11B10UFLOAT_RENDERABLE`, RHI/graph execution can materialize `TextureFormat::Rg11b10Ufloat`, and HDR `scene-color`, `TAA_OUTPUT`, plus SSR reflection pyramids now use the shared post-process HDR format while HZB, TAA history, and motion-vector resources remain `rgba16float`.
- PP-M1-S2b replaces the legacy `FinalComposite` contract node with `OutputTransfer` and introduces `postprocess.tonemapped` as the explicit handoff from the tonemap/uber stage to `post.output-transfer`. The WGPU side now owns `output_transfer.wgsl`, a dedicated bind group layout/pipeline, `execute_output_transfer`, and a registered `post.output-transfer` graph executor that writes `FINAL_COLOR`.
- 2026-06-22 tightened this handoff for first-party plugin descriptors that reuse built-in post-process executor ids. `descriptor_filtering.rs` now routes plugin-authored `post.uber`, `post.output-transfer`, terminal AA, upscale, and related built-in executor ids through the same stack filtering as built-ins, so terminal AA rewrites output-transfer to `FINAL_COMPOSITED` before FXAA/SMAA writes `FINAL_COLOR`. Effect-stack outputs always declare `TONEMAPPED`. The former `AMBIENT_OCCLUSION` read on `post.uber` was removed by the 2026-08-30 Runtime27 M0 containment because multiplying AO into the complete lit scene color darkened direct/specular/emissive/sky; AO remains disabled by default until it has qualified geometry inputs and an indirect-light composition owner.
- PP-M1-S3 clears the old runtime pass/executor names for color grading and effect-stack routing. The graph now uses `color-lut-bake`/`post.color-lut-bake` and `uber`/`post.uber`, while data-schema names such as `RenderColorGradingSettings` and `RenderPostProcessEffectStackSettings` remain until the PP-M2/PP-M3 schema migration.
- 2026-06-23 Plan 07 post-process stack owner split (`render_plan07_post_process_stack_owner_split_static_passed`) moves `PostProcessGraphResourceNames` into `core/framework/render/post_process/graph_resource_names.rs` and moves the 17 stack graph contract tests into `core/framework/render/post_process/stack/tests/{exposure,terminal_chain,screen_space_reflection,temporal_history,effect_stack}.rs`. `stack.rs` is now a 586-line descriptor owner that only mounts `mod tests;`; `runtime_15_post_process_stack_is_folder_backed` locks the resource-name owner, test owner layout, moved-test count, docs/status anchors, and line budgets. This is a structure-only owner split; no new Cargo/WGPU/RenderDoc pass is claimed here.
- 2026-06-23 Plan 07 volume component owner split (`render_plan07_volume_component_owner_split_static_passed`) moves `VolumeParamValue`, `VolumeParamSchema`, `VolumeParamInterpFn`, `interp_*`, and parameter default factories into `core/framework/render/post_process/volume_component/params.rs`, then moves the 5 volume component behavior tests into `core/framework/render/post_process/volume_component/tests.rs`. `volume_component.rs` is now a 642-line descriptor/registry-map owner that only mounts `mod params;` and `mod tests;`; `runtime_15_post_process_volume_component_is_folder_backed` locks params/tests owners, moved-test count, docs/status anchors, and line budgets. This is a structure-only owner split; scoped rustfmt/static/docs/diff checks passed, focused locked `render_volume_component` Cargo timed out after 184s with no result and no matching target residual, and no new WGPU/RenderDoc pass is claimed here.
- 2026-06-24 Render product post-process motion-blur test owner split (`render_plan07_product_post_process_motion_blur_test_owner_split_static_passed_cargo_deferred_active_compile_lane`) keeps `graphics/tests/render_product_post_process.rs` as the compact post-process product-test parent and moves the velocity-driven motion-blur product guard, particle velocity fixture, motion-blur framework setup, local executor-order assertions, and scene-velocity readback helper into `graphics/tests/render_product_post_process/motion_blur.rs`. `runtime_15_render_product_post_process_motion_blur_tests_are_child_owner` locks the moved-test boundary, docs/status anchors, and parent/child line budgets. This is a structure-only owner split; scoped rustfmt/static/docs/diff checks passed, Cargo/WGPU/RenderDoc remain deferred behind active compile lanes.
- 2026-06-23 Plan 07 volume camera transition product guard (`render_plan07_volume_camera_transition_product_guard_static_passed_cargo_timeout_no_result`) adds `graphics/tests/render_product_post_process_volume.rs::render_product_post_volume_camera_transition`. The guard submits outside/blend/inside camera positions through real headless WGPU, drives a sphere `PostProcessVolumeExtract` into `post.vignette`, and checks `post.uber`, `post.output-transfer`, corner luma falloff, and final-frame deltas. It is isolated from the 913-line `render_product_post_process.rs`; scoped rustfmt/static/line-count checks passed, focused locked Cargo timed out after 245s with no result, and no new RenderDoc pass is claimed here.
- PP-M2-S1 adds the contract-only volume component schema and registry. `VolumeParamValue`, `VolumeParamSchema`, and `VolumeComponentDescriptor` define parameter defaults plus interpolation/apply hooks, while `VolumeComponentRegistry::with_builtin_post_process_components()` exposes the planned built-in component ids from Plan 07 for later volume extract/evaluator wiring.
- PP-M2-S2 adds `PostProcessVolumeExtract` as the planned per-frame volume DTO. Scene extraction now emits global/box/sphere shape snapshots plus component override vectors, with local volume projection isolated in `scene/world/render_post_process.rs` so the main render extract module stays below the large-file threshold.
- PP-M2-S3 adds `VolumeEvaluator` and switches WGPU frame submission to `PostProcessExtract::resolved_settings_for_camera(...)`. Evaluator uses registered component descriptors to read current strong-typed values, apply per-param overrides, compute global/box/sphere weights, and write a `ResolvedPostProcessStack`; the former `volume_stack`/`volume.rs` fallback path has been hard-deleted.
- PP-M3-S1b adds the WGPU exposure path. Histogram mode declares `exposure-histogram` plus `exposure-resolve`, manual mode declares only resolve, and both modes feed the final post-process shader through the same binding 28 resolved exposure buffer. `SceneFrameHistoryTextures` owns the previous/current exposure buffer pair and runtime diagnostics report `render.history.copy.exposure_copied`.
- PP-M3-S2 adds `postprocess.color-lut` as a real graph texture and wires the WGPU compute bake. The compiler describes it as a 32x32x32 `Rgba16Float` 3D transient with sampled/storage/copy usage. `post.color-lut-bake` is an async compute workload with fixed `[4, 4, 4]` workgroups and `[8, 8, 8]` dispatch groups, reads `history.current.exposure`, samples optional user LUT textures, writes the storage 3D LUT, and records a compute dispatch before `post.uber`. Dispatch metadata now keeps both the legacy storage-write list and explicit resource accesses, so diagnostics can distinguish the exposure buffer read from the LUT texture write. Test builds also attach `RenderColorLutReadbackReport` to `RenderStats.last_color_lut_readback_report` by reading the graph-owned 3D `Rgba16Float` LUT before transient release, stripping per-row copy padding, decoding half floats, and measuring neutral identity error against the fixed 1/1024 tolerance. Runtime diagnostics mirror that report under `render.post_process.color_lut.readback.*`, including availability, dimensions, byte counts, invalid sample counts, maximum identity error, and out-of-tolerance counts. `post.uber` binds the baked LUT as an internal mode and skips duplicate tonemap/grading when that mode is active.
- PP-M3-S2 user-LUT readback now tags reports by reference source. `RenderColorLutReadbackReport` distinguishes `Identity` from `UserLut`, records both reference-space and identity-space error, and diagnostics expose `render.post_process.color_lut.readback.reference_*` fields. In test builds the scene renderer can derive a CPU reference from the same prepared RGBA8 user LUT resource used by the color-lut bake path for 1D 2D, 2D strip, and 3D layouts, then compare the baked 3D LUT against the user LUT blend result. The user-LUT reference path remains neutral-stack scoped; non-neutral tonemap/color-grading validation now uses the separate `ColorTransform` CPU reference below.
- 2026-06-21 PP-M3-S2 adds a product-level WGPU guard for non-neutral tonemap/color-grading bake results. `render_product_post_non_neutral_tonemap_grading_changes_final_frame` submits matching baseline and ACES/exposure/tinted color-grading extracts, verifies `post.color-lut-bake`, `post.uber`, and `post.output-transfer` executor execution, checks that the 32^3 readback LUT is valid and not identity, then compares final captured frame luma/RGB deltas. This proves non-neutral settings reach the baked GPU LUT and final product output; the later same-day CPU-reference/readback slice below extends it into measured non-neutral LUT error coverage.
- 2026-06-21 PP-M3-S2 closes the non-neutral CPU-reference/readback gap. `RenderExposureReadbackReport` decodes the live `history.current.exposure` buffer into multiplier, EV100, and validity fields, `read_buffer_f32x4` provides the test-only WGPU buffer readback helper, and diagnostics mirror exposure readback under `render.post_process.exposure.readback.*`. `RenderColorLutReadbackReference::ColorTransform` mirrors `color_lut_bake.wgsl` tonemap and color-grading math on CPU, so the non-neutral product guard now verifies exposure multiplier/default EV100 plus `color_transform_within_epsilon()` against the baked 32^3 `Rgba16Float` LUT. The red run also aligned `SceneFrameHistoryTextures` exposure history buffers with the graph descriptor by adding `COPY_SRC` to the real persistent WGPU buffers.
- 2026-06-21 PP-M3-S2 adds a product-level WGPU guard for real user LUT textures. `render_product_post_user_lut_texture_changes_final_frame_and_matches_readback_reference` registers a 1024x32 linear RGBA8 2D-strip LUT through `ProjectAssetManager`, verifies ResourceStreamer reports the LUT as ready without fallback, checks that `RenderColorLutReadbackReport` uses the `UserLut` reference and passes `user_lut_within_epsilon()`, then compares final captured frame RGB deltas so the same prepared asset is proven to affect product output.
- 2026-06-21 graph-contract focused validation refreshed older framework tests to the current Plan 07 chain. Disabled/default post-process stacks now expect `ExposureResolve` plus `OutputTransfer`, history-only stacks assert TAA remains absent, and fog/SSR stacks expect `SceneComposite` to own depth/SSR/fog composition before `post.uber` reads `SCENE_COMPOSITED` and `COLOR_LUT`. The `render_product_post_` Cargo filter passed 15 tests, covering those framework contracts and three WGPU product guards.
- PP-M3-S3 splits depth of field and motion blur out of the uber post-process pass. The stack now declares `postprocess.depth-of-fielded` and `postprocess.motion-blurred`, compiles `post.depth-of-field` before `post.motion-blur`, and routes the later stack/uber source through the most recent split output. The WGPU side owns `fs_depth_of_field`/`fs_motion_blur`, dedicated pipeline builders, `execute_depth_of_field`, `execute_motion_blur`, and registered graph executors. These split shader entry points do not sample the resolved exposure buffer, so their feature descriptors intentionally do not read `EXPOSURE_CURRENT`; scene-composite, color-LUT bake, and uber keep the real exposure reads. When those graph nodes exist, `record_post_process_stack` asks `post.uber` to skip the corresponding internal DoF/motion blur responsibilities so the split passes own the effect.
- 2026-06-21 PP-M3-S3 adds a product-level WGPU guard for split motion blur. `render_product_post_motion_blur_split_uses_velocity_and_changes_final_frame` submits baseline and motion-blur product frames, verifies temporal object velocity, particle velocity, the motion-vector tile/coarse/neighbor-max chain, `post.motion-blur`, `post.uber`, and `post.output-transfer` ordering, checks that the scene velocity texture is nonzero, then compares final captured frames. The red run exposed that split output pipelines were still built for the final target format; `create_pipeline_bundle` now builds depth-of-field, motion-blur, blur, and scene-composite output pipelines with `POST_PROCESS_INTERMEDIATE_HDR_FORMAT`. `effect_stack_resource_status` also scans every post-process graph node, so split motion blur's `MOTION_VECTOR_NEIGHBOR_MAX` dependency is no longer falsely reported as missing.
- PP-M3-S3 follow-up routes bloom extraction through the latest post-TAA scene color. The compiler rewrites the `post.bloom-extract` read from `scene-color` to `postprocess.motion-blurred`, `postprocess.depth-of-fielded`, or `taa.output.scene-color` when those upstream split passes are active. The executor mirrors the same resource priority from graph metadata, so runtime bloom no longer hard-codes the original scene color after DoF or motion blur have produced newer scene-color intermediates.
- 2026-07-04 Frameworks 02 M3 real-backend capture validation tightened the Bloom output format contract. `bloom-texture` is now an explicit intermediate HDR render-graph transient, and `create_pipeline_bundle` builds the Bloom fullscreen pipeline with `POST_PROCESS_INTERMEDIATE_HDR_FORMAT` instead of the final swapchain/output format. This keeps Bloom extraction aligned with split HDR intermediates even for non-HDR cameras and prevents WGPU render-pass/pipeline target mismatches during dynamic runtime-library capture.
- PP-M3-S3 scene-composite split moves SSR/fog composition into `post.scene-composite`. The stack declares `postprocess.scene-composited` after the latest scene color, bloom/exposure, and optional SSR resolve; `scene_composite_pipeline.rs` builds the `fs_scene_composite` fullscreen pipeline, `execute_scene_composite` records the pass, and the compiler routes `post.uber` to read `SCENE_COMPOSITED` while removing its redundant SSR-history dependency. When this split is active, `post.uber` receives disabled SSR/fog parameters so those responsibilities are not applied twice.
- 2026-06-21 PP-M3-S3 adds a product-level WGPU guard for split scene-composite fog. `render_product_post_scene_composite_fog_changes_final_frame` submits baseline and fog product frames, verifies `post.scene-composite`, `post.uber`, and `post.output-transfer` ordering, checks the `fog` active family plus distinct `postprocess.scene-composited`/`postprocess.tonemapped` aliases, then compares final captured frame luma/RGB deltas. The red run exposed that `product_postprocess_executor(...)` still validated all required/produced graph resources as texture views; it now dispatches validation by graph resource kind or external binding type, so `history.current.exposure` and future post-process buffers use `require_buffer_by_name` while texture resources still use texture-view validation. The warmed broad `render_product_post_` filter now passes 19 tests with the scene-composite guard included.
- 2026-06-21 PP-M3-S3 adds a wider product-level WGPU guard for split scene-composite SSR. `render_product_post_scene_composite_ssr_changes_final_frame` submits baseline and SSR product frames with real materials, geometry, lights, and temporal history, verifies the SSR reflection-pyramid/coarse/specular-occlusion/resolve chain before `post.scene-composite`, then checks `post.uber` and `post.output-transfer` ordering plus `screen-space-reflection` active-family diagnostics, SSR backing resources, and final-frame RGB deltas. The red run exposed that SSR reflection-pyramid passes share the full post-process bind group and therefore must declare `SCENE_DEPTH` even though their shader body downsamples color; `feature_descriptors/post_process.rs` now binds a real depth view for those passes. `execute_post_process_pass_graph(...)` also records post-process node execution from real `executed_executor_ids` before using the legacy resource simulation fallback, so SSR split execution no longer under-reports `scene-composite`/`uber`/`output-transfer` in stats.
- PP-M3-S3 blur split moves the general blur radius out of `post.uber` into `post.blur`. The stack declares `postprocess.blurred` after scene-composite or the latest available scene color, `blur_pipeline.rs` builds the `fs_blur` fullscreen pipeline, `execute_blur` records the pass with DoF parameters disabled, and the compiler routes `post.uber` to read `BLURRED` while asking the uber path to skip its duplicate general-blur work. `post.blur` also does not declare `EXPOSURE_CURRENT`; exposure stays owned by the later composite/LUT/uber stages that need it.
- 2026-06-21 PP-M3-S3 adds a product-level WGPU guard for split blur. `render_product_post_blur_split_changes_final_frame` submits baseline and blur product frames, verifies `post.blur`, `post.uber`, and `post.output-transfer` ordering, checks the `blur` active family plus distinct `postprocess.blurred`/`postprocess.tonemapped` aliases, then compares final captured frames. The red run exposed that split passes and uber reused one `post_process_params_buffer`; because all queue writes happen before a command submission executes, later uber writes could overwrite the parameters already bound by `post.blur`. The WGPU path now creates pass-local post-process params uniform buffers for blur, motion blur, DoF, scene composite, SSR, and uber recording, and the obsolete shared params buffer field was removed.
- 2026-06-21 PP-M3-S3 adds a product-level WGPU guard for split depth of field. `render_product_post_depth_of_field_split_changes_final_frame` submits baseline and DoF product frames, verifies `post.depth-of-field-prepare`, `post.depth-of-field`, `post.uber`, and `post.output-transfer` ordering, checks the `depth-of-field` active family, validates the `DEPTH_OF_FIELD_COC` and `DEPTH_OF_FIELD_BOKEH` aliases plus a distinct `postprocess.depth-of-fielded` backing, then compares final captured frames. The warmed broad `render_product_post_` filter now passes 18 tests with the DoF guard included.
- 2026-06-21 PP-M3-S3 adds a product-level WGPU guard for the light stylistic effects that still live in `post.uber`. `render_product_post_uber_light_effects_change_final_frame` submits matching baseline/effect-stack extracts, verifies `post.uber` and `post.output-transfer` executor execution plus active `vignette`, `film-grain`, `dither`, and `chromatic-aberration` families, then compares final captured frame luma/delta to prove those controls affect product output. The default manual exposure path may still execute `exposure-resolve`, so the guard validates the output-transfer graph contract and executor path rather than requiring a two-node post-process execution list.
- PP-M4-S1a reconnects the terminal FXAA graph path. `AntiAliasSettings::fxaa()` now enables the `PostProcessEffectKind::Fxaa` node, `post.output-transfer` writes `postprocess.terminal-aa-input` as a transient texture, and the `fxaa` pass reads that terminal input before writing external `final-color`.
- PP-M4-S1b moves terminal FXAA from the fallback transfer executor to a dedicated WGPU pass. `fxaa.wgsl` samples `postprocess.terminal-aa-input` and applies neighbor-luma edge filtering, `fxaa_pipeline.rs` owns the fullscreen render pipeline, `execute_fxaa` records the pass, and `record_fxaa_to_resource` calls it from `post.fxaa`. The compiler describes `postprocess.terminal-aa-input` as `Rgba8UnormSrgb`, single-sample, renderable/sampled transient; graph execution now lets that transient override the preimported final-color alias. Remaining PP-M4 work is product-scene capture, RenderDoc validation, and SMAA quality validation/upgrade.
- 2026-06-19 Plan 09 CO-M2 tightened product post-process executor resource validation after the texture-target overlay product guard exposed the mismatch. `product_postprocess_executor(...)` now validates `PostProcessPassGraph` `required_inputs` with `Read` access and `produced_outputs` with `Write` access instead of treating produced outputs as reads. This preserves the FXAA terminal contract: `fxaa` reads `postprocess.terminal-aa-input`/`FINAL_COMPOSITED` and writes `final-color`, with no false `final-color` read declaration.
- PP-M4-S1c adds anti-alias normalization diagnostics at the framework/runtime boundary. `AntiAliasFallbackReport` now carries requested/effective graph sample counts plus graph-sample and terminal-slot normalization flags. `build_frame_submission_context` passes the camera MSAA request into AA resolution, so TAA plus camera MSAA is reported as sample-count normalization even when TAA remains the effective mode. Runtime diagnostics mirror these states through `render.anti_alias.normalization.*`. Remaining PP-M4 work is product-scene capture, RenderDoc validation, and SMAA quality validation/upgrade.
- PP-M4-S1d/S1e moves SMAA from an external plugin-slot requirement to a built-in terminal AA pass and upgrades it to an internal three-stage WGPU path. `AntiAliasSettings::smaa()` resolves when capabilities expose `supports_smaa`, `PostProcessEffectKind::Smaa` maps to the same `TerminalAntiAlias` chain slot as FXAA, and `post.output-transfer` writes `postprocess.terminal-aa-input`. The built-in `post.smaa` executor now creates per-frame `Rgba8Unorm` edge and blend-weight textures, records `SmaaEdgePass`, `SmaaBlendPass`, and `SmaaResolvePass`, then writes external `final-color`. `smaa.wgsl` exposes `fs_edge`, `fs_blend`, and `fs_resolve`; `smaa_pipeline.rs` owns matching edge/blend/resolve pipelines over the dedicated two-texture SMAA bind group layout. The compiler still chooses between built-in FXAA and built-in SMAA by effective stack state, so SMAA no longer requires a plugin pass. Remaining PP-M4 work is product-scene capture, RenderDoc validation, and focused SMAA shader/graph tests.
- Historical PP-M4-S2a contract, superseded by the 2026-08-29 hard cut below: dynamic-resolution output originally used one chain-tail `PostProcessEffectKind::Upscale`, `post.upscale`, and `postprocess.upscaled` identity for every spatial transition. Its transient-pool regression established the initial 0.5/1.0 resource reuse baseline, but the identity could not represent separate primary-to-secondary and secondary-to-output passes.
- Historical 2026-06-22 validation, superseded by the 2026-08-29 hard cut below: the initial terminal-AA product guard validated the former single `post.upscale` path. That evidence does not validate the new dual-spatial or temporal-plus-secondary paths and must not be reused as their acceptance evidence.
- 2026-08-29 PP-M4-S3 current contract hard-cuts the former single identity into `PostProcessEffectKind::{PrimaryUpscale, SecondaryUpscale}`, `post.primary-upscale`/`post.secondary-upscale`, and `postprocess.primary-upscaled`/`postprocess.secondary-upscaled`. `RenderViewFamilyPipeline` is the only phase/geometry owner: spatial primary-to-secondary is optional, temporal reconstruction replaces that primary transition, and secondary-to-output is independently optional. The graph compiler routes terminal AA into the first enabled upscale, primary output into secondary input for the dual path, and the last upscale into `post.output-transfer`. Both passes share the WGPU pipeline, sampler, and layout but use distinct persistent params buffers so pre-submit uploads retain one CPU producer per target. Invalid phases and missing phase inputs return `UpscaleExecutionError` through the graph executor instead of panicking the render submit path. Source parsing, exact formatting, legacy-identity scans, and locked Cargo metadata pass; managed compile/product, RenderDoc, screenshot, frame-time, and power acceptance remain pending.
- 2026-08-29 post-process graph execution now treats a missing ViewFamily phase as a failed graph record instead of a renderer panic. OutputTransfer, FXAA, SMAA, Bloom, and the shared SceneLinear/DisplayMapping size and allocation resolvers return errors carrying pass and phase identity; 14 GPU record functions propagate those errors through their existing `Result` boundary, with the former duplicate lookup in `record_post_process_stack` reduced to one. The non-test production source under this execution owner contains no `panic!`, `expect`, or `unwrap` calls. Exact formatting, source guards, diff checks, and locked Cargo metadata pass; source-matched managed Rust and WGPU product validation remain pending after the existing Cargo ownership timeout.
- 2026-06-23 Plan 07 adds the full-chain all-effects product guard (`render_plan07_full_chain_all_effects_product_guard_static_passed_cargo_timeout_no_result`). `render_product_post_full_chain_all_effects_on` uses a real headless WGPU mesh + particle-velocity scene and enables histogram exposure, bloom, split DoF, motion blur, SSR/fog scene-composite, blur, color LUT bake/tonemap/user LUT, light uber effects, dynamic-resolution upscale, output-transfer, and SMAA terminal AA in one frame. The guard checks executor ordering, active effect families, graph aliases/backings, scene-velocity readback, and final-frame delta. Its scene/material/LUT/particle setup lives in `render_product_post_process_full_chain/fixture.rs` so the product guard stays folder-backed instead of expanding the legacy post-process test file.
- 2026-07-04 Plan 08 current-source full-chain repair advances that guard from static/timeout status to a counted WGPU product pass for the automatic `render_product_post_process` group. Plugin-authored scene-velocity consumers are now filtered when the effective stack has no scene-velocity producer; dynamic-resolution submissions use render-size internal resources while final color remains output-size; selected-camera post-process regions are scaled before history copies; `post.bloom-extract` is accepted as a Bloom executor id and ordered after scene-color split producers but before exposure/downstream stages. The full-chain assertion model now follows backend ownership: `EXPOSURE_HISTOGRAM` is a buffer, `EXPOSURE_CURRENT` is proven through the exposure history-copy report, `TONEMAPPED` is the physical `post.uber` output, and `EFFECT_STACKED` remains a logical stack output alias rather than a separate WGPU texture backing. `docs/tests/runtime/render/plan08_current_render_product_post_process_group_after_all_fixes_20260704.out.log` passed 12/12; RenderDoc capture and workspace/full CI remain separate gates.
- 2026-07-04 Plan 08 current-source full-chain repair advances that guard from static/timeout status to a counted WGPU product pass for the automatic `render_product_post_process` group. Plugin-authored scene-velocity consumers are now filtered when the effective stack has no scene-velocity producer; dynamic-resolution submissions use render-size internal resources while final color remains output-size; selected-camera post-process regions are scaled before history copies; `post.bloom-extract` is accepted as a Bloom executor id and ordered after scene-color split producers but before exposure/downstream stages. The full-chain assertion model now follows backend ownership: `EXPOSURE_HISTOGRAM` is a buffer, `EXPOSURE_CURRENT` is proven through the exposure history-copy report, `TONEMAPPED` is the physical `post.uber` output, and `EFFECT_STACKED` remains a logical stack output alias rather than a separate WGPU texture backing. `docs/tests/runtime/render/plan08_current_render_product_post_process_group_after_all_fixes_20260704.out.log` passed 12/12; RenderDoc capture and workspace/full CI remain separate gates.
- 2026-07-04 Plan 08 camera-target repair extends that post-process region contract to current-source camera-target products. `ViewportRenderRegion` now distinguishes the physical output rectangle from the graph-local render size; post-process parameter packing reports physical viewport origin and local scene-source origin separately, graph-owned post inputs use origin-zero sampling, and history copies clamp local extents before writing the selected camera's physical history region. Direct support logs `docs/tests/runtime/render/plan08_post_process_params_direct_after_camera_target_fixes_20260704.out.log` and `docs/tests/runtime/render/plan08_history_region_copy_direct_after_camera_target_fixes_20260704.out.log` passed the focused regressions; the broader 13/13 camera-target Cargo wrapper pass is recorded in the Plan 08 material/shader permutation status.
- 2026-06-19 Plan 09 CO-M2 carries the selected-camera viewport region through post-process. `PostProcessParams` now packs local viewport size, physical viewport origin, cluster dimensions, and the source origin for fixed full-frame scene textures separately. `post_process.wgsl` and `post_process_screen_space_reflection.wgsl` read graph-owned intermediates in local coordinates while adding the physical origin for fixed frame targets, history textures, G-buffer inputs, ambient occlusion, and scene color/depth/velocity sources. Split bloom, depth-of-field prepare, motion blur, scene-composite, blur, and SSR executors receive the source origin for their current scene-color input. `BLOOM` and `GLOBAL_ILLUMINATION` are graph-owned post-process textures, and the Hybrid GI history copy now copies the graph-owned GI result back into the selected camera's physical history region. Terminal output-transfer now chooses its render region by output target: it uses local origin-zero viewport/scissor and local params when writing graph-owned `FINAL_COMPOSITED` for terminal AA input, while the no-terminal-AA direct output path keeps physical target coordinates. FXAA and the final SMAA resolve always write imported/full-frame `FINAL_COLOR` with the physical region and origin; SMAA edge and blend stages remain local intermediate passes with origin zero. The validated slice used `cargo fmt --package zircon_runtime`, focused params/viewport/history tests, and `cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked` against `D:\cargo-targets\zircon-runtime-postprocess-region-tests-0619`; the later split-screen PrimarySurface guard and terminal local/physical split passed `graphics_primary_surface_split_screen_base_cameras_clear_only_their_viewport_regions` after the unrelated Runtime 05 compile blocker cleared.
- 2026-06-16 editor-focused validation repaired the dynamic-resolution upscale pipeline's shader-source import path. `upscale_pipeline.rs` now imports `UPSCALE_SHADER` from the renderer-private `resources::shader_sources` module with the same relative ownership pattern used by the other post-process pipeline builders, instead of depending on a non-existent top-level `post_process::shader_sources` export. This changes module ownership only; `upscale.wgsl` and the render pipeline descriptor remain unchanged.
- 2026-06-16 runtime fallback validation restored `render_pipeline_asset/compile.rs` test-module imports for `PostProcessStackDescriptor` and `RenderPipelineCompileOptions`. This is a test-owner compile repair only: it lets focused runtime filters compile existing post-process pipeline descriptor tests without changing render production behavior; the Runtime 06 `vm_lifecycle_fallback` Cargo filter passed 5/5 after the repair.
- 2026-06-17 editor UI validation restored the post-process constants module export for `SMAA_STAGE_FORMAT`. The format already lives in `constants/texture_formats.rs` and the SMAA executor/pipeline import it through `post_process::SMAA_STAGE_FORMAT`; `constants/mod.rs` now re-exports it with the rest of the post-process texture formats so downstream editor checks can compile through the runtime crate. No shader or pipeline descriptor behavior changed.
- 2026-07-01 Runtime 15 structure mirror records the Plan 07 post-process stack owner split for the current guard: `render_plan07_post_process_stack_owner_split_static_passed`, `core/framework/render/post_process/graph_resource_names.rs`, `core/framework/render/post_process/stack/tests/effect_stack.rs`, and `runtime_15_post_process_stack_is_folder_backed`. This documents the existing stack/graph-resource/test owner placement only; product rendering validation remains governed by the render plan gates.
