...
Frame integration now feeds the shader-visible path for built-in mesh shaders. `build_mesh_draws` expands real mesh draws first, then registers each pending draw with a stable key derived from source entity plus draw ordinal. `PendingMeshDraw` carries the transform revision from extract, while GPUScene writes primitive data from the same tint, shadow, motion, and transform inputs used by the existing mesh path and one instance record. Unchanged primitive/instance payloads are skipped at the CPU shadow comparison point, so the second submission of the same static scene returns a zero-byte upload report. The builder retains only live frame keys and flushes dirty GPUScene ranges before command recording. It records the resulting `GpuSceneEntry` span on the production `MeshDraw`, and every `MeshBatchRef` now requires that span before it can emit a command. For temporal velocity migration, pending draw sync keeps the old CPU `ViewportMotionVectorObjectHistory` previous transform as the first source and falls back to GPUScene's rolled previous transform only when that CPU history misses. The effective previous state is written into primitive flags, motion params, and `MeshDraw::has_previous_motion_vector_transform`, so unskinned dynamic objects can move off the old CPU object-history path without changing the velocity processor contract. `DrawInstanceSource` only carries GPUScene instance ranges, so built-in draws always use first-instance draw args plus `@builtin(instance_index)`.

`GpuScene` owns a scene-data bind group layout and a shared frame bind group. Bindings 0, 1, and 2 expose primitive, instance, and light buffers as read-only storage to vertex, fragment, and compute stages. Bindings 3 and 4 expose current and previous skinned joint palette uniforms. Binding 6 is the visible-instance remap params uniform; its `values.x` flag selects direct or remapped instance indices, and `values.y` carries the active light count so shaders can ignore spare light-buffer capacity. `flush_updates()` rewrites both direct and remapped params buffers when the light count changes, so those uniform buffers are allocated with `COPY_DST` in addition to `UNIFORM`. The shared bind group fills both palette slots with the renderer's empty skinned-palette fallback buffer, while `create_scene_bind_group_for_palettes(...)` creates command-local bind groups that reuse the same storage buffers and override only the palette buffers for skinned draws. If primitive, instance, or light buffers grow, GPUScene recreates the shared bind group after replacing the WGPU buffer so later shader consumption cannot keep a stale buffer handle.
...
The shader bridge performs the GS-M2 transform/palette reads and the LS-M1 light-buffer reads for built-in shaders. `zr_gpu_scene.wgsl` defines `ZrGpuPrimitiveData`, `ZrGpuInstanceData`, `ZrGpuLightData`, group3 storage bindings 0-2, group3 palette bindings 3-4, and helper functions such as `zr_world_from_local`, `zr_previous_world_from_local`, primitive tint/shadow/motion accessors, `zr_gpu_scene_light_count`, `zr_gpu_light`, `zr_gpu_light_type`, `zr_skinned_joint_matrix`, and `zr_previous_skinned_joint_matrix`. The forward fallback, normal prepass, shadow map, deferred geometry, and deferred lighting shader source modules prepend that include where they need GPUScene data. Forward fallback and deferred lighting now shade from group3 light data instead of scene-uniform light fields. `ModelUniform`/`model_data` declarations and pass-local `SkinnedJointPaletteUniform`/`@group(1)` palette declarations are no longer present in the mesh WGSL sources. Rust-side `ModelUniform`, `ModelUniformCache`, model buffers, model bind groups, and the old mesh compatibility bind slot have been removed from `scene_renderer`. The material set now combines material textures/samplers and `material_properties` at group2 binding10, so built-in and custom material shader ABI checks share the same group2/material plus group3/GPUScene contract.
...
7. Successful compiled-scene and legacy scene submissions call `roll_prev_transforms_after_success()` after `queue.submit(...)`, preparing GPUScene previous transforms for the next frame without mutating the previous values visible to the current frame's velocity pass.
...
On 2026-06-14, the Plan 06 TP-M1 GPUScene previous-transform roll slice added source tests for `prev_transform.rs`, wired both successful render submission paths to roll after `queue.submit(...)`, and changed mesh draw GPUScene sync so CPU motion-vector history remains the first previous-transform source while the rolled GPUScene previous transform is the fallback. Scoped `rustfmt --edition 2021 --check`, the 8-file source-contract scan, scoped tracked `git diff --check`, and an untracked/ignored-file trailing-whitespace scan passed for the new roll API, the submit call sites, docs, and the mesh fallback flow; `git diff --check` reported only LF/CRLF warnings. Locked Cargo validation with `cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-temporal-prev-roll-0614 --message-format short --color never` is still blocked before compilation by the same root `Cargo.lock` refresh requirement recorded in the render session; no lockfile mutation is claimed for this slice.
...
This module implements the first data-plane slices from `docs/plans/zircon_runtime/render/03-gpu-scene-gpu-driven.md`, the GS-M2 built-in shader consumption/final slot step, the GS-M3 CPU-shadow diff upload plus explicit V1 direct-write upload policy, the GS-M4 CPU indirect batch planning/telemetry plus WGPU multi-draw replay slice, and the Plan 06 TP-M1 previous-transform roll foundation for object velocity. It still prepares for later staging/graph upload refinement, real-adapter multi-draw validation, and the full temporal hard cut that removes the CPU object-history path.

## Open Issues

Staging-ring upload, render-graph upload node integration, GPU-generated draw counts, and real-adapter render-product validation remain later slices. The current frame-path integration reaches mesh commands, binds group3 GPUScene scene data, uses command-local group3 overrides for real skinned palettes, uses explicit direct queue writes for V1 uploads, produces capability-gated CPU indirect batch telemetry, builds phase-local indirect args buffers, and replays eligible batches through `multi_draw_indexed_indirect`. Plan 06 still needs the temporal feature descriptor, velocity-object/camera executor migration, previous skinned-palette rolling, TAA history store, and removal of `ViewportMotionVectorObjectHistory`. The next runtime validation should create the affected WGPU pipelines on the real adapter and confirm the final group1 shadow receiver, group2 material set, group3 GPUScene layout, instance-index/palette helper shader consumption, indirect execution path, and GPUScene previous-transform velocity handoff.
