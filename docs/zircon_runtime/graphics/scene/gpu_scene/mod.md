...
7. Successful compiled-scene and legacy scene submissions call `roll_prev_transforms_after_success()`, `roll_prev_skinned_palettes_after_success()`, and `roll_prev_skinned_gpu_sources_after_success()` after `queue.submit(...)`, preparing GPUScene previous transforms, previous skinned palettes, and previous CPU-morphed source meshes for the next frame without mutating the previous values visible to the current frame's velocity pass.
...
This module implements the first data-plane slices from `docs/plans/zircon_runtime/render/03-gpu-scene-gpu-driven.md`, the GS-M2 built-in shader consumption/final slot step, the GS-M3 CPU-shadow diff upload plus explicit V1 direct-write upload policy, the GS-M4 CPU indirect batch planning/telemetry plus WGPU multi-draw replay slice, and the Plan 06 TP-M1 previous-transform, previous skinned-palette, and previous CPU-morphed source roll foundation for object velocity. It still prepares for later staging/graph upload refinement, real-adapter multi-draw validation, particle velocity capture review, jitter, TAA, and history convergence.

## Open Issues

Staging-ring upload, render-graph upload node integration, GPU-generated draw counts, and real-adapter render-product validation remain later slices. The current frame-path integration reaches mesh commands, binds group3 GPUScene scene data, uses command-local group3 overrides for real current/previous skinned palettes, uses explicit direct queue writes for V1 uploads, produces capability-gated CPU indirect batch telemetry, builds phase-local indirect args buffers, replays eligible batches through `multi_draw_indexed_indirect`, and owns the unskinned object previous-transform plus guarded skinned previous-palette/source state for temporal velocity. Plan 06 still needs RenderDoc acceptance for the temporal chain plus broader particle/TAA visual review. The next runtime validation should create the affected WGPU pipelines on the real adapter and confirm the final group1 shadow receiver, group2 material set, group3 GPUScene layout, instance-index/palette helper shader consumption, indirect execution path, and GPUScene previous-transform/previous-palette/previous-source velocity handoff.
