...
Horizontal layout keeps `cosmic.rs` as the `FontSystem` and fallback orchestration owner, while `horizontal/direct.rs` shapes the authoritative spans directly into `ShapedGlyphRun`; cosmic-text is only the horizontal fallback when direct shaping cannot produce a run. `fallback_text_spans.rs` resolves every grapheme through the authoritative Zircon `FontDatabase` and carries the selected logical family together with `FontFaceId` and `InstancedFaceId`; adjacent spans merge only when all three identities match. This prevents multiple logical variable members backed by one physical family from collapsing to glyphon's default face/instance.

The folder-backed `text/shaping/horizontal/` leaf groups adjacent source segments by actual face, instance, direction, and resolved ISO15924 script, then shapes each segment directly. Each strong script is explicitly set on the RustyBuzz buffer before `guess_segment_properties`; Common, Inherited, and Unknown tags deliberately retain the backend guess path. RustyBuzz therefore receives one script together with language, OpenType features, kerning policy, size, and effective coordinates, so `locl` and variable axes share one authoritative face/cluster projection without allowing an adjacent Latin cluster to suppress Cyrillic localization. It projects real glyph IDs, cluster ranges, advances, and offsets into the shared `ShapedGlyphRun`. Vertical shaping applies the same coordinates, script, and language directly in the TTB/BTT backend.
