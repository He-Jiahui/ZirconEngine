...
The 2026-08-29 FontObject recovery correction is current-source static work and is not covered by the historical July product frames above. A focused source regression now binds an unshaped CJK scalar to the second face of the requested asset CompositeFont, but Cargo execution, real SDF raster pixels, WGPU capture, profile, RSS, and power evidence remain pending.

SDF scalar/stale-handle recovery uses the same candidate provenance as canonical shaping. A requested FontObject typeface remains owner-local when that owner lacks a same-name face; only CompositeFont, asset fallback, or base/platform fallback declarations may continue into the global database. Valid shaped handles still bypass recovery and retain their exact generation-checked face, instance, and glyph identity.

When every authored fallback misses, recovery selects the packaged generation-owned runtime last-resort face and reads that face through `standalone_face_bytes`; it does not reopen or reinterpret the original custom asset. The current source regression requires the packaged face's glyph 0 to produce a non-empty SDF outline. That regression and real WGPU pixels remain pending, and this interim packaged notdef policy does not replace a future dedicated all-codepoint LastResort font.
