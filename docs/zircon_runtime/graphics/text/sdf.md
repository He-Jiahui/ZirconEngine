...
The stricter high-contrast A-apex occupancy assertion passed in coordinator rerun `174af981a240444c8474eb7f03594769`. The earlier runtime-plugin report API-drift compile seam no longer blocks the current-source monolithic library test. The dynamic atlas exact regressions now pass: font bake/offline/fallback 12/12, mixed-format upload 11/11, shader/decode-mode contracts 7/7, and atlas/policy identity 3/3. This closes the previously deferred SM2-M2/M3 exact gate without modifying foreign plugin tests.
