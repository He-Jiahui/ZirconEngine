...
For every external provider definition, `complete_project_feature_selections(...)` is the single owner that also materializes a disabled `ProjectPluginSelection` for that provider package. The provider selection uses the same feature manifest projection for target modes, packaging, runtime crate, and editor crate. The former parallel `project_manifest::feature_provider_selections` projection has been deleted; no earlier path can hard-code `LibraryEmbed` or shadow the declared feature packaging.
...
The runtime status path continues to reject an enabled feature when its owner, declared dependency, or external provider package is disabled. Definition lookup resolves only the exact `feature_id@provider_package_id` key. A selection without a provider identity uses its owner identity and cannot fall back to a catalog-wide unique feature id; no alias or editor-only exception bypasses that rule.
...
- Native-aware Editor catalogs merge both ordinary runtime package registrations and native feature registrations, so `FeatureExtension` packages participate in the same exact provider projection instead of disappearing at discovery.
- Editor feature dependency enablement resolves the full feature manifest through `RuntimePluginCatalog::feature_manifest_for_selection(...)` using the completed selection's provider identity. It does not reconstruct feature truth from ordinary `package_manifests().optional_features`, which excludes standalone feature-registration packages.
- The canonical lookup projects an external provider identity stored on the feature definition back onto the returned feature manifest when a native runtime registration carries that identity separately. Dependency traversal therefore consumes the same provider selected by the exact definition key instead of silently treating the owner as provider.

## Test Coverage

The runtime catalog projection test asserts that an external feature provider appears as a disabled package selection with the declared runtime/editor crates and `NativeDynamic` packaging. The lookup unit regression rejects unique-id fallback when provider identity is absent. Editor regressions cover builtin dependency enablement and a discovered native `FeatureExtension` provider.

Current-source validation rebuilt the Editor library test target in 16m49s. The native external `FeatureExtension` exact passed 1/1 in 0.13s, and the builtin external-provider exact passed 1/1 in 0.06s on the same binary. The full Editor M1 library gate remains independently open on the failures archived to their functional plans.
