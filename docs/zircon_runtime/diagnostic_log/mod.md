...
Caller threads do not create timestamps, render complete lines, lock the file, perform I/O, or flush. They enqueue an owned record into the bounded FIFO. When the queue is full, records below `warn` (`verbose`, `debug`, and `log`) are dropped immediately with per-level metrics. `warn` and `error` wait only for `critical_enqueue_timeout` (2ms by default); an expired wait returns a failed admission and increments both critical-backpressure and per-level drop metrics instead of blocking a frame, UI, or job thread without a deadline. The single worker replaces embedded newlines with `\n`, timestamps and formats records in FIFO order, and batches writes by record count, estimated bytes, or elapsed time.
