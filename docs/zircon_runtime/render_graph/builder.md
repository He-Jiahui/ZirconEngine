...
tests:
  ...
  - zircon_runtime/src/graphics/scene/scene_renderer/graph_execution/render_graph_execution_resources/tests.rs::resource_registry_validates_declaration_kind_before_name_lookup
...
When several imported views address one physical external allocation, callers use `import_external_resource_with_usage_binding_and_alias_group(...)` with the same alias group. The graph keeps the views as distinct materialization bindings, while compilation gives their reads and writes one conservative allocation-level dependency history. All views in a group must declare the same external resource type; texture and buffer views cannot share a group. This MVP contract intentionally serializes all views of one allocation, including distinct mips. It is a correctness bridge for aliased WGPU views, not an assertion that subresources may overlap or execute in parallel.
...
The SRP compiler preserves that split when it lowers `RenderFeaturePassDescriptor` resources into the graph. Attachment writes still default to clear/store for first transient writes and load/store for later or external writes. Storage writes reject attachment ops and read resources reject storage write mode. `RenderResourceSchema` owns physical allocation independently of diagnostic resource names. `RenderTextureExtentPolicy::Relative` selects the render or view reference extent, an integer numerator/denominator, and explicit floor/ceil rounding; zero scale terms and results beyond the `u32` texture extent are rejected. Half-resolution products use `1/2 + Ceil`, so odd SceneLinear dimensions retain the final row and column.

The built-in `ao.ssao-evaluate` pass writes transient `ambient-occlusion.raw` through a storage texture access. Full-resolution AO keeps the two-compute chain: `ao.ssao-spatial-denoise` reads the exact evaluate version and writes persistent external `ambient-occlusion`. A compiled half-resolution profile instead applies relative half extents to evaluate/raw and spatial/intermediate products, then inserts `ssao-bilateral-upsample`; that pass reads the exact `ambient-occlusion.spatial` version plus full depth/normal and writes the full-resolution persistent AO output. All AO stages retain async-compute declarations and graphics-queue fallback evidence without assigning attachment load/store semantics to storage outputs. When a qualified AO profile is present, pipeline compilation adds a read of the final AO version to exactly one `deferred-lighting` pass; no AO profile leaves the default graph without that read.
...
`RenderGraphComputeWorkload` also carries a backend-neutral pipeline fallback policy. The default is `Reject`; last-good reuse requires an explicit logical family and non-zero caller-owned interface generation. The renderer keeps exact candidate artifacts separate from bounded family publication. A candidate is published only after Naga reflection and WGPU validation succeed, and reuse additionally requires the same entry point, expected workgroup, complete binding layout, scene layout, and materialized device id/generation. Scene-layout or device-epoch changes discard published WGPU handles. `RenderGraphComputeDispatchRecord` records the resulting `Ready` or `UsingLastGood` state, candidate and resolved fingerprints, device identity, and candidate failure; a rejected candidate that has no compatible publication remains an executor error and produces no false dispatch receipt.
...
The 2026-06-27 Runtime 15 M2 render graph fallback fixture naming hard cutover records `runtime_15_render_graph_fallback_fixture_naming_hard_cutover_static_passed_cargo_deferred`. `graphics/scene/scene_renderer/graph_execution/render_graph_execution_record/compute_workload.rs` now uses `unexpected-compute`, `unexpected.executor`, and `unexpected-pipeline` for the unexpected dispatch fixture instead of old generic legacy wording; the paired advanced-plugin resource fixture uses `fallback-virtual-geometry-without-resource-capability`. Guard `runtime_15_render_graph_fallback_fixtures_use_current_names` locks those source anchors, Runtime 15/status mirrors, module-convention docs, render-product-submit docs, and this render graph builder documentation. This is naming-only fixture cleanup and does not change planned workload matching, mismatch classification, unexpected dispatch counting, or backend-neutral diagnostic projection.
