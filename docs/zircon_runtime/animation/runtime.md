...
`zircon_runtime::animation` is a runtime-owned playback module, not a thin registration shell. It registers the canonical `animation.runtime` module identity, installs `DefaultAnimationManager`, and exposes a scene runtime hook that samples animation requests during `SystemStage::PostUpdate`.
