...
The builder also creates a source mip chain from mip 0 through `source_cubemap/mipmap.rs`. The source mip owner first builds a simple average chain for input acceleration, then each output texel samples a 4x4 grid across that texel's angular footprint. Each sub-sample is converted through the cube face axes and weighted by exact cubemap solid angle before sampling the previous average mip with cross-face linear sampling. This mirrors the UE `GenerateAngularFilteredMips` boundary at runtime scale: source mips are a blurred cubemap pyramid for skybox minification and FIS source LOD, not the specular PMREM result. The final source 1x1 mip averages all six faces, matching cmft's final-mip discipline and avoiding face-local low-mip blocks.
...
`SourceCubemapEnvironment` is carried by `EnvironmentExtract`. During scene-uniform upload, the renderer detects source revision and source hash changes, reallocates source and specular cube textures when face size or mip count changes, reallocates the optional irradiance cube texture when IEM face size changes, and uploads each face/mip slice with `queue.write_texture`.
...
The module keeps cmft/cmftStudio's face order, lat-long projection, source cubemap layout, cross-face neighbor sampling discipline, final-mip face averaging, linear roughness-to-LOD mapping, and sampling-side `fixCubeLookup`. As of EC-M3n, the current CPU validation bridge also uses cmft-style Blinn/cosine-power radiance filtering for PMREM mips instead of the earlier UE-style GGX FIS bridge. The implementation samples that lobe with Hammersley cone points rather than scanning every source texel bbox, so GPU/offline UE GGX FIS remains a later production artifact/readback parity target, not the description of the current screenshot path.

The result is intentionally staged. It provides a visually correct source-cubemap PMREM screenshot path, SH9 diffuse CPU reference, RGBA16F runtime upload, and BRDF LUT shader bridge now, without pretending that the full EC-M2 GPU/offline/IEM/cache architecture is complete.
...
The PMREM mip chain remains separate from the source mip pyramid. cmft/cmftStudio are now the current runtime validation references for PMREM radiance mips, linear LOD consumption, face order, equirect projection, cross-face edge discipline, final 1x1 face averaging, and the PMREM/IEM consumer split. UE `TextureCompressorModule.cpp` remains the source-mip reference for long-lat base cubemap generation, average input mips, and angular filtered cubemap mips, while UE reflection-environment GGX FIS remains future GPU/offline parity work rather than the active CPU screenshot bridge.
