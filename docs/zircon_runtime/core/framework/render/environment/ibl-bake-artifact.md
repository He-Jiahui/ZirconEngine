...
Source-cubemap and PMREM layouts are independent parts of that identity. Runtime code and tests that already hold an `IblBakeArtifactRequest` construct the descriptor through `current_for_request(...)`; rebuilding it with the older `current(...)` convenience constructor would replace the source layout with the PMREM layout, write a different cache identity, and turn an intended cache hit into runtime compute. The render-framework cache-hit and WGPU readback fixtures lock this full-request construction path.
...
Artifact application retains a full `SourceCubemapEnvironment::bake_artifact_hash` for asset provenance, while caching a separate PMREM section hash for GPU texture identity. `SourceCubemapUploadKey` contains the source revision/hash, PMREM section hash, and optional-IEM content hash; it deliberately excludes the full artifact hash and `IblBakeKey` remains source-based. `SceneEnvironmentCubemap::ensure_uploaded(...)` uploads source data when the source changes, PMREM when the source or PMREM section changes, and IEM when its content changes. SH9 is updated through its uniform path without reuploading a cube, so IEM-only artifact changes do not retransmit source or PMREM data.
