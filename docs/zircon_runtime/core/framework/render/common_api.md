...
tests:
  ...
  - rustfmt --edition 2021 --check zircon_runtime/src/core/framework/render/mod.rs zircon_runtime/src/text/shaping/cosmic.rs zircon_runtime/src/text/shaping/mod.rs zircon_runtime/src/text/layout/measure.rs (2026-06-28 text DTO facade import repair: passed)
...
`render::mod` re-exports only stable render product DTOs from the owning modules: camera snapshots and targets, image descriptors and sampler/fallback kinds, mesh bounds and topology, material descriptors/readiness reports, and shader entry/dependency/layout descriptors (`zircon_runtime/src/core/framework/render/mod.rs`). It no longer owns or re-exports a Text API. The renderer-neutral service contract is `core/framework/text::TextLayoutService`; shaped implementation models such as `ShapedGlyphRun` live under `text/model`, and the `text` subsystem owns the implementation. Runtime leaves therefore import the explicit Text contract or implementation owner instead of a retired private `render::text` path.
