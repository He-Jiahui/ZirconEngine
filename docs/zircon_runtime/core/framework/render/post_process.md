...
The compiled-scene renderer imports the physical offscreen target textures and buffers under these declared names. `scene-color`, `scene-depth`, `gbuffer-*`, `ambient-occlusion`, `global-illumination`, `final-color`, `bloom-texture`, `postprocess.depth-of-field.coc`, and `postprocess.depth-of-field.bokeh` map to concrete frame target resources; `light-list` maps to the clustered lighting buffer. `postprocess.color-graded`, `postprocess.effect-stacked`, `postprocess.history-resolved`, and `postprocess.final-composited` remain imported aliases for graph-resource consistency while concrete shader execution still writes through the existing postprocess target set. The DoF targets are allocated and graph-imported scratch resources; they are not yet populated by separate CoC or bokeh WGPU passes. `history.previous.scene-color` is imported only when `prepare_history_textures(...)` reports a compatible previous history texture, so execution evidence cannot claim history resolve on the first frame after allocation or rotation. `history.current.scene-color` is the label of the renderer history texture written by `HistoryCopy`; it is not the graph input consumed by history resolve.
...
Zircon is not yet Bevy-complete for post-processing. Motion blur is not implemented because the required motion-vector prepass contract has not been productized. Depth of field now has neutral focus/lens/bokeh controls plus graph-visible CoC and bokeh product-resource names backed by renderer-owned scratch targets, but it still lacks dedicated writer passes, temporal CoC history, near/far combine, and a resolved half-resolution bokeh policy. Chromatic aberration and vignette now have a neutral effect-stack DTO, graph/stat node, and baseline shader consumption, but they are not yet camera components or dedicated WGPU passes. MSAA writeback is represented only indirectly through camera MSAA settings and anti-alias fallback reporting; there is no Bevy-style sorted-camera MSAA writeback blit path yet.
...
2026-06-05 render-main-chain DoF resource-contract follow-up added `postprocess.depth-of-field.coc` and `postprocess.depth-of-field.bokeh` to `PostProcessGraphResourceNames`, makes active DoF effect-stack nodes produce those outputs, and imports renderer aliases for them through `import_frame_targets(...)`. The next 2026-06-05 target-allocation follow-up replaces those aliases with concrete `OffscreenTarget` scratch textures for CoC and bokeh, and imports those views by the same graph resource names. This is still split-pass preparation only; the current WGPU path still uses the final-pass lens/bokeh shader and no dedicated CoC/bokeh writer pass, temporal CoC history, near/far separation, or half-resolution policy is claimed. Focused source coverage was added in `post_process/stack.rs` and `execute_graph_stage.rs`. Scoped `rustfmt --edition 2021 --check`, path-scoped `git diff --check`, and conflict-marker scans passed. Cargo execution did not produce a clean success in this checkout: focused Cargo test wrappers timed out in the Windows runtime lib-test compile/link path, a warmed `cargo check -p zircon_runtime --lib --tests --locked --target-dir E:\cargo-targets\zircon-dof-resource-check-0605` emitted fresh runtime metadata before the wrapper timeout, and a final clean rerun was blocked by unrelated active `Cargo.lock` churn for `zircon_first_party_runtime_catalog` / plugin catalog packages under `--locked`.
