...
- rustfmt --edition 2021 zircon_runtime/src/core/framework/render/shader/variant_miss_report.rs zircon_runtime/src/core/framework/render/shader/mod.rs zircon_runtime/src/core/framework/render/mod.rs zircon_runtime/src/core/runtime/diagnostics/render_stats_store/shader_variant.rs zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_draw/queue_profile.rs zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pass/mesh_pass_processor.rs zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline_cache/mesh_pipeline_variant_registry.rs zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline_cache/mesh_pipeline_cache.rs zircon_runtime/src/graphics/scene/scene_renderer/mesh/mesh_pipeline_cache/ensure_pipeline.rs zircon_runtime/src/graphics/tests/render_product_mesh_cache/staged_prewarm/mod.rs (2026-06-27 runtime shader variant dimension correlation; current owner path updated by 2026-06-30 folder-backed follow-up)
  ...
  - zircon_runtime/src/bin/zircon_shader_prewarm/manifest/tests/raw_revision.rs::shader_prewarm_asset_root_manifest_does_not_promote_raw_wgsl_module
...
`ShaderAssetKind` separates generic validated source from the four explicit v2 `.zshader` product domains. Its five values are `Module`, `Surface`, `Include`, `Compute`, and `Fullscreen`. Raw WGSL/GLSL/SPIR-V importers produce `Module`; `.zshader v2` deliberately cannot declare that generic kind. Only `Surface` participates in material variant space; include shaders only provide import modules, compute shaders provide compute entry points/resources, and fullscreen shaders provide pass-local fragment resources. `ShaderRenderStateDescriptor`, `ShaderQueueDescriptor`, and `ShaderResourceDescriptor` are backend-neutral authoring contracts for the v2 parser; they remain plain serializable DTOs and do not contain `wgpu` layout objects.
...
`ShaderVariantKey` is the Plan 08 material pipeline variant key contract. It combines `material_shader`, `material_revision`, `geometry_source`, `shading_model`, `pass_type`, `features`, `quality`, and a backend `platform_token`. `packed_dims()` reserves non-overlapping segments only for fast in-memory specialization dimensions: the complete `u8` geometry source occupies bits `0..7`, shading model bits `8..15`, pass bits `16..19`, feature bits `20..51`, and quality bits `52..53`. It is not a persisted identity. `canonical_string()` serializes the full stable key, including material id/revision and platform token, for disk-cache hashing and shader prewarm manifests. The type remains backend-agnostic; WGPU shader modules, render pipelines, and cache entries still belong under `graphics`. `RenderQualityProfile::shader_quality` is the runtime-facing quality source and defaults to `ShaderQualityTier::Medium`; callers can override it with `RenderQualityProfile::with_shader_quality(...)`.
...
`ShaderVariantMissReport` is the neutral diagnostic DTO for variant cache behavior. It records variant requests, memory hits, disk hits, compile misses, disk writes, and disk errors for the last frame so runtime diagnostics can verify whether prewarm and disk-cache slices actually removed runtime compiles. Its structural gauges report the current full and shader-equivalent registry populations, its cache gauges report current WGPU mesh object residency, and its creation/async-queue count-time pairs are renderer-lifetime cumulative snapshots. Consumers must not sum the structural or residency gauges across frames; `accumulate(...)` retains the larger coherent snapshot for those fields while continuing to sum event counters. The PBR viewer exports these twelve performance fields after its Ready render in evidence schema v12.
...
`ShaderVariantPrewarmManifest`, `ShaderVariantPrewarmSource`, `ShaderVariantPrewarmRequest`, and `ShaderVariantPrewarmReport` are the neutral offline-cache DTOs. Schema version 3 stores final WGSL, its BLAKE3 hash, ordered include hashes, and template/compiler version strings exactly once in a content-addressed source table; every request contains only its final `ShaderVariantKey`, optional complete pipeline state, and `source_id`. The source ID is derived from that cache contract, while `source_label` is provenance only. Manifest integrity rejects duplicate, missing, non-canonical, or payload-hash-mismatched sources before graphics-side work begins. This prevents a pass x quality x geometry product from duplicating identical WGSL payloads while keeping WGPU types outside the framework contract. The report records requested, written, and failed counts, per-variant failures, a `dimension_summary` grouped by pass type, geometry source id, shading model id, and quality tier, source provenance, and the configured/resident/in-flight execution budget. The graphics prewarm executor currently uses one serial WGPU worker and rejects a manifest whose resident source table or individual source payload exceeds its configured budget. Geometry and shading dimensions use stable numeric id strings so report readers do not need renderer-private registries to diagnose gaps. The `zircon_shader_prewarm` tool can read an authored schema-3 manifest, emit the built-in fallback manifest, or scan asset roots for `.zmeta` compound shader packages, `.zshader` files, standalone `.wgsl` modules, and `.zmaterial` material instances. Only explicit `Surface` sources generate material requests; a standalone WGSL is inventoried and validated as `Module`, produces no material source-table row or variant, and causes a typed scan error when a `.zmaterial` resolves to it. Automatically generated built-in and asset-root requests can be expanded with repeated `--quality-tier low|medium|high|ultra` or `--quality-tier all`; no explicit tier still defaults to Medium so existing staging size stays stable. Authored manifests must be regenerated into schema 3 rather than using a compatibility request-source shim. WGPU product execution, cold/warm performance evidence, and RenderDoc capture are still pending managed validation.
...
Asset-root shader edit revision export closes the staged-cache edit-key gap without pretending the offline scan owns the live `ResourceManager` counter. `manifest/revision.rs` turns `.zmeta.source_digest` into a stable non-zero `ShaderVariantKey.material_revision`; explicit `.zshader` sources without `.zmeta` derive the revision from their include/source content hash list. `shader_source_from_zmeta(...)` and fallback `shader_prewarm_source(...)` consume those values, so changing a Surface package source hash produces a new prewarm key instead of reusing revision `1`. Raw WGSL modules still export source-hash-derived resource records, but because they have no material domain, editing one does not create a material prewarm key. `shader_prewarm_asset_root_manifest_uses_zmeta_source_digest_revision` and `shader_prewarm_asset_root_manifest_does_not_promote_raw_wgsl_module` lock those two boundaries, and `runtime_15_shader_prewarm_asset_revision_export_is_wired` locks the module split and docs/status anchors. Status: `render_plan08_asset_root_shader_edit_revision_export_passed_cargo_renderdoc_deferred`.
...
The scan path mirrors the shader package importer by reading `.zshader` `wgsl_files` in order and combining those files into the runtime WGSL payload before writing disk-cache entries. `.zshader` entry-point stages drive StandardPBR pass expansion for each selected geometry source: vertex+fragment sources emit Forward, GBuffer, DepthPrepass, Shadow, and Velocity; vertex-only sources emit DepthPrepass, Shadow, and Velocity; fragment-only sources emit Forward and GBuffer; compute-only sources do not enter the material-variant prewarm space. Standalone `.wgsl` sources have no serialized material domain, so they remain `Module` and emit zero material requests regardless of entry-point text. Scanned Surface requests use source-hash-derived material revisions for edit invalidation while source/include hashes remain part of the disk-cache key payload for stale-entry validation. `.zmaterial` files are parsed through `MaterialAsset`, joined back to scanned shader sources by shader `AssetReference` URL or resource id, and expanded into deduplicated material-dimension variants only when the resolved source is `Surface`; a resolved Module returns `MaterialShaderKindMismatch` before template expansion. The feature mapping matches runtime `PipelineKey`: `AlphaMode::Mask` sets `ShaderFeatureBits::ALPHA_TEST`, `double_sided = true` sets `ShaderFeatureBits::DOUBLE_SIDED`, and runtime `PipelineKey.receive_shadows` now sets `ShaderFeatureBits::RECEIVE_SHADOWS`. Built-in material lighting models also enter the prewarm key through `ShadingModelId::from_lighting_model`: PBR maps to StandardPBR, BlinnPhong maps to BlinnPhong, and Unlit maps to Unlit. `AlphaMode::Blend` material-instance requests are filtered to the Forward pass so transparent materials align with the current runtime transparent queue instead of prewarming unused G-buffer, depth, shadow, or velocity variants for that material instance. Custom lighting models can be mapped through explicit `--shading-model-id` / `--shader-shading-model-id` plugin ids; unknown custom models continue to fall back to StandardPBR until a project shading-model registry exporter can provide those ids automatically.
...
Runtime 15 R4.1/M3 的结构切片先移动 shader prewarm manifest 的测试 owner，后续 Plan 08 geometry-source 枚举、builtin standard material template prewarm、custom shading-model id prewarm、asset-root shader edit revision export 与 resource registry revision overlay 切片继续在 folder-backed owner 内扩展覆盖。`bin/zircon_shader_prewarm/manifest.rs` 当前 794 行，父文件只保留生产扫描/manifest 逻辑和 `#[cfg(test)] mod tests;` 挂载；`manifest/revision.rs` 承接 source-hash/content-hash revision projection，`manifest/paths.rs` 承接路径扫描 helper，`manifest/resource_registry.rs` 承接 exported `ResourceRecord` revision overlay。通用 manifest/material tests 位于 615 行 `manifest/tests.rs`，geometry-source 四组测试和 descriptor fixture 位于 250 行 `manifest/tests/geometry_sources.rs`，因此每个 owner 都保持在 Runtime 15 的 `<800` 行预算内。

子文件保留 `shader_prewarm_asset_root_manifest_reads_compound_zshader_package`，继续覆盖 compound Surface、`.zmaterial` feature bits、BlinnPhong/Unlit、material revision 与 alpha-blend Forward-only filtering；`shader_prewarm_asset_root_manifest_expands_requested_geometry_sources` 和 custom-geometry regression 改用显式 `kind = "surface"` package，避免测试夹具重新引入 raw-domain 猜测。`shader_prewarm_asset_root_manifest_uses_zmeta_source_digest_revision` 锁定 Surface package revision，`shader_prewarm_asset_root_manifest_does_not_promote_raw_wgsl_module` 锁定 standalone Module 编辑前后均不生成材质 source/variant，`shader_prewarm_asset_root_scan_rejects_material_bound_to_raw_wgsl_module` 锁定 `.zmaterial -> Module` 的类型化错误。现有 geometry/shading CLI、builtin standard material、revision 和 resource-registry 结构守卫继续保留；离线 kind hard-cut 的动态 Cargo/WGPU 验收仍待协调器运行。
...
The 2026-08-26 texture-presence identity hard-cut removes base-color, metallic-roughness, occlusion, and emissive presence from `PipelineKey` and `ShaderPipelinePrewarmState`. Those bindings are fixed by the renderer material ABI and always receive a real or neutral fallback texture, so presence does not change generated WGSL, layout, or PSO descriptor. Normal texture presence remains `ShaderFeatureBits::HAS_NORMAL_TEXTURE` because it changes tangent-frame requirements and source specialization. `MeshPipelineVariantRegistry` no longer scans 16 candidate keys merely to count equivalent variants; one canonical lookup now reuses one pipeline variant. Managed Cargo/WGPU and 1/1k/100k profile evidence remains pending.
...
This framework module is not a full Bevy `ShaderPlugin`, `ShaderCache`, or `PipelineCache`. It does not own WGSL import resolution, shader-definition composition, dependent-pipeline invalidation, bind-group-layout deduplication, compilation scheduling, or GPU pipeline publication. It does publish neutral pipeline diagnostics and fallback state so graphics owners do not have to collapse every unavailable pipeline into `None`.

Mesh pipeline admission distinguishes `Ready`, recoverable `Deferred`, and terminal `Failed` across Base, GBuffer, DepthPrepass, both Shadow kinds, Velocity, both TAA reactive kinds, and OIT. The frame report correlates canonical shader/material identity, pipeline variant id, entity, consumer, state/action/reason, state age, and occurrence count. Repeated contexts coalesce and the report retains at most eight fallback rows. This makes queue saturation and compilation failure observable without allocating on the normal ready path; after the bounded buffer fills, unseen contexts also return before diagnostic string allocation while aggregate counts continue. It is not yet a complete last-good or error-material system: an error PSO and its ABI-compatible material bindings must publish together, while view identity, bootstrap deadlines, and unified cross-pass artifact publication remain open. Managed Cargo/WGPU, product PNG, RenderDoc, profile, and power evidence remain pending.

The prepared-material side now has an explicit last-good leaf. A replacement candidate that reaches blocking readiness, shader dependency preparation failure, or texture residency failure cannot overwrite an existing renderable `PreparedMaterial`; the published revision, runtime pipeline key, custom uniform, and standard uniform remain the draw-facing bundle, while rejected readiness remains available to material management and frame statistics. Shader locator/id/root revision plus the process-local readiness revision of the complete shader dependency closure participate in material cache identity, so direct and transitive shader edits cannot disappear behind a cache hit. The dependency revision also participates in runtime `PipelineKey` identity across mesh passes, while persistent `ShaderVariantKey` remains content-addressed by assembled WGSL rather than a process-local epoch. Immutable readiness snapshots and maintained reverse edges make stable dependency-generation checks O(1). Root `PreparedShader` publication follows successful dependency preparation, and texture residency precedes candidate uniform allocation. Atomic shared-texture publication, PSO failure, unified cross-pass publication, cold error material, exact rejected-candidate retry suppression, old-generation retirement, and submission-fence ownership remain open, so pipeline admission still reports terminal failure rather than binding an unpaired fallback PSO.

`PreparedMaterialBundle` now owns the fields that must publish together: material revision, shader/texture dependency identities, `MaterialRuntime`, and both GPU uniform bindings. The current path still publishes a successful replacement immediately. The intended cross-pass owner is the renderer boundary between scene-resource preparation and mesh-draw build, where the actual frame's material, geometry, quality, and compiled-pipeline pass requirements are available; the asset streamer must not own or precompile renderer PSOs.

Mesh pipeline terminal-failure and state-age identity is target-scoped rather than variant-only. OIT intentionally reuses a transparent Base `MeshPipelineVariantId` while owning a separate PSO and layout, so the cache keys these states by `(PipelineCreationTarget, MeshPipelineVariantId)` and a Base async toggle removes only Base-domain entries. GBuffer command projection and every synchronous mesh PSO owner use the same typed store. DepthPrepass, Shadow, Velocity, and TAA reject failed draws with pass-specific diagnostics and reset replay state; Shadow and TAA preserve exact sub-kind identity. OIT records `oit_fragment_store`, distinguishes a missing `fs_oit` contract, and still returns a graph error after the diagnostic. The legacy synchronous `Option<&RenderPipeline>` admission APIs and their production consumer `expect` calls are removed.
...
The runtime shader contract now hard-cuts Surface assets to material-function source. A Surface must contain exactly one authored `fn zr_material_surface(`, exactly one `ZrVertexOutput` parameter (or the public `ZrSurfaceInput` alias), a `ZrSurfaceOutput` return, and no executable entry rows; missing, duplicate, signature-mismatched, mixed, or full-pass shapes fail readiness and streaming. Full executable WGSL belongs to an executable shader kind such as Module or Fullscreen and cannot participate in material variants merely by carrying Surface metadata. The asset/streaming boundary publishes the single `ShaderSurfaceSourceContract::MaterialFunction` result, and the mesh template path reads it in O(1) instead of scanning WGSL for every pipeline-source request. The built-in standard-PBR fallback follows the same contract and contains only its canonical material function. The retired raw Surface compatibility probe is historical evidence only and is not a supported current-source path.
