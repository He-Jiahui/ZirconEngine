...
The extent fields preserve the Bevy/WGPU distinction between a 3D texture depth and array-layered textures. `TextureAssetDescriptor` normalizes these fields before projection: 3D textures force `array_layer_count` to `1`, while 1D/2D array textures require `array_layer_count` and `depth_or_array_layers` to agree. `RenderImageDimension::Cube` is represented as a WGPU 2D texture with a non-zero `6N` layer count, where six layers form one cubemap and more layers form a cube array.
...
`TextureAsset::render_image_descriptor()` is the asset-facing projection. Container texture payloads can expose encoded format, mip count, and array layers even when the current GPU upload path cannot yet prepare that container directly. Descriptor overrides can request HDR formats, storage usage, custom asset usage, 3D dimensions, cube dimensions, or different fallback classes.

For cubemaps, import settings accept `dimension = "cube"` and the `cubemap` alias. When no layer count is supplied, the descriptor defaults to six layers. Explicit cube layer metadata must match between `array_layer_count` and `depth_or_array_layers` and must be a non-zero multiple of six. See [cube-texture-resource-contract.md](/E:/Git/ZirconEngine/docs/zircon_runtime/core/framework/render/cube-texture-resource-contract.md) for the renderer-side view mapping.
...
The cube dimension contract does not mean the IBL PMREM chain is complete. It only makes cube textures representable and uploadable. The skybox cutover, `equirect_to_cube.wgsl`, GGX filtered mip generation, SH9, and BRDF LUT are tracked by shader plan 06 and render plan 11.
...
`runtime_texture_cube_resource_contract` proves the public cubemap descriptor and upload-readiness contract: default six-face normalization, rejection of non-`6N` layer counts, and upload readiness for a complete square RGBA8 six-face payload.
