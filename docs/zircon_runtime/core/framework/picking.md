...
- `resolve_picking_outputs` resolves one backend-output batch into both `PickingHoverMap` and `PickingPipelineReport` from one sorted projection. Consumers that need both products must use this boundary instead of invoking the independent hover/report constructors.
...
- `PickingSettings` currently controls only whole-pipeline enablement and ray-map/backend execution. Removed target-priority/default-pickable fields had no consumers and were not real configuration contracts; `PickingScheduleLabel` names the fixed stages.
...
`resolve_picking_outputs` is the shared route-plus-diagnostics boundary. It groups and sorts each pointer once, derives the report while borrowing that projection, then consumes the same projection to build the hover map. The full pipeline and editor live route/debug adapter both use it; the independent constructors remain appropriate only when a caller needs one result.
...
The editor viewport now consumes this through `ViewportOverlayPointerRouter::debug_feed_at` and through the `ViewportPointerDispatch::picking_debug_feed` field returned by normal move/down dispatch. The consumer adapter uses the UI hit-test stack and editor precision candidates to build one runtime `PointerHits` output, then calls `resolve_picking_outputs` once so route selection reads the hover map while debug feed reads the matching report without a second sort. It does not mutate selection or treat the private editor route enum as the debug authority.
...
`run_picking_pipeline` executes the fixed runtime stage order `Input -> RayMap -> Backend -> Hover -> Events`. It returns all intermediate frame products plus stage counters and the diagnostics report, so editor overlays, remote dev tools, and future runtime debug panels can observe the same picking state without owning the pipeline. `PickingSettings::ray_map_enabled` disables ray/backend collection while retaining later empty-frame stages. When `PickingSettings::enabled` is false, the runner clears `PickingEventState` and returns empty output with every stage marked disabled; this prevents stale hover, press, drag, or click state from surviving a disabled picking frame.
