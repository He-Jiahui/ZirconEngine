...
- `job_system_boundary` now provides a static structure mirror for the task owner: `expected_module_count = 9`, `direct_rayon_paths = 2`, `schedule_parallel_executor_direct_rayon = []`, `diagnostic_anchor_count = 4`, `behavior_test_anchor_count = 10`, `missing_behavior_test_anchors = []`, `oversized_modules = []`, and `risks = []`.
