...
- exact traffic, queue depth/peak, blocked-receiver/publisher and drop counters; exhaustive or sampled queue-age/publish-duration timing; and exhaustive aggregate diagnostics for observed waits on per-topic delivery locks
...
Each concrete subscription holds a weak state reference and a poison-safe `VecDeque` queue state paired with a condition variable. Admission, drop-oldest replacement, physical dequeue, and queue-depth/peak accounting all commit under the same queue-state lock, so a capacity-one queue cannot report an impossible peak of two. A full bounded/latest queue replaces its oldest event without decrementing and then incrementing the unchanged global depth; delivered, dropped and sampled queue-age accounting still advance. Blocking receivers publish their waiting count before the condition-variable wait. Same-topic publishers first use a poison-safe `try_lock` fast path; only observed contention enters the waiting count and records a delivery-lock wait once acquired, so uncontended publishes do not inflate lock-wait evidence. The final `EventBusState` owner marks every queue inactive, drains it, and wakes all waiters, so blocked, polling, and timed receivers observe the framework `Disconnected` result instead of keeping the runtime alive indefinitely.

`EventBus::default()` and `CoreRuntime` use `EventBusDiagnosticsMode::Sampled` with the named interval 64. Exact traffic, depth, drop and waiter counters remain enabled, while independent publish and delivery sequences capture one routine timestamp per interval. `EventBus::new(EventBusDiagnosticsMode::Enabled)` is the exhaustive comparison/diagnostic mode. `EventBus::new(EventBusDiagnosticsMode::Disabled)` skips metric atomics and all queue, publish and delivery-lock timestamps; its snapshot keeps only live topic/subscriber topology and reports `enabled: false`, interval zero and metric fields at zero.

Publish keeps one immutable `Arc<EngineEvent>` across fan-out, clones it only for subscribers before the last, and transfers the original owner to the final subscriber. Clone count is therefore `N-1` for `N` subscribers without changing delivery order or pointer identity.
...
`core::runtime::tests::events::benchmark_evidence` owns the ignored, managed-only Runtime07 performance matrix. It measures publish P50/P95/P99/max for 1/2/5/100 subscribers across 64 B, 4 KiB and 256 KiB payloads; rotates exhaustive, sampled and disabled diagnostics over five repetitions for 1/100 subscribers with a 4 KiB payload; and records Windows process RSS plus queue age while an 8,704-event producer pressures a capacity-64 paused consumer. V2 rows include measured publish throughput, exact timing-capture counts, payload `Arc` clone counts and bounded-replacement depth RMW counts. The tests hard-gate exact publish/delivery/drop/depth counters, payload size, shared pointer identity, disabled-metric zeroing, bounded retention, final last-64 ordering and percentile ordering.
...
It then runs the performance evidence tests once in release mode; their internal five-repeat rotation controls order bias and emits every `EVENTBUS_BENCH_V2` row:

```powershell
cargo +1.94.1 test --release -p zircon_runtime --lib event_bus_runtime07_ --locked --jobs 1 -- --ignored --nocapture --test-threads=1
```

The ordinary library suite leaves these tests ignored, so broad correctness validation does not silently absorb a performance workload. Acceptance requires at least 98% routine timestamp-capture reduction, `N -> N-1` payload `Arc` clones, `2 -> 0` queue-depth RMW operations per full-queue replacement, valid P50/P95/P99 order, and no more than 5% sampled P95 or throughput regression against exhaustive mode. RSS remains machine evidence rather than a cross-machine absolute threshold; allocator-internal byte accounting remains outside this runtime owner.
