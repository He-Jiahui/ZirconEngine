...
`foundation::persistence` has no compatibility module or re-export. `foundation` remains a consumer for configuration persistence; it does not own generic filesystem commit behavior. The old `ResourceIo` contract and `ResourceIoError` were sealed, had zero implementations and zero consumers, and are now removed rather than exposed as a false capability. The functioning Resource I/O surface is the public `atomic_write`/`atomic_write_new` pair plus crate-private durable transaction assembly. A later filesystem/source/mount provider must arrive as a new, implemented contract with real Asset consumers; the deleted synchronous trait has no compatibility projection. Source status is `frameworks01_resource_io_false_capability_hardcut_static_and_managed_compile_passed_full_suite_pending`.
...
Within one mutation batch, remove followed by re-add of the same Resource ID retains the last staged record as the Ready revision and state baseline. A changed Ready identity therefore advances the existing revision and invalidates stale payload CAS even when removal is an intermediate operation; importer identity is part of that revision identity. A committed removal followed by a later independent add is still not globally monotonic, so consumer caches must observe `ResourceEvent::{Added, Updated, Removed}` and invalidate the affected identity. Animation's compiled evaluator does so. If a changed Ready record would advance `u64::MAX`, preflight returns `ResourceRegistryError::RevisionExhausted` and rejects the entire batch before registry, payload, generation, or event mutation. `resource_snapshot_never_pairs_a_new_revision_with_an_old_payload` stress-updates one resource while reading snapshots and requires every observed payload version to match its revision.
...
Current Runtime 04 source-owner synchronization (2026-08-30): `asset_pipeline_boundary` reports `expected_source_file_count = 26` and `expected_guard_file_count = 22`; canonical reload transaction state and tests live in `crates/zr_resource/src/manager/commit.rs` and `crates/zr_resource/src/tests.rs`. `core::resource` remains a curated projection, not a second manager/test implementation; broader Cargo gates remain pending.
