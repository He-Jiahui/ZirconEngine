...
tests:
  ...
  - zircon_runtime/src/scene/tests/render_extract/level_source_guards.rs::level_system_render_extract_does_not_resort_the_ordered_animation_pose_cache
  - zircon_runtime/src/scene/tests/render_extract/camera_order.rs::scene_frame_reuses_its_frozen_camera_descriptors_for_the_order_report
  - zircon_runtime/src/scene/tests/render_extract/particles.rs::particle_extract_filters_dynamic_component_candidates_before_scene_state_queries
  - zircon_runtime/src/scene/tests/asset_scene.rs
  ...
  - zircon_runtime/src/scene/tests/world_basics/render_extract.rs::render_mesh_phase_projection_consumes_entries_without_cloning_snapshots
...
The 2026-06-27 Runtime 15 M2 frame extract snapshot adapter naming hard cutover records `runtime_15_frame_extract_snapshot_adapter_naming_hard_cutover_static_passed_cargo_deferred`. `core/framework/render/frame_extract.rs` now describes `RenderFrameExtract::from_snapshot(...)` as building from a `scene viewport snapshot packet`, making the preview/roundtrip/synthetic validation adapter boundary explicit without changing `SceneViewportRenderPacket` input or `RenderSceneSnapshot` conversion. Guard `runtime_15_frame_extract_snapshot_adapter_uses_current_names` locks the source wording, Runtime 15/status mirrors, module-convention docs, and this scene render-extract documentation; Cargo remains deferred behind active external cargo/rustc lanes.
...
The mesh/phase projection consumes each sorted prepared mesh entry exactly once. A visitor writes snapshots directly into the caller-owned entry buffer instead of allocating a temporary Vec for every scene mesh entity. The projection then preallocates aligned mesh and phase-input vectors, builds the phase row while the snapshot is still owned locally, and moves the snapshot into the mesh vector. Visible material overrides are captured once per entity while the matching `MeshRenderer` is already borrowed, rather than re-looking up the component and cloning the block once per expanded primitive. This removes the former temporary allocations and per-frame deep clone of every `RenderMeshSnapshot` while preserving identical `mesh_index`, queue ordering, and override output.
...
The particle collector snapshots references to the four particle/HUD dynamic-component slots before it queries hierarchy, render layers, or transforms. Entities with none of those components exit immediately through a stack-only candidate check, so ordinary scene entities do not pay derived-scene lookup costs. Candidate entities keep the same active/layer filtering and output ordering.
...
Frame construction applies the request-adjusted selected-camera override while the descriptor list is built, then derives `RenderCameraOrderReport` from that same frozen list. It does not rebuild all scene camera descriptors a second time. The standalone `World::render_camera_order_report()` API still constructs its own snapshot because it has no frame-owned list to reuse.
