...
Project scanning injects the process runtime's compute `TaskPool` into `stage_environment_ibl_source_with_parallel_executor(...)`. The importer retains source decoding, cache lookup, request identity, and artifact writes; the framework cubemap builder receives only the `ParallelSliceExecutor` contract. The serial `stage_environment_ibl_source(...)` entry remains the deterministic baseline for direct tools and equivalence tests. Cache hits return before either builder is invoked, so a current `.zcube` plus `.zribl` pair performs no source-mip or PMREM recomputation.
...
The parallel-staging regression stages the same HDR once through the serial baseline and once through a caller-owned two-worker compute pool with an explicit 64-face PMREM layout. It requires byte-identical `.zcube` and `.zribl` outputs and then requires the parallel second pass to return `Reused`. The companion source-cubemap unit test compares the complete in-memory chain for the same serial/parallel explicit layout.
