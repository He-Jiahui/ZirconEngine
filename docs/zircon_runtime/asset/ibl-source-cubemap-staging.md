...
Project scanning injects `ProjectAssetManager`'s runtime IO `TaskPool` into `stage_environment_ibl_source_with_parallel_executor(...)`. The importer retains source decoding, cache lookup, request identity, and artifact writes; the framework cubemap builder receives only the `ParallelSliceExecutor` contract. The serial `stage_environment_ibl_source(...)` entry remains the deterministic baseline for direct tools and equivalence tests. The parallel builder schedules equirectangular base projection, source-mip filtering, and each PMREM mip's independent six-face GGX work through that executor, then copies completed faces back in stable face-major order. Cache hits return before either builder is invoked, so a current `.zcube` plus `.zribl` pair performs no source-mip or PMREM recomputation.

`stage_environment_ibl_source_with_parallel_executor_and_decoded_image(...)` is the viewer-facing variant for a caller that already has linear `DecodedTextureImageRgba32F` pixels. It consumes those pixels exactly once for staging while retaining the original context bytes and settings for source revision, artifact request identity, and current-cache validation. This avoids a second HDR/EXR decode for exposure/layout inspection without introducing a viewer-owned cache or a parallelism bypass.
...
`stage_source_cubemap_texture(...)` applies the same dual-artifact lifecycle to captured `ZRZCUBE1` plus directly decodable HDR cubemap containers. It accepts a validated native source-only `.zcube`, linear RGBA16F/RGBA32F DDS, KTX1, and uncompressed KTX2 payloads after strict six-face/full-mip classification. Captured `.zcube` classification is header-only and a current pair returns before pixel decode; a miss decodes its canonical RGBA16F source mips and rebuilds PMREM/SH9 through the existing asset-importer CPU recipe. DDS bytes remain face-major per cmft, while KTX bytes are reordered from mip-major disk layout. `stage_external_source_cubemap_texture(...)` remains the DDS/KTX compatibility entry.
...
The parallel-staging regression stages the same HDR once through the serial baseline and once through a caller-owned two-worker compute pool with an explicit 64-face PMREM layout. It requires byte-identical `.zcube` and `.zribl` outputs and then requires the parallel second pass to return `Reused`. The companion source-cubemap unit test compares the complete in-memory serial/parallel chain, while the PMREM module contract directly requires one executor dispatch per PMREM mip and byte-identical serial output.
