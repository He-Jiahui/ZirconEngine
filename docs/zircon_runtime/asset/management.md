...
tests:
  ...
  - zircon_runtime/src/asset/tests/assets/shader_readiness.rs::shader_asset_management_record_set_sorts_and_summarizes_records
  ...
  - cargo check -p zircon_runtime --lib --locked --jobs 1 --target-dir D:\cargo-targets\zircon-user-error-0611 --message-format short --color never (2026-06-11 asset family issue-index slice: passed; existing zircon_runtime warnings only)
  - cargo test -p zircon_runtime --lib --no-run --locked --jobs 1 --target-dir D:\cargo-targets\zircon-user-error-0611 --message-format short --color never (2026-06-11 asset family issue-view slice: passed lib-test compile/link after stale renderer-data pipeline fixture fix; existing zircon_runtime lib-test warnings only)
  - D:\cargo-targets\zircon-user-error-0611\debug\deps\zircon_runtime-de6f737e1b69a0f9.exe asset_management_record_sets_summarize_asset_family_lists --test-threads=1 --nocapture (2026-06-11 asset family issue-view regression: passed, 1 passed / 3355 filtered)
  - D:\cargo-targets\zircon-user-error-0611\debug\deps\zircon_runtime-de6f737e1b69a0f9.exe project_manager_imports_minimal_gltf_material_shader_mesh_sample --test-threads=1 --nocapture (2026-06-11 project sample family issue-view/default PBR root alignment: passed, 1 passed / 3355 filtered)
...
`AssetManagementOverview` is the lightweight carrier for this summary plus the family rows, status buckets, and issue buckets. It intentionally omits the per-family detail record sets, so headers, navigation chrome, and quick badge reads can use it without serializing model, mesh, scene, entity, material, or shader detail rows. `AssetManagementOverview::from_summary(...)` derives family rows, the status index, and the issue index from a precomputed summary, and `AssetManagementRecordSets::overview()` projects the same compact state from the full aggregate payload. The overview also derives row-bearing status and issue views from those stable family rows, so panels can ask for one top-level bucket without rescanning the aggregate payload.
...
`AssetManagementFamilyStatusView` is the row-bearing companion for one status bucket. It carries the selected status, the matching family ids, the matching `AssetManagementFamilySummary` rows, and summed total/ready/degraded/issue counts for that bucket. `AssetManagementOverview::family_status_view(...)` and `AssetManagementRecordSets::family_status_view(...)` derive this view from the family rows, so editor and runtime panels can render a "ready families" or "degraded families" table without fetching every detail row.

`AssetManagementFamilyIssueIndex` derives the sibling issue buckets from the same rows: `clean` for families whose `issue_row_count` is zero and `with_issues` for families that expose one or more issue rows. It provides helpers for total family count, issue-bearing family count, an issue-family presence check, clean/issue-family lookups, and a bucket lookup for `AssetManagementFamilyIssueBucket::{Clean, WithIssues}`. This index is intentionally family-grained; it points panels to the model, mesh, material, or shader table that owns the detailed rows instead of duplicating per-family issue detail.

`AssetManagementFamilyIssueView` is the row-bearing companion for one issue bucket. It carries the selected bucket, the matching family ids, the matching `AssetManagementFamilySummary` rows, and the summed issue-row count for that bucket. `AssetManagementOverview::family_issue_view(...)` and `AssetManagementRecordSets::family_issue_view(...)` derive this view from the already-stable family rows, so panels can render the "families with issues" table without rescanning or fetching every model, mesh, material, or shader detail row.

`AssetManagementRecordSets` carries these rows in `families`, the derived status buckets in `family_status_index`, and the derived issue buckets in `family_issue_index`. `AssetManagementRecordSets::family_summaries()`, `family_status_index()`, `family_status_view(...)`, `family_issue_index()`, `family_issue_view(...)`, and `overview()` expose compact views for callers that already hold the aggregate payload. `ResourceStreamer::asset_management_overview()`, `asset_management_family_summaries()`, `asset_management_family_status_index()`, `asset_management_family_status_view(...)`, `asset_management_family_issue_index()`, and `asset_management_family_issue_view(...)` provide narrow accessors for callers that only need top-level overview state and not the full record-set payload.
...
`ProjectAssetManager::asset_management_record_sets()` combines those families with an empty `RenderMaterialManagementRecordSet`, because asset-layer callers do not have renderer-prepared material rows. `asset_management_overview()`, `asset_management_family_summaries()`, `asset_management_family_status_index()`, `asset_management_family_status_view(...)`, `asset_management_family_issue_index()`, and `asset_management_family_issue_view(...)` expose the compact read surfaces without requiring a graphics device. This gives editor/asset tooling a neutral project-level management payload for model, mesh, scene/entity, material asset, and shader state while preserving renderer-prepared material detail as an optional overlay.
