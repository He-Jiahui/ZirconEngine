...
tests:
  ...
  - zircon_runtime/src/asset/tests/module_lifecycle.rs::asset_module_manager_uses_the_activating_runtime_io_owner
...
Runtime 11 now treats each `CoreRuntime` as the production task owner. `CoreRuntime::new()` creates an explicit `ExecutionRuntime`; the `AssetModule` manager factory upgrades its activation context and injects that runtime's `Io` pool into `ProjectAssetManager`. The manager and every spawned `AssetWorkerPool` therefore share the activating runtime's worker budget and shutdown admission gate instead of silently selecting the process-default pool.

`ProjectAssetManager::new(io_task_pool)` is the explicit injection entry. `ProjectAssetManager::default()` remains a standalone/process-default construction path for legacy consumers and tests, but the production `AssetModule` no longer uses it; remaining default consumers are separate Runtime11 migration work. `AssetWorkerPool` submits each unique decode through `TaskPool::spawn(...)`; it does not create dedicated threads. `AssetWorkerThreadBudgetSource` has only `TaskPoolIo`, and `asset.worker.budgeted_threads` reports the injected IO pool's parallelism rather than a second charged thread count.
...
Current Runtime 04 source-owner synchronization (2026-08-30): `asset_pipeline_boundary` reports `expected_source_file_count = 26` and `expected_guard_file_count = 22`; reload transaction state and its behavior matrix are canonical in `crates/zr_resource/src/manager/commit.rs` and `crates/zr_resource/src/tests.rs`. Retired Runtime projection paths are not counted as owners; broader Cargo gates remain pending.
