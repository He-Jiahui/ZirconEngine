...
tests:
  ...
  - zircon_runtime/src/asset/tests/pipeline/worker_pool.rs::project_asset_manager_spawns_worker_pool_with_frame_sampler
...
`AssetWorkerPool::request(...)` is the only public request entry. The former `request_sender()` channel escape hatch is retired, so callers cannot bypass in-flight coalescing, queue-full rollback, or worker diagnostics by cloning the raw request channel. Low-level queue tests keep using the workerless constructor inside the module test surface rather than a production sender clone.
