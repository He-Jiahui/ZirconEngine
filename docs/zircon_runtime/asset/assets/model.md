...
- Plan 08 skinning/VG joint-channel split (2026-07-05 status: `render_plan08_skinning_filter_vg_joint_channel_split_direct_binary_passed_current_source_text_blocked`; focused current-source glTF skinning channel regression passed 1/1; current-source production `cargo check -p zircon_runtime --lib` passed; direct generated-binary broad `skinning` filter passed 20/20; fresh current-source Cargo test wrapper blocked by active runtime text test compile drift)
  - zircon_runtime/src/asset/tests/assets/model.rs::virtual_geometry_vertex_ordinals_pack_into_joint_index_slots
  - zircon_runtime/src/asset/tests/assets/model.rs::virtual_geometry_vertex_ordinals_do_not_rewrite_skinned_primitives
  - zircon_runtime/src/asset/tests/assets/model.rs::virtual_geometry_vertex_ordinals_do_not_rewrite_non_vg_primitives
  - zircon_runtime/src/asset/tests/assets/gltf_importer/vertex_channels.rs::importer_preserves_gltf_skinning_channels_on_model_vertices
  - zircon_runtime/src/asset/tests/assets/mesh/conversion_import.rs::static_virtual_geometry_primitive_converts_to_mesh_asset_with_ordinal_channels
...
When a static primitive carries `virtual_geometry`, `ModelPrimitiveAsset::assign_virtual_geometry_vertex_ordinals()` standardizes the render-time vertex ordinal used by `zr_geometry_virtual_geometry.wgsl`. The ordinal is packed into `MeshVertex.joint_indices[0]` and `[1]` as a 16+16 bit value, and `decode_virtual_geometry_vertex_ordinal(...)` mirrors the shader-side unpack for tests and import guards. Non-VG primitives keep authored joint channels unchanged; weighted skinned primitives also keep authored joint/weight channels unchanged even if a VG payload is present, because the current VG custom geometry source treats those first two joint slots as its explicit vertex ordinal channel rather than as skinning data. Importer auto-cooking and `.model.toml` backfill therefore skip automatic VG payloads for primitives with non-zero skinning weights. ProjectAssetManager `.model.toml` import tests normalize their expected static VG primitive through the same helper before comparing against the loaded model, so root model payload assertions match the shared importer/MeshAsset conversion contract. The Runtime 15 guard also records that the stale `zircon_runtime-770562bad16f99eb.exe` binary from `2026-07-02 05:27:25 +08:00` still fails `asset_manager_imports_model_toml_with_virtual_geometry_payload` with the old expected payload; that audit is not counted as a post-fix pass or regression. Follow-up status `render_plan08_virtual_geometry_project_asset_manager_fixture_cargo_wrapper_passed_renderdoc_deferred` proves the rebuilt current-source fixture through direct latest-binary and fresh no-default Cargo-wrapper runs. The Plan 08 skinning follow-up status `render_plan08_skinning_filter_vg_joint_channel_split_direct_binary_passed_current_source_text_blocked` proves the split behavior for glTF skinning and static VG mesh conversion, while a fresh Cargo wrapper remains blocked outside this model module by active runtime text test compile drift.
