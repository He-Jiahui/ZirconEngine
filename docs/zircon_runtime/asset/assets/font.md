...
The FR-M2 importer tests copy the runtime Fira font fixture next to a temporary `.font.toml`, import it through `AssetImporter` and `ProjectManager`, assert that parsed metadata is attached, cmap coverage contains `A`, and family members are populated, and build an in-test TTC fixture to assert multi-face metadata enumeration. The graphics text database tests include temporary patched-font/TTC fixtures that verify best-match reads OS/2 weight from source bytes and keeps distinct `face_index` registrations, plus a font-asset registration test that verifies family-member overrides, fallback families, repeated asset registration dedupe, and two logical declarations sharing one physical face. The patched-font/TTC fixture construction now lives in `graphics/text/font/test_font_fixtures.rs` and the family-member projection lives in `graphics/text/font/asset_registration.rs` instead of the database owner. The UI font manifest, renderer font manifest, and UI registry tests cover render-strategy default selection, schema-v1 `render_mode` precedence, disallowed Auto clamping, UI registry fallback-family merge/dedupe, and the old direct `FontAsset` literal shape is no longer used. Runtime 15 M2 font/UI asset schema naming hard cutover coverage is `runtime_15_font_ui_asset_schema_names_use_current_policy_terms`, which rejects reintroducing the retired font parameter wording or the removed `UiV2DocumentImportProfile` branch in `asset/importer/ingest/ui_v2_document_import.rs`. Runtime 15 M2 font render-mode priority fixture naming hard cutover coverage is `runtime_15_font_render_mode_priority_fixture_uses_schema_v1_name`, which locks `graphics/scene/scene_renderer/ui/font_asset.rs::schema_v1_render_mode_takes_priority_over_strategy_default_mode` and the `module_convention_gate classified-and-clear` status. The SDF bake tests cover fallback when `fontsdf` cannot open a requested non-zero face index, and their temporary manifest/source pair now cleans up both paths through a Drop helper. On 2026-06-28 the warmed `zircon_runtime --lib` no-default cargo check passed with existing warnings; the focused UI registry test first failed on a Cargo target fingerprint path write error and then timed out during compile in a separate target directory, so it is not recorded as passed.
