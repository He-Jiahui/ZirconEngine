...
Texture container GPU upload, full MikkTSpace tangent parity, GPU upload of tangent and richer mesh-only morph channels, glTF animation-channel import for morph target weights, full animation/skin subasset payloads, multiple skin bindings per shared mesh, and removing the remaining root model compatibility envelope after renderer/plugin consumers no longer need it remain later milestones from the 2026-05-20 plan.
