...
The 2026-05-19 `.zmaterial` hard-cutover closeout passed the focused WSL runtime checks on `/mnt/f/cargo-targets/zircon-zmaterial-final-wsl`: runtime `material` tests (`68` passed), runtime `shader` tests (`13` passed), runtime `asset::tests::project::zmeta` tests (`8` passed), the Virtual Geometry runtime plugin library check, and `virtual_geometry_visibility_debug_contract` (`3` passed).
