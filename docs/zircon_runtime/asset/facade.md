...
- The public `AssetManager` service trait forwards the importer capability report helpers so tools that resolve the manager through the core service boundary can query importer availability without downcasting to `ProjectAssetManager`. It also exposes `relocate_project_source(uuid, target)`: `ProjectAssetManager` alone holds the project generation gate, reserves `ResourceMutationBatch::rename`, commits the source/meta/registry durable generation, installs the candidate project, and publishes the renamed change. Editor/tool callers never mutate a `ProjectManager` clone or source path directly. The current MVP accepts only active, digest-current, single-file project sources; compound sources, redirectors, and fix-up are not silently simulated.
...
Current Runtime 04 source-owner synchronization (2026-08-30): `asset_pipeline_boundary` reports `expected_source_file_count = 26` and `expected_guard_file_count = 22`; `crates/zr_resource/src/manager/commit.rs` owns reload transaction-state mutation and `crates/zr_resource/src/tests.rs` owns the transition behavior matrix. Retired `src/core/resource/manager/*` and `src/core/resource/tests.rs` paths are not compatibility owners; broader Cargo gates remain pending.
