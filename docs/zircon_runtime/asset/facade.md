...
The Runtime 04 `asset_pipeline_boundary` structural audit now mirrors the facade/query/resource-state anchors together with worker, watcher, artifact, and pending Cargo-gate anchors. Its current focused evidence reports source 19/19, guard 10/10, Runtime 04 guard anchors 21/21, worker diagnostics 5/5, artifact-store scene roundtrip guards 4/4, and `risks = []`; broader `asset::` / `worker_pool` Cargo filters are still pending.
