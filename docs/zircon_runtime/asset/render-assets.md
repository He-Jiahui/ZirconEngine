...
plan_sources:
  ...
  - user: 2026-06-08 continue glTF KHR_texture_transform standard material import
tests:
  - cargo test -p zircon_runtime --lib runtime_15_resource_streamer_resolve_texture_id_cleanup --no-default-features --features core-min --locked (2026-06-23 Runtime 15 F12 ResourceStreamer resolve texture id cleanup: blocked before compile by stale Cargo.lock/Cargo.toml mismatch; rustfmt and static guards passed)
  - zircon_runtime/src/asset/tests/assets/mesh.rs
  ...
  - zircon_runtime/src/asset/tests/assets/gltf_importer.rs::importer_preserves_gltf_texture_transform_on_standard_material_slots
  - zircon_plugins/gltf_importer/runtime/src/tests.rs::importer_preserves_gltf_material_texcoord_as_texture_slot_uv_channel
  - zircon_plugins/gltf_importer/runtime/src/tests.rs::importer_preserves_gltf_texture_transform_on_standard_material_slot
  - cargo test -p zircon_runtime --lib importer_preserves_gltf_texcoord_1_on_model_vertices_and_mesh_subasset --locked --jobs 1 --target-dir E:\cargo-targets\zircon-render-main-chain --message-format short --color never -- --test-threads=1 --nocapture (2026-06-08 glTF TEXCOORD_1 and material texCoord ingress: passed, 1 passed / 0 failed / 3161 filtered; existing zircon_runtime lib-test warnings only)
  ...
  - cargo test -p zircon_runtime --lib asset::tests::assets::gltf_importer --locked --jobs 1 --target-dir D:\cargo-targets\zircon-asset-test-splits-0605 --message-format short --color never -- --test-threads=1 --nocapture (2026-06-05 glTF importer test split: passed, 8 passed; existing zircon_runtime lib-test warnings only)
  ...
  - zircon_runtime/src/asset/assets/texture/upload_support/tests.rs::rgba8_upload_readiness_accepts_complete_mip_chain_payloads
...
Renderer preparation consumes the same query. `resolve_texture_reference_with_support(...)` uses the current device-derived support to decide whether a referenced texture can be used directly or should emit `TextureNotUploadReady` and fall back. `GpuTextureResource::from_asset(...)` now returns a `Result`, uploads RGBA8 through the descriptor sampler and the uncompressed upload plan format, and maps upload-ready 2D DDS BC1/BC2/BC3/BC4/BC5/BC6H/BC7, including legacy DDS BC4/BC5 FourCC forms and non-typeless DXGI variants, ASTC, KTX1 BC1-7/ETC2/ASTC, and KTX2 BC1-7/ETC2/ASTC payloads to `wgpu::TextureFormat` before writing only the declared mip-zero container payload to the GPU. Decoded RGBA8 assets use `Rgba8Unorm` for `rgba8unorm` plans and `Rgba8UnormSrgb` for `rgba8unorm_srgb` plans; other descriptor formats should already be rejected by `upload_readiness(...)`, and the GPU helper keeps a conservative sRGB fallback for impossible direct calls. The created WGPU texture now derives its usage from `RenderImageDescriptor.usage`: `Sampled` becomes `TEXTURE_BINDING`, `RenderTarget` becomes `RENDER_ATTACHMENT` only for renderable RGBA8 formats, `CopySrc`/`CopyDst` become copy usages, and `Storage` becomes `STORAGE_BINDING` only for storage-capable formats. Because this residency path writes source payload bytes into the texture, it also adds `COPY_DST` for the upload itself even when the authored usage list omits `CopyDst`. RGBA8 and compressed texture upload now share `sampler_descriptor(...)`, so imported sampler filter and address-mode settings reach renderer preparation for both decoded image and container paths. This keeps importer diagnostics, asset readiness, render-target metadata, and renderer fallback behavior aligned around the same support decision while preventing typeless DDS DX10 ids, non-renderable compressed formats, or storage-incompatible formats from being uploaded with invalid WGPU usage policy.
...
and skinned primitive writers in `gltf_primitive_fixtures.rs`. `importer.rs` keeps the generic
registry, suffix-routing, OBJ, TOML model, physics, animation, shader, UI, and data importer
coverage, while glTF label, external-texture, missing-buffer, primitive-mode, morph, and skinning
regressions are owned by the glTF-specific module. This mirrors the existing OBJ and
texture-readiness splits and keeps future model-subasset regressions from re-growing the generic
importer file.

The same glTF importer path now preserves a second texture-coordinate channel. `import_gltf.rs`
reads `TEXCOORD_0` and `TEXCOORD_1`, `primitive_from_indexed_mesh(...)` maps them onto
`MeshVertex.uv` and `MeshVertex.uv1`, and `MeshAsset::from_model_primitive(...)` emits the
`uv1` named attribute only when imported vertices contain non-default second-channel data. The
`importer_preserves_gltf_texcoord_1_on_model_vertices_and_mesh_subasset` regression uses a minimal
glTF triangle fixture to assert both the root `ModelPrimitiveAsset` vertices and labeled
`Mesh0/Primitive0` subasset carry `MESH_ATTRIBUTE_UV0` and `MESH_ATTRIBUTE_UV1`.

glTF material imports also preserve `textureInfo.texCoord` and `KHR_texture_transform` metadata for
the fixed PBR texture slots. Both the built-in runtime importer and
`zircon_plugin_gltf_importer_runtime` read the glTF texcoord selector for base-color, normal,
metallic-roughness, occlusion, and emissive textures, then store it on the matching
`MaterialTextureSlotValue.uv_channel`; a `KHR_texture_transform.texCoord` value overrides the base
texture-info channel when present. The same import path stores non-identity extension `scale` and
`offset` values on `MaterialTextureSlotValue.transform`, so
`MaterialAsset::standard_material_descriptor(...)` projects them into the sibling standard texture
transform fields. The legacy material texture fields still carry the same texture locator for
renderer compatibility. This closes the asset ingress side needed by standard material
`uv_channel = 1` and scale/offset transforms; extension `rotation`, higher texture-coordinate sets,
custom shader reflection over arbitrary UV channels, and editor UV-channel authoring UI remain future
work.

The split glTF runtime importer module was validated on the current 2026-06-05 tree with `cargo test -p zircon_runtime --lib asset::tests::assets::gltf_importer --locked --jobs 1 --target-dir D:\cargo-targets\zircon-asset-test-splits-0605 --message-format short --color never -- --test-threads=1 --nocapture`; all 8 glTF importer tests passed. The generic importer file remains covered by static source guards that reject glTF fixture writers and glTF-specific importer regressions returning to `importer.rs`. A later 2026-06-05 slice changed the same glTF-specific module and split glTF plugin to turn `Skin{n}` and `Skin{n}/InverseBindMatrices` from diagnostic text placeholders into JSON `DataAsset` rows: the skin row records joint `Node{n}` locators, skeleton metadata, matrix counts, and the matrix subasset locator, while the matrix row carries the extracted inverse-bind matrices. `Animation{n}` rows intentionally remain diagnostic placeholders until animation-channel import lands. That Skin JSON slice was then validated with `cargo test -p zircon_runtime --lib asset::tests::assets::gltf_importer --locked --jobs 1 --target-dir D:\cargo-targets\zircon-gltf-skin-json-0605 --message-format short --color never -- --test-threads=1 --nocapture` and `cargo test --manifest-path zircon_plugins/Cargo.toml -p zircon_plugin_gltf_importer_runtime --lib --locked --jobs 1 --target-dir D:\cargo-targets\zircon-gltf-skin-json-0605 --message-format short --color never -- --test-threads=1 --nocapture`; both passed with 8 tests.
...
Conventional mesh LOD uses the same flat extraction rule. `SceneMeshInstanceAsset.lods` stores distance-threshold levels with model, optional direct mesh, material, and optional primitive mesh/material bindings; `World::from_scene_asset(...)` maps those rows to `MeshRenderer.lods`. For each scene camera, render extraction selects the highest finite `min_distance` level that is within the camera-to-mesh distance and then emits the selected source as ordinary `RenderMeshSnapshot` rows. Base selections keep `RenderMeshSnapshot.mesh_lod` empty; selected LOD rows carry `RenderMeshLodSelection { level_index, min_distance }`, where `level_index` is the authored LOD vector index. Resource streaming therefore continues to prepare either the selected direct mesh or selected model path without a separate LOD DTO, while submit-side mesh queue diagnostics can count selected conventional LOD draws via `render.mesh.queue.lod_draw_count`. This is distance-based conventional mesh LOD only; screen-error selection, LOD group assets, cross-fade, streaming residency, editor authoring UI, and Virtual Geometry/Nanite coordination remain separate follow-ups.
...
Material direct dependencies include the shader plus every concrete texture-slot reference.
Fallback-only texture slots remain authoring/runtime fallback data and are omitted from dependency
locators until a real texture reference is authored. Texture-slot metadata can also carry an
optional transform table with `scale` and `offset`; that metadata is serialized with `.zmaterial`
slot data or imported from glTF `KHR_texture_transform`, but it does not create a dependency on its
own and defaults to identity when omitted.
...
texture, transform, UV-channel, and fallback state by the same asset locators used by authored
`.zmaterial` sources. glTF
mesh primitive subassets also carry morph target displacement maps and skin inverse bind matrices.
The standalone glTF skin labels are now machine-readable data assets rather than empty diagnostics:
`Skin{n}` records joint labels and matrix linkage, and `Skin{n}/InverseBindMatrices` records the
extracted bind-pose matrices. Direct mesh CPU skinning now consumes joint index/weight attributes
when the frame extract provides a matching skeleton pose; explicit morph-weight conversion exists at
the mesh asset boundary, but scene/animation producer coverage for animation-channel driven weights
and typed skin asset consumption remains later work.

Material/schema mismatches are diagnostics, not import blockers. `MaterialAsset::shader_contract_diagnostics(...)` compares `[overrides]` and `[textures.<slot>]` against the loaded `ShaderAsset` contract and emits typed `RenderMaterialValidationError` values for unknown overrides, override type mismatches, missing required shader properties, unknown texture slots, and missing required shader texture-slot references. Shader texture slots can declare `required = true`; a material must provide a concrete `{ uuid, url }` texture reference for that slot to satisfy the contract. Material-owned overrides such as `lighting_model`, `cast_shadows`, and `receive_shadows` are filtered out of shader-property override projection; the two shadow toggles additionally report `PropertyOverrideTypeMismatch` diagnostics at `overrides.cast_shadows` or `overrides.receive_shadows` when the authored value is not boolean, while runtime policy falls back to true. Fallback-only texture slots remain non-dependency data and still help authoring/runtime fallback, but they are now reported as missing when the shader says the slot is required. Standard renderer aliases such as `base_color`, `base_color_texture`, `albedo`, `diffuse`, `normal`, `metallic_roughness`, `occlusion`, and `emissive` are filtered out of unknown custom texture-slot diagnostics when they are being used only to carry fixed PBR texture references or transform metadata. `MaterialAsset::readiness_report_with_shader_contract(...)` also consumes `ShaderAsset::readiness_report()`: missing runtime WGSL becomes the existing blocking material shader-source row, WGSL capture strings stay WGSL-capture diagnostics, and invalid entry stages or shader definition rows are preserved as shader-readiness material diagnostics with stable paths. Material-local `validation_diagnostics` are carried as non-blocking readiness diagnostics under `material.validation_diagnostics[N]`, so imported glTF/default material notes remain visible without forcing fallback or blocking readiness.

`MaterialAsset::standard_material_descriptor_for_shader(...)` is the renderer bridge for shader-driven texture slots. It starts from the legacy PBR descriptor for compatibility, then lets shader-declared texture slot aliases such as `base_color`, `base_color_texture`, `albedo`, `diffuse`, `normal`, `normal_texture`, `metallic_roughness`, `metallic_roughness_texture`, `occlusion`, `occlusion_texture`, `emissive`, and `emissive_texture` override the fixed PBR texture fields and their optional `RenderMaterialTextureTransform` metadata. After promotion it rebuilds the descriptor dependency set from the standard texture fields, so stale legacy texture references that were shadowed by shader aliases do not keep participating in shader-aware readiness. The legacy `standard_material_descriptor()` path remains available for callers without a loaded shader contract, but renderer material preparation now prefers the shader contract when it can load one. `ResourceStreamer::ensure_material(...)` also classifies those aliases as standard after promotion, so they are not duplicated in `MaterialRuntime.non_standard_texture_slots`; `render_product_streamer_bridges_shader_standard_texture_alias_into_pbr_slot` covers the ready runtime case where shader-owned `albedo` overrides a pre-existing legacy base-color texture, `render_product_streamer_shader_standard_alias_shadows_unresolved_legacy_texture` covers the shadowed unresolved-legacy dependency case, and `shader_declared_texture_slot_overrides_standard_material_bridge` now proves transform metadata follows the alias override.
