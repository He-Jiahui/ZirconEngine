...
`asset/reference_resolver.rs` is the single owner for GUID, path-hint, and labeled-subasset resolution used by importers. It validates the project path against configured roots, resolves the GUID first, and accepts a path difference only as a `PathHint` observation for that same GUID. A missing GUID with an occupied path hint is `PathOccupiedCandidate`, and a GUID/subasset mismatch is `Conflict`; neither case falls back to a base asset, replaces a GUID, or deletes a subasset label.

The importer layer must not catch `Dangling` to retry without a label, add registry aliases, or silently delete subasset identity. Importers only publish the safe path-hint observation in `AssetImportOutcome`; a later explicit mutation/fix-up transaction owns any authoring rewrite. Regression coverage is kept with the resolver, importer outcome, registry path-reuse, and Runtime87 source-contract owners.
...
The importer also exposes `import_context(&AssetImportContext)` so project scanning can pass the same source-byte owner to the selected plugin and post-import staging without cloning large HDR files. After a successful first import, and again after restoring a ready cached asset, the scan invokes the image and source-cubemap staging owners. `stage_environment_ibl_source(...)` converts HDR/EXR 2:1 images; `stage_source_cubemap_texture(...)` accepts captured source-only `.zcube` plus supported linear-float DDS/KTX payloads. Both produce or validate current `.zcube` source mips and a companion PMREM/SH9/IEM `.zribl` under `.zircon/cache`. Captured `.zcube` uses a header-only classification and skips f32 decode/PMREM work on a current paired-cache hit; cache misses rebuild asset-derived PMREM/SH9 through the same CPU recipe. External source mips are preserved for display/FIS input but PMREM is always regenerated. This post-import host step intentionally leaves the plugin ABI neutral: texture plugins decode the ordinary asset/container, while the shared runtime importer owns render-derived files and their current algorithm key. Invalid explicit environment settings and unsupported compressed/supercompressed cubemap formats are reported through the normal failed-import diagnostic path; automatic HDR/EXR mode skips non-equirectangular images.
...
NativeDynamic importers use the `runtime.asset.importer.native` capability and the `asset.import/<importer_id>` command. The ABI payload is a `ZRIMP001` request envelope containing metadata JSON and raw source bytes. Native code returns a `ZRIMO002` response envelope with a neutral import DTO, dependency locators, optional schema migration reports, diagnostics, and a required typed `reference_repairs` list. That list publishes only safe path-hint resolution observations; it never authorizes a native importer to replace a stable GUID or rewrite a project source document. The host validates command status before reading a success payload, so denied, panic, and error statuses preserve native diagnostics even when no output payload is returned. Successful responses then validate importer id, output kind, and malformed buffers before mapping dependency locators, migration reports, diagnostics, and reference repair observations into `AssetImportOutcome` before artifact publication.
...
schema migration report propagation, required reference repair observation presence, and diagnostic
conversion without requiring a native DLL fixture.
...
importer id, parses JSON source bytes, and returns a `ZRIMO002` response carrying a neutral
`DataAsset`, a per-entry schema migration report, diagnostics, and an explicit reference repair
observation list.
