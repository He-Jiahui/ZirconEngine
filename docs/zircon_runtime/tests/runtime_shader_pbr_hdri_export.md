...
The 2026-07-15 current-source lifecycle correction keeps the viewer on the production manager boundary after `SceneRenderer` stopped accepting a concrete `Arc<ProjectAssetManager>`. `scene.rs` now owns a `CoreRuntime`, registers the real foundation, `TasksModule.descriptor()`, and asset descriptors, activates them in dependency order, resolves `ProjectAssetManagerAccess` from the runtime handle, and passes that access object into `SceneRenderer::new(...)`. The scene retains the runtime for the renderer lifetime so the manager's weak Core handle remains valid. A same-name generic descriptor, a direct concrete-manager constructor, and a viewer-only renderer fallback are all rejected because they would bypass the runtime module lifecycle exercised by normal clients.
...
## 2026-07-15 current-source interactive viewer delivery

- The final viewer lifecycle uses the real Foundation, Tasks, and Asset module descriptors and dependency-order activation, resolves `ProjectAssetManagerAccess` from its retained `CoreRuntime`, and constructs the production `SceneRenderer`. Struct field order drops the renderer before the runtime services. Executable source guards reject a direct default asset manager and test-only renderer construction.
- Managed Windows job `1b86e228d43f4fad9edd9f0ef69d48df` completed both the `zircon_app` build and tests. Its viewer binary passed 18/18 tests, including runtime/renderer drop order, real module lifecycle, and wheel zoom bounds. Independent review then restricted the lifecycle guard to source before `#[cfg(test)]`; the production-anchor-removal simulation is RED as required. Managed job `03c5072d8e5b4214b146fd620027324d` executed the corrected current source and passed the viewer binary 18/18. The 72,369,664-byte production EXE has SHA256 `F8EEAD721B125E9D4CAEF374E9A532F07EDE0909B4DCC724B173F3508994233A` and remains in the coordinator-managed external target.
- A fresh Lakes 2K DX12 source512/PMREM512 run stayed responsive for 26/26 five-second samples. A native wheel message changed 446,585 screenshot pixels (34.4932%, mean absolute RGB delta 8.4627); a 343-pixel left drag changed the title to yaw 120. The accepted 1296x999 image has SHA256 `90D45BD3256C323275BCF112551264DD26DBFEA7D5550F24C7685C7B1D3A1354` and visibly contains the continuous Lakes environment and matching enlarged mirror-sphere reflection.
- The fresh 48,462,499-byte DX12 capture has SHA256 `1F12B9B03C0E3C2B8D1ED5068868C3FD589DB76578F639E06E277C11ABDBD0BC`; one-loop local replay exits 0. Exact-name scans found no PNG or RDC copies under the repository `target` or managed Cargo target.
