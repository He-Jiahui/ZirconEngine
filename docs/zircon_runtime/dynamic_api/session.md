...
`dynamic_api::exports` owns the final panic containment layer for the exported C ABI. `zircon_runtime_get_api_v7` validates `ZrHostApiV1` before returning the static `ZrRuntimeApiV7` table, and performs that lookup through an inner helper wrapped by `catch_unwind`; an unexpected unwind during table acquisition returns a null table pointer instead of crossing the `extern "C"` boundary.

Every advertised `ZrRuntimeApiV7` function pointer points at an `_ffi` wrapper in `exports.rs`. The wrappers delegate normal validation and behavior to private Rust-ABI `dynamic_api::session` and `session::operation` owner functions, then translate any unexpected unwind into `ZrStatusCode::Panic` with the stable diagnostic `runtime dynamic API panic caught at FFI boundary`. This keeps version checks, handle checks, allocation release, project loading, event routing, capture, surface, profile-control, tick, host-request, plugin-event, operation, and world-sync semantics in their owners while keeping the ABI panic guard at the final dynamic-library edge.

`runtime_api_table_entries_are_panic_wrapped_at_ffi_boundary` is the focused API-table source guard for this split: it rejects direct function-table entries that bypass the wrappers, requires the shared panic translator, locks the null-return path for panics during `zircon_runtime_get_api_v7`, and rejects `extern "C"` declarations from the private session and operation owner functions so unwind containment remains catchable.
...
## Runtime API V7 Operation and World Sync Lifecycle
...
`unwatch_world`, and `drain_world_invalidations` are required members of `ZrRuntimeApiV7`. The
app loader validates the complete V7 group, including the session-bound `release_allocation`, and has no old-table fallback. Release acquires a dedicated lifecycle lease for the supplied session; the lease remains available in teardown-retry-pending state but excludes the active closing phase, so release participates in destroy quiescence without preventing cleanup after an outstanding-allocation rejection. Registry storage is removed only when that session matches the recorded owner; crossed-session attempts return `NotFound` and preserve allocation/census state. `session/ffi.rs` owns
...
`dynamic_runtime_api_boundary` tracks 59 source files, 12 function tables, the 25-field V7 table,
and `runtime_session_ffi_wrappers = 23/23`. The permanent guard remains
...
The `dynamic_runtime_api_boundary` structural mirror ties this module's Runtime 10 evidence to the ABI inventory, FFI wrapper table, allocation registry, loader failure paths, behavior tests, runtime diagnostics, and host-request payload chain. Current V7 output reports `expected_source_file_count = 60`, `function_table_structs = 12/12`, `field_count_mismatches = 0`, `missing_repr_c_tables = 0`, `runtime_session_ffi_wrappers = 23/23`, `direct_session_table_entry_bypasses = 0`, `session_owner_extern_c_present = false`, `ffi_panic_anchors = 9/9`, and `risks = []`. `runtime_10_dynamic_runtime_api_mirror_docs_match_structure_audit_counts` keeps this module doc, the M0 review, runtime-interface convergence, and the cdylib loader doc aligned with those counts. Cargo evidence is recorded separately from this static structure result.
