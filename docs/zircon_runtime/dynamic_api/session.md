...
tests:
  ...
  - cargo test -p zircon_runtime --lib vampire_project_session --locked --jobs 1 --target-dir E:\cargo-targets\zircon-runtime-03-gates-0704 --message-format short --color never -- --test-threads=1: passed 2026-07-04 with 1 passed / 10 ignored after all real-backend Vampire project-session behavior tests were marked as requiring `zr-vm-real-backend`; the static WASD source check still runs by default
