...
`TextStyle.font` and `TextStyle.font_family` are separate selection identities. `font` is the exact registered FontObject/asset owner; `font_family` is an optional typeface/family selector inside that owner. The owner supplies ordered faces, its CompositeFont, and its fallback declarations to shaping and line-metric certification. Shaped/fallback cache keys retain the owner separately from the family, and owner mapping changes advance the shared font generation even when source bytes remain deduplicated. An asset URI must never be queried or registered as a global family alias. If an explicit FontObject is unavailable, its local typeface is removed from the effective query before the project/runtime default chain runs; a global face with the same name cannot impersonate the missing object. For registered owners, a request typeface remains owner-local even when absent; only CompositeFont or fallback declarations grant owner-then-global family lookup.
...
- `TextStyle::font` identifies a FontObject owner while `font_family` selects a typeface within that owner. Registered owners retain a generation-local face slice and compiled CompositeFont index shared by shaping, line metrics, and SDF recovery. A requested typeface is owner-local; only authored CompositeFont, asset fallback, or base/platform fallback entries authorize owner-then-global lookup. An unavailable owner discards its local family before entering the project/runtime default chain.
- Exhausted fallback selects the packaged generation-owned runtime last-resort face. Missing diagnostics keep the original codepoint, while neutral glyph publication and Native/SDF consumers retain the real face lineage; no source-derived synthetic glyph ID is allowed. Fixed-height metric certification includes that face.
