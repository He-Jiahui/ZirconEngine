...
tests:
  ...
  - cargo test -p zircon_runtime --lib wgpu_ui_surface_presenter_ignores_damage_for_direct_surface_redraw --locked
  ...
  - cargo test -p zircon_editor --lib gpu_presenter_damage_present_submits_full_direct_surface_draw_list --locked --jobs 1
...
The native renderer no longer preserves damage patches with a retained UI texture. A direct swapchain image cannot rely on the previous frame's contents, so the GPU presenter treats `damage` as invalidation and diagnostics metadata only. Even when the host reports region damage, the editor GPU presenter builds a full command stream, runtime clears the acquired surface on the first pass, and later solid/image/text batch passes load from that same in-flight surface view. Softbuffer can still use region damage for CPU copy efficiency; the native WGPU path prioritizes complete direct presents and color-consistent output.
...
- `cargo test -p zircon_runtime --lib ui_surface --locked` covers draw-list stats, descriptor validation, headless presenter stats, clip geometry trimming, legacy damage/clip geometry helpers, border geometry expansion, image UV clipping, partial-order batch layering, same-resource image grouping, text batching, text bounds, direct-surface damage ignoring, and GPU upload stats.
...
- `cargo test -p zircon_editor --lib gpu_presenter --locked` covers RHI failure propagation, upload/draw counters, damage diagnostics, full direct-surface draw-list submission for damaged presents, and corner-radius transfer from chrome commands to runtime surface commands.
...
- The 2026-05-18 direct-screen closeout removed the retained offscreen UI target and final blit from the native WGPU UI surface. Focused validation passed `cargo check -p zircon_runtime --lib --locked`, `cargo check -p zircon_editor --lib --locked`, `cargo test -p zircon_runtime --lib ui_surface --locked --jobs 1` (42 passed), `cargo test -p zircon_editor --lib gpu_presenter --locked --jobs 1` (3 passed), `cargo test -p zircon_editor --lib native_material_painter --locked --jobs 1` (5 passed), and, after updating a stale active render M7A test assertion to current generic graph execution stats, `cargo test -p zircon_runtime --lib material_button_style --locked --jobs 1` (4 passed). A direct `cargo test -p zircon_runtime --locked render_product_ui --jobs 1` run now compiles but still fails in the active M7A lane because `runtime-ui` references unregistered executor `ui.screen-space`; that product-pipeline executor wiring remains outside this direct-screen UI surface closeout. The fresh profiling editor build passed, then `20260518-004002-startup` recorded `software_fallback_present_count=0`, `gpu_draw_calls=39`, `gpu_visible_draw_items=225`, `batch_success_rate=0.827`, `hit_consistency_samples=39 failed=0`, and zero UI hotspot alerts. `20260518-004025-viewport_image` recorded `software_fallback_present_count=0`, `gpu_draw_calls=47`, `gpu_visible_draw_items=286`, `gpu_upload_bytes=1325080`, `batch_success_rate=0.836`, `hit_consistency_samples=93 failed=0`, zero UI hotspot alerts, and screenshot `differing_sample_ratio=0.0165`.
