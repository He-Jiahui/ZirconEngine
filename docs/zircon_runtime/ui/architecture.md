...
- `ui/` top-level entries: 20 = 15 directories plus `editable_text_composition.rs`, `module.rs`, `prelude.rs`, `public_runtime_frame.rs`, and `style.rs` (`mod.rs` is the root façade and is excluded from this audit count).
- `surface/` entries: 26 in the current worktree scan.
- Full UI-tree `legacy` hits: `ui_legacy_hits=70`; all current hits are confined to tests/fixtures, while production remains zero.
...
- current `surface/` scan is recorded as 26 entries after the Runtime Text leaf-owner additions and the retained control index, focus split, and invalidation owner additions;
