...
`pipeline_report` is the Bevy-aligned M7 schedule bridge. `UiSurfaceFrame` carries it as a serde-compatible `Arc<UiPipelineFrameReport>` with an independent pipeline domain generation, while `UiSurfaceDebugSnapshot` carries the owned wire DTO. A rebuild/report change replaces the shared report; focus/window-only frame publication reuses it. Its `frame_index` records the surface generation in which the report was actually built, not every later top-level state publication. Editor diagnostics can display the required `InputCollect -> Focus -> WidgetBehavior -> TextMeasure -> Layout -> PostLayout -> Picking -> A11yExtract -> RenderExtract -> BatchPrepare` order without depending on runtime-private rebuild fields. The current runtime projection is conservative: `UiSurfaceRebuildReport::pipeline_report(...)` fills measured timing/counters for layout, arranged post-layout, picking, and render extract; input/focus/widget/text/a11y/batch remain ordered skipped stages with notes until those owners expose direct timing.

`ecs_projection` is the M5 Bevy-like schedule visibility bridge. `UiSurfaceDebugSnapshot` carries a defaulted `UiEcsProjectionSnapshot` derived on demand from retained `UiSurface` nodes, dirty flags, component state, focus/capture/hover facts, render extract counts, hit-grid counts, and the shared effective-disabled gate. `UiSurfaceFrame` does not carry this retained diagnostic payload. Frame-only debug helpers leave it default-empty, while `UiSurface::debug_snapshot*` explicitly supplies the live projection. Diagnostics consume it as read-only evidence; edits still go through retained runtime mutation paths.
...
`debug_surface_frame_with_options(...)` builds the frame-only baseline snapshot. `debug_surface_frame_for_selection(...)` adds selected-node capture and selection overlay primitives. `debug_surface_frame_for_pick(...)` runs query-aware frame hit testing, stores the pick dump, selects the top hit, and emits hit-path/rejected-bounds overlay primitives. These free helpers have no retained tree/component source, so the snapshot's ECS projection remains default-empty. `UiSurface::debug_snapshot*` is the live diagnostics path: it explicitly builds the current ECS projection and passes it beside the same submitted frame.
...
Surface frame generation includes `UiPipelineFrameReport` generated from the latest `UiSurfaceRebuildReport`, but it does not eagerly generate `UiEcsProjectionSnapshot`. `debug_snapshot()` and `debug_snapshot_json()` request the current projection from the retained surface only when diagnostics are captured. Render-only rebuilds still report skipped layout/post-layout/picking stages while exposing render extract dirty reasons and command reuse/rebuild counters; ordinary pointer/focus/window publication no longer scans all retained nodes, render commands, and hit entries merely to keep a dormant diagnostic DTO current.
