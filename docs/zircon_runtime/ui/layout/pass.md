...
related_code:
  ...
  - zircon_runtime/src/ui/surface/slots.rs
...
plan_sources:
  ...
  - dev/UnrealEngine/Engine/Source/Runtime/SlateCore/Private/Widgets/SPanel.cpp
  ...
  - dev/UnrealEngine/Engine/Source/Runtime/UMG/Private/Components/CanvasPanelSlot.cpp
...
tests:
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/layout/pass/responsive_mui.rs zircon_runtime/src/tests/runtime_absorption/ui_architecture.rs (2026-06-16 Runtime 09 responsive MUI visibility flag rename: static verification; package behavior filters deferred)
  ...
  - CARGO_TARGET_DIR=D:\cargo-targets\zircon-layout-visual-profile-20260528b cargo build -p zircon_app --bin zircon_editor --profile profiling --features "target-editor-host profiling profiling-chrome" --locked (2026-05-28: passed, produced isolated profiling editor with existing warning noise)
  - CARGO_TARGET_DIR=D:\cargo-targets\zircon-layout-visual-profile-20260528b tools/ui-profile-capture.ps1 -ScenarioList drawer_resize -AutoInteract -RequireScenarioEvidence -CaptureSoftbufferScreenshot -AutoCloseSeconds 4 -OutputRoot target/zircon-profiles/layout-visual-20260527 -SkipBuild -ScreenshotDiffMaxAverageChannelDelta 12.0 (2026-05-28: passed after drawer_resize evidence compared the dragged splitter frame against after-geometry; artifacts under `20260528-073139-drawer_resize`; `resize_changed_layout=true`, `left_width=80.0`, `document_width=-80.0`, `software_fallback_present_count=0`, hit consistency `87 failed=0`, GPU-vs-softbuffer `differing_sample_ratio=0.0828`, `average_channel_delta=7.8908` under the explicit 12.0 interaction threshold)
  - cargo test -p zircon_runtime --lib taffy_layout_docs_keep_visual_profile_gate --locked --jobs 1 --target-dir D:\cargo-targets\zircon-layout-visual-doc-gate-20260527 --message-format short --color never (2026-05-28: passed, 1 passed; 0 failed; 2104 filtered out after final visual evidence and drawer_resize splitter-frame guard update)
  ...
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/surface/arranged.rs zircon_runtime/src/ui/tests/layout_slots.rs zircon_runtime/src/scene/ecs/query/query_state/mod.rs zircon_runtime/src/scene/ecs/query/query_mut_iter.rs zircon_runtime/src/scene/tests/ecs_query_structure.rs (2026-06-05 Overlay slot z_order arranged authority plus ECS compile unblock: passed)
  - git diff --check -- zircon_runtime/src/ui/surface/arranged.rs zircon_runtime/src/ui/tests/layout_slots.rs docs/zircon_runtime/ui/layout/pass.md zircon_runtime/src/scene/ecs/query/query_state/mod.rs zircon_runtime/src/scene/ecs/query/query_mut_iter.rs zircon_runtime/src/scene/tests/ecs_query_structure.rs docs/zircon_runtime/scene/ecs/query_state.md .codex/sessions/20260603-1955-host-editor-ui-foundation.md (2026-06-05 Overlay slot z_order arranged authority plus ECS compile unblock: passed with LF-to-CRLF warnings only)
  - cargo check -p zircon_runtime --lib --locked --jobs 1 --target-dir D:\cargo-targets\zircon-layout-slot-z-order-0605 --message-format short --color never (2026-06-05 Overlay slot z_order arranged authority: passed with existing warning noise only after the ECS QueryState cached-entity accessor compile unblock)
  - cargo test -p zircon_runtime --lib overlay_slot_geometry_feeds_arranged_render_hit_and_z_order_from_one_surface_frame --locked --jobs 1 --target-dir D:\cargo-targets\zircon-layout-slot-z-order-0605 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-05 warmed rerun passed, 1 passed; 0 failed; 2796 filtered out; first cold test build timed out during dependency compilation without Rust diagnostics or a produced test binary)
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/surface/slots.rs zircon_runtime/src/ui/surface/mod.rs zircon_runtime/src/ui/tests/mod.rs zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs (2026-06-05 Overlay slot z_order mutation dirty-domain slice: passed)
  - cargo check -p zircon_runtime --lib --locked --jobs 1 --target-dir D:\cargo-targets\zircon-slot-z-order-dirty-0605 --message-format short --color never (2026-06-05 Overlay slot z_order mutation dirty-domain slice: passed with existing warning noise only)
  - cargo test -p zircon_runtime --lib overlay_slot_z_order_mutation_rebuilds_arranged_hit_and_render_without_layout --locked --jobs 1 --target-dir D:\cargo-targets\zircon-slot-z-order-dirty-0605 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-05 Overlay slot z_order mutation dirty-domain slice: foreground command timed out while compiling without Rust diagnostics; warmed direct binary rerun `D:\cargo-targets\zircon-slot-z-order-dirty-0605\debug\deps\zircon_runtime-b34ee8d8fc52f1fd.exe overlay_slot_z_order_mutation_rebuilds_arranged_hit_and_render_without_layout --nocapture --test-threads=1` passed, 1 passed; 0 failed; 2800 filtered out)
  - git diff --check -- zircon_runtime/src/ui/surface/mod.rs zircon_runtime/src/ui/surface/slots.rs zircon_runtime/src/ui/tests/mod.rs zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs docs/zircon_runtime/ui/layout/pass.md .codex/sessions/20260603-1955-host-editor-ui-foundation.md (2026-06-05 Overlay slot z_order mutation dirty-domain slice: final verification rerun passed with LF-to-CRLF warnings only)
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/layout/pass/child_frame.rs zircon_runtime/src/ui/surface/slots.rs zircon_runtime/src/ui/tests/layout_slots.rs zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs (2026-06-07 Canvas/Free slot placement consumption slice: passed)
  - git diff --check -- zircon_runtime/src/ui/layout/pass/child_frame.rs zircon_runtime/src/ui/surface/slots.rs zircon_runtime/src/ui/tests/layout_slots.rs zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs (2026-06-07 Canvas/Free slot placement consumption slice: passed with LF-to-CRLF warnings only)
  - cargo test -p zircon_runtime --lib free_layout_consumes_canvas_slot_placement_before_child_default_anchor --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-slot-placement-layout --message-format short --color never -- --nocapture --test-threads=1 (2026-06-07 Canvas/Free slot placement consumption slice: timed out after 604 seconds while compiling dependencies without Rust diagnostics; matching cargo/rustc processes for this target were stopped, unrelated render-chain Cargo lane left running)
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-slot-coremin-0607 --message-format short --color never (2026-06-07 Canvas/Free slot placement consumption slice: passed with existing warning noise only)
  - cargo test -p zircon_runtime --lib free_layout_consumes_canvas_slot_placement_before_child_default_anchor --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-slot-coremin-0607 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-07 Canvas/Free slot placement consumption slice: timed out after 604 seconds before producing a `zircon_runtime` test binary; no Rust diagnostics were emitted)
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/surface/slots.rs zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs (2026-06-07 Canvas slot placement mutation dirty-domain continuation: passed)
  - cargo check -p zircon_runtime --lib --tests --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-slot-placement-mut-coremin-0607 --message-format short --color never (2026-06-07 Canvas slot placement mutation dirty-domain continuation: passed with existing warning noise only)
  - cargo test -p zircon_runtime --lib canvas_slot_canvas_placement_mutation_recomputes_layout_authority --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-slot-placement-mut-coremin-0607 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-07 Canvas slot placement mutation dirty-domain continuation: timed out after 904 seconds while compiling the runtime test binary without Rust diagnostics or a completed test result; matching Canvas validation cargo/rustc processes were stopped, unrelated render/graphics Cargo lane left running)
  - cargo test -p zircon_runtime --lib canvas_slot_layout --locked --jobs 1 --target-dir D:\cargo-targets\zircon-window-pump-runtime-0607 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-07 Canvas slot axis authority correction: passed, 4 passed; 0 failed; 3098 filtered out, with existing warning noise)
  - D:\cargo-targets\zircon-window-pump-runtime-0607\debug\deps\zircon_runtime-b34ee8d8fc52f1fd.exe surface_slot_dirty_domains --nocapture --test-threads=1 (2026-06-07 Canvas slot axis authority correction: passed, 4 passed; 0 failed; 3098 filtered out)
  - D:\cargo-targets\zircon-window-pump-runtime-0607\debug\deps\zircon_runtime-b34ee8d8fc52f1fd.exe layout_slots --nocapture --test-threads=1 (2026-06-07 Canvas slot axis authority correction: passed, 10 passed; 0 failed; 3092 filtered out)
  - D:\cargo-targets\zircon-window-pump-runtime-0607\debug\deps\zircon_runtime-b34ee8d8fc52f1fd.exe canvas_slot_template --nocapture --test-threads=1 (2026-06-07 Canvas slot axis authority correction: passed, 2 passed; 0 failed; 3100 filtered out)
  - rustfmt --edition 2021 --check zircon_runtime_interface/src/ui/layout/scroll.rs zircon_runtime/src/ui/layout/pass/slot.rs zircon_runtime_interface/src/ui/surface/arranged.rs zircon_runtime_interface/src/ui/surface/diagnostics.rs zircon_runtime_interface/src/ui/surface/mod.rs zircon_runtime/src/ui/surface/arranged.rs zircon_runtime/src/ui/surface/diagnostics.rs zircon_runtime/src/ui/tests/diagnostics.rs zircon_runtime_interface/src/tests/contracts.rs (2026-06-07 slot-derived surface diagnostics continuation: passed)
  - git diff --check -- zircon_runtime_interface/src/ui/layout/scroll.rs zircon_runtime/src/ui/layout/pass/slot.rs zircon_runtime_interface/src/ui/surface/arranged.rs zircon_runtime_interface/src/ui/surface/diagnostics.rs zircon_runtime_interface/src/ui/surface/mod.rs zircon_runtime/src/ui/surface/arranged.rs zircon_runtime/src/ui/surface/diagnostics.rs zircon_runtime/src/ui/tests/diagnostics.rs zircon_runtime_interface/src/tests/contracts.rs docs/zircon_runtime/ui/layout/pass.md .codex/sessions/20260603-1955-host-editor-ui-foundation.md (2026-06-07 slot-derived surface diagnostics continuation: passed with LF-to-CRLF warnings only)
  - cargo check -p zircon_runtime --lib --tests --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-slot-diagnostics-coremin-0607 --message-format short --color never (2026-06-07 slot-derived surface diagnostics continuation: passed with existing warning noise only)
  - cargo check -p zircon_runtime_interface --lib --tests --locked --jobs 1 --target-dir D:\cargo-targets\zircon-slot-diagnostics-interface-0607 --message-format short --color never (2026-06-07 slot-derived surface diagnostics continuation: passed after adding the new optional `slot` field to existing interface test fixtures)
  - rustfmt --edition 2021 --check zircon_runtime_interface/src/ui/surface/arranged.rs zircon_runtime_interface/src/ui/surface/diagnostics.rs zircon_runtime_interface/src/ui/surface/mod.rs zircon_runtime/src/ui/surface/arranged.rs zircon_runtime/src/ui/surface/diagnostics.rs zircon_runtime/src/ui/tests/canvas_slot_layout.rs zircon_runtime/src/ui/tests/diagnostics.rs zircon_runtime_interface/src/tests/contracts.rs zircon_editor/src/tests/host/pane_presentation.rs zircon_editor/src/tests/host/template_runtime/pane_payload_projection.rs zircon_editor/src/ui/workbench/debug_reflector/tests.rs (2026-06-07 Canvas layer grouping metadata continuation: passed)
  - cargo check -p zircon_runtime_interface --lib --tests --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-layer-groups-interface-0607 --message-format short --color never (2026-06-07 Canvas layer grouping metadata continuation: passed)
  - cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-layer-groups-runtime-lib-0607 --message-format short --color never (2026-06-07 Canvas layer grouping metadata continuation: passed with existing warning noise only)
  - cargo test -p zircon_runtime --lib canvas_container_groups_same_z_order_children_into_one_layer --no-default-features --features core-min --locked --jobs 1 --target-dir D:\cargo-targets\zircon-canvas-layer-groups-runtime-0607 --message-format short --color never -- --nocapture --test-threads=1 (2026-06-07 Canvas layer grouping metadata continuation: timed out after 1204 seconds while compiling the runtime test target without Rust diagnostics or a produced `zircon_runtime` test binary; matching target Cargo/rustc processes were stopped)
  ...
  - Python tomllib static validation for zircon_editor/assets/ui/editor/material_components/layout/material_masonry.zui and zircon_editor/assets/ui/theme/editor_material.zui (2026-05-25: passed; sample_children=8, columns=4, gap=8.0, sequential=true, `.MuiMasonry-item` and `.material-masonry-tile` selectors present)
  ...
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/layout/pass/taffy_arrange.rs (2026-06-04: passed after Flow slot hard-cutover wording cleanup)
...
## Runtime 09 M2.1 Taffy Bridge Authority

runtime_09_m2_1_taffy_bridge_pass_order_static_passed_cargo_pending

Runtime 09 M2.1 makes `zircon_runtime/src/ui/layout/taffy_bridge/compute.rs` the sole Taffy tree-build and compute owner. `compute_taffy_child_frames(...)` owns `TaffyTree::new()`, Taffy node construction, and `compute_layout`; `layout/pass/taffy_arrange.rs` now only prepares eligibility/fallback/reporting input and applies the returned child frames.

`UI_LAYOUT_PASS_ORDER` in `layout/pass/pipeline.rs` is the authoritative pass order consumed by full and incremental layout. `runtime_09_m2_1_style_mapping_remains_taffy_dto_adapter` records that `layout/style_mapping.rs` may keep Taffy DTO conversion helpers, but it is not a backend executor or pass sequencer.

## Runtime 09 M2.2 Scroll Virtualization Authority

runtime_09_m2_2_virtualization_scroll_boundary_static_passed_cargo_pending

Runtime 09 M2.2 makes `layout/scroll.rs::UiScrollVirtualizationPlan` and `plan_scrollable_virtual_window(...)` the owner for scroll offset clamping, viewport/content extent propagation, and visible-range invalidation. `layout/virtualization.rs::compute_virtual_list_window(...)` stays pure window math; it does not mark dirty state or own layout-cache writes.

The invalidation timing is:

- Scroll offset changes enter through `tree/node/scroll.rs::set_scroll_offset(...)`, consume the planner, update `UiScrollState`, mark layout/hit/render/input dirty, and OR `visible_range` dirtiness so a prior data or viewport invalidation is not cleared.
...
- `linear_sizing`: consumed by linear panels and mapped into the Taffy flex path. `Auto` stays content/preferred sized, `Stretch` grows by slot value from a zero/min basis, and `StretchContent` grows from content/preferred basis. The Zircon axis solver also consumes the same slot sizing so fallback and Taffy paths keep the same main-axis intent for the supported growth cases. `WrapBox`/`FlowBox` slots deliberately ignore this field; Flow slots are the current contract for order, padding, and alignment, not a flex-growth contract.
- `canvas_placement`: consumed for Free/Canvas-like parents before child-owned anchor, pivot, and position fallback. This carries the Slate `SConstraintCanvas::FSlot` shape: minimum/maximum anchors, pivot/alignment, local position, offset left/top, offset right/bottom size, stretched anchor ranges, and auto-size desired extents.
...
- `z_order`: serialized and parsed for Overlay and Canvas slots, then promoted during `build_arranged_tree(...)` into the arranged node's effective z by adding it to the child node `z_index`. The retained `UiTreeNode.z_index` remains unchanged; render extract and hit testing consume `UiArrangedNode.z_index` so parent-owned layering reaches paint and picking through the same `UiSurfaceFrame`. `UiSurface::set_overlay_slot_z_order(...)` and `UiSurface::set_canvas_slot_z_order(...)` mutate this parent-owned value without reordering retained children, matching Slate-style slot-owned layering while preserving Zircon's retained tree order.
- `dirty_revision`: incremented when a slot mutation changes authored placement metadata, then exposed as mutation evidence without acting as a separate rebuild selector.
...
Overlay, scroll, virtualized list, popup-like, canvas/free, `SizeBox`, and editor docking semantics remain Zircon-owned. The bridge returns `None` for these families, and `UiLayoutEngineSelection` reports a Taffy-to-Zircon fallback with `ZirconOwnedSemantics` when Taffy is requested for those containers. Runtime `CanvasBox` authoring can compile to `UiContainerKind::Canvas`, while default free parents still report the `Free` family; Taffy-eligible containers with slot-authored canvas placement report the separate `SlotCanvasPlacement` fallback. `SizeBox` deliberately maps to the `Container` family but still requires Zircon semantics because its child frame is a contain-fit content rectangle, not a flex/grid track.
...
runtime_09_m1_2_layout_engine_backend_name_cutover_static_passed_cargo_pending records the public layout-engine backend name cutover. The interface contract now exposes `UiLayoutEngineBackend::Zircon`, `UiLayoutEngineCapability::zircon()`, and `zircon_selected_count`; `runtime_09_layout_engine_backend_name_cutover_reduces_ui_layout_debt` keeps the old `LegacyZircon`, `legacy_zircon`, and `legacy_selected_count` API names from returning. This is a hard cutover, not a compatibility alias layer.

`taffy_arrange.rs` is the first runtime pass integration point. `arrange.rs` asks it to solve `HorizontalBox`, `VerticalBox`, `WrapBox`, `GridBox`, and `BlockBox` before falling back to Zircon-owned arrange code. The bridge now uses the shared `UiLayoutEngineFamily::is_taffy_owned()` predicate and `UiLayoutEngineCapability::taffy_flex_grid_wrap_block()` contract, so the interface report and runtime style conversion share the same Flex/Grid/Wrap/Block family boundary instead of carrying two separate family lists. The helper accepts template metadata because it carries render/event descriptors. It now accepts linear slot sizing (`Auto`, `Stretch`, and `StretchContent`) only for true `HorizontalBox`/`VerticalBox` linear slots, grid placement, finite non-negative slot padding, grid fixed-axis alignment, and flex/wrap cross-axis alignment where the child constraint can preserve Zircon semantics. `BlockBox` consumes `Container` slots for ordering and finite non-negative padding, but it does not yet claim per-child alignment in the native Taffy path; non-default block slot alignment records `SlotFramePolicy` and uses the Zircon vertical block fallback. `WrapBox`/`FlowBox` keeps the current Zircon wrap contract: Flow slots can affect ordering, padding, and cross-axis alignment, but Flow slot `linear_sizing` is ignored instead of being treated as flex growth. It still rejects collapsed children, canvas placement, non-default child anchor/pivot/position, non-zero main-axis `AxisConstraint.priority`, non-finite parent frame or container style values, non-finite child constraint/desired-size values, non-finite consumed linear slot sizing values, negative or non-finite slot padding values, flex/wrap main-axis per-child `Center`/`End` alignment, block slot alignment, and cross-axis `Center`/`End` alignment when the child extent is not fixed, all with explicit fallback reasons (`UnsupportedChildVisibility`, `ChildPlacementPolicy`, `AxisConstraintPriority`, `InvalidLayoutValue`, `SlotFramePolicy`, `SlotCanvasPlacement`). This lets v2 template-authored component subtrees take the Bevy-style Taffy path while retaining Zircon ownership for absolute placement and parent-specific frame policies.
...
`BlockBox` is an explicit Taffy-owned block-flow panel. It uses `Container` slots for parent-owned ordering and padding, measures content as a vertical stack with zero block gap, and arranges visible children through Taffy `Display::Block` when the slot/frame contract stays inside the supported subset. If Taffy rejects the block contract, the Zircon fallback keeps the same `Container` slot kind and stacks visible children vertically from their desired outer height, so fallback does not accidentally look for `VerticalBox` linear slots.
...
Runtime layout invalidation uses structured dirty domains. `mark_layout_dirty(...)` bubbles layout invalidation through content-driven or auto-layout ancestors, marking layout, hit-test, and render dirty on affected nodes without setting the legacy `state_flags.dirty` compatibility bit. This keeps `UiSurface::dirty_flags()` diagnostics precise and avoids reporting input dirtiness for pure layout changes. Overlay and Canvas slot `z_order` mutation uses the narrower `hit_test` plus `render` dirty domains because geometry and focus/input state do not change; `rebuild_dirty(...)` still refreshes the arranged tree, hit grid, and render extract from the same `UiSurfaceFrame` authority.

`UiSurfaceFrame.arranged_tree` now carries an optional `UiArrangedSlotSummary` for each arranged node whose parent container has a runtime child slot kind. The summary copies the parent-owned slot payload that Slate would keep in `SPanel` slot objects: kind, padding, alignment, linear sizing, canvas placement, grid placement, order, z-order, and dirty revision. `UiContainerKind::child_slot_kind()` is the shared container-to-slot mapping used by both the layout pass and arranged-tree provenance, so diagnostics cannot drift from the slot that actually drove frame placement or effective z-order. `UiWidgetReflectorNode.slot` mirrors that summary into debug snapshots and JSON export; it is optional and serde-defaulted for older snapshots.

## Shared Frame Contract

`zircon_runtime/src/ui/tests/layout_slots.rs` now covers four M1.3 focused authority cases:

- `overlay_slot_geometry_feeds_arranged_render_hit_and_z_order_from_one_surface_frame` arranges overlapping overlay children with slot padding/alignment, node z-index, and parent-owned slot `z_order`, then proves `UiSurfaceFrame.arranged_tree`, `render_extract`, `hit_grid`, and `hit_test_surface_frame(...)` agree on frame, clip, effective z-order, stacked hit order, and bubble route without mutating the retained node `z_index`.
...
`zircon_runtime/src/ui/tests/surface_slot_dirty_domains.rs` covers runtime slot mutation. It proves unchanged Overlay and Canvas slot `z_order` updates are no-ops, changed updates increment the slot revision and mark only hit-test/render dirty, and the next dirty rebuild refreshes arranged z-order, render z-order, hit-grid top-most order, and hit testing without a layout recompute or retained node `z_index` mutation. The same file also proves Free-slot and Canvas-slot canvas placement mutation dirties layout, hit, and render because geometry changes, then rebuilds the same arranged frame through render extract, hit grid, and hit testing.

`zircon_runtime/src/ui/tests/block_box_layout.rs` covers the explicit block container contract. It proves `BlockBox` selects the Taffy `Block` family natively, stacks visible fixed children through the shared `UiSurfaceFrame` authority consumed by render extract, hit-grid, and hit testing, keeps `Container` slots for template and v2 authoring (`BlockBox`/`Block`), maps finite slot padding on the native path, and records a `SlotFramePolicy` fallback when block slot alignment leaves the currently supported subset.

`zircon_runtime/src/ui/tests/taffy_layout_pass.rs` now covers the runtime Taffy route contract. It proves simple flex/wrap/grid containers select Taffy natively, free/canvas-like, generic container, space, overlay, scrollable, virtual-list, and `SizeBox` containers report Zircon-owned fallback, flex main-axis alignment, non-fixed cross-axis alignment, GridBox non-fixed alignment, negative/non-finite slot padding values, collapsed children, non-default child anchor/position, non-zero main-axis constraint priority, non-finite container config, non-finite axis constraints, non-finite consumed linear slot sizing, and canvas placement report explicit fallback reasons, grid placement/span plus out-of-bounds track expansion and `Auto`/`Stretch`/`StretchContent` linear slot sizing with min/max bounds can remain on the Taffy-native path, linear slot padding plus fixed-size cross-axis alignment can stay Taffy-native for both horizontal and vertical boxes, wrap slot padding plus fixed-size cross-axis alignment can stay Taffy-native, Flow slot `linear_sizing` is ignored by WrapBox without forcing a Zircon fallback, grid slot padding/alignment can stay Taffy-native for fixed-size cells, and measured text/image desired sizes from the measure pass feed Taffy-native child frames. `zircon_runtime/src/ui/tests/taffy_layout_diagnostics.rs` keeps the new tree-build metric coverage out of the already-large route file and proves a two-child Taffy-native flex pass reports one transient tree and three Taffy nodes. The route tests also guard the current Block boundary: runtime arrange routing must explicitly include `UiContainerKind::BlockBox` while generic `Container` remains outside the native Taffy arrange arm.
...
`zircon_runtime/src/ui/tests/diagnostics.rs` covers JSON/debug snapshot export for both sides of the route report. The Taffy-native snapshot test proves `layout_engine_report` survives JSON roundtrip with a native Taffy backend; the Zircon fallback snapshot test proves `zircon`, `fallback`, and `zircon_owned_semantics` are serialized and deserialize back to the same `UiLayoutEngineSelection` for a `SizeBox` root.

The same diagnostics suite now covers slot-derived reflector provenance with `surface_debug_snapshot_reports_parent_owned_canvas_slot_provenance`. It builds a Canvas parent whose child frame and effective z-order come from a parent-owned Canvas slot, then proves the reflected `UiWidgetReflectorNode.slot` exposes the Canvas slot kind, parent/child ids, order, z-order, dirty revision, anchor, pivot, position, offset, and auto-size state that produced the frame. It also asserts `UiSurfaceDebugSnapshot.canvas_layers` mirrors the Slate-style layer grouping for the same Canvas parent.
...
- Overlay slot `z_order` now reaches arranged/render/hit authority for direct overlay children, and runtime `set_overlay_slot_z_order(...)` mutation marks the narrower hit-test/render dirty domains without layout or input invalidation. Later slot-mutation APIs still need separate contracts for other placement fields such as padding, alignment, and grid/canvas metadata.
- `canvas_placement` now has runtime Free/Canvas-panel consumption: parent-owned anchors, stretched anchor ranges, pivot, position, offset size, and auto-size desired extents can replace child-owned anchor/pivot/position where intended. Canvas slot `z_order` also reaches arranged/render/hit authority through `set_canvas_slot_z_order(...)`, `UiWidgetReflectorNode.slot` exposes the parent-owned slot payload that produced arranged frame/z provenance, and `UiCanvasLayerGroup` records Slate-style same-z diagnostic layer groups. Later Canvas parity still needs renderer batching or backend `LayerId` consumption if the renderer should batch by those groups.
...
- `BlockBox` now exists as the explicit Taffy-owned block container, and generic `Container` remains Zircon-owned. Later block work should define richer block style semantics only when needed, such as gaps, alignment, min-content behavior, or inline/flow variants.
...
The final rerun passed `1 passed; 0 failed; 1764 filtered out` after a `3m 06s` warmed rebuild. An earlier run failed because the test asserted compact JSON while `UiSurface::debug_snapshot_json(...)` intentionally uses pretty JSON; the assertion now matches the exported pretty JSON and still deserializes the snapshot to verify the structured `Zircon`/`Fallback`/`ZirconOwnedSemantics` selection. The passing run used `--locked` and reported existing `zircon_runtime` warning noise.
...
The 2026-06-07 Canvas/Free slot placement continuation consumes the preserved `UiSlot.canvas_placement` payload inside `free_child_frame(...)` before falling back to child node anchor/pivot/position. This follows Slate `SConstraintCanvas::OnArrangeChildren`: parent slot anchor, pivot/alignment, position, offset left/top, offset right/bottom size, auto-size desired extents, and stretched anchor pairs produce the child frame, and the same arranged frame feeds render extract and hit grid. The axis-size rule now distinguishes the two slot families at the shared helper: Free slots still route the slot extent through Zircon `arranged_axis_extent(...)`, so fixed child constraints can intentionally cap a Free placement axis; true Canvas slots make stretched axes and explicit nonzero fixed offset sizes slot-authoritative before child constraints, while zero-size/default offsets and auto-size axes still fall back to desired/constraint sizing. This keeps Free placement flexible for existing node-owned layouts while making Canvas slot offset size the arranged/render/hit authority expected from Slate and UMG Canvas slots. `UiSurface::set_free_slot_canvas_placement(...)` and `UiSurface::set_canvas_slot_canvas_placement(...)` mutate parent-owned placement payloads without rewriting the child node and mark layout, hit, and render dirty through the shared dirty-domain path; `surface_slot_dirty_domains.rs` now covers both Free slots and true Canvas slots for that mutation path. `UiSurface::set_canvas_slot_z_order(...)` mirrors Slate `SConstraintCanvas::FSlot::SetZOrder` for the retained runtime surface: it updates parent-owned Canvas layering, refreshes arranged/render/hit authority, and avoids a layout recompute because geometry does not change. `UiArrangedSlotSummary` and `UiWidgetReflectorNode.slot` now make the parent slot payload visible in surface diagnostics so a Widget Reflector view can explain which Slate-style slot produced the arranged frame and z-order. `UiCanvasLayerGroup` extends the arranged tree and debug snapshot with Slate `ArrangeLayeredChildren` evidence: render-visible Canvas children are sorted by slot `z_order`, paint order, and node id under each Canvas parent; adjacent children with the same `z_order` share one layer group, while a higher `z_order` starts the next group. This is runtime metadata for retained UI diagnostics and future renderer batching; it does not yet replace the renderer's command-level `z_index` split policy or expose backend `LayerId` values.

The 2026-06-07 Canvas measure compile unblock keeps `measure_content_size(...)` exhaustive after `UiContainerKind::Canvas` entered the runtime container set. Canvas now uses the same stacked desired-size aggregation as `Free`, `Container`, and `Overlay`, matching the existing arrange/slot treatment where Canvas owns absolute placement rather than linear/grid flow. This was required so render validation could compile the full runtime crate; it does not change the slot-placement authority described above.
...
The BlockBox/Container bridge boundary passed with:

```powershell
cargo test -p zircon_runtime --lib taffy_bridge_keeps_block_box_display_explicit_and_container_zircon_owned --locked --jobs 1 --target-dir D:\cargo-targets\zircon-layout-impl --message-format short --color never
cargo test -p zircon_runtime_interface ui_layout_engine_block_is_explicit_block_box_not_generic_container --locked --jobs 1 --target-dir D:\cargo-targets\zircon-layout-interface --message-format short --color never
```

The runtime bridge test passed `1 passed; 0 failed; 1723 filtered out` and originally proved the abstract `Block` display stayed separate from generic `Container`; the 2026-06-05 continuation updates that boundary so only explicit `UiContainerKind::BlockBox` emits `UiLayoutEngineFamily::Block`, while generic `Container` still returns no Taffy style and reports Zircon-owned fallback. The interface test now proves `BlockBox` selects Taffy natively and every non-block container, including `Container`, remains outside the Block family.

The 2026-05-23 runtime arrange boundary guard has been superseded by the 2026-06-05 `BlockBox` route. `layout_pass_routes_supported_containers_through_taffy_arrange` now requires the runtime pass to mention `UiContainerKind::BlockBox`, while still guarding that `Overlay`, `ScrollableBox`, `SizeBox`, and generic `Container` do not appear as native Taffy arrange arms.
