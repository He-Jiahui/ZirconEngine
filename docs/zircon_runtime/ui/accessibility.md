...
related_code:
  ...
  - zircon_runtime/src/ui/accessibility/action/activate/result.rs
  ...
  - zircon_runtime/src/ui/accessibility/action/focus/result.rs
  ...
  - zircon_runtime/src/ui/accessibility/action/target/result.rs
  ...
  - zircon_runtime/src/ui/accessibility/action/text/selection/result.rs
...
`action/text/selection/result.rs` owns missing `text_selection` rejection shaping and accepted `SetTextSelection` result shaping plus retained selection metadata synchronization. `action/text/selection.rs` stays focused on action exposure checks, TextInput role validation, payload parsing, and result delegation.

The current `SetTextSelection` payload boundary mirrors the other action splits: `action/text/selection/payload.rs` owns only neutral payload presence metadata and caret/anchor/focus UTF-8 boundary clamping. `action/text/selection/result.rs` maps missing payload metadata to `UiAccessibilityActionStatus::Rejected`, so action status vocabulary stays in result shaping rather than payload parsing.
...
`zircon_runtime/src/dynamic_api/exports.rs` exposes `ZrRuntimeApiV7.capture_accessibility_tree`. The function validates `ZrRuntimeAccessibilityTreeRequestV1.abi_version`, rejects non-default viewport handles with `NotFound`, captures the current dynamic preview accessibility snapshot, serializes it as JSON `UiAccessibilityTreeSnapshot`, and writes an immutable `ZrOwnedResultV2`.

Accessibility tree byte ownership mirrors frame byte ownership through the runtime allocation registry. Each non-empty result receives an opaque allocation id and is released only through the mandatory V7 `release_allocation(originating_session, allocation)` entry; the host cannot supply pointer, capacity, token, or allocator metadata. Null output pointers return `InvalidArgument` with the diagnostic `missing accessibility tree output`. Invalid result shapes are rejected by the shared host output policy, and duplicate, forged, or wrong-session allocation ids return `NotFound` without reaching the allocator or changing the owner census.
...
Accessibility popup early result helper split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/popup/result.rs` now owns popup `Dismiss` lookup-error rejection shaping and unsupported dismiss fallback shaping, in addition to the existing close-mutation result shaping helpers.
- `zircon_runtime/src/ui/accessibility/action/popup.rs` now stays focused on popup owner lookup, tooltip fallback routing, popup close mutation dispatch, and result-helper delegation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/popup.rs" "zircon_runtime/src/ui/accessibility/action/popup/result.rs" "zircon_runtime/src/ui/accessibility/action/popup/tooltip.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for popup dispatch, popup result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated Hub, material, ZUI, and texture Cargo/Rust compiler jobs active in the shared checkout.

Accessibility tooltip result helper split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/popup/result.rs` now owns accepted tooltip `Dismiss` result shaping, including routed diagnostics, accepted status note, handled phase, and `accessibility_tooltip_hidden` note.
- `zircon_runtime/src/ui/accessibility/action/popup/tooltip.rs` now stays focused on Tooltip role matching, runtime tooltip-state lookup, tooltip hide effect construction, dispatch-reply application, and result-helper delegation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/popup.rs" "zircon_runtime/src/ui/accessibility/action/popup/result.rs" "zircon_runtime/src/ui/accessibility/action/popup/tooltip.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for tooltip dispatch, popup result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated Hub, ZUI, and material Cargo/Rust compiler jobs active in the shared checkout.

Accessibility tooltip finish-handled reuse evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/popup/result.rs` now routes accepted tooltip `Dismiss` result shaping through shared `finish_handled`, so routed diagnostics, route target, handled phase, and accepted status note use the same helper as other accepted actions.
- Tooltip-specific evidence remains a narrow `accessibility_tooltip_hidden:<id>` diagnostic note appended after `finish_handled`.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/popup.rs" "zircon_runtime/src/ui/accessibility/action/popup/result.rs" "zircon_runtime/src/ui/accessibility/action/popup/tooltip.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for popup result helper, docs, plan, and active session files.
- No new Cargo check was started because process inspection showed unrelated Cargo/Rust compiler jobs active in the shared checkout.
...
Accessibility activate result submodule split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/activate/result.rs` now owns typed-widget handled report propagation, typed-widget mutation-error rejection, and accepted fallback result shaping for `Activate`.
- `zircon_runtime/src/ui/accessibility/action/activate.rs` now stays focused on `Activate` action exposure checks, typed widget activation dispatch, and fallback delegation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/activate.rs" "zircon_runtime/src/ui/accessibility/action/activate/fallback.rs" "zircon_runtime/src/ui/accessibility/action/activate/result.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for activate dispatch, activate result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated material, editor ZUI, sound, Hub screenshot, and other Cargo/Rust compiler jobs active in the shared checkout.

Accessibility focus result submodule split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/focus/result.rs` now owns accepted `Focus` result shaping and runtime focus rejection diagnostics.
- `zircon_runtime/src/ui/accessibility/action/focus.rs` now stays focused on `Focus` action exposure checks and programmatic focus API invocation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/focus.rs" "zircon_runtime/src/ui/accessibility/action/focus/result.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for focus dispatch, focus result helper, docs, plan, and active session files.
- No new Cargo check was started because process inspection showed unrelated ZUI, material, sound, and other Cargo/Rust compiler jobs active in the shared checkout.

Accessibility target result submodule split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/target/result.rs` now owns rejected/stale result shaping for hidden snapshot targets, disabled non-focus actions, stale runtime nodes, hidden tree targets, and excluded snapshot targets.
- `zircon_runtime/src/ui/accessibility/action/target.rs` now stays focused on included-target gates, missing-target classification, accessibility diagnostic note propagation, and effective hidden-tree traversal.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/target.rs" "zircon_runtime/src/ui/accessibility/action/target/result.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS after targeted `rustfmt`.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for target dispatch, target result helper, docs, plan, and active session files.
- No new Cargo check was started because process inspection showed unrelated ZUI, Hub, material, sound, and other Cargo/Rust compiler jobs active in the shared checkout.
...
Accessibility ReplaceSelectedText early result helper split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/text/replace/result.rs` now owns missing-value and read-only rejection shaping for `ReplaceSelectedText`, in addition to the existing mutation-result shaping helpers.
- `zircon_runtime/src/ui/accessibility/action/text/replace.rs` now stays focused on `ReplaceSelectedText` action exposure checks, TextInput/value-target gates, selected replacement construction, runtime property mutation request construction, and result-helper delegation.
...
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for replace dispatch, replace result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated Hub, material, and texture Cargo/Rust compiler jobs active in the shared checkout.
...
- At the payload split stage, `zircon_runtime/src/ui/accessibility/action/text/selection.rs` stayed focused on `SetTextSelection` action exposure checks, TextInput role validation, accepted result shaping, and retained selection metadata synchronization; the 2026-05-28 result split below moves that result/sync responsibility into `action/text/selection/result.rs`.
...
Accessibility SetTextSelection result submodule split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/text/selection/result.rs` now owns missing `text_selection` rejection shaping and accepted `SetTextSelection` result shaping plus retained selection metadata synchronization.
- `zircon_runtime/src/ui/accessibility/action/text/selection.rs` now stays focused on `SetTextSelection` action exposure checks, TextInput role validation, payload parsing, and result delegation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/text.rs" "zircon_runtime/src/ui/accessibility/action/text/selection.rs" "zircon_runtime/src/ui/accessibility/action/text/selection/payload.rs" "zircon_runtime/src/ui/accessibility/action/text/selection/result.rs" "zircon_runtime/src/ui/accessibility/action/text_state.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for selection dispatch, selection payload helper, selection result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated material, Hub, and ZUI Cargo/Rust compiler jobs active in the shared checkout.

Accessibility SetTextSelection payload status boundary evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/text/selection/payload.rs` no longer imports `UiAccessibilityActionStatus` or exports a missing-payload status constant; it keeps only missing-payload code/reason metadata and caret/anchor/focus boundary clamping.
- `zircon_runtime/src/ui/accessibility/action/text/selection/result.rs` now maps missing `text_selection` payloads to `UiAccessibilityActionStatus::Rejected`, keeping status vocabulary inside result shaping.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/text.rs" "zircon_runtime/src/ui/accessibility/action/text/selection.rs" "zircon_runtime/src/ui/accessibility/action/text/selection/payload.rs" "zircon_runtime/src/ui/accessibility/action/text/selection/result.rs" "zircon_runtime/src/ui/accessibility/action/text_state.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS after applying rustfmt to the focused selection files.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for selection payload, selection result helper, docs, plan, and active session files.
- No new Cargo check was started because process inspection showed unrelated Cargo/Rust compiler jobs active in the shared checkout.
...
Accessibility SetValue early result helper split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/value/result.rs` now owns missing-value rejection shaping and TextInput preparation rejection propagation for `SetValue`, in addition to the existing mutation-result shaping helpers.
- `zircon_runtime/src/ui/accessibility/action/value.rs` now stays focused on `SetValue` action exposure checks, role/property/payload gates, TextInput preparation delegation, runtime property mutation request construction, and result-helper delegation.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/value.rs" "zircon_runtime/src/ui/accessibility/action/value/result.rs" "zircon_runtime/src/ui/accessibility/action/value/text.rs" "zircon_runtime/src/ui/accessibility/action/value/payload.rs" "zircon_runtime/src/ui/accessibility/action/value_target.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for value dispatch, value result helper, docs, plan, and session archive files.
- No new Cargo check was started because process inspection showed unrelated Hub, ZUI, and sound Cargo/Rust compiler jobs active in the shared checkout.

Accessibility SetValue TextInput rejection status boundary evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/value/text.rs` no longer imports or stores `UiAccessibilityActionStatus`; TextInput preparation rejections now carry only the read-only code and reason.
- `zircon_runtime/src/ui/accessibility/action/value/result.rs` now maps TextInput preparation rejections to `UiAccessibilityActionStatus::Rejected`, keeping action status vocabulary inside result shaping.
- `rustfmt --edition 2021 --check "zircon_runtime/src/ui/accessibility/action.rs" "zircon_runtime/src/ui/accessibility/action/value.rs" "zircon_runtime/src/ui/accessibility/action/value/result.rs" "zircon_runtime/src/ui/accessibility/action/value/text.rs" "zircon_runtime/src/ui/accessibility/action/value/payload.rs" "zircon_runtime/src/ui/accessibility/action/value_target.rs" "zircon_runtime/src/ui/accessibility/action/result.rs"`: PASS after applying rustfmt to the focused value files.
- Scoped `git diff --check` for tracked touched files passed with LF/CRLF warnings only. Touched-file trailing-whitespace scan and conflict-marker scan passed for value TextInput preparation, value result helper, docs, plan, and active session files.
- No new Cargo check was started because process inspection showed unrelated Cargo/Rust compiler jobs active in the shared checkout.
...
Accessibility ScrollTo early result helper split evidence from 2026-05-28:

- `zircon_runtime/src/ui/accessibility/action/scroll/result.rs` now owns missing-value rejection shaping for `ScrollTo`, in addition to the existing mutation-result shaping helpers.
- `zircon_runtime/src/ui/accessibility/action/scroll.rs` now stays focused on `ScrollTo` action exposure checks, offset payload reuse, previous-offset capture, runtime scroll mutation dispatch, and result-helper delegation.
...
- No new Cargo check was started because process inspection showed unrelated material, Hub, ZUI, and sound Cargo/Rust compiler jobs active in the shared checkout.
