...
- `ids/plugin_id.rs` owns `RuntimePluginId`, including key, label, and parse behavior.
...
The 2026-06-07 root-facade cutover made `zircon_runtime::builtin` the direct public owner for assembly helpers such as `builtin_runtime_modules()` and `runtime_modules_for_target(...)`. The crate root keeps the narrow id/report DTO exports for now, but no longer forwards the assembly functions. App entry, dynamic API startup, runtime plugin-extension tests, and structural guards import the assembly helpers through `zircon_runtime::builtin` or `crate::builtin`.

The same M2 hard-cut pass tightened internal id/report ownership without changing the public DTO facade. Implementation files under `runtime_modules` now import `RuntimePluginId` and `RuntimeTargetMode` from the `ids` owner and import `RuntimeModuleLoadReport` from the `load_report` owner directly, instead of depending on parent `runtime_modules.rs` re-export wiring. Public callers may still use the curated `zircon_runtime::builtin` or crate-root DTO facade until the broader plugin-provider lane is ready for a public DTO-path contraction.

## Invariants

- Root `runtime_modules.rs` must stay structural: child module declarations, curated re-exports, and test module wiring only.
- `zircon_runtime::builtin` is the public namespace for runtime module assembly helpers. `zircon_runtime` crate root must not forward `builtin_runtime_modules()`, `runtime_modules_for_target(...)`, profile assembly helpers, or manifest assembly helpers.
