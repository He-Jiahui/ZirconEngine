...
The 2026-06-23 Plugins 12 M3/T1 importer-family cutover adds temporary closed-enum entries for `AssetImporterData`, `AssetImporterModel`, and `AssetImporterShader`. Their keys preserve the package ids `asset_importer.data`, `asset_importer.model`, and `asset_importer.shader`, and `plugin_modules/loader.rs` maps them to externalized warnings for `zircon_plugins/asset_importers/{data,model,shader}`. This is a scoped compatibility step for the trait-backed importer family descriptors; Plugins 12 M5/D6 still owns the real string-newtype or custom-id cutover so future plugins do not need core enum edits.

The same Plugins 12 M3/T1 split-importer follow-up adds the temporary `OpusImporter` closed-enum entry and externalized loader mapping for `zircon_plugins/opus_importer`. Existing split importer ids now point at trait-backed descriptors in their plugin crates, while D6 still owns replacing the closed enum with a string-newtype or custom-id surface.
