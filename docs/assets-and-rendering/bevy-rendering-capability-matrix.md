...
The accepted Zircon state remains intentionally narrower. `RenderStats` is a submit/product snapshot for capability gates, history validity, graph counts, product readiness/fallbacks, material/sprite/light stats, advanced provider reports, Solari status, and VG/HGI counters. `collect_runtime_diagnostics(...)` bridges frame/viewport rows, `render.capability.*`, `render.history.*`, the legacy `render.last_graph_executed_pass_count`, stable planned/executed `render.graph.*` count rows, `render.post_process.graph.*`, material/light/mesh/sprite/UI readiness, post-process effect-stack readiness, AA fallback state, GPU particle feedback, VG/HGI runtime counters, advanced provider degradation, and Solari status/degradation into the runtime `DiagnosticStore`. The profiling feature records CPU timeline spans for submit, present, capture, framework locking, and render graph stage/pass execution, and can export native/perfetto/hotspot artifacts.
