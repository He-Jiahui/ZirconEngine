...
2026-06-28 的 R6 跟进把已共享的 text measurement 继续推进到 shaped text DTO：`UiResolvedTextLine.glyph_advances` 保存 per-grapheme measured advances，`UiShapedText` 用这些 advances 生成非均匀 glyph frames 与 paint-run frames，`UiShapedGlyph` 预留 `font_id`、cluster flags 与 `None/Cw90` rotation 合同。该切片仍不是 HarfBuzz/cosmic-text 级完整 shaping；它关闭的是接口层和现有 layout projection 的等分行宽债务。
