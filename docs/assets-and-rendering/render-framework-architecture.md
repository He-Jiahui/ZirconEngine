...
- `zircon_runtime` 的 M6 sprite renderer 现在也有 renderer-side batching 输入与公开诊断：`prepare_sprite_draw_batches(...)` 在不重排 `SpriteExtract.phase_queue` 的前提下只合并相邻且纹理相同的 sprite，把多个 quad 追加到同一个 vertex buffer / render pass；`prepare_sprite_queue_stats(...)` 复用同一排序语义，统计 draw batch、batched sprite、vertex 与分 phase batch 数，并通过 `SceneRendererCompiledSceneOutputs` 写入 `RenderStats.last_sprite_*`，再由 `collect_runtime_diagnostics(...)` 投射为 `render.sprite.*` 与 `render.sprite.queue.*`。这样能减少最小 Core2d 路径的逐 sprite pass 开销，同时保留 z/order/layer 语义；非相邻同纹理 sprite 不会为了 batching 被跨越重排，完整 Bevy-like binned batching 和 per-view pipeline specialization 仍是后续要求。
...
- 2026-06-07 的 `MeshLod` follow-up 把普通 mesh LOD 也接入同一 M8 descriptor-first 主链。`BuiltinRenderFeature::MeshLod` 必须显式 feature opt-in，启用后只把 `mesh_lod` neutral extract section 纳入 compiled pipeline，不添加 pass、executor、history binding 或 backend capability。这个 token 只服务 renderer data source、profile gating 和后续普通 mesh-renderer LOD 专项接入点；距离/屏幕误差选择、LOD group 资产结构、mesh draw 替换、streaming residency、编辑器 authoring UI 以及与 Virtual Geometry/Nanite 路径的互斥/协作策略仍是后续工作。
