...
Execution is graph-owned. `RenderPassGpuExecutionContext` carries the renderer-owned `ScreenSpaceUiRenderer`, `RenderPassExecutorRegistry` registers `ui.screen-space`, and the executor records the existing screen-space UI renderer into the named final-color resource during graph execution. The previous manual late UI record call after overlay is no longer the product placement authority. `RenderStats` reports both shared extract payload counts and graph placement evidence: `last_ui_graph_executed_pass_count`, `last_ui_target_size`, and `last_ui_graph_pass_order`. The order field becomes `postprocess-ui-overlay` only when the real executed pass list contains `post-process`, `runtime-ui`, and `overlay-gizmo` in that order.
