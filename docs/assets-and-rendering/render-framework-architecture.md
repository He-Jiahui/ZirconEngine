...
RenderDoc 启动路径通过环境变量而不是新 editor/dynamic API 扩展 façade：`WGPU_BACKEND=dx12|vulkan` 进入 `RenderBackendConfig` 后约束 `wgpu::InstanceDescriptor.backends`，`WGPU_DEBUG=1` 和 `WGPU_VALIDATION=1` 映射到 `wgpu::InstanceFlags`，`ZR_RENDERDOC_CAPTURE_NEXT=1` 则把第一个创建的 viewport 预置成 pending capture。当前 live trigger surface 只有 `RenderFramework::request_graphics_debugger_capture(viewport)` 和 `ZR_RENDERDOC_CAPTURE_NEXT=1`。`GraphicsDebuggerStatus.backend_name` 记录实际 adapter backend，例如 `wgpu(dx12)` 或 `wgpu(vulkan)`，focused tests 在对应 `WGPU_BACKEND` 下断言 exact status string，方便手工捕获时确认 RenderDoc 正在观察预期 API。
