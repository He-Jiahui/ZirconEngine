...
M7A moves runtime screen-space UI placement under the product graph while preserving `UiRenderExtract` as the shared runtime/editor input contract. `BuiltinRenderFeature::Ui` now contributes a `runtime-ui` pass at `RenderPassStage::Ui` with executor id `ui.screen-space`, `QueueLane::Graphics`, a read of `PostProcessGraphResourceNames::FINAL_COLOR`, and a write to external `viewport-output`. The graph descriptor therefore states the intended product ordering directly: postprocess produces final color, runtime UI composites onto that target, and overlay/final presentation remains later in the pass list.
...
volume-stack coverage 也同步到这组字段：`volume_stack_resolves_extended_effect_stack_settings` 现在覆盖 vignette smoothness/roundness、film-grain response、dither scale、chromatic-aberration sample spread、fog density/height/color，确保 Unity SRP 风格 volume override path 解析出的 settings 与 final WGPU upload contract 使用同一组中立字段。
...
同一 M7 方向现在还接入了 Unity SRP 风格的中立 volume stack。`RenderPostProcessVolumeStack` 在 `PostProcessExtract` 中携带 global/local volumes、priority、weight、render-layer mask、local blend influence 与可选 bloom/color-grading/effect-stack profile override；`build_frame_submission_context(...)` 会按当前 camera render layers 调用 `resolved_settings_for_layers(...)`，先把 volume stack 解析为有效 per-camera settings，再执行 quality/profile feature gates 和 history key 构造。renderer frame 收到的是已解析设置与有效 graph，raw `volume_stack` 会被清空，避免后续 concrete shader path 绕过主链解析规则。local volume 的空间影响力由 scene extract 根据 scene camera、volume transform、same-entity `ColliderComponent` box/sphere/capsule 和 `blend_distance` 预计算为 `local_blend`；无 collider 或 blend band 外的 local volume 不进入提交栈。完整 project/editor authoring、trigger lifecycle 和序列化仍留给后续 scene-volume 子计划。
...
- `zircon_runtime` 的动态分辨率主链接口现在先固定为中立 DTO 与 graph resource extent 合同：`ViewportCameraSnapshot.dynamic_resolution` 保存 `RenderDynamicResolutionSettings`，`RenderViewExtract::effective_view_size()` 继续表示物理 viewport / present / UI damage 尺寸，`RenderViewExtract::effective_render_size()` 才应用 clamped render scale。`RenderPipelineAsset::compile(...)` 用 render size 派生 `scene-color`、`scene-depth` 等 transient texture/buffer descriptor，而 external `viewport-output` 仍保持 imported target，不被缩放。submit context/status 同步保留两套尺寸：`FrameSubmissionContext::size()` / `RenderStats.last_frame_target_size` 是呈现目标尺寸，`FrameSubmissionContext::render_size()` / `RenderStats.last_frame_render_size` 是内部 graph 尺寸；runtime UI 也保持在 presentation target 侧，新的 submitted-frame coverage 要求动态分辨率下 `last_ui_target_size` 仍等于 unscaled viewport，并保持 `postprocess-ui-overlay`。history 记录同样保存 `target_size` 和 `render_size`，target 变化报告 `viewport_resized`，render-size 变化报告 `render_size_changed`。这一步只给后续 FSR/TAA/upscaler、动态分辨率策略和 history resize 判定留下稳定接口，不声明当前已经有上采样 pass 或动态质量控制算法。
...
- The 2026-06-07 scene post-process extract follow-up makes scene extract an upstream authoring source for postprocess without moving resolution into scene code. Runtime-only `PostProcessSettingsComponent` instances seed base bloom/color-grading/effect-stack settings from the selected scene camera only, while explicit request cameras stay on neutral defaults. Runtime-only `PostProcessVolumeComponent` instances live in fixed scene component storage, are filtered by active hierarchy plus camera render layers, and are forwarded as `PostProcessExtract.volume_stack`; local volumes now compute `local_blend` from the scene camera position against same-entity box/sphere/capsule colliders before submit resolves the stack, with no-collider locals excluded. This prevents base defaults and spatial influence from masquerading as complete project/editor postprocess authoring while leaving serialized authoring and trigger lifecycle for later slices.
- The neutral volume-stack resolver coverage now also locks inactive-volume filtering, zero global weight, zero local blend, and saturation of overweight global/local influence before profile blending. That keeps scene extract's spatial `local_blend` as input evidence while preserving submit as the single layer-aware settings resolver.
