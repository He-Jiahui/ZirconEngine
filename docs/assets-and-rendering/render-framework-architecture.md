...
- `zircon_runtime` 的 advanced-plugin resource owner 现在把 render-feature capability scan 收成显式 state，而不是在 construction 阶段扫描后丢弃：`SceneRendererAdvancedPluginResources` 会持有 `VirtualGeometry` / `HybridGlobalIllumination` capability bit，mesh draw path 只有在 VG feature 与 resource capability 同时存在时才进入 VG draw build。`execute_runtime_prepare_passes(...)` 仍然只返回中立空 readback 包，直到插件侧 producer 能通过中立输出接进来；这里不引入 runtime -> plugin 依赖，也不提前初始化 HGI/VG 私有重状态。
