...
- `zircon_app` 的 M9A advanced profile provider collection 现在接上了 app feature 边界：`first-party-advanced-render-runtime-plugins` 只链接 `virtual_geometry` 与 `hybrid_gi` provider crates，`first_party_runtime_plugin_registrations_for_config(...)` 会从 `EntryConfig::render_profile` 读取 `AdvancedRender` 的 `VirtualGeometry` / `HybridGlobalIllumination` product features，并在临时 project manifest 中补入 target-scoped VG/HGI runtime plugin selections 后走现有 provider-aware bootstrap path；`DefaultRender` 不会收集 advanced providers，已有显式 manifest selection 也不会被覆盖。VG/HGI 的 `plugin.toml`、runtime registration descriptor 与 `builtin_catalog` 同步声明 `runtime.render.advanced.virtual_geometry` / `runtime.render.advanced.hybrid_gi` capability，并用 manifest-contribution 测试锁住 package manifest、module descriptor、projected descriptor manifest 与 catalog descriptor 的一致性。
