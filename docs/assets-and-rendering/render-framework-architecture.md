...
同一 owner 现在还把 linked render descriptor capability metadata 固化成 runtime 侧的中立 activation state：`SceneRenderer::new_with_plugin_render_features(...)` 会把 linked descriptors 传给 `SceneRendererCore::new_with_icon_source(...)`，再由 `SceneRendererAdvancedPluginResources::new(...)` 记录 `VirtualGeometry` 与 `HybridGlobalIllumination` capability bit。没有 linked VG/GI descriptor 的基础 renderer 仍保留 base render、mesh、post-process、overlay 和 UI resources；runtime 侧不会提前创建或命名插件私有 `VirtualGeometryGpuResources` / `HybridGiGpuResources`，mesh indirect path 也会在 feature enablement 与 VG resource capability 同时成立时才开启。runtime prepare 当前仍返回中立空 readback 包，真实 producer 接线必须通过插件侧执行并填充 `RenderPluginRendererOutputs`。
...
- `zircon_plugins/virtual_geometry` 的中立 readback 投影也对齐到同一边界：插件私有 `VirtualGeometryGpuReadback` 现在可以转换成 runtime 中立 `RenderVirtualGeometryReadbackOutputs`，覆盖 page-table entry、completed page assignment/replacement、selected cluster、VisBuffer64 entry、hardware-raster typed records 与 NodeAndClusterCull traversal/worklist payload。`VirtualGeometryReadbackOutputs::take_neutral_readback_outputs(...)` 使用一次性 take 语义把插件 owner 的完成态 GPU readback 投影出去；runtime 侧 `SceneRenderer::take_last_virtual_geometry_readback_outputs(...)` 再把 `RenderPluginRendererOutputs.virtual_geometry` 交给 `VirtualGeometryGpuCompletion::from_readback_outputs(...)`，由 submit-time feedback collector 统一写入 `VirtualGeometryRuntimeFeedback`。`SceneRendererAdvancedPluginOutputs::has_virtual_geometry_gpu_readback()` 同步识别 completion-only、render-path 和 NodeAndClusterCull neutral payload，不再只把 page-table/selected/VisBuffer64 当成有效 VG readback。这一步只移动中立 DTO 与 completion handoff，不让 runtime 读取 VG 插件私有状态；仍待推进的是真实插件 prepare/resource 执行把这些 neutral outputs 填进 renderer last-output，并把 pass output 接到 `VirtualGeometryGpuReadback` 的 render-path/NodeAndClusterCull writeback 方法上。
