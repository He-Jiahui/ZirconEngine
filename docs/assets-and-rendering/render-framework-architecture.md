...
M7 后处理参数链先落中立接口，再接入基础 shader 消费：`RenderPostProcessEffectStackSettings` 把 tonemap、LUT、blur、depth of field、screen-space reflection、vignette、film grain、dither、chromatic aberration 与 fog 参数挂到 `PostProcessExtract`，submit context 会把它们和 bloom/color-grading 一起派生有效 `PostProcessStackDescriptor`。只有参数非默认时才插入 `effect-stack` product node，并通过 `postprocess.effect-stacked` 资源别名进入 final composite；DoF 启用时该节点还声明 `postprocess.depth-of-field.coc` 与 `postprocess.depth-of-field.bokeh` 两个 product output，renderer 为它们分配 frame-size `OffscreenTarget` scratch textures 并按同名 graph resource 导入；默认路径不新增 skipped placeholder。当前这一步提供图节点、资源名、executor id、`RenderStats.last_post_process_graph_executed_nodes`、`RenderStats.last_post_process_effect_stack_report` 与 `RenderStats.last_post_process_lut_{request,ready,fallback}_count` 证据；effect-stack report 按帧记录 active families、当前仍是 approximation-backed 的 families，以及 LUT handle / invalid LUT layout / graph-missing SSR normal 资源标签。SSR normal 现在由 postprocess graph 资源状态驱动：当 `effect-stack` 节点声明 `gbuffer-normal` 输入时，submit stats 会清掉 `effect-stack.ssr.normal` 缺口。LUT counters 则记录启用 LUT family 后 authored texture 是否准备成功或回退到 renderer fallback。`collect_runtime_diagnostics(...)` 同步把 enabled、active family count、approximated family count、missing resource count 写入 runtime `DiagnosticStore`，并把 LUT request/ready/fallback 写入 `render.post_process.lut.*`，让工具侧可以读稳定 count/bool path，而不需要解析 renderer 私有状态。同时 `build_post_process_params(...)` 已把有效 effect-stack、camera depth、camera projection 和 camera view-basis 参数编入 `PostProcessParams`，`post_process.wgsl` 会消费这些 uniform 执行基础 tonemap/exposure、authored-or-fallback LUT texture sampling、blur、camera-linearized scene-depth DoF-radius sampling、view-space-normal/material/depth-backed projection-aware bounded SSR march、vignette、grain、dither、chromatic aberration 与 view-depth-weighted fog。WGPU postprocess bind group 现在会在 binding 10 优先采样 `ResourceStreamer` 已准备的 2D/2D-strip authored LUT texture view，缺失时回到 renderer-owned 64x1 S-curve fallback LUT；binding 12 绑定 decoded 3D LUT texture view 或 2x2x2 identity cube，binding 13 绑定 clamp/linear sampler 并用 `textureSampleLevel(...)` 做 3D LUT texel-center 线性采样；binding 11 在非 GL 路径绑定 graph `scene-depth`，GL/WebGL/ANGLE 路径通过 `PostProcessDepthSamplingMode` 编译 viewport-depth fallback shader 并绑定普通 float fallback texture，避免 WGPU/Naga GLSL shadow-sampler 形状失败；binding 14 绑定 graph `gbuffer-normal` 供 SSR direction 使用，binding 16 在 active graph 写入材质 G-buffer 时绑定 `gbuffer-material` 供 roughness attenuation 使用。基础 `.cube` 文本导入已经进入资产层：`zircon.builtin.texture.cube_lut` 会把 LUT 解成 linear D3 RGBA8 `TextureAsset`，明确拒绝 1D shaper section，再走同一 authored 3D LUT 路径。DoF 现在已有 focus/lens/bokeh 参数、CoC/bokeh 资源名和物理 scratch targets，但 dedicated CoC/bokeh writer pass、half-resolution bokeh policy、temporal CoC history、near/far combine、temporal SSR resolve、specular occlusion、SSR depth/reflection pyramids、GL 路径的线性深度颜色资源、half-float/shaper/baked LUT authoring policy 和 dedicated split passes 仍属于后续 M7/M8 子切片。

2026-06-05 更新：SSR 现在还声明并消费 `gbuffer-material`。最终 postprocess bind group 在 binding 16 只有当 active compiled graph 存在未裁剪的 `gbuffer-material` writer 时才绑定材质 G-buffer，否则绑定 renderer-owned black fallback，shader 将其解释为 roughness 1.0，从而在 forward / 未写材质缓冲路径中关闭该反射贡献。同日后续切片还把旧单点 reflected-coordinate seed 替换为 final-pass bounded screen-space ray march：shader 会按 authored max steps / max distance / thickness 在深度缓冲中寻找首个命中，并按距离与屏幕边缘淡出。随后 projection follow-up 又把 camera-derived `effect_projection` 编入 postprocess uniform，shader 先由 depth 重建 view-space origin，再把 view-space ray candidate 投影回屏幕像素采样。normal-space follow-up 继续保留 `gbuffer-normal` 的 world-space contract，但把 camera-derived `effect_view_x/y/z` 编入 postprocess uniform，让 SSR 在 view-space ray march 前把 sampled world normal 转成 view-space normal。DoF follow-up 已把 focus range / focal length / bokeh blades / rotation 编入中立设置与 final-pass shader，并继续补上 `postprocess.depth-of-field.coc` / `postprocess.depth-of-field.bokeh` product-resource names；当前已分配 renderer-owned CoC/bokeh scratch targets，但仍没有 dedicated CoC/bokeh writer pass、half-resolution policy、temporal CoC history、near/far combine、temporal SSR resolve、specular occlusion、SSR depth/reflection pyramids、稳定 StandardMaterial GPU ABI 或 dedicated split pass。
...
- `post_process.wgsl` 不再只做 `AO + clustered + history` composite，而是继续消费：
  ...
  - baked lighting 参数
- resolved effect-stack uniform，包括 tonemap/exposure、authored-or-fallback LUT texture sampling、camera-linearized blur/DoF radius、view-depth-gated SSR seed、vignette、grain、dither、chromatic aberration 与 fog
...
- `zircon_runtime` 的 M6 mesh queue preparation 现在在 renderer 内部形成显式 draw profile：`PendingMeshDraw` 透传 `RenderMeshSnapshot.mobility`，`MeshDraw` 保留 prepared/dynamic geometry source、mobility、material phase 与 indirect 状态，`prepare_mesh_queue(...)` 汇总 early-z、static/dynamic batch、GPU instancing 候选。compiled-scene graph 的 depth/normal prepass 直接消费 early-z 候选，只让 opaque 与 alpha-mask mesh 进入 prepass；同一批 queue-prep counters 也会通过 `SceneRendererCompiledSceneOutputs` 写入 `RenderStats.last_mesh_*`，并由 `collect_runtime_diagnostics(...)` 投射为 `render.mesh.queue.*`，作为 submit 级公开诊断，而不是等 mesh allocator/slab 诊断完成后才可见。
...
- The same 2026-06-05 DoF resource-contract follow-up reserves the split-pass resource names without claiming the split passes. `PostProcessGraphResourceNames` now includes `postprocess.depth-of-field.coc` and `postprocess.depth-of-field.bokeh`; active DoF effect-stack nodes produce both outputs; `import_frame_targets(...)` first imported renderer aliases, and the target-allocation follow-up replaces those aliases with concrete `OffscreenTarget` scratch texture views. This still does not claim dedicated writer passes, half-resolution policy, temporal CoC history, or near/far separation. Scoped `rustfmt --edition 2021 --check`, path-scoped `git diff --check`, and conflict-marker scans passed for the resource-contract slice. Clean Cargo validation is blocked by current checkout churn: focused wrappers timed out in Windows runtime lib-test compile/link, warmed `cargo check -p zircon_runtime --lib --tests --locked --target-dir E:\cargo-targets\zircon-dof-resource-check-0605` emitted fresh runtime metadata before wrapper timeout, and a final rerun fails under `--locked` because unrelated active `Cargo.lock` changes for the first-party runtime catalog/plugin packages need lockfile regeneration.
