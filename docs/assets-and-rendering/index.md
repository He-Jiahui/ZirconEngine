...
- [Directory Project Asset Rendering](./directory-project-asset-rendering.md): `zircon_runtime_interface::resource` locator/handle/state/record 合同与 `zircon_runtime::core::resource` 执行层分工，`Project/assets` 与 `Project/library` 的职责，`res://`/`lib://`/`package://`/`builtin://`/`mem://` 统一来源，`.zmeta` schema，importer dependency locator -> `AssetMetaDocument.entries[*].dependencies` -> `ResourceRecord.dependency_ids` 的 M2 graph path，root/subasset `AssetMetaDocument.entries` -> one artifact/record per entry UUID 的 M3 path，`AssetManager`/`ResourceManager`/asset-owned `EditorAssetManager`、`SceneAssetSerializer`、`LevelManager -> LevelSystem -> World` 与 graphics revision cache 的自动刷新路径。
- [ZMeta Shader Material Assets](../zircon_runtime/asset/zmeta-shader-material.md): `.zmeta` UUID/URL identity、`package://{package_id}/path`、single/compound source units、`.zshader` TOML、compound shader subassets，以及 material `property_values` 对 shader property schema 的校验规则。
...
- [Runtime Render Camera Contracts](../zircon_runtime/core/framework/render/camera.md): camera target/order/layer/HDR/exposure/MSAA neutral contracts and scene projection.
- [Render Material Contracts](../zircon_runtime/core/framework/render/material.md): material descriptor, readiness, dependency, validation, and PBR submit/fallback contracts.
- [Runtime Render Sprite Contracts](../zircon_runtime/core/framework/render/sprite.md): Core2d sprite scene/extract/phase/render stats contracts.
- [Runtime Render Post Process Contracts](../zircon_runtime/core/framework/render/post_process.md): postprocess stack descriptor, graph execution, and final composite contracts.
- [Runtime Render Anti Alias Contracts](../zircon_runtime/core/framework/render/anti_alias.md): AA settings, FXAA fallback, capability gating, and graph stats contracts.
- [Runtime Advanced Render Contracts](../zircon_runtime/core/framework/render/advanced.md): provider-backed VG/HGI profile gating, degradation reports, and submit payload source contracts.
- [Runtime Solari Contracts](../zircon_runtime/core/framework/render/solari.md): SolariExperimental settings, capability requirements, provider availability, and gated unavailable status.
...
- [Runtime UI Graphics Integration](./runtime-ui-graphics-integration.md): runtime builtin fixture 的 v2 `.v2.ui.toml` 入口如何迁到 `zircon_runtime/assets/ui/runtime/fixtures/`，以及 runtime 通过 `UiV2PrototypeStoreFileCache -> UiV2SurfaceBuilder` 构建 `UiSurface` 并把结果送进 graphics frame。
...
- `res://` / `lib://` / `package://` / `builtin://` / `mem://` 的统一资源来源模型
