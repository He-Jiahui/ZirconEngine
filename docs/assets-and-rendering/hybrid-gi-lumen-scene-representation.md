...
- source-stripped runtime RT scene truth no longer depends on the legacy `RenderHybridGiExtract` container either. When the old extract payload has already been removed entirely but `HybridGiPrepareFrame` still names a resident probe and `HybridGiResolveRuntime` carries supported packed RT scene truth, `encode_hybrid_gi_probes(...)` still synthesizes the scene-driven probe and `hybrid_gi_hierarchy_rt_lighting.rs` now returns exact runtime RT lighting instead of early-returning black. The red `encode_hybrid_gi_probes_keeps_source_stripped_rt_truth_without_legacy_extract_container` first failed with zero RT lighting; after the fix it passes with neutral support, authored prepare irradiance demoted, and packed runtime RT preserved.
