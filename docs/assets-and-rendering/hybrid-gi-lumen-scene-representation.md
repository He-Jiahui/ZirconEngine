...
related_code:
  ...
  - zircon_plugins/hybrid_gi/runtime/src/hybrid_gi/scene_representation/global_sdf_scene_state.rs
  - zircon_plugins/hybrid_gi/runtime/src/hybrid_gi/scene_representation/mesh_sdf_scene_state.rs
...
`prepare_runtime_submission(...)` 把中立场景输入交给 provider：当前 `RenderHybridGiExtract`、frame meshes、directional/point/spot lights、visibility update plan 和 predicted generation；它还附带 renderer-owned 的视图状态，即当前 camera position 与本帧 history invalidation。插件 provider 内部再调用 `HybridGiRuntimeState::register_scene_extract_with_view_state(...)`、`ingest_plan(...)` 与 `build_prepare_frame()`，因此 scene representation、screen probes、radiance cache、surface cache、voxel cache 和 pending-update queue 都留在 `zircon_plugins/hybrid_gi/runtime`，runtime 不需要知道这些结构的字段布局，也不拥有 RC 的可变资源。
...
The follow-up screen-probe seed keeps the new state entirely inside the HGI plugin-owned scene representation. `HybridGiScreenProbeState` now derives deterministic probe candidates from sorted scene cards, honors `trace_budget`, attaches the matching surface-cache `page_id` when the card is resident, and keeps `None` as the explicit surface-cache-miss fallback when card budget is lower than trace budget. `collect_resident_probes(...)` projects the current scene descriptors into `HybridGiPrepareFrame.resident_probes`, so the existing GPU completion path consumes their irradiance through `GpuResidentProbeInput`; this is not a separate Lumen `ScreenProbeGather` resource or pass.

`HybridGiRadianceCacheState` owns the bounded 32-demand CPU lifecycle beside the screen probes. It derives samples from persisted Surface Cache truth first and runtime voxel radiance second, retains deterministic slots plus generation/usage/trace metadata, and keeps missing input explicit with zero confidence. Same-cell camera motion performs no source sampling. A clipmap-cell scroll remaps retained demand coordinates to the same world-space probes, preserves their slots and committed samples, advances metadata generation on GPU, and traces only newly exposed cells; source/participation changes still retrace the selected set, while history invalidation clears it atomically. The per-frame neutral payload also carries a full committed bootstrap snapshot so an LRU-evicted renderer instance can rebuild its atlas without forcing stable resident instances to upload unchanged updates.

M5 software-SDF preparation starts from an independent object/page owner rather than renaming the coarse Voxel or Radiance Cache state. Runtime resource preparation caches the effective selected mesh/model geometry, local bounds, deformation state, source revision, and composite revision. The HGI collector transforms those bounds with full local center, rotation, and non-uniform scale, resolves visible/movable/shadow/emissive/hidden-indirect flags from the instance plus material capture seed, and sorts objects by stable instance key. Active morph targets and skinning remain typed voxel fallbacks instead of publishing undersized static SDF bounds. A separate four-level Global SDF CPU state snaps each camera center to page-sized world increments, deterministically divides the quality page budget (`32/64/128`) across levels, retains overlapping resident pages, projects only old/new object dirty bounds, generation-tags asynchronous page work, and keeps new or dirty pages non-sampleable until the matching generation commits.

Mesh SDF is a schema-versioned imported artifact. Runtime loading never cooks it synchronously: importer requests enforce per-primitive triangle/voxel/byte/work limits plus cumulative model budgets, artifact schema 4 rejects stale schema-3 binary layouts, and external model primitive mesh references retain their own resource revision and SDF payload. Global SDF GPU packing accepts only complete, validated `Ready` payload sets that fit the object and frame upload budgets. Missing, invalid, deforming, payload-count overflow, upload overflow, and dense-page candidate overflow resolve the affected page as non-sampleable and preserve the Voxel route; they do not block a later ready page in the same batch. Page/upload budget deferral instead keeps the request dirty and rotates the next bounded batch. Candidate influence expands by the page distance band so adjacent geometry cannot be skipped by an overestimated marching step; dirty-region projection reuses that same expanded band, so a terminal fallback is re-dirtied when an adjacent ready contributor appears. Successful build readback uses generation compare-and-commit; stale completion cannot clear a newer dirty generation.

The production trace dispatch now encodes the capability-graph backend mask, Global SDF lighting policy, and typed fallback reason into its uniform ABI. Shader tracing executes the admitted Surface Cache, Global SDF, and Voxel routes in order and writes one bounded 13-word diagnostic record per admitted probe: probe id, dominant weighted intersection source, dominant weighted lighting source, intersection-backend usage mask, lighting-source usage mask, distance, confidence, fallback reason, texture samples, page tests, SDF steps, voxel candidates, and hardware-ray count. Voxel distance is the contribution-weighted probe-to-cell-AABB world distance rather than a synthetic zero; its descriptor centers use the same signed `64x` quantization and `+2048` bias as the CPU producer. The masks retain mixed-tile provenance even when a deterministic tie-break selects one dominant source. The record returns through `HybridGiScenePrepareResourcesSnapshot` and neutral `RenderHybridGiScenePrepareReadbackOutputs::probe_trace_diagnostics`, so renderer/runtime observers consume GPU-authored provenance rather than the CPU graph's hypothetical `record_hit` path. The core trace algorithms remain in `trace_probe_tiles.wgsl`; the aggregation and entrypoint/output writes live in concatenated suffix owners so neither source file crosses the large-file threshold. Current-source Cargo/WGPU execution, PNG generation, performance measurement, and RenderDoc replay for this slice remain coordinator-managed and are not claimed here.

The plugin owns one cached immutable pipeline set and a bounded map of mutable GPU instances keyed by the provider's camera-history state identity. Each instance has independent metadata/mark buffers, trace/filtered/final atlases, and an asynchronous readback queue capped to the shared three-frame staging ring; LRU cleanup is capped at 32 instances. A new or changed instance remains bootstrap-required until a matching GPU readback completes successfully, so dropped, failed, or saturated observations retry from the full snapshot instead of assuming an unsubmitted update became resident. Incremental update records execute `mark -> allocate trace tiles -> trace -> filter -> border fixup/mip` before Screen Probe consume and `HybridGiCompletionPass`. Each 4x4 buffer-atlas tile contains a filtered 2x2 interior, replicated border, four mip-1 values, and one final mip-2 value. Consume uses the precomputed resident-probe index and eight generation-checked atlas slots, avoiding a per-probe linear resident scan. Stable resident instances execute consume only. GPU-authored item counters for `mark`, `allocate`, `trace`, `filter`, and `border/mip`, plus a committed resident-probe write count for `consume`, return through neutral readback and public `RenderStats`; the ignored evidence exporter writes its PNG/report only after observing a full update readback followed by a successfully committed consume-only stable readback. This is still a buffer-atlas MVP without directional radiance, texture-atlas sampling, sparse GPU allocation, or Lumen parallax/depth semantics. Current-source WGPU execution, PNG generation, and RenderDoc replay for this slice remain pending and are not claimed here.

Current CPU readback composition uses the same eight interpolation corners that demand marking and residency allocate. It quantizes each axis to Q16, blends RGB with confidence-weighted trilinear contributions, reports confidence against total geometric weight, and refuses a mixed generation before interpolation. This replaces the former highest-confidence-corner shortcut without changing the missing-neighborhood fallback.

The provider bridge extends `RenderStats` with scene, residency, invalidation, and update counters so external runtime observers can tell whether HGI scene representation is placing screen probes and maintaining the radiance cache without reading plugin-private state:
...
- `last_hybrid_gi_radiance_cache_update_probe_count`
- `last_hybrid_gi_radiance_cache_truncated_demand_count`
- `last_hybrid_gi_radiance_cache_generation`
- `last_hybrid_gi_radiance_cache_scroll_count`
- `last_hybrid_gi_radiance_cache_history_clear_count`
- `last_hybrid_gi_radiance_cache_gpu_stage_dispatch_counts` (`mark`, `allocate`, `trace`, `filter`, `border/mip`, `consume`)
