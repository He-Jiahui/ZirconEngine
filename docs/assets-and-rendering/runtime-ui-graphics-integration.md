...
tests:
  ...
  - cargo test -p zircon_runtime --lib runtime_ui_manager_dispatches_window_pump_through_shared_surface --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-ui-window-pump-0611
  - cargo test -p zircon_runtime --lib runtime_ui_manager_rebuilds_after_window_pump_layout_metrics --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-ui-window-pump-0611
  - cargo test -p zircon_runtime --lib runtime_ui_manager_window_pump_batch_rebuilds_before_followup_pointer_input --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-ui-window-pump-0611
  - cargo test -p zircon_runtime --lib runtime_ui_manager_runtime_event_batch_rebuilds_before_followup_pointer_input --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-ui-runtime-event-0611
  - cargo test -p zircon_runtime --lib runtime_ui_manager_runtime_event_batch_keeps_prior_events_when_later_adapter_fails --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-ui-runtime-event-0611
...
R1-R7 render contract work adds a derived paint/batch/cache/text-shape/debug-visualizer layer on top of this same extract instead of adding a second mutable frame authority. `UiRenderCommand` can now derive `UiPaintElement` records with typed brush, text, resource, clip, and effect payloads, `UiBatchPlan` can explain stable merge/split decisions from those paint elements, `UiRenderCachePlan` can report paint/batch cache reuse or rebuild reasons, `UiShapedText` can carry glyph ids, advances, font/atlas resources, atlas UVs, ellipsis ranges, and edit decorations, and `UiRenderVisualizerSnapshot` can export paint rows, batch rows, overlays, overdraw regions, resource bindings, and text/backend stats for Widget Reflector style panels. Resource-bearing brush payloads now preserve revision, atlas page, UV rect, pixel size, fallback resource, and material variant state in `UiRenderResourceKey` / `UiRenderResourceState`, so future runtime atlas/cache/debug work can split, invalidate, and visualize by shared DTO fields instead of renderer-local guesses. Each runtime surface keeps the latest flat `UiRenderExtract` as its only mutable complete command authority. The adjacent `UiSurfaceRenderCache` is a transient derived index containing only per-node emission ordinals, command indexes, frames, contiguous ranges, and geometry eligibility; comparisons borrow the previous extract, and local patches compare the current extract directly. It is omitted from `UiSurface` serialization and rebuilt fail-closed after deserialization.

`UiSurfaceFrame` publishes that authority as immutable `UiRenderFrameExtract`, whose command sequence uses 64-command leaves under a 32-way persistent ordered directory while preserving the serialized `list.commands` array schema. A fixed-cardinality local render patch passes its exact retained command ranges from the rebuild boundary; publication copies each touched leaf once and path-copies only its directory ancestors. Untouched leaves remain shared with the prior frame. Full extraction, command cardinality changes, missing range authority, and the first publication rebuild the complete snapshot fail-closed. Raster-scale-only publication advances the render domain metadata while sharing every command leaf. Incremental `rebuild_dirty` publishes before returning, and the Runtime project render path obtains the already-published `Arc<UiRenderFrameExtract>` without flattening or rescanning the mutable command authority. Cross-surface `UiRenderSubmissionSegment` records carry that shared extract plus a small route-tree/node-id projection; the renderer applies the integer node namespace while iterating, so accessibility keeps its existing global node identity without cloning and rewriting every command. Flat `UiRenderExtract` fixtures remain accepted at explicit compatibility boundaries and are converted once there; `UiRenderFrameExtract::to_extract()` remains a debug/serialization materialization, not a frame-loop path. Dirty rebuilds still report reuse, rebuild, removal, and old/new frame damage through `UiSurfaceRebuildReport`, including multiple commands emitted by one node. `UiPaintElement.cache_generation` is populated from a stable command hash, so downstream debug/cache DTOs can observe reuse without the renderer guessing at local state. Runtime surfaces also retain a first `UiSurfaceNodePool` for template-backed controls: removed controls detach from the tree into a surface-owned pool, compatible future insertions reuse the retained node shell, and ordinary resize/property changes continue through dirty rebuilds without reloading `.ui.toml` descriptions or rebuilding the whole surface. The one-slot queue retains the ordered `Arc<UiRenderSubmission>` unchanged; renderer-local segment plans derive from those same shared extracts and do not create another surface command authority.

`UiSurfaceNavigationIndex` is the retained authority for tab/spatial candidate lists, position maps, resolved group/modal membership, and final published candidate geometry. Dirty rebuilds do not classify local input/style/text/visible-range flags as an automatic full navigation rebuild. They compare only changed nodes against retained navigation signatures and resolve inherited group/modal/MUI scope by walking each changed node's ancestor chain; stable changes keep the existing sorted lists. The index stores one subtree-authority bit per retained node so removal of a plain leaf can skip while removal of a container with navigation descendants rebuilds. New roots absent from the retained map scan only their new subtree once. Node-local frame-only geometry changes update retained navigation-authority frames in place, while projected popup changes scan only that authority set; non-focus roots referenced by another node's modal-group contract are retained explicitly in the same authority. Candidate membership, z-index/paint-order changes, popup-stack reconciliation, missing authority, or unknown dirty state remain fail-closed and rebuild the complete index. Full-root layout publication also keeps the complete rebuild boundary. Pointer/navigation event dispatch only queries the published index and never performs these rebuild-time traversals.

The scene renderer retains one complete derived `PreparedScreenSpaceUi` plan plus ordered `Arc<PlannedScreenSpaceUi>` segment entries. Exact submission identity returns the complete plan directly. A new submission wrapper first matches viewport, resolved framebuffer background, and shared font-database generation, then matches each segment by retained extract allocation, route tree, node-id projection, and incoming background generation. A segment miss rebuilds only that command range. If its newly observed background-effect delta is identical to the prior delta, it preserves the outgoing background generation so unchanged suffix segments still hit; a changed delta invalidates the dependent suffix fail-closed. Prepared composition now retains each segment's vertices, base draws, and post-text draws in local ranges instead of cloning and rebasing them into one flat payload; only text/image batches and the saturating glyph-route report are still composed into flat downstream inputs. The renderer owns an aligned segment-local vertex buffer vector. Exact prepared identity skips the complete vertex path, stable segment-plan identity skips BLAKE3 and queue writes for that segment, and a changed segment hashes and conditionally writes only its local vertex bytes while reusing geometrically grown capacity. Profiling separates segment hits, rebuilt command visits, remaining text/image composition clones, segment-plan vertex reuse, hash bytes, writes, and buffer allocations. These caches do not bypass per-frame text/image preparation, draw traversal, buffer binding, or GPU command recording. Segment-local buffers can also trade whole-buffer work for more bindings and retained capacity; product CPU/RSS/GPU captures must decide whether later chunking or an arena/subrange allocator is justified rather than assuming it is free.

Text background inheritance now records opaque fills and invalidating blockers in one paint-ordered effect log. Each lookup walks that log once in reverse and stops at the newest covering background or intersecting blocker, preserving paint order without the former full blocker scan followed by a second candidate scan. Renderer profiling publishes `ui.screen_space_ui_background.query_count`, `effect_visit_count`, and `max_effect_visit_count` in one counter batch. This is an early-exit improvement, not a worst-case constant-time guarantee: a query can still cross many irrelevant historical effects. A spatial index is justified only if current-source interaction captures show sustained visit depth through these counters.

The 2026-06-11 runtime-event facade slice keeps the same frame boundary while making the manager the first runtime-owned consumer of the ABI adapter. `runtime_ui_window_event_routes::runtime_ui_manager_dispatches_runtime_event_batch_through_window_adapter` proves viewport metrics, occlusion, and close-request `ZrRuntimeEventV1` values reach the retained surface frame through the manager, `runtime_ui_manager_routes_runtime_pointer_events_through_owned_dispatcher` proves ABI mouse button input reaches a manager-registered pointer handler through the manager-owned router, `runtime_ui_manager_routes_runtime_pointer_moved_through_window_hover_pump` proves ABI pointer movement enters as `Window(CursorMoved)`, normalizes through the window pump into pointer `Move`, reaches a manager-registered move handler on the direct hover route, and updates hover/cursor/component state before dirty domains are consumed, `runtime_ui_manager_routes_runtime_cursor_left_through_window_pointer_cancel` proves ABI cursor left re-enters that same window pump after a mouse move and replays pointer `Cancel` at the stored cursor point while clearing retained hover/cursor state, `runtime_ui_manager_routes_runtime_touch_events_through_owned_dispatcher` proves ABI touch start shares that pointer router while preserving touch pointer id/source diagnostics and leaving the mouse cursor point untouched, `runtime_ui_manager_routes_runtime_keyboard_enter_through_focused_window_path` proves ABI keyboard Enter can follow an ABI pointer focus prefix into the same focused manager route and default Button activation while preserving window metadata and producing the QuestLog Track button `activated` commit, `runtime_ui_manager_routes_runtime_gamepad_dpad_right_through_focused_navigation_path` proves ABI DPad Right can follow the same pointer focus prefix into `keyboard.navigation` and apply a manager navigation focus effect from QuestLog Track to Close while preserving the `Gamepad_DPad_Right` keyboard vocabulary, `runtime_ui_manager_routes_runtime_gamepad_axis_right_through_focused_analog_navigation_path` proves ABI left-stick X positive can follow the same focus prefix into `analog.navigation`, retain `Gamepad_LeftX` analog state, and apply the same Navigation focus effect, `runtime_ui_manager_routes_runtime_wheel_at_point_through_owned_scroll_route` proves ABI wheel events with point metadata can follow Slate-style cursor hit routing into InventoryList scrolling while preserving precise pixel delta and retained cursor state, and `runtime_ui_manager_reports_runtime_event_adapter_errors_without_surface_mutation` proves no-equivalent single-event adapter errors do not mutate frame window facts. The manager-owned normalized-input continuation adds `RuntimeUiManager::dispatch_input_event(...)` and `dispatch_input_batch(...)`, so pre-normalized `UiInputEvent::Pointer` and `UiInputEvent::Navigation` values also enter through the same retained router before dirty-surface rebuild and `build_frame()`, whether they arrive one at a time or in host callback order. The platform-input continuation adds `dispatch_platform_input_event(...)` and `dispatch_platform_input_batch(...)`, keeping GenericApplication-style callback vocabulary at the manager edge while normalizing into the shared `UiInputEvent` route before pointer/navigation dispatch; `runtime_ui_manager_normalizes_platform_pointer_event_through_owned_router` covers the point-bearing press/capture path, and `runtime_ui_manager_platform_wheel_scrolls_inventory_list_through_owned_router` covers the point-bearing wheel path by scrolling the retained InventoryList ScrollableBox through shared pointer fallback while preserving precise pixel delta metadata. The manager-owned phase registration continuation adds `register_pointer_phase_handler(...)`, so host code can attach Slate-style PreviewTunnel pointer handlers to the same retained router; `runtime_ui_manager_pointer_preview_phase_stops_before_target_handler` proves a preview handler receives the shared hit path, reports the original target, and stops before the target handler without bypassing manager rebuild/diagnostics. The window-pump root-size continuation makes accepted Created/Resized metrics update the manager's root viewport before rebuild/build_frame, and keeps pump batches event-ordered so pointer input after resize observes the refreshed layout. The 2026-06-11 manager-batch diagnostics continuation extends that prefix contract to normalized, platform, and window-pump batches: a later `UiTreeError` stops remaining events while preserving already accepted focus/window/viewport state, and host diagnostics receive `normalized input batch index N`, `platform input batch index N`, or `window input pump batch index N`. The ABI batch continuation extends the same contract to `dispatch_runtime_event_batch(...)`: viewport metrics converted by the interface adapter are reduced first, then a following ABI mouse button is hit-tested against the resized layout. The manager-level ABI batch now converts and dispatches each event one at a time, so a later adapter error or a later successfully adapted dispatch error stops the remaining batch but keeps earlier accepted window, focus, and rebuilt frame state; host diagnostics receive `runtime event batch index N` on the manager error string for the failed ABI event. `runtime_ui_manager_runtime_event_batch_reports_dispatch_error_index_after_adapter_success` covers the dispatch-error side by routing a DPad ABI event into focused keyboard navigation after a prior ABI pointer event established focus. `runtime_host::runtime_ui_manager_clears_node_bound_input_handlers_when_fixture_changes` protects ordinary pointer handler lifecycle across rebuilt fixture surfaces, and `runtime_ui_manager_clears_phase_and_navigation_handlers_when_fixture_changes` extends that protection to phase-qualified pointer handlers plus navigation handlers. Earlier focused validation for this filter produced a current-source binary that passed the then-current two runtime-event facade tests; later owned-router continuations add the pointer/touch/keyboard/gamepad/analog/wheel-at-point ABI route tests plus normalized/platform input route tests and record fresh scoped validation in the session note before any broader green claim.
...
## Runtime Screen-Space Image Resource Cache

The WGPU screen-space image path keeps resource resolution, GPU texture residency, and draw geometry as separate invalidation domains. `ScreenSpaceUiImagePrepareTextureCache` retains requested-to-imported resource IDs while the `ResourceManagementGeneration` identity is unchanged, including unresolved results. A new management generation invalidates those mappings; a missing asset-manager authority is never negative-cached across prepare calls.

Image geometry and draw metadata stay with each `Arc<PlannedScreenSpaceUi>` segment instead of being cloned into one frame-wide image vector. An exact segment-plan identity plus viewport size reuses the segment's draw ranges and vertex buffer without visiting its image batches or hashing its vertices. A changed segment or viewport rebuilds only that segment. The renderer keeps one dependency slot for each unique requested texture in the segment. Each slot retains its resolved texture ID for the current resource-management generation, so a stable prepare goes directly to the current GPU-texture map instead of re-querying the requested-ID map. It then compares the retained and current GPU-resource `Arc` identities. Upload completion, resource revision replacement, and mip residency changes therefore refresh the dependency's bind-group handle without rebuilding unchanged image geometry.

This follows the same ownership boundary as Slate's vector cache and resource handles: source identity plus requested raster size owns SVG raster reuse, while explicit resource invalidation owns replacement. Bind-group eviction keeps all dependencies touched in the current prepare epoch even when the active set exceeds the nominal idle-cache limit; only inactive entries are trimmed, so a cache bound cannot make an active image disappear before render. The retained path publishes `ui.screen_space_ui_image.segment_plan_reuse_count`, `batch_visit_count`, and `texture_dependency_check_count` so a stable frame can prove zero image-batch visits while still proving that live GPU resources were checked. Segment-local vertex buffers can add buffer-bind commands when many image-bearing segments are present; product profiling must measure that tradeoff instead of treating the CPU work model as a GPU acceptance result.
...
同日的 M7 text prepare report slice 把 atlas/cache/bake 事实向上汇聚到 runtime text system，而不是让 debug 面板或后续 renderer 统计从底层对象反推。`ScreenSpaceUiSdfRenderer` 在 `prepare(...)` 后保存 `ScreenSpaceUiSdfPrepareReport`，记录 SDF text batch 数、atlas slot 数、atlas size、atlas resize、bake report、当前 atlas upload byte 数、是否全量 texture upload，以及最终 SDF vertex 数；`ScreenSpaceUiTextSystem` 再保存唯一的 `ScreenSpaceUiTextPrepareReport` authority，记录输入 auto/native/sdf batch 数、解析后的 native/sdf batch 数，以及对应的 `SdfAtlasCacheReport` 与 `ScreenSpaceUiSdfPrepareReport`。2026-05-24 的 upload-report slice 进一步把 upload 计算抽到 [`sdf_upload.rs`](../../zircon_runtime/src/graphics/scene/scene_renderer/ui/sdf_upload.rs)，并把 `ScreenSpaceUiSdfPrepareReport.atlas_upload` 固定为内部 DTO：当前 GPU path 仍以 `FullTexture` 写入保证正确性，但 report 会同步给出 dirty slot count/dirty byte count；未 resize 且全部 stable retained 时 dirty 为 0，新增或 relocated slot 会计入 dirty，为后续把 full texture upload 收束成局部 texture writes 提供可验证边界。2026-08-27 的 single-report-authority slice 移除了 `ScreenSpaceUiRenderer` 的第二份完整 report 快照；renderer 只保留 prepare-valid 位，在无 UI 或 prepare 失败时返回默认报告，在成功后从 text system 读取按需快照。后续 render stats 或 debug reflector 仍从 screen-space renderer 边界读取，不需要直接穿透进 glyphon/SDF backend，也不再为每个 prepared frame 无条件深拷贝一次报告。该层仍然是 `scene_renderer::ui` 内部的 renderer-local 可观测层，不把 GPU atlas 或 glyphon 类型泄漏到 shared `UiRenderExtract`、editor widget 合同或 runtime interface。
...
- viewport HUD producer 仍生成 shared `UiRenderExtract`，`RetainedViewportController` 在提交边界把它包装为单段 `UiRenderSubmission`；`RenderFramework::submit_frame_extract_with_ui(...)` 只接受该分段 authority
...
- [`render_frame_submission_hud_text_renders_through_runtime_glyph_capture`](/E:/Git/ZirconEngine/zircon_editor/src/tests/editing/state.rs) 直接拿 `EditorState::render_frame_submission()` 产出的 scene extract + HUD `UiRenderExtract`，在 viewport 提交边界包装一次单段 `UiRenderSubmission` 后走 `WgpuRenderFramework::submit_frame_extract_with_ui(...) -> capture_frame(...)`
...
- `$env:CARGO_TARGET_DIR='target/codex-native-material-painter'; cargo test -p zircon_runtime --locked render_product_ui --jobs 1 --message-format short --color never`
  - 2026-05-18 M7A pass: 2 passed / 0 failed. This proved the original graph-owned runtime UI placement and shared extract counters. The 2026-06-20 Plan 09 focused run supersedes the ordering expectation: Core2D still reports `last_ui_graph_pass_order = "postprocess-ui-overlay"`, while default Forward+/Deferred 3D now report `postprocess-overlay-ui` so scene overlay/debug executes below terminal screen-space UI. The same refreshed lane covers dynamic-resolution UI target sizing and a dense overlay/UI pixel product.
- `$env:CARGO_TARGET_DIR='target/codex-native-material-painter'; cargo test -p zircon_runtime --lib builtin_registry_covers_runtime_ui_executor_id --locked --jobs 1 --message-format short --color never`
