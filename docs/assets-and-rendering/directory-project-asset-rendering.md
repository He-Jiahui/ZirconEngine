...
- `zircon_asset/src/pipeline/manager/mod.rs` 现在只负责声明 folder-backed 子模块；驱动声明、`ProjectAssetManager` 构造、runtime resident 载入、watcher 同步、facade 实现、builtin 资源和模块描述符分别落到 `pipeline/manager/` 下
