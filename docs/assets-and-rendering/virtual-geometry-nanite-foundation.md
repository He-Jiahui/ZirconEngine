...
related_code:
  ...
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core_render_compiled_scene/render/virtual_geometry_node_and_cluster_cull_pass/*
  ...
  - zircon_runtime/src/graphics/scene/scene_renderer/virtual_geometry/gpu_resources/virtual_geometry_gpu_resources/new.rs
  ...
  - zircon_runtime/src/graphics/scene/scene_renderer/virtual_geometry/shaders/node_and_cluster_cull_instance_work_items.wgsl
  ...
  - zircon_runtime/src/graphics/types/virtual_geometry_node_and_cluster_cull_child_work_item.rs
  - zircon_runtime/src/graphics/types/virtual_geometry_node_and_cluster_cull_cluster_work_item.rs
  - zircon_runtime/src/graphics/types/virtual_geometry_node_and_cluster_cull_traversal_record.rs
...
Virtual Geometry 现在通过 `BuiltinRenderFeature::VirtualGeometry` 声明 prepare、node/cluster cull、page feedback、visbuffer/hardware fallback、debug overlay 等 RenderGraph pass descriptor。现有 runtime state、GPU readback、page upload feedback 与 debug snapshot 仍保留原行为，但是否进入 compiled graph 由 profile/caps/feature opt-in 控制；关闭 VG 时 graph 不含对应 pass。
...
- recursive child-node split/keep evaluation that turns the current global-state compat-SSE/frustum knobs into camera-derived policy, then connects pass-local page requests into runtime upload authority
