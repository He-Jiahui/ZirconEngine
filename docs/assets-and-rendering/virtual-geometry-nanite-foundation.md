...
plan_sources:
  ...
  - user: 2026-05-03 continue workspace closeout by fixing the shared VG integration-test fixture constructor drift
...
tests:
  ...
  - zircon_plugins/virtual_geometry/runtime/src/virtual_geometry/renderer/root_output_sources/virtual_geometry_plugin_renderer_outputs.rs
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_core/advanced_plugin_resources/runtime_prepare.rs
  - cargo test --manifest-path zircon_plugins\Cargo.toml -p zircon_plugin_virtual_geometry_runtime --lib --locked --offline --target-dir D:\zircon-render-workspace-validation
  - zircon_runtime/src/graphics/scene/scene_renderer/core/scene_renderer_runtime_outputs/store_last_runtime_outputs.rs
  - zircon_plugins/virtual_geometry/runtime/src/virtual_geometry/renderer/root_render_passes/virtual_geometry_visbuffer64_pass/mod.rs
  - zircon_plugins/virtual_geometry/runtime/src/virtual_geometry/renderer/root_render_passes/mod.rs
  - zircon_plugins/virtual_geometry/runtime/src/virtual_geometry/renderer/root_render_passes/virtual_geometry_indirect_stats_store_parts.rs
...
2026-07-04 Frameworks 02 M3 follow-up (`frameworks_02_m3_virtual_geometry_deferred_sources_temporal_history_hard_cutover_static_passed`): the deferred renderer sources `virtual_geometry_execution_stats.rs` and `virtual_geometry_unified_indirect.rs` no longer mention retired `RenderQualityProfile::with_history_resolve(false)` and now use `with_temporal_history(false)`. This is only stale API cleanup inside still-deferred sources; `virtual_geometry_renderer_test_promotion_guard.rs::deferred_renderer_test_sources_stay_unwired_until_neutral_contracts_exist` passed 1/1 and continues to block wiring those broad renderer snapshots until they move to plugin-local/public neutral seams.
...
The 2026-05-03 continuation adds the plugin-local neutral renderer-output packaging seam for the next producer slice. `virtual_geometry_plugin_renderer_outputs.rs` wraps either `VirtualGeometryIndirectStats::node_and_cluster_cull_readback_outputs()` or an already-neutral `RenderVirtualGeometryNodeClusterCullReadbackOutputs` into `RenderPluginRendererOutputs.virtual_geometry` through the plugin-owned `VirtualGeometryReadbackOutputs` projection. `root_output_sources/mod.rs` exposes those helpers only inside `crate::virtual_geometry::renderer`; it is not a compatibility re-export for any deleted runtime VG owner path. Runtime prepare output now has a neutral `RenderPluginRendererOutputs` sideband, `PreparedRuntimeSubmission` carries its VG slice into `ViewportRenderFrame`, and the registered `virtual-geometry.runtime-prepare` collector mirrors only that prepared neutral VG readback packet into the central renderer output path. `collect_runtime_feedback(...)` then drains renderer-stored readbacks once, takes node/cluster page requests, and projects the remaining payload into `VirtualGeometryGpuCompletion`; it no longer independently merges the prepared VG sideband a second time. The current VG provider still leaves the sideband empty because `prepare_frame(...)` only owns CPU residency/visibility state, not the plugin GPU readback object; fabricating node-cull page requests from prepare data would create a fake feedback loop. The seam is ready for the real plugin-owned producer once actual NodeAndClusterCull pass output reaches prepare/submission state, without adding a `zircon_runtime -> zircon_plugins` dependency or broadening the shared particles/VG/HGI renderer surface.
...
- The Plan 02 follow-up now makes that MeshDraw projection production-visible through compiled-scene stats. `CompiledSceneDraws::virtual_geometry_execution_stats()` materializes `PreparedMeshVirtualGeometryExecutionStats` from `MeshDraw::virtual_geometry_execution_draw(...)`, `render_compiled_scene(...)` forwards it into `PreparedMeshQueueStats`, and `update_stats/virtual_geometry_stats.rs` merges those execution counts into public `RenderStats`. Ordinary indirect draws without a VG execution-selection key are excluded from the VG execution counters, so the public surface now reflects submitted MeshDraw execution truth without relying only on later context reconstruction.
...
- remaps cooked `VirtualGeometryAsset.page_dependencies` into the same global page id space and publishes them through the neutral `RenderVirtualGeometryExtract.page_dependencies` DTO,
...
The production extract DTO now also carries a cooked page-dependency view. `RenderVirtualGeometryPageDependency` lives in the neutral render extract layer, so `zircon_runtime` can transport page graph data without knowing the plugin runtime's residency structures. The renderer-owned debug snapshot clones the same neutral dependency rows, which gives host-side inspection a direct way to verify that production frame extract and runtime residency are looking at the same cooked graph. The VG plugin runtime treats a non-empty `RenderVirtualGeometryExtract.page_dependencies` list as authoritative, including the root-only case where the cooked graph intentionally has no parent links; only older authored fixtures with no dependency rows fall back to deriving a parent-page map from cluster lineage. This closes the first M2 gap between asset cook output and runtime residency decisions without moving page residency, child-index storage, or request priority policy out of `zircon_plugins/virtual_geometry`.
...
2026-05-12 runtime-surface closeout rechecked the lower VG support layers after full-runtime validation exposed stale effective-payload behavior. The neutral runtime-world render extract no longer injects a non-empty default `RenderVirtualGeometryExtract` for ordinary runtime frames, and frame submission now preserves an explicitly authored empty VG extract as a `RenderPathClearOnly` frame instead of treating it as absent and asking the provider to synthesize automatic geometry. Focused validation passed for `cargo test -p zircon_runtime --test virtual_geometry_stats_contract --locked --verbose` and `cargo test -p zircon_runtime --test virtual_geometry_execution_snapshot_contract --locked --verbose`; broad `cargo test -p zircon_runtime --locked --verbose` then passed with `1268 passed; 0 failed` in the runtime lib target plus green integration/doc-test targets.
...
The Plan 02 mesh draw cleanup removes the last unconsumed VG draw-ref scratch metadata from the runtime mesh bridge. `VirtualGeometryIndirectDrawRef` now carries only the `segment_key` consumed by `submission_detail_from_draw_ref(...)`; the old Rust-side `mesh_index_count` / `mesh_signature` scratch fields and `mesh_signature_for_pending_draw(...)` are gone. The authoritative draw index count for WGPU metadata remains in `VirtualGeometryIndirectDrawRefInput.mesh_index_count`, which `build_indirect_inputs(...)` fills from the pending draw index count when it builds the draw-ref metadata buffer.

The 2026-05-02 M0 executor follow-up replaces the plugin runtime's placeholder render-pass executor bodies with `zircon_plugins/virtual_geometry/runtime/src/render_pass_executors.rs`. The module keeps the pass body plugin-owned while still accepting only neutral `RenderPassExecutionContext` data: pass name, executor id, declared queue, allowed graphics fallback for async lanes, pass flags, and render-graph resource accesses. The checks intentionally validate the compiled graph contract before later Nanite-like helper dispatch is wired in, so a stale runtime-owned pass name, old executor id, or resource-shape drift now fails inside the VG plugin instead of silently behaving like a no-op executor. Focused formatting (`rustfmt --edition 2021 --check` over the touched VG/HGI plugin files) passed; the focused plugin package test command `cargo test --manifest-path zircon_plugins\Cargo.toml -p zircon_plugin_virtual_geometry_runtime -p zircon_plugin_hybrid_gi_runtime render_pass_executors --lib --locked --offline --jobs 1 --target-dir E:\cargo-targets\zircon-vg-hgi-executor-contract --message-format short --color never` was attempted but timed out while other active sessions held Cargo/rustc work, so this slice's code-level contract tests are present but still need a quiet rerun for acceptance evidence.

The 2026-05-02 M1 cook follow-up adds explicit page dependency metadata to `VirtualGeometryAsset`. `VirtualGeometryPageDependencyAsset` records `page_id`, optional `parent_page_id`, and stable `child_page_ids`; the native cook derives those rows from the same four-way BVH hierarchy that produces hierarchy nodes, clusters, pages, root pages, and root cluster ranges. This mirrors the Nanite-style streaming graph at the DTO level without copying UE binary formats: runtime/plugin streaming code can now reason about root pages, child-page prefetch, and parent fallback through a serialized neutral asset field instead of reconstructing dependencies from node ids every frame. Inspection dumps now include `section page_dependencies`, binary dumps move to version `2` and include a dependency count plus sorted dependency rows, and the cook tests have pending coverage for deterministic post-order dependency emission, dump text, and binary header shape. This slice was intentionally not Cargo-tested in this pass per the user request to defer reruns; `rustfmt` and `git diff --check` were used as the non-build implementation gate.
...
The follow-up readback-payload slice removes the previous typed-payload hole inside `VirtualGeometryGpuReadback` itself. `render_path_writeback.rs` now stores `RenderVirtualGeometryHardwareRasterizationRecord` rows beside the source/count, `node_cluster_cull_writeback.rs` stores a neutral `RenderVirtualGeometryNodeClusterCullReadbackOutputs` package, and `virtual_geometry_neutral_readback_outputs.rs` copies both into `RenderVirtualGeometryReadbackOutputs`. This means the one-shot neutral handoff can now represent page residency completion, selected clusters, VisBuffer64, hardware-raster records, and NodeAndClusterCull traversal/worklist data without asking runtime code to inspect VG plugin-private owners. The renderer runtime-prepare side now has a mutable neutral collector boundary on `SceneRendererAdvancedPluginResources::execute_runtime_prepare_passes(...)` that can carry `RenderPluginRendererOutputs` and uses the same non-empty feature-packet replacement merge as compiled-scene graph outputs. The plugin-side registration closeout publishes `virtual-geometry.runtime-prepare` through the public runtime extension registry and checks that the plugin report includes exactly one runtime-prepare collector. That collector now mirrors `RuntimePrepareCollectorContext::prepared_virtual_geometry_readback_outputs()` into `RenderPluginRendererOutputs.virtual_geometry`; the packet remains empty until a real plugin-owned VG GPU readback producer writes one into `VirtualGeometryRuntimePrepareOutput::with_renderer_outputs(...)`.

The 2026-05-03 page-request handoff follow-up closes the first feedback-loop gap in that neutral NodeAndClusterCull package. `RenderVirtualGeometryNodeClusterCullReadbackOutputs` now carries `page_request_ids` beside traversal, child-work, cluster-work, and launch-worklist debug payloads, and `RenderVirtualGeometryReadbackOutputs::take_node_and_cluster_cull_page_request_ids(...)` gives submit-time collection a one-shot way to move those ids into `VirtualGeometryRuntimeFeedback`. `collect_runtime_feedback(...)` now takes the page requests from renderer-neutral readback before projecting page-table completion through `VirtualGeometryGpuCompletion::from_readback_outputs(...)`, so the plugin provider can ingest page pressure through the existing `node_and_cluster_cull_page_requests()` accessor without runtime reading VG private state. `VirtualGeometryNodeAndClusterCullPassOutput::to_neutral_readback_outputs()` now performs the plugin-local pass-output projection for traversal/worklists/page requests, `VirtualGeometryIndirectStats::node_and_cluster_cull_readback_outputs()` exposes that package from the indirect-stats owner, and `VirtualGeometryReadbackOutputs::store_node_cluster_cull_readback(...)` can store a NodeAndClusterCull-only payload even when no page-table completion readback exists yet. `virtual_geometry_neutral_readback_outputs.rs` keeps the plugin-private `VirtualGeometryGpuReadback -> RenderVirtualGeometryReadbackOutputs` projection authoritative once the one-shot owner is drained. Cargo validation remains deferred by instruction; focused DTO/provider commands to rerun later are `cargo test --manifest-path zircon_plugins\Cargo.toml -p zircon_plugin_virtual_geometry_runtime neutral_outputs_project_virtual_geometry_gpu_readback --lib --locked --offline -- --exact --nocapture`, `cargo test --manifest-path zircon_plugins\Cargo.toml -p zircon_plugin_virtual_geometry_runtime take_neutral_readback_outputs_projects_node_cluster_cull_without_gpu_completion --lib --locked --offline -- --exact --nocapture`, `cargo test -p zircon_runtime --lib virtual_geometry_readback_outputs_report_node_cluster_cull_payloads --locked --offline -- --exact --nocapture`, and a focused runtime submit feedback test once the collector has a public harness.

The newest submit-side sideband means the same neutral NodeAndClusterCull packet can ride through `VirtualGeometryRuntimePrepareOutput::with_renderer_outputs(...)`, `PreparedRuntimeSubmission`, `RenderPreparedRuntimeSidebands`, and the registered runtime-prepare collector when the real plugin producer can publish it before feedback collection. Until then, the provider intentionally returns an empty sideband rather than inventing CPU-prepare page requests as renderer feedback.

## Next Expected Layers

The next Nanite plan steps should build on this foundation instead of redefining it:

- wider cook payload metadata for page residency priority, dependency weighting, and debug provenance
...
- recursive child-node split/keep evaluation that turns the current global-state baseline-SSE/frustum knobs into camera-derived policy, then wires live pass-output producers into the neutral page-request feedback field
