...
tests:
  ...
  - cargo test -p zircon_app --lib physical_keys_map_to_runtime_values --locked --jobs 1 --message-format short --color never -- --nocapture: passed 2026-06-09; validates stable ASCII-style WASD physical-key conversion for standalone game projects
  - cargo test -p zircon_app runtime_entry_application_handler_stays_folder_backed_hook_surface -- --nocapture: passed 2026-06-09; validates Windows startup window creation through resumed hook coverage plus retained can-create-surfaces delegation
  - cargo test -p zircon_app --lib runtime_library_project_capture_frame_draws_vampire_hud --features "target-client,first-party-runtime-plugins,first-party-navigation-runtime-plugin,first-party-zr-vm-language-runtime-plugin,backend-zr-vm" --locked --message-format short -- --nocapture --test-threads=1 with CARGO_TARGET_DIR=D:\cargo-targets\zircon-vampire-app: passed 2026-06-10
  - cargo build -p zircon_app --features "target-client,first-party-runtime-plugins,first-party-navigation-runtime-plugin,first-party-zr-vm-language-runtime-plugin,backend-zr-vm" --bin zircon_runtime --locked --message-format short with CARGO_TARGET_DIR=D:\cargo-targets\zircon-vampire-app: passed 2026-06-10
  - ZR_RUNTIME_FORCE_CAPTURE_PRESENT=1 D:\cargo-targets\zircon-vampire-app\debug\zircon_runtime.exe --log-level=log --project E:\Git\ZirconEngine\examples\vampire: launched 2026-06-10, created a 1280x720 runtime window, used capture/softbuffer fallback, and produced examples/vampire/screenshots/vampire-runtime-window-current.png
...
Frame-loop pumping is kept in `runtime_entry_app/frame_loop.rs`: `ApplicationHandler::about_to_wait()` profiles the winit hook and delegates to `pump_frame_loop(...)`, while the child module owns event-loop policy application, optional gilrs polling, dynamic runtime `tick_frame()`, host-request drain, and redraw scheduling. Event-loop policy mapping is folder-backed under `runtime_entry_app/event_loop_policy/`: root `mod.rs` is structural, `frame_cadence.rs` owns demand-aware deadlines, and `control_flow.rs` applies the selected winit `ControlFlow`. Focused `Game` and `Continuous` remain continuous; an unfocused game defaults to a 100 ms low-power interval (10 timer-driven pumps/s instead of about 60), while an explicit frame request or earlier runtime demand deadline can still wake it before that timer. Occluded game windows retain the one-second background interval. `DesktopApp` remains reactive, and Mobile/Headless keep their dedicated cadence policies. This follows Bevy's split where `WinitSettings` / `UpdateMode` define policy vocabulary in `bevy_winit/src/winit_config.rs`, while `bevy_winit/src/state.rs` applies the resulting control flow to the active event loop.
...
The 2026-07-04 runtime-preview startup smoke added an opt-in first-present exit policy for validation. `RuntimeEntryAppConfig` represents it as a nonzero presented-frame limit; `EntryRunner::run_runtime_with_args(...)` maps `ZIRCON_RUNTIME_EXIT_AFTER_FIRST_FRAME` to limit `1`. The historical first-frame record ran scoped rustfmt, full app lib tests `134 passed; 0 failed; 1 ignored`, and a bounded `cargo run -p zircon_app --no-default-features --features target-client --bin zircon_runtime` with `ZIRCON_RUNTIME_EXIT_AFTER_FIRST_FRAME=1` and `ZR_RUNTIME_FORCE_CAPTURE_PRESENT=1`, which exited with code 0. That record does not validate the later `ZIRCON_RUNTIME_EXIT_AFTER_PRESENTED_FRAMES` control. The latter accepts an explicit positive multi-frame limit for controlled profiling; an enabled first-frame variable takes precedence. `surface_present/redraw.rs` increments the count only after a successful native-present or softbuffer fallback-present frame. Normal runtime-preview behavior is unchanged when both variables are absent or disabled. Current multi-frame performance captures require a new managed Windows product run, with all profile, ETW, and screenshot artifacts rooted under `E:\ZirconBuilds`, rather than reusing this historical smoke result. Editor startup-to-first-frame remains a separate Frameworks02 gate.
...
The runtime window-creation split moves the concrete `ActiveEventLoop::create_window(...)` startup path from `application_handler.rs` into `runtime_entry_app/window_creation.rs`. Source guards now require `mod window_creation;`, verify that both `ApplicationHandler::resumed()` and `ApplicationHandler::can_create_surfaces()` delegate to `create_primary_window_surface(...)`, and keep no-primary-window policy, monitor-aware attribute creation, initial nonzero viewport sizing, resize-before-bind ordering, and fallback presenter seeding source-visible in the child module.
