...
related_code:
  ...
  - .codex/sessions/archive/20260418-1910-runtime-absorption-boundary-cutover.md
...
- `zircon_runtime/src/scene/module/mod.rs` 直接解析和构造 `DefaultLevelManager`、`WorldDriver`
- `zircon_runtime/src/scene/mod.rs` 仍然通过 `pub use zircon_scene::*;` 暴露整块 legacy surface
- `zircon_runtime/src/scene/mod.rs` 同时导出 world authority、runtime orchestration、level lifecycle 三类职责
...
其中 `zircon_runtime/src/scene/module/mod.rs` 只允许作为极薄 façade；任何真实实现都必须放进 `module/` 子树，不允许再次把根 wiring 文件堆成实现热点。
...
- `zircon_runtime/src/scene/mod.rs` 不再导出 `LevelSystem`、`DefaultLevelManager`、`WorldDriver`
