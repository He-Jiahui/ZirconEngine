# Native Dynamic Sample Closure Design

## Goal

Close the real dynamic-library path for native plugins by adding one buildable sample plugin that exports the ABI v1 descriptor symbol plus runtime and editor entry symbols. The sample must prove that `NativePluginLoader` can load an actual `.dll`/`.so`, read descriptor metadata, call both entries, and merge diagnostics into registration reports.

## Scope

- Add a narrow `zircon_plugins/native_dynamic_sample` plugin package with a `cdylib` crate.
- Keep the sample independent of editor UI, render feature migration, and app entry reshaping.
...
- The sample package lives under `zircon_plugins/native_dynamic_sample` with `plugin.toml` plus `native/Cargo.toml` and `native/src/lib.rs`.
- The native crate builds as `cdylib` and exports:
  - `zircon_native_plugin_descriptor_v1`
  - `zircon_native_dynamic_sample_runtime_entry_v1`
  - `zircon_native_dynamic_sample_editor_entry_v1`
...
1. A focused runtime test builds the sample `cdylib` in the plugin workspace.
...
3. `NativePluginLoader.load_discovered(...)` discovers the package, loads the library, probes `zircon_native_plugin_descriptor_v1`, and calls both entry symbols from the descriptor.
...
- The sample entries intentionally return deterministic diagnostics so the test proves entry diagnostics are preserved and attributed to the plugin id.

## Testing

- First add a failing focused runtime test for loading the real sample dynamic library.
- Then add the sample crate and any minimal workspace wiring needed for `cargo build` under `zircon_plugins`.
- Validate with a narrow command first, expected shape:
  - `cargo test -p zircon_runtime --lib native_dynamic_sample --locked --jobs 1`
...
- No broad export runner matrix in this slice beyond what is needed to make the sample loadable.
