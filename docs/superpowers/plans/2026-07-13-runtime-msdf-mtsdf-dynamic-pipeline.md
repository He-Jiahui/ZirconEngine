...
- Modify `zircon_runtime/src/graphics/text/mod.rs`
- Create `zircon_runtime/src/graphics/text/sdf/mod.rs`
- Create `zircon_runtime/src/graphics/text/sdf/mode.rs`
- Create `zircon_runtime/src/graphics/text/sdf/params.rs`
- Create `zircon_runtime/src/graphics/text/sdf/glyph_data.rs`
- Create `zircon_runtime/src/graphics/text/sdf/generation_error.rs`
- Create `zircon_runtime/src/graphics/text/sdf/tests.rs`
...
- [x] Add wiring-only `pub(crate) mod sdf;` to `graphics/text/mod.rs`.
...
- Create `zircon_runtime/src/graphics/text/sdf/decode.rs`
- Create `zircon_runtime/src/graphics/text/sdf/tests/decode.rs`
- Modify `zircon_runtime/src/graphics/text/sdf/mod.rs`
...
- Create `zircon_runtime/src/graphics/text/sdf/fdsm_gen.rs`
- Create `zircon_runtime/src/graphics/text/sdf/geometry_preprocess.rs`
- Create `zircon_runtime/src/graphics/text/sdf/tests/fdsm_gen.rs`
- Modify `zircon_runtime/src/graphics/text/sdf/mod.rs`
...
- [x] Run `cargo check -p zircon_runtime --lib --no-default-features --features graphics --locked --jobs 1 --target-dir <managed-target>`; `graphics` enables `text` and is required for `graphics/text/sdf` to enter the crate.
...
- [x] Update `docs/zircon_runtime/graphics/text/sdf.md` with machine-readable headers, contracts, errors, dependency rationale, and current test evidence.
...
- [x] Update `docs/zircon_runtime/graphics/text/sdf.md` and the existing text atlas module docs.
...
- Modify `zircon_runtime/src/graphics/text/raster/policy.rs`
...
- [x] Update `docs/zircon_runtime/graphics/text/sdf.md`, Text05 numbered output records, and the active Session note with exact evidence.
...
|---|---|---|---|---|
| SM2-M1 | S1 dependency and declaration surface | implemented-testing-pending | 2026-07-13 | `graphics/text/sdf/{mode,params,glyph_data,generation_error}.rs`; optional fdsm dependencies wired; declaration tests authored; scoped rustfmt/diff-check passed。 |
| SM2-M1 | S2 reference decode math | implemented-testing-pending | 2026-07-13 | `graphics/text/sdf/decode.rs` and folder-backed decode tests cover median channel ordering, MTSDF alpha, 0.5 boundary, and minimum pixel range; scoped rustfmt/diff-check passed。 |
...
| SM2-M2 | S2 shared generator/stride/fallback integration | implemented-testing-pending | 2026-07-13 | UI font bake delegates pixels to shared `graphics/text/sdf`, emits R8 and RGBA page byte spans, mode-keyed cache reuse, typed generation failures, format-aware dirty upload commands, and normal native-overlay fallback. Focused tests are authored; production `graphics` and `target-client` checks pass, while the monolithic lib-test gate is currently blocked by 25 unrelated runtime-plugin test API-drift errors. |
