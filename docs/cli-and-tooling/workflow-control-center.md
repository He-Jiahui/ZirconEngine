...
Install, Update, and Remove are two-click tray operations. The first click runs a read-only Query for both the coordinator and tray startup entries and stores only a SHA-256 fingerprint of that bounded result; the menu changes to an explicit confirmation label. The preview expires after 120 seconds. The second click repeats Query and executes the mutation only when the action kind, fingerprint and expiry still match. A concurrent startup-entry change therefore invalidates the preview instead of being overwritten. Query itself remains single-click and read-only, Cancel Pending clears either a lifecycle or startup preview, and `tray-diagnostics.json` reports only the pending startup action kind rather than command output or credentials.
