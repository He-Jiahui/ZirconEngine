...
RendererData diagnostic rows preserve the runtime feature name and map SRP contract diagnostics into stable editor paths. Missing shader/material references are dependency-resolution diagnostics; missing entry points/properties/texture slots are shader-schema or texture-slot diagnostics; material validation errors reuse the same messages and `RenderMaterialDiagnosticSource` mapping as the material projection, including material-owned lighting-model failures, required texture-slot failures, and shader payload readiness rows emitted by runtime shader/material contract validation; stored material validation strings remain generic material diagnostics; shader validation strings remain generic unless they carry the importer `wgsl_capture` prefix.
