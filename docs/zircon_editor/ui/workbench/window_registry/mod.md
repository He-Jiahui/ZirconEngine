...
Runtime mutation still flows through layout commands. `SetDrawerMode`, per-slot `SetDrawerExtent`, atomic `SetDrawerRegionExtent`, `ActivateDrawerTab`, drawer attach, drawer detach, and drawer focus resolve through the active `ActivityWindowLayout` rather than legacy root drawers. Geometry-only extent changes do not rebuild this registry because drawer ownership and selection are unchanged. Collapsing a drawer list means the tabs/order can remain, but no drawer view is selected/open. Switching to a window without a registered drawer-capable layout therefore leaves `selected_drawer_for_active_window()` empty and prevents the previous window's drawer from leaking into the new window.
