...
logical drawer projection tree inside `workbench_window.v2.ui.toml` as a transparent root overlay
...
`workbench_window.v2.ui.toml`. Real Workbench components now own the drawer frame contract:
`workbench_main_band.zui` exposes `LeftDrawerShellRoot` and `RightDrawerShellRoot`, SceneTree and
Inspector expose left/right drawer header and content roots, `workbench_window.v2.ui.toml` wraps
...
`workbench_window.v2.ui.toml`. `workbench_main_band.zui` now owns `LeftDrawerShellRoot` and
`RightDrawerShellRoot`, `workbench_scene_tree_panel.zui` and `workbench_inspector_panel.zui` own
the side drawer header/content frames, and `workbench_window.v2.ui.toml` wraps the component drawer
