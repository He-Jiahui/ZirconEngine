...
The 2026-06-17 Workbench layout-frame convergence rerun exposed a stale editor-side naming reference after the runtime-owned layout report renamed the Zircon fallback counter to `zircon_selected_count` and the selected backend to `Zircon`. `EditorUiDebugReflectorModel` now displays the `UiLayoutEngineSelectionReport.zircon_selected_count` field directly, and layout-route tests assert `UiLayoutEngineBackend::Zircon` for fallback routes. Validation used `rustfmt --edition 2021 --check` over the touched reflector/layout-route files and `cargo check -p zircon_editor --lib --locked`; both passed, with only existing warning noise from unrelated modules.
