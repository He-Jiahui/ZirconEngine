...
`target/zircon-profiles/20260514-212822-startup` then subdivided the now-retired auto-open path: it spent roughly 19.7 ms validating all recent projects, 8 ms validating the last project again, and 18.7 ms opening the project. The follow-up capture `20260514-214204-startup` measured that former path at roughly 29.2 ms for `new_resolve_startup_session` and roughly 76.2 ms for `retained_host:new`. These are historical observations, not current performance evidence: no-argument startup now produces the Welcome project chooser and does not activate a recent project. Current startup qualification must measure the chooser path separately before making a latency, RSS, or power claim.
