...
- `zircon_editor/src/ui/retained_host/app/profiling/actions.rs` owns performance timeline action ids, action-to-command mapping, profiling-enabled dispatch, profiling-disabled status fallback, and action mapping tests.
...
Performance action dispatch and profile snapshot merging are separate behavior families. Action dispatch is user-command handling with status-line side effects. Diagnostics enrichment is pane data preparation. Snapshot merge is pure profile data policy gated behind the `profiling` feature. Splitting these files lets future performance controls or merge rules change without reopening an unrelated owner.
...
- Keep future performance timeline commands in `actions.rs`, diagnostics access in `diagnostics.rs`, and profile merge policy in `snapshot_merge.rs`.
