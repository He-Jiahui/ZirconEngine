...
- `zircon_editor/src/ui/retained_host/app/asset_tree_pointer/dispatch.rs` owns click, move, and scroll dispatch for Activity and Browser asset tree surfaces.
...
The dispatch module remains separate because click dispatch crosses into the runtime asset surface bridge and can return UI effects, while move and scroll only update local pointer state. This keeps route-specific behavior visible without duplicating target setup.
...
- Keep future shared asset tree target preparation in `target.rs`; route-specific click/move/scroll behavior belongs in `dispatch.rs`.
