...
`app/host_lifecycle/startup/resources.rs` is the structural startup resources entry. `startup/resources/bundle.rs` owns the `StartupManagers` bundle shape consumed by construction. `RetainedHostRuntimeLease` is the composition root's sole strong runtime owner; it grants `startup/resources/managers.rs` a `RetainedHostStartupRuntimeAccess` that can resolve only the runtime asset manager, editor asset manager, resource manager, and editor manager needed for final bundle assembly. The startup resource leaf must not receive `CoreHandle` or a generic manager resolver, so panes, callbacks, and workspace state cannot grow a second runtime entry point. `startup/resources/events.rs` owns asset/editor-resource/resource change receiver subscriptions.
