...
The last-project auto-open measurements are historical and no longer describe the production startup path. They recorded a former path that called `open_project` once and fell back to Welcome validation if that open failed, with `new_resolve_startup_session` moving from roughly 83 ms in `20260514-225912-startup` to roughly 29 ms in `20260514-232056-startup`, and retained-host construction from roughly 157 ms to roughly 82 ms. No-argument startup now produces the Welcome project chooser without activating a recent project, so new managed profiling must measure that chooser independently before claiming startup latency, memory, or power results.
...
`GpuChromePresenter<P: UiSurfacePresenter>` converts the chrome command stream into `zircon_runtime::rhi::UiSurfaceDrawList`. It records command full/patch counters, propagates fatal runtime surface failures, and treats `RetryableNoSubmit` as an uncommitted frame: cache readiness, paint diagnostics, first-present capture, and damage-to-submit completion do not advance. The failed present is retained outside the normal redraw queue and retried with bounded 8/16/32/64/128/250 ms backoff, so repeated `Timeout` or `Occluded` results cannot form a native redraw busy loop. A real pointer, resize, or external redraw remains immediate, merges the deferred present, and consumes it; the first submitted frame resets the backoff. The native RHI must acquire the swapchain texture before advancing its present generation or preparing batch, text, image, vertex, and retained-target state, so a retryable acquire also performs zero resource-preparation work. Product capture exports `ui.surface.submitted_count`, `ui.surface.retryable_no_submit_count`, and `ui_surface_present_outcomes.json` to keep retries out of submitted-frame and latency evidence. It emits `gpu_upload_bytes`, actual `gpu_draw_calls`, `gpu_visible_commands`, `gpu_visible_draw_items`, `gpu_batch_layers`, and `gpu_batch_dependencies`. It also maps `ChromeImageUvRect` directly into `UiSurfaceImageUvRect`, so atlas metadata crosses the editor/runtime boundary without exposing editor asset-manager or renderer-specific types. It never imports `wgpu`, and its factory no longer names `rhi_wgpu`; the concrete native surface, swapchain, offscreen retained UI target, quad/image pipelines, glyphon text atlas, texture uploads, batch planning, and surface present are selected by the runtime RHI factory.

Native window resize suppresses duplicate values at the event, Editor presenter, and RHI boundaries. Same physical-size and scale notifications do not restart the deferred reflow, invalidate the ordinary damage baseline, or call `surface.configure`; changed metrics continue through the retained snapshot path, and `Lost`/`Outdated` acquisition still forces native reconfiguration. Profiling records duplicate size/scale suppression beside actual surface reconfigure counts.

Owned chrome streams now enter the RHI through the compact-style constructors. Repeated quad, border, and text paint state is interned once per producer generation; command order, clip, z, dynamic text, image payload, and generation remain authoritative. The WGPU implementation compiles ordinary rectangle primitives into contiguous 32-byte instance ranges while retaining the tessellated vertex path for rounded or clipped geometry. F4 profiling publishes `gpu_visible_command_styles`, submitted/compiled solid instances, solid vertex invocations, batch merges, upload bytes, draw/pass counts, and the existing cold/stable compiled-plan counters, so capture comparison can distinguish logical fanout from actual GPU submission.
...
Drawer resize now uses one typed `SetDrawerRegionExtent` event. The layout owner validates all slots before mutation, patches at most two active drawer entries plus their legacy mirrors, and geometry-only commands skip global drawer normalization and complete session metadata recomputation. The remaining issue is downstream invalidation: layout events still request broad layout and presentation work, so EditorUI08 must complete the separate geometry-generation publication path and prove one bounded geometry publish per resize sample. Notification history is already bounded to 64 entries; per-notification string encoding is not treated as an MVP blocker without storm evidence. Viewport RGBA copies remain tracked by PERF-MVP-023/120 rather than being hidden in the primitive wrapper.
