...
PERF-MVP-177 first replaces uniform-list scans with visible ranges and combines same-frame Welcome/asset discovery. The final presentation generation owns immutable parent/subtree membership, control-to-frame, clip-section paint ranges, and the active extension root. EditorUI01 supplies the same virtual-row visible range used by input; the painter does not maintain another row authority. Stable generations perform no topology/map/set/string rebuild, damage visits only intersecting paint segments, and large lists visit visible rows plus overscan. Slint's dependency-tracked per-item cache and pre-render item filtering are the reference lifecycle. Pixel acceptance covers extension selection, clips, z-order, menus, scroll, hover, focus, floating windows, and fallbacks.
...
PERF-MVP-212 follows Slint's retained `Path`/`ArcTo`, Material UI's SVG circle/dash parameters, and Godot's retained texture/geometry submission: Render13 or the host backend should own typed ring/arc geometry, while the UI updates only progress or indeterminate angle. A transitional raster path must be generation-owned, bounded, and keyed by size, quantized progress, full RGBA colors, thickness, and theme generation. Stable determinate frames perform no raster, key formatting, allocation, or upload; indeterminate animation changes typed dynamic parameters rather than rebuilding pixels.
...
PERF-MVP-216 first projects sources once and bounds sampled grid dimensions by plot pixels plus a total-cell budget, preserving every source in the max-influence calculation. Editor07 then owns immutable heat generations and worker scheduling; Render13 should retain a texture or compute result and batch markers. Stable generations perform no heat compute, source clone, command build, or upload.
