...
This closes the host-page popup/select-list path and the first `15a/15e` tier linkage for page-tab overflow. Remaining `15a/15e` work is broader breakpoint polish: tokenizing the remaining popup anchor defaults, linking narrow-tier table-column degradation, lowering window minimums further where needed, and deciding whether an Ultra tier belongs in this milestone.
