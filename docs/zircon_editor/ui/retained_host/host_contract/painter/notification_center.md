...
related_code:
  ...
  - zircon_editor/src/ui/retained_host/ui/pane_data_conversion/pane_component_projection/notification_center_tests.rs
...
Rows come from `TemplatePaneNodeData.structured_options`. `pane_component_projection/notification_center.rs` fills those rows from `notifications`, `selected_notification_id`, `focused_index`, `visible_limit`, `title`, and `empty_text`. It accepts both TOML table entries and pipe-string entries so authored Workbench assets and runtime state can share the same retained host contract.
...
The Workbench primitive is `workbench_notification_center.zui`. It keeps the same popup shell props under `.zui` governance as the descriptor: open flags, placement/anchor metadata, portal/modal flags, notification entries, selected/focused row state, visible limit, title, unread count, and empty text. `workbench_window.v2.ui.toml` imports the primitive so asset reachability checks can see the production reference chain.
