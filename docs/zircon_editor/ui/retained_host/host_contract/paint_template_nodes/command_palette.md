...
Rows come from `TemplatePaneNodeData.structured_options`, which is filled by the `pane_component_projection/command_palette/` owner tree from `commands`, `filtered_commands`, `selected_command_id`, `focused_index`, `disabled_commands`, `recent_commands`, and `query`. The painter does not re-filter commands; it renders the already-projected host contract.
