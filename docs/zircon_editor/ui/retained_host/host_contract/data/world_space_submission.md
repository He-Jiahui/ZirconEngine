...
`data/world_space_submission.rs` is the retained-host data entry for projecting template-pane UI nodes into world-space UI surface submissions. It remains a structural module root: model shape lives in `world_space_submission/model.rs`, collection and sorting logic lives in `world_space_submission/builder.rs`, and private regression coverage lives in `world_space_submission/tests.rs`.
...
`world_space_submission/builder.rs` owns submission collection from a `ModelRc<TemplatePaneNodeData>` and from the aggregate `HostWindowSceneData`. It filters nodes through `world_space_enabled`, derives missing world extents from arranged frame size and pixels-per-meter, skips non-positive projected extents, and performs stable render-order sorting.
