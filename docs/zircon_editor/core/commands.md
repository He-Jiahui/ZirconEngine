...
`EditorCommandDescriptor` contains the former command and operation metadata in one record: display text, category, menu path, default chord, structured `when`, keywords, payload schema, remote-call policy, undo metadata, and required capabilities. Its key is `EditorOperationPath`, retained as the validated stable identifier type used by UI bindings and the operation-control DTOs.
...
- `EditCommandFactoryPending` marks commands that require the Editor 03 edit-command factory. Invocation returns `EditorCommandDispatchError::EditCommandFactoryPending`; it never fabricates an event or fake undo record.
...
Built-ins are declared in `defaults.rs`; their previous enablement variants map directly to structured clauses. Menu rows and palette rows are projections of registered descriptors, while `assets/ui/editor/keymap/default.keymap.toml` binds the same typed command identifiers. Contributed-menu three-source merging, user keymap overlays, and the real edit-command factory belong to later Plan 08 milestones.
