...
| # | 切片 | 涉及文件 | 验证命令 | 硬切换 |
|---|------|---------|---------|--------|
| M1.S1 | EditorShellSurfaceHost 骨架：主窗口挂 UiSurface，实例化最小 shell 树（仅 status bar 区） | shell_surface_host.rs、app.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipTest` | 无删除 |
| M1.S2 | status bar 区域首迁：workbench_status_bar.zui 承载 + 数据绑定（状态消息/chips） | shell_surface_host.rs、status_bar.zui | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter status_bar` | 删 painter status bar 段 |
| M1.S3 | 实机：status bar 由 runtime 路径渲染/命中（GPU stream 验收 software_fallback_count=0 沿用） | 实机 | editor 实机 | 删 presentation_cache 对应区段 |
| M2.S1 | activity rail 迁移（与 01 M5.S2 协同删桥） | activity_rail.zui、shell_surface_host | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter activity_rail` | 删 rail 桥命中态 + painter 段 |
| M2.S2 | main tabs 迁移：tab 切换/拖拽重排走 runtime（06 TabStrip） | main_band.zui | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter document_tab` | 删 tab 桥命中态 + painter 段 |
| M2.S3 | drawers 框架迁移 + docking 接缝接通（02 M4 PaneContentRootConstraint）：开合/改宽/切 tab 全走新路径 | component_drawer.zui、autolayout | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter drawer` + 实机 | 删 drawer 桥命中态 + painter 段 |
| M3.S1 | 剩余区域（top toolbar、document workspace、viewport 挂点）迁移 | top_toolbar.zui、viewport_panel.zui | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests` | 删 root_shell_projection.rs |
| M3.S2 | 旧路径总删除：painter 投影族、presentation_cache、presenter 残余；全壳实机交互回归 | retained_host/ | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests` + `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -TestTarget integration_contracts -Features integration-contracts` + 实机 | **删除确认清单出文档** |
| M4.S1 | EditorCommandRegistry + 默认命令集（File/Edit/View/Window/Help 全菜单项） | host/commands/ | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter commands` | 快速按钮旧实现删除 |
| M4.S2 | keymap 资产 + 焦点链未消费按键解析（01 路由 default-action 后接） | keymap.rs、keymap.toml | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter keymap` | 无删除 |
| M4.S3 | 菜单栏真实菜单树 + CommandPalette 接命令源（06 M3 骨架）；命令矩阵测试 + 实机快捷键 | 菜单模板、palette | 同上 + 实机 | 无删除 |
| M5.S1 | 浮窗：window_registry 接独立 UiSurface + 原生子窗口；drawer ↔ 浮窗互转 | window_registry/ | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter window_registry` | 删 floating_window_projection.rs |
| M5.S2 | 布局持久化四级恢复：序列化 + 启动恢复 + preset 切换 | layout_persistence.rs、preset/ | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter layout_persistence` + 重启实机 | 无删除 |
| M6.S1 | context menu：hit path → provider 查找 → 节点语义菜单 | shell_surface_host、菜单 | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter context_menu` | 无删除 |
| M6.S2 | toast 触发链 + NotificationCenter 历史（06 M3 组件） | editor 事件 → toast 队列 | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter toast` | 无删除 |
...
- 当前失败交接（协调器权威图 `10 open / 待修复`）：[`runtime-diagnostics-physics-state-format`](08/failure-2026-07-11-runtime-diagnostics-physics-state-format.md)、[`retained-window-hard-cutover-expectations`](08/failure-2026-07-11-retained-window-hard-cutover-expectations.md)、[`editor-event-full-reflection-rebuild`](08/failure-2026-07-17-editor-event-full-reflection-rebuild.md)、[`layout-metadata-full-recompute-and-sync-io`](08/failure-2026-07-17-layout-metadata-full-recompute-and-sync-io.md)、[`viewport-fallback-scene-rebuild-under-live-frame`](08/failure-2026-07-17-viewport-fallback-scene-rebuild-under-live-frame.md)、[`viewport-toolbar-surface-rebuild-storm`](08/failure-2026-07-17-viewport-toolbar-surface-rebuild-storm.md)、[`runtime-ui-detached-unbounded-subscriptions`](08/failure-2026-07-18-runtime-ui-detached-unbounded-subscriptions.md)、[`runtime-ui-diagnostics-snapshot-eager-full-materialization`](08/failure-2026-07-18-runtime-ui-diagnostics-snapshot-eager-full-materialization.md)、[`runtime-ui-render-extract-cache-command-identity`](08/failure-2026-07-18-runtime-ui-render-extract-cache-command-identity.md)、[`runtime-ui-surface-frame-full-copy-and-ecs-reprojection`](08/failure-2026-07-18-runtime-ui-surface-frame-full-copy-and-ecs-reprojection.md)。
- fixed 已修复：[componentized-workspace-test-export](../editor/14/fixed-2026-07-12-componentized-workspace-test-export.md)
- 当前性能交接仍处于 `in_progress`，23 条逐项审阅证据已无损迁入 [`08/2026-08-01-performance-review-handoffs.md`](08/2026-08-01-performance-review-handoffs.md)；父计划只保留当前状态，具体 owner、预算与验收条件以该记录和 Performance 01 为准。
