...
以下条目解释 M1-M5 的设计起点；其中 manager、pointer table、routing、timers 和平台翻译 owner 已落地，不得继续当作当前缺失能力。
...
- `zircon_runtime/src/ui/dispatch/input_manager/` 已包含 `UiInputManager`、`UiActivePointerTable`、`routing.rs`、`timers.rs` 和 outcome/IME host request owner；历史缺口 2-4 已进入实现阶段。
- `zircon_runtime/src/ui/platform_input/winit_translation.rs` 是当前平台翻译 owner；旧 `zircon_editor/src/ui/retained_host/host_contract/native_input_translation.rs` 已删除，editor 入口收束到 `host_contract/window/event_loop/platform_input.rs`。
- M5 的 route-intent 迁移已有历史 focused 回执，但 retained bridge 的全量 projection、payload clone、hit/query 与 diagnostics 成本仍由 12 个 canonical failure 跟踪；计划保持 `in-progress / resolving_failure`，不得因历史 M1-M5 结果关闭。
...
|---|------|------------------|---------|--------|
| M1.S1 | 翻译盘点矩阵：以 editor `native_input_translation` 现行为金标准，固化 winit↔`UiWindowPlatformInputEventKind` 全 variant 映射测试；列出触摸/IME 缺口清单。（2026-07-02 评审收口）盘点矩阵补条款：**IME 激活期间 KeyboardText 抑制**——printable 字符与 `Ime::Commit` 必须去重（IME enabled 时抑制 KeyboardText/字符事件直发，仅走 Commit 通道），防止双输入 | `zircon_editor/src/tests/host/retained_window/platform_input_translation.rs`（扩充） | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter platform_input_translation` | 无删除 |
| M1.S2 | `platform_input::winit_translation` 单实现落地，rhi/rhi_wgpu ui_surface 翻译段并入 | 新增 platform_input/；核验 rhi/ui_surface.rs、rhi_wgpu/ui_surface.rs 当前仅有 surface descriptor 逻辑，无 winit 输入翻译段可迁 | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipTest` | rhi 两处输入翻译段按当前代码核验为不存在；S3 删除 editor 本地翻译 |
| M1.S3 | editor/app 切换调用方；删除 editor 本地翻译 | event_loop/platform_input.rs；删 native_input_translation.rs 与 native_input_translation/**；保留经核验非翻译的 native_keyboard.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests` | 删 editor-local 翻译树 + event_loop/input.rs 指针/滚轮翻译段 |
| M1.S4 | interface 触摸/IME variant 补缺（按 S1 清单，集中一次）。（2026-07-02 评审收口，U8）iface dispatch DTO（`UiImeInputEvent` 等）变更与 text/08 **协同一次合并**，不各自演进 | `zircon_runtime_interface/src/ui/window/input.rs` | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime_interface -SkipBuild -LibTests` | 无删除 |
| M2.S1 | input_manager 骨架：UiInputManager 持双分发器，window_pump 批入口改走 manager（行为等价收编） | 新增 input_manager/{mod,manager,outcome}.rs；改 window_pump.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter window_pump` | window_pump 旧签名删除 |
| M2.S2 | 路由次序单点化：routing.rs 固化七阶段次序；dispatch.rs 改由其驱动 | 新增 routing.rs；改 dispatch.rs、route_policy.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipTest` | dispatch.rs 内散落次序逻辑删除 |
| M2.S3 | 路由矩阵测试：capture 抢占、popup 外点关闭只关最上层、preview 先于 bubble、focus-path 键盘、default-action 兜底 | input_manager 模块内 #[cfg(test)] | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter input_manager` | 无删除 |
| M3.S1 | effect 覆盖盘点：`UiDispatchEffect` 每 variant 应用测试；RejectedEffect 全部带 reason | apply_dispatch_reply 所在模块 + 测试 | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter dispatch` | 无删除 |
| M3.S2 | 计时 ownership 移交：timers.rs 接管 tooltip/双击；tick() 注入合成事件 | 新增 timers.rs；改/删 tooltip_timer.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter tooltip` | tooltip_timer.rs 旧计时态删除 |
| M3.S3 | 既有 drawer/menu/drag 测试在新路径全绿（回归闸门） | 无新文件 | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests`、`.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests` | 无删除 |
| M4.S1 | UiActivePointerTable：per-pointer hover/capture 状态 | 新增 pointer_table.rs；改 state/mod.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter pointer_table` | state 内单指针旧字段删除 |
| M4.S2 | primary touch 鼠标语义合成 + cancel 清理 | routing.rs、pointer_table.rs | `.\.codex\skills\zircon-dev\scripts\validate-matrix.ps1 -Package zircon_runtime -SkipBuild -LibTests -TestFilter touch` | 无删除 |
| M4.S3 | 多指针矩阵测试：两指独立 hover/press、cancel 清理、capture 隔离 | input_manager 测试 | 同上 | 无删除 |
| M5.S1 | route_intent 适配层 + 首迁 shell_pointer 家族（8 文件改读 route id，drag/resize 捕获走 reply） | 完成：新增 route_intent/；改 shell_pointer/* | 历史回执：shell_pointer focused offline 通过；locked 当时被 lockfile 漂移阻断 | shell_pointer 内命中码删除 |
| M5.S2 | 迁 document_tab / drawer_header / menu / activity_rail 四桥 | 完成：四桥改读 `route_intent`；删除四桥本地 target/conversion 命中码 | 历史回执：editor focused offline 通过；locked 当时被 lockfile 漂移阻断 | 四桥命中码删除 |
| M5.S3 | 迁 hierarchy / asset / detail / host_page / viewport_toolbar / welcome_recent + tab_drag / drawer_resize | 完成：hierarchy/detail/host_page/viewport_toolbar/welcome_recent 五桥改读 `route_intent`；tab_drag 复用 shell_pointer route intent；asset_pointer 无独立 runtime-surface 桥；drawer_resize 已在 M5.S1 shell route 中收编 | 历史回执：editor lib check 与 6 组 focused filter offline 通过；locked 当时被根 lockfile 漂移阻断 | 五桥本地 target/map 命中码删除 |
| M5.S4 | 清残：删除 surface_hit_test 被取代部分；实机回归 | `host_contract/surface_hit_test/` 仅保留 template-node owner；viewport toolbar 原生命中改走已投影 `UiSurfaceFrame` + `route_intent` | 历史回执：editor lib check、integration_contracts 27/27 与 editor-host app check offline 通过；真实窗口交互未执行 | 残余 toolbar 手写 surface_hit_test 命中删除 |
...
- 性能交接归档：[`01/2026-08-01-performance-review-handoffs.md`](01/2026-08-01-performance-review-handoffs.md)（14 条原始记录，保留后续验收约束）
- 当前状态：M1-M5 的基础输入 owner、路由顺序、effect、timer 与多指针表已进入源码；当前仍为 `resolving_failure`，以下 failure 未完成前不得提升计划状态。
- fixed 已修复：[plan-output-archive-notice](../editor/09/fixed-2026-08-05-plan-output-archive-notice.md)
- open 待修复：[retained-asset-pointer-full-surface-rebuild](01/failure-2026-07-17-retained-asset-pointer-full-surface-rebuild.md)、[retained-control-index-and-virtual-row-sync](01/failure-2026-07-17-retained-control-index-and-virtual-row-sync.md)、[retained-route-intent-owned-payload-cloning](01/failure-2026-07-17-retained-route-intent-owned-payload-cloning.md)、[runtime-ui-accessibility-full-snapshot-per-action](01/failure-2026-07-18-runtime-ui-accessibility-full-snapshot-per-action.md)、[runtime-ui-default-interaction-routing-and-popup-scan](01/failure-2026-07-18-runtime-ui-default-interaction-routing-and-popup-scan.md)、[runtime-ui-dispatch-route-clone-and-timer-scan](01/failure-2026-07-18-runtime-ui-dispatch-route-clone-and-timer-scan.md)、[runtime-ui-focus-input-history-unbounded](01/failure-2026-07-18-runtime-ui-focus-input-history-unbounded.md)、[runtime-ui-hit-query-scratch-and-quadratic-dedupe](01/failure-2026-07-18-runtime-ui-hit-query-scratch-and-quadratic-dedupe.md)、[runtime-ui-input-route-diagnostics-and-effect-ownership](01/failure-2026-07-18-runtime-ui-input-route-diagnostics-and-effect-ownership.md)、[runtime-ui-navigation-candidates-rebuilt-per-key](01/failure-2026-07-18-runtime-ui-navigation-candidates-rebuilt-per-key.md)、[retained-template-table-row-identity-selection](01/failure-2026-07-27-retained-template-table-row-identity-selection.md)。
