...
| M3 | damage paint 与有序命令提取 | 执行中 | - | 正常 hover 帧 fallback sort 为 0，模板访问已降至 72/frame；当前精确版本的 full/patch PNG 对拍被共享 shader IDE stub 与 renderable-empty 场景 mobility 基线错误阻断。 |
...
此前切片的首帧 full repaint 对拍通过现有阈值：software reference vs GPU 的 differing-pixel ratio 为 `0.024788`、average-channel delta 为 `0.8704`，GPU vs Softbuffer 仅 `16` 个像素不同（ratio `0.0000102`、average delta `0.0000194`）。最新 paint-index catalog 版本先因未打开项目返回 `editor product frame capture requires one opened project`；随后使用独立 bundle 创建 `renderable-empty` 临时工程，启动先被 `zr_bindless_material.wgsl` 缺失 `ZR_BINDLESS_MATERIAL_SLOT_CAPACITY` 的 shader IDE stub 校验阻断，禁用该临时工程的 shader 资产后又被模板场景 `entity 2 cannot become Static under Dynamic parent 0` 阻断。两个进程均正常退出且没有生成 PNG；不通过手工改 shader 或场景伪造产品证据，因此该证据不能冒充当前精确版本对拍，M3 像素门槛保持开放。10 分钟压力证据仍为 `14,406` 次 hover/正向 scroll/keyboard/短 document-tab drag 循环、`0` 次无响应、平均 `0.034 core`，Private Bytes 后 5 分钟保持平台；反向滚轮因 harness 的 PowerShell 无符号转换告警不计入证据。聚焦命令 `cargo test -p zircon_editor --lib host_window_refresh_diagnostics_update_state_overlay_text` 仍未进入目标用例：共享工作树的全量 `lib test` 配置先因 569 个既有测试 API 漂移错误编译失败。
