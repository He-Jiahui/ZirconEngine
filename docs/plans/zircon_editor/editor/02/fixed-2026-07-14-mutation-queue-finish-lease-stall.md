...
- 根因：全局 mutation lock 同时覆盖无界观察/维护与前台写入；orphan reconcile 可先于排队的正常 cargo finish 落库。
- 架构修复：后台观察、Cargo reconcile 和清理移出前台 mutation mutex，受控动作使用独立锁；cargo owner finish 可从 orphaned 恢复真实 exit code。
- 验证：并发回归：慢观察、长受控动作不阻塞前台 mutation；cargo finish/reconcile 竞争保留 exit code 0，三项测试通过。
- 回传：前台会话写入与后台维护已分离，cargo finish/reconcile 竞态不再丢失真实完成结果。
