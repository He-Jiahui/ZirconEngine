...
1. **编辑器是 runtime 的客户端，不是宿主**。runtime 经 `zircon_runtime_get_api_v1` → `ZrRuntimeApiV1`（11 指针函数表）动态加载，每 session 独立 `CoreRuntime` + `LevelSystem`。编辑器一切世界访问走门面（01 `EditorRuntimeGateway`），不直接持有 world 引用穿透边界。
