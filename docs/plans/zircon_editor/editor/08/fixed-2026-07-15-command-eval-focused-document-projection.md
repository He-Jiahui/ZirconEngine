...
| `OPEN / current-source RED 已修复 / Cargo 复验排队` | 2026-07-15 | current-source Windows binary 先通过 `focused_document` 2/2、command when 4/4 与 descriptor when 4/4；补跑 runtime when 为 5/6，唯一 RED 是仅打开项目时默认 Scene 被误投影为显式焦点。最低共享根因在 `recompute_session_metadata`：它把“从未设置焦点”的 `None` 与“原焦点已失效”合并回退到 active main-page tab。修复已收紧为仅对 `Some(invalid)` 回退，保留显式 focus/open/page activation 与 focused-close 行为；等待协调器测试槽复编后再决定回传。 |
| `COMPLETED / current-source GREEN` | 2026-07-15 | 协调器 current-source job `178982916e2d4f2281cc20e90966566a` 生成的新 Windows test binary `zircon_editor-57c0b3f1608553a4.exe` 上，runtime when 6/6、command when 4/4、descriptor when 4/4、focused-owner hard-cut 1/1、Chrome typed projection 1/1，共 16/16 通过。默认项目无焦点保持 `None`，浮动文档显式聚焦与关闭后回退同时通过；`rustfmt --edition 2021 --check zircon_editor/src/ui/host/workspace_state.rs` 与 scoped diff-check 通过，可回传 Editor08。 |
...
- 根因：EditorUiHost::recompute_session_metadata treated an absent focused_view and an invalid former focused_view as the same fallback case, fabricating the active Scene tab as document focus on project open.
- 架构修复：Keep EditorSessionState.focused_view as the sole typed owner and fall back to the active main-page document only when a previously explicit Some(instance) becomes invalid; None remains None. Chrome continues to project only focused_view -> ViewInstance -> ViewDescriptor.document_kind.
- 验证：Current-source Windows zircon_editor binary: runtime when 6/6, command when 4/4, descriptor when 4/4, focused owner hard-cut 1/1, Chrome typed projection 1/1 (16/16); direct rustfmt check and scoped diff-check passed.
- 回传：Editor08 can resume the focused-document command-evaluation gate. Animation asset-index validation remains independently open behind the Render18 Shader IDE dependency handoff.
