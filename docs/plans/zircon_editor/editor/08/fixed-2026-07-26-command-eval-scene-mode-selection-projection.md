...
tests:
  - .codex/skills/zircon-dev/scripts/validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter command_eval_projection
  - .codex/skills/zircon-dev/scripts/validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter scene_mode
  - .codex/skills/zircon-dev/scripts/validate-matrix.ps1 -Package zircon_editor -SkipBuild -LibTests -TestFilter edit_and_play_selection_are_isolated
---
...
- 根因：CommandEvalCtx was projected from chrome approximations instead of the authoritative scene mode stack and active-domain selection model.
- 架构修复：SceneModeStack now projects its active SceneModeId and SelectionModel active-item count through SceneViewportController and EditorState into the shared command-evaluation snapshot.
- 验证：Managed zircon_editor lib filters command_eval_projection, scene_mode, and edit_and_play_selection_are_isolated each completed with Cargo test OK.
- 回传：Editor08 may resume SceneModeActive and SelectionNonEmpty consumers against the production projection.
