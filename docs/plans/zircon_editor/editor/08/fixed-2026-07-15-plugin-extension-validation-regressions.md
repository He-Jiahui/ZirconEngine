...
| Editor12 M1/M2 / Editor08 M1 | 当前源码架构修复与 upward gate | `fixed-ready-待原子回传` | 2026-07-15 | 受管 Windows job `4fe99dc603c34ac28d58148390d752f0` 以当前共享源码完成 `cargo test -p zircon_editor --lib --no-run --locked --jobs 1`，exit 0；新 binary 上三条原始 fully-qualified exact 与 remote-discovery exact 为 4/4，authoring/registration/validation/registry/commands/Workbench 六组为 57/57。修复将纯贡献冲突预检前置到命令候选物化之前，并用 `EditorCommandMenuProjection` 保留 View 命令 discovery 元数据、唯一化 capability-filtered 菜单投影 owner；无退役 operation registry、兼容 alias 或失败后部分注册。 |

## 修复结果与回传

- 根因：The schema assertion retained retired operation vocabulary; pure view/contribution validation ran after command candidate materialization, so duplicate views surfaced as generated-command collisions; generated View commands also let the command registry and capability-filtered extension projection both own the same menu row.
- 架构修复：Hard-cut the assertion to editor command vocabulary, prevalidate views/contribution ids/asset types before cloning and materializing command candidates, and add EditorCommandMenuProjection so generated View commands retain canonical menu metadata for discovery while only the enabled extension registry materializes their Workbench menu rows.
- 验证：Managed Windows job 4fe99dc603c34ac28d58148390d752f0 completed cargo test -p zircon_editor --lib --no-run --locked --jobs 1 with exit 0; the three originating fully-qualified exact tests plus remote-discovery exact passed 4/4; authoring, extension registration, extension validation, registry, commands, and Workbench menu groups passed 57/57; scoped rustfmt and diff checks passed.
- 回传：Editor08 may resume its M1 unified-command full gate; invalid plugin contributions are rejected atomically, disabled plugin views stay out of menus, discovery metadata remains intact, and no EditorOperationRegistry compatibility path was restored.
