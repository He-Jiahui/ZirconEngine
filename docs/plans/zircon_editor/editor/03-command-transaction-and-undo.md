...
- §M4「`serialize_journal` 实现度清点（未实现记债）」：产出记录已提到 `journal_transaction` 在 engine mutex 内物化但「全仓当前没有生产caller」。建议 M4.1 增加一条显式验证——`core/editing/engine/journal.rs` 的序列化路径若无生产调用方，应有一个 round-trip 单测锁定其行为，否则 11（序列化/版本迁移）接线时无法判断 journal 格式是否已冻结。
