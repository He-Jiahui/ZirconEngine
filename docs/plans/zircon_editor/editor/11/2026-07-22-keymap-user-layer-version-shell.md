...
| 2026-08-02 | `static_complete_dynamic_pending` | Editor17 已完成 registry-only 硬切：`EDITOR_KEYMAP_OVERRIDES_KEY` 注册为 User setting，`EditorKeymapOverrides` 保留 typed tombstone，`SettingsStore` 统一使用 `zircon.editor.settings` 壳；旧 `keymap/persistence.rs`、私有 `zircon.editor.keymap-user-layer` fixture 与只镜像旧 owner/测试名的 Python contract 已删除。 | 聚焦 Python 运行以 2 个 missing-file error 和 1 个旧测试名 failure 证明旧 contract 已失效；删除后保留的 Editor02/09 源码 contract 14/14 通过。当前 Rust 行为由 `commands/keymap/tests.rs` 与 `settings/tests.rs` 持有；本次维护未运行 Cargo，父 M2 仍不据此宣称动态验收完成。 |
