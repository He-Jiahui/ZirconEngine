...
**字号单位裁决(2026-07-10 复核)**:UE Starship 的 Normal=10/Small=8/Large=14 来自 point size,不是像素;`SlateFontInfo.h` 明确以 96 DPI 转 Slate Units。编辑器中央 typography token 因此存 13.33/10.67/18.67 逻辑像素。此转换只发生在 token 默认值构造/旧配置迁移处;后续仍按 `physical_px = logical_px × scale_factor` 重栅格,不得二次乘 96/72。
...
- 迁入记录：[`17/2026-07-09-text-rendering-and-typography-output-records.md`](17/2026-07-09-text-rendering-and-typography-output-records.md)
