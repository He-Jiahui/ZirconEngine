...
- 本次尝试受管 `zircon_editor` build（check lane `0a203ed1001a478795869bf98c9ed805`）最初停在本会话范围外、刚创建的 `zircon_runtime_interface/src/runtime_api/operation.rs:60`：`matches!(Self::Completed | Self::Failed, self)` 的 pattern/value 顺序错误，报 E0424/E0369。该 interface 修复随后由其 owner 写入；Layout15 未改写或以局部 shim 绕过。
