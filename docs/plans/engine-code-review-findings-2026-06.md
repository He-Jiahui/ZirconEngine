...
当前 Frameworks 审查同步（2026-07-30）：结构/P1 发现的物理收口不再引用 2026-07-02 的声明顺序注释或 graphics→scene/UI 旧计数。[`Frameworks 01 M0`](zircon_runtime/frameworks/01/2026-07-13-m0-current-structure-and-dependency-baseline.md) 已用 9,188 个 Rust 输入 pre/post 同指纹 `348fe59c72c798e5e64babb5489910f30dbd00a58441ec11c1e176826519339e` 固化 2,391 refs / 76 edges 时点快照；M1 顺序是 `zr_math/zr_resource → zr_contracts → zr_kernel → zr_diagnostics`。审查必须阻止 `asset→text=2`、`scene→animation=2`、`rhi→rhi_wgpu=1` 三条生产反向边进入新 crate，并先把 source-cubemap projection 的两处 concrete Runtime `TaskPool` 测试移交 kernel integration owner。公开 facade 只做最终 curated surface，不能成为旧内部 owner、alias、shim 或兼容双轨。该同步状态为 `static-baseline-implemented / timings-pending / physical-migration-not-started`；共享源码继续演进时，物理迁移前必须重新原子采集，不能把本时点快照冒充永久 current source。
