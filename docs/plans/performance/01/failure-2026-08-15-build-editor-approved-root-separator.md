...
Open state: current source accepts a unique `D:\ZirconBuilds` child through the
official product-staging acquire/release preflight, and the live artifact audit
reports no unmanaged path. The current Windows PowerShell Pester suite passes
17/17. No product Cargo build, bundle publication, smoke launch, WPR/xperf
