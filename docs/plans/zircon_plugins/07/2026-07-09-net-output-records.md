...
|------|------|------|---------------|----------|------|
| 2026-07-13 | M2 HTTP route/body 生命周期回归修复 + 其余 feature testing stage | HTTP 回归已修复；HTTP/Replication/RPC/WebSocket 复测完成 | `features/http/runtime/src/backend/server.rs` 在路由未命中时流式丢弃 `Incoming` frames 后返回 404，不分配 body buffer；已命中路由继续走 1 MiB `Limited` 并在超限时返回 413，未改路由优先级或客户端成功路径。`docs/zircon_plugins/net/runtime.md` 同步行为、根因和测试证据。 | RED：`http_feature_manager_matches_route_before_applying_body_limit` 精确运行 100 次失败 2 次，HTTP suite 10 次失败 3 次，错误为 `Io("client error (SendRequest)")`；根因为未消费 HTTP/1 request body 就返回 404 导致连接关闭竞态。GREEN：focused Cargo 1/1；精确回归 200/200；八线程 HTTP suite 30/30；Windows 固定 1.94.1 MSVC、保留式隔离 target、`--offline --locked -j 2` 完整第二批为 HTTP 11/11、Replication 9/9、RPC 27/27、WebSocket 8/8（55/55）。结合下一行 Content Download/Reliable UDP/base runtime 48/48，本轮 Net runtime/feature 复测总计 103/103。独立 reviewer 结论 Ready，无 actionable findings。锁文件未改。 | Net 现有 runtime/六 feature 测试执行缺口关闭；M6 BLAKE3 hard cut 仍由 Runtime 04 open handoff 持有，不能据 103/103 宣称整份 07 完成。 |
| 2026-07-13 | M5/M6/M7 runtime 待重跑测试收束 + M6 hash 契约复核 | Reliable UDP、Content Download 旧语义与 Net runtime 复测完成；M6-T2 BLAKE3 契约仍为 `open`，不声明整项完成 | 未改生产源码；复核当前共享 DTO 已硬切到 `zircon_runtime/src/asset/pack/manifest.rs`，但共享 `zrpack_content_hash(...)` 仍为四路 FNV-1a 拼接，下载侧仍为独立 SHA-256；已创建 Runtime 04 交接 [`../../zircon_runtime/runtime/04/failure-2026-07-13-zrpack-blake3-contract-drift.md`](../../zircon_runtime/runtime/04/failure-2026-07-13-zrpack-blake3-contract-drift.md) | Windows 固定 1.94.1 MSVC 工具链、保留式隔离 target、`--offline --locked`：`cargo test --manifest-path zircon_plugins/Cargo.toml -p zircon_plugin_net_reliable_udp_runtime -p zircon_plugin_net_content_download_runtime -p zircon_plugin_net_runtime --offline --locked` 通过 48/48（Content Download 15、Reliable UDP 12、Net runtime 21）；`interrupted_download_resumes_from_bitmap`、`corrupt_chunk_refetched`、`thirty_percent_loss_delivers_in_order`、`oversize_payload_fragments_and_reassembles`、`diagnostic_paths_registered`、`net_runtime_diagnostics_records_bandwidth_counters` 均真实执行通过。第一次 target 被临时清理、第二次 rustup stable 更新回滚均属于验证基础设施失败，未计为代码失败或 pass；第三次固定工具链后完成。锁文件未改。 | M5 测试执行缺口与 M7 runtime diagnostics 缺口关闭；M6 仅关闭“旧 SHA-256 路径测试待重跑”，须由 Runtime 04 完成共享 BLAKE3 hard cut，并向上重跑 ZrPack writer/reader/delta + Content Download 集成门禁。 |
