...
本次 D10 animation/physics bridge call migration（2026-06-28）：Physics runtime 现在按本计划 §6 的 `physics.query.v1` 方向导出 runtime-owned `PhysicsQueryInterface`，并通过 SDK `RuntimePluginModuleRegistration::export_interface::<dyn PhysicsQueryInterface>(...)` 进入 frozen bridge table。Animation/physics 合约测试通过 `WeakBridge<dyn PhysicsQueryInterface>` 调用 ray cast、shape overlap 和 shape cast，不再解析 concrete `DefaultPhysicsManager`。Runtime 15 状态锚为 `d10_animation_physics_bridge_call_static_passed_cargo_deferred`，守卫为 `review_d10_animation_physics_tests_use_sdk_bridge_call`；Cargo gate 按实施切片节奏 deferred。
...
```
zircon_runtime/src/core/framework/bridge/mod.rs    [新增] PluginInterface / BridgeError / BridgeInvocationTable
...
| M4-T2 | VM 经现有 host export module 接桥（已落地 `zr.zircon.bridge` builder、中性 descriptor / `BridgeInvocationTable` 边界、`ScriptCallTable` dense call site 和真实 `zr_vm` backend 预解析接线；package manifest 注册 helper 已在 2026-07-13 Frameworks05 硬切中删除） | script/vm/host/bridge_host_module.rs、core/framework/bridge/mod.rs、script/vm/host/script_call_table.rs、script/vm/backend/zr_vm_project_backend/real_backend/host_modules.rs、08 host_interface 接线 | 08-M2、M4-T1 | `bridge_host_module_dispatches_vm_calls_through_resolved_bridge_slots`、`bridge_host_module_reports_disabled_bridge_to_vm_callers`、`test_script_bridge_host_does_not_depend_on_plugin_manifests`、`script_call_table_pre_resolves_host_export_callbacks`、`zr_vm_real_backend_uses_script_call_table_for_host_callbacks`、`bridge_performance_baseline_vm_bridge_callbacks_capture_resolved_slot`、`bridge_performance_baseline_script_call_table_calls_dense_id_without_name_lookup`、`bridge_performance_baseline_real_zr_vm_callbacks_capture_call_sites` |
