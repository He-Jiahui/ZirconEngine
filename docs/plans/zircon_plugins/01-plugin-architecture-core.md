...
> **当前恢复状态（2026-08-10）**：仍为 `resolving_failure`。M1–M5 表中的 25 个命名验收测试入口已在当前源码逐项找到，证明计划的静态测试边界完整，但不替代 current-source 动态验收。Native hot-reload 已通过受管 core-min focused gate并获两轮独立复审 `C0/I0/M0`；上次 Plan08 commandlet 的五个 graphics 编译点已在当前所有者路径静态确认修复，但当前仍有一个外部受管 Cargo/rustc 任务运行，尚未取得 fresh upward target-test 终态。当前唯一已确认的 Plugins01 算法回归是 discovery 的 `refresh_manifest` / `remove_path` 丢弃变更路径并提交 `RootScan`；Frameworks04 保持修复责任。性能 M0 的 development/release/profiling profile 边界已通过 `19/19` 纯契约、`17/17` 受管 CLI 验收与独立复审 `C0/I0/M0`，且 failure 已回传；M1 基准工具静态契约已完成，但其 current-source 编译、immutable source-manifest 绑定、release 吞吐/功耗与 profiling CPU 栈取证仍待 FIFO 授权。吞吐/功耗必须用 release，ETW CPU 栈归因必须用同一 immutable source 的 profiling；基线、优化后复测、里程碑 commit 与企微量化结果均未完成。

> **当前恢复状态（2026-08-13）**：仍为 `resolving_failure`。M4 Step 1 已把 `host_api_adapter` 的纯 ABI 解码（UTF-8/slice、V4 header/access、stage 与 affinity）移入私有 `abi_decode` 目录；静态边界契约 `3/3`、native benchmark harness 契约 `12/12`、Rust `1.94.1` scoped `rustfmt --check` 与 `git diff --check` 均通过。受管 focused job `784cbb25cd9148b5aa40c3029826a3f1` / run `0c318642355048afb0fd306e615b5d57` 在 `syn` 编译时因 coordinator 分配的 `D:\cargo-targets\zircon-engine\plugins01-host-api-abi-decode-20260813` 缺少 `proc_macro2` rmeta 而以 `101` 终止，未进入 `zircon_runtime`、零个目标测试执行；后续不同盘符、不同 fresh target 的 job `a790e2f9673c462fafb02b3558628d47` / run `3c32d521abc4427c9663ce39a6a6005e` 又在 `cfg-if` 写入 `.fingerprint` 时因路径消失以 `101` 终止，同样零个目标测试。最低已证原因位于 coordinator-managed target 物化/压力清理生命周期，canonical open handoff 已迁入 Coordinator01：[`host-api-abi-decode-target-cache-rmeta-missing`](../zircon_tooling/session_coordinator/01/failure-2026-08-13-host-api-abi-decode-target-cache-rmeta-missing.md)。不得将此静态迁移或历史 warmed binary 记为 dynamic GREEN；须待 Coordinator01 以并发 TDD、durable deletion provenance 和 fresh target 验证返回后，以同一 current source 重跑 focused、ignored benchmark 与 broad/review，foreign validation copy 保持未触碰。

> **M4 当前源码进度（2026-08-13）**：仍为 `resolving_failure`，尚未验收。计划规定的五个私有责任目录 `abi_decode`、`context_handles`、`bridge_scope`、`registration_policy`、`ecs_registration` 已完成零行为迁移；旧 `context_registry.rs` 与聚合 `host_api_adapter/tests.rs` 已硬切删除，根适配器从 1,161 行收束为仅模块声明和窄 public type re-export。注册与 bridge handle 仍共享同一 tagged generation namespace，context writer 只由 `context_handles` 的窄操作暴露；bridge scope 继续持有 library-generation owner，registry mutation 只存在于 `ecs_registration`。ABI、handle/closeout、bridge、V3 ECS 与 V4 policy/authorization 测试均已迁入各自 domain 的 `tests.rs`。runtime 新增 cloneable `NativePluginHostHandle` / weak handle，app 与 Editor 生产代码已硬切到该类型化边界及 runtime-owned discovery/load 函数；`NativePluginLiveHost` / `NativePluginLoader` 已从 `plugin::native` facade 移除，上层具体 backend 引用和 Editor 的 `native_plugin_live_host` 旧标识符均为 `0`。复审发现首版 handle 只代理常用路径、会缩减正式 API，已用 RED 契约复现后补齐：handle 现保留 concrete live host 的 35 个公开方法，runtime-owned 函数保留 loader 的 11 个发现、增量刷新、移除和 load 入口，不恢复 backend 导出。静态契约现在从 Rust `impl` 块自动枚举 backend 能力，并与 typed handle/runtime-owned 函数做双向集合相等校验，后续新增 backend 方法但遗漏上层投影会直接失败。当前静态证据为 adapter/upper-layer boundary `13/13`、native benchmark harness `12/12`（组合 `25/25`）、Rust `1.94.1` scoped `rustfmt --check` 与 `git diff --check` 通过；独立 hot-reload publication/rollback 复审为 `C0/I0/M0`。尚无本次完整迁移的 managed current-source 编译或行为测试结果；两次 fresh target 均在基础依赖物化阶段以 `101` 终止且零个目标测试，canonical failure 已交给 Coordinator01。外部 validation copy `5945e3ef29d74bd69602adca02e243b5` 保持未触碰，M4 不得标为 dynamic GREEN 或 accepted。
...
>
> **测试命名追溯修正（2026-08-13）**：上述 `plugin_resource_event_and_system_registrations_apply_to_world` 于 2026-08-03 仅硬切更名为当前计划表使用的 `plugin_resource_available_to_systems`；测试体仍覆盖 resource/event/system 注册、ownership、`apply_to_world`、system 执行与 event catalog，不应把历史旧名误报为当前源码缺口。
...
| 里程碑 | 切片 | 状态 | 完成日期 | 证据 |
|---|---|---|---|---|
| M4 / 后续结构收束 | native adapter 五域拆分、typed host/loader 上层边界与 hot-reload publication rollback | `static_complete / managed_dynamic_validation_pending` | 2026-08-13 | `host_api_adapter` 根已收束为声明/窄重导出，五个私有责任域与各自测试 owner 完成；app/Editor 生产 concrete backend 引用为 0。`NativePluginHostHandle` 精确投影 live host 35 个公开方法，11 个 runtime-owned 函数精确投影 loader 公开入口；静态守卫从 Rust `impl` 自动枚举并校验集合与委托关系。计划表命名验收入口 current-source `25/25`，三组 Plugins01 Python 静态契约 `27/27`，scoped Rust `1.94.1` rustfmt 与 diff-check 通过，hot-reload rollback 独立复审 `C0/I0/M0`。两次 fresh managed target 均在基础依赖物化阶段 `exit 101`、目标测试 0，Coordinator01 canonical failure 仍 open；本行不声明 dynamic GREEN。 |
