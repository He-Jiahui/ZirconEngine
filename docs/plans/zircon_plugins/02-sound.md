...
- 2026-07-22 SoundAsset导入性能交接：PERF-MVP-524已删除WAV PCM16/32/float逐sample checked Result，但资产仍在导入时把完整source解码为完整`Vec<f32>`。Plugins02按PERF-MVP-528消费Runtime04 `Arc<[u8]>`/metadata generation与Runtime11有界decode ring；短音效可prepared，长音频按prefetch/seek/cancel预算流式解码，audio callback分配/锁=0。参考Bevy `AudioSource { bytes: Arc<[u8]> }`和实例`Decodable`，同时保留Kira唯一后端合同。
- 2026-07-30 Performance01 current-source交接：PERF-MVP-586归M5 dynamic events。按catalog generation编译event-id→pre-sorted handler/executor slots，pending必须有entry+bytes+age预算，payload单一shared owner，drain有count/time/bytes预算，任意executor经Runtime11 bounded ticket/explicit affinity执行并具timeout/cancel/late-result合同；禁止caller同步等待foreign executor。
- 2026-07-30 Performance01 current-source交接：PERF-MVP-587强化M5-T1。timeline binding/curve在schedule或graph generation一次验证编译为direct slot+Kira Tween/Clock/Modulator计划；active sequence稳定slot/cursor推进、scratch复用、sample report opt-in，state mutex不得覆盖sequence×track×keyframe工作。
- 2026-07-30 Performance01 current-source交接：PERF-MVP-588归M1/M5-T2。immutable graph generation与高频meter ring/cursor分离，stable UI poll只取handle/delta，full capture显式预算；Kira production边界hard-remove手工`render_mix`/backend callback owned-block API或限定offline test，audio callback分配/锁=0。
- 2026-08-01 current-source同步：shared `SoundPluginOptions`、package manifest与framework sound默认值单测均已统一为唯一后端`kira-cpal`，退役`software-mixer`不再作为default/alias保留。静态修复已完成；managed Windows作业`38911e64b09e4cefa6b2e5d6069676f6`已释放，但runtime lib-test编译因15个其他owner错误退出`1`（内部Cargo `101`、327 warnings、目标测试执行`0`），所以不得把default contract或PERF-MVP-586..588记为动态验收完成。
