...
- 2026-07-30 current-source gate前置：shared `SoundPluginOptions`和package manifest已统一`kira-cpal`，但framework sound单测仍断言退役`software-mixer`。Plugins02验收必须机械更新该期望且不得恢复compatibility default/alias；在managed Cargo和PERF-MVP-586..588规模counter通过前，framework sound只记static complete。
