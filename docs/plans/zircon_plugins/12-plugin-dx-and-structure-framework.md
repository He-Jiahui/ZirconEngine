...
- 2026-07-22 event mirror生命周期性能交接：公开connected subscription直接drop不会回减World reader/callback count，Navigation等按需producer可因此永久保持debug capture；Plugins12已实现direct token、dynamic session、World destroy、process-log worker与plugin reload统一的generational owner/reclaim重试状态机。`resolving_failure / source implemented / validation pending`：[`12/failure-2026-07-22-runtime-event-mirror-drop-lifecycle.md`](12/failure-2026-07-22-runtime-event-mirror-drop-lifecycle.md)，见PERF-MVP-455；未宣称性能数值或fixed return。
- 2026-08-10 neural skeleton/manifest交接：完整40文件package、root runtime manifest、linked feature delegation、真实ONNX authoring operation、runtime/editor catalog、App wiring、项目asset-root路径authority及16-family空扩展回归已实现；Neural同时进入runtime capability与editor/runtime mirror强制审计。snapshot1560 的属性/shape/initializer 修复后，exact77 二审仍以 `C0/I4/M1` 拒绝终端 view 假可执行、Pool 全padding空窗口、Resize `f32 -> u32` 饱和、非输出大张量索引及全范围格式。当前已继续前向拒绝终端 Reshape/Flatten graph output，要求每个 Pool 窗口命中输入，对全部张量实施 V1 `u32` 元素地址上限，并只接受精确可表示的整数 Resize scale；exact 61 个 Rust 输入已统一通过 Rust 1.94.1 格式门，shape owner 保持在 `onnx/executable_contract.rs`，`convert.rs`/contract/tests 为 773/326/898 行。`resolving_failure / exact77 review repair implemented / fresh candidate rebuilding`：[`12/failure-2026-08-10-neural-plugin-skeleton-and-manifest-regression.md`](12/failure-2026-08-10-neural-plugin-skeleton-and-manifest-regression.md)；snapshot1552/1554/1558/1560/exact77及其旧票据均不可用于验收，新不可变快照、terminal Cargo、最终二审与accepted均未宣称。
