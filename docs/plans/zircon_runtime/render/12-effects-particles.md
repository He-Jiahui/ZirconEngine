...
|------|-------------|------|------|------------|------|
| 2026-06-23 | Render index 当前状态总览拆分 | FX-M1/M2 部分完成,FX-M3/FX-M4 未启动;GPU particle backend owner、runtime-prepare 多 system 聚合、indirect args 生成、shared-owner transparent draw、offscreen visual readback、count parity 与 scene neutral GPU-frame auto-collection 已在 focused lane 起步 | 从 docs/plans/zircon_runtime/render/index.md 的第 9 节迁入本计划；本行保留 12 Effects/Particles 的当前事实，render 总索引不再维护计划级明细。 | 文档重组；本次未改生产代码，render/index.md 只保留状态路由说明。 | 仍未完成：decals/projector 收敛、lens flare/halo、GPU transparent draw product/RenderDoc 验收；验收缺口：需要 transparent draw 产品对拍、decal receiver filtering、flare occlusion/product tests、concrete scene-to-manager `ParticleSystemComponent` integration 与 wider CPU/GPU image parity |
...
| 2026-06-18 | FX-M2 particle GPU indirect transparent focused readback | 部分完成: GPU backend owner、alive compaction、indirect args、count parity 与 shared-owner transparent draw 已有 focused WGPU evidence;三模式/product 场景仍未闭环 | 计划 01 RG-M1 的 particles runtime-prepare owner 已把 playing GPU systems 聚合进一个 concrete `ParticleGpuBackend`,写出 alive indices 和 non-indexed indirect draw args,并让 `particle.transparent` 消费同一 shared owner。`tests/gpu.rs::particle_gpu_runtime_owner_records_transparent_draw_from_executed_backend` 现在对 32x32 offscreen `Rgba8Unorm` target 执行 transparent indirect draw 后做 RGBA8 readback,断言写入非透明像素。`render_particle_cpu_gpu_parity_small_scene_matches_counts_and_indirect_args` 用同一 burst-only asset 对比 CPU fallback extract 与 GPU backend readback 的 live/spawn counts,并确认 indirect args word1 等于 CPU live count。 | `cargo test -p zircon_plugin_particles_runtime particle_gpu_runtime_owner --locked --jobs 1 --target-dir D:\cargo-targets\zircon-particles-gpu-owner-0618 --message-format short --color never -- --nocapture` 通过(4 focused owner tests,含 offscreen visual readback;既有 warning set)。`cargo test -p zircon_plugin_particles_runtime render_particle_cpu_gpu_parity_small_scene_matches_counts_and_indirect_args --locked --jobs 1 --target-dir D:\cargo-targets\zircon-particles-gpu-owner-0618 --message-format short --color never -- --nocapture` 通过(1 focused parity test)。期间修复 GPU readback helper map-offset alignment 与 per-emitter spawned counter normalization。 | 补 `render_product_particle_three_modes`、`render_product_particle_cpu_gpu_match`、per-view particle sorting、concrete scene-to-manager `ParticleSystemComponent` integration 与 RenderDoc resource/marker 对拍。 |
| 2026-06-18 | FX-M2 scene neutral particle GPU-frame auto-collection | 完成(focused): visible scene dynamic particle payloads can feed the neutral runtime-prepare GPU-frame fallback path | `World::collect_render_particles(...)` 现在从 `render.particle_sprites` / `gameplay.particle_sprites` 动态组件解析可选 `gpu_frame`,按 camera layer / active hierarchy 过滤后聚合 `alive_count`、`spawned_total`、`per_emitter_spawned` 与 `[6,alive_count,0,0]` indirect args,并把可选 GPU bounds 合并进粒子 emitter bounds。该路径只生产 `ParticleExtract.gpu_frame`;不把 concrete particles manager、WGPU 或插件依赖引入 scene。 | `rustfmt --edition 2021 zircon_runtime/src/scene/world/render_particles.rs zircon_runtime/src/scene/tests/render_extract.rs` 通过;`cargo check -q -p zircon_runtime --lib --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-scene-particle-gpu-frame-0618` 通过(既有 warning set);`cargo test -p zircon_runtime --lib render_frame_extract_collects_dynamic_particle_gpu_frames_by_camera_layers --locked --jobs 1 --target-dir D:\cargo-targets\zircon-runtime-scene-particle-gpu-frame-0618 --message-format short --color never -- --nocapture` 通过(1 focused scene test)。 | 继续 product scene/RenderDoc 对拍、concrete `ParticleSystemComponent` scene-to-manager integration、per-view sorting、三模式渲染与更宽 CPU/GPU image parity。 |
...
- 2026-07-18 particle history诊断交接：submission同帧为previous-state与anonymous ambiguity重复构建anonymous BTreeMap/BTreeSet及previous identity BTreeMap，至少三轮current扫描。Particles owner需产出一次`ParticleHistoryMatchReport`，以stable sprite key+dense/reused hash scratch单遍汇总matched/missing/ambiguous并由stats复用；见PERF-MVP-341。
- 2026-07-18 neutral fallback资源抖动交接：`runtime_prepare`在persistent owner缺席时每帧clone `gpu_frame`并创建particles A/B、emitter、counter、alive、indirect、debug七类GPU buffer，empty frame也建最小资源。FX-M2须让fallback接入per-device/viewport persistent owner，按capacity增长复用、dirty-range写入、static binding IDs；验收empty/stable create=0、buffer总数≤7，见PERF-MVP-348及sideband/relevance静态证据。
- 2026-07-18 plugin prepare线程边界交接：particle等runtime collector当前在submission线程串行执行，允许同步准备和owned payload回传。FX-M2联动Plugins01/Render03/18把simulation/CPU prepare发布为有界generation artifact，render callback只record/apply ready buffers；stable heavy prepare/payload copy/binding rebuild=0，未ready使用last-good/neutral且统计age/drop。见PERF-MVP-379。
- 2026-07-18 legacy particle renderer逐文件交接：`scene_renderer/particle/**` 19/19确认current sprites按depth/overlay双扫描、velocity重建anonymous+previous树并第三次扫描，三路均CPU展开6 vertices且每frame新建buffer/pass；constructor另建3条eager pipelines。FX-M2既定整目录删除现在由PERF-MVP-396量化约束：唯一instance/history/indirect artifact、vertex-stage展开、stable create/upload=0、particle-off pipeline=0，不接受旧目录局部cache续命。
- 2026-07-18 particle feedback/history owner交接：camera-slot previous result Vec现复用容量，history兼容不再重复比较；但camera stack每camera收集particle GPU feedback，非终端camera直接丢弃，anonymous/identity分类仍由stats/velocity/history多次重建。FX-M2发布viewport simulation generation的唯一feedback ticket和`ParticleHistoryMatchReport`，只由shared-product owner消费，reused dense scratch单次分类；见PERF-MVP-415并复用341/396。
- 2026-07-22 scene particle extract交接：neutral dynamic JSON路径已从每camera全World entities×4外层map probes改为只访问排序dynamic owners、每owner一次外层lookup，并删除emitters/bounds重复sort；仍需FX-M2与Runtime07把JSON authoring payload在change/import边界编译为typed/revision emitter artifact，stable generation跨camera extract/parse/sort=0，GPU档只传buffer/indirect handle。见PERF-MVP-465与`docs/plans/performance/01/2026-07-22-runtime-scene-world-render-projection-static-review.md`。
