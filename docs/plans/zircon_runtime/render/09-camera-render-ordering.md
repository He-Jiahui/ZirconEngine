...
- 迁入记录：[`09/2026-07-09-camera-render-ordering-output-records.md`](09/2026-07-09-camera-render-ordering-output-records.md)
- 2026-07-18 phase queue性能交接：sorted queue的每stage全表filter与summary每item线性查row已由phase-order partition span/O(1) index止损；每camera/frame仍重复物化phase inputs/items并全量排序。CO-M3需扩展为retained per-view generation phase owner、复用storage和明确phase ranges，stable generation rebuild/sort=0；见PERF-MVP-339。
- 2026-07-18 camera sequence性能交接：无planar capture的camera-list无条件clone和borrowed resolver预拷贝已直接止损；scene order report仍新建Vec/BTreeSet/BTreeMap并复制descriptor，base stack reference仍O(S×C)线性find。CO-M1/M2须建立generation camera artifact、entity index和复用排序storage，使order report/sequence/submission共享handle，stable sort/build=0且lookup近O(C+S)；见PERF-MVP-345及`docs/plans/performance/01/2026-07-18-runtime-core-framework-render-camera-view-static-review.md`。
- 2026-07-18 mesh phase投影补充交接：material-adjusted queue的per-item `phase_inputs.iter().find`已由dense mesh-index表从O(M²)止损为O(M)，但queue物化、material offset读取和全量sort仍每camera/frame发生。CO-M3须发布per-view generation phase inputs/ranges并共享scene-static排序部分；stable build/sort/material lookup=0、changed build≤1/affected generation。见PERF-MVP-387及build-mesh-draws静态证据。
- 2026-07-18 mixed transparent ordering交接：mesh/sprite混排已预分配并改in-place sort，但仍先presence scan、再物化M+S并全量排序；两producer当前tie-break合同不同，不能直接merge。CO-M3联动Render02/14发布统一`(sort_key,entity,source,index)` generation ranges，stable build/sort=0、changed linear merge。见PERF-MVP-339及transparent静态证据。
- 2026-07-18 camera clear policy交接：当前clear intent未携带full-target/partial与stack-first identity，导致所有camera先画region clear。CO-M1/M2须发布coverage-aware attachment policy：full-target首camera由Render01首次attachment write clear，split/overlay保留region draw；full-target region pass=0、partial color+depth≤1。见PERF-MVP-394。
- 2026-07-18 viewport camera slot交接：CO-M1/M2为compiled camera artifact发布稳定`ViewportCameraSlotId`，entity+stack/target slot为identity，viewport/layer/type变化进入validation generation；anonymous camera用generation内稳定order slot。Runtime07据此合并7张状态表并prune removed slot，保持history continuity且动态设置不泄漏旧state。见PERF-MVP-410。
