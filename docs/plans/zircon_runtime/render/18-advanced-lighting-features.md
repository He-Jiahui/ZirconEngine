...
- 2026-08-27 SSS prepared-frame实现：setup成为SSS params/profile的唯一per-camera producer，80/512-byte uniform由RDG transient resource pool提供并以一个592-byte immutable payload进入帧上传事务；scatter只消费graph handles，不再重复resolve profile table、求逆、clone profiles或每帧创建buffer。indirect reset改为encoder clear + setup shader恢复Y/Z=1。源码/静态契约通过；真实WGPU、截图、RenderDoc、300帧profile与功耗仍待统一验收，见`docs/plans/optimize/zircon_runtime/90/2026-08-27-pfo-4d1p-subssurface-rdg-prepared-frame-plan.md`。
- 2026-08-30 Hybrid GI exact history：plugin resolve 的 lighting/temporal-metadata previous history 已从 report-only view 迁为 View-sized `Rgba16Float` typed external access-ID lease；renderer owner/binder 发布完整 physical backing/descriptor。该切片只修正资源身份和执行接线，不改变 Lumen/Hybrid GI tracing 或 temporal weighting；静态格式、metadata、source/diff checks 通过，受管 Cargo/WGPU、PNG/RDC、profile/功耗仍 pending，状态为 `render_plan18_hybrid_gi_exact_history_source_implemented_dynamic_validation_pending`。
- 2026-08-30 Volumetric dynamic 3D exact history：previous scattering history 已从 descriptor-less report-only view 迁为 catalog-backed compute sampled access-ID lease；shader quality 统一决定 current/previous `160x90x48/64/96` D3 geometry，renderer owner/binder 发布 physical texture/view/descriptor。UE 对照为 `RegisterExternalTexture(LightScatteringHistory)` + current scattering extraction。该切片不改变 temporal weight/jitter/rejection、lighting 或 layout 算法；静态格式、metadata、source/diff checks 通过，受管 Cargo/WGPU、PNG/RDC、profile/功耗仍 pending，状态为 `render_plan18_volumetric_dynamic_external_history_exact_lease_source_implemented_dynamic_validation_pending`。
