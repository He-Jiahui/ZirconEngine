...
|------|-------------|------|------|------------|------|
| 2026-07-05 | EL-M1 / sampled HDRI mip 链纠偏 | 中间实现,待最终验证 | 镜像 shader 06 EC-M1a:scene group0 binding1 新增 read-only environment sample storage buffer;`SkyboxMode::SampledEquirectangular` 的 16x8 table 替换为 128x64 base + 8 mip 链;scene uniform 删除内嵌 `environment_samples`;skybox 与 PBR helper 使用 bilinear/lod 采样,roughness→mip 对齐 UE reflection capture 公式,真实 HDRI export 改写为 `runtime_shader_pbr_real_hdri_lakes_pmrem_reflection_20260705.png`。 | 静态扫描:旧 `16x8` 常量、旧 PNG 文件名、uniform 内嵌 sample table 残留为 0;`git diff --check` 通过(仅 CRLF 提示);`cargo check -p zircon_runtime --lib --no-default-features --features core-min --locked --jobs 1 --target-dir E:\cargo-targets\zircon-hdri-pmrem-core-check-0705 --message-format short --color never` 通过(9m46s);focused unit test 在并发默认特性链接负载下 304s 超时,不计通过证据。 | 继续补跑默认特性 `cargo check -p zircon_runtime --lib --locked`、shader string/Naga tests 与 ignored screenshot export;随后按 shader 06 EC-M1 硬切 GPU `texture_cube` 源 skybox,EC-M2 接 GGX FIS/SH9/BRDF LUT。 |
