...
- 2026-08-27 cubemap GPU上传交接（PFO-4d1n源码闭合、动态待验收）：source/PMREM/irradiance现由进入renderer前的current、row-aligned immutable artifact完整提供，no-IEM预编码1x1 black cube；changed key只打包一个共享staging payload、向唯一`FrameBufferUpload`追加一个range并记录每mip encoder copy，stable key转换/上传=0。旧submission-thread f32转码与face×mip `write_texture`回退已删除，revision只在scene submit成功后commit。真实WGPU HDRI/no-IEM PNG、RenderDoc、changed/stable profile和功耗仍待里程碑验证；见PERF-MVP-380与PFO-4d1n计划。
- 2026-08-27 system texture owner 前置切片（PFO-4d2a0源码闭合、完整owner与动态待验收）：冷启动source/specular/irradiance三个回退槽现共享一张1x1x6 black cube和同一个view，原生texture/view create与48-byte上传均由3降为1；filtering sampler只在cubemap owner构造时创建，真实环境尺寸/mip rebind不再重建或替换sampler。该结果仍是per-core内部物理身份收敛，不能冒充BRDF/black cube/sampler跨core或shadow renderer共享；device-generation owner、PNG、RenderDoc、profile和功耗继续pending，见PFO-4d2a计划。
- 2026-08-27 shadow environment binding lease（PFO-4d2a1源码闭合、动态待验收）：full-scene shadow不再为完整scene layout另建3张cube、1张BRDF texture、sampler和SH9 buffer；scene bundle发布同generation只读lease，shadow的source/specular/irradiance三槽共享已上传black cube，BRDF/sampler/SH9复用scene资源。shadow cold增量由4 texture + 4 view + 1 sampler + 1 buffer降为0。每atlas slot的camera uniform/bind-group仍是独立热路径；真实shadow WGPU、PNG、RenderDoc、profile和功耗继续pending，见PFO-4d2a计划。
- 2026-08-27 shadow slot scene workspace（PFO-4d2b源码闭合、动态待验收）：每atlas slot、每帧的`create_buffer_init`与bind-group create已删除；outer frame按device uniform alignment把active slot camera uniforms打包进一个payload/一个upload range，grow-only workspace只在2倍capacity增长时创建一个buffer并重建固定offset bind groups。稳态native create N/N→0/0，recorder queue权限为0。1/4/16/64 slot真实WGPU、PNG、RenderDoc、CPU/GPU profile和功耗继续pending，见PFO-4d2b计划。
- 2026-08-27 deferred provider binding发布（PFO-4d2c源码闭合、动态待验收）：environment-only reflection placeholder首次升级为完整provider时，frame setup现先向deferred owner发布最终buffer/texture binding lease，再切换generic environment variant并清理旧mesh command cache。此前deferred只保存构造期placeholder bindings，旧`Arc`合法存活会导致静默采样占位资源；该正确性缺口已在源码边界闭合。真实provider注入PNG、WGPU validation与RenderDoc resource identity继续pending；forward receiver bind-group缓存需先取得profile证据，未在本切片擅自实施。
- 2026-08-27 lightmap atlas generation binding（PFO-4d2d源码闭合、动态待验收）：lightmap不再仅凭`ResourceId`判定GPU binding仍current。streamer现从同一prepared row返回revision与resource lease；asset id、revision、physical `Arc` identity全部相同才允许早退，同ID热重载或同revision residency替换都会发布新view/sampler给forward/deferred。missing prepared row不再借fallback伪装ready。真实hot reload/residency PNG、WGPU validation与RenderDoc identity继续pending。
...
- 2026-07-18 shadow neutral环境补充交接：`ShadowMapRenderer`重复环境对象已于2026-08-27 PFO-4d2a1源码收敛为scene-generation binding lease，增量原生资源create为0；PERF-MVP-390的动态WGPU identity、截图、RenderDoc、profile与功耗验收仍未关闭。
