...
- 2026-08-30 HZB 历史资源合同已从 report-only view 收敛为精确 external lease：feature 声明 compute sampled full-range access，pipeline catalog 从 SceneLinear allocation 经唯一 `HzbBuilder` 解析动态尺寸与 mip，history binder 发布 physical texture/view/descriptor；compiled packet 因而把 full range canonicalize 为真实 mip 数。状态锚点 `render_plan04_hzb_dynamic_external_history_exact_lease_source_implemented_dynamic_validation_pending`。该切片只修正 RDG 所有权/访问合同，不修改 cull shader、mip 选择、depth bias 或 compact replay 算法；受管 Cargo/WGPU、产品墙场景 PNG、RenderDoc mip/barrier 和 300 帧 profile/功耗仍待完成。
- 2026-08-30 viewport hit-proxy失败事务顺序已收敛：GPU-scene upload不再在bounded diagnostic admission/prepare之前进入共享submission queue，也不在terminal hit-proxy packet提交前commit。当前固定顺序为diagnostic prepare、upload enqueue、command+diagnostic terminal submit、GPU-scene commit；预算拒绝和prepare失败不会留下可被后续无关frame flush的孤儿upload。516行owner内的源码回归锁定严格顺序与唯一enqueue/commit调用点。该切片不改变draw/readback算法或submit数量；受管Cargo、真实WGPU picking、PNG/RDC和profile仍pending，状态为`render_plan04_hit_proxy_submission_transaction_order_source_implemented_static_checks_passed_dynamic_validation_pending`。
