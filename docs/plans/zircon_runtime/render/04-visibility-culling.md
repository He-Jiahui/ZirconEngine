...
- 2026-06-22 追加 VC-M2 SSAO 共享 HZB 产品路径守卫:`ssao_quality_profile_darkens_scene_when_enabled` 现在在真实 headless WGPU SSAO 产品帧中断言 `hzb-build` 与 `ssao-evaluate` graph pass/executor 已执行、HZB mip 数大于 1、HZB graph work 非零、`hzb-furthest` transient alias 已物化、required materialization 无缺失,并保留最终 SSAO darkening 产品断言。
