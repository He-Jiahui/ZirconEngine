...
related_code:
  ...
  - zircon_runtime/src/tests/runtime_absorption/structure_convention/production_file_budget/hzb_occlusion_culler.rs
...
- 2026-06-22 追加 VC-M2 SSAO 共享 HZB 产品路径守卫:`ssao_quality_profile_darkens_scene_when_enabled` 现在在真实 headless WGPU SSAO 产品帧中断言 `hzb-build` 与 `ssao-evaluate` graph pass/executor 已执行、HZB mip 数大于 1、HZB graph work 非零、`hzb-furthest` transient alias 已物化、required materialization 无缺失,并保留最终 SSAO darkening 产品断言。2026-06-24 Project render quality/deferred tests owner split 已把该产品守卫从 `graphics/tests/project_render.rs` 迁入 `graphics/tests/project_render/render_quality.rs`；状态锚为 `render_project_render_quality_deferred_tests_owner_split_static_passed_cargo_deferred_implementation_cadence`，结构守卫为 `runtime_15_project_render_quality_tests_are_child_owner`。
- 同轮修复 post-process 资源声明底层问题:启用 effect stack 时 `PostProcessStackDescriptor` 始终声明 `postprocess.tonemapped` 作为 `post.uber` 到 `post.output-transfer` 的 handoff;first-party/plugin rendering descriptor 若复用内建 `post.*` executor id,会通过内建 post-process filtering/routing 处理,因此 `post.uber` 写 `TONEMAPPED`、`post.output-transfer` 在 terminal AA 启用时写入 `FINAL_COMPOSITED` 而不是直接写 `FINAL_COLOR`;`AMBIENT_OCCLUSION` 作为 `post.uber` 的有效输入保持 active,不再随 SSR/effect-stack 过滤丢失。
...
- 2026-06-22 非 RenderDoc 验证补充:`ssao_quality_profile_darkens_scene_when_enabled`、`rendering_plugin_post_process_routes_output_transfer_through_terminal_anti_alias_input`、`enabled_effect_stack_declares_tonemapped_for_uber_descriptor` 均在 core-min focused lib-test 中通过;scoped `rustfmt --edition 2021 --check` 通过。RenderDoc MCP 当前无运行实例,所以 mip 链抓帧、SSR/SSAO 视觉对拍和反射 hit gating 质量确认仍待后续。
...
| `visibility_context_builds_shadow_views_for_atlas_light_slots` | Plan 05 atlas 所需方向光 4 cascade、point 6 face、spot 1 view key 全部生成,且方向光 cascade camera 随 slice 深度产生不同 ortho size/transform | `visibility/context/from_extract_with_history/construct/tests.rs` |
...
| `render_visibility_static_index_incremental_matches_full_rebuild` | 增/删/移对象后 grid 查询结果 == 全量重建结果;`static_index` 过滤测试已通过 core-min lib-test,覆盖增量查询对拍、full rebuild 替换、上一帧索引复用不触发本帧 full rebuild、上一帧索引缺失时安全重建、10_000 静态实例阈值预筛启用且粗筛候选数下降、统计字段映射;渲染框架级静态大场景 source test 覆盖第二帧零 full rebuild 和 main-view prefilter candidate 下降 | `graphics/visibility/static_index/mod.rs`、`context/from_extract_with_history/construct.rs`、`context/from_extract_with_history/construct/tests.rs`、`update_stats/base_stats.rs`、`graphics/tests/render_framework_visibility_submit.rs`(VC-M4) |
