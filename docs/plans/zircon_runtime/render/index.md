...
| 04 Visibility/HZB | VC-M1/M2/M4 完成,VC-M1 legacy visibility 平铺字段已收束,VC-M1/CO-M1 custom-target visibility payload bridge 已接入,VC-M3 compact replay 核心、phase-local dispatch diagnostics 与 HZB phase helper 模块化已接入但仍部分完成;2026-06-18 `hzb_occlusion_culler`/phase-dispatch/indirect-compaction/mesh-indirect-draw/multi-draw focused filters、`render_product_hzb_occlusion_wall_scene` clean rerun、custom-target camera visibility focused tests 与 diagnostics focused tests 已通过,并修复 `post.uber` 对 `light-list` 的缺失声明 | 完整 custom-target WGPU 多相机输出链、更宽 HZB diagnostics sweep | 需要 RenderDoc、完整 render-product/custom-target 输出对拍和更宽 diagnostics focused tests |
| 05 Lighting/Shadows | light buffer/light grid 完成,CSM/PCF/contact shadow 部分完成 | CSM 多光型稳定性、shadow atlas 风险、PCF/contact shadow 真实捕获 | 需要 forward/deferred parity、真实多灯/阴影场景、RenderDoc 与 locked checks |
...
| 09 Camera/Ordering | CO-M1/3/4 部分完成,CO-M2 sequence/output policy 部分完成;CO-M1 `CameraRenderDescriptor`/Base+Overlay sequence resolver、extract-side `RenderViewExtract.cameras`、descriptor-driven scene layer union/custom-target visibility consumer、selected descriptor target/layer submit preflight、descriptor-owned snapshot hard cutover、offscreen submit loop、terminal UI routing、target-sensitive graph cache key、camera clear/load attachment policy、fixed `scene-color`/`scene-depth` physical target binding、stack-terminal output-write suppression、generated offscreen viewport-terminal record/history/stats owner gate、selected-camera `ViewportRenderRegion` viewport/scissor graph-raster enforcement、fullscreen post-process origin policy、terminal output-transfer/FXAA/SMAA region writeback 与 graph-owned Hybrid GI history copy 已接入 core framework/scene/visibility/submit/graphics | editor authoring 面板、最终 custom-target composite、完整独立 per-camera history/light/VG/particle state、统一 sort key、surface present/direct runtime-frame 多相机路径 | 需要 Base/Overlay/custom-target WGPU 合成产品规则、clear/depth/history 产品规则、UI/2D/3D 排序对拍、split-screen product、像素/Product/RenderDoc 验收 |
