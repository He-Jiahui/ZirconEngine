...
- 2026-07-18 mesh-pass command artifact交接：全command预排序后phase再排序及dynamic/cache-ineligible per-draw Vec已删除。MD owner仍须用phase-owned arena/range替代9 Vec搬移，camera-stack有序merge；indirect batch/args/compaction plan每generation只建一次并供execution/stats共享，stable sort/partition/batcher=0。见PERF-MVP-383及mesh-pass静态证据。
- 2026-07-18 pending cache full-hit补充交接：pre-mesh extract即使100% static hit仍为每draw分配capacity3 phase Vec和commands Vec，逐phase clone owned command后再搬入全局表。当前production root由本Render02会话持有租约；请在MD-M2迁移中改固定phase mask/array并直接写generation-owned phase arena，stable hit/pruned per-draw alloc与resource/command clone=0。见PERF-MVP-382及pending-cache静态证据。
