...
2026-08-26 当前落地状态:`RenderMeshSnapshot` 到 `PendingMeshDraw`/`MeshBatchRef` 已携带静态性、geometry/material revision、stable instance/draw identity 与 GPUScene span。`CachedMeshDrawCommands` 只保存 `Arc<MeshDrawCommandPayload>` 的不可变pipeline/material/geometry/direct-topology数据；当前帧 `MeshDrawCommand` 从payload投影source index、sort key、GPUScene instance span和direct args，uncached/indirect payload保持inline。cache key已覆盖stable instance、draw ordinal、phase、disabled passes和shader quality，generation retirement及transform/geometry/material invalidation diagnostics已接入；reflection provider触发environment-only→generic PBR profile单向切换时会在draw构建前清空旧静态payload。pre-MeshDraw extraction会在创建material bind group前吸收eligible static hit与安全的opaque shadow rebuild，visibility-pruned draw仍touch匹配entry以维持source lifetime；phase/command/cache-store暂存已收敛为固定3槽。serial与TaskPool路径共用owner-thread variant/cache transaction和current-view payload projection。尚未关闭通用resolver-configuration typed epoch/diagnostic、单一generation phase arena/ranges、真实current-source managed compile/WGPU产品对拍、WPR/xperf性能/功耗数据、RenderDoc与PNG证据，因此MD-M2不得标记完成。
...
2026-08-26 当前落地状态:已按 Unreal `FMeshDrawCommandStateCache`/`SubmitDraw` 重新复审全部 mesh command replay 调用点。重放器现在区分 owned/borrowed bind identity，命令持有的 `MeshBindHandle` 使用非零单调稳定 ID，不再把短命 `MeshDraw` 字段地址带入可跨帧的 static payload；unbatched indexed-indirect 统计不再误归 direct；OIT pipeline 解析失败会在跳过 draw 前失效已记录的 replay state。shadow atlas 在存在 per-view entity filter 时会关闭 global indirect execution并逐命令过滤，避免以 batch 首命令可见性提交/丢弃整批；无 filter 路径仍保留 multi-draw。详细结论、复杂度与性能验证协议见 `02/2026-08-26-mesh-draw-replay-state-cache-correctness-review.md`。当前仅完成代码与静态回归约束，managed compile/test、真实WGPU产品、RenderDoc绑定序列、PNG及WPR/xperf数据均未闭合，因此MD-M3不得标记完成。
...
- 2026-08-26 mesh-pass command artifact交接：current-source审查确认每个producer仍先建generic command Vec，再由`from_command_list`搬入10个phase Vec；prebuilt/residual各自finalize后，`extend`再次逐phase搬移/排序，half-resolution fallback还会追加一次merge/sort。目标硬切为单一`MeshPassCommandBuildArena`，在half-resolution策略已知后一次seal成只读arena+10个range；先按`02/2026-08-26-mesh-pass-command-arena-architecture-performance-plan.md`采集旧路径WPR/xperf基线，再迁移production ABI，profile前不计PERF-MVP-383通过。
- 2026-08-26 pending cache full-hit补充交接：pre-mesh extract的phase选择、命令暂存和cache-store暂存已改为固定3槽array，stable hit/pruned不再为这些per-draw集合分配heap Vec；命中路径从共享静态payload投影当前sort/source/GPUScene span，不再clone完整owned command或WGPU resource handles。剩余工作是直接写generation-owned phase arena，去掉局部槽到全局表的二次移动，并评估以稳定arena handle替代命中时payload `Arc`引用计数复制；在profile前不计stable路径alloc/clone目标通过。见PERF-MVP-382及pending-cache静态证据。
- 2026-07-18 material binding owner交接：`create_mesh_draw`每draw创建custom/standard两套13-entry bind group，skinned还按draw创建palette group。Render02只消费Render08/03发布的generation-owned dense binding handles，禁止draw层重建；stable create=0、changed material≤2/unique generation、palette≤1/unique pair。见PERF-MVP-384。
- 2026-07-18 deform/VG prepared-handle补充交接：Render02只消费PERF-MVP-385/386/388/389发布的morph descriptor、skin palette、VG segment range和resident mesh dense handle；GPU skin路径禁止先CPU skin/clone primitive，stable draw不得创建mesh/VG buffer或复制静态morph delta。phase输入则消费Render09的generation ranges，禁止在命令侧保留第二排序权威。见`docs/plans/performance/01/2026-07-18-graphics-build-mesh-draws-root-static-review.md`。
- 2026-07-18 MeshPipelineCache owner交接：7条ensure稳定命中已前置返回，Render02仍须把cache构造拆为core state与按需feature bundles；transmission/cookie/irradiance/probe/lightmap/volumetric/OIT neutral资源按device共享，minimal F2不得实例化全部optional真实资源。联动Render11/18与PERF-MVP-390。
- 2026-07-18 shadow replay补充交接：Render02向Render05提供按shadow view过滤后的borrowed/dense command ranges，并让slot scene uniform走persistent ring+dynamic offset；禁止每slot重建command owner、uniform buffer或bind group。shadow atlas单pass内切viewport/scissor后重放，保持pipeline/material/GPU-scene去重状态。见PERF-MVP-391。
- 2026-07-18 Deferred binding补充交接：GBuffer每pass重建forward shadow-receiver group，Deferred lighting每camera构造20+ entry大binding；color attachment Vec已改固定数组。Render02联动Render05/18让geometry/lighting消费resource-generation bundle与dynamic offsets，stable bind-group/entry alloc=0，禁止用无generation的裸WGPU handle cache。见PERF-MVP-368及Deferred静态证据。
- 2026-07-18 object velocity补充交接：空stream+LoadStore现已在begin pass前返回，Clear/Discard保持；有draw仍每pass创建forward shadow-receiver group。Render02联动Render06把velocity command/binding纳入generation bundle，stable bind create=0，empty no-op pass=0。见PERF-MVP-368及temporal静态证据。
...
- 2026-08-26 Render17 PF-M2 handoff：owner-thread variant/cache transaction、immutable `PreparedBatchPlan` worker input、调用方 `TaskPool` 与source-order单点merge已经落地，重复cache key回退serial；serial/parallel current-view投影与统计parity focused guards已补。handoff保持open，仅因current-source managed mesh gate与Render17 PF-M2 runtime gate尚未执行，不得写成fixed。见 [`02/failure-2026-07-31-parallel-mesh-command-preparation-contract.md`](02/failure-2026-07-31-parallel-mesh-command-preparation-contract.md)。
