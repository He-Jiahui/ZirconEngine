...
- 迁入记录：[`14/2026-07-09-2d-stack-output-records.md`](14/2026-07-09-2d-stack-output-records.md)
- 2026-07-18 sprite性能交接：九宫/平铺per-slice matrix与6-vertex临时Vec已直接止损；当前scene Vec/phase queue仍双排序/物化，stats与真实record各完整build一次vertices/batches，每batch还新建GPU buffer与render pass。TD-M2/M4需参考Bevy `ComputedTextureSlices`硬切generation-owned slices、single prepared-batch authority、persistent instance/vertex arena和phase级pass；量化1/100/10k sprites、1/9/1k slices及stable/1% changed，见PERF-MVP-337。
- 2026-07-18 OIT sprite补充交接：OIT fragment-store当前再次调用`build_sprite_vertices`，并为每个透明sprite创建texture bind group和独立vertex buffer。TD-M2/M4必须让普通transparent与OIT消费同一prepared sprite artifact、按image generation缓存bind group并写持久instance/vertex arena；OIT不得形成第二套CPU/GPU prepare owner。见PERF-MVP-337/368。
- 2026-07-18 mixed transparent sprite交接：混排Vec增长/stable-sort temp已删除；sprite phase仍被presence与build扫描两次，并与mesh按另一全序每camera重排。Render14须向Render09发布统一ordering key的generation sprite range，普通transparent/OIT共享prepared identity；stable mixed sprite projection=0。见PERF-MVP-339及transparent静态证据。
- 2026-07-18 `scene_renderer/sprite`整目录交接：6/6文件确认stats、2D record、transparent overlay与OIT形成最多四个prepare消费者；phase-empty fallback另全扫sprites，2D按batch开pass，overlay/OIT按sprite建buffer。TD-M2/M4的`PreparedSpriteArtifact`必须同时持phase/slice/instance/batch ranges与Copy stats，并让全部消费者共享identity；不得给stats或OIT增设局部第二cache。见PERF-MVP-337及sprite静态证据。
- 2026-07-18 scene UI image补充交接：每image command当前独立6-vertex buffer/bind/draw，stable frame全重建。TD-M2与EditorLayout21统一static quad/vertex-index角点、instance rect/UV/tint/scissor ranges和ordered compatible batches，texture binding消费Render13 generation handle；不得跨clip/order边界合批。见PERF-MVP-397。
- 2026-07-18 scene UI root补充交接：screen-space prepare当前每frame重建七组Vec，逐command执行paint payload/serde hash/debug label投影，并深clone text line、advance与style。TD-M1/EditorLayout21须发布唯一prepared UI generation供brush/text/image/decorations共享，persistent geometry arena只更新dirty ranges；本轮重复paint投影2→1不是终态。见PERF-MVP-398。
- 2026-07-18 scene UI render子目录补充交接：17/17文件已读；paint projection真实调用链2→1、rich inline parse每对象→每command、vertical glyph×grapheme扫描→range定位、background candidate×blocker→C+B均已止损。TD-M1仍须消除stable frame七组plan Vec、rich-run/prefix重复查找、CPU rect六顶点与text/style/report深clone，见PERF-MVP-398。
- 2026-07-18 scene UI SDF/text后端交接：SDF renderer每frame重建decoration/glyph/material plan、6 vertices/glyph和vertex buffer，native路径也重建buffer/text-area/storage-submission Vec；identical SDF material uniform本轮已跳过上传并复用scratch。TD-M1须消费Text05 single compiled run并维护persistent geometry/instance arena，stable CPU vertex bytes/GPU buffer create/upload=0；见PERF-MVP-249/398。
- 2026-07-18 native atlas renderer补充交接：每storage pass当前逐frame新建全部glyph vertex buffer并clone draw commands，render再按draw线性找pipeline；texture residency本身可复用。TD-M1须让Text04 generation draw artifact落入persistent vertex/instance arena与dense pipeline handle，stable buffer create/command clone=0；见PERF-MVP-231。
- 2026-07-18 UI texture identity交接：rich text texture id在resource prepare与draw侧重复解析；direct id miss会扫描全registry并对每record重算locator-derived id。Render14联动Runtime04发布locator/direct/derived-id O(1)索引并复用唯一rich parse artifact，commands 1/1k/100k下stable parse/registry scan=0、lookup O(1)，hot reload只替换对应resident binding。见PERF-MVP-404。
