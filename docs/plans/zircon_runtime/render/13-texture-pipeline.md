...
|------|-------------|------|------|------------|------|
| 2026-08-27 | TX-M1/M3 render-asset device generation recovery residency slice | `render_plan13_device_generation_recovery_residency_slice_source_static_passed_dynamic_pending` | 新增原子 `recover_device_epoch`：同 logical device generation 必须严格前进，全部 live entry/catalog/readiness/ticket id 在突变前预检；成功后保留 reference count，对旧 pending/active 发布 exactly-once release，统一放弃旧 active/pending/detached/ready-retirement handle投影和completion cursor，并按稳定ResourceId重发replacement ticket。scene delta/reconcile也在ticket签发前拒绝未恢复的device换代。 | 对照 UE `FRenderResource::ReleaseRHIForAllResources` / `InitRHI` 完成责任复核；scoped rustfmt、16文件 whitespace/conflict、恢复9字段reset、admission顺序、唯一retirement入队owner和production panic/unwrap/expect静态检查通过。owner行数 manager/recovery/ticket issuance = 747/246/63；新增2个恢复行为测试、1个GPU stream reset测试和1个Runtime15结构门均仅为源码入口，未执行Cargo/GPU。设计证据见 `13/2026-08-27-render-asset-device-generation-recovery-design.md`。 | 产品 device owner swap、failed `WgpuRenderDevice` registry drop、synthetic/真实device-loss、managed Cargo、RenderDoc/PNG、GPU/CPU/memory/power基线仍pending；不得据此标记TX-M1/M3或总渲染里程碑完成。 |
| 2026-08-26 | TX-M1.2 texture metadata validation owner split | `render_plan13_texture_metadata_validation_owner_split_implemented_static_passed_managed_validation_deferred_algorithm_unchanged` | `metadata_validation.rs` 从 723 行收敛为 173 行公开诊断/规则路由；DDS/KTX/KTX2/ASTC 与 scalar format 分类迁入 162 行 `metadata_validation/format.rs`；18 条既有测试迁入 394 行 `metadata_validation/tests.rs`。公开 ABI、规则顺序、诊断文案与格式表均未改变。 | scoped rustfmt 完成；静态迁移对比 functions 13/13、production strings 61/61、tests 18/18，差异均为 0；root 中 DDS/KTX/KTX2/ASTC/DXGI 表 token 为 0；whitespace/conflict 与 diff-check 通过。managed Cargo/行为/GPU/profiler 验证延后。 | TX-M1.2 与 Render 13 整体仍为进行中；下一结构切片按 Runtime optimize 92 / Editor optimize 109 收敛 typed schema、source/recipe、platform artifact、bulk mip/page、install receipt、upload/residency/budget owner。算法优化前必须先采集 import/cook/install/profile 数据，本切片不声明性能或功耗改善。 |
| 2026-07-10 | TX-M3 通用 cubemap/texture-array 资产、manifest 与材质槽维度契约 | `render_plan13_tx_m3_cubemap_array_manifest_slot_dimension_passed` | 完成 `CubemapAsset` / `Texture2DArrayAsset` 到 canonical `TextureAsset` 的 mip-first、face/layer-second 装配；`.zcube` 支持六文件、cmft 横/竖十字与 2:1 equirectangular 双线性重采样，`.zarray` 支持多文件与竖向切片；D2 多层映射 `D2Array` view，Cube 映射 Cube/CubeArray view；shader slot kind 进入 expected/actual texture dimension 诊断，维度不符走 typed non-blocking fallback。owner 叶子均低于 800 行，root 仅保留接线。 | `runtime_texture_cube_resource_contract` 8/8；texture importer cubemap/array manifest 5/5；D2Array view private test 1/1；cube-slot dimension fallback product-streamer test 1/1；scoped rustfmt、asset schema naming 1/1、plugin structure audit 通过。对照 `cmft/image.cpp::imageCubemapFromCross` 验证竖十字 `-Z` 旋转。 | TX-M3 仍为部分完成：通用 manifest 当前输出 RGBA8 source cubemap/array；真实 HDR 继续走既有 `.hdr/.exr` 到 `.zcube + .zribl` 双产物链。后续完成 BC/Basis/KTX2 supercompression transcoder、cubemap array、离线 angular source mip/GPU bake 与更广 CI。 |
| 2026-07-10 | TX-M3 external float DDS/KTX cubemap conversion | render_plan13_tx_m3_external_float_dds_ktx_conversion_passed | 已完成直接可解码外部 cubemap 子切片:DDS D3DFMT/DX10 RGBA16F/32F、KTX1 RGBA16F/32F、无压缩 KTX2 RGBA16F/32F 实际转为 Zircon face-major `.zcube`;KTX mip-major 重排、cmft 数值 FourCC、完整 source mip 身份、BLAKE3 current artifact reuse 均已接线。外部 mip 仅作 source/display/FIS 输入,PMREM/SH9/IEM 一律引擎重建为 `.zribl`。 | `runtime_environment_external_cubemap_import_staging_contract` 3/3、`runtime_texture_external_cubemap_source_only_contract` 5/5、插件 numeric D3DFMT parser 1/1,共 9/9 通过；模块文档更新。BC/Basis/KTX2 supercompression 未提供真实 transcoder,因此严格拒绝而非错误标记完成。 | TX-M3 仍为部分完成:继续 compressed transcode、六面/cross 输入、通用 CubemapAsset/array 资产与槽位 ABI；随后用 staged 真实 Poly Haven artifact 做产品截图与多视角验证。 |
| 2026-07-10 | TX-M3 HDR/EXR equirect cubemap 双产物导入 | render_plan13_tx_m3_hdr_exr_equirect_dual_artifact_staging_passed | 已完成 HDR/EXR 子切片:共享 RGBA32F decode 保留线性 HDR；2:1 equirect 自动生成 face-major RGBA16F 全 source mip `.zcube` 与 PMREM/SH9/IEM `.zribl`,支持 current artifact reuse、显式开关和不放大 face-size override；项目首次 import/restore 均接线,插件 ABI 不变。 | 独立 target focused test 编译通过；直接运行 `runtime_environment_ibl_source_import_staging_contract` 通过 4/4(7.00s)。模块文档已更新；后续 Cargo rerun被并发字体代码缺字段阻断,不计本切片失败。 | TX-M3 仍为部分完成:继续 DDS/KTX payload decode、六面/cross 输入、通用 CubemapAsset/array 资产与槽位 ABI；shader 06 继续生产 GPU/offline bake。 |
| 2026-07-05 | TX-M3 消费方需求登记 | 登记(文档,无代码) | shader 计划集子计划 06(`docs/plans/zircon_runtime/shader/06-environment-ibl-and-pbr-correctness.md`)与计划 11 EL-M1 把 TX-M3 `.zcube` 列为环境 IBL 资产链前置,需求定稿:RGBA16F、face-major、含全 mip、equirect/六面/十字三形态导入(TX-M3 原有范围),源面尺寸上限 1024;`.zcube` 本体不含 SH 与 GGX 预滤波产物(归计划 11 readback 回写机制)。TX-M3 就绪前 shader 06 EC-M1 以运行期 HDR→cubemap 生成顶替,TX-M3 落地时切换为导入期产物,不留双轨;外部含 mip 的 cubemap 容器(cmft DDS/KTX)导入时其 mip 只作源降采样链,PMREM 一律引擎内生成,`.zcube` 无 `prefiltered` 声明位(shader 06 §4.7)。 | 文档登记;详见 shader 06 §5(依赖序)与 §8(跨计划登记)。 | TX-M3 实施时与 shader 06 EC-M1 协调切换点。 |
...
- 2026-07-18 UI image upload性能交接：WGPU surface的upload-key String clone与cache-budget内prune Vec已止损，但static image/atlas仍会因每个GPU chrome stream固定携RGBA而重复`queue.write_texture`。Render13联动EditorUI08/Render17为UI texture建立content generation与resident handle，bytes只在首次/变更时提交；stable generation upload calls/bytes=0，hot reload同generation写入≤1。见PERF-MVP-225。
- 2026-07-18 environment cubemap staging交接：source/PMREM/irradiance generation变化仍在render submission线程同步f32→RGBA16F并按face×mip多次`write_texture`；局部修复仅删除重复临时Vec。TX-M3联动Runtime04、Render11/17定义row-aligned预编码cubemap upload artifact、持久staging arena与单batch copy，stable upload=0、changed转换不占frame thread且artifact build≤1/generation。见PERF-MVP-380。
- 2026-07-18 SDF/MSDF page upload交接：dirty report clone与per-dirty重复source-offset投影已止损为借用+一次page table。Render13联动Text05/Render17让resident page generation持有offset/stride与dirty ranges，写入只消费prepared commands；stable upload=0、changed table≤1/generation且近dirty pages。见PERF-MVP-249及graphics UI font/SDF upload静态证据。
- 2026-07-18 scene UI image binding交接：`ResourceStreamer::ui_texture`虽返回resident texture，但每image command仍新建相同view/sampler bind group并clone owner。Render13须按texture/view/sampler generation发布唯一binding handle，hot reload/device loss精确替换；Render14/EditorLayout21只消费dense handle。stable bind create/Arc clone=0。见PERF-MVP-397。
- 2026-07-18 scene UI SDF atlas补充交接：upload command offset/report clone已止损，但stable SDF keys仍重建atlas set/shelf/slot/transition report，renderer仍先build全atlas pixels再由dirty report决定写入。Render13须消费Text05 generation page artifact（resident texture、offset/stride、dirty ranges），stable bake/page plan/write=0，changed只写dirty pages；见PERF-MVP-249及UI text/SDF core证据。
- 2026-07-18 native bitmap atlas renderer交接：texture array和bind group已按size/layer/format复用，upload有page generation、missing page与face invalidation fail-closed；但mixed storage每帧仍重建prepared-upload/frame binding/report临时结构。Render13须直接消费Text04 single resident page artifact与prepared dirty commands，stable frame upload plan/binding Vec/write=0；见PERF-MVP-231及UI atlas renderer/upload证据。
- 2026-07-18 scene texture resident交接：每个`GpuTextureResource`当前独立创建sampler/bind group，miss在render线程同步load/upload；压缩mip/array因缺显式level layout仍被readiness拒绝。Render13联动Runtime04发布level/layer upload artifact、device级sampler cache与texture/view/sampler generation binding，并按bytes/frame预算提交；stable load/create/upload=0、同descriptor sampler≤1/device generation、compressed多级开放前逐level偏移合同完整。见PERF-MVP-404。
- 2026-07-22 texture/model importer source ownership补充：glTF images从已decode buffers再次复制RGBA，root/labeled model/mesh subassets重复vertices/indices/VG/morph/skin；IBL hit前完整RGBA32F decode。Render13只消费Runtime04 PERF-MVP-504 content-addressed shared image/mesh/upload artifact，不建立renderer私有副本；stable payload clone/upload=0，changed decode/cook≤1/content generation。WOC meshopt/WebP扩展继续由既有Runtime04 failure负责格式正确性。
- 2026-07-22 chunked artifact消费补充：Render13只消费PERF-MVP-506 manifest中的upload-ready texture/cubemap chunks与shared resident handle，不从普通/IBL cache整blob解压后再复制RGBA/f32。记录requested/read/decode/upload chunks、owner/clone bytes与generation hit；stable payload I/O/decode/upload=0，changed只触及dirty mip/face/layer chunks，并与352/354/380的IBL上传合同统一。
- 2026-07-22 texture descriptor/upload plan补充：PERF-MVP-521已删除metadata投影的第二次owned descriptor clone/normalize；Render13按PERF-MVP-522只消费Runtime04 content+descriptor+device generation的parsed level/layer layout与compact upload plan，descriptor相等时复用resident texture/view/sampler，stable header parse/format String/GPU object create=0。对照Bevy `GpuImage::prepare_asset`的previous descriptor复用与`TextureCache`；不得在render线程重复调用readiness helpers。
- 2026-07-22 texture assembly峰值补充：array/cubemap/lightmap/IBL/`.zcube`与external cubemap按PERF-MVP-523统一消费504/506 upload-ready chunks，提交预算按bytes+objects+time并只触及dirty mip/face/layer；source payload、assembled output与decode scratch不得同时整块常驻。Godot 3D texture的全量consolidation仅作格式/层布局对照，不是MVP内存策略。
