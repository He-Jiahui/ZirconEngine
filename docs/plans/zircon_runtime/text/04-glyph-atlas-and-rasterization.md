...
- `text/raster/swash/request.rs` + `rasterizer.rs`:2026-07-07 补齐 glyphon `CacheKey` 到 swash raster request 的语义 parity。`SwashRasterRequest::glyphon_cache_key(...)` 现在保留 glyph id、px size、x/y subpixel offset、`DISABLE_HINTING`、`PIXEL_FONT` offset 规则、`FAKE_ITALIC` transform、font weight variation 与 glyphon source fallback 顺序 `[ColorOutline(0), ColorBitmap(BestFit), AlphaOutline]`;`SwashRasterizer` 将 offset、render format、fake-italic skew 与 `wght` variation 转发给 swash。该切片只关闭 request/rasterizer 数据面,不声明 production native bitmap atlas miss scheduling、per-page upload merge、live editor-window typography QA 或完整 glyphon `TextAtlas` cutover 完成。
