...
- 128×32 `rg16float`,UV=(NoV, roughness),128 样本 Hammersley GGX 重要性采样积分 Karis split-sum 的 A/B 两项;设备生命周期 CPU 生成一次(不进 `IblBakeKey`)。该规模对齐 UE `SystemTextures.cpp` 默认 `PreintegratedGF`,相对旧 128×128×1024 工作量减少 32 倍;同一 Zircon 积分器的 16×16 稀疏 128-vs-4096 样本对比为 RG 平均绝对误差 0.00238490、最大 0.01624132,仍须由 fresh 托管启动计时与画面验收确认。
...
The PBR viewer Ready-frame evidence schema is now v12. It snapshots the same twelve `ShaderVariantMissReport` structural, residency, WGPU creation-cost, and async Base queue-wait metrics after the Ready render, so a process-cold DX12 capture can separate shader/PSO creation from admitted-worker queue delay while retaining every v11 IBL/IEM field. The standalone verifier keeps v2-v11 only behind explicit legacy inspection, requires all v12 fields, rejects u64 overflow, zero registered variants or capture-ready GPU residency, inconsistent texture-presence equivalence, cache residency greater than creation events, and nonzero queue time without an admitted job. A successful v12 CLI result emits those twelve integer metrics plus renderer initialization, deferred Standard PSO, IBL restore, total startup, one-shot Base wait, viewer Ready, and Ready-render durations as structured JSON for cold-run comparison. Its Python contract suite passes; Rust producers are formatted and source-wired, but this remains non-WGPU infrastructure evidence and does not replace the pending current-source screenshot/RenderDoc gate.
