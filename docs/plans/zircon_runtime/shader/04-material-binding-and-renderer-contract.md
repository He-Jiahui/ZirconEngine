...
| 2026-07-14 | SH04-M3 global shader executor migration | 全局执行器迁移切片完成；M3 总体仍进行中 | 共享 built-in global shader contract 与 typed WGPU layout projection 成为唯一 binding owner；HZB single/4x MSAA 硬切到生成的 0/1/2/3 binding、独立 pipeline identity、逐 mip 参数与 named lookup；motion-vector tile-max 硬切到 fullscreen group1 ABI 和共享 fullscreen triangle vertex；删除 feature-local fullscreen owner；compute/fullscreen builder 全部改为 consuming `Self -> Self`；HZB 回归补强为非均匀 4x4、每个 MSAA sample 不同和逐 mip furthest/closest/span 精确断言。 | Windows managed runtime build 通过；fresh lib-test binary 聚焦验证 28/28 通过，其中 HZB real-adapter readback 1/1、完整 motion-blur product 1/1；scoped rustfmt、diff-check、handoff validator 96/0 通过。完整包测试仅被三个 Runtime04 外部集成测试的退役 asset API 调用阻断，不声明 broad gate 通过。详见 [验收记录](04/2026-07-14-global-shader-executor-migration-acceptance.md)。 | non-empty fullscreen group2 参数通用上传与 backend construction error 转入 `FrameDiagnostics` 仍属后续 SH04-M3；不标记 Shader04 或 M3 总体完成。 |
