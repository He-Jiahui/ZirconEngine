...
- 2026-07-22 runtime script热路径交接：`script/**` 96/96逐文件静态审查确认scene hook每Fixed/Update全World JSON投影、binding identity/package callback重复解析，以及gameplay host每call clone runtime context/String、重复manager/input snapshot、World锁和node scan。Runtime13新增承接PERF-MVP-442/443，并继续与PERF-MVP-331共用borrowed HostCallFrame、generation-owned active binding projection、typed query index和预解析callback hard cut；见`13/failure-2026-07-22-runtime-script-binding-hotpath.md`。
