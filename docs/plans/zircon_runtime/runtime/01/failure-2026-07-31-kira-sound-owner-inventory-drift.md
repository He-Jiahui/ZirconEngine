...
- Current static evidence after the fail-closed correction: Python lower-layer owner/pin/scan cases 4/4 GREEN; original Runtime01 upward methods 2/2 GREEN; complete tech-stack 8/8 GREEN; Python py_compile and scoped diff-check GREEN. The Runtime Rust mirror test remains the explicit expected RED until its source-safe hard cut.
- A third lower-layer regression is intentionally RED against the two clean Runtime Rust guards: `tech_stack_dependency_guard.rs` still treats Kira as a non-dependency and `runtime_absorption/tech_stack/mirror_docs.rs` still requires count 5. Their exact source-safe hard cut is the remaining implementation item; no Runtime source edit is made across a foreign full-lib compile window.

Open state: `python_and_authority_fixed_runtime_rust_guard_hardcut_pending`; no fixed return or complete pass is claimed.
