...
- 2026-08-28 current-source static return found one additional supporting-guard drift: the new
  `platform/test_support.rs` helper names `PreferenceStorageBackend` only to inject a backend into a one-worker
  test driver. It is mounted exclusively through `#[cfg(test)]`, and it contains no direct `open_read`/write/remove/
  flush primitive execution. The backend-type allowlist now admits that exact test leaf while the stricter
  primitive-call allowlist remains limited to `preferences/persistence/work.rs`. The focused manager authority
  check is GREEN; the full suite intentionally remains RED only at the stale nested WOC lock. The lock is not
  hand-edited and must still be regenerated through the coordinator-managed Cargo path.
