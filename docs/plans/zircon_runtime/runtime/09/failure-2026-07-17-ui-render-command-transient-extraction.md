...
- rustc 在进入 Runtime09 目标前发现 6 个外部夹具编译错误：Layout18 `input_response_contracts.rs` 的 3 个 `UiNodePath: From<&str>` 漂移，以及 Layout19 `focus_tests.rs` 的 `UiNodePath: From<String>`、`UiTreeId: From<&str>` 和 `serde_json::from_value` 结果类型缺失。最低 owner 分别记录于 [Layout18 open failure](../../../zircon_editor/editor_layout/18/failure-2026-07-22-input-response-fixture-node-path-hardcut-drift.md) 与 [Layout19 open failure](../../../zircon_editor/editor_layout/19/failure-2026-07-22-focus-fixture-typed-id-hardcut-drift.md)。
- 下层修复已按硬切边界改用 `UiNodePath::new`、`UiTreeId::new` 和显式 `UiNavigationBoundary` 类型；最终 snapshot `954` / `955` exact2 哈希稳定，独立 review 为 `Critical 0 / Important 0 / Minor 0`。Layout18 focused job `1f7474ecc92e4617a36afaafc971541f` 已 released/exit0/no PIDs，原始 stdout 实际执行 7 个测试并 `7 passed / 0 failed / 287 filtered`；Layout19 focused 与原 reproduction 尚未完成，两个 failure 仍为 open。
