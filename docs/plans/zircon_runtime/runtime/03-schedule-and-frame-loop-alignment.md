...
- 2026-08-10 App current-source补充：DesktopApp已消费`Idle/Immediate/After`并合并Immediate wake；显式window/device relevance、每pump单次最终control-flow publish、normalized same-size resize双层早退以及Game/Mobile focus/occlusion低功耗profile已完成源码修复。显式Continuous仍为吞吐覆盖，Headless保持16ms固定deadline；shutdown summary新增focus/occlusion与low-power计数。既有schedule完成状态不重开；按[`03/failure-2026-07-19-app-entry-cadence-and-event-trigger-budget.md`](03/failure-2026-07-19-app-entry-cadence-and-event-trigger-budget.md)补current-source受管Cargo、30秒idle、1k/10k storm和duplicate resize产品counter。
- 2026-08-10 production schedule current-source补充：当前`SceneScheduleRunner`已按compiled conflict graph / `SystemParamAccess`形成worker-safe native batch，在不共享`&mut World`的前提下通过`JobScheduler::join`执行，并按stable order合并worker command queues；其他lane保持main-thread authority，worker callback与command apply共享统一unwind边界并在rethrow前恢复taken systems。`NativeSystemScheduleDiagnostics`新增临时控制缓冲数量与capacity-byte代理，先测量再决定是否复用执行状态；overlap当前只有行为回归证据，不是发布计数。既有M0-M3历史验收不重写；受管Cargo及F2 overlap/World-lock/allocation产品矩阵仍按[`03/failure-2026-07-22-production-schedule-remains-serial.md`](03/failure-2026-07-22-production-schedule-remains-serial.md)与[`03/failure-2026-07-17-schedule-executor-frame-allocations.md`](03/failure-2026-07-17-schedule-executor-frame-allocations.md)保持open。
