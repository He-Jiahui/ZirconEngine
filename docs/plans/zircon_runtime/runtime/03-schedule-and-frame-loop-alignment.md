...
- **帧循环归一审计已完成，UI extract 旁路已定稿为 runtime 03 合法旁路（Cargo 待跑）**。实测链路已写入 `docs/zircon_runtime/core/frame_schedule.md`：`dynamic_api/session.rs` `tick_frame`（C ABI 出口 :301，内部 `fn tick_frame(&mut self)` :548）→ `tick_time` → `LevelSystem::tick` → `WorldDriver::tick_level` → 逐 stage `SceneScheduleRunner::run_stage`（`schedule_runner.rs:13`，步骤类型 `ScheduledSceneStepRef::{Internal, Native, ApplyDeferred, Hook}`）。runtime 侧 `RenderFrameExtract` 的生产构建点仍是 `dynamic_api/session.rs:695` `current_extract()`；UI extract 旁路 `current_ui_extract()`（:703）+ `session/hud.rs` + `session/menu.rs` + `runtime_loop.rs` 已裁决为"合法 dynamic-session side path"，并由 `session_ui_extract_remains_documented_dynamic_session_side_path` 源守卫锁定。
...
- 目标文件：`dynamic_api/session.rs`、`dynamic_api/session/{hud.rs,menu.rs}`、`dynamic_api/runtime_loop.rs`。
...
- 验收（归属 `zircon_runtime/src/scene/tests/ecs_schedule/` 新文件 `fixed_update.rs`）：
  - `fixed_update_stage_runs_once_per_drained_step`（lag 累积 3 步 → FixedUpdate 系统执行 3 次，Update 执行 1 次）
  - `fixed_update_stage_skips_on_zero_lag_frame`
  - `fixed_update_steps_are_capped_by_max_fixed_steps`（对照 `max_fixed_steps_per_frame`）
