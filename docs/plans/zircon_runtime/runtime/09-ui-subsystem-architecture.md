...
- taffy 引用文件基线：`ui_taffy_production_hits=173` / `ui_taffy_production_files=9`（排除 tests/fixtures；M2.1 已裁决为 bridge compute + style DTO adapter + pass selection/reporting，Taffy tree build/compute 仅 `taffy_bridge/compute.rs`）
