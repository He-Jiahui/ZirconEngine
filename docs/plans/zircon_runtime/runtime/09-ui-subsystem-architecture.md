...
| 横切 | UI architecture 结构镜像 | structure_audit_static_passed_cargo_pending | 2026-06-14 | 新增 `runtime_structure_audits/ui_architecture_boundary.py` 并接入 `audit_runtime_structure.py`；当前静态事实：`expected_source_file_count = 11`、`expected_ui_entry_count = 17`、`expected_surface_entry_count = 20`、`legacy_full_hits = 167`、`expected_legacy_full_hits = 167`、`legacy_production_hits = 102`、`expected_legacy_production_hits = 102`、`legacy_production_file_count = 12`、`expected_legacy_production_file_count = 12`、`taffy_production_hits = 161`、`expected_taffy_production_hits = 161`、`taffy_production_file_count = 7`、`expected_taffy_production_file_count = 7`、`runtime_v2_anchor_count = 10`、`interface_v2_anchor_count = 9`、`guard_anchor_count = 5`、`cargo_gate_anchor_count = 7`、`doc_anchor_count = 11`、`missing_doc_anchors = []`、`missing_cargo_gate_anchors = []`、`mirror_docs_guard_present = true`、`risks = []`；这仍是静态结构证据，M1-M3 源码切片与 UI Cargo 仍等待 editor UI owner/Cargo lanes 空窗。 |
| 横切 | UI architecture 镜像文档守卫 | mirror_docs_guard_static_passed_cargo_pending | 2026-06-14 | 新增 `runtime_absorption::ui_architecture::runtime_09_ui_architecture_mirror_docs_match_structure_audit_counts`，并在 `ui_architecture_boundary` 中新增 `mirror_docs_guard_present`。该守卫把 `docs/zircon_runtime/ui/architecture.md`、本计划、runtime index、M0 review 与 runtime-interface convergence 固定到同一组结构审计事实：`expected_source_file_count = 11`、`expected_ui_entry_count = 17`、`expected_surface_entry_count = 20`、`legacy_full_hits = 167`、`expected_legacy_full_hits = 167`、`legacy_production_hits = 102`、`expected_legacy_production_hits = 102`、`legacy_production_file_count = 12`、`expected_legacy_production_file_count = 12`、`taffy_production_hits = 161`、`expected_taffy_production_hits = 161`、`taffy_production_file_count = 7`、`expected_taffy_production_file_count = 7`、`runtime_v2_anchor_count = 10`、`interface_v2_anchor_count = 9`、`guard_anchor_count = 5`、`cargo_gate_anchor_count = 7`、`doc_anchor_count = 11`、`missing_doc_anchors = []`、`missing_cargo_gate_anchors = []`、`mirror_docs_guard_present = true`、`risks = []`。验证：rustfmt check、Python py_compile、direct `ui_architecture_boundary_audit`、aggregate `audit_runtime_structure.py --json` Runtime 09 assertions、standalone rustc 4/4、conflict/trailing scans 通过；该切片不改 UI 生产代码，`ui/input/naming_boundary/layout/template` Cargo gates 仍 pending。 |
| M1 | 1.1 路由单点 | 待开始 | — | — |
