...
| M2 | 2.2 rayon 守卫 | code_static_pending_render_cutover_cargo | 2026-06-13 | 非 graphics 侧已执行：`ScheduleParallelExecutor` 去掉 `use rayon` / `rayon::join` / `into_par_iter`，固定 2-6 系统批次改走 `JobScheduler::join(...)`，泛化批次改走 balanced `run_parallel_tasks(...)`；结构锚 `schedule_parallel_executor_does_not_call_rayon_directly` 已补。新增 `runtime_absorption::rayon_boundary` 生产源码守卫，允许 `core/runtime/tasks/{pool,parallel_for}.rs` 作为 task primitive owner，并把 `graphics/visibility/culling/parallel_frustum.rs` 登记为 M2.1/render-owner 待迁移例外；负例 `rayon_boundary_guard_rejects_unclassified_runtime_source` 固定 `schedule_parallel_executor.rs` 不可再作为例外。2026-06-13 07:05 +08 尝试 `cargo test -p zircon_runtime --lib rayon --locked --jobs 1 --target-dir E:\cargo-targets\zircon-runtime-11-validation-0613 --message-format short --color never -- --nocapture`，20 分钟超时且未返回测试结果；本轮孤立的 rayon cargo/rustc 进程已停止。完整白名单清零需 M2.1 render owner 窗口，M2.2 Cargo 仍待干净编译窗口重跑。 |
...
| 横切 | Cargo pending gate | code_static_pending_cargo | 2026-06-13 | 新增 `runtime_absorption::plan_status::cargo_gates::runtime_11_job_system_cargo_gate_stays_visible_until_job_system_filters_pass`，锁定 Runtime 11 在 `tasks/ecs_schedule/worker_pool/rayon` 过滤验证、render-owned `parallel_frustum` 真实 cutover 与 broader lib gate 通过前保持 `in_progress`；同步 Runtime 11、本 runtime index P14/子计划行、Runtime 05 M3.2、`docs/zircon_runtime/core/job_system.md` 与 M0 评审。 |
| 横切 | JobSystem 结构审计 owner | structure_audit_static_passed_cargo_pending | 2026-06-13 | 新增并接入 `runtime_structure_audits/job_system_boundary.py`，静态复核 `core/runtime/tasks` 9 个 folder-backed owner 模块、`JobHandle` / `JobScheduler::schedule_after` / `parallel_for` / `JobSchedulerReport` / `tasks.*` 诊断锚、`ScheduleParallelExecutor` 的 dependency scheduling + `JobScheduler::join(...)` 路径、Runtime 11 M1/M3 行为测试锚，以及 Runtime 11 direct-Rayon 白名单。targeted audit: `expected_module_count = 9`, `direct_rayon_paths = 3`, `schedule_parallel_executor_direct_rayon = []`, `diagnostic_anchor_count = 4`, `behavior_test_anchor_count = 9`, `missing_behavior_test_anchors = []`, `oversized_modules = []`, `mirror_docs_guard_present = true`, `risks = []`。Cargo 仍等待 active lanes 清空。 |
| 横切 | JobSystem 镜像文档守卫 | mirror_docs_static_passed_cargo_pending | 2026-06-14 | 新增 `runtime_absorption::job_system::runtime_11_job_system_mirror_docs_match_structure_audit_counts`，锁定 `job_system_boundary` 的 `expected_module_count = 9`、`direct_rayon_paths = 3`、`schedule_parallel_executor_direct_rayon = []`、`diagnostic_anchor_count = 4`、`behavior_test_anchor_count = 9`、`missing_behavior_test_anchors = []`、`oversized_modules = []`、`mirror_docs_guard_present = true` 与 `risks = []` 必须同步到 JobSystem 模块文档、Runtime 11、runtime index、M0 review 和 runtime-interface convergence。未改 JobScheduler、TaskPool、ECS 或 render 生产代码；Cargo/rustc 仍待 active lanes 清空。 |
...
| 横切 | JobSystem 行为测试锚审计同步 | mirror_docs_static_passed_cargo_pending | 2026-06-15 | `job_system_boundary` 与 `runtime_11_job_system_mirror_docs_match_structure_audit_counts` 现在锁定 Runtime 11 M1/M3 的 9 个 `zircon_runtime/src/tests/tasks.rs` 行为锚，当前 `behavior_test_anchor_count = 9`、`missing_behavior_test_anchors = []`；同步 JobSystem 模块文档、Runtime 11、本 runtime index、M0 review、runtime-interface convergence 与状态输出表守卫。验证：rustfmt check、Python py_compile、direct `job_system_boundary_audit`、aggregate Runtime 11 assertions、standalone job_system 1/1、standalone status-output 2/2；tasks/ecs_schedule/worker_pool/rayon Cargo gates pending，render-owned `parallel_frustum.rs` cutover pending。 |

基线数值（开工首日记录）：

- `JobScheduler` 公共原语基线：3（spawn/install/join，job_scheduler.rs:31-47）；2026-06-13 M1 静态扩展后为 5（spawn/install/join/schedule/schedule_after）
- rayon 使用文件基线：production 当前 3 文件（`core/runtime/tasks/pool.rs`、`core/runtime/tasks/parallel_for.rs`、`graphics/visibility/culling/parallel_frustum.rs`）；`schedule_parallel_executor.rs` 已在 11-M2.2 非 graphics 收编中移出 direct rayon，结构测试只保留 source guard 断言。完整 production 白名单清零仍等 11-M2.1 render owner 窗口迁移 `parallel_frustum.rs`。
...
- `cargo test -p zircon_runtime --lib tasks --locked` 通过数基线：待 clean cargo window；2026-06-13 有其他 active cargo/rustc lanes
- `cargo test -p zircon_runtime --lib rayon --locked` 通过数基线：待 clean cargo window；M2.2 当前静态收编 ECS direct-rayon，仍保留 render-owned `parallel_frustum` 例外至 M2.1。
...
- `parallel_frustum.rs` 在 graphics 区，受 10fps 会话与 render 计划影响：M2.1 动工前 `git status` + 与 render owner 确认；**禁止回退其改动**；若 render 计划重写剔除（GPU 驱动剔除），M2.1 让位并改为"新剔除路径直接使用统一原语"的接入要求。
- worker 内 `wait()` 死锁策略是本计划最大技术风险：M1.1 实现前必须有判词与对应测试（`worker_thread_wait_does_not_deadlock_scheduler`），rayon 的 work-assist 行为（`yield_now`/scope 嵌套）需实测验证而非假设。
