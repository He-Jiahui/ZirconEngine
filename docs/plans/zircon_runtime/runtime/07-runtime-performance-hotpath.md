...
| M1 | 1.3 性能热路径镜像文档守卫 | mirror_docs_guard_static_passed_cargo_pending | 2026-06-14 | `runtime_absorption::performance_hotspots::runtime_07_performance_hotpath_mirror_docs_match_structure_audit_counts` 与 `performance_hotpath_boundary` 的 `mirror_docs_guard_present` 把本计划、runtime index、`docs/zircon_runtime/performance/hotspot_inventory.md`、`docs/zircon_runtime/dynamic_api/session.md`、`docs/zircon_runtime/scene/ecs.md`、M0 review 与 runtime-interface convergence 固定到同一组结构审计事实：`expected_source_file_count = 10`、`expected_test_file_count = 5`、`frame_span_anchor_count = 9`、`query_counter_anchor_count = 13`、`change_counter_anchor_count = 9`、`extract_counter_anchor_count = 10`、`asset_worker_anchor_count = 5`、`hotspot_guard_anchor_count = 20`、`test_anchor_count = 12`、`doc_anchor_count = 17`、`cargo_gate_anchor_count = 5`、`stale_hotspot_placeholder_present = false`、`large_file_m1_gate_status = migration-debt-present`、`large_file_hotspot_count = 38`、`large_file_migration_debt_count = 5`、`large_file_owner_class_count = 5`、`large_file_unclassified_hotspot_count = 0`、`missing_large_file_owner_classes = []`、`missing_doc_anchors = []`、`missing_cargo_gate_anchors = []`、`mirror_docs_guard_present = true`、`risks = []`；owner-budget 摘要继续保留 `hotspots 38, debt groups 5, owner classes 5, unclassified hotspots 0`。该切片只锁定镜像文档一致性，不启动 M2 优化；验证：rustfmt check、Python py_compile、direct `performance_hotpath_boundary_audit`、aggregate `audit_runtime_structure.py --json` Runtime 07 assertions、standalone rustc 6/6、旧计数锚点扫描、conflict/trailing scans 通过；extract/ecs_query/performance profiling/FPS Cargo gates pending。 |
...
- Runtime 07 owner-budgeted optimization gate：`performance_hotpath_boundary` 复用 `large_file_ownership_gate`，当前 1000 行阈值下 large-file gate 为 `migration-debt-present`，hotspots 39, debt groups 5, owner classes 5, unclassified hotspots 0。M2 优化若触及大文件，必须先按 owner class 拆分或取得对应活跃 owner handoff；large production files remain above the owner budget 时不得把性能改动继续堆进同一个热点文件。
