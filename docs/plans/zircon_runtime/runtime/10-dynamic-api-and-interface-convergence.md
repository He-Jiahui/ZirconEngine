...
以下内容保留 V3 迁移前的实仓盘点，用于追溯已完成的版本演进；它不是当前 ABI、加载器或后续切片的实现依据。
...
3. 事实重核：
   ...
   - `grep -n "pub struct Zr" zircon_runtime_interface/src/runtime_api/abi/api_table.rs zircon_runtime_interface/src/plugin_api.rs`（核表清单与行号）
   - `grep -c "repr(C)" zircon_runtime_interface/src/runtime_api/abi/api_table.rs zircon_runtime_interface/src/plugin_api.rs`（repr 覆盖基线）
...
- 2026-08-27 Runtime10/15 project test-owner split：`dynamic_api/session/project.rs` 的 14 个既有 unit tests 已原样迁入 `dynamic_api/session/project/tests.rs`，父/child 从单文件 917 行收敛为 562/353 行；独立的 `project/runtime61_characterization.rs` 120 行与其 route 保持不动。HEAD 旧测试体与新 child whitespace-normalized 等价，结构守卫 exact 1/1、rustfmt 与 scoped diff check 通过。该切片只关闭 project owner 的内联测试/file-budget 债，受管 Cargo 尚未执行，不改变或验收 project startup、Runtime61 exact Play restore 或 Runtime10 整体状态。
...
## 2026-08-27 Dynamic Session Input Event Owner Split

状态：`runtime_10_12_15_dynamic_event_keyboard_ime_gamepad_owner_split_static_passed_cargo_deferred`。

Dynamic session 的 ABI event root 现在保留 event-kind routing、viewport/pointer/window/lifecycle
编排和共享 UI dispatch；keyboard/IME 的 payload 解析与 runtime/UI 双投影迁入 223 行
`events/keyboard_ime.rs`，gamepad connection/button/axis 与 UI navigation/analog mapping 迁入
115 行 `events/gamepad.rs`。父 `events.rs` 从 1038 行降到 734 行并保留原 6 个测试，两个
child 方法只对父路由开放 `pub(super)`，没有扩张 dynamic API 或 interface 公共面。

结构 RED/GREEN guard 2/2、定向 rustfmt 和 scoped diff check 通过。3 个迁移项与 `HEAD`
规范化等价，另 4 个 handler 保留拆分前已存在的 physical-input-before-UI、typed clock 与
metadata sequencing 变更；这些并发行为不属于本结构切片。没有修改 event ordering、payload
limit、UTF-8 校验、UI consumed 语义或 input buffer 算法；Cargo/app/UI 动态验证延后，
Runtime10 未完成。
