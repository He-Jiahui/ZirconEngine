...
Viewport point bounds now track minimum and maximum coordinates as Vec2 values, reducing four
scalar component updates to two vector min/max operations while preserving the resulting UiFrame.

## Static Evidence

- The first point initializes both vector bounds.
- Each later point updates `min` and `max` with Vec2 operations.
- Behavior and source-contract tests cover frame coordinates and the vector implementation.

## Performance Gate

The ignored Windows release benchmark emits `EDITOR307_VEC2_FRAME_BOUNDS_BENCH_V1`. It compares the
previous scalar bounds loop with the optimized Vec2 bounds loop over 256 points and 20,000
repetitions per sample across 17 interleaved samples and requires
...
Editor307 is included in the six-group Runtime/Editor batch under request
...
`a1102110c7daee234ea89c1f19491a267e64f7a0e4a2882fa61e1c1c47920606`. The point
fixture is opaque before timing and compares scalar and Vec2 bounds over identical input; the 30%
p95 gate remains. Cargo, performance, independent review, commit, and WeCom remain
