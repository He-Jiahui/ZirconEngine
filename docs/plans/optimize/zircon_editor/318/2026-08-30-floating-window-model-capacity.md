---
title: Editor318 Floating Window Model Capacity
category: zircon_editor
report_id: Editor318-floating-window-model-capacity-2026-08-30
date: 2026-08-30
...
# Editor318 Floating Window Model Capacity

`build_floating_windows` now reserves the exact floating-window snapshot length before building
`FloatingWindowModel` values. Window identity, title, frame, focus, tab projection, and source
order are unchanged.

Regression coverage checks the capacity contract and output-order contract. The ignored Windows
Release benchmark emits `EDITOR318_FLOATING_WINDOW_CAPACITY_BENCH_V1` over 17 paired samples with
256 windows per sample and requires `optimized_p95_ns <= legacy_p95_ns * 0.70`.
...
Editor318 is submitted in the six-task batch under request
