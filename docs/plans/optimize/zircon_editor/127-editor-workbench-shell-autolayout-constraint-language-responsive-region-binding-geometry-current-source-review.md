...
# 127 · Editor Workbench Shell AutoLayout / Constraint Language / Responsive Tier / Region Binding / Geometry Authority 当前源码复核

## 1. 结论

Zircon Editor已经有一组值得保留的壳层布局基础：`WorkbenchSkeleton`、typed region、轴向约束、breakpoint tier、逻辑像素解析、Taffy template bridge、drawer resize和97项当前聚焦测试都是真实代码；产品渲染、pointer、drag和viewport也确实会消费componentized template layout frames。因此问题不是“没有布局算法”，而是声明、求解和发布没有收敛成一个产品authority。
...
第二处是同一次shell recompute同时维护两套几何权威：custom `WorkbenchShellGeometry`求解Left/Document/Right/Bottom，componentized bridge再用另一套drawer rule和Taffy求解真实frame。渲染、hit-test、drag与viewport优先使用template frames，最小窗口、floating window和template frame复用却依赖legacy geometry；窄屏时两边对Bottom Drawer的折叠规则本身就不同。当前根template bridge的`recompute_layout_with_workbench_model_at_scale`还明确把`_model`与`_metrics`留作未消费参数，只重建host surface，API名称无法证明模型状态已参与布局。更危险的是template recompute失败只记日志，随后仍把旧template frames与新model/legacy geometry一起发布，没有generation、原子commit或rollback。
...
|---|---:|---:|---|---|
| 聚焦Zircon源码与资产 | 68 / 11,028 / 380,706 | 97个`#[test]`、1 ignore | E3 | 44个autolayout文件、产品recompute/template/render链、共享constraint合同与聚焦测试 |
| `autolayout/**` | 44 / 4,665 / 158,196 | 37个`#[test]`、1个ignore | E3 | skeleton、asset、binding、constraint、tier、geometry和fallback逐文件检查 |
...
| 参考源码 | 20 / 16,103 / 589,880 | Unreal/Godot/Fyrox/Bevy/Unity Graphics | E2/E3 | splitter、dock persistence、responsive arrangement、single-tree publication与真实产品pane |
| 计划/契约文档 | 8 / 3,527 / 502,157 | 2个测试标记、0 ignore | E2 | Editor/Runtime owner边界、Workbench契约与既有layout计划 |

68份聚焦文件按normalized relative path排序，写入`path + NUL + raw bytes + NUL`后取SHA-256，working-tree fingerprint为`13b882486f371704ca7a4c988f0dd703ac65e11362f98c1c2bad4952aaea734a`。20份参考源码fingerprint为`e4db5ee5108ad855ae62c2d9bab1796e9d20ced3b50fdcae090d29da3ac36e0c`。8份计划/契约文档fingerprint为`0dd6769549c4286cf1b3266404f5c178c202fc7c28f0fc34a4bf624f87268d34`；三类证据去重union为96文件、30,658行、26,642非空行、1,472,743 bytes，fingerprint为`2fadaf1ef931f1f9ecc611e09385f8c0093f7995192de6305fbf26b9ad945ba1`。本轮源码基线为当前HEAD `8e56165c4c789416c328898d3d8937d934b52efa`；未查询协调器状态，审查以当前工作树内容为准。

聚焦范围内当前仍存在非本轮产生的在途修改，包括constraint defaults/parser/tests、region/floating/vertical/window geometry、autolayout root、两份Editor layout contract tests以及host recompute/presentation。本轮按working tree内容审查并设置`source_recheck_required: true`；实施前必须重算68文件fingerprint并重取product caller矩阵，不能回退共享工作树。
...
| root `BuiltinHostWindowTemplateBridge` model/metrics参数 | `recompute_layout_with_workbench_model_at_scale`签名 | 参数被`_model`/`_metrics`忽略，仅按logical shell size重建host surface | API表面宣称模型驱动，实际没有Workbench布局输入 |
...
template path则直接把drawer尺寸写到真实ZUI/Taffy节点，另行执行breakpoint、side balance、responsive visibility和toolbar priority。它在Ultra/Narrow width下明确把Bottom Drawer压到panel header，legacy path只按可用高度compact。现有产品测试还明确断言template frames优先于legacy geometry，甚至在legacy frame全为零时仍使用template frame。`apply_workbench_drawer_layout`每次还遍历全部surface roots并调用`mark_layout_dirty`，而`EditorWorkbenchTemplateSurface::refresh_projection`会先修改surface/frame，再决定增量或full projection；projection错误没有把surface与host projection一起回滚。两者不能被解释为“同一算法的缓存层”。

### 2.4 当前源码证据链

1. `recompute/shell/builder.rs`先调用`compute_workbench_shell_geometry_with_region_defaults_and_scale_mode`，再调用`recompute_shell_template_bridge_layout_frames`；`reuse_shell_layout`由legacy `shares_mounted_layout_frames_with(...)`比较决定，而不是由template tree的dirty/dependency key决定。
2. `recompute/shell/template_bridges.rs`对root和componentized bridge分别捕获`Err`并写日志，函数仍返回`self.workbench_window_bridge.layout_frames()`；`recompute.rs`随后照常建立`CommittedShellState`、清除dirty标志并调用`mark_host_projection_committed()`，失败没有阻止发布。
3. `componentized_window/mounted_layout.rs`把model投影到drawer constraints，再执行responsive visibility，最后才调用`template_surface.recompute_layout(...)`；这条链和legacy几何使用不同的输入、单位和折叠规则。
4. `drawer_layout.rs`中的`reserve_document_width`只按最小文档比例反向压缩左右drawer，`mark_roots_layout_dirty`无条件标记所有root；`responsive_layout.rs`扫描surface全部节点，从字符串`responsive_min_tier`解析tier，未知值直接被`filter_map`丢弃。
5. `shell_regions_asset.rs`的`from_toml_str`与`WorkbenchSkeleton::from_shell_regions_asset`只提供解析/验证和测试装配；全量Editor生产源码没有该asset的读取、依赖注册或reload入口。`WorkbenchSkeleton::from_shell_regions_asset`仍从`jetbrains_default()`继承未声明的chrome/default mode字段。
6. `css_like_constraint/declaration_parser.rs`的入口是调用者已经拆好的`(&str, &str)`迭代器，不含文件、行列、token stream、selector/cascade或include依赖；`CssLikeDimension`仅支持`auto/px/%/$token`，`repeat`、`fit-content`、viewport单位和多项style语义明确拒绝。
7. `zircon_runtime/src/ui/layout/constraints.rs::solve_axis_constraints`返回裸`Vec<ResolvedAxisConstraint>`；即使总minimum大于available，也只尽力收缩并返回结果，没有`feasible/overflow/residual`状态供Editor拒绝发布。`ResolutionContext`对非法尺寸/scale则归一化为0或1，不能把输入错误与合法零尺寸区分开。
...
`recompute_shell_template_bridge_layout_frames`失败只记录日志，caller继续从bridge取既有frames并构造新presentation；没有prepare/commit、frame generation、rollback或last-good source identity。`recompute_if_dirty`在后续仍写入`committed_shell_state`，并无条件调用`mark_host_projection_committed()`，这会清掉bridge的pending/full-refresh标记。这样一次普通layout error就能把新的pane/model/tier与旧的frame或旧projection组合，render、pointer、resize和drag看到的状态也无法证明同代。
...
| ED54-P1-02 | `from_shell_regions_asset`只替换regions，chrome assets、mode和其他字段仍来自hardcoded skeleton；根template bridge的`_model`/`_metrics`参数也未参与host layout | source schema必须完整声明或显式继承每一层，并把resolved provenance写入compiled snapshot，删掉会掩盖未消费输入的API |
...
| ID | 当前差距 | 需要重构的内容 |
|---|---|---|
| ED54-P1-45 | model、legacy geometry、template frames没有共同revision；`CommittedShellState`只保存值对象，没有source/token/layout generation | 单一published layout generation绑定全部consumer snapshot，并把generation写入每个projection/interaction lease |
| ED54-P1-46 | 非权威legacy frame equality决定是否复用权威template frame，且bridge失败后仍可能把旧frames当成成功结果 | reuse只由compiled dependency key和template tree dirty set决定，失败只能保留完整last-good generation |
...
| ED54-P1-49 | drawer dimension变化前对所有root调用`mark_roots_layout_dirty`，无论实际受影响的drawer subtree | 建立dependency-indexed dirty roots/subtrees和增量layout receipt |
| ED54-P1-50 | responsive pass每次扫描全部nodes并从`template_metadata.attributes`解析字符串tier，非法值被静默跳过 | template compile时建立typed responsive index，按tier delta更新受影响节点并把非法属性变为compile error |
...
| Gate | 验收内容 |
|---|---|
| G01 | 68份聚焦文件、20份参考文件与8份计划/契约文档路径存在；当前HEAD为`8e56165c4c789416c328898d3d8937d934b52efa`，实施时仍须重取fingerprint并处理在途修改 |
...
本报告不涉及外部脚本与自动化辅助目录，不重写Taffy，不把完整Web CSS当作目标，也不复制Unreal Slate/Godot object system。性能目标是在保持source可追踪、错误恢复、DPI/a11y和一致性后，以单树、编译索引、增量dirty和可测数据布局超越参考实现，而不是删掉工程语义换取局部微基准。
...
本轮完成68份Zircon聚焦源码/资产、97项聚焦测试（1项标记ignore）、全量产品caller反查及20份参考源码对照；同时复核runtime constraint/style与8份计划/契约文档。新增本专项当前源码复核报告并同步Editor索引、顶层索引、coverage与跨报告P0 owner总账。未修改production/tests，未运行Cargo、GUI、真实DPI/monitor、fault injection、pixel、a11y、soak或性能基准。
