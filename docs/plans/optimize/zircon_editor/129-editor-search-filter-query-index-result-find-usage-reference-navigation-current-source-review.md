...
refreshes: docs/plans/optimize/zircon_editor/56-editor-search-filter-query-index-result-find-usage-reference-navigation-product-integration-review.md
review_status: current_source_refresh_complete
implementation_status: pending
source_recheck_required: true
---

# 129 · Editor Search / Filter / Query Index / Result / Find Usage / Reference Navigation 当前源码复核

## 1. 结论

Zircon Editor当前没有统一的工程级搜索系统，而是并存多套粒度不一致的实现。Asset Browser搜索是真实可达的，但在`AssetWorkspaceState::build_snapshot`中把query转成ASCII小写，按当前目录对`display_name`、`file_name`和`locator`做线性子串扫描，并附加单一`ResourceKind`过滤；catalog虽已建立UUID/locator HashMap和projection cache，却没有为搜索建立可复用的候选索引、facet或分页协议。Hierarchy搜索真实可达，能保留命中节点祖先、处理Unicode小写并通过5,000行扁平/深层fixture，但过滤激活后会把每次稀疏层级变化升级为全量 authoritative reflow；Blend Space仍只过滤模板内写死的三个演示行。
...
当前Runtime `AssetRegistryIndex`已经统一维护UUID/path依赖和反向引用，并对查询结果做稳定排序；Editor catalog details generation只把这些权威边投影为`EditorAssetReferenceRecord`，这关闭了旧报告关于Editor重复维护`ReferenceGraph`的具体结构问题，但仍不构成跨资产、Scene对象、脚本符号、Gameplay Tag、UI资源与插件定义的Find Usage服务。UI document dependency generation是打开文档的刷新路由，动态reference row/pointer bridge仍是一次性全量UI投影。工程目标必须是一个带typed query、provider、index generation、取消、分页、结果地址、stale校验、导航receipt和能力真实性状态的Editor Search Runtime，而不是继续给每个Workspace手写字符串匹配。

本报告登记 **2项P0、44项P1、12项P2与38个资格门**。其中旧报告的Editor `ReferenceGraph`重复存储与无序反向边已在当前源码中关闭，现有P1改写为“Runtime registry与Editor projection之间仍缺统一Search provider/result generation/field-path合同”，不再重复计数结构性缺陷。Editor129拥有跨Editor的query/result/provider/index-session/navigation产品合同及搜索入口真实性；Editor04继续拥有资产注册表、import、dependency/referencer数据权威，Editor03拥有Scene identity/hierarchy业务，Editor08拥有command palette与command基础设施，Editor10拥有通用background operation，Editor15/20/21/23/25拥有各业务域结果语义，Runtime24拥有qualified identity，Runtime51拥有Runtime asset registry/index持久化。不得把这些父owner的全部问题复制进本报告。
...
|---|---:|---:|---|---|
| 聚焦Zircon源码与资产 | 135 / 27,804 / 1,116,528 | 268个`#[test]`、0个ignore | E3 | asset search事件链、hierarchy filter/reflow、module搜索、registry/details/row/pointer/navigation |
| 真实搜索产品链 | Asset Browser与Hierarchy两条 | 两者均有行为测试；Hierarchy含5k规模fixture；Asset projection有缓存/增量测试 | E3 | query输入到snapshot、filtered rows、selection/navigation的实际效果 |
...
135份聚焦文件按normalized relative path排序，将每个`path + NUL + raw file bytes + NUL`串联后计算working-tree fingerprint，结果为`22680455d5b7ea9f1eeebed3b8b62adb27412218b7a98aae40204f0800b20c4a`。17份参考源码按同一算法计算，fingerprint为`29bd096ef3beb2fb75d0c7744c5ef9b1a5dba6821b6d2b82f9bd0d4db6cab34a`。本轮读取的Git HEAD为`8e56165c4c789416c328898d3d8937d934b52efa`。

聚焦语料按当前工作树扩展为135份文件；其中若干asset binding/normalization、reference pointer与presentation/tests存在在途差异。本轮只读复核，不覆盖这些变更。实施前必须重算fingerprint、重查所有`Search|Filter|Find Usage|Reference Scan`caller，并确认P0入口仍可见且仍无真实consumer。

### 2.2 当前源码证据链

| 证据 | 当前源码事实 | 影响 |
|---|---|---|
| Asset query projection | `asset_workspace_state.rs`在`build_snapshot`和`visible_asset_generation`中调用`to_ascii_lowercase`，遍历`catalog.assets`并按当前`selected_folder_id`、三字段子串和单kind过滤；`EditorAssetCatalogGeneration`的UUID/locator索引只服务直接定位 | 已有局部真实搜索与缓存，但没有全项目scope、typed query、facet、score或page |
| Hierarchy query/reflow | `hierarchy_filter.rs`构建parent index并保留祖先；`scene_hierarchy_refresh.rs`在query非空时拒绝sparse fragment，转为`resync_scene_hierarchy_from_message`的authoritative reflow | 正确性基础存在，增量性能合同缺失 |
| Workbench Scene | `workbench_scene_tree_panel.zui`的`WorkbenchSceneSearchField`只有`query`/`placeholder`和`value_property`，没有events；Filter按钮route为`workbench.hierarchy.open_filter`，bindings登记`workbench.hierarchy.filter.open` | 可见控件不可执行，且route漂移仍是P0 |
| Effect/Tags fields | `module_navigation.rs`把search edit/commit列为field action，但`module_command_feedback.rs::module_command_feedback`没有对应分支；generic field bridge只回写value/value_text并刷新模板 | 输入事件可返回成功却没有query owner/result consumer |
| Blend Space | `blend_space_search.rs`只遍历`ASSET_ROWS`中的`BS_Idle_Run`、`BS_Strafe_Grid`、`BS_Sprint_Lean` | fixture过滤不能代表Animation asset provider |
| Icon/Tag scan | `extension_module_feedback.rs`固定输出`14 references`；`generated_bottom_panel_feedback.rs`对filter只写“updated”，Gameplay Tags Reference Scan仅切换静态route | 伪造operation/result仍是P0 |
| Reference authority | Runtime `AssetRegistryIndex`维护`referencers_by_uuid`/`referencers_by_path`，`query.rs`按UUID排序；`catalog_generation/details.rs`把unknown reference降级为`known_project_asset=false`并排序 | 旧Editor重复图问题关闭；仍缺edge field path、scope、generation、provider receipt与跨域聚合 |
| Reference UI/navigation | `asset_reference_rows.rs`对每条结果克隆四个节点；pointer bridge仅允许`known_project_asset`行点击；`navigate_to_asset`找不到UUID直接不改变状态 | unknown/stale/large result的产品合同仍缺失 |
| Command palette（独立域） | `core/commands/palette.rs`已有倒排字节posting、fuzzy score和分页查询 | 这是当前唯一较完整的索引/分页搜索底座，但没有与内容/引用搜索共享query/provider/result合同，不能作为Editor Search Runtime完成证据 |
...
4. 保留Runtime `AssetRegistryIndex`的UUID/path索引、稳定排序查询、持久化重建与Editor catalog details generation的UUID优先/locator fallback；将这条链降为一个asset-reference provider，不让它继续冒充全局引用权威。
...
| ID | 差距 | 必须形成的合同 |
|---|---|---|
| E56-P1-35 | Runtime `AssetRegistryIndex`已统一保存UUID/path依赖和反向边并稳定排序，但Search层没有消费它的统一provider、scope、generation与completeness合同 | 保留Runtime51权威实现；Editor129只编排provider receipt/result generation，不再复制Editor图 |
| E56-P1-36 | `catalog_generation/details.rs`仍把direct references投影成UUID/locator/display/kind，unknown只标`known_project_asset=false`，没有来源字段、property path、edge kind或解析诊断 | 由Editor04/各asset domain注册typed reference extractor与field path，并保留missing/stale target语义 |
...
| E56-P2-04 | Runtime反向引用查询已经按UUID稳定排序；但UI动态row仍以数组index生成control id，结果generation变化时缺stable result key | provider contract要求deterministic order，并以stable key驱动virtual row |
...
|---|---|---|---|
| Asset | Editor04/Runtime51 catalog与Runtime AssetRegistryIndex | asset、folder、type、tag、dependency、source | Editor129编排query/page/navigation，不复制registry |
...
| Unreal `IAssetRegistry` | enumerate/query、dependency/referencer、sync/async scan、priority、completion与变更事件 | Runtime51拥有registry，不把接口搬进Editor129 |
...
- Domain report继续拥有“一个Gameplay Tag引用是什么”“一个Shader节点如何创建”等业务语义；Editor129只要求它们以provider/result contract暴露。
- 外部脚本/自动化范围按本轮排除；本报告只记录后续Rust实现必须满足的产品和运行时合同。
