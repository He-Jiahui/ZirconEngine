...
refreshes: docs/plans/optimize/zircon_editor/08-command-registry-keymap-menu-palette-context-routing-remote-automation-review.md
review_status: current_source_refresh_complete
implementation_status: pending
source_recheck_required: true
---

# 130 · Command Registry、Keymap、Menu、Palette、Context Routing 与 Remote Automation 当前源码复核

## 1. 结论

Zircon Editor的命令系统并非空壳。当前源码已经有统一的`EditorOperationPath`、descriptor registry、operation factory、`WhenClause`、generation snapshot、键位override、menu/palette投影和headless commandlet；palette采用immutable catalog、byte posting、bounded heap/window和精确enablement复核，键位hash命中后还会做完整chord比较。这些基础应保留，不能退回到每个按钮直接写业务回调。

但当前“统一命令系统”只统一了部分metadata，没有统一身份、执行、授权、生命周期与呈现。对当前源码逐条重读后，最严重的三个断点仍然是确定的P0：

1. `UiControlRequest::InvokeBinding/InvokeRoute`承载`EditorOperation` payload时绕过`callable_from_remote`，并把来源改写成`UiBinding/RetainedHost`；`EditorCommand` payload当前会重新映射为Remote并复核gate，但同一control family的策略与审计仍不一致。
...
这意味着继续增加菜单、快捷键或远程自动化会放大一组彼此不一致的旁路，而不是形成Unreal级别的可扩展Editor控制面。本报告记录3个P0、44个P1、10个P2；相对旧版只关闭“命令面板没有索引/分页”这一类过时判断，保留其余仍由当前源码复现的风险。没有修改生产代码，也没有运行Cargo、真实Editor、插件DLL、MCP/网络transport、键盘布局矩阵或万级命令交互benchmark；确定性P0来自公开DTO、解析规则、路由分支和现有行为测试的静态闭环，不把尚未审查的网络暴露面推断成已存在的外网漏洞。
...
| 子域 | 文件/行数/bytes | 本轮状态 |
|---|---:|---|
| command core与commandlet | 25 / 6,676 / 6038 nonempty / 218,333 bytes | E3：当前`core/commands`、`core/commandlet`、operation注册与执行契约；含59个测试属性、4个ignored benchmark |
| product routing与presentation bridge | 43 / 7,572 / 7003 nonempty / 278,819 bytes | E3：当前settings、eval projection、control/event/operation dispatch、retained menu/palette、workbench路由；含48个测试属性、1个ignored benchmark |
| public plugin/control boundary | 4 / 856 / 785 nonempty / 28,472 bytes | E3：runtime interface contribution/control DTO、plugin SDK builder与Editor materializer；含6个测试属性 |
| selected combined scope | 72 / 15,104 / 13,826 nonempty / 525,624 bytes | 当前工作树静态复核集合；共113个测试属性、5个ignored benchmark；fingerprint `af18b704e5e19c67c094669c7fb7d1e74fdabc8876de38ab51e7bd1a09475323` |

fingerprint按相对路径排序，将`path + NUL + per-file SHA-256 + LF`拼接后计算SHA-256。它标识本轮阅读集合，不是schema或兼容性ID；实现前若任一相关文件变化，必须重新计算并复核。

### 2.2 当前源码复核说明

本轮直接读取了命令核心、commandlet、宿主control/event/operation路由、keymap服务、workbench menu/palette桥、插件贡献DTO/SDK/materializer及对应行为测试。工作树中仍有与本报告无关的dirty文件；没有回退、格式化、暂存或提交它们，也没有用无关dirty测试证明命令系统正确。`source_recheck_required`表示任何实现前仍需复取完整文件和fingerprint。
...
UI树、绘制、popup geometry和layout persistence分别由Editor 01/02拥有；本报告只审查它们进入command authority的边界。当前命令面板的`EditorCommandPaletteCatalog::query_window_with_mru`已经具备候选posting、bounded heap、分页window和generation handle，因此旧版“没有查询索引/分页”结论在本刷新中关闭；它仍不是跨域内容搜索，也没有query预算、Unicode一致性和统一disabled reason。

### 2.4 当前源码关键证据

| 证据 | 当前实现 | 工程化判断 |
|---|---|---|
| `zircon_editor/src/core/commands/palette.rs:214-334` | immutable catalog按registry generation缓存；query使用稀有byte posting、fuzzy score、bounded heap并返回轻量handle window | 命令发现基础已成立；需要补query输入预算、Unicode规范化、取消/帧预算和跨surface disabled reason |
| `zircon_editor/src/core/commands/keymap.rs:57-137` | signature bucket后比较完整chord；冲突可计算，但resolve遇到两个enabled候选直接返回`None` | 局部防误派发成立；冲突仍是诊断而不是设置admission/优先级策略 |
| `zircon_editor/src/ui/host/editor_event_control_requests.rs:17-205` | `CallAction`检查`callable_from_remote`；`EditorOperation` binding由`InvokeBinding/InvokeRoute`直接以`UiBinding` source执行；`EditorCommand` binding则经`Headless -> Remote`重新复核 | P0-01在operation payload分支仍可复现，同一control family的授权和来源语义不一致 |
| `zircon_editor/src/core/plugin/materializer.rs:109-115` | 序列化`Command`转成`EditorCommandDescriptor::operation`，没有同步factory | P0-03当前仍可复现，发现与执行没有闭环 |
| `zircon_runtime_interface/src/editor_contribution.rs:216-220`、`zircon_plugins/plugin_sdk/src/editor_contribution.rs:119-126` | public fixture继续接受`command.a`/`sample.command`；宿主`EditorOperationPath`仍要求至少三段 | P0-02当前仍可复现，跨crate ID parser未收敛 |
...
4. 若携普通`EditorCommand`，入口虽以`EditorEventSource::Headless`调用`dispatch_binding`，当前`operation_source_for_event_source`会把它映射为Remote并复核descriptor gate，这一旧分支已关闭；但普通`MenuAction`等非command binding仍可直接归一化为event，control family没有统一principal/policy入口。
...
| ID | 当前差距 | 必须重构为 |
|---|---|---|
| E-CMD-P1-21 | 当前resolver会在同一signature bucket内评估所有enabled候选，并在出现第二个候选时fail-close为`None`；但这个歧义没有typed outcome、owner/priority或产品修复入口。 | conflict是设置事务的一等结果；按scope/context/priority解析，无法唯一决定时返回可展示的双方owner、来源与修复动作。 |
| E-CMD-P1-22 | disabled候选已不会遮蔽唯一enabled候选，这一旧差异关闭；剩余缺口是resolver只返回`Option<&str>`，无法区分未绑定、输入不支持、全部disabled、歧义或orphan。 | 返回typed `KeyResolutionOutcome`，携候选、enablement/shadow原因、context/keymap generation与最终决策。 |
| E-CMD-P1-23 | override仍可引用未知command；当前dispatch closure会把它视为disabled而不会截获已注册候选，但active keymap仍保留orphan且缺admission/诊断/清理生命周期。 | 设置写入与registry generation联检；orphan binding隔离显示并提供迁移/删除，不进入active dispatch index。 |
...
| E-CMD-P1-40 | 当前已有100,000命令的ignored release benchmark，并断言MRU membership相对旧算法的P95至少改善50%；但它只测MRU membership，不测完整query/rebuild/commit，也不覆盖Unicode、插件churn和worst-case common byte。 | required benchmark定义10k/100k catalog、冷/热query、增删owner、Unicode和worst-case分布的p50/p95/p99、allocation、cancel latency与frame预算。 |
...
| E-CMD-P2-10 | 当前113项测试属性仍缺InvokeRoute remote gate、InvokeBinding deny、plugin command端到端执行/卸载、Unicode搜索、冲突fallback和keymap UI合同。 | 把这些加入required behavior/fault/compat lanes，source-shape断言不能替代执行测试。 |
...
| Unity Graphics | 本轮没有找到Editor command shell权威实现 | `dev/Graphics`是graphics packages/test/build局部源码 | 不推断闭源Unity Editor命令系统，也不以缺失参考为Zircon旁路辩护。 |
...
13. unbound、unsupported input、all-disabled、ambiguous与orphan override返回彼此可区分的typed outcome；orphan不进入active dispatch index。
