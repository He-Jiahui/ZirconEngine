---
title: Runtime Gameplay Framework、Game Instance、World Context、Level、Game Mode、Game State、Local Player、Controller、Pawn、Possession、Spawn、Travel、Network、Save 与 Product Integration Current Working Tree Review
category: zircon_runtime
report_id: Runtime197
review_date: 2026-08-31
baseline_head: 369ddacdd48498beaccc436a5c712d258c4b20d9
observed_head: 14c89f9776bed828cc85e05e4b9914b3f8d1e784
baseline_epoch: 380
supersedes:
...
本轮对 Runtime127 原有账本按当前源码重判为：**0 项本地新增 P0；49 P1 Open、23 P1 Partial、0 Closed；16 P2 Open；30 Gate Fail、9 Gate Partial、1 Gate Pass**。没有新增 finding，也没有把旧 Partial 误报为 Closed。目标不是复制 Unreal 的 UObject 类层次，而是建立同等工程语义：`GameplayFrameworkService -> GameInstanceRegistry -> WorldContextRegistry -> ExperienceRuntime/GameRuleAuthority -> LevelSet -> PlayerDirectory -> Controller/Pawn possession -> per-player Input/View/Audio -> Network/Save/Travel adapters`，所有转换必须 generation-scoped、异步可取消、事务发布、具有 terminal receipt，并在 client/listen/dedicated/headless/PIE/multi-world 下隔离。
...
|---|---:|---|
| Zircon current-worktree focused selection (deduplicated) | 297 / 56,310 / 51,644 / 2,047,689 / 478 / 0 | `25aec46e1f24d78509e3ab6a8f0d3b1cd57dd81e91b3e248d07a5ac7a690ce57` |
| Selected five-engine evidence | 25 / re-read in this review / 26 Rust test declarations / 0 | `reference slices rechecked; no runtime fingerprint reused` |
...
- registration baseline 是 `369ddacdd48498beaccc436a5c712d258c4b20d9`，baseline epoch 为 380；报告落笔时共享 HEAD 为 `14c89f9776bed828cc85e05e4b9914b3f8d1e784`。
- 工作树含大量其他 session 与用户在途修改；本轮只租用本报告、Runtime 索引、根索引和覆盖台账，不回退、不归因其他文件。
- 旧 Runtime127 的 153 文件冻结已经不能代表当前实现；本轮按 runtime、app、interface、Vampire/WOC consumer 和相关测试路径重新去重为 297 文件，并重新读取 25 个参考文件。
- 当前 focused 集合没有 ignored test marker；测试声明计数为 478。零 ignored 只描述选中文件的静态 marker，不等于产品测试已执行。
- source zero scan 对 12 个 gameplay 概念仍没有 production owner/consumer；结论来自 source、owner、consumer、lifecycle、product test 同时缺失，而不是单纯依赖零命中。
...
| 领域 | 当前工作树证据 | 工程含义 |
|---|---|---|
| Session construction | `zircon_runtime/src/dynamic_api/session/construction.rs:72-340` 构造单个 `CoreRuntime`、单个 task scope、单个 `LevelSystem`、输入管理器、UI 与 camera；`RuntimeDynamicSession` 在 `:340` 返回 | session 是 ABI/运行时容器，不是跨 World 的 GameInstance owner |
| Profile/role | `zircon_runtime/src/dynamic_api/session/profile.rs:15-76` 将 `Headless` 映射到 `ProductTimeProfile::Headless`，但 `target_mode()` 的 Headless 分支返回 `ClientRuntime` | dedicated/headless authority profile 仍未闭合 |
| World replacement | `zircon_runtime/src/scene/level_system.rs:52-80,123,232-242,322-365,399-453,499-559` | 有 generation/epoch/CAS 与 frame snapshot，但生命周期仍只有 Loaded/Unloaded，snapshot 仍复制 World |
| Level registry | `zircon_runtime/src/scene/module/default_level_manager.rs:14-30` 是 `HashMap<WorldHandle, LevelSystem>`；`level_manager_lifecycle.rs:90-147` 创建并插入，`core/framework/scene/world_handle.rs:4-14` 是裸 `u64` | 无 context owner、retirement、stale-handle 拒绝或多 World product contract |
| Scene artifact I/O | `zircon_runtime/src/scene/module/level_manager_project_io.rs:20-114` save 使用 snapshot + async ticket，load 同步创建新 Level | 这是场景工件 I/O，不是 Gameplay SaveGame participant/restore graph |
| Script context | `zircon_runtime/src/script/vm/runtime_context.rs:9-15,76-128` context 只有 CoreWeak、LevelSystem、EntityId、delta；`scene_system.rs:168-288` 按单 world/entity 回调 | 无 instance/world/player/authority scope，脚本无法获得 typed gameplay owner |
| Spawn/despawn | `zircon_runtime/src/script/vm/gameplay_host/lifecycle.rs:17-82,177-188` 直接 `spawn_node`/`remove_entity`/改 transform；`scene_transition.rs:11-54` 将请求写入单个 World resource | 没有 SpawnService、admission、预算、队列或 terminal receipt；后一个 transition 可覆盖前一个 |
| Network | `zircon_runtime/src/core/framework/net/manager.rs:8-72` 只有 trait；`session.rs:5-136` 与 `sync.rs:5-319` 是 DTO；`rpc.rs:174-203` 明确为 diagnostics-first | 没有 NetManager 实现，也没有 login/player/possession/travel consumer |
| App/headless | `zircon_app/src/entry/entry_runner/headless.rs:7-11` 仅 compose 后返回；`runtime_entry_app/frame_loop.rs:5-44` 只 tick 一个 runtime session | 无 dedicated/headless gameplay loop、network poll、save/travel/shutdown 合同 |
| Input/camera | `zircon_runtime/src/dynamic_api/session/events.rs:57-140` 处理 session-global viewport/input 事件；`camera_controller.rs:21-145` 初始化单 Orbit controller | 没有 LocalPlayer/Controller/View ownership 或 split-screen isolation |
| Vampire/WOC consumers | `examples/vampire/scripts/vampire_game/main.zr:60-115` 硬编码 camera entity `1`；`examples/woc/native/apps/woc_client/src/presentation/authority.rs:10-78` 私有 `ClientAuthority` 包装 WOC runtime | 产品仍各自拥有玩法真相，不能作为 Zircon framework completion 证据 |
...
- `review_status: review_complete` 仅表示 Runtime197 当前工作树复核和差异文档完成；implementation、MVP 和产品资格仍为 pending。
...
- 本轮未执行 Cargo 或产品动态验证，因为没有 production/test 代码改动；静态验证只覆盖文档格式、路径、链接、编号、计数与 currentness。聚焦集合的 478 个测试声明均未因本报告而执行。
