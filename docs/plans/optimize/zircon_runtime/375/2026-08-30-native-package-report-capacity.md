---
title: Runtime375 Native Package Report Capacity
category: zircon_runtime
report_id: Runtime375-native-package-report-capacity-2026-08-30
date: 2026-08-30
session_id: root-runtime-editor-optimize-20260829-r5
implementation_status: implementation_complete
validation_status: managed_validation_pending
---

# Runtime375 Native Package Report Capacity

`from_native_package_manifest` now reserves one diagnostic-vector slot per package module before
validation and registration. Package projection, runtime-module filtering, contribution
registration, and diagnostic ordering remain unchanged.

Regression coverage checks the capacity contract and validation-before-registration order. The
ignored Windows Release benchmark emits `RUNTIME375_NATIVE_PACKAGE_REPORT_CAPACITY_BENCH_V1` over
17 paired samples with 128 modules per sample and requires
`optimized_p95_ns <= legacy_p95_ns * 0.70`.

No direct Cargo validation was run. The coordinator owns combined Runtime/Editor Release
compilation, regression tests, ignored benchmarks, record finalization, manifest-only commit/push,
and one-shot WeCom publication after measured evidence passes the declared gate.

## Current batched validation handoff (2026-08-30)

Runtime375 is submitted in the eight-task batch under request
`runtime375-378-editor321-324-performance-batch-20260830-v1`. Receipt, validation ticket, and source
manifest details are recorded in the session submission log after acceptance. Cargo, performance,
review, commit, push, and WeCom remain coordinator-owned.
