# Runtime361 Action Reference Cache
...
Action-policy validation now caches the optional action reference once per binding before deriving
route and action names. Existing action-over-legacy-route precedence and diagnostics are unchanged.

## Static Evidence

- One `binding.action.as_ref()` lookup feeds both route and action extraction.
- Behavior and source-contract tests cover action precedence and the single lookup contract.

## Performance Gate

The ignored Windows release benchmark emits `RUNTIME361_UI_ACTION_REF_CACHE_BENCH_V1`. It compares
the previous repeated optional-reference lookup with the cached-reference path over 2,000,000
bindings per sample across 17 interleaved samples and requires
...
Runtime361 is included in the six-group Runtime/Editor batch under request
...
binding fixture is held behind `black_box` before measurement; the 30% p95 gate remains. Cargo,
