...
`EntityPath::parse` now reserves a bounded vector capacity from the input length before the
existing split, trim, filter, and ownership pipeline. This removes repeated segment-vector
reallocation without adding a second input scan or changing parsing and error semantics.

## Static Evidence

- Input scans: `1 -> 1`.
- Segment-vector reallocations: variable growth -> `0` for the benchmark path.
- Parsed segment contents and empty-path errors remain unchanged.
...
compares the baseline unreserved segment vector with the length-preallocated candidate over 1,024
segments, 8,192 checks per sample, and 31 interleaved sample pairs and requires
