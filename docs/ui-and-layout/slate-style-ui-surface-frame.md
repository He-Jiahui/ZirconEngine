...
The retained render-cache boundary lives below `zircon_runtime/src/ui/surface/render/cache.rs`. `UiSurface::render_extract` is the only complete `UiRenderCommand` authority; `UiSurfaceRenderCache` beside it is transient derived metadata rather than a second command store. Per-node emission ordinals retain command indexes and old frames, so full rebuild comparison borrows the previous extract, local patch comparison reads the current extract, and stale/mismatched metadata rebuilds fail-closed while preserving damage. A node may emit multiple commands; each ordinal is tracked independently, global command reorder updates indexes without changing the payload authority, and non-contiguous node runs remain ineligible for range-based local access. Geometry changes damage the union of old and new command frames; visual-only changes damage the current frame; unchanged render-only dirty flags reuse the existing commands and report no damage. `render_cache` is omitted from `UiSurface` serialization and rebuilt from the retained extract on the next render update; legacy serialized cache entries are deliberately not an active compatibility contract. Paint conversion still fills `UiPaintElement.cache_generation` with a stable hash of the command payload so render-debug/cache DTOs can identify reused paint output instead of always receiving `None`.
...
2026-05-11 dirty-domain tightening adds render-only guards for both reflected property mutation and `DirtyRedraw` dispatch effects. These paths keep `UiDirtyFlags { render: true }` precise and avoid setting legacy `state_flags.dirty`, so retained surfaces do not rebuild arranged output or hit grids for paint-only Material changes. Runtime `mark_layout_dirty(...)` now also emits only structured layout, hit-test, and render dirtiness instead of setting the legacy bit and widening the report to input dirtiness.
...
`debug_surface_frame(...)` is the shared frame-only debug entry point for the Widget Reflector-style milestone. It consumes only `UiSurfaceFrame`, so editor and runtime geometry tooling cannot drift into separate coordinate systems. The snapshot contains reflected arranged nodes, render counters, material batch groups, hit-grid occupancy, overdraw samples, and the focus/capture/hover state that was current when the frame was submitted. Retained tree/component ECS rows are intentionally absent from the hot frame and remain default-empty in this frame-only snapshot; callers that need them use `UiSurface::debug_snapshot*`, which explicitly combines the current frame with an on-demand `UiEcsProjectionSnapshot`.
...
Toolbar hit control ids are separated from projected control ids only where the current editor state supplies a parameterized action or an existing semantic alias must be preserved. For example, projected `ActivateSceneMode` preserves the full built-in or custom scene-mode identity, `FrameSelection` keeps the `frame.selection` semantic id, and direct no-argument buttons such as play-mode controls can use their projected control id. This keeps hit geometry owned by layout while preserving viewport command semantics.
