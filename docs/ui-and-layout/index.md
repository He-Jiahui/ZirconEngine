...
tests:
  ...
  - cargo test -p zircon_editor --lib workbench_reference_visual_asset_remains_design_baseline_not_runtime_overlay --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --nocapture
  - cargo test -p zircon_editor --lib runtime_component_projection_loads_editor_svg_image_preview --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --nocapture
  ...
  - cargo test -p zircon_editor --lib host_projection_does_not_promote_workbench_reference_image_to_root_overlay_node --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_editor --lib host_window_template_bridge_keeps_workbench_reference_out_of_projection --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_editor --lib apply_presentation_keeps_workbench_reference_out_of_root_template_overlay --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_editor --lib full_command_stream_replays_root_overlay_image_pixels --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_editor --lib patch_command_stream_repaints_root_overlay_image_damage_pixels --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --test-threads=1 --nocapture
  - cargo test -p zircon_editor --test integration_contracts --features integration-contracts workbench_window_uses_componentized_workbench_layout --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-workbench-reference-0529 --message-format short --color never -- --nocapture
...
The retained editor shell now treats `docs/ui-and-layout/workbench.png` and its byte-identical editor asset copy only as design baselines. `ui.host_window` still starts on the same `1672x941` canvas, but `workbench_shell.zui` no longer appends `WorkbenchShellReferenceImage`, and `workbench_window.zui` no longer wraps `WorkbenchReferenceImage`. The active documents import `editor_workbench_strict.zui` and are composed from template nodes: top toolbar, activity rail, scene tree, viewport panel, inspector, component drawer, and status bar. `workbench_reference_visual_asset_remains_design_baseline_not_runtime_overlay`, `host_projection_does_not_promote_workbench_reference_image_to_root_overlay_node`, `apply_presentation_keeps_workbench_reference_out_of_root_template_overlay`, and the workbench template integration tests lock that the full PNG remains available for comparison but cannot become a runtime shell or window node.

`root_template_overlay` is now an opt-in generic root overlay path keyed by `root_template_overlay = true`; the workbench shell does not set that property. Presenter command-stream tests still cover root image overlay replay and damage repaint with a small synthetic image, so the rendering feature remains validated without making the editor first screen depend on a full-window screenshot.
...
- [Editor Input Response Contract](./input-response-contract.md): Layout18 的节点级 pointer-events/cursor、单一 root-to-leaf 命中路径和 capture/target/bubble 逻辑次序契约；明确 S1 DTO 已落地，命中 grid/bridge 迁移与指针捕获/拖拽仍分别归 S2/S3。
- [Focus And Navigation Contract](./focus-navigation-contract.md): Layout19 的焦点 mode、focus-visible cause、稳定 Tab 链与导航边界 DTO；方向几何求解与模态焦点作用域仍分别归 S2/S3。
...
- [Componentized Workbench Shell](./componentized-workbench-shell.md): 记录 `workbench.png` 从运行时整窗覆盖回退为设计基线后的当前落点，明确 `workbench_shell.zui`、`workbench_window.zui` 和 `editor_workbench_strict.zui` 的组件化职责，以及 runtime Taffy/input/wgpu 与 editor 模板的边界。
...
- `editor.host.workbench_shell` 和 `editor.window.workbench` 已从整窗参考图覆盖切换到组件化模板：`workbench_shell.zui` 移除 `WorkbenchShellReferenceImage`，`workbench_window.zui` 移除 `WorkbenchReferenceImage`，两者统一导入 `editor_workbench_strict.zui`。`docs/ui-and-layout/workbench.png` 与 `zircon_editor/assets/ui/editor/reference/workbench.png` 继续作为 byte-identical 设计基线保存，但测试只允许它们作为比较资源存在，不允许作为运行时 shell/window 节点出现。
- `root_template_overlay` 现在是通过 `root_template_overlay = true` 显式启用的通用根覆盖层；workbench host projection 默认不会产生 root template overlay。`full_command_stream_replays_root_overlay_image_pixels` 和 `patch_command_stream_repaints_root_overlay_image_damage_pixels` 使用小型合成图片继续验证 presenter command stream 的完整重放与 damage patch 重放路径，避免恢复整窗 PNG 契约。
...
- runtime UI fixture acceptance 已证明 `HudOverlay`、`PauseMenu`、`SettingsDialog` 和 `InventoryList` 全部能从 `.ui.toml` 经 `RuntimeUiManager -> UiSurface -> UiRenderExtract -> UiRenderSubmission -> WgpuRenderFramework::submit_runtime_frame(...)` 进入 screen-space UI pass
