...
- [Componentized Workbench Shell](./componentized-workbench-shell.md): 记录 `workbench.png` 从运行时整窗覆盖回退为设计基线后的当前落点，明确 `workbench_shell.v2.ui.toml`、`workbench_window.v2.ui.toml` 和 `editor_workbench_strict.v2.ui.toml` 的组件化职责，以及 runtime Taffy/input/wgpu 与 editor 模板的边界。
...
- `editor.host.workbench_shell` 和 `editor.window.workbench` 已从整窗参考图覆盖切换到组件化模板：`workbench_shell.v2.ui.toml` 移除 `WorkbenchShellReferenceImage`，`workbench_window.v2.ui.toml` 移除 `WorkbenchReferenceImage`，两者统一导入 `editor_workbench_strict.v2.ui.toml`。`docs/ui-and-layout/workbench.png` 与 `zircon_editor/assets/ui/editor/reference/workbench.png` 继续作为 byte-identical 设计基线保存，但测试只允许它们作为比较资源存在，不允许作为运行时 shell/window 节点出现。
