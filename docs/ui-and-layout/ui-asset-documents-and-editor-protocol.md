...
The editor import audit does not change that ownership split. `zircon_editor/src` already imports many neutral records directly from `zircon_runtime_interface::ui`, but it still has runtime `zircon_runtime::ui` imports where the editor is constructing or dispatching concrete runtime surfaces. The critical lower-layer constraint is that `UiSurface` still owns a runtime behavior `UiTree`; therefore editor files that insert `UiTreeNode`s, set `UiInputPolicy`, dispatch through `UiPointerDispatcher`, or call runtime tree accessors cannot switch those tree names to interface DTOs without a type-identity cutover in `zircon_runtime::ui::surface` first. `UiAssetLoader` and the schema migrator are an example of the intended split: the loader behavior stays in runtime, while its `UiAssetMigrationOutcome` and schema report rows are imported from `zircon_runtime_interface::ui::template`.
...
M16 back-half 在同一 package owner 下新增 binary artifact 与 package manifest surface。`UiDocumentCompiler::compile_package_artifact(...)` 返回 `UiCompiledAssetArtifact { report, compiled }`，`UiCompiledAssetArtifact::to_bytes()` 写入稳定 magic `ZRUIA016`、little-endian `u32` binary artifact schema version 和 little-endian `u64` payload length，再保存 UTF-8 TOML payload。payload 仍是 typed `UiCompiledAssetArtifact { report, compiled: UiTemplateInstance }`，但避免对 `toml::Value` compiled payload 使用 `bincode`。`from_bytes(...)` 校验 envelope 后恢复 artifact。`UiCompiledAssetCacheRecord` 从 artifact bytes 记录 header、完整 M12 `UiCompileCacheKey`、由 key 生成的 `UiInvalidationSnapshot`、artifact fingerprint 和 byte length，用作 future persistent cache index。`UiCompiledAssetPackageManifest` 用 TOML writer/importer 保存 header、dependency manifest、cache record 和 artifact entry；它消费同一个 `resource_dependencies` / `localization_dependencies` manifest surface，不创建第二套 resource 或 localization list。
...
- `rustfmt --edition 2021 zircon_runtime/src/ui/template/asset/compiler/mod.rs zircon_runtime/src/ui/template/asset/compiler/package/mod.rs zircon_runtime/src/ui/template/asset/compiler/package/artifact.rs zircon_runtime/src/ui/template/asset/compiler/package/cache_record.rs zircon_runtime/src/ui/template/asset/compiler/package/header.rs zircon_runtime/src/ui/template/asset/compiler/package/manifest.rs zircon_runtime/src/ui/template/asset/compiler/package/package_manifest.rs zircon_runtime/src/ui/template/asset/compiler/package/profile.rs zircon_runtime/src/ui/template/asset/compiler/package/report.rs zircon_runtime/src/ui/template/asset/compiler/package/validate.rs zircon_runtime/src/ui/template/asset/mod.rs zircon_runtime/src/ui/template/mod.rs zircon_runtime/src/ui/template/instance.rs zircon_runtime/src/ui/tests/asset_package_validation.rs`
