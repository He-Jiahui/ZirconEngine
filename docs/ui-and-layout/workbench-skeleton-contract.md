...
`workbench_main_band.zui`, `workbench_scene_tree_panel.zui`, and `workbench_inspector_panel.zui` import `editor_tokens.zui` and author fixed drawer widths as `$editor.density.left_drawer_width` / `$editor.density.right_drawer_width`. The old inline `332.0` and `404.0` sizes are no longer the source for these shell assets; `$--left-drawer-width` and `$--right-drawer-width` remain runtime-only compatibility aliases for older assets.
