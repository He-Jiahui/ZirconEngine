...
related_code:
  ...
  - zircon_editor/assets/ui/editor/layout/shell_regions.v2.ui.toml
  - zircon_editor/assets/ui/editor/layout/presets.v2.ui.toml
...
`WorkbenchShellRegionsAsset::from_toml_str(...)` parses `shell_regions.v2.ui.toml` into a typed asset header plus validated `RegionBinding` rows. The loader rejects wrong asset kind/id/version, duplicate regions, missing regions, and region-role mismatches before the rows can replace the built-in skeleton regions. `WorkbenchSkeleton::from_shell_regions_asset(...)` keeps the default workbench chrome assets and swaps only the verified region declarations.
...
Page templates are declared in `PageLayoutTemplate::builtin_templates()` and mirrored by `page_templates.v2.ui.toml`. The built-in set now covers 13 editor pages: scene, game, material, material preview, inspector, prefab, UI designer, UI source, animation timeline, animation graph, asset browser, console, and runtime diagnostics. Each template fills the same six semantic regions, chooses a default preset, declares default drawer modes, and optionally defines a center split profile. The templates do not own module data wiring; they only decide which panel asset belongs in which workbench region.
