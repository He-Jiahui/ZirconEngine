...
当前 Zircon 已经具备基础 measure/arrange、`UiArrangedTree`、clip frame、z/paint order、scrollable virtual window 和 shared hit/render 消费同一 arranged frame 的起点。L1 起步契约已在 `zircon_runtime_interface::ui::layout` 中加入 `UiGeometry`、`UiLayoutMetrics`、`UiPixelSnapping`、`UiPixelSnappingPolicy`、`UiSlot`、基础 slot padding/alignment DTO，以及 Linear parent-owned `UiLinearSlotSizing` / `UiLinearSlotSizeRule` size-rule contract；这些类型先作为中立 contract 存在。当前 template build 已经把 parent-owned slot identity 保留进 `UiTree.slots`，同时暂时维持旧 `slot_attributes.layout` 并入 child layout contract 的迁移行为；Linear slot sizing 现在仍只保留在 `UiSlot.linear_sizing`，Overlay slot z-order 已从 `UiSlot.z_order` 进入 arranged/render/hit order，Canvas/Free anchor/pivot/position/offset/auto-size placement 已从 `UiSlot.canvas_placement` 进入 Free/Canvas-style arrange，包含 stretched anchor range 与 Canvas layer group diagnostics。runtime layout pass 现在先在 Linear、Overlay、Free/Container 三类既有 panel 中消费 slot padding、alignment 和 layout iteration order，并让 WrapBox 在 available width 完成换行后把 content-driven non-stretch frame 收束到实际 row content width；hit/render/input 仍只消费 arranged tree。后续 runtime/editor 迁移必须继续从该 contract 往上接入，不能重新把 parent slot 语义塞回 child node。每节点 pixel-snapping 的 source candidate 已接入最终 paint geometry；与 Slate 相比，剩余缺口集中在 transform 运算、完整 slot policy、Grid/Flow/Splitter/Scale panel 算法、DPI/scale 脏传播、可视化调试和布局失效边界。
...
| `SConstraintCanvas.h` / `SConstraintCanvas.cpp` | canvas slot 持有 offset、anchors、alignment、auto-size、z-order；arrange 时从 parent allotted size 计算 anchored local position/size，并输出 layer breaks。 | Zircon 已有 `UiCanvasSlotPlacement` 保存 Free/Canvas parent-owned anchor/pivot/position/offset/auto-size contract；runtime Free/Canvas arrange 现在消费 non-stretch 与 stretched anchor pair，并优先于 child node default placement；Canvas layer grouping 已作为 diagnostics 元数据输出，renderer backend 尚未消费 layer id。 |
...
`UiLayoutMetrics` 现在记录 root logical/physical size、DPI scale、font scale、layout scale、flow direction 和默认 pixel snapping policy；这是 future prepass/arrange 根输入，而不是渲染端私有参数。`UiSlot` 现在作为 parent-owned placement identity 存在，包含 parent/child id、slot kind、padding、alignment、optional Linear sizing、optional Canvas/Free placement、layout iteration order、Overlay z-order 和 dirty revision；`UiTree.slots` 会保存 template build 产生的 parent-child slot record，并通过 serde default 兼容旧 tree payload。`UiLinearSlotSizing` 记录 Linear parent main-axis policy：`Auto`、`Stretch`、`StretchContent`、grow/shrink coefficient、min 和 max，直接对应 Slate `SBoxPanel` slot 的 `AutoWidth/AutoHeight`、`FillWidth/FillHeight`、`FillContentWidth/FillContentHeight`、min/max 语义。Overlay parent 下的 `slot.layout.z_order` 会保留到 `UiSlot.z_order`，直接对应 Slate `SOverlay::FOverlaySlot::ZOrder` 的 parent-owned layer intent，并已进入 arranged/render/hit z-order authority。Free/Canvas-style parent 下的 `slot.layout.anchor`、`pivot`、`position`、`offset`、`anchor_max` 和 `auto_size` 会保留到 `UiSlot.canvas_placement`，对齐 Slate `SConstraintCanvas::FSlot` 的 parent-owned placement input，并已由 runtime Free/Canvas arrange 消费 fixed 与 stretched anchor ranges。runtime layout pass 的 `slot.rs` 只做局部 `(parent_id, child_id, container kind)` lookup，不扩大 shared tree API；Linear measure/arrange 会把 slot padding 计入 desired/main-axis extents，slot order 会稳定 child layout iteration，Free/Container/Overlay 会把 slot padding/alignment 应用于 parent content frame。当前 runtime 尚未消费 `linear_sizing`，因此 L4 BoxPanel size-rule parity 仍只是 preserved input；Overlay z-order 和 Canvas/Free placement 已有 runtime consumption。缺失或 default-only slot 继续回退到旧 anchor/pivot/position 行为。

V2 `.zui` 节点现在可声明 `pixel_snapping = "inherit" | "disabled" | "snap_to_pixel"`。声明随 component expansion 和 compiled arena 进入 template metadata；省略 mount 值会保留组件原型值，显式 `inherit` 则恢复父节点策略。render extract 在文本产物准备完成后按树一次解析继承并写入 `UiResolvedStyle`，paint-element construction 再以 surface `UiLayoutMetrics.pixel_snapping` 为 fallback，同时处理 render frame 与实际 clip frame。该策略只改变 paint geometry，arranged tree、hit-test、滚动和动画状态仍保留逻辑坐标；resolved style 进入 command stable serialization，因此策略变化会使可复用 paint command generation 变化。当前 source tests 覆盖 compiled round trip、继承/override，以及 125%/150% DPI 的 subpixel-preserve 与 device-aligned 两条路径；Rust 编译和 Editor native WGPU 多 DPI framebuffer 验收仍待 current-source validation。
...
| Slot policy | `TBasicLayoutWidgetSlot` / `LayoutUtils` 统一处理 padding、horizontal/vertical alignment、size rule、min/max、flow direction。 | `.ui.toml` 和 tree node 当前主要表达 node-level constraints、gap、anchor/pivot/position；`UiSlot` / `UiMargin` / `UiAlignment2D` 已进入 interface contract，template build 会为每条 parent-child edge 保留 `UiTree.slots` record，runtime layout pass 已开始消费 Linear/Overlay/Free padding、alignment 和 layout order；Overlay `z_order` 与 Canvas/Free placement 已进入 runtime arrange/frame authority。 | 仍需 main-axis size rule runtime consumption、cross-axis default policy、RTL flow、grid row/column/span、flow flags、layer/nudge、slot mutation diagnostics。 |
| Slot invalidation | Slate slot attributes 是 `TSlateContainedAttribute`，slot padding/alignment 等变化能直接打 layout dirty。 | `UiSlot` 已有 `dirty_revision`，Overlay z-order 与 Free canvas placement 运行时 mutation 会更新 parent slot revision 并触发相应 dirty domain；template build 仍保留旧 merge 迁移行为。 | 需要把更多 slot attribute mutation 纳入同一 revision/dirty reason，并让 diagnostics 明确 source slot。 |
| Canvas / free placement | `SConstraintCanvas` 用 slot offset、anchors、alignment、auto-size、z-order 从 parent geometry 派生 child local geometry。 | `UiSlot.canvas_placement` 已能保留 Free/Canvas-style parent-owned anchor/pivot/position/offset/anchor_max/auto-size；`free_child_frame()` 现在优先消费 parent slot placement，fixed 或 stretched anchor slot size、auto-size desired extent、Canvas z-order 和 Canvas layer-group diagnostics 会进入 shared arranged/render/hit/debug authority。 | 仍需 renderer backend `LayerId`/batching consumption、richer desired-size auto-size edge cases 和 slot diagnostics。 |
...
| Overlay / Container | `SOverlay`/single-child helpers 通过 slot padding/alignment 把 child 放入 allotted geometry，Overlay slot 还携带 parent-owned z-order。 | Free/Container/Overlay 现在在存在非默认 slot padding/alignment 时使用 parent content frame + slot alignment；Overlay slot `z_order` 已能从 template 保留到 `UiSlot` 并进入 arranged/render/hit order。缺失或 default-only slot 继续使用旧 `free_child_frame()` anchor/pivot/position fallback，除非 Free slot 带有 `canvas_placement`。 | 仍需 paint layer grouping、slot diagnostics 和更多 runtime slot mutation coverage。 |
...
| `UiCanvasSlotPlacement` | anchor, anchor_max, pivot, position, offset, auto_size | Free/Canvas parent placement payload；runtime Free/Canvas arrange 已消费 fixed 与 stretched anchor ranges，并输出 Canvas layer grouping diagnostics。 |
...
- Overlay parent 下的 z/layer intent 归 slot；当前 `slot.layout.z_order` 会保留到 `UiSlot.z_order`，并已由 runtime arranged/render/hit order 消费。后续 Overlay parity slice 仍需要补 layer grouping diagnostics。
- Canvas/Free parent 下的 anchor/offset/alignment 归 slot；`slot.layout.anchor`、`anchor_max`、`pivot`、`position`、`offset` 和 `auto_size` 会在 Free/Canvas slot 下保留到 `UiSlot.canvas_placement`。runtime frame 现在优先消费该 parent slot placement，再回退到 child node default placement；stretched anchor ranges 与 Canvas layer grouping diagnostics 已进入 runtime authority，后续 Canvas/Free parity 还需要 renderer layer consumption 和 richer slot diagnostics。child node 只保留默认 placement intent，避免同一个 child 在不同 parent 中语义冲突。
...
5. 再补 Canvas/Free slot placement contract，作为 `SConstraintCanvas::FSlot` parity 的 parent-owned input。当前 `UiCanvasSlotPlacement` 已进入 interface contract，template build 会从 Free/Canvas parent 下的 `slot.layout.anchor`、`anchor_max`、`pivot`、`position`、`offset` 和 `auto_size` 保留到 `UiSlot.canvas_placement`；runtime Free/Canvas arrange 已消费 fixed 与 stretched anchor ranges，并让 parent slot 覆盖 child default placement。后续还要补 renderer layer consumption、richer slot diagnostics 和更完整的 edge-case coverage；非 Free/Canvas parent 会忽略该 payload。
...
9. **L8 DPI / Pixel / Debug**：surface root 持有 logical/physical size 与 DPI/font scale；每节点 `.zui` pixel-snapping policy 已进入 resolved render command，并只在 paint-element construction 改写 render/clip geometry。仍需让 Widget Reflector 输出 prepass cache、slot policy、layout/render transform、逻辑/绘制 geometry、pixel snapping 和 rejected reason，并完成 native WGPU 多 DPI framebuffer 验收。
...
- Overlay tests：padding/alignment、declaration order、slot `z_order` preservation、same-z declaration-order tie break、paint/hit order consumption 都需要 golden；当前 `template_tree_builder_preserves_overlay_slot_z_order_contracts` 覆盖 parent-owned `z_order` contract preservation，`overlay_slot_geometry_feeds_arranged_render_hit_and_z_order_from_one_surface_frame` 和 `overlay_slot_z_order_mutation_rebuilds_arranged_hit_and_render_without_layout` 覆盖 runtime arranged/render/hit order 与 slot mutation dirty domain。Overlay layer grouping diagnostics 仍待后续 parity slice。
- Canvas/Free tests：slot `anchor`、`pivot`、`position`、`offset`、`anchor_max`、`auto_size` preservation、非 Free/Canvas parent 忽略 authored placement、slot-vs-child default precedence、stretched anchors、auto-size desired-size 和 z/layer grouping 都需要 golden；当前 `ui_canvas_slot_placement_contract_round_trips_and_defaults` 与 `template_tree_builder_preserves_canvas_free_slot_placement_contracts` 覆盖 parent-owned placement contract preservation，`free_layout_consumes_canvas_slot_placement_before_child_default_anchor` 覆盖 runtime fixed-anchor consumption、auto-size 和 shared render/hit frame authority，`free_canvas_slot_stretches_between_min_and_max_anchors` 与 `free_canvas_slot_can_stretch_one_axis_and_keep_pivot_on_the_other` 覆盖 stretched anchor ranges，`canvas_container_canvas_slot_drives_layout_render_hit_and_z_order` 与 `canvas_container_groups_same_z_order_children_into_one_layer` 覆盖 true Canvas slot authority and layer grouping diagnostics。renderer layer consumption 仍待后续 Canvas parity slice。
...
- DPI/pixel tests：DPI scale 改变 desired size 与 arranged frame；pixel snapping 只影响 render paint geometry，不改变 hit debug layout geometry。当前 source regressions 已覆盖 125%/150% DPI 下 disabled quarter-physical-pixel translation 与 snap-to-pixel render/clip alignment，并覆盖 `.zui` component 继承/显式 override；focused Rust 编译和产品 framebuffer 仍待 current-source validation。
