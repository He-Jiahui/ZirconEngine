...
related_code:
  ...
  - zircon_editor/src/ui/retained_host/callback_dispatch/viewport/mod.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/viewport/route_mapping.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/viewport/snap_cycle.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/mod.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/projection_support.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/floating_window_source/mod.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/floating_window_source/bridge.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/floating_window_source/error.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/floating_window_source/source_frames.rs
  ...
  - zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/viewport_toolbar/mod.rs
  ...
  - zircon_editor/src/ui/retained_host/callback_dispatch/layout/floating_window/tests.rs
  - zircon_editor/src/ui/retained_host/detail_pointer/mod.rs
...
tests:
  ...
  - zircon_editor/src/tests/host/retained_menu_pointer/layout.rs
  - zircon_editor/src/tests/host/retained_menu_pointer/surface_contract.rs
  - zircon_editor/src/tests/host/retained_activity_rail_pointer/mod.rs
  - zircon_editor/src/tests/host/retained_host_page_pointer/mod.rs
  - zircon_editor/src/tests/host/retained_document_tab_pointer/
  - zircon_editor/src/tests/host/retained_drawer_header_pointer/mod.rs
  - zircon_editor/src/tests/host/retained_viewport_toolbar_pointer/mod.rs
  - zircon_editor/src/tests/host/retained_tab_drag/
  ...
  - cargo test -p zircon_editor --lib inspector_template_body_projection_replaces_legacy_inspector_view_data_for_retained_conversion --locked --jobs 1
  ...
  - cargo test -p zircon_editor retained_callback_dispatch -- --nocapture
  ...
  - cargo test -p zircon_editor --test integration_contracts --features integration-contracts --locked workbench_retained_shell -- --nocapture
  ...
  - cargo test -p zircon_editor retained_tab_drag -- --nocapture
  ...
  - cargo test -p zircon_editor --lib retained_detail_pointer --offline -- --nocapture
  - cargo test -p zircon_editor --lib retained_activity_rail_pointer --offline -- --nocapture
  - cargo test -p zircon_editor --lib retained_host_page_pointer --offline -- --nocapture
  - cargo test -p zircon_editor --lib retained_document_tab_pointer --offline -- --nocapture
  - cargo test -p zircon_editor --lib retained_drawer_header_pointer --offline -- --nocapture
  - cargo test -p zircon_editor --lib retained_ --offline -- --nocapture
  - cargo test -p zircon_editor --test integration_contracts --features integration-contracts --locked workbench_retained_shell -- --nocapture
  ...
  - cargo test -p zircon_editor --lib --locked tests::host::retained_window::child_window_callback_wiring_tracks_source_window_for_pane_interactions
  ...
  - rustfmt --edition 2021 --check "zircon_runtime_interface/src/ui/layout/scroll.rs" "zircon_runtime_interface/src/ui/layout/mod.rs" "zircon_runtime/src/ui/template/build/parsers.rs" "zircon_runtime/src/ui/template/build/layout_contract.rs" "zircon_runtime/src/ui/layout/pass/measure.rs" "zircon_runtime/src/ui/layout/pass/arrange.rs" "zircon_runtime/src/ui/tests/material_layout.rs" "zircon_runtime/src/ui/tests/shared_core.rs" "zircon_editor/src/tests/ui/boundary/global_material_surface_assets.rs" "zircon_editor/src/tests/ui/boundary/mod.rs" (M3 responsive/global-surface gate: passed with no output)
  - cargo test -p zircon_runtime --lib material_layout --locked --jobs 1 --target-dir D:\cargo-targets\global-ui-m3-validation --message-format short --color never -- --nocapture (M3 responsive/global-surface gate: passed with 23 passed, 0 failed)
  - cargo test -p zircon_runtime --lib shared_core --locked --jobs 1 --target-dir D:\cargo-targets\global-ui-m3-validation --message-format short --color never -- --nocapture (M3 responsive/global-surface gate: passed with 39 passed, 0 failed)
  - cargo check -p zircon_runtime_interface --lib --locked --jobs 1 --target-dir D:\cargo-targets\global-ui-m3-validation --message-format short --color never (M3 responsive/global-surface gate: passed)
  - cargo test -q -p zircon_editor --lib global_material_surface_assets --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui -- --nocapture (M3 final post-portability global surface gate: passed with 2 passed, 0 failed, 1071 filtered out)
  - cargo test -q -p zircon_runtime --lib material_layout --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui -- --nocapture (M3 final responsive/material regression gate: passed)
  - cargo test -q -p zircon_runtime --lib shared_core --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui -- --nocapture (M3 final WrapBox/shared-core regression gate: passed)
  - cargo check -p zircon_runtime_interface --lib --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui --message-format short --color never (M3 final interface gate: passed)
  - cargo check -p zircon_runtime --lib --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui --message-format short --color never (M3 final runtime gate: passed with existing warning noise)
  - cargo check -p zircon_editor --lib --locked --jobs 1 --target-dir E:\zircon-build\targets\global-ui --message-format short --color never (M3 final editor gate: passed with existing warning noise)
...
- [`resolve_callback_surface_size_for_kind(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/helpers.rs) 现在继续作为真实宿主的统一兜底入口，优先顺序固定为 `callback size -> cached size -> shared host frame`
...
- `browser_asset_details_pointer_scrolled(...)` 已经从“只相信 host callback 上传的局部尺寸”切到 `ViewContentKind::AssetBrowser` 的 shared frame fallback，所以 exclusive asset browser page 在真实宿主里也不会再因为零尺寸回调失去 shared scroll authority
...
- [`build_host_menu_pointer_layout(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/menu_pointer/build_host_menu_pointer_layout.rs) 不再接收或依赖 legacy `get_*_menu_button_frame()` 结果；当 `WorkbenchMenuBarRoot` 暂时缺席时，会直接从 shared `shell_frame` 继续推导 top-level menu button row
- [`app/pointer_layout.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/pointer_layout.rs) 已删除 `legacy_menu_button_frames(...)` 调用，真实宿主里 menu hit-test/popup presentation 现在只剩 shared root-frame authority
- [`BuiltinWorkbenchRootShellFrames`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/workbench/root_shell_frames.rs) 现在额外导出 `Left/Right/BottomDrawerContentRoot` frame；[`resolve_host_frame_backed_size_for_kind(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/helpers.rs) 会先消费这些 shared drawer content frame，再回退到 `geometry.region_frame(...)`
...
这一轮 shared template/layout authority 不再只给 callback fallback 提供局部读取的 frame，root `retained_host` 的 presentation 主链也开始真正消费这份 shared frame authority：

- [`BuiltinWorkbenchTemplateBridge::root_shell_frames()`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/workbench/bridge.rs) 现在把 `WorkbenchMenuBarRoot`、`WorkbenchBody`、`DocumentHostRoot`、`PaneSurfaceRoot`、`StatusBarRoot` 这组 builtin root-shell control frame 固定打包成宿主结构
...
- leaf 节点现在会从 template metadata 的文本载荷解析 intrinsic text size；Material-capable controls then apply authored `layout_*` metrics in the shared runtime measure pass, so text width plus Material padding/min-height determines desired size before the native painter sees a frame
...
2026-05-05 的 native editor text/button follow-up 把这个规则从 editor painter workaround 下沉到 shared runtime layout pass。`text_measure.rs` 复用 render extract 的 text/style resolver 估算 leaf text intrinsic size，`measure.rs` 只在 leaf 内容存在时为 `Button` / `IconButton` 加水平/垂直 padding，`axis.rs` 只把默认子约束改成 measured fixed main extent，避免覆盖 authored constraints。对应 regressions 是 `layout_pass_measures_label_leaf_from_text_intrinsic_size` 和 `layout_pass_measures_button_leaf_as_text_plus_padding`；editor projection 侧的 `inspector_template_body_projection_replaces_legacy_inspector_view_data_for_retained_conversion` 进一步锁住真实 `ApplyDraft` frame 已经带着 shared intrinsic text size + padding 进入 native host contract。

2026-05-06 的 global Material measurement milestone moves the newer Material path into `layout/pass/material.rs`. `measure.rs` first resolves intrinsic text through `measure_text(...)`; `measure_material_content(...)` then applies authored metrics such as `layout_padding_left/right/top/bottom`, `layout_spacing`, `layout_min_width`, `layout_min_height`, `layout_icon_size`, and optional leading/trailing slot widths to both leaf text content and aggregate child content. The shared visible-text resolver always honors explicit non-empty `text` and `label`, but it only falls back to `value`, `placeholder`, and `options` for text-bearing/control components such as buttons, fields, combo/list/table/menu rows, tabs, switches, labels, dropdown/radio/segmented controls, and related Material roots. IconButton labels remain accessibility-only when an actual icon/image/source is present or when `layout_icon_size` is positive; `layout_icon_size = 0` means there is no visual icon, so the label is allowed to provide intrinsic text width. That keeps InputField/TextField/ComboBox/NumberField/RangeField desired sizes based on what the user sees without turning asset-bearing Image/Icon/SvgIcon nodes into text render commands; the shared image resolver reads `source` as an image asset so SvgIcon renders as image rather than Group. Option fallback is scalar-only and deterministic: strings, numbers, and bools become labels; table options scan `text`, `label`, `value`, `name`, then `title`, skipping empty or non-scalar values instead of stringifying arrays/tables wholesale. The supported Material-capable roots now cover buttons, icon/toggle/checkbox controls, input/text/number/range fields, combo/list rows, switch, menu item/frame roles, tabs, table/virtual rows, progress/spinner, and conservative labels. Plain `Label` nodes remain text-only unless at least one explicit `layout_*` attribute is present, which prevents non-Material labels from growing just because `Label` is in the supported-role list. Component Showcase projection coverage now reads the shared surface `desired_size` for Material controls before asserting final host frames, so wide stretched parents cannot mask a collapsed intrinsic measurement. Popup anchors stay outside the measurement algorithm: Material popup wrappers forward `popup_anchor_x` and `popup_anchor_y` as authored or retained component metadata, while shared layout continues to own only the arranged frame and desired size. Runtime HUD/dialog Button controls now carry the same explicit `layout_*` Material metrics, so their desired sizes are produced by this shared pass rather than native painter fitting; the duplicated raw values are intentionally mechanical Milestone 2 participation and remain a Milestone 4 theme/token normalization candidate.

同一天的 viewport follow-up 不是 layout 规则本身，但继续沿用同一条 shared frame/clip authority：`HostViewportImageData` 只保存 `RetainedViewportController` 已接受的新 RGBA viewport image，`HostWindowPresentationData` 把它作为 paint-only overlay 叠进 native host presentation，`draw_rgba_image_clipped(...)` 再通过 painter 的统一 clip 规则把它采样到 Scene/Game pane body。`native_host_painter_composites_latest_viewport_image_into_scene_body` 覆盖这条 bridge，防止 runtime capture 成功但 Rust-owned native pane 只显示 placeholder chrome。
...
- dispatcher 现在显式实现 `Handled`、`Blocked`、`Passthrough`、`CapturePointer`、`ReleasePointerCapture`、`SetFocus`、`ClearFocus`、`RequestDirty` 和 `RequestDamage`
- `Blocked` 会停止当前 bubble route 并尝试下一个同位置 stacked target
- `Passthrough` 会允许当前 route 之后继续把事件派给后续 stacked target
- `CapturePointer` 会把 pointer capture 收口回 `UiSurface::focus.captured`，`ReleasePointerCapture` 只会清掉当前 captor 自己持有的 capture
...
2026-05-06 的 widget behavior closure 把这条 pointer path 补成 Slate-style 共享事件闭环，但不复制 Unreal API。`UiPointerDispatchEffect` 现在表达一次性 reply effect：capture/release capture、set/clear focus、dirty flags 和 damage frame request 都先写入 `UiPointerDispatchResult`，再由 `UiSurface::dispatch_pointer_event(...)` 消费一次。Durable 状态仍只存在于 surface focus/capture/navigation/retained tree；handler reply 不会变成 widget-owned state。

这条闭环的 runtime 语义固定为：`UiSurfaceFrame.arranged_tree + hit_grid` 是 pointer route 的空间事实；capture 优先于 hit query；same-target hover 是 idle diagnostic 而不是重复 envelope；primary release 只有在 pressed target 和物理 release-inside target 一致时才产生 default click；unhandled scroll 会从命中链里的 scrollable candidate 开始尝试，只有实际改变 offset 才标记 handled/defaulted，最近候选被 clamp 时会继续尝试祖先候选。focus/capture reply effects 会更新 shared router state，并在有匹配 binding 时追加 typed focus/blur、hover、press、release、click component envelopes。没有 matching binding 的节点仍可更新 focus/capture state，但不会给 host 编造 component event。

Descriptor-driven interaction inference also moved out of editor-host coordinate branches. Template-built nodes now derive input participation from binding event capabilities instead of a single broad boolean: click-like bindings opt into clickable/hoverable/focusable behavior, hover and drag/drop bindings opt into hover participation, focus/blur bindings opt into focusability, scroll bindings only opt into receiving input unless authored metadata says more, and resize bindings remain non-input layout metadata unless the authored control also declares input metadata. Explicit `.ui.toml` bool metadata (`input_interactive`, `input_focusable`, `input_clickable`, `input_hoverable`) stays authoritative over inferred flags, and explicit `false` metadata is authoritative over the temporary authored-asset fallback for `Button` / `IconButton` / `TextField`; new components should use binding events or explicit input metadata instead of relying on component names. This keeps custom Material/runtime controls on the shared router path without adding host-side `control_id` or component-name dispatch tables.
...
- 如果显式 handler 没有消费 scroll 事件，surface 会从最近的 scrollable 节点开始尝试滚动；offset 没变时不会把事件标记为 handled，并会继续尝试后续祖先 scrollable candidate
...
### 8. Lifecycle, Property Mutation, And Reflector Snapshots
...
- `UiReflectorNode`
- `UiReflectorSnapshot`
- `WorkbenchWidgetReflectorModel`

2026-05-06 的 lifecycle/reflection slice 把 Widget Reflector 风格的检查面固定成 shared surface 的派生读模型，而不是第二棵 UI tree：

- `zircon_runtime_interface::ui::event_ui::reflection` owns the serializable lifecycle, reflected-property, hit-context, and reflector snapshot DTOs.
- `UiSurface::mutate_property(...)` mutates the retained `UiTree` through a narrow runtime-safe property seam and returns dirty/invalidation evidence instead of directly rebuilding or persisting authored source.
- `UiSurface::reflector_snapshot(...)` derives node lifecycle, properties, actions, focus/hover/capture state, frames, and optional hit context from the current runtime surface.
- `WorkbenchWidgetReflectorModel` consumes that snapshot read-only, keeping only local selected-node presentation state.

The authority split is deliberate: `.ui.toml` remains authored truth, `UiSurfaceFrame` and retained `UiSurface` remain runtime frame/state truth, and reflector data remains a debug/editor projection. The first mutation seam covers visibility, input policy, state flags, and template metadata attributes; descriptor validation, undo/redo, and authored source persistence stay outside this layer.
...
- `zircon_editor::ui::retained_host::callback_dispatch::SharedViewportPointerBridge` 持有一个最小 `UiSurface`
...
- `zircon_editor::ui::retained_host::app` 的 viewport pointer/scroll callback 现在先生成 shared `UiPointerEvent`
...
- `zircon_editor::ui::retained_host::menu_pointer::HostMenuPointerBridge` 持有一棵独立的 menu `UiSurface`
...
- `Scroll` 事件现在先更新 shared scroll state，再把结果回灌到 retained host `window_menu_scroll_px`
...
- [`build_host_menu_pointer_layout(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/menu_pointer/build_host_menu_pointer_layout.rs) 现在只从 `BuiltinWorkbenchRootShellFrames::{shell_frame, menu_bar_frame}` 派生六个顶层 menu button frame；[`app/helpers.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/helpers.rs) 已不再保留 `legacy_menu_button_frames(...)` getter bridge
- [`app/pointer_layout.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/pointer_layout.rs) 现在会把同一组 projection-backed button frame 回灌给 Rust-owned menu chrome host data，因此 popup presentation anchor 和 shared hit-test 终于共用同一份 root authority，而不是“命中来自 shared surface，但 popup 位置仍由 `top_bar.*_local_frame` 决定”的分裂状态
- `zircon_editor::ui::retained_host::callback_dispatch::dispatch_shared_menu_pointer_click(...)` 把：
  host 坐标
  -> menu `UiSurface` 命中
  -> `UiPointerDispatcher`
...
- 再由 [`callback_dispatch/mod.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/mod.rs) 解释成 builtin template binding 或 typed fallback
...
同时，host 并没有把文本测量或视觉布局重新搬回 shared core；对动态宽度 strip，它只把 retained host 当前实例几何当作结构输入上传：

- tab 的 strip-local `x`
- tab 的当前 `width`
- 局部点击点坐标

真正的业务 target 解析仍然发生在 shared surface 侧，而不是由 host leaf layer 自己决定：
...
- [`callback_wiring.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/callback_wiring.rs) 把 pointer fact 接回宿主
- [`shell_pointer.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/shell_pointer.rs) 先通过 shared drag surface 解析 `WorkbenchShellPointerRoute::FloatingWindow(..)` / `FloatingWindowEdge { .. }`
- [`callback_dispatch/layout/floating_window/dispatch.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/layout/floating_window/dispatch.rs) 再把 route 收口成 `LayoutCommand::FocusView`
...
- [`BuiltinWorkbenchTemplateBridge::control_frame(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/template_bridge/workbench/bridge.rs) 现在暴露 builtin workbench control 的 shared frame 查询
...
- [`zircon_editor::ui::retained_host::detail_pointer::ScrollSurfacePointerBridge`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/detail_pointer/mod.rs) 会建立一棵最小 retained surface：
  - root 节点
...
- bridge 再把最新 `scroll_offset` 回灌给 retained host `scroll_px`
- [`scroll_surface_host.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/scroll_surface_host.rs) 现在把 scroll-only 宿主状态封成 `ScrollSurfaceHostState`，避免 `app.rs` 为每个 pane 重复持有 `bridge + state + size`
...
现在这三块 pane 都已经不再保留 host-local `ScrollView.viewport-y` 作为第二份滚动真源：
...
- retained host 只消费 `scroll_px` 做平移，不再单独维护 viewport state

## Responsive WrapBox And Global Surface Metadata

Global UI Material Milestone 3 adds the first shared responsive flow primitive to the runtime layout core instead of giving Asset Browser or any other screen a private coordinate table. `UiWrapBoxConfig` now lives beside `UiScrollableBoxConfig` in `zircon_runtime_interface::ui::layout`, and template authoring can opt into it with `layout.container.kind = "WrapBox"`, `horizontal_gap`, `vertical_gap`, and `item_min_width`.

The runtime measure/arrange path treats WrapBox as a real container:

- measure walks children in parent slot order, adds slot padding to each item, applies `item_min_width`, and uses bounded/preferred width when deciding where rows wrap
- arrange lays children into rows using the actual available frame width, then records the wrapped content size back into `layout_cache.content_size`
- content-driven non-stretch roots keep their actual wrapped row width instead of preserving a temporary wide measurement frame

The same Milestone 3 gate also records collection/overflow intent in authored `.ui.toml` assets. `console.ui.toml`, `welcome.ui.toml`, `console_body.ui.toml`, `module_plugins_body.ui.toml`, and `runtime_diagnostics_body.ui.toml` now declare vertical scroll or stretched viewport metadata where the surface owns list/log/diagnostic overflow. The native host still consumes arranged frames from the shared runtime path; the metadata is not a painter-side clipping workaround.

Current evidence for this slice is scoped rather than workspace-wide. The M3 formatting gate passed for the touched runtime/editor files, the earlier `D:\cargo-targets\global-ui-m3-validation` run kept the `material_layout` and `shared_core` lower-layer coverage, and the final `E:\zircon-build\targets\global-ui` run accepted the post-portability `global_material_surface_assets` harness with `2 passed; 0 failed; 1071 filtered out`. Runtime-interface, runtime, and editor library checks also passed on that same final target with existing warning noise. The acceptance file records the earlier explicit-marker Cargo interruption and the later `cargo clean --target-dir "E:\zircon-build\targets\global-ui"` cleanup after `E:` dropped below the 50 GB validation threshold; this document therefore claims Milestone 3 scoped acceptance only, not workspace-wide green.

同一轮里，[`callback_dispatch/viewport/snap_cycle.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/viewport/snap_cycle.rs) 的 helper 可见性也已经补到 `viewport` 子树内，使 [`route_mapping.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/viewport/route_mapping.rs) 可以稳定复用同一套 shared viewport toolbar route 映射，而不会因为模块拆分再次卡住验证链路。
...
因此 editor shell 里的 transient pane surface 现在也不再保留一条 host-local 菜单 ABI。即使 payload 最终仍然属于 `MenuAction` 家族，真正的 action normalization 也重新回到了 template/runtime authority，而不是散落在 retained 壳层。
...
- [`zircon_editor::ui::retained_host::asset_pointer`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/asset_pointer.rs) 现在同时承载三类 shared list bridge：
...
- `known_project_asset == false` 的 reference row 不再注册成可点击 target，因此 hover/click 语义会自然回退到 list surface，而不是由 retained host leaf control 单独决定
- [`callback_dispatch/mod.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/mod.rs) 新增 `dispatch_shared_asset_reference_pointer_click(...)`，把：
  坐标
...
宿主侧 [`app.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app.rs) 现在也不再只维护 asset `tree/content` 两块 shared pointer state，而是为 `activity` 与 `browser` 两套 surface 都追加：
...
- [`app.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app.rs) 也移除了对应 `ui.on_*` 注册和 helper，shared `UiPointerDispatcher` 成为这些 list surface 唯一允许的交互 authority
- 这让 list surface 和 menu popup、scroll-only pane 一样，都只剩“上传 pointer/scroll 事实 + 消费 host 投影状态”的宿主边界

这层约束由 [`surface_contract.rs`](/E:/Git/ZirconEngine/zircon_editor/src/tests/host/retained_list_pointer/surface_contract.rs) 明确锁定，防止后续 cutover 又把旧 callback ABI 从 retained 壳层接回来。
...
- [`app/callback_wiring.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/callback_wiring.rs) 只负责把这两个 generic callback 交给宿主
- [`app/assets.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/assets.rs) 再把稳定 `control_id` 翻译成 builtin `AssetSurface/*` template 所需的 `UiEventKind + arguments`
...
- `zircon_editor::ui::retained_host::shell_pointer::WorkbenchShellPointerBridge` 持有一棵统一的 `UiSurface`
...
- resize dispatcher 在 `Down` 时对 splitter target 触发 shared `CapturePointer`，后续 `Move / Up` 即使移出 splitter hit bounds 也继续回到同一个 target

这保留了你原文档里“重叠区域按更合理边界归属”和“捕获后继续由同一对象处理”的两层语义，但判断与 capture 都已经收口到 shared route/dispatcher，而不是 host 本地字符串条件链。
...
- Deleted Slint shell copies 不再拥有 `StretchMode`/`frame`/solver 的真源地位
- Deleted Slint shell copies 也不再拥有 editor shell drag target hit-test / dock target route 的真源地位
...
- `cargo test -p zircon_ui shared_core -- --nocapture`
- `cargo test -p zircon_editor --lib retained_menu_pointer --locked -- --nocapture`
...
- `WrapBox` 的 available-width 换行、item minimum width、row gap、slot padding/order 和 bounded content-size 测量语义
...
- 未显式处理的 scroll 事件会从最近的 shared scrollable container 开始尝试；最近 candidate clamp/no-op 时继续交给祖先 scrollable
...
- [`document_routes.rs`](/E:/Git/ZirconEngine/zircon_editor/src/tests/host/retained_tab_drag/document_routes.rs) 和 [`floating_routes.rs`](/E:/Git/ZirconEngine/zircon_editor/src/tests/host/retained_tab_drag/floating_routes.rs) 已覆盖“shared pointer route vs fallback group key”在 document edge split 和 floating-window attach 两种情况下产出等价 route snapshot
- floating-window focus parity 也已经进入 shared harness：
  ...
  - `retained_callback_dispatch/layout.rs` 现在直接比较 `dispatch_builtin_floating_window_focus(...)` 与 `LayoutCommand::FocusView` 的 journal delta，防止 shared route cutover 造成 focus event/effect 漂移
...
- `cargo test -p zircon_editor --lib retained_asset_pointer --offline -- --nocapture`
- `cargo test -p zircon_editor --lib retained_callback_dispatch --offline -- --nocapture`
- `cargo check -p zircon_editor --lib --offline`
- `cargo test -p zircon_editor --lib shared_drag_capture_surface_replaces_legacy_direct_drop_callback_abi --locked -- --nocapture`
- `cargo test -p zircon_editor --lib retained_tab_drag --locked -- --nocapture`
- `cargo test -p zircon_editor --lib shared_resize_surface_replaces_legacy_direct_resize_callback_abi --locked -- --nocapture`
- `cargo test -p zircon_editor --lib retained_drawer_resize --locked -- --nocapture`
- `cargo test -p zircon_editor --test integration_contracts --features integration-contracts --locked workbench_retained_shell -- --nocapture`
- `cargo check -p zircon_editor --lib --locked`
- direct run of the already-built `target/debug/deps/zircon_editor-*.exe` test binary for:
  - `tests::host::retained_callback_dispatch::layout::builtin_floating_window_focus_matches_legacy_layout_focus_dispatch_event_log`
  - `tests::host::retained_tab_drag::resolved_workbench_tab_drop_route_snapshot_matches_shared_pointer_and_group_key_for_document_edge`
  - `tests::host::retained_tab_drag::resolved_workbench_tab_drop_route_snapshot_matches_shared_pointer_and_group_key_for_floating_window`
...
- [`FloatingWindowProjectionBundle`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/floating_window_projection.rs) 现在在 host 重算期一次性缓存每个 floating window 的 `outer/tab/content` frame、解析后的 `host_frame`，以及 `native_host_present` 标记
- [`app/host_lifecycle.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/host_lifecycle.rs) 会在 `recompute_if_dirty()` 期间调用 `build_floating_window_projection_bundle(...)`，并把结果同时喂给 document-tab pointer、shell drag surface、pane callback-size fallback、floating window presentation 和 native presenter target collection
