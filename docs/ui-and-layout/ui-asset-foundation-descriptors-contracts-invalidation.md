...
tests:
  ...
  - rustfmt --edition 2021 --check zircon_runtime/src/ui/component/descriptor/default_node_template.rs zircon_runtime/src/ui/template/asset/schema/flat_nodes.rs zircon_editor/src/ui/asset_editor/tree/tree_editing.rs zircon_editor/src/ui/asset_editor/palette/instantiate.rs zircon_editor/src/tests/ui/ui_asset_editor/palette_descriptor_registry.rs zircon_editor/src/tests/support.rs zircon_editor/src/ui/asset_editor/document_diff.rs
...
The runtime binding-expression schema does not replace M21 action side-effect policy, execute arbitrary scripts, or move editor preview helper functions such as `concat`, `get`, `at`, or node-path mock expressions into runtime semantics. Those remain editor preview behavior until a later productization milestone defines a broader runtime evaluator.
...
M12 consumes the collector through `resource_dependencies_fingerprint(...)`, `UiCompileCacheKey.resource_dependencies_revision`, and `UiInvalidationStage::ResourceDependency`. M16 back-half consumes the compiled document dependencies as deterministic `resource_dependencies` rows in `UiCompiledAssetDependencyManifest`. The 2026-05-07 editor slice now stores compiled `resource_dependencies()` / `resource_diagnostics()` on `UiAssetEditorSession`, refreshes them after valid source edits, clears them on resource compile failure, and exposes read-only pane rows. The resolver closure adds runtime-owned `UiResourcePathResolver` file-existence diagnostics for configured `res://`, `asset://`, and `project://` roots, including placeholder fallback checks. Resource browser UX, watcher/hot reload flow, runtime loader backends, and graphics/RHI consumption remain future productization.

M15/M16 resource reconciliation intentionally does not add a resource browser, watcher/hot reload flow, image/font/media loader backend, or graphics/RHI changes. Those remain future productization milestones that must reuse the runtime typed collector, resolver diagnostics, and dependency manifest instead of creating parallel resource classification rules.

## M21 Action Policy And M14 Localization Surface

M21 adds `zircon_runtime::ui::template::asset::action_policy` as the runtime-owned package policy behavior surface for authored UI actions, while policy/report declarations are canonical interface DTOs after the runtime-interface split. It consumes existing `UiBindingRef` route/action data and classifies side effects as local UI, editor mutation, asset IO, scene mutation, external process, or network. Runtime package validation uses `UiActionHostPolicy::runtime_default()` and therefore only allows local UI actions; editor package validation uses `UiActionHostPolicy::editor_authoring()` and also allows editor mutation plus asset IO. The resulting `UiActionPolicyReport` is stored on `UiCompiledAssetPackageValidationReport`; the UI Asset Editor runtime-report projection now shows editor action-policy diagnostics, runtime-default/editor-authoring host enforcement rows, and unsafe action authoring guidance without becoming the side-effect classifier.

M14 adds `zircon_runtime::ui::template::asset::localization` as the runtime-owned collector behavior for localized text metadata, while localized text/report declarations are canonical interface DTOs after the runtime-interface split. `UiLocalizedTextRef` models authored localization tables with `text_key`, optional table/fallback data, and `UiTextDirection`; compiler preconditions validate empty localization keys before expansion. `collect_document_localization_report(...)` scans node props/layout/params and stylesheet declarations, emits deterministic `UiLocalizationDependency` rows for localized refs, and emits `UiLocalizationTextCandidate` rows for literal `text`/`label`/`title` values that should be considered for extraction. Component prop validation accepts a non-empty localized text table as a string-family prop without flattening it to fallback text, so compiled attributes and package manifests keep the structured localization metadata. Package validation stores the full `UiLocalizationReport`, and `UiCompiledAssetDependencyManifest.localization_dependencies` persists locale dependency rows beside widget/style/resource dependencies. The resolver closure adds `UiLocalizationTableCatalog`, TOML key flattening, missing-table diagnostics, and missing-key diagnostics for selected locales; editor `resolver_state.rs` registers externally loaded table keys and projects those diagnostics without parsing localized refs itself. Resource browser, runtime loader backend, graphics/RHI resource consumption, and a dedicated interactive action-policy inspector remain future productization.
...
M16 back-half adds a deterministic framed TOML envelope and a TOML package manifest writer/importer without changing compiler authority. Runtime74 now uses magic `ZRUIA018`, TOML envelope schema version 3, and compiler schema version 5 because the compiled `UiTemplateInstance` includes a generation-qualified `UiCompiledBindingProgram` with typed action-payload values; artifacts without typed binding/property/control/route/action IDs, payload IR, and endpoint slots are rejected as stale. The envelope uses a little-endian schema version, little-endian payload length, and UTF-8 TOML payload, avoiding `bincode` assumptions for compiled values containing `toml::Value`. Runtime-interface continues to reserve `UiCompiledAssetArtifact` for the neutral `{ report, bytes }` DTO and owns `UiCompiledAssetCacheRecord` and `UiCompiledAssetPackageManifest`. The cache record persists the cache key, invalidation snapshot, artifact fingerprint, and byte length; the package manifest records the header, dependency manifest, cache record, and artifact entry. This schema does not claim binary sections that the compiler does not yet emit.
