...
tests:
  ...
  - cargo test -p zircon_editor --lib startup_session_defaults_to_component_showcase_without_recent_project --locked --target-dir target/codex-shared-b (2026-05-15: passed, 1 passed)
  ...
  - python tomllib parse of zircon_editor/assets/ui/editor/components/showcase/showcase_visual_section.zui (2026-06-15 Dialog/ConfirmDialog showcase visibility baseline: passed)
  - rustfmt --edition 2021 --check zircon_editor/src/ui/template_runtime/showcase_demo_state/categories.rs zircon_editor/src/ui/template_runtime/showcase_demo_state/defaults.rs zircon_editor/src/ui/retained_host/ui/tests/component_showcase.rs zircon_editor/src/tests/host/template_runtime/component_showcase_category.rs (2026-06-15 Dialog/ConfirmDialog showcase visibility baseline: passed)
  - git diff --check -- zircon_editor/assets/ui/editor/components/showcase/showcase_visual_section.zui zircon_editor/src/ui/template_runtime/showcase_demo_state/categories.rs zircon_editor/src/ui/template_runtime/showcase_demo_state/defaults.rs zircon_editor/src/ui/retained_host/ui/tests/component_showcase.rs zircon_editor/src/tests/host/template_runtime/component_showcase_category.rs (2026-06-15 Dialog/ConfirmDialog showcase visibility baseline: passed with LF-to-CRLF warnings only)
  - conflict marker scan for Dialog/ConfirmDialog showcase visibility files (2026-06-15: passed with no matches)
...
- top command toolbar imported from `.zui`;
- left category navigation imported from `.zui`, preserving stable category control ids and `UiComponentShowcase.SelectCategory.*` routes;
- center scroll area with the Visual/Feedback, Input/Numeric, Selection/References, and Collections/Inspector groups imported from `.zui`;
- right state panel imported from `.zui`, with `PropertyRow` nodes for selected category, last control/action, current value, validation, drag/drop payload summary, and retained event log.
- bottom output log imported from `.zui`.

The center gallery's `ComponentShowcaseScroll` node is a real shared `ScrollableBox` container, not a `VerticalBox` with an editor-only `scroll` hint. That keeps the authoring asset on the same shared layout path as runtime scroll panes: `UiSurface.arranged_tree` computes the inherited scroll viewport clip, host projection copies that clip into `TemplatePaneNodeData`, and the native painter clips off-screen showcase rows before they can overlap the bottom log.
...
Asset Browser content-row pointer events now arm a real `UiDragPayload` for asset items. The payload keeps `kind = Asset` and writes the asset locator as the reference value, while optional `UiDragSourceMetadata` carries source surface, source control id, asset UUID, locator, display name, asset kind, and extension. Accepted `AssetField` drops retain that metadata in `UiComponentState` and project a generic `drop_source_summary` string so the Rust-owned host projection can show the source without hardcoding asset-browser behavior. Direct retained value replacement through the interface DTO helper clears that per-property source metadata, matching the runtime reducer's non-drop value replacement path and preventing stale asset-browser provenance from surviving manual overrides.
...
The follow-up module-slimming pass first moved real drag-source tests out of the oversized retained-host app test root and into `zircon_editor/src/ui/retained_host/app/tests/drag_sources.rs`. The extracted module owns Asset Browser payload metadata tests, AssetField real/drop fallback tests, Hierarchy scene-instance payload tests, and ObjectField scene-instance payload consumption. The 2026-06-22 Editor UI 10 pass then completed the root split: `app/tests/mod.rs` is now only the local module entry, `app/tests/support.rs` owns the shared host harness, and pointer-routing regressions live in named files such as `command_palette.rs`, `child_window_focus.rs`, `menu_pointer.rs`, `projection_geometry.rs`, `viewport_template_bridge.rs`, `root_pointer_fallbacks.rs`, and `child_window_tabs.rs`. Formatting and structure audit pass; retained app lib-test compilation completes after the support visibility fix, while the non-exact focused execution rerun timed out after 184 seconds without diagnostics and left no running cargo/rustc process after cleanup.
...
The follow-up test-structure cleanup moved the contiguous `component_showcase_*` retained host assertions into `zircon_editor/src/ui/retained_host/ui/tests/component_showcase.rs`. A later Editor UI 10 M2 cleanup deleted the old retained-host UI root test file entirely: `tests/mod.rs` now wires focused owners, `tests/support.rs` owns the shared host fixture/projection harness, and `component_showcase.rs` keeps the showcase-specific template/projection checks. Formatting and structure-audit checks passed for the final owner-tree split; focused Cargo execution still needs a quiet validation window because the retained-ui exact-filter run timed out without diagnostics.
...
This slice introduces TOML-authored Material-style meta components based on the structure used by `dev/slint/internal/compiler/widgets/material`: a state layer component plus base button, icon button, toggle, checkbox row, outlined field, and slider field wrappers. The implementation stays native to Zircon: the meta components expand to existing Runtime UI controls, carry semantic style attributes such as `surface_variant`, `button_variant`, `text_tone`, `font_size`, `corner_radius`, and `border_width`, and keep the layout frame as the only hit/render authority.
...
Validation evidence on `E:\zircon-build\targets`:
...
- `cargo test -p zircon_editor --lib showcase_demo_state --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed.
- `cargo test -p zircon_editor --lib native_host_pointer_click_routes_projected_material_showcase_button --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed.
- `cargo test -p zircon_editor --lib native_host_repeated_hierarchy_hover_moves_do_not_rebuild_presentation --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed.
- `cargo test -p zircon_editor --lib native_host --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed.
...
- `cargo test -p zircon_editor --lib native_host_pointer_click_routes_projected_material_showcase_button --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed.
- `cargo test -p zircon_editor --lib native_host --locked --jobs 1 --target-dir E:\zircon-build\targets --message-format short --color never`: passed with 19 tests.
...
## 2026-06-15 Dialog showcase visibility
...
The Component Showcase feedback group now includes open `DialogDemo` and `ConfirmDialogDemo` nodes in the visual/feedback section. Both demos use real runtime component ids through `showcase_demo_state::defaults`, stay visible in the `Feedback` and `All` category filters, and project through the retained host with stable `dialog` and `confirm-dialog` component roles. The authored demo props intentionally include both `title` and `text`: `title` feeds runtime dialog rendering, while `text` keeps existing host projection tests able to identify the card without adding a new host DTO field.

This was the showcase visibility baseline for M3.S3. Dialog and ConfirmDialog now also have a retained native painter baseline; focused editor Cargo execution is still pending for both the showcase projection tests and native painter pixel tests.

Validation evidence:
- `python -c "import tomllib, pathlib; tomllib.loads(pathlib.Path(r'zircon_editor/assets/ui/editor/components/showcase/showcase_visual_section.zui').read_text(encoding='utf-8'))"`: passed.
- `rustfmt --edition 2021 --check zircon_editor/src/ui/template_runtime/showcase_demo_state/categories.rs zircon_editor/src/ui/template_runtime/showcase_demo_state/defaults.rs zircon_editor/src/ui/retained_host/ui/tests/component_showcase.rs zircon_editor/src/tests/host/template_runtime/component_showcase_category.rs`: passed.
- `git diff --check -- zircon_editor/assets/ui/editor/components/showcase/showcase_visual_section.zui zircon_editor/src/ui/template_runtime/showcase_demo_state/categories.rs zircon_editor/src/ui/template_runtime/showcase_demo_state/defaults.rs zircon_editor/src/ui/retained_host/ui/tests/component_showcase.rs zircon_editor/src/tests/host/template_runtime/component_showcase_category.rs`: passed with only LF-to-CRLF warnings.
- Conflict marker scan over the same files: passed with no matches.
- Focused `component_showcase_pane_projects_runtime_component_nodes_for_template_pane` and `showcase_category_selection_filters_projected_demo_controls` Cargo execution was deferred because unrelated cargo/rustc lanes were active under `D:\cargo-targets\zircon-runtime-temporal-s4d-0614` and `D:\cargo-targets\zircon-export-m6-editor-dispatch-0614`.
...
The retained host now paints open Dialog and ConfirmDialog roots through a dedicated native route instead of the generic surface/text fallback. The route consumes closed dialog roots without pixels, draws runtime-aligned dialog panel/title/body/action geometry, and renders ConfirmDialog severity state with a left mark, error border/title, and disabled confirm action color.

Projection now feeds the painter directly: `title` maps to host `text`, `message` maps to `value_text`, Dialog actions populate the existing `actions` list, and ConfirmDialog emits cancel/confirm action rows plus severity/destructive/confirmDisabled owner-state tokens. The component showcase assertions now cover those projected body/action/state fields for `DialogDemo` and `ConfirmDialogDemo`.

Validation evidence:
- `rustfmt --edition 2021` over touched Dialog native painter/projection/showcase test Rust files: passed.
- `git diff --check -- ...` over touched Dialog native painter/projection/docs/session files: passed with only LF-to-CRLF warnings.
- Conflict marker scan over touched Dialog native painter/projection files: passed with no matches.
- Focused editor Cargo execution for `native_material_painter_dialog`, pane projection, and component showcase tests is deferred while shared cargo/rustc lanes are active.
...
The Component Showcase now has a direct L1 state-matrix contract for the planned atom set. The input section adds `TabDemo`, `TabStripDemo` backed by the existing runtime `Tabs` descriptor, `SliderDemo`, and `RangeSliderDemo`; the visual section adds `SkeletonDemo`. Existing button variants already carry default, focused, hovered, pressed, danger, and disabled states, while the new slider and tab rows add percent, focused thumb, selected, focused, hovered option, and disabled option metadata.

`showcase_demo_state` maps the new controls into Input, Numeric, Feedback, and All categories with real runtime component ids and default state values. The all-demo guard now also requires every `*Demo` control id in the visual, input, selection, and collections sections to have both a category mapping and a default component id mapping, the default component id must match the `.zui` authored `component`, and the category must fit the authored section family. The reverse direction is guarded too: every demo id left in `demo_category_for_control` or `component_id_for_control` must still be authored by one of the showcase section assets, so deleted rows cannot leave stale Rust mappings that keep tests or filters green for controls no longer displayed. That pulls the static visual/feedback atoms, button state variants, and TableRow/VirtualList/PagedList/WorldSpaceSurface showcase rows into the same filtering contract instead of letting unmapped demo rows leak across categories, silently point at the wrong runtime component, land in the wrong gallery section, or remain as dead mapping arms after the asset is removed. `view_projection.rs` now also exposes stable retained-host component roles for `RangeSlider`, `SegmentedControl`, `Tab`, `Tabs`, and `Separator`, so the showcase projection does not depend on blank fallback roles for these L1 entries.

The retained-host showcase projection test now asserts these new L1 nodes after template expansion. `SliderDemo` and `RangeSliderDemo` carry numeric values, percent mirrors, drag/value action ids, and focused/pressed state; `TabDemo` and `TabStripDemo` carry selected/focused/hovered/disabled option rows and value-change action ids; `SkeletonDemo` carries the feedback category plus `rounded` variant. The metadata check also requires the new L1 action ids (`slider_drag_update`, `slider_changed`, `range_slider_drag_update`, `range_slider_changed`, `tab_changed`, and `tab_strip_changed`) so event wiring cannot silently lose the generated showcase route names. The category-filter test also asserts that the new L1 controls plus the newly mapped collection rows appear only in their intended gallery categories before All.

The authored event guard now closes the remaining static gap between `.zui` events and Rust-owned showcase plumbing. `showcase_template_bindings.rs` owns every `UiComponentShowcase/*` binding authored by the category nav and demo sections, including the Tab/TabStrip/Slider/RangeSlider events added by the L1 matrix. `showcase_event_inputs.rs` maps those action ids to `SelectOption`, `ValueChanged`, and `DragDelta` demo inputs, while `showcase_actions.rs` prefers the same binding ids when projecting retained-host action, drag, and edit fields. The same guard now derives typed transient input requirements from Rust binding action kinds: `ListRowHovered`, `ListRowPressed`, `AssetFieldDropHovered`, and `AssetFieldActiveDragTarget` must have deterministic `Hover`, `Press`, `DropHover`, and `ActiveDragTarget` demo inputs instead of silently falling back to `None`. Collection value-change bindings are covered too: `ArrayFieldChanged` and `MapFieldChanged` now have deterministic `UiValue::Array` and `UiValue::Map` demo inputs, and the script fails if those authored value-change bindings lose the `Value` path. The delta path is also explicit: every authored `DragDelta` and `LargeDragDelta` binding must have a deterministic delta input, including `RangeFieldDragUpdate`, so generic action dispatch and pointer-drag dispatch stay aligned. The required-input guard now covers the broader action-kind surface that cannot be resolved through `None`: `ValueChanged`, `Change`, `SelectOption`, `DropReference`, collection row mutations, visible-range/page requests, and world-surface updates all need an explicit demo input branch in `showcase_event_inputs.rs`. The guard accepts intentionally broad matcher branches such as `input_field` and `text_field` when they cover both changed and committed suffixes, but still fails if a required action kind has no matching input window. The retained-host projection suffixes are also part of the matrix: every preferred action, drag, edit, pointer-drag, and action-chip suffix parsed from `showcase_actions.rs` must resolve to a real `UiComponentShowcase/<suffix>` binding. Authored routes now have the same semantic owner: every `.zui` route under `component_lab.showcase.*` must match the Rust binding demo action, so `ValueChanged.RangeField` stays paired with `component_lab.showcase.value_changed.range_field` and category routes stay paired with their `SelectCategory.*` actions. The category navigation itself is now part of the same contract: every `CATEGORY_*` constant in `categories.rs` must be reachable through `nav_category_for_control`, every mapped `Show*Category` control must exist in `showcase_category_nav.zui`, and every category nav control must have a matching `SelectCategory.<Name>` binding. The category-filter test now has the same coverage obligation: every `SelectCategory.*` binding must be directly exercised by `component_showcase_category.rs`; the test now groups all 60 authored demo controls by gallery category and uses `assert_demo_controls_for_category(...)` plus `assert_all_demo_controls_visible(...)` so each category switch proves the full visible set and the full hidden set, not only a few representative controls. The script also parses those grouped constants, rejects stale or duplicated demo ids, requires every group to match `demo_category_for_control`, requires the aggregate `SHOWCASE_DEMO_CONTROL_GROUPS` helper list to include every expected group exactly once, and requires each `SelectCategory.*` block to call the matching group helper, so the category-filter test cannot drift away from runtime category mappings or silently skip a group in the All category. The category-filter blocks also assert full nav exclusivity through `assert_selected_category(...)`: after each `SelectCategory.*` binding, the selected `Show*Category` control must project `selected = true`, and every other nav control must project `selected = false`. Runtime action support is checked too: every Rust binding demo action kind must be present in `showcase_demo_state/events.rs` via `SelectCategory.*` or `component_event_for_action`, so adding a new binding kind requires adding the state-machine branch in the same design path. Retained-host projection tests are now included in the same loop: every concrete `ui_component_showcase.*` action id asserted in `component_showcase.rs` must resolve back to a Rust binding suffix, and every direct `UiComponentShowcase/<suffix>` binding id used by `component_showcase.rs` or `component_showcase_category.rs` must exist in the binding table, so test expectations cannot keep referencing removed or renamed binding ids. The input mapping itself is now guarded in the other direction: every `action_matches(...)` matcher and `action_matches_binding_suffix(...)` constant in `showcase_event_inputs.rs` must still match at least one authored Rust binding suffix, so deleted showcase events cannot leave stale demo-input branches behind. `verify-showcase-state-matrix.mjs` reads both Rust tests, the category nav asset, all four showcase section assets, and the Rust category/default-component/binding/input/action-projection/action-runtime match arms as part of the lightweight contract, so the asset, runtime descriptor taxonomy, gallery section taxonomy, category filtering, component identity, authored event wiring, typed demo inputs, action naming, runtime demo-state support, and native host DTO projection stay aligned before the live visual pass.

The focused Cargo follow-up also closed the remaining retained-host projection gaps that the lightweight contract could not prove by string parity. `ConfirmDialogDemo` now carries `validation_level = "error"` in the template asset and a real default `UiValidationState::Error`, because host projection derives `validation_level` from retained validation state rather than from a generic value field. `showcase_demo_state.rs` now projects CommandPalette and NotificationCenter default collection fields back into retained-host attributes, including commands, filtered commands, disabled commands, notifications, selected ids, focus index, visible limit, unread count, keyboard-navigation flags, and empty text. This makes the native `TemplatePaneNodeData` rows visible to the same component-showcase projection assertions that paint the runtime cards. The same pass synced the welcome startup demo route assertion, the showcase action-id marker list, the Skeleton variant token assertion, and the scroll clip-frame assertion with the current runtime output.
...
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe component_showcase_pane_projects_runtime_component_nodes_for_template_pane --nocapture --test-threads=1`: passed, 1 passed / 0 failed / 1994 filtered, covering retained-host projection for runtime component nodes such as CommandPalette and NotificationCenter.
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe showcase_category_selection_filters_projected_demo_controls --nocapture --test-threads=1`: passed, 1 passed / 0 failed / 1994 filtered, covering projected demo control category filtering after the overlay showcase additions.
- `cargo test -p zircon_editor --lib native_material_painter --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-component-showcase-0615 --message-format short --color never -- --test-threads=1 --nocapture`: passed, 49 passed / 0 failed / 1945 filtered, after the render lane cleared the earlier `TextureFormat::Rg11b10Ufloat` compile blocker and this pass fixed resolved button interaction-state projection plus brittle Dialog/DragOverlay pixel samples.
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe native_material_painter_dialog --nocapture --test-threads=1`: passed, 3 passed / 0 failed / 1992 filtered, covering open Dialog panel/text/action painting, ConfirmDialog error/disabled-confirm styling, and closed Dialog no-fallback consumption.
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe runtime_component_projection_applies_mui_overlay_surface_defaults --nocapture --test-threads=1`: passed, 1 passed / 0 failed / 1994 filtered, covering retained-host MUI overlay surface default projection.
- `cargo test -p zircon_editor --lib native_template_button --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-component-showcase-0615 --message-format short --color never -- --test-threads=1 --nocapture`: passed, 3 passed / 0 failed / 1991 filtered, covering the shared painter button priority path used by the native Material tests.
- `npm run check` in `docs/ui-and-layout/ai-workbench-style/component-prototype`: passed; interaction contract, action naming, control route audit, responsive validation, live resize, and no full-reference screenshot dependency checks all passed. The run reported `webActions=4162`, `nativeActions=2880`, `uniqueActions=4523`, 13 top-level modules, 44 extension modules, 3011 explicit panel-route controls, 6589 visible control route responses, 6 responsive viewports, and 6 live resize steps.
- `cargo test -p zircon_editor --lib component_lab_search_input_has_edit_and_commit_bindings --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-component-showcase-0615 --message-format short --color never -- --test-threads=1 --nocapture`: passed, 1 passed / 0 failed / 1994 filtered, after the Workbench builtin binding table was brought back in sync with the `WorkbenchSearchInput` Component Lab asset events.
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe componentized_workbench_surface_paints_native_preview_pixels_and_interaction_state --nocapture --test-threads=1`: passed, 1 passed / 0 failed / 1994 filtered. The normal Cargo foreground command timed out during Windows lib-test compilation, then the warmed binary proved the native Workbench preview path, including nonblank pixels and repaint after Move-tool interaction.
- Native preview artifact: `target/editor-workbench-visual-check/m1s5-native-workbench-preview-1672x941.png`, 1672x941, visually checked as nonblank Workbench/Component Lab output.
- Direct execution of `E:\cargo-targets\zircon-editor-ui-component-showcase-0615\debug\deps\zircon_editor-0fea0c836fb2d960.exe capture_m3_gui_acceptance_visual_artifacts --ignored --nocapture --test-threads=1`: passed, 1 passed / 0 failed / 1994 filtered. The ignored software-rendered artifact test wrote 8 native editor screenshots under `target/visual-layout`: Workbench, Welcome input, Asset Browser, assets drawer, menu popup SVG icons, drag-after-release, and small/large SVG icon scale scenes.
- Native GUI screenshot artifact dimensions: six 900x620 PNGs, one 640x420 PNG, and one 1260x780 PNG; `editor-window-m3-workbench-900x620.png` was visually checked as nonblank native UI output.
- `cargo test -p zircon_editor --lib native_workbench --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-component-showcase-0615 --message-format short --color never -- --nocapture --test-threads=1`: passed, 24 passed / 0 failed / 1971 filtered. The first warmed-binary batch exposed stale native Workbench assertions that still expected display labels or old binding IDs; the final Cargo rerun verifies the current stable route/action IDs for popup menu rows, text input commit, module field edit/commit, dropdown rows, keyboard navigation, Escape/outside dismissal, and screenshot-preview paths.
- `cargo test -p zircon_editor --lib command_palette --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-component-showcase-0615 --message-format short --color never -- --nocapture --test-threads=1`: passed, 3 passed / 0 failed / 1992 filtered, covering retained CommandPalette command-row projection plus open/closed native painter consumption.
...
`showcase_demo_state/events.rs` now owns the authored binding action parser, `UiComponentShowcaseDemoEventInput`, runtime showcase errors, and `component_event_for_action(...)`. The root `showcase_demo_state.rs` stays focused on retained showcase state, default state lookup, event-log projection, and host-model attribute synchronization. This keeps the state machine branch that mirrors `showcase_template_bindings.rs` visible as an event owner instead of letting the root state/projection file remain a mixed declaration, projection, and action-routing module.

Validation evidence:
- `cargo fmt -p zircon_editor --check`: passed.
- `cargo check -p zircon_editor --lib --locked --message-format short --color never`: passed with existing warning noise.
- `python .codex/skills/zircon-project-skills/zr-runtime-interface-convergence/scripts/audit_editor_structure.py --json`: passed; `oversized_production_file_count = 11`, `migration_debt_count = 11`.
- `git diff --check -- zircon_editor/src/ui/template_runtime/showcase_demo_state.rs zircon_editor/src/ui/template_runtime/showcase_demo_state/events.rs`: passed with only LF-to-CRLF warnings.
