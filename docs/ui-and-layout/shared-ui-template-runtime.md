...
Runtime layout pass 现在已经开始消费这条 slot contract 的最小安全子集。`layout/pass/slot.rs` 在 pass 内部用 `(parent_id, child_id, container kind)` 查找 parent-owned `UiSlot`，避免把 lookup 先扩成 shared tree API；`measure.rs` 会把 slot padding 纳入 Free/Container/Overlay 和 Linear content desired size；`axis.rs` 会把 Linear 主轴 padding 纳入 axis solve；`child_frame.rs` 会把 slotted children 对齐到 padded parent content frame；`arrange.rs` 会按 slot order 排列 Linear/Overlay/Free/Container 的布局迭代。

这个 slice 仍有意不切换 hit/render/input/editor host 行为。缺失 slot 或 default-only slot 会继续回退到旧 anchor/pivot/position frame 计算；slot order 只约束 layout iteration，不声明新的 paint layer 或 hit priority；`UiSlot.linear_sizing`、`UiSlot.z_order` 和 `UiSlot.canvas_placement` 都只是 preserved contract，还没有进入 runtime Linear main-axis solve、Overlay paint/hit ordering 或 Canvas/Free arrange behavior。后续 Slate layout milestone 还需要补 slot mutation invalidation、slot-derived diagnostics、runtime 消费 `linear_sizing`、runtime 消费 Overlay `z_order` / layer grouping、runtime 消费 Canvas/Free placement、Grid/Flow/Splitter/Scale payload，以及 reflector 对 resolved slot policy 的可视化。
