...
related_code:
  ...
  - zircon_editor/src/tests/host/slint_window/native_runtime_text_painter.rs
...
2026-05-06 Slint Material export coverage extends that same reference-component contract from a first slice to the full material primitive set. The TOML authority now declares each exported `dev/slint/internal/compiler/widgets/material` primitive as a `Material*` component and maps it onto existing Runtime UI controls where possible: buttons and menu items to `Button`, line edits to `InputField`/`TextField`, selection widgets to `ComboBox`/`ToggleButton`/`Checkbox`, scroll/table widgets to `ScrollableBox`/`VirtualList`, and popup widgets to `ContextActionMenu`. This keeps the shared compiler and host projection path generic: adding a Material-flavored button or table row is now a TOML asset change, not a new Rust coordinate or widget shim. Static coverage is locked by `material_meta_components_cover_slint_material_exports`, and projection coverage is extended in `component_showcase_projection_carries_runtime_component_semantics`.

Focused validation for the shared-template side used `E:\zircon-build\targets` and passed `cargo fmt --package zircon_runtime --package zircon_editor -- --check`, `material_meta_components_cover_slint_material_exports`, `component_showcase_projection_carries_runtime_component_semantics`, `component_showcase_uses_material_meta_component_assets`, and `component_showcase_authored_props_are_declared_by_runtime_catalog`. These tests cover the shared reference expansion, instance binding retention, and editor host projection contract for the Material meta component set.
