...
2. `UiTemplateValidator` 约束模板节点、slot 和组件引用
3. `UiTemplateInstance` 展开 composite/template 调用和 slot 内容
4. `UiTemplateTreeBuilder` 把模板里的显式 layout 契约映射到 shared `UiTree` 节点
...
- `version`
- `components: BTreeMap<String, UiComponentTemplate>`
- `root: UiTemplateNode`
...
- `root: UiTemplateNode`
- `slots: BTreeMap<String, UiSlotTemplate>`
...
这里的 `bindings` 目前不是宿主 callback 名称，而是稳定的 `UiBindingRef`：
...
`UiTemplateInstance::from_document(...)` 当前会：
...
当前 builder 会把 `UiTemplateInstance` 转成 `UiTree`，并做两类 shared-core 推断：
