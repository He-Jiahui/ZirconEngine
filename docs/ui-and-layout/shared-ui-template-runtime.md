...
2026-05-07 Global UI Material M4 keeps Material state vocabulary in the `.ui.toml` template/runtime path instead of adding a host-only visual shim. `editor_material.ui.toml` now owns the full shared state palette used by editor and runtime Material surfaces: pressed, selected, disabled, accent-soft, disabled text, warning, error, and focus-ring tokens sit beside the existing surface/accent/border/text roles. `editor_base.ui.toml` delegates to that Material theme, so surfaces that still import the base theme inherit the same token family instead of maintaining a second editor visual language.

`material_meta_components.ui.toml` now emits stable `material-*` classes and state props on the expanded roots for Button, IconButton, ToggleButton, CheckboxRow, OutlinedField, ListItem, MenuItem, TabImpl, and TextEdit. The important runtime-template contract is that hover/press/focus/selected/checked/disabled state remains authored and projected as node metadata; `TemplatePaneNodeData` carries those flags to the native painter, while shared layout continues to own the arranged frame. Runtime HUD/dialog assets use the same class and Material prop vocabulary as editor controls, so HUD and pause/settings/inventory/quest dialogs no longer form a separate runtime-only theme dialect.

The M4 asset boundary tests live in `zircon_editor/src/tests/ui/boundary/global_material_surface_assets.rs`. They extend the existing 54-file global `.ui.toml` inventory with token coverage, stylesheet coverage for classes emitted by the Material meta component asset, state metadata coverage for stateful Material roots, and runtime HUD/dialog Material class/prop coverage. Focused validation on `E:\zircon-build\targets\global-ui` passed `global_material_surface_assets`, `component_showcase`, `runtime_dialog_and_hud_buttons_participate_in_material_measurement`, and `cargo check -p zircon_editor --lib` with existing warning noise; exact evidence is recorded in `tests/acceptance/global-ui-material-responsive-diagnostics.md`.
