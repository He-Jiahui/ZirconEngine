...
这意味着 style asset 和 inline override 在 template compiler 里完成归并以后，shared surface 已经能把这些视觉结果继续传给 preview/runtime consumer；文本测量/分行/对齐/裁剪、方向标记、rich runs、ellipsis、caret/selection/composition DTO 已经先以 `UiResolvedTextLayout` 进入 render extract，SDF 字形 cache key / slot run / whitespace advance / atlas rect / GPU quad path 也已经在 renderer-local `ScreenSpaceUiSdfAtlas` + `ScreenSpaceUiSdfRenderer` 边界收敛。后续还没做的部分是 HarfBuzz 级 shaping/fallback、真实 glyph reordering、平台 IME 接入、图片资源装载和更完整 GPU pass，而不是再回头重建另一套 visual payload 模型。
