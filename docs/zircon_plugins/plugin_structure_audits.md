...
## D9 typed runtime-mirror gate

The D9 editor/runtime mirror audit covers Animation, Physics, Net, and Navigation. Navigation adds the stricter typed-consumer checks: its editor declaration must register the consumer through the SDK, its runtime plugin must use `register_mirrored_event::<NavAgentTickReport>`, its stable schema constants must be present, and its tests must exercise `EditorRuntimeEventConsumerHost` delivery.

Manifest schema and export validation accept `modules.event_consumers` only on editor modules. Every row requires a namespaced consumer id, namespaced event id, non-empty payload schema, editor capability, and unique consumer id.
