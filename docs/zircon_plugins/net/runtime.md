...
tests:
  ...
  - rustfmt --edition 2021 --check zircon_runtime/src/core/framework/net/*.rs (planned for current framework contract slice)
...
- Copied `NetDiagnostics` and bounded event draining.
...
Runtime tests cover plugin module registration, package optional features, UDP loopback packets, TCP loopback accept/send/poll, diagnostics/events, listener close behavior, local HTTP route dispatch, dynamic HTTP route handlers, explicit HTTP backend absence, WebSocket loopback frame budgets, and explicit WebSocket backend absence. The base runtime test tree follows those same boundaries so adding protocol-specific assertions does not revive a monolithic `tests.rs`.

The framework-level tests in `zircon_runtime/src/core/framework/net/tests.rs` now cover DTO defaults and protocol layering without network IO. Focused Cargo validation for runtime and framework tests is pending until active workspace Cargo lanes quiet down; low-interference validation for this slice should run rustfmt and diff checks first.
