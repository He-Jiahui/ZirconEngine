...
`runtime.feature.ai.perception` is `Complete` at the M4 runtime boundary. The package, behavior-tree, and blackboard rows remain `Partial`; project/profile/export tooling therefore exposes the production perception runtime without pretending that the patrol/detect/chase product scenario, EQS-style querying, or editor authoring is complete.
...
M4 does not claim M3-T4 or M5. The patrol/detect/chase product scenario is the next dependency-unblocked slice, and graph/editor/debug overlay work remains owned by M5.
