...
The implementation deliberately does not claim M3 subsystem task handoffs, M4 perception scanning/aging, or M5 editor/debug tooling. Those features consume the compiled tree, dense Blackboard, observer-abort, injected catalog, manager, event, and Update-system seams established by M1–M2.

## Test Coverage

The M1–M2 suite covers versioned `.btree.toml` loading and full validation, preorder/direct-child consistency, category mismatch, duplicate ownership and cycle rejection, the exact 18-entry standard catalog snapshot, dense stable slots, duplicate catalog ids, stateful external Rust factory injection, a live `RuntimeExtensionRegistry` contribution-and-owner-revocation path, actual nonzero AI owner binding, in-flight execution barriers and linearized next-generation registration, all standard node families across Success/Failure/Running including RunSubtree, composite Running resume without terminal sibling replay, RandomSelector weight/stability, retained timer state, LOD scheduling, schema-to-slot layout, same-value generations, atomic store synchronization, typed observer binding failures, all four abort policies, subtree cleanup, root switch/disable cleanup, cooldown preservation, pending manager notifications, and Parallel terminal-sibling preservation. The Windows M2 acceptance run is `cargo test --manifest-path zircon_plugins/Cargo.toml -p zircon_plugin_ai_runtime --locked --offline --jobs 1 --target-dir E:\\cargo-targets\\zircon-ai-m2`: 58 passed, 0 failed; warnings shown by the run are pre-existing shared `zircon_runtime` warnings, with no AI crate warnings.
