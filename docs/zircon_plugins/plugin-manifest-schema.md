...
| `[[optional_features]]` | 可选功能 | `id`、`display_name`、`owner_plugin_id`、`capabilities`、`default_packaging`、`enabled_by_default`、可选 `provider_package_id` / `[[optional_features.dependencies]]` / `[optional_features.distribution]` / `[[optional_features.modules]]` |
...
`[[optional_features]]` 自身必须有非空 `id`、`display_name`、`owner_plugin_id`、`capabilities`、`default_packaging`，并且 `enabled_by_default` 必须是 bool；`default_packaging` 当前只允许 `source_template` / `library_embed` / `native_dynamic`。
`provider_package_id` 可省略；出现时必须是非空 trimmed string。省略时，feature-provider build/validate 会按 feature id 派生默认 provider package id。
`[[optional_features.dependencies]]` 存在时必须是 table array；每行必须声明非空 `plugin_id`、非空 `capability` 与 bool `primary`，用于明确 feature owner、主依赖和附加 capability 依赖。
`[optional_features.distribution]` 存在时必须是 table；`forms` 与 `default_packaging` 必须是非空字符串数组，取值分别封闭到 `embed` / `dist` 与 `source_template` / `library_embed` / `native_dynamic`；`abi_version` 必须是正整数；`engine_compat`、`dist_crate`、`descriptor_symbol` 必须是非空字符串；`runtime_entry` / `editor_entry` 至少声明一项，出现时必须是非空字符串；可选 `assets` 必须是非空字符串数组。
`[[optional_features.modules]]` 与根 `[[modules]]` 使用同一模块行 schema：`name`、`kind`、`crate_name`、`target_modes`、`capabilities` 必须齐全，`kind` 当前只允许 `runtime` / `editor` / `native` / `vm`，`target_modes` 当前只允许 `client_runtime` / `server_runtime` / `editor_host`。轻量 optional feature 可不强制拥有模块行。
