...
tests:
  ...
  - 2026-06-08 window-state-frame-validation: cargo test -p zircon_runtime_interface --lib ui_surface_frame_contract_carries_arranged_render_and_hit_state --locked --jobs 1 --target-dir D:\cargo-targets\zircon-ui-window-pump-0608 --message-format short --color never -- --nocapture --test-threads=1 (1 passed, 0 failed, 141 filtered out)
  - 2026-06-08 window-state-frame-validation: rustfmt --edition 2021 --check zircon_runtime_interface/src/ui/surface/frame.rs zircon_runtime_interface/src/tests/contracts.rs (passed)
  - 2026-06-13 SearchField widget-inference validation: rustfmt --edition 2021 --check zircon_runtime_interface\src\ui\widget.rs zircon_runtime\src\ui\surface\render\resolve.rs zircon_runtime\src\ui\surface\render\text_fields.rs zircon_runtime\src\ui\accessibility\extract.rs zircon_runtime\src\ui\tests\render_text_fields.rs zircon_editor\src\tests\ui\boundary\zui_asset_governance\workbench_primitives.rs zircon_editor\src\tests\ui\boundary\template_assets.rs (passed)
  - 2026-06-13 SearchField widget-inference validation: node docs/ui-and-layout/ai-workbench-style/component-prototype/verify-native-component-contract.mjs (passed; zuiAssets=24)
  - 2026-06-13 SearchField widget-inference validation: git diff --check -- docs/zircon_runtime/ui/components.md docs/zircon_runtime/ui/surface/render.md docs/zircon_runtime_interface/ui/contract-spine.md docs/ui-and-layout/ai-workbench-style/component-prototype/README.md docs/ui-and-layout/ai-workbench-style/component-prototype/web-native-handoff-matrix.md .codex/sessions/20260612-0904-editor-ui-architecture-implementation.md zircon_editor/assets/ui/editor/components/workbench/primitives/inputs/workbench_search_input.zui zircon_editor/assets/ui/editor/components/workbench/shell/workbench_component_drawer.zui zircon_editor/assets/ui/editor/components/workbench/shell/workbench_scene_tree_panel.zui zircon_runtime_interface/src/ui/widget.rs zircon_runtime/src/ui/surface/render/resolve.rs zircon_runtime/src/ui/surface/render/text_fields.rs zircon_runtime/src/ui/accessibility/extract.rs zircon_runtime/src/ui/tests/render_text_fields.rs zircon_editor/src/tests/ui/boundary/zui_asset_governance/workbench_primitives.rs zircon_editor/src/tests/ui/boundary/template_assets.rs docs/ui-and-layout/ai-workbench-style/component-prototype/verify-native-component-contract.mjs (passed with LF-to-CRLF warnings only)
  - 2026-06-14 RangeSlider widget-inference validation: rustfmt --edition 2021 --check zircon_runtime_interface\src\ui\widget.rs zircon_runtime_interface\src\tests\ui_contract_spine.rs zircon_runtime\src\ui\surface\surface\default_interactions\range.rs zircon_runtime\src\ui\tests\v2_asset.rs (passed)
  - 2026-06-14 RangeSlider widget-inference validation: cargo test -p zircon_runtime_interface --lib ui_widget_text_and_cursor_contracts_serialize_typed_events --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-ui-rangeslider-pointer-0614-interface --message-format short --color never -- --nocapture (passed 1 focused interface contract test)
...
`UiSurfaceFrame` and `UiSurfaceDebugSnapshot` carry the ECS projection as defaulted fields so old serialized diagnostics remain readable and new editor/runtime tools can observe projection facts through the same surface boundary as layout, render, hit testing, focus, and pipeline reports. `UiSurfaceFrame` also carries defaulted `UiSurfaceWindowState` facts for the most recent window metrics, pixel position, focus state, application activation state, occlusion state, close/closed/destroyed lifecycle facts, and redraw request status; this keeps window-driven lifecycle, layout, and redraw diagnostics on the shared frame snapshot instead of in a host-local side channel.
...
`zircon_runtime_interface/src/tests/surface_frame_contracts.rs` covers frame-level window facts directly. It roundtrips `UiSurfaceWindowState` with focus, application activation, occlusion, close-request, closed, and destroyed lifecycle values, then verifies legacy `{}` payloads keep deserializing with neutral defaults.

`zircon_runtime_interface/src/tests/dispatch_reply_contracts.rs` covers route reply merging directly. It pins that unhandled route steps remain diagnostic-only even if a malformed step carries effects, passthrough steps preserve effect-carrying continuation, and handled steps stop propagation with stable effect ranges. The same focused contract also roundtrips the transient-dismissal reply effect and matching host-request kind so runtime/editor hosts can share a typed popup/tooltip teardown request.

`zircon_runtime_interface/src/tests/ui_painter_style_contracts.rs` covers the shared painter selector contract directly. It pins the family table for Button, IconButton, Toggle, Checkbox, Radio, Slider, Dropdown, PopupRow, Alert, Tooltip, TextField, ListRow, TreeRow, TableRow, Tab, Toast, Chrome, and Generic, verifies disabled dominates every family, verifies loading is honored by Button, Toggle, Checkbox, Radio, Slider, Chrome, and the generic interactive families, and checks that the `UiPainterState` convenience helpers remain thin wrappers over `UiPainterStyleSelector`.
