...
`text_shape.rs` defines `UiTextPaint`, `UiTextShapeArtifact`, `UiShapedText`, `UiShapedGlyph`, and `UiTextPaintDecoration` as the shared shape artifact boundary.

`UiResolvedTextLayout` carries neutral source/visual geometry, not glyph or face authority. `UiTextShapeArtifact` therefore records either an explicitly published `Canonical(UiShapedText)` artifact or `Unavailable`; interface command conversion never derives glyph ids, font resources, atlas resources, or equal-width fallback frames from a resolved layout. R6 extends a canonical artifact with optional font resource keys, text atlas resources, ellipsis range, per-line glyph records, glyph ids, source ranges, visual frames, advances, atlas resources, and atlas UV rects. `UiTextPaint` also carries optional selection, caret, composition, and decoration records so selection fills, caret strokes, composition underline, and outline payloads can be shared with runtime/editor painters instead of recreated per backend.

`UiRenderCommand::text_paint(...)` uses resolved advances only for text-run and editable-decoration geometry, and only when the advance sequence exactly covers the line's grapheme clusters. It reports `Unavailable` until the runtime glyph route publishes a canonical artifact. Selection, caret, and composition underline geometry remains snapped to grapheme cluster edges, so a selection inside `a\u{0301}` or an emoji ZWJ sequence expands to the visible cluster instead of splitting an accent or emoji component.
