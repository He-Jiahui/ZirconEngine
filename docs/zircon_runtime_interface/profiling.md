...
`ProfileSnapshot` is the ABI-safe timeline view. It contains session metadata, active/feature flags, the frame budget, vectors of frame, span, and counter snapshots, and a `recorder_retention` vector. Each retention row preserves one recorder's independent frame/span/counter ring authority: configured capacity, total writes, overwritten samples, retained samples, and oldest/newest write sequence. Hosts that merge editor and runtime snapshots append these rows instead of collapsing counts, so an overwrite in either source cannot be hidden by the other. The field uses `serde(default)` so snapshots produced before retention evidence was introduced still deserialize to an empty vector; an empty vector is compatibility data, not proof that a performance capture retained every sample. Spans carry ids, optional parent ids, optional frame indices, stream/category/name/path strings, timestamps, durations, and nesting depth.
