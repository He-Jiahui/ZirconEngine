...
related_code:
  ...
  - zircon_hub/src/state/hub_message/build.rs
  - zircon_hub/src/state/hub_message/delivery.rs
  - zircon_hub/src/state/hub_message/engine.rs
  - zircon_hub/src/state/hub_message/id.rs
  - zircon_hub/src/state/hub_message/learn.rs
  - zircon_hub/src/state/hub_message/message.rs
  - zircon_hub/src/state/hub_message/mod.rs
  - zircon_hub/src/state/hub_message/process.rs
  - zircon_hub/src/state/hub_message/project.rs
  - zircon_hub/src/state/hub_message/settings.rs
  - zircon_hub/src/state/hub_message/shell.rs
...
`web/src/tauri/hubApi.ts` also exposes `subscribeHubStateChanged`, which listens for the backend `hub-state-changed` event and returns Tauri's unlisten cleanup function. `App.tsx` mounts that listener once, applies pushed `HubViewModel` payloads directly to window state, cleans the listener up on unmount, and surfaces a warning only if live updates cannot be registered. Direct `hub_action` promise replies are guarded by `actionSequenceRef` and `stateGenerationRef`: only the newest action response may apply, and only if no newer backend state push or initial load has already advanced the window model. That keeps rapid Settings `update-settings-draft` edits and other local workflows from being repainted by stale direct responses while still letting live backend events remain authoritative. Build, package, install, and open-editor use the event path for both the immediate running state and the final success/error state while long-running work executes without holding the shared session lock.
...
The current architecture closes the runtime/frontend contract with typed action identifiers and schema validation on both sides. Rust parses action ids through `HubActionId` before payload-specific validation, while TypeScript exposes `HUB_ACTION` and action payload mappings from `web/src/types/hub.ts`; legacy string aliases remain only at the Rust IPC parse edge. `web/src/tauri/hubStateValidator.ts` validates every `hub_state`, `hub_action`, and `hub-state-changed` payload before React applies it. Invalid runtime payloads are ignored or surfaced as recoverable task feedback instead of silently repainting the shell with partial data.

`App.tsx` wraps the routed shell in `HubErrorBoundary`, which can reload the latest backend state through `loadHubState` after a render failure. `HubWindow.tsx` routes through an explicit `Record<HubPageId, HubPageComponent>` table, so a page id must have a component owner before it becomes a first-class route. The fallback state in `web/src/data/hubData.ts` is intentionally minimal and flagged as `demoMode`: it contains localized shell/settings copy but no sample projects, tasks, source engines, catalog rows, team rows, action history, or selected ids. That keeps first paint and non-Tauri recovery honest without treating demo fixtures as runtime data.
...
The follow-up React/MUI coverage slice extends the shell beyond buttons/cards/tables. The input layer now includes checkbox, switch, tabs, text field, combo box, and the project toolbar composition; the data layer now includes list, tree view, metric card, empty-state, project card rail, project detail metric grid, project detail sidebar, and Settings section primitives; the feedback layer exposes snackbar, status banner, and error-boundary components; and the overlay layer exposes shared dialog, menu, popover, source-engine popup, user-menu popup, and New Project dialog components. `TopBar` anchors `SourceEnginePopover` and `UserMenuPopover` while routing settings/learn/team actions through the same `onAction` dispatcher, `ProjectsDashboard` keeps the card dashboard through `ProjectsToolbar`, `ProjectCardRail`, and `CreateProjectDialog` plus routes `project-browser`/`project-detail` subpages to focused React pages, `ProjectBrowserPage` composes shared search/select/toggle/table/source-engine components against `browserProjects`, `ProjectDetailPage` composes cover, `ProjectMetricsGrid`, list, tree, status, and `ProjectDetailSidebar` components against `selectedProject`, `EditorPage` composes launch target, Source Engine, editor-plugin, reserved plugin operation, readiness, and editor activity panels against selected project, plugin, `comingSoon`, source-engine, and action-history DTOs, `BuildsPage` composes build/package/install workflows, output trees, action history, progress, and Source Engine panels against backend action and settings DTOs, `CatalogPage` composes Assets/Plugins/Learn catalog rows, reserved capability rows, tabs, status, trees, quick actions, and source-engine panels against backend catalog and `comingSoon` DTOs, `CloudPage` composes local package outputs, device install handoff, reserved service slots, and task feedback against settings, action history, and `comingSoon`, `TeamPage` composes Git identity, member lists, reserved collaboration rows, action-history detail, source-engine panels, and quick actions against `team`, `actionHistory`, and `comingSoon`, `SettingsPage` delegates build defaults, path defaults, health, source-engine lists, and advanced path trees to `SettingsSection`, `WorkspacePage` remains a fallback layout, and `App` displays real Rust task feedback through `HubSnackbar`.
...
`ui_overlay_primitives_contract.rs` now checks React/MUI overlay primitives instead of historical popup, dropdown, drawer, and window wrappers. The contract locks `web/src/components/overlays/HubDialog.tsx`, `HubMenu.tsx`, and `HubPopover.tsx` as the Material Dialog, Menu, and Popover wrappers; `CreateProjectDialog.tsx` as the project-creation dialog body that consumes `HubDialog`, `HubTextField`, and `HubComboBox`; `SourceEnginePopover.tsx` and `UserMenuPopover.tsx` as the business popup bodies; `web/src/components/shell/TopBar.tsx` as the anchor/action owner for source-engine and user popups; and `web/src/pages/ProjectsDashboard.tsx` as the New Project dialog consumer. `UserMenuPopover` now keeps the sign-out row as a disabled local-v1 account-service reservation with localized coming-soon detail passed from `TopBar`, disabled MUI styling, and a click guard before `onAction`.
...
`ui_project_scope_contract.rs` now checks React/MUI project scope projection instead of historical passive native DTO fields. The contract locks `src/state/scope.rs`, `src/state/hub_snapshot.rs`, `src/settings/hub_config.rs`, and `src/projects/metadata.rs` as the Rust project-scope and metadata owners; `src/tauri_app/view_model.rs` as the visible project, selected-project, and quick-action DTO projection; `src/tauri_app/runtime_state/quick_actions.rs` as the action target resolver; and `web/src/components/data/ProjectCard.tsx`, `ProjectMetricsGrid.tsx`, `ProjectDetailSidebar.tsx`, `QuickActions.tsx`, plus `web/src/pages/ProjectDetailPage.tsx` as React consumers of those DTOs. Project-scoped workflow buttons now dispatch the same selected-project DTO through `projectTargetPayload(project)` instead of passing only a project id.
...
The workflow contract also treats `src/tauri_app/runtime_state/settings_actions.rs` as the `update-settings-draft` owner. The action mutates the editable draft and recomputes Configuration Health, while `save-settings` remains the only path that persists Hub config, registers Source Engines, and refreshes catalogs.
