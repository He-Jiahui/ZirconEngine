...
The Settings page is the local repair and configuration surface for Hub v1. It is now a React/MUI page backed by Tauri state: Rust projects persisted settings and the editable draft as `HubSettingsSummary`, React edits the draft, and only `save-settings` commits that draft to `hub.toml` and refreshes Source Engine catalogs.
