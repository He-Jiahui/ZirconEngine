...
tests:
  ...
  - cargo test --manifest-path zircon_hub\Cargo.toml --locked --offline --jobs 1 --target-dir D:\cargo-targets\zircon-hub-current-visual-0524 --test ui_workspace_layout_contract --test ui_project_layout_contract --test ui_page_surface_coverage_contract --message-format short --color never
  - cargo test --manifest-path zircon_hub\Cargo.toml --locked --offline --jobs 1 --target-dir D:\cargo-targets\zircon-hub-current-visual-0524 --test ui_shell_header_contract --test ui_shell_composition_contract --test ui_shell_navigation_contract --test ui_shell_window_contract --message-format short --color never
...
> Validation truth: `zircon_hub/tests/ui_contract.rs`, `ui_shell_contract.rs`, and `ui_taffy_layout_contract.rs` were empty auto-discovered integration targets and have been removed. The dated commands and result paths retained in the frontmatter or historical sections are receipts only, not current copyable validation. Current validation selects focused existing targets; `zircon_hub/tests/project_workflow_contract.rs` remains active with 9 tests.
...
Current Hub contract coverage lives in focused integration targets such as `zircon_hub/tests/tauri_react_shell_contract.rs`, `zircon_hub/tests/ui_global_rules_contract.rs`, `zircon_hub/tests/ui_material_usage_contract.rs`, `zircon_hub/tests/ui_project_scope_contract.rs`, `zircon_hub/tests/ui_project_navigation_contract.rs`, `zircon_hub/tests/ui_selected_project_catalog_contract.rs`, `zircon_hub/tests/ui_selected_project_runtime_contract.rs`, `zircon_hub/tests/ui_data_display_contract.rs`, `zircon_hub/tests/ui_inputs_contract.rs`, `zircon_hub/tests/ui_navigation_contract.rs`, `zircon_hub/tests/ui_shell_header_contract.rs`, `zircon_hub/tests/ui_shell_composition_contract.rs`, `zircon_hub/tests/ui_shell_navigation_contract.rs`, `zircon_hub/tests/ui_shell_window_contract.rs`, `zircon_hub/tests/ui_panel_slot_contract.rs`, `zircon_hub/tests/ui_typography_contract.rs`, and `zircon_hub/tests/hub_docs_contract.rs`. The removed `ui_contract.rs`, `ui_shell_contract.rs`, and `ui_taffy_layout_contract.rs` files are not owners, compatibility entrypoints, or discoverability stubs. Slint/Material/Taffy ownership details later in this historical section describe the pre-React implementation and do not override the Tauri/React hard cut above.
...
Historical receipt: the 2026-05-24 Slint shell slice introduced `NavStatusPanel` and first moved detailed chrome checks into `zircon_hub/tests/ui_shell_contract.rs`; the 2026-05-25 split moved those assertions into `ui_shell_header_contract.rs`, `ui_shell_composition_contract.rs`, `ui_shell_navigation_contract.rs`, and `ui_shell_window_contract.rs`. The empty broad target has since been deleted, so `ui_shell_contract.rs` is no longer a legacy entrypoint or discoverability stub.

Historical receipt: the 2026-05-24 and 2026-05-25 modularization moved broad Slint checks into focused companion contracts. The former `ui_contract.rs` and `ui_taffy_layout_contract.rs` targets were later reduced to empty files and are now deleted rather than retained as stubs. `project_workflow_contract.rs` was subsequently reactivated and remains a current cross-cutting Tauri/React contract with 9 tests; path scope, Source Engine, page copy, and quick-action behavior also remain covered by their focused companions.

The 2026-05-28 `hub-docs-contracts` milestone reactivated `project_workflow_contract.rs` as the cross-cutting workflow seam while keeping path-scope, Source Engine, UI scope, navigation, and selected-project runtime assertions in focused companion files. It now contains 9 tests spanning Tauri runtime routing and persistence, scoped refresh, action history, React dispatch and typed search payloads, documentation cutover, NSIS routing, and the React-source hard cut. It is an active integration target, not a stub.
