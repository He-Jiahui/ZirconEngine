...
- Runtime 01 dependency governance is mirrored by `tech_stack_source_inventory.py`, `tech_stack_anchor_inventory.py`, `tech_stack_boundary.py`, and `tech_stack_markdown.py`; the split leaves manifest inventory, dependency/version anchors, corrected non-dependency list, ZIP dependency policy line, and expected manifest/non-dependency/ZIP/editor-only candidate counts in the source inventory, leaves doc/guard/behavior/decision/Cargo gate anchors in the anchor inventory, keeps the boundary as the 341-line audit reader/dependency scanner/missing-anchor/risk layer, and keeps the Markdown renderer at 103 lines. It reports `expected_manifest_count = 5`, `expected_non_dependency_count = 5`, `zip_dependency_count = 1`, `expected_zip_dependency_count = 1`, `zip_dependency_violations = []`, `tech_stack_guard_count = 12`, `behavior_test_anchor_count = 4`, `missing_behavior_test_anchors = []`, `editor_only_candidate_count = 3`, `jolt_feature_slot_count = 2`, `declared_removed_dependencies = []`, `rapier_or_avian_dependencies = []`, `mirror_docs_guard_present = true`, and `risks = []`. `runtime_01_tech_stack_mirror_docs_match_structure_audit_counts` keeps this document, Runtime 01, the runtime index, the M0 review, and `runtime-tech-stack.md` aligned with those structure-audit counts; `tech_stack_markdown_split_static_passed_cargo_deferred_tests_deferred` records standalone `tech_stack.rs` 1/1, `tech_stack_dependency_guard.rs` 11/11, and `plan_status.rs` 33/33 evidence. This does not promote Runtime 01 beyond static evidence; `tech_stack/extensions/text_shaper/plugin physics/export_build_plan` Cargo gates remain pending.
...
- Runtime 11 `zircon_runtime::core::runtime::tasks` is folder-backed by JobSystem owner; `job_system_boundary` reports `expected_module_count = 9`, `direct_rayon_paths = 2`, `schedule_parallel_executor_direct_rayon = []`, `diagnostic_anchor_count = 4`, `behavior_test_anchor_count = 13`, `missing_behavior_test_anchors = []`, `oversized_modules = []`, `mirror_docs_guard_present = true`, and `risks = []`. `JobScheduler::wait_all(...)` is the scheduler-owned multi-handle sync point and is covered by `scheduler_wait_all_waits_for_all_handles_and_records_sync_time`; `worker_wait_assist_static_passed_cargo_deferred` adds `worker_thread_wait_does_not_deadlock_scheduler`, with `JobHandle::wait()` calling the task-pool-owned `assist_current_thread_once(...)` while keeping direct Rayon ownership in `core/runtime/tasks/{pool,parallel_for}.rs`. The 2026-06-21 `job_system_inventory_split_static_passed_cargo_deferred_tests_deferred` slice moves source/Rayon ownership into `job_system_source_inventory.py` and declaration/API/test/doc anchors into `job_system_anchor_inventory.py`; `job_system_boundary.py` remains the audit reader/risk layer at 193 lines, and `job_system_markdown.py` owns the Markdown renderer at 64 lines under `job_system_markdown_split_static_passed_cargo_deferred_tests_deferred`. `runtime_11_job_system_mirror_docs_match_structure_audit_counts` keeps JobSystem module docs, Runtime 11, the runtime index, the M0 review, and this convergence document aligned with those structure-audit counts. This does not promote Runtime 11 beyond static evidence; `tasks/ecs_schedule/worker_pool/rayon` Cargo filters remain pending, while the render-owned `parallel_frustum` cutover is recorded as `runtime_11_m2_1_graphics_frustum_rayon_cutover_static_passed_cargo_pending`.
