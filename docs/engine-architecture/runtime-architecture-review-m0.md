...
The first runtime-other large-file slice kept the then-current runtime function table unchanged and split only private session implementation helpers; the later V2-only hard cut removed the retired table surface.
...
## Runtime 10 V2-only ABI Sync (2026-07-15)

Current V2-only owner inventory: `expected_source_file_count = 48`.

The dynamic runtime boundary exposes only the 19-field `zircon_runtime_get_api_v2` / `ZrRuntimeApiV2` pair. V2 includes plugin-event subscription/delivery plus the required submit/poll/harvest operation lifecycle; the app rejects missing or incomplete V2 libraries without downgrade. `dynamic_runtime_api_boundary` reports `expected_source_file_count = 48`, `function_table_structs = 10/10`, `field_count_mismatches = 0`, `missing_repr_c_tables = 0`, `runtime_session_ffi_wrappers = 17/17`, `direct_session_table_entry_bypasses = 0`, `session_owner_extern_c_present = false`, `ffi_panic_anchors = 9/9`, `loader_failure_anchors = 13/13`, V2-only loader anchors, and `risks = []`. The parent `dynamic_api/session.rs` is now a 52-line façade over explicit `ffi`, `state`, `hooks`, `linked_session`, and `construction` owners. The permanent guard is `runtime_10_dynamic_runtime_api_mirror_docs_match_structure_audit_counts`.
