...
2026-07-28 Runtime11 JobSystem historical mirror: `timer.rs` owns bounded process-level deadline dispatch, and `job_system_boundary` then reported `expected_module_count = 10`; asset workers consume that owner instead of creating lifecycle-maintenance threads.
...
Runtime 11 historical guard-owner snapshot (2026-07-10): `job_system_boundary` then reported `expected_guard_file_count = 2`, `missing_guard_files = []`, `mirror_docs_guard_present = true`, and `risks = []` by reading both the route parent `job_system.rs` and the real folder-backed `job_system/mirror_docs.rs` owner. `runtime_11_job_system_mirror_docs_match_structure_audit_counts` remains the aggregate mirror guard. This does not describe the later scoped execution owner or its current aggregate audit risks.
...
Historical snapshot note: the Runtime09 M3.1 template-pipeline statements below describe the
2026-06-13 guard state. Runtime74 RTB-P1-001 supersedes that slice: `UiTemplateRuntimePipeline`,
`UiTemplateLoader`, `UiTemplateValidator`, `UiTemplateDocument`, and `UiTemplateError` are deleted;
the current source path is `UiAssetLoader -> UiDocumentCompiler -> UiCompiledDocument`, with
`UiTemplateSurfaceBuilder` consuming the compiled result. The old names and counts below remain
historical audit evidence, not current API or compiler authority.
...
2026-08-26 Runtime02 M1 implementation registers `tasks/callback_dispatcher.rs` and `tasks/execution/` as canonical JobSystem owners (`expected_module_count = 12`). Its explicit runtime and scopes establish fallible pool construction, admission closure, requested-versus-acknowledged cooperative cancellation, typed terminal status, and queue/running task-body accounting without a process-static fallback. `JobHandle` and `schedule_after(...)` propagate prerequisite cancellation without launching dependent work; dynamic-scene loading acknowledges observed scope cancellation. The folder-backed structure slice reduced every audited task root below 500 lines (`oversized_modules = []`). `ExecutionWorkerInventory` exposes only that runtime's three owned domains and conserved worker total, explicitly excluding process-default pools, timer, and private worker owners. Scope quiescence is not a worker-join or DLL-unload receipt. The mesh builder direct-Rayon path, scene target staging, worker-owner hard cutover, the legacy scheduler, process timer, and private worker paths remain open, keeping this change implementation-pending rather than accepted.

2026-08-27 Runtime11 extends the focused JobSystem inventory with bounded `tasks/diagnostic_observation/`, `tasks/bounded_stream_io/`, and `tasks/retained_byte_budget.rs` owners: the current mirror is `expected_module_count = 15` and `behavior_test_anchor_count = 67`. Terminal task facts remain in an independently enabled, bounded runtime-neutral journal. Blocking stdout/stderr readers are admitted per complete capture, capped by physical Runtime `Io` parallelism, decoded with fixed chunk/line limits, retained/drained under entry, byte and time budgets, and retried after interrupted reads. They execute through `ExecutionScope` without private reader threads; process termination and pipe closure remain host operations, while tail drain and reader retirement remain Runtime-owned. Retained task results hold clone-safe byte and lease reservations through the final consumer clone. Folder-backed splits leave `job_scheduler.rs = 347` and `diagnostics.rs = 427`, preserving `oversized_modules = []`, and the audit rejects `zircon_editor` dependencies. Managed Cargo, dynamic-scene sync-facade migration, Editor14 migration, performance/power evidence and independent acceptance remain pending.

2026-08-27 Runtime11 current single-owner cut supersedes the retained three-domain
snapshots above. `tasks/task_graph/EngineTaskGraph` owns exactly one physical
worker set; `TaskGraphWorkerInventory` reports one set and its exact worker count,
and Core work-kind pool selectors are removed. The current focused mirror is
`expected_module_count = 22` and `behavior_test_anchor_count = 73`, including
`tasks/bounded_stream_io/` and `tasks/retained_byte_budget.rs`. Dynamic session
shutdown drains scope and modules before TaskGraph close/join. Managed Cargo,
affinity/priority/quota, keyed-I/O parallelism, remaining process/private owners,
and product performance/power acceptance remain pending.
...
- Done on 2026-06-04: the support-crate ABI surface split reduced the runtime API root from 1082 lines to a small facade. The current V7 facade is `zircon_runtime_interface/src/runtime_api/mod.rs`, backed by the `runtime_api/{abi,constants,frame,host,session}` owner domains while preserving the public `runtime_api::*` re-export shape and a boundary test against facade regression.
...
- `query_state/mod.rs` owns only focused child-module wiring and the curated `QueryState` export;
- `query_state/state.rs` owns `QueryState`, construction, access descriptors, cache fields, telemetry fields, and narrow entity projection;
...
This follows Unreal MassEntity's declaration/implementation separation and Bevy's query-state authority: keep state, cache rebuilds, data/filter, iterator, telemetry, and system-param roles navigable instead of stacking them into the folder root. `scene::tests::ecs_query_structure` and the structural audit's `ecs_query_state_boundary` both guard against recreating `query_state.rs`, missing owner files, state or behavior impl families in the root file, and owner files above the current budget. The current source target is old file absent, 11/11 owner modules present, root within 32 non-empty lines, and no oversized owner modules. `ecs_query_state_markdown.py` renders the audit section so `ecs_query_state_boundary.py` stays an audit/risk owner and `audit_runtime_structure.py` stays a short orchestration script.
...
`core_spine_root_generated_boundary.py` now mirrors the Runtime 02 core/root/generated state through the Python structural audit, while `core_spine_root_generated_markdown.py` owns the Markdown renderer. 2026-08-24 current evidence reports core root entries 6/6, core public modules 5/5, retired core root entries 0, runtime root public modules 21/21, public `pub use` sites 2/2, crate-visible graphics alias debt 0/0, root-surface M1 gate `classified-and-clear`, generated export templates 10/10, generated behavior 6/6, generated allowed adapters 6/6, generated migration debt 0/0, generated-code M1 gate `classified-and-clear`, root_entries guard tests 13, root_surface guard tests 6/6, generated-code guard tests 7/7, `guard_test_anchor_count = 26`, `missing_guard_test_anchors = []`, `mirror_docs_guard_present = true`, and `risks = []`; the boundary owner is 315 lines and the Markdown owner is 60 lines. The 21 public module classifications retain `text` as the shared text/typography contract and `operation` as the runtime operation-registry/task-lifecycle surface, while `runtime_diagnostics` is feature-independent private root support so graphics-only tests do not acquire an optional dynamic-api dependency. `runtime_root_surface.py` itself remains the 268-line root public-surface audit/risk owner, while `runtime_root_surface_markdown.py` is the 35-line renderer owner for `render_runtime_root_surface_markdown`. `runtime_02_core_spine_root_generated_mirror_docs_match_structure_audit_counts` keeps Runtime 02, the runtime index, `root_surface.md`, generated-code-boundary, this review, and runtime-interface convergence aligned with those counts. `root_surface_interface_convergence_mirror_uses_current_audit_counts` keeps `runtime-interface-convergence.md` aligned with the same 21-module, 2-public-use, zero-root-debt root-surface evidence, `rhi_wgpu` crate-private backend owner status, and builtin facade cutover. This is static structure evidence only; the full `core/root/generated/export_build_plan/app/editor/plugin` Cargo lane remains pending.
