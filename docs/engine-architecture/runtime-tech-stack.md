...
tests:
  ...
  - zircon_runtime/src/tests/plugin_extensions/export_build_plan_feature_provider.rs::source_template_catalog_completion_links_active_external_optional_feature_runtime_crates
  - zircon_runtime/src/tests/plugin_extensions/export_build_plan_feature_provider.rs::source_template_reports_missing_native_dynamic_feature_provider_as_fatal_when_required
...
Runtime 01 is reopened for the 2026-07-31 Kira Sound owner guard hard cut. `runtime_01_tech_stack_cargo_gate_records_completed_dependency_validation` remains a historical five-gate record, not proof that the current open failure is accepted. The dependency authority remains protected by the existing guards, while the Python owner/pin contract now requires exactly one structured Kira 0.12.2 declaration in the Sound runtime manifest; the two Runtime Rust mirrors and managed gates remain pending.

Historical validation snapshot (2026-07-11): `tech_stack`, `extensions`, `text_shaper`, `export_build_plan`, and the Physics plugin workspace gate were closed on that source snapshot. The result remains in the numbered archive, but current status is `in_progress` until the Kira owner Rust guard, mirror, current-source managed gate, review, and fixed return close.

The same-day manifest-audit refresh treats `glyphon` and `fontsdf` as optional graphics/text dependencies and recognizes only the hard-cut backend feature names `backend-zr-vm` and `backend-jolt`. The retired feature spellings have no compatibility aliases. The direct Runtime 01 audit reads `zircon_plugins/zr_vm_language/runtime/Cargo.toml` as the concrete ZrVM backend owner, verifies every `zr_vm_rust_binding*` path dependency is optional and gated by the plugin feature, and verifies `zircon_runtime/Cargo.toml` has no ZrVM feature or binding dependency. It reports no missing version anchors, no dependency-boundary violations, two visible Jolt slots, and `risks = []`.

The dependency guard's manifest reader is bounded to the root manifest and current `zircon_*` product trees, skips generated/cache/documentation/tool trees and symbolic-link directories, and caches the resulting manifest contents once per test process. Any product subtree or manifest read error is reported as `manifest_scan_errors` and fails the audit closed. The Sound Kira pin must be the sole Kira declaration and must live in package `[dependencies]` or `target.*.dependencies`; dev/build/workspace tables remain visible for leak detection but cannot prove the runtime owner. This keeps the repository-wide direct-dependency policy intact while preventing WSL cross-volume scans from being repeated by every guard case. The Windows graphics baseline additionally keeps the complete WGPU 29.0.3 patch family together: `wgpu`, `wgpu-core`, `wgpu-core-deps-windows-linux-android`, `wgpu-hal`, `wgpu-naga-bridge`, and `wgpu-types` all resolve to `29.0.3`, while `gpu-allocator 0.28.0` resolves `windows 0.62.2`. `wgpu_windows_dependency_family_stays_on_one_compatible_patch_line` rejects a partial patch upgrade that would create distinct DX12 COM type identities.

`tech_stack_source_inventory.py` mirrors the Runtime 01 manifest/dependency/version/count inventory, `tech_stack_anchor_inventory.py` mirrors the doc, guard, behavior, decision, and Cargo-gate anchors, `tech_stack_boundary.py` owns audit reads/dependency scanning/risk aggregation, and `tech_stack_markdown.py` owns the Markdown renderer. The five core product manifests remain the fixed count inventory; the ZrVM plugin manifest is audited separately as a concrete backend owner and does not inflate that count. The mirror guard now resolves its real folder-backed owner at `runtime_absorption/tech_stack/mirror_docs.rs`, rather than the route-only `tech_stack.rs` parent. Current evidence reports `expected_manifest_count = 5`, `expected_non_dependency_count = 4`, `kira_dependency_owners = [zircon_plugins/sound/runtime/Cargo.toml]`, `kira_owner_version_pinned = true`, `kira_owner_violations = []`, `zip_dependency_count = 1`, `expected_zip_dependency_count = 1`, `zip_dependency_violations = []`, `tech_stack_guard_count = 12`, `behavior_test_anchor_count = 6`, `missing_behavior_test_anchors = []`, `editor_only_candidate_count = 3`, `jolt_feature_slot_count = 2`, `declared_removed_dependencies = []`, `rapier_or_avian_dependencies = []`, `mirror_docs_guard_present = true`, `zr_vm_plugin_manifest_present = true`, `runtime_zr_vm_owner_absent = true`, and `risks = []`. The six behavior anchors cover two text-stack checks plus Jolt feature-off unavailability/no-fallback and feature-on ready/native-step behavior. `runtime_01_tech_stack_mirror_docs_match_structure_audit_counts` keeps this tech-stack authority doc, Runtime 01, the runtime index, the M0 review, and runtime-interface convergence aligned with those structure-audit counts. Direct `tech_stack_boundary_audit` verifies one runtime profile hook, one dependency-backed Physics feature, the optional plugin-owned `joltc-sys`, concrete Jolt backend owners, and no runtime solver dependency. It also accepts one or more plugin-owned `zr_vm_rust_binding*` dependencies only when every dependency remains under `../../../../zr_vm/`, optional, and referenced by `backend-zr-vm`; no compatibility path or Runtime forwarding owner is allowed. The completed executable gates remain recorded separately from this structure evidence.
...
| `wgpu` / `naga` | workspace requirements `29.0.1` / `29.0.1`; validated WGPU patch family `29.0.3` | `zircon_runtime::graphics` | default runtime client/editor-host profile through render features | Renderer plan owns upgrades; the lock must keep the WGPU family on one patch line and `gpu-allocator 0.28.0` on `windows 0.62.2`; `zircon_runtime_interface` must stay free of both dependencies. |
...
| `kira` | `0.12.2` | `zircon_plugins/sound/runtime/Cargo.toml` only | Sound runtime plugin | The Sound execution core uses the Kira-owned CPAL backend while retaining plugin-owned DSP/HRTF/occlusion policy. Kira must not enter `zircon_runtime`, `zircon_runtime_interface`, or `zircon_editor`; the Runtime01 owner guard rejects every non-Sound declaration. |
