...
The platform host scaffold contract is part of SourceTemplate generation. Desktop targets keep the existing executable `src/main.rs`. Android and iOS targets emit `src/lib.rs` with the exported `zircon_export_start` entry, mobile library crate-type settings in `Cargo.toml`, `src/zircon_plugins.rs`, `assets/zircon-project.toml`, and packageable host files under `platform/android/` or `platform/ios/`. Android emits `settings.gradle.kts`, root and app Gradle manifests, `app/src/main/AndroidManifest.xml`, `MainActivity.kt`, resource strings, a `jniLibs` placement contract, `package-export.ps1`, `signing.properties.example`, `play-publish.json`, and `release-bundle.ps1`. iOS emits `Package.swift`, `ZirconRuntimeHost/Sources/ZirconRuntimeHostApp.swift`, resource metadata, a C header/module map for the Rust static library, `package-export.ps1`, `ExportOptions.plist`, `app-store-connect.env.example`, and `archive-export.ps1`. These files describe the release-signing credential environment and archive/bundle steps without embedding secrets or pretending local CI has Apple/Google credentials.

WebGPU and WASM targets emit the same Rust library entry plus browser shells under `platform/webgpu/` or `platform/wasm/`, with `package.json`, `vite.config.mjs`, `public/zircon-export.manifest.json`, `package-export.mjs`, JavaScript hosts that declare the canvas, WebGPU adapter or compiled WASM module, and `assets/zircon-project.toml` browser-fetch resource manifest. Browser targets also emit `public/_headers`, `public/zircon-export.cdn-manifest.json`, and `deploy-cdn.mjs`, making immutable cache policy, COOP/COEP headers, compression expectations, asset-integrity handoff, and `ZR_CDN_BASE_URL` explicit. The scaffold remains generated data owned by `zircon_runtime::plugin::ExportBuildPlan`, while the runtime bootstrap remains routed through `zircon_app::EntryRunner`.

`zircon_runtime_interface::runtime_api` now carries the platform-host ABI surface needed by those generated hosts. `ZrRuntimeEventV1` still preserves the desktop viewport/pointer/mouse fields, but it now also declares lifecycle events, touch events, keyboard events, viewport/canvas metrics, and stable lifecycle/touch/key constants. `ZrHostApiV1` includes an optional `fetch_resource` callback, and `ZrRuntimeHostFetchRequestV1` plus `ZR_RUNTIME_FETCH_FLAG_STREAMING_V1` define the browser/mobile resource-fetch streaming contract. These are ABI DTOs only; native host implementations still decide how to translate Android/iOS/browser platform callbacks into the shared event table.
...
- `zircon_runtime_interface` ABI contract tests now cover lifecycle, touch, keyboard, viewport metrics, and streaming host fetch DTOs
