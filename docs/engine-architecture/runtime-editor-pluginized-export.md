...
tests:
  ...
  - Windows Task 12: cargo check --manifest-path "zircon_plugins\Cargo.toml" -p zircon_plugin_editor_support -p zircon_plugin_sound_editor -p zircon_plugin_net_editor -p zircon_plugin_navigation_editor -p zircon_plugin_particles_editor -p zircon_plugin_texture_editor -p zircon_plugin_virtual_geometry_editor -p zircon_plugin_hybrid_gi_editor -p zircon_plugin_runtime_diagnostics_editor -p zircon_plugin_ui_asset_authoring_editor -p zircon_plugin_native_window_hosting_editor --locked --jobs 1 --target-dir "D:\cargo-targets\zircon-runtime-dynamic-milestone" (passed)
...
The hard-cut runtime crate boundary now also reflects that split at compile time. `target-client` and `target-editor-host` no longer pull a synthetic graphics-base plugin feature. `WorldDriver` keeps the core scene tick path alive without optional physics or animation plugin crates. Optional sound, texture, network, navigation, particles, virtual geometry, and hybrid GI implementations live in plugin packages; physics and animation have now regained package/catalog ownership, while this slice still exposes their shared manager/tick contracts from `zircon_runtime::{physics,animation}` until the deeper behavior relocation is complete. This keeps `core-min` startup from activating optional physics or animation modules by default even before all source files have moved.

The current independent-plugin cut restores `physics` and `animation` as real `zircon_plugins` packages with `plugin.toml`, runtime crates, editor crates, runtime catalog entries, editor catalog entries, and workspace consistency tests. Runtime module selection now treats their project selections like other external packages: a required selection without a linked registration report is fatal, and source-template/library-embed exports generate calls to `zircon_plugin_physics_runtime::plugin_registration()` or `zircon_plugin_animation_runtime::plugin_registration()`. The shared runtime contract code still lives under `zircon_runtime::{physics,animation}` for this slice so scene tick and manager behavior keep compiling while the deeper behavior move is staged; catalog/export/editor enablement no longer treats either package as a runtime built-in domain.
...
Integration tests that exercise optional runtime plugins follow the same rule. Physics/animation runtime behavior tests continue to exercise the shared contracts directly while new plugin package tests prove their runtime/editor crates, manifests, and catalog entries exist. As the deeper move proceeds, plugin-specific contracts should migrate into the matching plugin workspace member instead of re-entering the runtime core module list.
...
The editor shell now has a built-in Desktop Export pane (`editor.build_export_desktop`) in the bottom-right tool area. The pane projects Windows, Linux, and macOS export plans from the active project manifest, shows strategies, plugin counts, linked crates, native package counts, generated file counts, fatal state, and joined diagnostics, and uses the same `EditorManager::generate_native_aware_export_plan(...)` path as programmatic exports. Each row exposes `BuildExport.Execute.<profile>` and switches to `BuildExport.Cancel.<profile>` while that profile is queued, running, or cancel-requested. The Slint host routes execution through a single-job `DesktopExportJobQueue`, runs `EditorManager::execute_native_aware_export_build_with_cancellation(...)` on a background thread with a project-local default output root at `Builds/zircon/<profile>`, and stores the latest `EditorExportBuildReport` summary per profile after the backend returns. The queue projects coarse progress states (`Queued`, `Running`, `Cancel requested`, `Exported`, `Failed`, `Cancelled`), copied package/generated file counts, output root, and diagnostics onto the next pane projection. Cancellation now propagates into SourceTemplate Cargo builds and native dynamic plugin Cargo builds; the shared Cargo helper drains stdout/stderr on reader threads, polls the cancellation token, and terminates the running build tree before returning a cancelled invocation. Windows cancellation uses `taskkill /PID <cargo-pid> /T /F`; Unix hosts start Cargo in an isolated process group and kill that group, with single-process kill kept as a fallback. `BuildExport.SetOutput.<profile>|<path>` and `BuildExport.ClearOutput.<profile>` provide the host-side output override contract; the remaining editor UX work is a native folder picker and richer stage/percentage telemetry.

CI now includes an `export-platform-contract` matrix over Windows, Linux, macOS, Android, iOS, WebGPU, and WASM. The job runs the runtime export policy test with `ZR_EXPORT_CONTRACT_PLATFORM` set for each matrix row, so second-stage platform policy drift is caught even before mobile/browser app-host packaging exists.
...
`RuntimeExtensionRegistry` is the first strongly typed registration layer for runtime plugins. It accepts manager descriptors, module descriptors, render feature descriptors, render pass executors, Virtual Geometry runtime providers, component type descriptors, runtime UI component descriptors, plugin options, plugin event catalogs, and asset importer handlers/descriptors before those contributions are applied to startup, render, UI, asset import, or scene runtime surfaces. `RuntimePluginRegistrationReport::from_plugin(...)` now mirrors package-manifest-declared options, event catalogs, component descriptors, UI component descriptors, and asset importer descriptors into the registry, while preserving a real importer backend when the plugin already registered one. Duplicate manager service names, module names, render feature names, executor ids, provider ids, component type ids, UI component ids, option keys, event namespaces, and importer matchers are rejected with explicit diagnostics, so plugin manifests and VM host handles can share the same id authority before `CoreRuntime::register_module(...)` or the asset pipeline sees the merged service table. Runtime UI component ids must also stay under the contributing plugin prefix, such as `weather.Ui.CloudLayerInspector`, matching the ECS component id rule. The registry is split into declaration, registration behavior, read access, and module/UI/world-application files.
