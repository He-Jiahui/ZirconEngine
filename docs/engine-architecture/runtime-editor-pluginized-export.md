...
tests:
  ...
  - 2026-06-01 module capability namespace ownership contract: cargo test --manifest-path .\Cargo.toml -p zircon_runtime --lib plugin_tomls_declare_module_capabilities_stay_under_owner_namespace --locked --offline --jobs 1 --target-dir D:\cargo-targets\zircon-plugin-bevy-refs --color never -- --test-threads=1 (failed first because `animation.editor` exposed a valid editor module-local capability that is not package-owned; passed after the contract was corrected to require module capabilities to stay under the declaring package or optional-feature namespace)
  - 2026-06-01 module capability namespace ownership full static contract rerun: cargo test --manifest-path .\Cargo.toml -p zircon_runtime --lib plugin_tomls_declare --locked --offline --jobs 1 --target-dir D:\cargo-targets\zircon-plugin-bevy-refs --color never -- --test-threads=1 (56 passed; same static manifest contract set with the module capability namespace-ownership guard)
  ...
  - 2026-05-03: rustfmt --edition 2021 --check --config skip_children=true zircon_editor/src/ui/retained_host/app/build_export_actions.rs zircon_editor/src/ui/retained_host/app/build_export_actions/output_folder.rs zircon_editor/src/ui/retained_host/ui/pane_data_conversion/build_export.rs (passed)
  - 2026-05-03: git diff --check -- zircon_editor/src/ui/retained_host/app/build_export_actions.rs zircon_editor/src/ui/retained_host/app/build_export_actions/output_folder.rs zircon_editor/src/ui/retained_host/ui/pane_data_conversion/build_export.rs docs/editor-and-tooling/editor-workbench-shell.md docs/engine-architecture/runtime-editor-pluginized-export.md (passed; LF-to-CRLF warnings only)
  - 2026-05-03: cargo check -p zircon_editor --lib --locked --jobs 1 --target-dir E:\cargo-targets\zircon-editor-gap-check --message-format short --color never (passed with existing warnings)
...
- registry collection and duplicate diagnostics for managers, modules, render features, render pass executors, Virtual Geometry runtime providers, components, UI components, plugin options, plugin event catalogs, and asset importers, including catalog-level duplicate manager merge diagnostics
- plugin UI component id prefix and `.zui` document validation plus `UiComponentDescriptorRegistry` installation
...
- all static `zircon_plugins/**/plugin.toml` files are guarded for known top-level and nested-row `PluginPackageManifest` fields and explicit top-level package identity (`id`, lowercase snake-style package id, package id matching the `zircon_plugins/<plugin_id>/plugin.toml` directory, globally unique package id, semantic `version`, trimmed public `display_name`/`description`, trimmed optional-feature display names, and trimmed capability-status notes), semantic SDK API version, known package category and maturity values, known supported target values, package capabilities, unique string-array entries, and known default packaging strategies before package availability and export planning consume them
- optional static reverse-DNS package coordinate fields are guarded as an all-or-none set (`package_prefix`, `package_company`, and `package_name`), with trimmed lowercase coordinate segments and globally unique resolved package ids before registry and export surfaces consume `PluginPackageManifest::package_id()`
- all declared static package `supported_platforms` arrays are guarded for known `ExportTargetPlatform` names and duplicate-free entries, while declared `asset_roots` and `content_roots` are guarded for duplicate-free relative package paths with forward slashes and no empty/current/parent segments before export packaging and asset discovery consume them
- all static capability-like fields are guarded for trimmed lowercase dot-separated namespace segments across package capabilities, capability statuses, package dependencies, optional-feature capabilities/dependencies, module capabilities, importer required capabilities, and option capability gates; package-level and optional-feature capabilities also have globally unique static owners, status rows must reference capabilities declared by the same manifest, and module rows must keep capabilities under their package or optional-feature namespace
- all static capability-status rows are guarded for non-empty capability ids, known `CapabilityStatus` TOML spellings, manifest-local unique capability rows, optional known target modes, optional unique target/reference arrays, repository-relative existing `dev/bevy` reference files, and non-empty notes before package readiness diagnostics consume them
- all static asset importer rows are guarded for globally unique lowercase dot-separated importer ids, owner package ids that match the declaring package, integer `priority` and positive `importer_version`, normalized source-extension or full-suffix selectors, known `ResourceKind` output kinds, and unique required capability lists before asset-import selection consumes them
- all static plugin option rows are guarded for globally unique trimmed lowercase dot-separated keys, trimmed non-empty display/default values, known `value_type` spellings, parseable typed defaults, and non-empty optional capability gates before editor configuration panels or runtime reports consume them
- all static event catalog rows are guarded for globally unique trimmed lowercase dot-separated catalog namespaces, positive `version`, explicit non-empty `events` arrays, lowercase namespace-scoped event ids, non-empty event display names, and optional lowercase dot-separated payload schema ids ending in positive `vN` version segments before event reflection or runtime reports consume them
...
- all static optional feature rows are guarded for matching owner plugin id, globally unique owner-namespaced lowercase dot-separated feature id that does not collide with package ids, non-empty display name/capabilities, known default packaging strategies, and explicit `enabled_by_default` before feature selection or export planning consumes them
- all static optional feature dependency rows are guarded for non-empty trimmed lowercase provider `plugin_id` and capability, explicit primary marker, exactly one primary dependency pointing back to the owning package capability, and feature-local unique plugin/capability pairs
- dependency capability references that point at a known static package must resolve to a capability declared by that package or one of its optional features; dependency rows without a static package target must use host-owned `runtime.module.*` or `runtime.capability.*` namespaces, and option/importer capability gates must resolve to declared static capabilities or explicit host-owned capabilities
- all static package and optional-feature module rows are guarded for non-empty identity fields (`name`, known `kind`, and `crate_name`), manifest-local unique lowercase dot-separated module names, non-empty string capabilities, and known target modes before loader, plugin-window, availability, and export projections consume module-local metadata
- all static module names are guarded under their package or optional-feature namespace, and every module `crate_name` is resolved against `zircon_plugins/Cargo.toml` workspace member package names while requiring the resolved member path to stay under the declaring package root
...
- all static package and optional-feature module target modes must be covered by the declaring package's `supported_targets`, so package availability cannot be narrower than module availability
...
- static package and optional-feature module capability rows must stay under the declaring package or optional-feature namespace, so module-local rows cannot claim another plugin's readiness, plugin-window projection, availability, or export-planning surface
