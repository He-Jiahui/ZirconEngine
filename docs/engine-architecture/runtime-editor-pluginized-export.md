...
The current physics/animation hard-cutover restores those domains as runtime-owned built-ins under `zircon_runtime::{physics,animation}` and deletes the obsolete plugin package owners for those two domains. Builtin runtime module selection still accepts legacy project manifest keys for `physics` and `animation`, but those keys now resolve to the built-in runtime modules and only report that the implementation is built into `zircon_runtime`. The source-template/export fixtures use remaining external plugins such as `sound` when they need to prove linked plugin crate behavior.
...
Integration tests that exercise optional runtime plugins follow the same rule. Physics/animation runtime contracts now stay in `zircon_runtime` because those domains are built into the runtime owner; plugin-specific contracts for remaining external packages should live with the matching plugin workspace member.
...
Runtime-backed editor plugin crates now follow the same non-placeholder rule. `sound`, `texture`, `net`, `navigation`, `particles`, `virtual_geometry`, and `hybrid_gi` each register an authoring view, drawer, UI template, menu item, and open-view operation. Physics and animation authoring no longer comes from `zircon_plugin_*_editor` crates; those package owners were deleted with the runtime-owned physics/animation cutover. The small `zircon_plugin_editor_support` crate owns the shared authoring-surface registration helper so each plugin crate declares only its ids, labels, categories, template document, and capability.
...
- plugin workspace runtime-package behavior tests for sound, net, navigation, particles, texture, virtual geometry, and hybrid GI, including render feature pass/capability metadata for the advanced render plugins
