...
- `SoundManager::default_spatial_scale` and `SoundManager::set_default_spatial_scale` expose the Bevy `DefaultSpatialScale` / `PlaybackSettings::spatial_scale` concept from `dev/bevy/crates/bevy_audio/src/audio.rs` and `audio_output.rs` as a neutral scalar. The concrete runtime stores this on `SoundConfig::default_spatial_scale`, exposes `sound.default_spatial_scale` in the plugin manifest, and applies it to source/listener relative distance during spatial attenuation, panning, and preview HRTF ear-distance calculation.
...
Spatial source rendering now has a Bevy-style default scale knob. `default_spatial_scale` defaults to `1.0`, rejects non-finite or negative values, and scales source/listener positions before distance-sensitive spatial calculations. This lets a project choose whether one scene unit means one audio meter without rewriting every `AudioSource` descriptor.
