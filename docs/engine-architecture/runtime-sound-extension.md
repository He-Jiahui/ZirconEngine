...
- `acoustics.rs` defines ray-tracing convolution status and the neutral ray-traced impulse-response submission descriptor used by optional geometry/ray-query providers.
...
- Framework `automation.rs` defines Timeline-style sound automation bindings and serializable scalar automation curves/keyframes/interpolation modes.
- Framework `events.rs` defines the versioned dynamic event catalog, descriptors, and opaque runtime invocation payloads. The sound runtime owns only registry/queue mechanics for now; cross-plugin event meaning still belongs to the future event system.
- `options.rs` defines runtime/editor-facing sound plugin options and quality gates.
- Framework `output.rs` defines the first output-device contract: descriptor, start/stop state, and render statistics. It is intentionally backend-neutral so the current software output path and a later CPAL/platform driver can share the same manager surface.
...
Timeline integration now has runtime value and curve-entry points. `apply_automation_value(binding, value)` resolves a registered `SoundAutomationBinding`, checks finite input, and applies the scalar to the target track, effect, source, listener, volume, or synthesizer parameter. `apply_automation_curve_sample(binding, curve, time_seconds)` accepts a serializable `SoundAutomationCurve`, validates finite strictly-increasing keyframes, samples step/linear/smooth-step interpolation with endpoint clamping, and then applies the sampled scalar through the same typed binding path. Timeline still owns sequence playback and editor curve authoring, while sound owns deterministic parameter sampling, target resolution, and validation.

Dynamic events are now an interface-level runtime queue rather than only an empty manifest slot. `dynamic_event_catalog(...)`, `register_dynamic_event(...)`, and `unregister_dynamic_event(...)` manage the sound-owned versioned catalog; `submit_dynamic_event(...)` validates event IDs, finite event time, and payload schema before appending an opaque invocation; `drain_dynamic_events(...)` hands pending invocations to the future multi-plugin event dispatcher and clears the queue. No event handler semantics are executed in this slice.
...
Output device lifecycle is now explicit in the manager surface. `configure_output_device(...)` validates the backend ID, display name, sample rate, channel count, block size, and latency blocks, then updates the runtime config and mixer snapshot format. Reconfiguration stops the software output device and clears DSP runtime state because sample-rate/channel changes invalidate delay lines and effect history. `start_output_device(...)`, `stop_output_device(...)`, `output_device_status(...)`, and `render_output_device_block(...)` form the first pull-based backend entry point: a platform callback can request exactly the configured block size while the sound runtime records rendered blocks, rendered frames, underrun count, and the last render error.

Spatial rendering has a deterministic HRTF preview path. When an active `AudioListener` has an `hrtf_profile`, the runtime uses listener ear offsets to estimate left/right ear distance, interaural gain, and a capped interaural delay before the usual source gain/pan step. This keeps the current CPU renderer testable while giving the later real HRTF backend a concrete listener/source data flow to replace.
...
3. Mix explicit `SoundSourceDescriptor` voices into dry source buffers, including clip, external block, synth parameter, and silence inputs. Clip and external inputs use the same deterministic nearest-frame source cursor policy.
...
- Output devices are modeled as backend-neutral DTOs instead of binding the sound plugin to a concrete OS audio library in this slice. The software output path is pull-based, which matches real audio callbacks while keeping tests deterministic.
...
- HRTF preview is intentionally driven by neutral listener ear-offset/profile fields rather than an embedded HRTF library. It gives authored listener data an audible deterministic effect now, while true profile loading and convolution/HRTF kernels remain an advanced backend concern.
...
- A versioned `sound.dynamic_events` event catalog plus manager-level register/submit/drain APIs. The runtime queue carries opaque payload bytes and schema IDs only; handler dispatch waits for the later multi-plugin event system.
...
- External source blocks reject zero sample rate, zero channels, non-finite samples, and partial frames; clearing a missing external handle returns `UnknownExternalSource`.
- Source parameter bindings reject empty IDs and unsupported source parameter names before insertion. Missing synth parameter values are treated as absent modulation for that block rather than as render failures.
- Output device descriptors reject empty IDs/backend/display names, zero sample rates, zero channel counts, zero block sizes, and zero latency blocks. Pulling output before start returns `BackendUnavailable` with a stopped-device detail.
...
- Dynamic event descriptors reject empty IDs, display names, or payload schemas; submitted invocations reject unknown events, mismatched schemas, and non-finite event times. Unregistering an event also removes pending invocations for that event ID.
...
- Automation bindings must name a Timeline-style track path. Runtime value application and curve sampling are available for scalar targets, but Timeline sequence scheduling remains a gated integration point.
...
- `cargo check --manifest-path zircon_plugins\sound\runtime\Cargo.toml --tests --locked --offline --jobs 1 --target-dir E:\cargo-targets\zircon-sound-presets --message-format short --color never` then stopped before the sound crate because `zircon_runtime/src/lib.rs` declares `pub mod animation;` and `pub mod physics;`, while the corresponding `zircon_runtime/src/animation/*` and `zircon_runtime/src/physics/*` files are currently deleted in the shared worktree.
...
- real OS audio-device adapter on top of the software output-device contract,
- production-grade low-latency DSP details such as higher-quality filters/resamplers, denormal handling, SIMD/partition tuning, and audio-device scheduling,
- production HRTF profile loading/kernel processing and geometry-backed occlusion,
- actual ray traversal/impulse-response generation from scene geometry,
- Timeline sequence scheduling plus dynamic multi-plugin event handler semantics,
