...
related_code:
  ...
  - zircon_editor/src/tests/fixtures/ui_zui/editor/asset_browser.zui
...
本篇重点说明 ownership、会话边界和 Rust-owned retained host 约束，而不是重复 shared `.zui` 资产格式本身。资产格式见 [`UI Asset Documents And Editor Protocol`](../ui-and-layout/ui-asset-documents-and-editor-protocol.md)。
...
- save-local-copy 只把当前本地 canonical source 写入显式路径或原文件旁保留源扩展名的 `*.local-copy.zui`，不更新 disk baseline、不清理 dirty/conflict，也不覆盖外部变更
...
Template-node projection keeps render extraction as runtime behavior. `asset_editor/node_projection.rs` and `layouts/views/view_projection.rs` now call runtime extraction when they need command/style/text rows for host projection; they do not call an interface-owned `UiRenderExtract::from_tree(...)` constructor. This preserves `zircon_runtime_interface::ui::surface::UiRenderExtract` as a data-only DTO while letting editor host projection reuse the canonical runtime extraction path. The UI Asset Editor shell projection now uses `ui_asset_editor.zui` as its production projection source: the first call hydrates the flat v2 document through `UiV2PrototypeStoreFileCache`, stores the compiled document handles in heap-backed session state, and builds a retained `UiSurface`; later size-only calls mark retained roots layout dirty and use `UiSurface::rebuild_dirty(size)` instead of reloading old layout/widget/style TOML or rebuilding from recursive `UiCompiledDocument`.
...
The following section is retained only as historical cutover context for reviewers reading older validation logs. It does not define current owner paths; current implementation and tests must use `retained_host`, `.zui`, and Rust-owned `host_contract` paths above.
...
- [`template_assets.rs`](../../zircon_editor/src/tests/ui/boundary/template_assets.rs) rejects fallback to deleted hardcoded pane geometry by checking the `.zui` shell regions that each pane must consume.
...
剩余旧形状兼容只留在 runtime 模板层的 `#[cfg(test)]` 迁移 helper，以及 editor 自己的 `src/tests/fixtures/ui_zui/**` `.zui` 夹具和测试迁移 helper；production editor authoring path 只接受 tree authority，它已经不再是 editor authoring session 的内部工作模型，`assets/ui/**` 也不再保存旧 schema 输入。
...
- `cargo test -p zircon_editor --lib ui_asset_editor_bootstrap --locked --jobs 1 --target-dir E:\cargo-targets\zircon-ui-cutover-move-first --message-format short --color never -- --test-threads=1 --nocapture`
  - 证明 `ui_asset_editor.zui` 的 tool-mode row、diagnostic overlay 和 emergency shell band 能经 v2 loader/compiler/surface projection 导出有效 frame；当前 bootstrap authoring 路径不再依赖旧 `ui_asset_editor.ui.toml`
...
- `cargo test -p zircon_editor --lib ui_asset_reference_and_promotion --locked -- --nocapture`
  - 追加验证 reference/promotion host 回归，确认 promotion undo/redo 删除或恢复外部 `.zui` 时 dependent refresh 不再把 missing file 当作 fatal host error；旧 `.ui.toml` 覆盖仅作为历史日志语境保留
...
- `cargo test -p zircon_editor --lib ui_asset_editor_bootstrap --locked --jobs 1 --target-dir D:\cargo-targets\zircon-ui-designer-m6 --message-format short --color never -- --nocapture --test-threads=1`
  - 追加验证 UI Asset Editor bootstrap/projection 回归，确认 `ui_asset_editor.zui` authored shell、header rows、panel columns 和 route 能打开并投影，且 v2 authoring edit/canonical save 不会回退成旧 schema
...
- `cargo test -p zircon_editor --lib tests::ui::ui_asset_editor --jobs 1 -- --nocapture --test-threads=1`
  - 追加验证 40 个 UI Asset Editor v2 bootstrap、historical fixture、preview/session 合同回归，确认 `from_v2_source`、schema-aware replay、v2 imported composite authoring preview 和剩余历史 fixture 能共存
...
### 2026-07-01 Editing Fixture Suffix Guard

`editor_ui_11_m5_ui_asset_editing_test_fixture_zui_guard_passed` records the post-cutover UI Asset Editor editing fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_ui_asset_editing_tests_use_zui_suffix` now scans `zircon_editor/src/tests/editing/ui_asset`, `zircon_editor/src/tests/editing/ui_asset_replay.rs`, and `zircon_editor/src/tests/editing/ui_asset_theme_authoring.rs`, so active authoring/session fixtures cannot keep retired `.ui.toml` / `.v2.ui.toml` suffixes as normal asset ids. The slice migrated 301 retired suffix references in 14 editing test files to `.zui`; it did not restore retired loader/importer support, path aliases, or read-old/write-new session compatibility.

`editor_ui_11_m5_host_manager_ui_asset_fixture_zui_guard_passed` records the host-facing manager fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_host_manager_ui_asset_tests_use_zui_suffix` now scans `zircon_editor/src/tests/host/manager`, including `zircon_editor/src/tests/host/manager/ui_asset_reference_and_promotion.rs`, `zircon_editor/src/tests/host/manager/ui_asset_session_preview.rs`, and `zircon_editor/src/tests/host/manager/ui_asset_style_and_inspector.rs`. The slice migrated 94 retired suffix references in host manager temp file names, asset ids, widget/style imports, and component refs to `.zui`, without restoring retired `.ui.toml` / `.v2.ui.toml` suffixes as accepted host/session inputs.

`editor_ui_11_m5_ui_asset_editor_ui_tests_fixture_zui_guard_passed` records the UI-level authoring/productization fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_ui_asset_editor_tests_use_zui_suffix` now scans `zircon_editor/src/tests/ui/ui_asset_editor`, including `zircon_editor/src/tests/ui/ui_asset_editor/bootstrap_assets.rs`, `zircon_editor/src/tests/ui/ui_asset_editor/resource_dependency_view.rs`, and `zircon_editor/src/tests/ui/ui_asset_editor/action_localization_reports.rs`. The slice migrated 26 retired suffix references in UI Asset Editor bootstrap/report/resource fixtures to `.zui`; only `editor_widgets.ui.toml` and `editor_base.ui.toml` remain as explicit negative assertions proving the bootstrap layout no longer imports legacy editor widget/base assets.

`editor_ui_11_m5_host_theme_tooling_fixture_zui_guard_passed` records the host theme tooling fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_host_theme_tooling_tests_use_zui_suffix` now scans `zircon_editor/src/tests/host/ui_asset_editor_theme_tooling`, including `zircon_editor/src/tests/host/ui_asset_editor_theme_tooling/batch_helpers.rs` and `zircon_editor/src/tests/host/ui_asset_editor_theme_tooling/support.rs`. The slice migrated 15 retired suffix references in theme tooling open routes, style imports, and temporary fixture filenames to `.zui`, without restoring retired `.ui.toml` / `.v2.ui.toml` suffixes as accepted host/theme-tool inputs.

`editor_ui_11_m5_component_adapter_fixture_zui_guard_passed` records the UI component adapter fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_ui_component_adapter_tests_use_zui_suffix` now scans `zircon_editor/src/tests/ui/component_adapter.rs`. The slice migrated 3 retired suffix references in component adapter temporary UI asset filenames to `.zui`, without restoring retired `.ui.toml` / `.v2.ui.toml` suffixes as accepted asset-editor adapter inputs.

`editor_ui_11_m5_retained_host_projection_zui_guard_passed` records the retained host projection fixture cleanup. `tools/tests/test_zui_static_suffix_convergence.py::test_editor_retained_host_projection_tests_use_zui_suffix` now scans `zircon_editor/src/tests/host/retained_window/native_host_contract.rs`, `zircon_editor/src/ui/retained_host/ui/tests/host_scene_projection.rs`, and retained host projection assertions. The slice migrated 4 retired suffix references in projection asset ids and assertion messages to `.zui`, without restoring retired `.ui.toml` / `.v2.ui.toml` suffixes as current retained-host template route names.
...
本地 v2 `component` asset 也进入同一条 authoring 线。`lifecycle.rs` 会把当前文档的 `[components.*]` roots 投影成可编辑的 layout-document authoring tree，但 canonical save 会重新展平为 v2 `[components.*]` 和 `[nodes.*]`，不会把组件资产降级为递归 `widget` schema。没有 `[root]` 的 component 文档会在 preview 阶段临时选择第一个 component root 作为预览根，`session_state.rs` 与 `hierarchy_projection.rs` 也会在没有 view root 时回退到 component root 做默认选中和层级展示。这个能力解决的是“导入后只能预览、不能作为组件资产继续编辑”的缺口；可视化 import 管理器和 slot picker 仍是后续产品化工作。
...
M21/M14 editor productization now has a focused UI Asset Editor closure. `runtime_report_state.rs` is the session owner for runtime-owned report projection: it validates the last-valid `.zui` document against both `UiActionHostPolicy::runtime_default()` and `UiActionHostPolicy::editor_authoring()`, collects localization dependency/extraction/diagnostic rows, collects resource dependency/diagnostic rows, and exposes locale preview selection state through `UiAssetEditorPanePresentation`.
