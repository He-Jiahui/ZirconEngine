...
related_code:
  ...
  - zircon_editor/src/ui/retained_host/callback_dispatch/viewport/bridge.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/viewport/pointer_dispatch.rs
  - zircon_editor/src/ui/retained_host/callback_dispatch/pane/surface_control.rs
...
- user: 2026-05-02 继续收紧 InspectorCustomization operation binding 注册校验
...
- cargo test -p zircon_editor --lib editor_runtime_exposes_plugin_inspector_customization_surface_for_inspector_lookup --locked --jobs 1 -- --nocapture
...
- TDD red: InspectorCustomization operation binding validation initially failed after Slint contract repairs because an invalid customization binding registered successfully.
- InspectorCustomization operation binding validation is now covered by the inspector customization registration suite.
  ...
  - cargo test -p zircon_editor --lib event_listener_control_queries_delivery_pages_after_a_delivery_cursor --locked --jobs 1 -- --nocapture
  - cargo test -p zircon_editor --lib event_listener_control_acknowledges_deliveries_through_delivery_cursor --locked --jobs 1 -- --nocapture
  - TDD red: cargo test -p zircon_editor --lib event_listener_control_rejects_unknown_listener_queries --locked --jobs 1 --target-dir D:\cargo-targets\zircon-codex-editor-listener-audit-green --message-format short -- --test-threads=1 --nocapture (failed before unknown-listener page and acknowledgement requests returned structured errors)
  ...
  - blocked attempt: cargo test -p zircon_app --lib editor_cli_operation_parser --features target-editor-host --no-default-features --locked --jobs 1 --target-dir D:\cargo-targets\zircon-codex-app-editor-cli-group --message-format short --color never -- --test-threads=1 --nocapture (failed before parser tests executed because the then-active UI runtime-interface cutover left the now-deleted `zircon_editor/src/ui/slint_host/viewport/submit_extract.rs:21:62` path with mismatched `UiRenderExtract` identities; current retained-host owner is `zircon_editor/src/ui/retained_host/viewport/submit_extract.rs`)
  - pending focused test: cargo test -p zircon_editor --lib editor_runtime_rejects_inspector_customization_bindings_to_missing_operations --locked --jobs 1 --message-format short --color never -- --test-threads=1 --nocapture
  ...
  - zircon_editor/src/tests/host/retained_callback_dispatch/pane/trigger_action.rs
  - zircon_editor/src/tests/host/retained_event_bridge/mod.rs
...
- zircon_editor/src/tests/host/retained_tab_drag/
  ...
  - cargo test -p zircon_editor retained_drawer_resize -- --nocapture
...
- retained host 不暴露 7 个分散的 viewport move/down/up/wheel 直接回调
- 宿主只接收统一 `viewport_pointer_event(kind, button, x, y, delta)` pointer fact
- [`app/viewport.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app/viewport.rs) 负责把这个 fact 还原成 `UiPointerEvent`
- [`callback_dispatch/viewport/bridge.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/viewport/bridge.rs) 与 [`callback_dispatch/viewport/pointer_dispatch.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/viewport/pointer_dispatch.rs) 再用 shared `UiSurface + UiPointerDispatcher` 产出稳定的 `EditorViewportEvent`
...
Workbench pointer bridges treat projected surface keys as ABI. The main document tab strip uses the stable `"document"` key because the retained host document-dock projection and root host callback context submit that key when routing document tab activation and close events. `build_host_document_tab_pointer_layout(...)` must preserve this key even when the surrounding layout model changes, otherwise the shared pointer dispatcher cannot map retained pointer facts back to `LayoutCommand::FocusView` or `CloseView`.

Viewport image polling is edge-triggered from the retained host perspective. `RetainedViewportController::poll_image()` returns `Some(image)` only when the render framework exposes a new captured-frame generation; repeated captures with the same generation keep the cached image internally but return `None` so the host does not republish unchanged pixels.

## Builtin Template Asset Authority

Builtin editor host templates enter the runtime through [`EditorUiHostRuntime::register_document_file(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/template_runtime/runtime/runtime_host.rs). Active editor view/style roots and imported component prototypes such as [`activity_drawer_window.zui`](/E:/Git/ZirconEngine/zircon_editor/assets/ui/editor/components/workbench/shell/activity_drawer_window.zui) use `.zui`. Registration hydrates each root through the shared [`UiV2PrototypeStoreFileCache`](/E:/Git/ZirconEngine/zircon_runtime/src/ui/v2/file_cache.rs), which resolves transitive locator imports such as `res://ui/editor/components/workbench/shell/activity_drawer_window.zui#ActivityDrawerWindow`, stores prototype documents and compiled arenas on the heap, and returns `Arc` handles reused by later `EditorUiHostRuntime` instances when the source file keys have not changed. [`load_builtin_host_templates(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/template_runtime/runtime/build_session.rs) no longer embeds production template strings, and builtin host assets bypass the old recursive document and compiler caches.
...
No-preview asset icons are centralized in retained host visual asset loading and shared selection preview components. Thumbnail/list fallbacks and selection/details fallbacks all classify physics and animation assets through the same kind-to-icon mapping.
...
1. retained host / headless / MCP 输入先被适配成 `EditorEventEnvelope` 或 `EditorUiBinding`
...
因此 `nativeBinding`、retained host callback 名称、route id 都只是外层 transport / adapter 输入，不再是语义层日志格式。
...
- retained toolbar 不再为每个按钮暴露独立 direct callback 路径，而是统一走 shared pointer 点击入口
- [`viewport_toolbar_pointer/mod.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/viewport_toolbar_pointer/mod.rs) 负责 surface-local hit route 归一化
- [`callback_dispatch/shared_pointer/viewport_toolbar.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/shared_pointer/viewport_toolbar.rs) 与 [`callback_dispatch/viewport/toolbar_control.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/viewport/toolbar_control.rs) 把 route 转回 typed `ViewportCommand` 或 cycle/toggle 语义
- runtime journal 中记录的仍是统一 `EditorViewportEvent`，而不是某个 host-only callback 名称
...
## Retained Host Adapter Path

桌面宿主现在也必须遵守和 headless / reflection 相同的 runtime authority：

- `zircon_editor/src/ui/retained_host/app.rs`
  - `RetainedEditorHost` 持有 `EditorEventRuntime`
...
- `zircon_editor/src/ui/retained_host/callback_dispatch/mod.rs`
  - 把 raw retained host callback 输入收敛成 `EditorEventEnvelope` 或直接语义化 `LayoutCommand`
...
- workbench tab drag 的 `left / right / bottom / document` target route 不再由 deleted shell-local `drag_target_group` 公式决定
- retained host shell 只通过 `update_drag_target(x, y)` callback 把 pointer 位置交回 host
...
- retained host 只消费 host 写回的 `active_drag_target_group`，以及 host 内部基于同一 shell bridge 解析出的 resize group
...
`EditorExtensionRegistry` 是尚未迁到 `ContributionBatch` 的 editor 插件贡献的强类型入口。它收集 view、drawer、Unity 风格菜单路径、Inspector customization、`.zui` 模板和 operation descriptor，并把重复 id 诊断收束在注册阶段。`EditorEventRuntime::register_editor_extension(...)` 会把 extension registry 中的 operation descriptor 合并进 live operation registry，并把 capability-filtered contribution snapshot 交给后续 workbench 菜单、view、drawer 投影消费。注册前 runtime 会先在 scratch operation registry 中合并内建 operation、本扩展 operation、以及扩展 View 自动生成 open operation，再用这份候选集合校验菜单项和组件 Inspector bindings；扩展 View 也会先经过 Workbench ViewRegistry 冲突预检。已注册扩展的 drawer id、菜单路径、inspector customization target type、ui template id 会作为 live contribution set 参与冲突校验，避免两个插件把同一投影入口发布成不同语义。只有所有校验都通过时，runtime 才把 scratch operation registry、扩展 View 和 contribution batch 提交到 live 状态，避免注册失败时留下半注册 operation。组件 Inspector 的 `InspectorCustomizationDescriptor` 记录 target type、ui document、controller 和 bindings；bindings 的最终执行目标仍然应该归一化为 `EditorOperationInvocation`，避免自定义 UI 脚本绕过 journal。
...
组件 Inspector 的扩展入口现在是可查询元数据，而不是脚本旁路。`InspectorCustomizationDescriptor::with_binding(...)` 记录 controller 预期注册或触发的 operation path，`EditorExtensionRegistry::register_inspector_customization(...)` 会用同一个 `EditorOperationPath::parse(...)` 校验这些 binding，拒绝 `weather.refresh` 这类不满足 lower-case dotted 命名契约的 customization 贡献。`EditorEventRuntime::register_editor_extension(...)` 在合并 extension 前还会把内建 operation、本扩展声明的 operation、以及扩展 View 自动生成的 `view.<id>.open` operation 合并成候选集合，并拒绝绑定到不存在 operation 的 InspectorCustomization；这样插件 Inspector 不会发布一个 UI 控件能显示但无法通过 operation registry 派发的按钮。`EditorEventRuntime::inspector_customization(...)` 和 `ui_template_descriptor(...)` 提供运行时查询入口。执行阶段仍应由 controller 把控件事件提交成 `EditorOperation`，再进入 journal / operation stack。
...
`EditorEventListenerRegistry` 是 runtime 内部的事件监听控制层。外部历史面板、自动化桥或后续 MCP/WebSocket transport 可以先注册 listener，再用 `SetEnabled` 暂停/恢复投递，用 `SetFilter` 约束接收范围，用 `ClearFilter` 恢复全量监听，用 `ListListeners` 枚举 descriptor，用 `QueryListenerStatus` 查询单个 listener 的 descriptor、pending delivery 数量、首个 pending sequence 和最后一个 pending sequence。读取投递只允许 `QueryDeliveriesPage { listener_id, after_delivery_cursor, max_deliveries }`：`max_deliveries` 必须在 `1..=256`，响应中的每项均包含 listener-local `delivery_cursor`、event id、sequence、source、operation metadata 与 result。非空页返回最后一项的 `next_delivery_cursor` 并以 `has_more` 表示紧接的下一页；空页才返回空 continuation。轮询端必须保留已见的 delivery cursor watermark 并传回它，因此即使页尾之后才抵达 sequence 较低的事件，也会被后续查询读取，不会被 event sequence 顺序跳过。消费成功后提交 `AckDeliveriesThrough { listener_id, delivery_cursor }`；runtime 只移除该 listener 该 delivery cursor 及之前已确认的投递，绝不以 event sequence 错删迟到但尚未读取的低 sequence 记录。filter 支持 operation path prefix、operation group 精确匹配、event source 列表以及 success/failure 状态开关，例如只订阅 `scene.node.`、只订阅 `viewport.transform_drag.42`、只订阅 `EditorEventSource::Cli`、或只订阅失败的 CLI operation control request；这样外部工具可以按 lower-case dotted 命名空间、连续操作批次、调用来源和审计结果订阅事件，而不需要自己消费全量 journal。外部连接关闭时应调用 `Unregister`，runtime 会同时移除 listener descriptor 和该 listener 的投递缓存，避免断开的远控面板继续累积事件；注销后继续 `QueryDeliveriesPage` 或 `AckDeliveriesThrough` 会得到结构化 failure，而不是一个看起来成功的空投递结果。dispatch 成功或失败都会形成带 operation metadata 的 `EditorEventRecord` 并写入 journal；listener 投递共享同一条 immutable record 的身份信息，因此外部历史面板不会再经历“匿名事件先投递、随后再补写 operation id”的中间状态。
...
这样菜单点击仍可沿用旧 `MenuAction` 热路径，进程内集成可以从同一反射节点读到 `file.project.save` / `window.layout.reset` 等 operation 路径，再通过 operation control request 触发同一行为。
...
- 产品命令行不再暴露 `--operation`、`--args`、`--operation-group`、`--list-operations`、`--operation-history` 或 `--headless`。无头自动化统一使用 `zircon_editor --run <commandlet>`；commandlet 自己定义经注册表验证的输入、稳定 JSON 报告和退出码。
- 未来跨进程或远程协议必须显式适配上述进程内请求，不能重新引入以 CLI 参数转发 operation control 的兼容通道。
...
- `UiHostEventEffects` 把它翻译成 host-level present-welcome 标志
...
当前 runtime 里，retained host `Window` 菜单上的 preset 条目仍通过宿主菜单回调进入 `LayoutCommand::SavePreset/LoadPreset`，但稳定协议层已经把它们定成 `DockCommand::SavePreset/LoadPreset`。这保证了 headless / reflection / nativeBinding 可以直接走 typed contract，而不是复制一套字符串分派。
当前 runtime 里，retained host `Window` 菜单上的 preset 条目仍来自 host menu callback，因为 builtin template 还不会实例化每个动态 preset item；但字符串归一化已经不再停在 [`app.rs`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/app.rs)。现在的真实链路是：

- `dispatch_menu_action(...)`
  -> [`host_menu_action(...)`](/E:/Git/ZirconEngine/zircon_editor/src/ui/retained_host/callback_dispatch/workbench/menu_action.rs)
  -> `EditorEvent::Layout(core::editor_event::LayoutCommand::SavePreset/LoadPreset)`
  -> `UiHostEventEffects.active_layout_preset_name`
...
- `retained_host/ui.rs` 不再需要根据按钮文案硬编码 `"OpenScene"` / `"CreateScene"`
...
当前真实 retained tab drag/drop 仍是宿主内部回调，不是反射公开协议：

- retained tab 释放时先落成内部 `drop_tab(tab_id, target_group)` 回调
- `target_group` 只表达 shell 级别的 `left / right / bottom / document`
- `target_group` 的来源现在是 host-owned `active_drag_target_group`，不再是宿主 UI 壳自己算出来的字符串属性
...
- `RetainedEditorHost` 负责 `EditorStartupSessionDocument` 生命周期与 `EditorManager` 调用
...
当前桌面宿主里的 viewport pointer/scroll 已经不是直接由 retained host callback 手写映射成 `EditorViewportEvent`：

- retained host callback 先生成 shared `UiPointerEvent`
...
The Widget Reflector consumer is deliberately separate from the existing workbench action/reflection model. `WorkbenchWidgetReflectorModel` consumes runtime `UiReflectorSnapshot` data, projects tree rows and selected-node details, and owns only local selection state. It does not query the host projection, rebuild layout, or keep an alternate hit index; runtime `UiSurface` remains the source for reflected lifecycle, properties, frames, and hit context.
...
- 反射树不再依赖 live host tree 查询 transient state
...
- desktop retained callback adapters
  - menu action
  - hierarchy selection
  - asset search
...
- retained effect bridge
  - `EditorEventRecord.effects` -> `UiHostEventEffects`
...
- workbench Widget Reflector snapshot consumption
  - row projection
  - selected-node reflected property view
  - missing-node selection rejection without losing current selection
...
- WSL validation setup: installed `pkg-config` and `libfontconfig-dev` in Ubuntu 22.04 so fontconfig-backed test builds can link under Linux
...
- shared retained 投影领先旧 geometry 时，viewport content frame 仍优先使用 `PaneSurfaceRoot`，避免短暂回退到 stale document frame
...
- retained host shell 不再本地拥有 `drag_target_group` 公式，只保留 `active_drag_target_group` 展示态和 `update_drag_target(...)` 回调
- retained callback adapter 通过 runtime dispatch 执行
...
- `retained_host/app.rs` 的 menu / docking / hierarchy / inspector / asset / viewport callback 已经改成 adapter + runtime dispatch
- `InvokeBinding` / `CallAction` / retained callback 现在可以对齐到同一个 normalized event path
