...
def _server_tokens(line: str) -> list[str]:
    return [
        token
        for token in SERVER_TOKEN_RE.findall(line)
        if _token_has_server_component(token)
    ]


def _is_allowed_server_context(path: Path, line: str, tokens: list[str]) -> bool:
    ...
    if "/net/" in normalized or "/network" in normalized or "/net_features/" in normalized:
        return True
    if "serverruntime" in lower_line:
        return True
    if "runtimeprofileid::server" in lower_line:
        return True
    if "target-server" in lower_line or "target_server" in lower_line:
        return True
    if "headless_server" in lower_line or "headless server" in lower_line:
        return True
    if "dedicatedserver" in lower_line or "listenserver" in lower_line:
        return True
    if "server_client_targets" in lower_line:
        return True
    if normalized.endswith("zircon_runtime/src/plugin/runtime_profile/descriptor.rs"):
        return True
    ...
    ) and ("server config" in lower_line or "dev server" in lower_line or "server:" in lower_line):
        return True
    if normalized.endswith("zircon_editor/src/ui/workbench/startup/display_project_path.rs"):
        return True
    if normalized.endswith(
        "zircon_runtime/src/ui/component/catalog/material_foundation/mui_x.rs"
    ) and '"server"' in line and "mui_enum_prop" in line:
        return True
    if normalized.endswith(
        "zircon_runtime/src/ui/component/state_reducer/table.rs"
    ) and 'Some("server")' in line:
        return True
    if normalized.endswith(
    ...
    if normalized.endswith(
        ...
        return True
    if normalized.endswith("zircon_hub/src/projects/metadata.rs") and (
        "r\"\\\\?\\unc\\server\\share" in lower_line
        or "r\"\\\\server\\share" in lower_line
    ):
        return True
    if normalized.endswith("zircon_runtime/src/platform/capability/matrix/mod.rs") and (
        "server/headless" in lower_line or "server runtime" in lower_line
    ):
        return True
    return False


def _classify_server_reference(relative_path: str, line: str) -> str:
...
def _server_reference_decision(
    ...
    classification = _classify_server_reference(relative_path, line)
...
def non_network_server_references(
    ...
    for path in files:
        for line_no, line in enumerate(_read_text(path).splitlines(), start=1):
            raw_tokens = SERVER_TOKEN_RE.findall(line)
            if not raw_tokens:
                continue
            tokens = [
                token for token in raw_tokens if _token_has_server_component(token)
            ]
            if not tokens:
                observer_false_positive_count += 1
                continue
            if _is_allowed_server_context(path, line, tokens):
    ...
    sample_locations = [asdict(decision) for decision in decisions[:max_locations]]
    risks: list[str] = []
