...
try {
    ...
    $runtimeRoot = Join-Path 'D:\ZirconBuilds' ("tooling15-wave150-runtime-{0}" -f [DateTimeOffset]::Now.ToString('yyyyMMdd-HHmmss'))
    ...
    Write-Host "TOOLING15_WAVE150_RUNTIME_ROOT $runtimeRoot"
    ...
    if (-not [string]::Equals($archiveSha256, $env:MVP_POWERSHELL_SHA256, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Wave150 PowerShell archive digest mismatch: expected=$env:MVP_POWERSHELL_SHA256 actual=$archiveSha256."
    }
