...
try {
    if ($PSVersionTable.PSVersion.ToString() -ne $env:MVP_POWERSHELL_VERSION) {
        throw "Wave148 PowerShell version mismatch: expected=$env:MVP_POWERSHELL_VERSION actual=$($PSVersionTable.PSVersion)."
    }
    ...
    if ((Get-Module Pester).Version.ToString() -ne $env:MVP_PESTER_VERSION) {
        throw "Wave148 Pester version mismatch: expected=$env:MVP_PESTER_VERSION actual=$((Get-Module Pester).Version)."
    }

    Write-Host "TOOLING15_WAVE148_RUNTIME pwsh=$($PSVersionTable.PSVersion) pester=$((Get-Module Pester).Version)"
    $suites = @(
    ...
    $controlPlaneResult = Invoke-Pester `
        ...
        -OutputFile (Join-Path $env:MVP_EVIDENCE_ROOT 'tooling15-wave148-control-plane-nunit.xml')
    ...
    $scriptContractResult = Invoke-Pester `
        ...
        -PassThru `
        -OutputFormat NUnitXml `
        -OutputFile (Join-Path $env:MVP_EVIDENCE_ROOT 'tooling15-wave148-required-scripts-nunit.xml')
    ...
    Write-Host 'TOOLING15_INTEGRATED_WAVE148_PINNED_VALIDATION_PASSED'
}
catch {
    Write-Error $_
    exit 1
