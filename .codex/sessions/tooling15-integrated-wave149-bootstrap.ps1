...
try {
    ...
    $runtimeRoot = Join-Path 'D:\ZirconBuilds' ("tooling15-wave149-runtime-{0}" -f [DateTimeOffset]::Now.ToString('yyyyMMdd-HHmmss'))
    ...
    Write-Host "TOOLING15_WAVE149_RUNTIME_ROOT $runtimeRoot"
    ...
    if (-not [string]::Equals($archiveSha256, $env:MVP_POWERSHELL_SHA256, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Wave149 PowerShell archive digest mismatch: expected=$env:MVP_POWERSHELL_SHA256 actual=$archiveSha256."
    }
    ...
    & $pinnedPwsh `
        ...
        -File '.\.codex\sessions\tooling15-integrated-wave149-pinned-runner.ps1'
    exit $LASTEXITCODE
}
catch {
    Write-Error $_
