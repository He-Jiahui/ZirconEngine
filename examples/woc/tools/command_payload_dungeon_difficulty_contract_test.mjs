import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { dirname, join, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

const projectRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const payloads = JSON.parse(readFileSync(join(projectRoot, 'contracts', 'command_payloads.json'), 'utf8'));
const zr = readFileSync(join(projectRoot, 'scripts', 'woc_game', 'src', 'protocol', 'command_payloads.zr'), 'utf8');
const generated = readFileSync(join(projectRoot, 'native', 'crates', 'woc_protocol', 'src', 'generated_command_payloads.rs'), 'utf8');
const protocol = readFileSync(join(projectRoot, 'native', 'crates', 'woc_protocol', 'src', 'command_payload.rs'), 'utf8');
const difficulty = readFileSync(join(projectRoot, 'native', 'crates', 'woc_protocol', 'src', 'dungeon_difficulty_payload.rs'), 'utf8');
const input = readFileSync(join(projectRoot, 'native', 'apps', 'woc_client', 'src', 'input', 'intent.rs'), 'utf8');
const coverage = JSON.parse(readFileSync(join(projectRoot, 'reference', 'current-head', 'command_payload_coverage.json'), 'utf8'));

assert.equal(payloads.schema_version, 51);
assert.deepEqual(payloads.entries.find((entry) => entry.id === 141), {
  id: 141,
  name: 'set_dungeon_difficulty',
  kind: 'dungeon_difficulty',
  min_byte_length: 1,
  max_byte_length: 1,
  encoding: 'u8_dungeon_difficulty_normal_heroic',
  source_shape: { kind: 'client_send', method: 'setDungeonDifficulty', fields: ['difficulty'] },
});
assert.match(zr, /pub setDungeonDifficultyCommandId\(required: bool\): uint/);
assert.match(generated, /pub const SET_DUNGEON_DIFFICULTY_COMMAND_ID: u16 = 141;/);
assert.match(generated, /DungeonDifficulty/);
assert.match(difficulty, /DungeonDifficultyPayload/);
assert.match(difficulty, /InvalidDungeonDifficulty/);
assert.match(protocol, /CommandPayloadKind::DungeonDifficulty/);
assert.match(input, /SetDungeonDifficulty/);
assert.equal(coverage.totals.typed_contract_commands, 148);
assert.equal(coverage.totals.typed_contract_client_send_commands, 147);
assert.equal(coverage.totals.source_shape_only_commands, 9);
assert.equal(coverage.totals.unmapped_dispatch_commands, 8);
process.stdout.write('Dungeon difficulty command payload contract is complete\n');
