...
use super::AssetCreationTemplateDescriptor;
...
fn operation_path() -> EditorOperationPath {
    EditorOperationPath::parse("fixture.asset.create").expect("valid fixture operation")
}

fn legacy_descriptor(capabilities: &[String]) -> AssetCreationTemplateDescriptor {
    let mut descriptor = AssetCreationTemplateDescriptor::new(
        "fixture.template",
        "Fixture template",
        operation_path(),
    );
...
fn optimized_descriptor(capabilities: &[String]) -> AssetCreationTemplateDescriptor {
    AssetCreationTemplateDescriptor::new("fixture.template", "Fixture template", operation_path())
        .with_required_capabilities(capabilities.iter().cloned())
}

#[test]
fn editor04_type_registry_creation_template_capability_order_is_unchanged() {
...
#[test]
fn editor04_type_registry_creation_template_source_contract() {
    let source = include_str!("../creation_template.rs");
...
fn editor04_type_registry_creation_template_capability_bench() {
    let capabilities = capability_fixture();
    ...
    println!(
        "EDITOR04_TYPE_REGISTRY_CREATION_TEMPLATE_CAPABILITY_BENCH_V1 legacy_p95_ns={} optimized_p95_ns={} samples={} iterations={} capabilities={} unique_capabilities={} stable_sort=1->0 preallocation=0->{}",
        legacy_p95.as_nanos(),
