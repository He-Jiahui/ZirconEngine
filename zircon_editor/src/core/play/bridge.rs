...
pub struct EditorPlayBridge {
    backend: Mutex<SharedEditorRuntimePlayModeBackend>,
}

impl EditorPlayBridge {
    pub fn new() -> Self {
        Self {
            backend: Mutex::new(Arc::new(NoopEditorRuntimePlayModeBackend)),
        }
    }

    pub fn set_backend(&self, backend: SharedEditorRuntimePlayModeBackend) {
        *self
            .backend
            .lock()
    ...
    pub fn backend(&self) -> SharedEditorRuntimePlayModeBackend {
        self.backend
            .lock()
