...
mod mock_expression;
#[path = "preview/mock_suggestions.rs"]
mod mock_suggestions;
#[path = "preview/mock_value_resolution.rs"]
...
pub(crate) fn build_preview_mock_fields(
    ...
    UiAssetPreviewMockFields {
        ...
        schema_items: selected_subject_node_id
            .as_deref()
            .and_then(|node_id| {
                document.nodes.get(node_id).map(|node| {
...
fn preview_mock_expression_graph_items(
    ...
    for (node_id, node) in &document.nodes {
        for (key, value) in &node.props {
            ...
            {
                let Some(target_node) = document.nodes.get(&target_node_id) else {
                    continue;
                };
                let target_key =
...
pub(crate) fn reconcile_preview_mock_state(
    ...
    state.selected_subject_node_id = state.selected_subject_node_id.take().filter(|node_id| {
        document.nodes.contains_key(node_id) && preview_mock_node_has_entries(document, node_id)
    });
...
pub(crate) fn select_preview_mock_subject_node(
    ...
    if !document.nodes.contains_key(node_id) || !preview_mock_node_has_entries(document, node_id) {
        return false;
    }
    state.overrides.retain(|override_node_id, values| {
        let keep_node = document.nodes.get(override_node_id).is_some();
        if !keep_node {
            return false;
        }
    ...
    state
        ...
        .or_else(|| {
            document
                .nodes
                .iter()
                .find(|(node_id, _)| preview_mock_node_has_entries(document, node_id))
...
fn preview_mock_subject_entries(document: &UiAssetDocument) -> Vec<UiAssetPreviewMockSubjectEntry> {
    let mut entries = document
        .nodes
        .iter()
        .filter(|(node_id, _)| preview_mock_node_has_entries(document, node_id))
        .map(|(node_id, node)| UiAssetPreviewMockSubjectEntry {
            node_id: node_id.clone(),
            label: format!(
                "{} • {}",
                node.control_id.as_deref().unwrap_or(node_id),
                node_id
            ),
        })
        .collect::<Vec<_>>();
...
fn resolve_preview_mock_expression_node<'a>(
    ...
    if reference == "self" {
        return document
            .nodes
            .get_key_value(current_node_id)
            .map(|(node_id, _)| node_id.as_str());
    }
    if let Some((node_id, _)) = document.nodes.get_key_value(reference) {
        return Some(node_id.as_str());
    }
    document
        .nodes
        .iter()
        .find(|(_, node)| node.control_id.as_deref() == Some(reference))
        .map(|(node_id, _)| node_id.as_str())
}
...
fn preview_mock_property_value<'a>(
    ...
    state
        ...
        .get(node_id)
        .and_then(|props| props.get(key))
        .or_else(|| document.nodes.get(node_id)?.props.get(key))
}
