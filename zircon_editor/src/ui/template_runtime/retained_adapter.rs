...
impl RetainedUiHostComponentKind {
...
pub enum RetainedUiHostValue {
    String(String),
    Integer(i64),
    Float(f64),
    Bool(bool),
    Array(Vec<RetainedUiHostValue>),
    Table(BTreeMap<String, RetainedUiHostValue>),
    Datetime(String),
}
...
pub struct RetainedUiHostNodeModel {
    ...
    pub disabled: bool,
    pub properties: BTreeMap<String, RetainedUiHostValue>,
    pub style_tokens: BTreeMap<String, String>,
    pub routes: Vec<RetainedUiHostRouteProjection>,
}
...
impl RetainedUiHostProjection {
    pub fn node_by_control_id(&self, control_id: &str) -> Option<&RetainedUiHostNodeModel> {
        self.nodes
            .iter()
...
#[derive(Default)]
pub struct RetainedUiHostAdapter;
...
fn extract_string(properties: &BTreeMap<String, RetainedUiHostValue>, key: &str) -> Option<String> {
...
fn map_value(value: &Value) -> RetainedUiHostValue {
    match value {
        Value::String(value) => RetainedUiHostValue::String(value.clone()),
        Value::Integer(value) => RetainedUiHostValue::Integer(*value),
        Value::Float(value) => RetainedUiHostValue::Float(*value),
        Value::Boolean(value) => RetainedUiHostValue::Bool(*value),
        Value::Datetime(value) => RetainedUiHostValue::Datetime(value.to_string()),
        Value::Array(values) => RetainedUiHostValue::Array(values.iter().map(map_value).collect()),
        Value::Table(values) => RetainedUiHostValue::Table(
            values
                .iter()
                .map(|(key, value)| (key.clone(), map_value(value)))
                .collect(),
