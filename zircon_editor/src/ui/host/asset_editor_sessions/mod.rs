...
pub(super) fn build_ui_asset_editor_session_from_source(
    ...
    if is_v2_ui_asset_id(&route.asset_id) {
...
pub(super) fn ui_asset_editor_route_from_source(
    ...
    let asset_kind = if is_v2_ui_asset_id(&asset_id) {
        let document = UiV2AssetLoader::load_toml_str(source).map_err(|error| error.to_string())?;
...
pub(super) fn is_v2_ui_asset_id(asset_id: &str) -> bool {
    asset_id
        .trim()
        .to_ascii_lowercase()
        .ends_with(".v2.ui.toml")
}
