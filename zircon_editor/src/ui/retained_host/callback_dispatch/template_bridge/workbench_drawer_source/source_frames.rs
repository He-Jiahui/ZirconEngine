...
fn surface_control_frame(surface: &UiSurface, control_id: &str) -> Option<UiFrame> {
    ...
    surface
        .tree
        .node(node_id)
        .map(|node| round_frame_to_centipixels(node.layout_cache.frame))
}

fn round_frame_to_centipixels(frame: UiFrame) -> UiFrame {
    UiFrame::new(
        round_to_centipixel(frame.x),
        round_to_centipixel(frame.y),
        round_to_centipixel(frame.width),
        round_to_centipixel(frame.height),
    )
}
