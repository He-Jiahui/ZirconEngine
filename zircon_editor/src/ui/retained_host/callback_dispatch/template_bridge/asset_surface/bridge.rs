...
use crate::ui::retained_host::callback_dispatch::constants::BUILTIN_ASSET_SURFACE_DOCUMENT_ID;
...
impl BuiltinAssetSurfaceTemplateBridge {
    ...
    ) -> Result<Self, BuiltinAssetSurfaceTemplateBridgeError> {
        let (bindings_by_id, host_projection) =
            project_builtin_surface_with_runtime(runtime, BUILTIN_ASSET_SURFACE_DOCUMENT_ID)?;
        Ok(Self {
            bindings_by_id,
            host_projection,
        })
    ...
    pub(crate) fn new_minimal() -> Result<Self, BuiltinAssetSurfaceTemplateBridgeError> {
        let runtime = load_builtin_runtime_for_documents(&[BUILTIN_ASSET_SURFACE_DOCUMENT_ID])?;
