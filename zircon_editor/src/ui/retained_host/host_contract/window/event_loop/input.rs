...
impl UiHostWindowEventLoop {
    pub(super) fn sync_ime_allowed(&mut self) {
    ...
    pub(super) fn next_input_metadata(&mut self) -> UiInputEventMetadata {
        let metadata = super::super::native_input_metadata(self.next_input_sequence);
        self.next_input_sequence = self.next_input_sequence.saturating_add(1);
        metadata
    }
}

pub(super) fn pointer_button(button: ButtonSource) -> Option<UiPointerButton> {
...
pub(super) fn pointer_button_state(state: ElementState) -> Option<NativePointerButtonState> {
    match state {
...
pub(super) fn scroll_delta(delta: MouseScrollDelta) -> f32 {
    match delta {
