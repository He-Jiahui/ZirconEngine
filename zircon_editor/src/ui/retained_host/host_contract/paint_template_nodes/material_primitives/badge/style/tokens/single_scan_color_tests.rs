...
fn legacy_badge_color_variant(value: &str) -> Option<&'static str> {
    [
        "primary",
        "secondary",
        "error",
...
fn measure(value: &str, optimized: bool) -> u128 {
    ...
    for _ in 0..CHECKS_PER_SAMPLE {
        token = if optimized {
            badge_color_variant(black_box(value))
        } else {
            legacy_badge_color_variant(black_box(value))
        };
    }
    black_box(token);
...
#[test]
fn optimization_batch_20260829br_editor290_badge_color_variants_preserve_results() {
    for value in [
        "primary",
        "warning secondary",
        "DANGER/error",
        ...
        assert_eq!(
            badge_color_variant(value),
            legacy_badge_color_variant(value),
            "{value:?}"
        );
    }
}
...
#[test]
fn optimization_batch_20260829br_editor290_badge_color_uses_one_scan() {
...
fn optimization_batch_20260829br_editor290_single_scan_badge_color_bench() {
    ...
    println!(
        "EDITOR290_SINGLE_SCAN_BADGE_COLOR_BENCH_V1 sample_pairs={SAMPLE_PAIRS} checks_per_sample={CHECKS_PER_SAMPLE} variant_bytes={VARIANT_BYTES} baseline_variant_scans=8 candidate_variant_scans=1 baseline_p50_ns={baseline_p50_ns} candidate_p50_ns={candidate_p50_ns} baseline_p95_ns={baseline_p95_ns} candidate_p95_ns={candidate_p95_ns} baseline_raw_ns={} candidate_raw_ns={}",
        sample_csv(&baseline),
