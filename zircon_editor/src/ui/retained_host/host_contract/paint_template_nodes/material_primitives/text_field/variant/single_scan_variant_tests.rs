...
use super::{text_field_variant_from_component, TextFieldVariant};
...
fn legacy_text_field_variant(value: &str) -> TextFieldVariant {
    if legacy_contains(value, "filled") {
        TextFieldVariant::Filled
    } else if legacy_contains(value, "standard") {
...
fn measure(value: &str, optimized: bool) -> u128 {
    ...
    for _ in 0..CHECKS_PER_SAMPLE {
        variant = if optimized {
            text_field_variant_from_component(black_box(value))
        } else {
            legacy_text_field_variant(black_box(value))
        };
    }
    black_box(variant);
...
#[test]
fn optimization_batch_20260829bu_editor293_text_field_variants_preserve_results() {
    for value in [
        "",
        "filled",
        "STANDARD",
        ...
        assert_eq!(
            text_field_variant_from_component(value),
            legacy_text_field_variant(value),
            "{value:?}"
        );
    }
}
...
#[test]
fn optimization_batch_20260829bu_editor293_text_field_variant_uses_one_scan() {
    ...
    assert!(production.contains("return TextFieldVariant::Filled"));
    assert!(!production.contains("component_variant_contains"));
}

#[test]
#[ignore = "managed Windows release performance evidence"]
fn optimization_batch_20260829bu_editor293_single_scan_text_field_variant_bench() {
    ...
    println!(
        "EDITOR293_SINGLE_SCAN_TEXT_FIELD_VARIANT_BENCH_V1 sample_pairs={SAMPLE_PAIRS} checks_per_sample={CHECKS_PER_SAMPLE} variant_bytes={VARIANT_BYTES} baseline_variant_scans=2 candidate_variant_scans=1 baseline_p50_ns={baseline_p50_ns} candidate_p50_ns={candidate_p50_ns} baseline_p95_ns={baseline_p95_ns} candidate_p95_ns={candidate_p95_ns} baseline_raw_ns={} candidate_raw_ns={}",
        sample_csv(&baseline),
