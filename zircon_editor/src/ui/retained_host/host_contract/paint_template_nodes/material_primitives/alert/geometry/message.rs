...
pub(in crate::ui::retained_host::host_contract::paint_template_nodes) fn alert_message_frame(
    ...
    let width = alert_message_width(measure_runtime_text_width(message, font_size), right - left);
    Some((
        FrameRect {
            x: left,
            y: alert_message_y(rect, line_height),
            width,
            height: line_height,
        },
        font_size,
...
mod tests {
    ...
    fn alert_message_frame_uses_runtime_text_width() {
        ...
        let frame = alert_message_frame(&node, &rect(), 0.0, 260.0, message)
            ...
            .0;
        let expected_width = alert_message_width(
            measure_runtime_text_width(message, alert_font_size(&node)),
            260.0,
        );

        assert!((frame.width - expected_width).abs() <= 0.01);
        let old_heuristic_width =
            alert_message_width(message.chars().count() as f32 * 13.0 * 0.56, 260.0);
