...
use super::{divider_extent_variant, DividerExtentVariant};
...
fn legacy_divider_extent_variant(value: &str) -> DividerExtentVariant {
    if legacy_contains(value, "middle") {
        DividerExtentVariant::Middle
    } else if legacy_contains(value, "inset") {
...
fn measure(value: &str, optimized: bool) -> u128 {
    ...
    for _ in 0..CHECKS_PER_SAMPLE {
        variant = if optimized {
            divider_extent_variant(black_box(value))
        } else {
            legacy_divider_extent_variant(black_box(value))
        };
    }
    black_box(variant);
...
#[test]
fn optimization_batch_20260829bv_editor294_divider_variants_preserve_results() {
    for value in [
        "",
        "middle",
        "INSET",
        ...
        assert_eq!(
            divider_extent_variant(value),
            legacy_divider_extent_variant(value),
            "{value:?}"
        );
    }
}
...
#[test]
fn optimization_batch_20260829bv_editor294_divider_variant_uses_one_scan() {
    let source = include_str!("../extents.rs");
    ...
    assert!(production.contains("return DividerExtentVariant::Middle"));
    assert!(!production.contains("component_variant_contains"));
}

#[test]
#[ignore = "managed Windows release performance evidence"]
fn optimization_batch_20260829bv_editor294_single_scan_divider_variant_bench() {
    ...
    println!(
        "EDITOR294_SINGLE_SCAN_DIVIDER_VARIANT_BENCH_V1 sample_pairs={SAMPLE_PAIRS} checks_per_sample={CHECKS_PER_SAMPLE} variant_bytes={VARIANT_BYTES} baseline_variant_scans=2 candidate_variant_scans=1 baseline_p50_ns={baseline_p50_ns} candidate_p50_ns={candidate_p50_ns} baseline_p95_ns={baseline_p95_ns} candidate_p95_ns={candidate_p95_ns} baseline_raw_ns={} candidate_raw_ns={}",
        sample_csv(&baseline),
