...
use super::{push_text, TimelineStripMetrics};
...
#[test]
fn optimization_batch_20260826gc_editor170_timeline_text_preserves_empty_filter() {
    let frame = FrameRect {
        x: 0.0,
        y: 0.0,
        width: 80.0,
...
#[test]
fn optimization_batch_20260826gc_editor170_timeline_text_owns_after_frame_gate() {
    ...
    assert!(!source.contains("tick.label().to_owned()"));
    assert!(!source.contains("generation.track_label().to_owned()"));
}

#[test]
#[ignore = "managed Windows release performance evidence"]
fn optimization_batch_20260826gc_editor170_timeline_text_deferred_allocation_bench() {
    let label = "t".repeat(LABEL_BYTES);
    ...
    println!(
        "EDITOR170_TIMELINE_TEXT_DEFERRED_ALLOCATION_BENCH_V1 sample_pairs={SAMPLE_PAIRS} \
