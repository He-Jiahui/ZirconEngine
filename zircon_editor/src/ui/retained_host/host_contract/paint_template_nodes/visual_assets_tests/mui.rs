...
fn mui_material_icon_modules_render_from_local_dev_source() {
    ...
    let add_pixels = load_visual_asset_pixels_for_size(&add, 24, 24, None)
        .expect("MUI Add icon should render from the local dev source");
    let add_large_pixels = load_visual_asset_pixels_for_size(&add, 36, 36, None)
    ...
    let search_pixels = load_visual_asset_pixels_for_size(&search, 32, 32, None)
...
fn template_mui_icon_ligatures_render_from_local_dev_source() {
    ...
    let folder =
        template_image_pixels(&preview, "", "folder", 24, 24, Some(ICON_TINT), false, None)
            .expect("MUI Icon ligature should resolve to the local Material icon module");
    let add_circle = template_image_pixels(
        &preview,
        "",
        "add_circle",
        32,
