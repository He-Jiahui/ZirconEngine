...
fn retained_glyph_bitmap_pixel_x_uses_raster_bearing_from_layout_pen_origin() {
    ...
    assert_eq!(
        retained_glyph_bitmap_pixel_x(&glyph, origin_pixel_x, 2),
        origin_pixel_x + 2,
...
fn retained_glyph_bitmap_pixel_x_falls_back_to_layout_bitmap_left_for_invalid_origin() {
    ...
    assert_eq!(
        retained_glyph_bitmap_pixel_x(&glyph, grayscale_placement.pixel_x, -1),
        20,
        "invalid-origin grayscale fallback must still use the smoothing-aware alpha-phase placement for layout bitmap-left"
    );
    assert_eq!(
        retained_glyph_bitmap_pixel_x(&glyph, subpixel_placement.pixel_x, -1),
        20,
