...
#[test]
fn optimization_batch_20260828il_editor_menu_click_reuses_path_capacity() {
...
#[test]
fn optimization_batch_20260828il_editor_click_routes_use_reused_menu_paths() {
    let source = include_str!("../host_menu_pointer_bridge_handle_click.rs");
    let production = source
        .split("#[cfg(test)]")
        .next()
        .expect("menu click production implementation");

    assert_eq!(production.matches("reuse_menu_path(").count(), 3);
...
fn optimization_batch_20260828il_editor_reused_menu_click_paths_benchmark() {
    ...
    println!(
        "EDITOR230_REUSED_MENU_CLICK_PATHS_BENCH_V1 legacy_p95_ns={legacy_p95_ns} optimized_p95_ns={optimized_p95_ns} legacy_samples_ns={} optimized_samples_ns={}",
