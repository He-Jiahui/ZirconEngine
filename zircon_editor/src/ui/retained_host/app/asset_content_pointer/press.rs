...
impl RetainedEditorHost {
    pub(super) fn dispatch_asset_content_pointer_press(
        &mut self,
        surface_mode: &str,
        x: f32,
        y: f32,
        ...
        let Some(target) =
            self.prepare_asset_content_pointer_target(surface_mode, width, height, true)
        else {
            return;
        };
        let point = UiPoint::new(x, y);
        let Some(dispatch) =
            self.dispatch_prepared_asset_content_pointer(surface_mode, &target, true, |bridge| {
                bridge.handle_press(point)
            })
        else {
            return;
        };
