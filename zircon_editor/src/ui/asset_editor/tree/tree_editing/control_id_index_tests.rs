...
const BENCHMARK_MARKER: &str = "EDITOR23_TREE_WRAP_CONTROL_ID_HASH_INDEX_BENCH_V1";
...
fn dense_control_ids() -> Vec<String> {
    ...
    ids.push("Panel".to_string());
    ids.extend((2..=DENSE_ID_COUNT).map(|index| format!("Panel{index}")));
    ids
}
...
#[test]
fn optimization_batch_20260826bc_tree_wrap_control_id_preserves_smallest_gap() {
    let existing = ["Panel", "Panel2", "Panel4"]
        .into_iter()
        .collect::<HashSet<_>>();
    assert_eq!(
        unique_control_id_from_existing(&existing, "Panel"),
        "Panel3"
    );
    assert_eq!(
        unique_control_id_from_existing(&existing, "Stack Panel"),
        "StackPanel"
    );
...
#[test]
fn optimization_batch_20260826bc_tree_wrap_control_id_uses_one_hash_index() {
    let source = include_str!("../tree_editing.rs");
...
fn optimization_batch_20260826bc_tree_wrap_control_id_hash_index_p95() {
    let existing = dense_control_ids();
    assert_eq!(
        legacy_unique_control_id(&existing, "Panel"),
        optimized_unique_control_id(&existing, "Panel")
    );
    for _ in 0..4 {
        black_box(legacy_unique_control_id(&existing, "Panel"));
        black_box(optimized_unique_control_id(&existing, "Panel"));
    }
    ...
    for pair in 0..SAMPLE_PAIRS {
        if pair % 2 == 0 {
            legacy_samples.push(sample_ns(|| legacy_unique_control_id(&existing, "Panel")));
    ...
    assert!(
        ...
        "expected hash-indexed tree-wrap control ID P95 to be at least 90% below repeated node scans; legacy={legacy_p95}ns optimized={optimized_p95}ns reduction={reduction:.3}%"
    );
}
