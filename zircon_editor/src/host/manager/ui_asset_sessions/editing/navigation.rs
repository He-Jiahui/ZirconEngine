...
impl EditorManager {
    pub fn undo_ui_asset_editor(&self, instance_id: &ViewInstanceId) -> Result<bool, EditorError> {
        ...
        let external_effect = entry.session.next_undo_external_effect();
        let changed = entry
            .session
            .undo()
        ...
        drop(sessions);
        if changed {
            if let Some(effect) = external_effect.as_ref() {
    ...
    pub fn redo_ui_asset_editor(&self, instance_id: &ViewInstanceId) -> Result<bool, EditorError> {
        ...
        let external_effect = entry.session.next_redo_external_effect();
        let changed = entry
            .session
            .redo()
            .map_err(|error| EditorError::UiAsset(error.to_string()))?;
