...
pub(super) fn host_tab_data(page: &HostPageTabModel, active_page: &crate::MainPageId) -> TabData {
    TabData {
...
pub(super) fn document_tab_data(tab: &DocumentTabModel) -> TabData {
    TabData {
...
pub(super) fn side_expanded(model: &WorkbenchViewModel, slots: &[ActivityDrawerSlot]) -> bool {
    slots
        .iter()
        .filter_map(|slot| model.tool_windows.get(slot))
        .any(|stack| {
