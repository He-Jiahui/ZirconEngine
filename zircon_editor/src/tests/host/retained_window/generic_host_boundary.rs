...
#[test]
fn editor_host_sources_do_not_depend_on_deleted_retained_trees() {
    ...
    let forbidden = deleted_retained_source_forbidden_markers();
    ...
    assert!(
        violations.is_empty(),
        "editor host must not depend on deleted Retained source trees or generated compatibility aliases:\n{}",
        violations.join("\n")
    );
}
...
fn deleted_retained_source_forbidden_markers() -> Vec<String> {
    let former_owner = former_generated_host_owner();
    vec![
        format!("temp/{former_owner}-migration"),
        format!("temp\\{former_owner}-migration"),
        [
        ...
        .concat(),
        [former_owner.as_str(), "_build"].concat(),
        [former_owner.as_str(), "-build"].concat(),
        ["as ", "retained_ui"].concat(),
    ]
}

fn former_generated_host_owner() -> String {
    ["sli", "nt"].concat()
}
...
fn editor_host_source_guard_rejects_hyphenated_generated_build_dependency() {
    let former_owner = former_generated_host_owner();
    let dependency_name = [former_owner.as_str(), "-build"].concat();
    ...
    collect_source_violations(
        &temp_root,
        &deleted_retained_source_forbidden_markers(),
        &mut violations,
    );
    let _ = fs::remove_dir_all(&temp_root);
    ...
    assert!(
        ...
        "source guard should reject generated Retained build dependencies"
    );
}

#[test]
fn editor_host_build_does_not_compile_or_stage_retained_sources() {
    let build_script = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/build.rs"));
    let manifest = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/Cargo.toml"));
    let former_owner = former_generated_host_owner();
    let build_dependency = [former_owner.as_str(), "-build"].concat();

    for forbidden in [
        "compile_retained_ui",
        "stage_migration_fence_retained_sources",
        "MIGRATION_FENCE_UI_SOURCE_ROOT",
    ] {
        assert!(
            !build_script.contains(forbidden),
            "editor build script must not preserve rejected Retained source compatibility through `{forbidden}`"
        );
    }
    assert!(
        !manifest.contains(&build_dependency),
        "editor manifest should not keep a Retained build dependency after the Rust-owned host cutover"
    );
}

#[test]
fn retained_host_module_exports_rust_owned_contracts_without_generated_modules() {
    let host_module = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/ui/retained_host/mod.rs"
    ));
...
fn rust_owned_host_contract_declares_window_globals_and_projection_data() {
    let window = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/ui/retained_host/host_contract/window.rs"
    ));
    ...
    let host_components = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/ui/retained_host/host_contract/data/host_components.rs"
    ));
    let host_root = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/ui/retained_host/host_contract/data/host_root.rs"
    ));
    ...
    assert!(
        ...
        "host root DTOs should own the presentation payload once Retained generation is removed"
    );
}
...
fn generic_host_catalog_uses_shared_runtime_component_names() {
    ...
    let scanned_roots = [
        ...
        manifest_dir.join("src/ui/template_runtime/retained_adapter.rs"),
