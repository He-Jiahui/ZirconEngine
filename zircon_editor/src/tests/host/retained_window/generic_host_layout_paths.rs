...
fn assert_no_retained_sources_under(path: PathBuf) {
    ...
    for entry in entries {
        ...
        if entry_path.is_dir() {
            assert_no_retained_sources_under(entry_path);
            continue;
        }
        assert_ne!(
            entry_path
            ...
            "former generated UI source should remain absent from `{}`",
            entry_path.display()
        );
    }
}
...
fn former_ui_source_path(stem: &str) -> String {
    format!("{stem}.{}", former_generated_ui_extension())
}

#[test]
fn active_editor_ui_tree_contains_no_retained_sources() {
    let root = editor_root();
    for absent in [
        former_ui_source_path("ui/workbench"),
        former_ui_source_path("ui/workbench/host_context"),
    ] {
        assert!(
            !root.join(&absent).exists(),
            "former generated UI source `{absent}` should not exist under zircon_editor/ui"
        );
    }

    assert_no_retained_sources_under(root.join("ui"));
}
