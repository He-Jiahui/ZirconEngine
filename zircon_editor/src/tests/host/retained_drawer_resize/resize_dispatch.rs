...
use crate::ui::retained_host::drawer_resize::apply_resize_to_group;
...
fn apply_resize_to_bottom_group_updates_all_bottom_slots() {
    let _guard = env_lock().lock().unwrap();
    let path = unique_temp_path("zircon_editor_retained_bottom_resize");
    let runtime = editor_runtime_with_config_path(&path);
    let manager = runtime
        .resolve_manager::<EditorManager>(EDITOR_MANAGER_NAME)
        .unwrap();
...
fn apply_resize_to_right_group_updates_all_right_slots() {
    let _guard = env_lock().lock().unwrap();
    let path = unique_temp_path("zircon_editor_retained_right_resize");
    let runtime = editor_runtime_with_config_path(&path);
    let manager = runtime
        .resolve_manager::<EditorManager>(EDITOR_MANAGER_NAME)
        .unwrap();
...
fn apply_resize_to_unknown_group_returns_error() {
    let _guard = env_lock().lock().unwrap();
    let path = unique_temp_path("zircon_editor_retained_unknown_resize");
    let runtime = editor_runtime_with_config_path(&path);
    let manager = runtime
        .resolve_manager::<EditorManager>(EDITOR_MANAGER_NAME)
        .unwrap();
