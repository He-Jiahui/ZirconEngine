...
fn editor_manager_applies_theme_batch_adopt_helper_items_for_imported_sources() {
    ...
    let (config_path, project_root, runtime) = setup_theme_project(
        "zircon_editor_theme_batch_adopt_helper",
        "zircon_editor_theme_batch_adopt_helper_project",
...
fn editor_manager_prunes_selected_theme_compare_duplicates() {
    let _guard = crate::tests::support::env_lock().lock().unwrap();
    let (config_path, project_root, runtime) = setup_theme_project(
        "zircon_editor_theme_compare_prune_helper",
        "zircon_editor_theme_compare_prune_helper_project",
    ...
    let document = crate::tests::support::load_test_ui_asset(&saved)
        .expect("saved compare prune helper ui asset");
...
fn editor_manager_applies_theme_batch_refactor_helper() {
    let _guard = crate::tests::support::env_lock().lock().unwrap();
    let (config_path, project_root, runtime) = setup_theme_project(
        "zircon_editor_theme_batch_refactor",
        "zircon_editor_theme_batch_refactor_project",
        DUPLICATE_THEME_UI_LAYOUT_ASSET,
        DUPLICATE_IMPORTED_THEME_ASSET,
    );
    let manager = runtime
    ...
    let document =
        crate::tests::support::load_test_ui_asset(&saved).expect("saved batch refactored ui asset");
    assert!(document.tokens.is_empty());
    assert!(document.stylesheets.is_empty());
    assert!(
