...
fn chrome_fixture() -> EditorChromeSnapshot {
    EditorChromeSnapshot {
        ...
        console_output: "Animation ready".into(),
    ...
    crate::ui::layouts::windows::workbench_host_window::PaneData {
        ...
        native_body: crate::ui::layouts::windows::workbench_host_window::PaneNativeBodyData {
            ...
            animation: AnimationEditorPaneViewData {
                ...
                track_items: crate::ui::retained_host::primitives::ModelRc::default(),
                parameter_items: crate::ui::retained_host::primitives::ModelRc::default(),
                node_items: crate::ui::retained_host::primitives::ModelRc::default(),
                state_items: crate::ui::retained_host::primitives::ModelRc::default(),
                transition_items: crate::ui::retained_host::primitives::ModelRc::default(),
            },
        },
...
#[test]
fn animation_sequence_template_body_projects_hybrid_timeline_slot_for_retained_conversion() {
    ...
    let nodes = (0..projected.nodes.row_count())
        ...
        .collect::<Vec<_>>();
    assert!(
        !nodes.iter().any(|node| node.control_id == "ScrubTimeline"),
        "the timeline pane must not expose a fixed-frame scrub action before preview transport exists"
    );
    assert!(
        nodes
            .iter()
...
#[test]
fn animation_graph_template_body_projects_hybrid_canvas_slot_for_retained_conversion() {
    ...
    assert!(
        !nodes.iter().any(|node| node.control_id == "AddNode"),
        "the graph pane must not expose the unsupported placeholder node action"
    );
}
