...
#[test]
fn inspector_template_body_projection_replaces_legacy_inspector_view_data_for_retained_conversion()
{
    ...
    assert!(!nodes.iter().any(|node| node.control_id == "ApplyDraft"));
    assert!(!nodes
        .iter()
