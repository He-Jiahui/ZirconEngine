...
fn builtin_host_page_activation_dispatches_activate_main_page_from_template_binding() {
    ...
    let harness = EventRuntimeHarness::new("zircon_retained_template_bridge_host_page");
...
fn builtin_host_page_activation_matches_legacy_layout_command_dispatch() {
    ...
    let legacy_harness = EventRuntimeHarness::new("zircon_retained_parity_host_page_legacy");
    ...
    let builtin_harness = EventRuntimeHarness::new("zircon_retained_parity_host_page_builtin");
    let bridge = BuiltinHostWindowTemplateBridge::new(UiSize::new(1280.0, 720.0)).unwrap();
    let builtin_effects =
