...
fn core_editor_event_stops_owning_ui_binding_and_projection_surfaces() {
    ...
    let core_runtime_inner_source =
        std::fs::read_to_string(core_runtime_root.join("editor_event_runtime_state.rs"))
    ...
    assert!(
        core_runtime_mod_source.contains("mod editor_event_runtime_state;"),
        "expected core::editor_event::runtime wiring to own editor_event_runtime_state directly"
    );
    for forbidden in [
    ...
    for forbidden in [
        ...
        "EditorState",
        "fn new(",
        "crate::ui::host::editor_event_runtime_state::EditorEventRuntimeState",
    ] {
        assert!(
    ...
    for required in [
        ...
        assert!(
            ...
            "expected core/editor_event/runtime/editor_event_runtime_state.rs to own `{required}`"
        );
    }
    for required in [
    ...
    assert!(
        !ui_host_root.join("editor_event_runtime_state.rs").exists(),
        "expected ui::host/editor_event_runtime_state.rs to be deleted after core runtime inner cutover"
    );
    for required in [
        "mod dispatch;",
        "mod execution_outcome;",
