...
impl EditorCommand {
    pub(crate) fn create_node(scene: &mut Scene, kind: NodeKind) -> Result<Self, String> {
    ...
    pub(crate) fn delete_node(scene: &mut Scene, node_id: NodeId) -> Result<Self, String> {
    ...
    pub(crate) fn rename_node(
        ...
        Ok(UpdateNodeCommand::capture_name(scene, node_id, name)?.map(Self::UpdateNode))
    }
    ...
    pub(crate) fn set_parent(
        ...
        Ok(UpdateNodeCommand::capture_parent(scene, node_id, parent)?.map(Self::UpdateNode))
    }
    ...
    pub(crate) fn set_transform(
        ...
        Ok(UpdateNodeCommand::capture_transform(scene, node_id, after)?.map(Self::UpdateNode))
    }
    ...
    pub(crate) fn apply(&self, scene: &mut Scene) -> Result<(), String> {
        match self {
    ...
    pub(crate) fn undo(&self, scene: &mut Scene) -> Result<(), String> {
        match self {
...
impl CreateNodeCommand {
    fn spawn_node(scene: &mut Scene, kind: NodeKind) -> Result<Self, String> {
        let previous_selected = scene.selected_node();
    ...
    fn apply(&self, scene: &mut Scene) -> Result<(), String> {
        ...
        scene.set_selected(Some(self.record.id));
        Ok(())
    }

    fn undo(&self, scene: &mut Scene) -> Result<(), String> {
        if !scene.remove_entity(self.record.id) {
            return Err(format!("cannot remove missing node {}", self.record.id));
        }
        scene.set_selected(self.previous_selected);
        Ok(())
    }
...
impl DeleteNodeCommand {
    fn capture(scene: &mut Scene, node_id: NodeId) -> Result<Self, String> {
        ...
        scene.set_selected(selection_after);
    ...
    fn apply(&self, scene: &mut Scene) -> Result<(), String> {
        ...
        scene.set_selected(self.selection_after);
        Ok(())
    }
...
impl UpdateNodeCommand {
    ...
    fn capture_name(
        ...
        after.name = name;
        Self::capture(scene, node_id, after)
    }
