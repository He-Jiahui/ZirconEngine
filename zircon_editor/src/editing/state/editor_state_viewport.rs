...
impl EditorState {
    ...
    pub fn frame_selection(&mut self) -> bool {
        let selection = self.world.try_with_world(|scene| scene.selected_node());
        let Some(Some(node_id)) = selection else {
            return false;
        };
    ...
    pub fn handle_viewport_input(&mut self, input: ViewportInput) -> ViewportFeedback {
        let selected_before = self.world.try_with_world(|scene| scene.selected_node());
        ...
        if !was_handle_drag && is_handle_drag {
            ...
            let _ = self.world.try_with_world(|scene| history.begin_drag(scene));
        }
        ...
        let selected_after = self.world.try_with_world(|scene| scene.selected_node());
