...
impl SceneViewportController {
    pub(in crate::editing::viewport::controller) fn apply_orbit(
        &mut self,
        previous: Vec2,
        current: Vec2,
    ) -> bool {
    ...
    pub(in crate::editing::viewport::controller) fn apply_pan(
        &mut self,
        previous: Vec2,
        current: Vec2,
    ) -> bool {
    ...
    pub(in crate::editing::viewport::controller) fn apply_zoom(&mut self, delta: f32) -> bool {
        let Some(camera) = self.state.camera.as_mut() else {
            return false;
        };
