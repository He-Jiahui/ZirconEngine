...
impl SceneViewportController {
    pub(in crate::editing::viewport::controller) fn handle_overlays(
        &self,
        scene: &Scene,
        camera: &ViewportCameraSnapshot,
    ) -> Vec<HandleOverlayExtract> {
        self.handles
            .build_overlays(scene, &self.state.settings, camera)
    }

    pub(in crate::editing::viewport::controller) fn begin_handle_drag(
        &mut self,
        scene: &Scene,
        cursor: Vec2,
        axis: GizmoAxis,
        ...
        let Some(session) =
            self.handles
                .begin_drag(scene, &self.state.settings, &camera, cursor, axis)
        else {
            return false;
        };
    ...
    pub(in crate::editing::viewport::controller) fn end_handle_drag(&mut self) {
