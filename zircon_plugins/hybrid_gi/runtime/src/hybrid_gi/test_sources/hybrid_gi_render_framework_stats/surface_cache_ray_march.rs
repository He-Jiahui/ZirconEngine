...
#[test]
#[ignore]
fn export_hybrid_gi_surface_cache_ray_march_wgpu_png() {
    ...
    let frame = server
        ...
        .unwrap()
        .expect("surface-cache ray-march Wgpu product frame capture should be available");
    let metrics = frame_metrics(&frame);
    assert!(
        metrics.visible_pixels > 0 && metrics.max_luma > 8.0,
        "expected nonblank surface-cache ray-march Wgpu frame; metrics={metrics:?}"
    );

    let output_dir = render_test_output_dir();
    fs::create_dir_all(&output_dir).unwrap();
    write_png(output_dir.join(SURFACE_CACHE_RAY_MARCH_WGPU_PNG), &frame);
    fs::write(
        output_dir.join(SURFACE_CACHE_RAY_MARCH_WGPU_REPORT),
        format!(
            ...
            SURFACE_CACHE_RAY_MARCH_WGPU_PNG,
            frame.width,
            frame.height,
            frame.generation,
