...
fn surface_cache_ray_step_coord(
    ...
    let direction = tile_sample_id & 3u;
    var offset = vec2<i32>(step, 0);
    if (direction == 1u) {
        offset = vec2<i32>(0, step);
    } else if (direction == 2u) {
        offset = vec2<i32>(-step, 0);
    } else if (direction == 3u) {
        offset = vec2<i32>(0, -step);
    }
    return clamp_surface_cache_coord(base_coord + offset);
}
...
fn surface_cache_texel_sample(coord: vec2<i32>) -> SurfaceCacheTexelSample {
    ...
    return valid_surface_cache_texel_sample(apply_scene_light_seed(vec3<u32>(
...
fn surface_cache_tile_sample(
    ...
    let max_steps = max(1u, min(4u, ray_count / 4u));
    var weighted_rgb = first_sample.rgb * (max_steps + 1u);
    var total_weight = max_steps + 1u;

    // Until the full HZB/screen-depth path exists, the trace tile walks the
    // surface-cache atlas in the tile's ray direction and rejects depth jumps.
    for (var step_index = 1u; step_index < max_steps; step_index = step_index + 1u) {
        let step_coord = surface_cache_ray_step_coord(coord, tile_sample_id, step_index);
        let step_sample = surface_cache_texel_sample(step_coord);
        if (step_sample.valid == 0u) {
            continue;
        }

        let depth_delta = abs_diff_u32(step_sample.depth_q, first_sample.depth_q);
        let thickness_q = 16u + step_index * 8u + min(ray_count, 64u) / 8u;
        if (depth_delta > thickness_q) {
            continue;
        }

        let step_weight = max_steps + 1u - step_index;
