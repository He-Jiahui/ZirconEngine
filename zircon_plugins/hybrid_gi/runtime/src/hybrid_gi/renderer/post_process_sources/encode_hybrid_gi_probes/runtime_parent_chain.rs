...
use super::super::constants::MAX_HYBRID_GI_TRACE_REGIONS;
...
pub(super) fn gather_runtime_parent_chain_rgb<F>(
    frame: &HybridGiProbeEncodeFrame,
    probe_id: u32,
...
pub(in super::super) fn frame_has_runtime_scene_truth(frame: &HybridGiProbeEncodeFrame) -> bool {
...
pub(super) fn runtime_parent_topology_is_authoritative(frame: &HybridGiProbeEncodeFrame) -> bool {
    frame
...
#[cfg(test)]
pub(super) fn frame_has_scheduled_trace_region_payload(frame: &HybridGiProbeEncodeFrame) -> bool {
...
pub(super) fn frame_has_scheduled_trace_region_source(frame: &HybridGiProbeEncodeFrame) -> bool {
    if frame.hybrid_gi_resolve_runtime.is_some() {
...
fn current_extract_trace_region_source_ids(frame: &HybridGiProbeEncodeFrame) -> BTreeSet<u32> {
    fallback_trace_region_sources_by_id(frame.extract.lighting.hybrid_global_illumination.as_ref())
        .into_keys()
        .collect()
}

pub(in super::super) fn scheduled_live_trace_region_ids(
    frame: &HybridGiProbeEncodeFrame,
) -> Vec<u32> {
    if frame.hybrid_gi_resolve_runtime.is_some() {
        return Vec::new();
...
fn frame_hybrid_gi_extract_is_enabled(frame: &HybridGiProbeEncodeFrame) -> bool {
    frame
        .extract
        .lighting
        .hybrid_global_illumination
...
fn current_extract_probe_parent_probes(frame: &HybridGiProbeEncodeFrame) -> BTreeMap<u32, u32> {
    frame
        .extract
        .lighting
        .hybrid_global_illumination
...
mod tests {
    ...
    fn frame_with_flat_runtime_and_legacy_scheduled_trace_region() -> HybridGiProbeEncodeFrame {
        ...
        HybridGiProbeEncodeFrame::from_extract(extract, UVec2::new(32, 32))
            .with_hybrid_gi_prepare(Some(crate::hybrid_gi::types::HybridGiPrepareFrame {
    ...
    fn frame_with_runtime_trace_region_scene_data() -> HybridGiProbeEncodeFrame {
        let trace_region_id = 40;
        let snapshot = RenderSceneSnapshot {
            scene: RenderSceneGeometryExtract {
                camera: ViewportCameraSnapshot::default(),
    ...
    fn frame_with_budgeted_scene_representation_and_legacy_scheduled_trace_region(
        ...
        HybridGiProbeEncodeFrame::from_extract(extract, UVec2::new(32, 32)).with_hybrid_gi_prepare(
            Some(crate::hybrid_gi::types::HybridGiPrepareFrame {
                resident_probes: Vec::new(),
                pending_updates: Vec::new(),
                scheduled_trace_region_ids: vec![trace_region_id],
                evictable_probe_ids: Vec::new(),
