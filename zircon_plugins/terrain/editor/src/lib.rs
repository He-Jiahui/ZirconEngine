mod authoring;
mod capability;
mod extension_ids;
mod plugin;

pub use authoring::{
    plan_terrain_import, terrain_import_output_kind, validate_heightfield_import,
    TerrainHeightfieldImportRequest, TerrainImportKind, TerrainImportPlan,
};
pub use capability::{CAPABILITY, EDITOR_CAPABILITIES, PLUGIN_ID};
pub use extension_ids::{TERRAIN_AUTHORING_VIEW_ID, TERRAIN_DRAWER_ID, TERRAIN_TEMPLATE_ID};
pub use plugin::{
    editor_capabilities, editor_plugin, editor_plugin_descriptor, package_manifest,
    plugin_registration, TerrainEditorPlugin,
};

#[cfg(test)]
mod tests;
