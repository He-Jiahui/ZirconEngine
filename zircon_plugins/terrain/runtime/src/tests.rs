...
#[test]
fn terrain_package_manifest_declares_dist_contract() {
    ...
    let distribution = manifest
        .distribution
        .as_ref()
        .expect("terrain distribution manifest");
    ...
    assert_eq!(distribution.dist_crate, TERRAIN_DIST_CRATE_NAME);
    assert_eq!(
        distribution.descriptor_symbol,
        "zircon_native_plugin_descriptor_v3"
    );
    assert_eq!(distribution.runtime_entry, TERRAIN_DIST_RUNTIME_ENTRY);

    let native_module = manifest
        .modules
        .iter()
        .find(|module| module.name == "terrain.dist")
        .expect("terrain native dist module");
    assert_eq!(
        native_module.kind,
        zircon_runtime::plugin::PluginModuleKind::Native
    );
    assert_eq!(native_module.crate_name, TERRAIN_DIST_CRATE_NAME);
    assert_eq!(
