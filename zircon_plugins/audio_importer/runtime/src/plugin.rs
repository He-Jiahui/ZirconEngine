...
const AUDIO_IMPORTER_DIST_ENGINE_COMPAT: &str = ">=0.1, <0.2";
...
pub fn dist_module_manifest() -> PluginModuleManifest {
    PluginModuleManifest::native("audio_importer.dist", AUDIO_IMPORTER_DIST_CRATE_NAME)
        .with_target_modes(supported_targets())
        .with_capabilities(runtime_capabilities().iter().copied())
}
...
fn package_manifest_from_descriptor(descriptor: &RuntimePluginDescriptor) -> PluginPackageManifest {
    ...
    manifest = manifest.with_distribution(PluginDistributionManifest {
        ...
        engine_compat: AUDIO_IMPORTER_DIST_ENGINE_COMPAT.to_string(),
        dist_crate: AUDIO_IMPORTER_DIST_CRATE_NAME.to_string(),
        descriptor_symbol: NATIVE_DESCRIPTOR_SYMBOL_V3.to_string(),
        runtime_entry: AUDIO_IMPORTER_DIST_RUNTIME_ENTRY.to_string(),
