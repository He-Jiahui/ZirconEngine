...
const PLUGIN_ID: &[u8] = b"texture\0";
const RUNTIME_ENTRY: &[u8] = b"zircon_plugin_texture_runtime_entry_v3\0";
const REQUESTED_CAPABILITIES: &[u8] = b"runtime.plugin.texture\0";
const NEGOTIATED_CAPABILITIES: &[u8] = b"runtime.plugin.texture\0";
const RUNTIME_DIAGNOSTICS: &[u8] =
    b"texture dist entry ready; texture processing services remain hosted by the runtime module\0";
const MISSING_HOST_DIAGNOSTICS: &[u8] =
    b"texture dist entry requires runtime.plugin.texture host capability\0";
const EMPTY_MANIFEST: &[u8] = b"\0";
const RUNTIME_REGISTRATION_MANIFEST: &[u8] = b"schema = \"zircon.native.registration-manifest/3\"\ncapabilities = [\"runtime.plugin.texture\"]\n[[modules]]\nname = \"runtime\"\nkind = \"runtime\"\n[[extensions]]\npoint = \"runtime.texture.processing\"\ncontribution = \"plugin.texture.runtime\"\nschema = \"zircon.runtime.texture-processing/1\"\n\0";
...
zircon_plugin_sdk::native_dist_runtime_plugin_v3! {
    ...
    runtime_entry: zircon_plugin_texture_runtime_entry_v3,
    runtime_entry_name: RUNTIME_ENTRY,
    requested_capabilities: REQUESTED_CAPABILITIES,
    missing_host_diagnostics: MISSING_HOST_DIAGNOSTICS,
    runtime: {
        required_capabilities: ["runtime.plugin.texture"],
        denied_capabilities: [],
...
mod tests {
    ...
    #[test]
    fn texture_dist_descriptor_exports_runtime_entry() {
    ...
    #[test]
    fn texture_dist_runtime_entry_reports_registration_manifest() {
        let granted = b"runtime.plugin.texture\0";
        ...
        let report = zircon_plugin_texture_runtime_entry_v3(&host);
