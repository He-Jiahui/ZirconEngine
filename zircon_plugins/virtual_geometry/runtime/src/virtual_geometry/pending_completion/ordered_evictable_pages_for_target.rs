...
impl VirtualGeometryRuntimeState {
    pub(in crate::virtual_geometry) fn page_or_lineage_is_hot_in(
        &self,
        page_id: u32,
        frontier_hot_pages: &BTreeSet<u32>,
    ) -> bool {
        if frontier_hot_pages.contains(&page_id) {
            return true;
        }
    ...
    pub(in crate::virtual_geometry) fn ordered_evictable_pages_for_target(
        &self,
        target_page_id: u32,
        evictable_pages: &[u32],
    ) -> Vec<u32> {
    ...
    pub(in crate::virtual_geometry) fn page_or_lineage_is_hot(&self, page_id: u32) -> bool {
        let frontier_hot_pages = self.frontier_hot_resident_pages();
        self.page_or_lineage_is_hot_in(page_id, &frontier_hot_pages)
    }

    pub(in crate::virtual_geometry) fn frontier_hot_resident_pages(&self) -> BTreeSet<u32> {
