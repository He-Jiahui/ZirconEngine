...
const PLUGIN_ID: &[u8] = b"asset_importer.data\0";
const RUNTIME_ENTRY: &[u8] = b"zircon_plugin_asset_importer_data_runtime_entry_v3\0";
const REQUESTED_CAPABILITIES: &[u8] = b"runtime.plugin.asset_importer.data\nruntime.asset.importer.data.toml\nruntime.asset.importer.data.json\nruntime.asset.importer.data.yaml\nruntime.asset.importer.data.xml\0";
const NEGOTIATED_CAPABILITIES: &[u8] = REQUESTED_CAPABILITIES;
const RUNTIME_DIAGNOSTICS: &[u8] =
    b"data asset importer dist entry ready; importers remain hosted by the runtime module\0";
const MISSING_HOST_DIAGNOSTICS: &[u8] =
    b"data asset importer dist entry requires runtime.plugin.asset_importer.data host capability\0";
const EMPTY_MANIFEST: &[u8] = b"\0";
const RUNTIME_REGISTRATION_MANIFEST: &[u8] = b"schema = \"zircon.native.registration-manifest/3\"\ncapabilities = [\"runtime.plugin.asset_importer.data\", \"runtime.asset.importer.data.toml\", \"runtime.asset.importer.data.json\", \"runtime.asset.importer.data.yaml\", \"runtime.asset.importer.data.xml\"]\n[[modules]]\nname = \"runtime\"\nkind = \"runtime\"\n[[extensions]]\npoint = \"runtime.asset.importer.data\"\ncontribution = \"plugin.asset_importer.data.runtime\"\nschema = \"zircon.runtime.asset-importer.data/1\"\n\0";
...
zircon_plugin_sdk::native_dist_runtime_plugin_v3! {
    ...
    runtime_entry: zircon_plugin_asset_importer_data_runtime_entry_v3,
    runtime_entry_name: RUNTIME_ENTRY,
    requested_capabilities: REQUESTED_CAPABILITIES,
    missing_host_diagnostics: MISSING_HOST_DIAGNOSTICS,
    runtime: {
        required_capabilities: ["runtime.plugin.asset_importer.data"],
        denied_capabilities: [],
...
mod tests {
    ...
    #[test]
    fn data_asset_importer_dist_descriptor_exports_runtime_entry() {
    ...
    #[test]
    fn data_asset_importer_dist_runtime_entry_reports_registration_manifest() {
        let granted = b"runtime.plugin.asset_importer.data\0";
        ...
        let report = zircon_plugin_asset_importer_data_runtime_entry_v3(&host);
