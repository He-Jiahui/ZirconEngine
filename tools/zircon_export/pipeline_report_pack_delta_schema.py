...
def pack_report_delta_publication_diagnostics(
    ...
    if isinstance(delta_pack, str) and delta_pack:
        if delta_manifest is None:
            return ["pack report delta_pack is present but delta_manifest is missing"]
        return []
    if isinstance(delta_manifest, dict):
        return ["pack report delta_manifest is present but delta_pack is missing"]
    return []
...
def pack_report_delta_asset_set_diagnostics(
    ...
    if (
        ...
        and all(isinstance(asset, str) for asset in removed_assets)
    ...
    if (
        ...
        and all(isinstance(asset, str) for asset in report_removed_assets)
    ...
    if (
        ...
        and all(isinstance(asset, str) for asset in report_reused_assets)
...
def delta_changed_asset_entries(changed_assets: list[Any]) -> list[dict[str, Any]] | None:
    ...
    for asset in changed_assets:
        if (
            ...
            or not is_byte_hash(asset.get("chunk_hash"))
