...
class PluginValidateFeatureProviderExtensionMetadataSchemaTests(
    unittest.TestCase
):
    def test_plugin_validate_rejects_generated_feature_provider_enabled_default_type(
        self,
    ) -> None:
        ...
        self.assertIn(
            ...
            "feature_extensions[0].enabled_by_default must be a bool",
            diagnostics,
        )

    def test_plugin_validate_rejects_generated_feature_provider_default_packaging_schema_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir) / "repo"
            ...
            self.assertIn(
                'plugin sound_timeline_animation_track generated '
                'feature_extensions[0].default_packaging[0] '
                '"nightly_dynamic" is unsupported; expected one of '
                "source_template, library_embed, native_dynamic",
                diagnostics,
            )
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "feature_extensions[0].default_packaging[1] "
                "nightly_dynamic duplicates default_packaging[0]",
                diagnostics,
            )
