...
class PipelineReportSourceTemplateBuildValidationTests(unittest.TestCase):
    ...
    def test_report_rejects_passed_source_template_build_validation_non_cargo_build_command(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            out = Path(temp_dir) / "out"
            ...
            self.assertTrue(
                any(
                    "SourceTemplate build_validation command must run cargo build"
                    in diagnostic
                    for diagnostic in report["diagnostics"]
                ),
                report["diagnostics"],
