...
class PluginValidateFeatureProviderExtensionSchemaTests(unittest.TestCase):
    def test_plugin_validate_rejects_generated_feature_provider_extension_fields(
        self,
    ) -> None:
        ...
        self.assertIn(
            ...
            "feature_extensions[0].preview_label is not a known feature "
            "provider extension field",
            diagnostics,
        )

    def test_plugin_validate_rejects_generated_feature_provider_extension_schema_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir) / "repo"
            ...
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "feature_extensions[0].owner_plugin_id Sound__ should contain "
                "only lowercase ASCII letters, digits, and underscores",
                diagnostics,
            )
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "feature_extensions[0].id Sound..Timeline should not contain "
                "empty namespace segments",
                diagnostics,
            )
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "feature_extensions[0].id Sound..Timeline should contain only "
