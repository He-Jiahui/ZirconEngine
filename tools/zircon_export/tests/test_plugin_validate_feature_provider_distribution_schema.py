...
class PluginValidateFeatureProviderDistributionSchemaTests(unittest.TestCase):
    def test_plugin_validate_rejects_generated_feature_provider_distribution_fields(
        self,
    ) -> None:
        ...
        self.assertIn(
            ...
            "distribution.preview_channel is not a known feature provider "
            "distribution field",
            diagnostics,
        )

    def test_plugin_validate_rejects_generated_feature_provider_distribution_schema_drift(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir) / "repo"
            ...
            validate_plugin_feature_provider_package_projection(
                ...
                package_manifest_text="\n".join(
                    [
                        ...
                        'engine_compat = ">=0.1, <0.2"',
                        'dist_crate = "zircon_plugin_sound_timeline_animation_dist"',
                        'descriptor_symbol = "wrong_descriptor_symbol"',
                        'runtime_entry = "zircon_plugin_sound_timeline_animation_runtime_entry_v3"',
                        "",
                        "[[feature_extensions]]",
                        'id = "sound.timeline_animation_track"',
            ...
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "distribution.abi_version must be 3",
                diagnostics,
            )
            self.assertIn(
                "plugin sound_timeline_animation_track generated "
                "distribution.descriptor_symbol must equal "
