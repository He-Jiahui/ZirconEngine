...
test("runtime contracts accept persisted producer shapes", () => {
  ...
  snapshot.validation.validationCopies = [{ job_id: "j", session_id: "s", job_root: "r", source_root: "s", target_root: "t", head_commit: "h", manifest: ["Cargo.toml", "zircon_runtime/src"], status: "planned", created_at: "now", removed_at: null }];
...
test("runtime contracts reject missing IDs, invalid enums, and malformed arrays in every control domain", () => {
  const cases: Array<(snapshot: ReturnType<typeof validSnapshot>) => void> = [
    ...
    (snapshot) => { snapshot.collaboration.patches = [{ patch_id: 1, session_id: "s", patch_object_hash: "h", targets: ["a"], base_hashes: {}, base_objects: {}, current_objects: null, status: "unknown", error_text: null, created_at: "now", updated_at: "now", applied_at: null }]; },
    ...
    (snapshot) => { snapshot.validation.validationCopies = [{ job_id: "j", session_id: "s", job_root: "r", source_root: "s", target_root: "t", head_commit: "h", manifest: ["Cargo.toml"], status: "invalid", created_at: "now", removed_at: null }]; },
