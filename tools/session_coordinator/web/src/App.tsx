  ...
  return <Box sx={{ minHeight: "100vh" }}>
    ...
    <Box component="main" id="main-content" tabIndex={-1} sx={{ ml: { md: `${drawerWidth}px` }, pt: "92px", px: { xs: 2, md: 3 }, pb: 4 }}>
      <Stack spacing={2} sx={{ maxWidth: 1800, mx: "auto" }}>
        ...
        {state.snapshot && <CurrentPage route={route} snapshot={state.snapshot} />}
      </Stack>
    </Box>
  </Box>;
}

function CurrentPage({ route, snapshot }: { route: RouteKey; snapshot: NonNullable<ReturnType<typeof useControlStore>["snapshot"]> }) {
  switch (route) {
