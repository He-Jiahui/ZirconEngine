    ...
    (0..LAYER_COUNT)
        .map(|layer| Layer {
            descriptor: Some(Descriptor {
                format: format!("rgba8unorm-srgb-array-layer-{layer:08}"),
                usage: (0..8).collect(),
                asset_usage: (0..4).collect(),
                metadata: vec![(layer & 0xff) as u8; 64],
                mip_count: 5,
            }),
        })
        .collect()
...
fn legacy(layers: Vec<Layer>) -> Measurement {
    let started = start_counting();
    let first_descriptor = black_box(layers[0].descriptor.as_ref().unwrap().clone());
    let mut checksum = descriptor_checksum(&first_descriptor);
    for layer in black_box(&layers) {
        let descriptor = black_box(layer.descriptor.as_ref().unwrap().clone());
        checksum = checksum.wrapping_add(descriptor_checksum(&descriptor));
    }
    finish_counting(started, black_box(checksum))
}

fn optimized(mut layers: Vec<Layer>) -> Measurement {
    let started = start_counting();
    let first_descriptor = black_box(layers[0].descriptor.take().unwrap());
    let mut checksum = descriptor_checksum(&first_descriptor);
    for layer in black_box(&mut layers).iter_mut().skip(1) {
        let descriptor = black_box(layer.descriptor.take().unwrap());
        checksum = checksum.wrapping_add(descriptor_checksum(&descriptor));
    }
...
fn main() {
    ...
    assert_eq!(legacy_allocations, LAYER_COUNT * 4 + 4);
