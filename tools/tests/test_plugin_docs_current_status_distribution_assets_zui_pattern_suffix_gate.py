...
class PluginDocsCurrentStatusDistributionAssetsZuiPatternSuffixGateTests(
    unittest.TestCase
):
    def test_current_status_records_distribution_assets_zui_pattern_suffix_gate(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_distribution_assets_zui_pattern_suffix_gate"
        )
        ...
        export_tool_status = export_tool_text[
            export_tool_text.index("`plugin_validate_distribution_assets.py`") :
        ]
        structure_text = (
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record distribution.assets .zui "
                "pattern suffix gate:\n"
                + "\n".join(failures)
            )
