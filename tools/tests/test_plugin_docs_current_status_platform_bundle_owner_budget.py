...
class PluginDocsCurrentStatusPlatformBundleOwnerBudgetTests(unittest.TestCase):
    def test_current_status_records_platform_bundle_owner_budget_recovered(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_09_e1_current_status_platform_bundle_owner_budget_recovered"
        ...
        required_phrases = [
            status_id,
            "tools/tests/plugin_docs_current_status_platform_bundle_support.py",
            ...
            "tools/tests/test_plugin_docs_current_status_platform_bundle_strategy_owner_splits.py=83",
            "test_platform_bundle_status_tests_live_in_focused_owners",
            "test_platform_bundle_status_owners_stay_under_line_budgets",
            ...
            "focused owner/docs 11/11",
            "current-status docs 290/290",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PlatformBundle owner budget recovery:\n"
                + "\n".join(failures)
            )
