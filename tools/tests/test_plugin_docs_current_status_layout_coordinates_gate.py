...
class PluginDocsCurrentStatusLayoutCoordinatesGateTests(unittest.TestCase):
    def test_current_status_records_layout_coordinates_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_layout_coordinates_gate"
        ...
        required_phrases = [
            ...
            "plugin_validate_layout_coordinates.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record package layout coordinate validation gate:\n"
                + "\n".join(failures)
            )
