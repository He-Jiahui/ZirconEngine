...
class PluginDocsCurrentStatusNativeDynamicOperationAuditSummaryTests(
    unittest.TestCase
):
    def test_current_docs_reflect_operation_audit_summary_schema_owner_split(
        self,
    ) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        slug = (
            "plugins_13_m5_t1_native_dynamic_operation_audit_summary_schema_"
            "owner_split"
        )
        owner_file = "pipeline_report_native_dynamic_operation_audit_summary_schema.py"
        phrase = "NativeDynamic operation-audit summary schema owner"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not reflect NativeDynamic "
                "operation-audit summary schema owner split:\n"
                + "\n".join(failures)
            )
