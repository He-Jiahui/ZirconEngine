...
class PluginDocsCurrentStatusCapabilityStatusesGateTests(unittest.TestCase):
    def test_current_status_records_capability_statuses_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_capability_statuses_gate"
        ...
        required_phrases = [
            status_id,
            "plugin_validate_capability_statuses.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record capability status validation gate:\n"
                + "\n".join(failures)
            )
