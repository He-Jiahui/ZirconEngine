...
class PluginDocsCurrentStatusPluginValidateTestOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_test_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_test_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=4519",
            "tools/tests/test_plugin_validate_test_owner_boundaries.py=313",
            "test_recent_test_owner_boundaries_leave_general_owner_file",
            "test_all_target_tests_live_in_all_target_test_owner",
            "python -m unittest tools.tests.test_plugin_validate_test_owner_boundaries",
            "7/7",
            "py_compile",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate test-owner "
                "boundary file split:\n" + "\n".join(failures)
            )
