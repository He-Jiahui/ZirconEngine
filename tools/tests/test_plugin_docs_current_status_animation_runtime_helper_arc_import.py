...
class PluginDocsCurrentStatusAnimationRuntimeHelperArcImportTests(
    unittest.TestCase
):
    def test_current_status_records_animation_runtime_helper_arc_import(
        self,
    ) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_animation_runtime_helper_arc_import"
        ...
        animation_text = (
            repo_root / "docs/assets-and-rendering/runtime-physics-animation-assets.md"
        ).read_text(encoding="utf-8")
        structure_text = (
        ...
        sections = {
            ...
            "animation runtime docs": animation_text,
        ...
        required_phrases = [
            ...
            "EntityPath",
            "cargo check --manifest-path zircon_plugins\\Cargo.toml -p zircon_plugin_animation_runtime --locked --all-targets",
            "test_animation_runtime_helpers_arc_import",
            "workspace build",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record animation runtime helper Arc import support:\n"
                + "\n".join(failures)
            )
