...
class PluginDocsCurrentStatusValidateSchemaTestOwnerSplitTests(unittest.TestCase):
    def test_current_status_records_validate_schema_test_owner_split(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_09_e1_validate_schema_compile_host_plan_test_owner_split"
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_pipeline_report_validate_schema.py=731",
            "tools/zircon_export/tests/test_pipeline_report_validate_compile_host_plan_schema.py=237",
            "tools/tests/test_zircon_export_validate_schema_test_owner_boundaries.py=79",
            "test_compile_host_plan_schema_tests_live_in_dedicated_owner",
            "test_validate_schema_test_owners_stay_under_line_budgets",
            ...
            "focused owner/tests 22/22",
            "current-status docs 308/308",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record Validate schema test owner split:\n"
                + "\n".join(failures)
            )
