...
class PluginDocsCurrentStatusAssetImporterRequiredCapabilityDeclaredGateTests(
    unittest.TestCase
):
    def test_current_status_records_required_capability_declared_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_required_capability_declared_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_asset_importer_required_capability_gates.py",
            "required_capabilities",
            "declared static package/feature capability",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer required capability declared gate:\n"
                + "\n".join(failures)
            )
