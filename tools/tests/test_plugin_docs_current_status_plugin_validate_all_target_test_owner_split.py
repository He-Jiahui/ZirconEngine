...
class PluginDocsCurrentStatusPluginValidateAllTargetTestOwnerSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_all_target_test_owner_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_all_target_test_owner_split"
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_plugin_validate.py=14",
            "tools/zircon_export/tests/test_plugin_validate_all_targets.py=349",
            "test_all_target_tests_live_in_all_target_test_owner",
            "test_plugin_validate_all_reports_malformed_root_distribution",
            "python -m unittest tools.zircon_export.tests.test_plugin_validate_all_targets",
            "4/4",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate all-target "
                "test owner split:\n" + "\n".join(failures)
            )
