...
class PluginDocsCurrentStatusPackDeltaAssetSetTests(unittest.TestCase):
    def test_current_docs_reflect_pack_delta_asset_set_owner_split(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        slug = "plugins_13_m5_t1_pack_delta_asset_set_semantics_owner_split"
        owner_file = "pipeline_report_pack_delta_asset_set_semantics.py"
        phrase = "Pack delta asset-set semantics owner"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not reflect Pack delta asset-set "
                "semantics owner split:\n"
                + "\n".join(failures)
            )
