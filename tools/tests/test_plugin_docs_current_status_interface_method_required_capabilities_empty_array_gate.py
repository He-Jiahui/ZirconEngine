...
class PluginDocsCurrentStatusInterfaceMethodRequiredCapabilitiesEmptyArrayGateTests(
    unittest.TestCase
):
    def test_current_status_records_interface_method_required_capabilities_empty_array_gate(
        self,
    ) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_interface_method_required_"
            "capabilities_empty_array_gate"
        )
        ...
        required_phrases = [
            ...
            "test_plugin_validate_rejects_empty_interface_method_required_capabilities",
        ...
        if failures:
            self.fail(
                ...
                "required_capabilities empty array gate:\n"
                + "\n".join(failures)
            )
