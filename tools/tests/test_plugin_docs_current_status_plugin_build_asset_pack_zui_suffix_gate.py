...
class PluginDocsCurrentStatusPluginBuildAssetPackZuiSuffixGateTests(unittest.TestCase):
    def test_current_status_records_plugin_build_asset_pack_zui_suffix_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_build_asset_pack_zui_suffix_gate"
        )
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record plugin build asset-pack .zui suffix gate:\n"
                + "\n".join(failures)
            )
