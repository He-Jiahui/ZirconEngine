...
class PluginDocsCurrentStatusManifestSchemaFeatureExtensionDependencySemanticsGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_feature_extension_dependency_semantics_guard(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_feature_extension_dependency_semantics_guard"
        )
        ...
        if failures:
            self.fail(
                ...
                "extension dependency semantics guard:\n" + "\n".join(failures)
            )
