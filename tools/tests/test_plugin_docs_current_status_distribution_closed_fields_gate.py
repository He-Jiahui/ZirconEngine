...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_distribution_closed_fields_gate"
...
class PluginDocsCurrentStatusDistributionClosedFieldsGateTests(unittest.TestCase):
    def test_current_status_docs_record_distribution_closed_fields_gate(self):
        ...
        required_phrases = [
            STATUS_ID,
            "plugin_validate_distribution_contract.py",
            ...
            "editor_entry",
            "assets",
            "test_plugin_validate_rejects_unknown_distribution_fields",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record distribution closed-fields gate:\n"
                + "\n".join(failures)
            )
