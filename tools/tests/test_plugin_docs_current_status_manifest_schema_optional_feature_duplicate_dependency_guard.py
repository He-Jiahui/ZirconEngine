...
class PluginDocsCurrentStatusManifestSchemaOptionalFeatureDuplicateDependencyGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_optional_feature_duplicate_dependency_guard(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_optional_feature_duplicate_dependency_guard"
        )
        ...
        if failures:
            self.fail(
                ...
                "feature duplicate dependency guard:\n" + "\n".join(failures)
            )
