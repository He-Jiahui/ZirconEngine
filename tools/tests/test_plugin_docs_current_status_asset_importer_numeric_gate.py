...
class PluginDocsCurrentStatusAssetImporterNumericGateTests(unittest.TestCase):
    def test_current_status_records_asset_importer_numeric_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_numeric_range_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_asset_importer_numbers.py",
            "priority",
            "importer_version",
            "must fit i32",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer numeric gate:\n"
                + "\n".join(failures)
            )
