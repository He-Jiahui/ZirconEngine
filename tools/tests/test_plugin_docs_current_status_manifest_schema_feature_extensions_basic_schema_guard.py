...
class PluginDocsCurrentStatusManifestSchemaFeatureExtensionsBasicSchemaGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_feature_extensions_basic_schema_guard(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_feature_extensions_basic_schema_guard"
        )
        ...
        required_phrases = [
            ...
            "feature_extensions must not be empty when declared",
            "feature_extensions[0] must be a table",
            "missing feature_extensions[0].owner_plugin_id",
