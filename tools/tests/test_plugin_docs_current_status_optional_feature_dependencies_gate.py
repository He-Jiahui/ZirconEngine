...
class PluginDocsCurrentStatusOptionalFeatureDependenciesGateTests(unittest.TestCase):
    def test_current_status_records_optional_feature_dependencies_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_optional_feature_dependencies_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record optional feature dependencies gate:\n"
                + "\n".join(failures)
            )
