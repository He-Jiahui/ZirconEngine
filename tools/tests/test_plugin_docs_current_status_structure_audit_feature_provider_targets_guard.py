...
STATUS_ID = "plugins_13_m5_t1_structure_audit_feature_provider_target_identity_guard"
...
class PluginDocsCurrentStatusStructureAuditFeatureProviderTargetsGuardTests(
    unittest.TestCase
):
    def test_current_status_records_feature_provider_target_identity_guard(self):
        ...
        required_phrases = [
            STATUS_ID,
            "tools/plugin_structure_audits/manifest_schema_feature_provider_targets.py",
            "tools/plugin_structure_audits/manifest_schema.py",
            "collect_feature_provider_target_identity_violations",
            "feature_provider_package_id",
            "plugin validate target",
            "root 与 feature-provider target id",
            "test_plugin_structure_audit_manifest_schema_feature_provider_targets.py",
            "test_manifest_schema_rejects_duplicate_feature_provider_target_ids",
            "test_manifest_schema_rejects_feature_provider_target_colliding_with_root_id",
        ...
        if failures:
            self.fail(
                ...
                "feature-provider target identity guard:\n" + "\n".join(failures)
            )
