...
class PluginDocsCurrentStatusAssetImportersZuiSuffixGateTests(unittest.TestCase):
    def test_current_status_records_asset_importers_zui_suffix_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_asset_importers_zui_suffix_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset_importers .zui "
                "suffix gate:\n"
                + "\n".join(failures)
            )
