...
class PluginDocsCurrentStatusPluginValidateManifestOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_manifest_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_manifest_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=2047",
            "tools/tests/test_plugin_validate_manifest_owner_boundaries.py=358",
            "test_manifest_boundaries_leave_general_owner_file",
            "test_manifest_shape_lives_in_manifest_shape_owner",
            "test_package_kind_lives_in_package_kind_owner",
            "python -m unittest tools.tests.test_plugin_validate_manifest_owner_boundaries",
            "7/7",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "manifest owner boundary file split:\n"
                + "\n".join(failures)
            )
