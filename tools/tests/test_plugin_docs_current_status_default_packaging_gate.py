...
class PluginDocsCurrentStatusDefaultPackagingGateTests(unittest.TestCase):
    def test_current_status_records_default_packaging_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_default_packaging_metadata_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record default_packaging metadata gate:\n"
                + "\n".join(failures)
            )
