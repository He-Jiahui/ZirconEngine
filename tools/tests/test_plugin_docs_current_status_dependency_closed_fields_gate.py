...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_dependency_closed_fields_gate"
...
class PluginValidateDependencyClosedFieldsStatusTests(unittest.TestCase):
    def test_current_status_docs_record_dependency_closed_fields_gate(self):
        required_phrases = [
            STATUS_ID,
            "validate_plugin_dependency_known_fields",
            "PLUGIN_VALIDATE_DEPENDENCY_FIELDS",
            "test_plugin_validate_rejects_unknown_dependency_fields",
        ...
        self.assertFalse(
            missing,
            "PluginValidate dependency closed-fields status is incomplete:\n"
            + "\n".join(missing),
        )
