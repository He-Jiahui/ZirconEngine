...
class PluginDocsCurrentStatusAssetImporterStringArraysGateTests(unittest.TestCase):
    def test_current_status_records_asset_importer_string_arrays_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_string_arrays_gate"
        )
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer string-array "
                "schema gate:\n"
                + "\n".join(failures)
            )
