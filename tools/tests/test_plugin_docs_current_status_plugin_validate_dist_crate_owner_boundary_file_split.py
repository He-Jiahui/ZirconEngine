...
class PluginDocsCurrentStatusPluginValidateDistCrateOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_dist_crate_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_dist_crate_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=3793",
            "tools/tests/test_plugin_validate_dist_crate_owner_boundaries.py=200",
            "test_dist_crate_boundaries_leave_general_owner_file",
            "test_dist_crate_sdk_dependency_lives_in_dependency_owner",
            "python -m unittest tools.tests.test_plugin_validate_dist_crate_owner_boundaries",
            "5/5",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "dist-crate owner boundary file split:\n"
                + "\n".join(failures)
            )
