...
class PluginDocsCurrentStatusAssetImporterRequiredCapabilityNamespaceGateTests(
    unittest.TestCase
):
    def test_current_status_records_required_capability_namespace_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_required_capability_namespace_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_asset_importer_required_capabilities.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer required capability namespace gate:\n"
                + "\n".join(failures)
            )
