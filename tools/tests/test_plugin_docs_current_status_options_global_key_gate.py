...
class PluginDocsCurrentStatusOptionsGlobalKeyGateTests(unittest.TestCase):
    def test_current_status_records_options_global_key_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_options_global_key_gate"
        ...
        required_phrases = [
            status_id,
            "plugin_validate_option_global_keys.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record options global key gate:\n"
                + "\n".join(failures)
            )
