...
class PluginDocsCurrentStatusPluginValidateLayoutOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_layout_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_layout_owner_boundary_file_split"
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=2331",
            "tools/tests/test_plugin_validate_layout_owner_boundaries.py=321",
            "test_layout_boundaries_leave_general_owner_file",
            "test_layout_coordinates_live_in_layout_coordinates_owner",
            "test_layout_roots_live_in_layout_roots_owner",
            "python -m unittest tools.tests.test_plugin_validate_layout_owner_boundaries",
            "6/6",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "layout owner boundary file split:\n"
                + "\n".join(failures)
            )
