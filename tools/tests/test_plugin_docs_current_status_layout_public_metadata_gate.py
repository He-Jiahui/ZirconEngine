...
class PluginDocsCurrentStatusLayoutPublicMetadataGateTests(unittest.TestCase):
    def test_current_status_records_layout_public_metadata_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_layout_public_metadata_gate"
        ...
        required_phrases = [
            status_id,
            "plugin_validate_layout.py",
            "plugin_validate_layout_public_metadata.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record package layout public metadata validation gate:\n"
                + "\n".join(failures)
            )
