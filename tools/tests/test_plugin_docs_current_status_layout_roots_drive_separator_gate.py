...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_layout_roots_drive_separator_gate"
...
class PluginDocsCurrentStatusLayoutRootsDriveSeparatorGateTests(unittest.TestCase):
    def test_current_status_records_layout_roots_drive_separator_gate(self):
        ...
        required_phrases = [
            STATUS_ID,
            "plugin_validate_layout_roots.py",
            "validate_plugin_layout_root_path",
            "plugin_validate_layout_root_has_drive_separator",
            "asset_roots",
            "content_roots",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record package layout roots "
                "drive-separator gate:\n" + "\n".join(failures)
            )
