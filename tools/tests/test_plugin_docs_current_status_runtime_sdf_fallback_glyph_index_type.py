...
class PluginDocsCurrentStatusRuntimeSdfFallbackGlyphIndexTypeTests(
    unittest.TestCase
):
    def test_current_status_records_runtime_sdf_fallback_glyph_index_type(
        self,
    ) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_runtime_sdf_fallback_glyph_index_type"
        ...
        runtime_ui_text = (
            repo_root / "docs/assets-and-rendering/runtime-ui-graphics-integration.md"
        ).read_text(encoding="utf-8")
        structure_text = (
        ...
        sections = {
            ...
            "runtime UI graphics docs": runtime_ui_text,
        ...
        required_phrases = [
            ...
            "test_runtime_sdf_fallback_glyph_index_type",
            "Rust 单测超时未采信",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record runtime SDF fallback glyph-index type support:\n"
                + "\n".join(failures)
            )
