...
class PluginDocsCurrentStatusPluginValidateEntryReportOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_entry_report_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_entry_report_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=3138",
            "tools/tests/test_plugin_validate_entry_report_owner_boundaries.py=217",
            "test_entry_report_boundaries_leave_general_owner_file",
            "test_single_target_validation_lives_in_single_target_owner",
            "test_report_rendering_lives_in_report_owner",
            "python -m unittest tools.tests.test_plugin_validate_entry_report_owner_boundaries",
            "6/6",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "entry/report owner boundary file split:\n"
                + "\n".join(failures)
            )
