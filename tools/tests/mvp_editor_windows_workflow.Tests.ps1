...
Assert-WorkflowMatch 'tooling15-integrated-bootstrap\.Tests\.ps1' 'Control-plane batch must cover Coordinator-managed validation fixture lifecycle.'
Assert-WorkflowMatch 'mvp-project-open-evidence\.Tests\.ps1' 'Control-plane batch must cover bounded project-open diagnostic parsing.'
Assert-WorkflowMatch 'mvp-project-save-evidence\.Tests\.ps1' 'Control-plane batch must cover bounded project-save diagnostic parsing.'
Assert-WorkflowMatch 'mvp-process-timing-evidence\.Tests\.ps1' 'Control-plane batch must cover indexed F5 process timing validation.'
Assert-WorkflowMatch 'mvp-scene-persistence-evidence\.Tests\.ps1' 'Control-plane batch must cover linear scene parent validation and bounded delta comparison.'
Assert-WorkflowMatch 'mvp-persistence-comparison\.Tests\.ps1' 'Control-plane batch must cover no-follow persistence comparison publication.'
...
Assert-WorkflowMatch '\$controlPlaneResult\.FailedCount -ne 0 -or \$controlPlaneResult\.TotalCount -ne 415' 'Control-plane batch must reject failed or incomplete Pester execution.'
