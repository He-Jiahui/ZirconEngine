...
class PluginDocsCurrentStatusPayloadStringArrayHelperTests(unittest.TestCase):
    def test_current_docs_reflect_payload_string_array_helper_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        slug = (
            "plugins_13_m5_t1_native_dynamic_payload_string_array_helper_owner_split"
        )
        owner_file = (
            "pipeline_report_native_dynamic_payload_string_array_schema_helpers.py"
        )
        phrase = "object-array string-array helper owner"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not reflect NativeDynamic payload string-array helper owner split:\n"
                + "\n".join(failures)
            )
