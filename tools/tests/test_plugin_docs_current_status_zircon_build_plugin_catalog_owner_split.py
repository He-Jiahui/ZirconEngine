...
class PluginDocsCurrentStatusZirconBuildPluginCatalogOwnerSplitTests(
    unittest.TestCase
):
    def test_current_status_records_zircon_build_plugin_catalog_owner_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_zircon_build_plugin_catalog_owner_split"
        ...
        required_phrases = [
            status_id,
            "zircon_build_plugin_packages.py",
            "zircon_build_plugin_selection.py",
            "zircon_build_plugin_workspace_crates.py",
            "CargoPackage",
            "PluginPackage",
            "discover_plugin_workspace_crates",
            "select_plugins",
            "filter_plugins_by_carrier",
            "test_plugin_catalog_models_selection_and_workspace_crates_have_owners",
            "test_plugin_catalog_owners_preserve_package_workspace_and_selection_semantics",
            "test_zircon_build_classifies_forms_from_manifest",
        ]
        ...
        self.assertFalse(
            failures,
            "Current status docs do not record the zircon_build plugin catalog "
            "owner split:\n"
            + "\n".join(failures),
        )
