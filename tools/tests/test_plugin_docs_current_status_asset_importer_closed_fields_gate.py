...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_asset_importer_closed_fields_gate"
...
class PluginDocsCurrentStatusAssetImporterClosedFieldsGateTests(unittest.TestCase):
    def test_current_status_docs_record_asset_importer_closed_fields_gate(self):
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset_importer closed-fields gate:\n"
                + "\n".join(failures)
            )
