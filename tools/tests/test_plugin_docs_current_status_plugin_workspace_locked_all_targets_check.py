...
class PluginDocsCurrentStatusWorkspaceLockedCheckTests(unittest.TestCase):
    def test_current_status_records_plugin_workspace_locked_all_targets_check(
        self,
    ) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_workspace_locked_all_targets_check_passed"
        ...
        required_phrases = [
            ...
            "zui-only-clean",
            "test_plugin_docs_current_status_plugin_workspace_locked_all_targets_check",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record workspace locked all-targets check:\n"
                + "\n".join(failures)
            )
