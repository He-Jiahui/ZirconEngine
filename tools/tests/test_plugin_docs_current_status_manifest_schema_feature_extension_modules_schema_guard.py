...
class PluginDocsCurrentStatusManifestSchemaFeatureExtensionModulesSchemaGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_feature_extension_modules_schema_guard(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_feature_extension_modules_schema_guard"
        )
        ...
        required_phrases = [
            ...
            "feature_extensions[0].modules must not be empty when declared",
        ...
        if failures:
            self.fail(
                ...
                "extension modules schema guard:\n" + "\n".join(failures)
            )
