...
class PluginDocsCurrentStatusPluginValidateDependenciesTestOwnerSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_dependencies_test_owner_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_dependencies_test_owner_split"
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_plugin_validate.py=349",
            "tools/zircon_export/tests/test_plugin_validate_dependencies.py=591",
            "test_dependency_tests_live_in_dependency_test_owner",
            "test_plugin_validate_rejects_malformed_dependencies",
            "python -m unittest tools.zircon_export.tests.test_plugin_validate_dependencies",
            "11/11",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate dependencies "
                "test owner split:\n" + "\n".join(failures)
            )
