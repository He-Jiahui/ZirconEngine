...
STATUS_ID = "editor_ui_11_m5_plugin_workspace_zui_only_locked_all_targets_test_compile_passed"
REQUIRED_STATUS_PHRASES = [
    ...
    "ExecutableLines=123",
    "tools/tests/test_zui_docs_plugin_workspace_test_compile_check.py",
...
class ZuiDocsPluginWorkspaceTestCompileCheckTests(unittest.TestCase):
    def test_plugin_workspace_zui_only_test_compile_status_is_recorded(
        self,
    ) -> None:
        failures: list[str] = []
        ...
        self.assertFalse(
            failures,
            "Plugin workspace .zui-only test-compile status is missing:\n"
            + "\n".join(failures),
        )
