...
class PluginDocsCurrentStatusNativeDynamicPackageReportPayloadFilesTests(
    unittest.TestCase
):
    def test_current_docs_reflect_package_report_payload_files_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        slug = (
            "plugins_13_m5_t1_native_dynamic_package_report_payload_files_schema_"
            "owner_split"
        )
        owner_file = (
            "pipeline_report_native_dynamic_package_report_payload_files_schema.py"
        )
        phrase = "NativeDynamic package report payload.files schema owner"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not reflect NativeDynamic package "
                "report payload.files schema owner split:\n"
                + "\n".join(failures)
            )
