...
class PluginDocsCurrentStatusZirconBuildPluginDistributionAssetsZuiDocumentGateTests(
    unittest.TestCase
):
    def test_current_status_records_zircon_build_plugin_distribution_assets_zui_gate(
        self,
    ):
        ...
        required_phrases = [
            ...
            "test_zircon_build_rejects_distribution_assets_zui_document_kind_drift",
            "test_plugin_asset_roots_live_in_plugin_assets_owner",
            "distribution.assets",
            "matched .zui asset",
            "asset.kind",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record zircon_build plugin "
                "distribution.assets .zui document validation gate:\n"
                + "\n".join(failures)
            )
