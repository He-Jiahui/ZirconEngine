...
class PluginDocsCurrentStatusManifestShapeGateTests(unittest.TestCase):
    def test_current_status_records_manifest_shape_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_manifest_shape_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record manifest shape gate:\n"
                + "\n".join(failures)
            )
