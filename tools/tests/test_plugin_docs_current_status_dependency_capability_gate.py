...
class PluginDocsCurrentStatusDependencyCapabilityGateTests(unittest.TestCase):
    def test_current_status_records_dependency_capability_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_dependency_capability_resolution_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_dependency_capabilities.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record dependency capability gate:\n"
                + "\n".join(failures)
            )
