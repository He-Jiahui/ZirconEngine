...
class PluginDocsCurrentStatusManifestSchemaOptionalFeaturePrimaryTargetGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_optional_feature_primary_target_guard(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_optional_feature_primary_target_guard"
        )
