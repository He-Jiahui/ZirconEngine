...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_feature_provider_dependency_closed_fields_gate"
...
class PluginDocsCurrentStatusFeatureProviderDependencyClosedFieldsGateTests(
    unittest.TestCase
):
    def test_current_status_docs_record_feature_provider_dependency_closed_fields_gate(
        self,
    ):
        ...
        required_phrases = [
            ...
            "feature-provider",
            "generated feature_extensions[0].dependencies",
            "plugin_id",
            "capability",
            "primary",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record feature-provider dependency "
                "closed-fields gate:\n" + "\n".join(failures)
            )
