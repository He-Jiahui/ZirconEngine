...
class PluginDocsCurrentStatusSourceTemplateCompileHostOwnerBudgetTests(
    unittest.TestCase
):
    def test_current_status_records_source_template_compile_host_owner_budget_recovered(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_09_e1_current_status_source_template_compile_host_owner_budget_recovered"
        )
        ...
        required_phrases = [
            ...
            "tools/tests/test_plugin_docs_current_status_compile_host_owner_splits.py=186",
            "test_source_template_compile_host_status_tests_live_in_focused_owners",
            "test_source_template_compile_host_status_owners_stay_under_line_budgets",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record SourceTemplate/CompileHost "
