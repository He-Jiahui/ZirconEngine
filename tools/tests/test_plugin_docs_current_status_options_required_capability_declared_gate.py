...
class PluginDocsCurrentStatusOptionsRequiredCapabilityDeclaredGateTests(
    unittest.TestCase
):
    def test_current_status_records_options_required_capability_declared_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_options_required_capability_declared_gate"
        )
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record options required capability declared gate:\n"
                + "\n".join(failures)
            )
