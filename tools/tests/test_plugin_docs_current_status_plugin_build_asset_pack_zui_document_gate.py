...
class PluginDocsCurrentStatusPluginBuildAssetPackZuiDocumentGateTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_build_asset_pack_zui_document_gate(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_build_asset_pack_zui_document_gate"
        ...
        export_tool_status = export_tool_text[
            export_tool_text.index("`plugin_build_asset_pack.py`") :
        ]
        structure_text = (
        ...
        required_phrases = [
            status_id,
            "plugin_build_asset_pack.py",
            "plugin_validate_distribution_zui_assets.py",
            "validate_plugin_distribution_zui_asset",
            "test_plugin_build_rejects_asset_pack_with_zui_kind_drift",
            "distribution.assets",
            "matched .zui asset",
            "asset.kind",
            "component, style, theme_tokens, view",
            "asset pack command must not run",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record plugin build asset-pack "
                ".zui document validation gate:\n"
                + "\n".join(failures)
            )
