...
class PluginDocsCurrentStatusPluginValidateFeatureProviderOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_feature_provider_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_feature_provider_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=3941",
            "tools/tests/test_plugin_validate_feature_provider_owner_boundaries.py=694",
            "test_feature_provider_boundaries_leave_general_owner_file",
            "test_feature_provider_extension_lives_in_extension_owner",
            "python -m unittest tools.tests.test_plugin_validate_feature_provider_owner_boundaries",
            "11/11",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "feature-provider owner boundary file split:\n"
                + "\n".join(failures)
            )
