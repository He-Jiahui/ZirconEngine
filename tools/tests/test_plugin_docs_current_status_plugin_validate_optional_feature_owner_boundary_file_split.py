...
class PluginDocsCurrentStatusPluginValidateOptionalFeatureOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_optional_feature_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_optional_feature_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=621",
            "tools/tests/test_plugin_validate_optional_feature_owner_boundaries.py=543",
            "test_optional_feature_boundaries_leave_general_owner_file",
            ...
            "python -m unittest tools.tests.test_plugin_validate_optional_feature_owner_boundaries",
            "6/6",
            "py_compile",
            "below 1000-line structure budget",
        ]
        ...
        if failures:
            self.fail(
                ...
                "optional-feature owner boundary file split:\n"
                + "\n".join(failures)
            )
