...
class PluginDocsCurrentStatusAssetImporterGlobalIdGateTests(unittest.TestCase):
    def test_current_status_records_asset_importer_global_id_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_global_id_uniqueness_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_asset_importer_global_ids.py",
            "plugin validate --all",
            "is duplicated by",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer global id gate:\n"
                + "\n".join(failures)
            )
