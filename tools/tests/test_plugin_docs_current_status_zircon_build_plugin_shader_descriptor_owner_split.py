...
class PluginDocsCurrentStatusZirconBuildPluginShaderDescriptorOwnerSplitTests(
    unittest.TestCase
):
    def test_current_status_records_zircon_build_plugin_shader_descriptor_owner_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_zircon_build_plugin_shader_descriptor_owner_split"
        )
        ...
        required_phrases = [
            ...
            "geometry_sources",
            "shading_models",
            "test_plugin_shader_descriptors_live_in_plugin_shader_descriptor_owner",
        ...
        self.assertFalse(
            failures,
            "Current status docs do not record the zircon_build plugin shader "
            "descriptor owner split:\n"
            + "\n".join(failures),
        )
