...
class PluginDocsCurrentStatusPackageKindGateTests(unittest.TestCase):
    def test_current_status_records_package_kind_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_package_kind_gate"
        ...
        required_phrases = [
            status_id,
            "plugin_validate_package_kind.py",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record package_kind gate:\n"
                + "\n".join(failures)
            )
