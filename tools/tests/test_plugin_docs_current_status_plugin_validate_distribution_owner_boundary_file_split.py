...
class PluginDocsCurrentStatusPluginValidateDistributionOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_distribution_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_distribution_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=3300",
            "tools/tests/test_plugin_validate_distribution_owner_boundaries.py=574",
            "test_distribution_boundaries_leave_general_owner_file",
            "test_distribution_descriptor_symbol_lives_in_descriptor_owner",
            "test_distribution_scalar_fields_live_in_scalars_owner",
            "python -m unittest tools.tests.test_plugin_validate_distribution_owner_boundaries",
            "10/10",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "distribution owner boundary file split:\n"
                + "\n".join(failures)
            )
