...
class PluginDocsCurrentStatusOptionalFeatureDistributionGateTests(unittest.TestCase):
    def test_current_status_records_optional_feature_distribution_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_optional_feature_distribution_schema_gate"
        )
        ...
        required_phrases = [
            ...
            "plugin_validate_optional_feature_distribution.py",
            "validate_plugin_optional_feature_distribution",
            "validate_plugin_distribution",
            "distribution_label",
            "plugin_validate_distribution_packaging.py",
            "test_plugin_validate_rejects_optional_feature_distribution_contract",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record optional feature distribution "
                "schema gate:\n" + "\n".join(failures)
            )
