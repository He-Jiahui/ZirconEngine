...
STATUS_ID = "plugins_13_m5_t1_manifest_schema_layout_roots_drive_separator_guard"
...
class PluginDocsCurrentStatusManifestSchemaLayoutRootsDriveSeparatorGuardTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_layout_roots_drive_separator_guard(
        self,
    ):
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record manifest schema layout roots "
                "drive-separator guard:\n" + "\n".join(failures)
            )
