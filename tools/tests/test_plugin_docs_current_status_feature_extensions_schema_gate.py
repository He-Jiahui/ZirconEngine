...
class PluginDocsCurrentStatusFeatureExtensionsSchemaGateTests(unittest.TestCase):
    def test_current_status_records_feature_extensions_schema_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_feature_extensions_schema_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record feature extension schema gate:\n"
                + "\n".join(failures)
            )
