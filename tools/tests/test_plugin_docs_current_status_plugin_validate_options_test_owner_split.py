...
class PluginDocsCurrentStatusPluginValidateOptionsTestOwnerSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_options_test_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_options_test_owner_split"
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_plugin_validate.py=1624",
            "tools/zircon_export/tests/test_plugin_validate_options.py=269",
            "test_option_tests_live_in_option_test_owner",
            "test_plugin_validate_rejects_malformed_options_schema",
            "python -m unittest tools.zircon_export.tests.test_plugin_validate_options",
            "4/4",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate options "
                "test owner split:\n" + "\n".join(failures)
            )
