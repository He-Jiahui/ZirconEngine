...
CATALOG = ROOT / "zircon_hub/src/learn/catalog.rs"
...
class Hub06LearnCatalogTopKPerformanceContract(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.source = CATALOG.read_text(encoding="utf-8")

    def test_catalog_routes_ranking_through_bounded_helper(self) -> None:
        body = rust_function_body(self.source, "discover_learn_catalog_for_scope")
    ...
    def test_large_catalog_uses_linear_partial_selection(self) -> None:
        ...
        self.assertIn("entries.select_nth_unstable_by(LEARN_CATALOG_LIMIT", body)
        self.assertIn("entries.truncate(LEARN_CATALOG_LIMIT)", body)
        self.assertIn("entries.sort_by(ranked_learn_order)", body)

    def test_partial_selection_only_runs_above_the_limit(self) -> None:
        body = rust_function_body(self.source, "retain_top_ranked_entries")
        self.assertIn("entries.len() > LEARN_CATALOG_LIMIT", body)

    def test_total_order_is_shared_by_selection_and_final_sort(self) -> None:
        body = rust_function_body(self.source, "ranked_learn_order")
        for marker in (
            "source_priority",
            "root_rank",
            "entry.source",
    ...
    def test_release_evidence_benchmarks_the_real_topk_helper(self) -> None:
        self.assertIn("hub06_learn_catalog_topk_release_benchmark_evidence", self.source)
        self.assertIn("HUB06_LEARN_CATALOG_TOPK_BENCH_V1", self.source)
