...
class PluginDocsCurrentStatusCapabilityStatusTargetsGateTests(unittest.TestCase):
    def test_current_status_records_capability_status_targets_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_capability_status_targets_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record capability status target validation gate:\n"
                + "\n".join(failures)
            )
