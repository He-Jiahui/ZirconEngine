...
class PluginDocsCurrentStatusCapabilityStatusOptionalArraysGateTests(
    unittest.TestCase
):
    def test_current_status_records_capability_status_optional_arrays_gate(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_capability_status_"
            "optional_arrays_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_capability_status_targets.py",
            "plugin_validate_capability_status_references.py",
            "target_modes",
            "bevy_references",
            "optional arrays",
            "must not be empty when declared",
            "test_plugin_validate_rejects_empty_capability_status_optional_arrays",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record capability status optional "
                "arrays gate:\n"
                + "\n".join(failures)
            )
