...
class PluginDocsCurrentStatusPluginValidateOwnerBudgetTests(unittest.TestCase):
    def test_current_status_records_plugin_validate_owner_budget_recovered(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_09_e1_current_status_plugin_validate_owner_budget_recovered"
        ...
        required_phrases = [
            status_id,
            "tools/tests/plugin_docs_current_status_plugin_validate_support.py",
            ...
            "tools/tests/test_plugin_docs_current_status_plugin_validate_entry_status_owner_splits.py=27",
            "test_plugin_validate_status_tests_live_in_focused_owners",
            "test_plugin_validate_status_owners_stay_under_line_budgets",
            ...
            "focused owner/docs 18/18",
            "current-status docs 307/307",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate owner budget recovery:\n"
                + "\n".join(failures)
            )
