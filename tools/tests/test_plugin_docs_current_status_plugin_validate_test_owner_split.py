...
class PluginDocsCurrentStatusPluginValidateTestOwnerSplitTests(unittest.TestCase):
    def test_current_status_records_plugin_validate_test_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_test_owner_split"
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_plugin_validate.py=1876",
            "tools/zircon_export/tests/test_plugin_validate_dist_crate.py=234",
            "tools/zircon_export/tests/test_plugin_validate_asset_importers.py=413",
            "test_dist_crate_tests_live_in_dist_crate_test_owner",
            "test_asset_importer_schema_tests_live_in_asset_importer_test_owner",
            "python -m unittest tools.zircon_export.tests.test_plugin_validate",
            "tools.zircon_export.tests.test_plugin_validate_dist_crate",
            "tools.zircon_export.tests.test_plugin_validate_asset_importers",
            "45/45",
            "py_compile",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record PluginValidate test "
                "owner split:\n" + "\n".join(failures)
            )
