...
class PluginDocsCurrentStatusOptionalFeaturesEmptyArrayGateTests(unittest.TestCase):
    def test_current_status_records_optional_features_empty_array_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_optional_features_empty_array_gate"
        )
        ...
        required_phrases = [
            ...
            "test_plugin_validate_rejects_empty_optional_features_array",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record optional_features "
                "empty-array gate:\n" + "\n".join(failures)
            )
