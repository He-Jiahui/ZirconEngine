...
class PluginDocsCurrentStatusFeatureProviderPackageIdGateTests(unittest.TestCase):
    def test_current_status_records_feature_provider_package_id_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_feature_provider_package_id_schema_gate"
        )
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record feature-provider package id "
                "schema gate:\n" + "\n".join(failures)
            )
