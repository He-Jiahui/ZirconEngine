...
class PluginDocsCurrentStatusPackTrimObjectArrayTests(unittest.TestCase):
    def test_current_docs_reflect_pack_trim_object_array_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        slug = "plugins_13_m5_t1_pack_trim_object_array_schema_owner_split"
        owner_file = "pipeline_report_pack_trim_object_array_schema.py"
        phrase = "Pack trim object-array schema owner"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not reflect Pack trim object-array schema owner split:\n"
                + "\n".join(failures)
            )
