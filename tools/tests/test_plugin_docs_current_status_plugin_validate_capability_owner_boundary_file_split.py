...
class PluginDocsCurrentStatusPluginValidateCapabilityOwnerBoundaryFileSplitTests(
    unittest.TestCase
):
    def test_current_status_records_plugin_validate_capability_owner_boundary_file_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_capability_owner_boundary_file_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/tests/test_plugin_validate_owner_boundaries.py=1423",
            "tools/tests/test_plugin_validate_capability_owner_boundaries.py=538",
            "test_capability_boundaries_leave_general_owner_file",
            ...
            "python -m unittest tools.tests.test_plugin_validate_capability_owner_boundaries",
            "10/10",
            "py_compile",
        ...
        if failures:
            self.fail(
                ...
                "capability owner boundary file split:\n"
                + "\n".join(failures)
            )
