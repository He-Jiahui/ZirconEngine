...
class PluginDocsCurrentStatusAssetImporterSourceExtensionsFormatGateTests(
    unittest.TestCase
):
    def test_current_status_records_asset_importer_source_extensions_format_gate(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_asset_importer_source_extensions_"
            "format_gate"
        )
        ...
        required_phrases = [
            status_id,
            "plugin_validate_asset_importer_schema.py",
            "source_extensions",
            "full_suffixes",
            "lowercase extension without dots",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record asset importer "
                "source_extensions format gate:\n"
                + "\n".join(failures)
            )
