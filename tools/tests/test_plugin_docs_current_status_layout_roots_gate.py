...
class PluginDocsCurrentStatusLayoutRootsGateTests(unittest.TestCase):
    def test_current_status_records_layout_roots_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_plugin_validate_layout_roots_gate"
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record package layout roots validation gate:\n"
                + "\n".join(failures)
            )
