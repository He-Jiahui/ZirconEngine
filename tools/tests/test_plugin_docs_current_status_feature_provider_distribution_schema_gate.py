...
STATUS_ID = "plugins_13_m5_t1_plugin_validate_feature_provider_distribution_schema_gate"
...
class PluginDocsCurrentStatusFeatureProviderDistributionSchemaGateTests(
    unittest.TestCase
):
    def test_current_status_docs_record_feature_provider_distribution_schema_gate(
        self,
    ):
        ...
        required_phrases = [
            STATUS_ID,
            "plugin_validate_feature_provider_distribution_schema.py",
            ...
            "generated distribution.forms must include dist",
            "generated distribution.default_packaging must include native_dynamic",
            "generated distribution.abi_version must be 3",
            "generated distribution.descriptor_symbol must equal zircon_native_plugin_descriptor_v3",
            "test_plugin_validate_rejects_generated_feature_provider_distribution_schema_drift",
            "test_feature_provider_distribution_schema_stays_in_schema_leaf",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record feature-provider distribution "
                "schema gate:\n" + "\n".join(failures)
            )
