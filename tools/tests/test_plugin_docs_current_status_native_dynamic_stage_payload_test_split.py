...
class PluginDocsCurrentStatusNativeDynamicStagePayloadTestSplitTests(
    unittest.TestCase
):
    def test_current_status_records_native_dynamic_stage_payload_test_split(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_native_dynamic_stage_payload_test_owner_split"
        )
        ...
        required_phrases = [
            status_id,
            "tools/zircon_export/tests/test_pipeline_report_native_dynamic_stage_payload.py=513",
            "tools/zircon_export/tests/test_pipeline_report_native_dynamic_stage_loader_manifest.py=535",
            ...
            "test_loader_manifest_tests_have_dedicated_owner",
            "python -m unittest tools.zircon_export.tests.test_pipeline_report_native_dynamic_stage_payload tools.zircon_export.tests.test_pipeline_report_native_dynamic_stage_loader_manifest",
            "20/20",
            "python -m unittest tools.tests.test_zircon_export_native_dynamic_stage_payload_test_owner_boundaries",
            "3/3",
            "py_compile",
        ]
