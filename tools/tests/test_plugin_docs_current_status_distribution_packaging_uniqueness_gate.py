...
class PluginDocsCurrentStatusDistributionPackagingUniquenessGateTests(
    unittest.TestCase
):
    def test_current_status_records_distribution_packaging_uniqueness_gate(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_plugin_validate_distribution_packaging_uniqueness_gate"
        )
        ...
        required_phrases = [
            ...
            "duplicates distribution.forms",
            "duplicates distribution.default_packaging",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record distribution packaging "
                "uniqueness gate:\n" + "\n".join(failures)
            )
