...
STATUS_ID = "plugins_13_m5_t1_structure_audit_global_event_catalog_namespace_guard"
...
class PluginDocsCurrentStatusStructureAuditEventCatalogNamespaceGlobalGuardTests(
    unittest.TestCase
):
    def test_current_status_records_global_event_catalog_namespace_guard(self):
        ...
        required_phrases = [
            STATUS_ID,
            "tools/plugin_structure_audits/manifest_schema_event_catalog_namespaces.py",
            "collect_global_event_catalog_namespace_violations",
            "tools/plugin_structure_audits/manifest_schema.py",
            "plugin validate event_catalog namespace",
            "is duplicated by",
            "event_catalogs[0].namespace",
            "tools/tests/test_plugin_structure_audit_manifest_schema_event_catalog_namespaces.py",
            "test_manifest_schema_rejects_global_event_catalog_namespace_duplicates",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record structure-audit global "
                "event catalog namespace guard:\n" + "\n".join(failures)
            )
