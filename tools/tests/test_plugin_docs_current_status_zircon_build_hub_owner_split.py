...
class PluginDocsCurrentStatusZirconBuildHubOwnerSplitTests(unittest.TestCase):
    def test_current_status_records_zircon_build_hub_owner_split(self):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = "plugins_13_m5_t1_zircon_build_hub_owner_split"
        ...
        required_phrases = [
            ...
            "test_hub_tauri_build_lives_in_hub_owner",
            "test_hub_owner_preserves_staging_semantics",
            "Tauri",
            "installers",
        ]
        ...
        self.assertFalse(
            failures,
            "Current status docs do not record the zircon_build Hub/Tauri "
            "owner split:\n"
            + "\n".join(failures),
        )
