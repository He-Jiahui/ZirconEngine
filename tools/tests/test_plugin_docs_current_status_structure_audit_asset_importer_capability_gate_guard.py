...
STATUS_ID = (
    "plugins_13_m5_t1_structure_audit_asset_importer_required_capability_gate"
)
...
class PluginDocsCurrentStatusStructureAuditAssetImporterCapabilityGateGuardTests(
    unittest.TestCase
):
    def test_current_status_records_asset_importer_required_capability_gate(self):
        ...
        required_phrases = [
            STATUS_ID,
            "tools/plugin_structure_audits/manifest_schema_asset_importer_capability_gates.py",
            ...
            "should reference a declared static package/feature capability or an explicitly host-owned capability",
            "test_manifest_schema_rejects_undeclared_asset_importer_required_capability",
            "test_manifest_schema_accepts_declared_feature_required_capability",
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record structure-audit asset "
                "importer required capability gate:\n" + "\n".join(failures)
            )
