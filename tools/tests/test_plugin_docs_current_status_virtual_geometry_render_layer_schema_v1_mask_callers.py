...
class PluginDocsCurrentStatusVirtualGeometryRenderLayerSchemaV1Tests(unittest.TestCase):
    def test_current_status_records_virtual_geometry_schema_v1_mask_callers(
        self,
    ) -> None:
        ...
        required_phrases = [
            ...
            "test_virtual_geometry_render_layer_schema_v1_mask_callers",
            "cargo check --manifest-path zircon_plugins\\Cargo.toml -p zircon_plugin_virtual_geometry_runtime --locked --all-targets",
        ]
        ...
        if failures:
            self.fail(
                "Current plugin docs do not record Virtual Geometry render-layer schema-v1 mask callers:\n"
                + "\n".join(failures)
            )
