...
class PluginDocsCurrentStatusManifestSchemaOptionalFeaturesEmptyArrayGateTests(
    unittest.TestCase
):
    def test_current_status_records_manifest_schema_optional_features_empty_array_gate(
        self,
    ):
        repo_root = Path(__file__).resolve().parents[2]
        status_id = (
            "plugins_13_m5_t1_manifest_schema_optional_features_empty_array_guard"
        )
        ...
        required_phrases = [
            ...
            "test_plugin_structure_audit_manifest_schema.py",
        ...
        if failures:
            self.fail(
                ...
                "optional_features empty-array guard:\n" + "\n".join(failures)
            )
