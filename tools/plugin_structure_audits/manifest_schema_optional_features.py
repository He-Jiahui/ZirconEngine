...
def collect_optional_features_schema_violations(
    display_path: str,
    ...
    *,
    ...
    if not optional_features:
        violations.append(
            f"{display_path}: optional_features must not be empty when declared"
        )
        return

    seen_optional_feature_ids: dict[str, int] = {}
    for feature_index, feature in enumerate(optional_features):
        ...
        collect_optional_feature_dependencies_schema_violations(
            display_path,
            feature_index,
            manifest,
            feature,
        ...
        collect_optional_feature_modules_schema_violations(
            display_path,
            feature_index,
            feature,
            violations,
...
def collect_optional_feature_schema_violations(
    ...
    for field in sorted(feature):
        if field not in OPTIONAL_FEATURE_FIELDS:
            violations.append(
...
def collect_optional_feature_identity_violations(
    display_path: str,
    feature_index: int,
    manifest: dict[str, Any],
    feature: dict[str, Any],
    ...
    if is_non_empty_trimmed_string(feature_id):
        ...
        if is_non_empty_trimmed_string(package_id):
    ...
    if (
        is_non_empty_trimmed_string(owner_plugin_id)
        and is_non_empty_trimmed_string(package_id)
        and owner_plugin_id != package_id
    ):
        violations.append(
            f"{display_path}: {field_label}.owner_plugin_id {owner_plugin_id} "
            f"should match package id {package_id}"
        )


def collect_optional_feature_dot_namespace_violations(
    display_path: str,
    label: str,
    value: str,
    violations: list[str],
...
def collect_optional_feature_capability_violations(
    display_path: str,
    feature_index: int,
    feature: dict[str, Any],
    violations: list[str],
    ...
    for capability_index, capability in enumerate(capabilities):
        if not is_non_empty_trimmed_string(capability):
            continue
        item_label = f"optional_features[{feature_index}].capabilities[{capability_index}]"
...
def collect_optional_feature_capability_namespace_violations(
    display_path: str,
    label: str,
    value: str,
    violations: list[str],
...
def collect_optional_feature_dependencies_schema_violations(
    display_path: str,
    feature_index: int,
    manifest: dict[str, Any],
    feature: dict[str, Any],
    ...
    if feature_dependencies is None:
        violations.append(
            f"{display_path}: missing optional_features[{feature_index}].dependencies"
        )
    elif not isinstance(feature_dependencies, list):
        violations.append(
            f"{display_path}: optional_features[{feature_index}].dependencies "
...
def collect_optional_feature_dependency_primary_count_violation(
    display_path: str,
    feature_index: int,
    ...
    collect_feature_dependency_primary_count_violation(
        display_path,
        f"optional_features[{feature_index}].dependencies",
        dependencies,
        violations,
    )


def collect_optional_feature_dependency_duplicate_identity_violations(
    display_path: str,
    feature_index: int,
    dependencies: list[object],
    violations: list[str],
...
def collect_optional_feature_dependency_primary_target_violations(
    display_path: str,
    feature_index: int,
    manifest: dict[str, Any],
    dependencies: list[object],
    ...
    primary_target = optional_feature_dependency_primary_target(manifest)
    if primary_target is None:
        return
    package_id, package_capabilities = primary_target
    collect_feature_dependency_primary_target_violations(
        display_path,
        f"optional_features[{feature_index}].dependencies",
        dependencies,
...
def collect_optional_feature_distribution_schema_violations(
    ...
    collect_feature_distribution_schema_violations(
        display_path,
        f"optional_features[{feature_index}].distribution",
        distribution,
        violations,
    )


def collect_optional_feature_modules_schema_violations(
    display_path: str,
    feature_index: int,
    feature: dict[str, Any],
    violations: list[str],
    *,
    ...
    if not isinstance(feature_modules, list):
        violations.append(
            f"{display_path}: optional_features[{feature_index}].modules "
            "must be an array of tables"
        )
        return
    feature_id = feature.get("id")
    ...
    for module_index, module in enumerate(feature_modules):
        module_label = f"optional_features[{feature_index}].modules[{module_index}]"
        collect_module_schema_violations(
            display_path,
            module_label,
            module,
