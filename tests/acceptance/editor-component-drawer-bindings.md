...
- Component drawer template execution is host-mediated: `component_drawer` adapter envelopes may invoke only operations declared in the enabled drawer descriptor, and the existing `EditorOperation` dispatcher remains responsible for capability checks, undo metadata, journal records, and actual mutation.
...
- Execution case: `component_drawer_adapter_invokes_only_enabled_declared_operation_bindings` verifies a declared drawer operation dispatches through the host and an undeclared operation is rejected before invocation.
- Safe event case: `component_drawer_adapter_accepts_safe_action_events_beyond_press` verifies committed drawer controls can invoke declared operations while draft `ValueChanged` events remain rejected.
...
- Passed: `cargo test -p zircon_editor component_drawer_adapter_accepts_safe_action_events_beyond_press --locked --jobs 1 --target-dir target-codex-editor-check --message-format short --color never`.
...
- Safe action execution now reaches the existing `EditorOperation` dispatcher for declared bindings and rejects unsupported draft events before invocation; arbitrary native plugin UI embedding remains intentionally out of scope.
