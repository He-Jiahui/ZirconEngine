...
fn quick_action_documentation_records_tauri_react_cutover() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            "React/MUI scope-derived quick-action DTOs",
            "QuickActions and fallback-capable workflow targets now have separate React/MUI contracts.",
            "`src/tauri_app/view_model.rs` still derives selected/latest/stale/empty QuickAction DTO copy from `HubScope`",
            "Routed QuickActions panels send only `quickActionProjectTargetPayload(project)`",
            "Builds and Cloud main workflow buttons are the fallback-capable surfaces.",
            "Builds and Cloud local-delivery surfaces now share that workflow target rule.",
            "workflowProjectTargetPayload",
            "selected project first and the latest recent project second",
            "enabled quick-action DTO state",
        ],
    ...
    assert_not_contains_any(
        ...
        &responsive_doc,
        &[
            "Quick-action fallback is part of the responsive workflow contract",
            "Builds page project actions are deliberately stricter than Dashboard Quick Actions",
            "Builds callbacks use selected-project-only runtime entrypoints",
            "Dashboard Quick Actions remain the only place where no-selection fallback",
            "without allowing the Dashboard quick-action fallback",
        ],
    );
}
