...
use zircon_framework::render::{
    ...
    RenderFrameExtract, RenderMeshSnapshot, RenderOverlayExtract, RenderSceneGeometryExtract,
...
fn virtual_geometry_transparent_fallback_submission_order_follows_prepare_owned_unified_indirect_when_segments_are_absent(
    ...
    let red_dominant_frame = renderer
        .render_frame_with_pipeline(
            &EditorOrRuntimeFrame::from_extract(extract.clone(), viewport_size).with_virtual_geometry_prepare(Some(
                prepare_frame_without_segments((2, 2), (3, 1)),
            )),
            &compiled,
            None,
        )
    ...
    let blue_dominant_frame = renderer
        .render_frame_with_pipeline(
            &EditorOrRuntimeFrame::from_extract(extract, viewport_size).with_virtual_geometry_prepare(Some(
                prepare_frame_without_segments((2, 1), (3, 2)),
            )),
            &compiled,
            None,
        )
...
fn virtual_geometry_renderer_mesh_draw_submission_order_tracks_visibility_owned_unified_indirect_authority(
    ...
    renderer
        ...
        .unwrap();
    let red_dominant_order = renderer
        .read_last_virtual_geometry_mesh_draw_submission_order();
    ...
    renderer
        ...
        .unwrap();
    let blue_dominant_order = renderer
        .read_last_virtual_geometry_mesh_draw_submission_order();
