...
impl HybridGiRuntimeState {
    pub(in crate::runtime::hybrid_gi) fn assign_scheduled_trace_regions(
        &mut self,
    ...
    pub(in crate::runtime::hybrid_gi) fn refresh_recent_lineage_trace_support(&mut self) {
        let probe_ids = self.probe_scene_data.keys().copied().collect::<Vec<_>>();
        for probe_id in probe_ids {
            let current_q8 = quantize_support_q8(self.current_lineage_trace_support_score(probe_id));
    ...
    pub(in crate::runtime::hybrid_gi) fn effective_lineage_trace_support_score(
        &self,
        probe_id: u32,
    ) -> f32 {
    ...
    fn requested_descendant_support_score(&self, probe_id: u32) -> f32 {
        ...
        while let Some((candidate_probe_id, depth)) = stack.pop() {
            ...
            if self.current_requested_probe_ids.contains(&candidate_probe_id) {
                best_support = best_support.max(
