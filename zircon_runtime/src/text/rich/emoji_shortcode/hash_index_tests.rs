...
use super::{EmojiShortcodeRegistrationError, EmojiShortcodeRegistry};
...
#[test]
fn optimization_batch_20260826by_emoji_shortcode_hash_index_preserves_expansion_contract() {
    let mut registry = EmojiShortcodeRegistry::with_builtins();
    registry
        .register(" Zircon_Code ", "💎")
        .expect("custom shortcode should register");

    assert_eq!(
        registry
            .expand("Build :ZIRCON_CODE: :unknown: :rocket:", 0, usize::MAX)
            .expect("test expansion fits output budget"),
        "Build 💎 :unknown: 🚀"
    );
    assert_eq!(
        registry.register("zircon_code", "✨"),
        Err(EmojiShortcodeRegistrationError::DuplicateName(
            "zircon_code".to_string()
        ))
    );
}

#[test]
fn optimization_batch_20260826by_emoji_shortcode_hash_index_has_no_ordered_iteration() {
    let source = include_str!("../emoji_shortcode.rs");

    assert!(source.contains("use std::collections::HashMap;"));
    assert!(source.contains("replacements: HashMap<String, String>"));
    assert!(!source.contains("BTreeMap"));
    assert!(!source.contains("replacements.iter"));
    assert!(!source.contains("replacements.keys"));
    assert!(!source.contains("replacements.values"));
}

#[test]
#[ignore = "release performance evidence"]
fn optimization_batch_20260826by_emoji_shortcode_hash_index_p95() {
    let names = (0..ENTRY_COUNT)
        .map(|index| format!("project_shared_emoji_shortcode_prefix_{index:04}"))
        .collect::<Vec<_>>();
    let ordered = names
        .iter()
        .cloned()
    ...
    let hashed = ordered
        ...
        .collect::<HashMap<_, _>>();
    let target = names.last().unwrap().as_str();
    ...
    assert!(
        ...
        "emoji shortcode hash lookup P95 must be at least 30% below BTreeMap: ordered={ordered_p95}ns hash={hash_p95}ns"
    );

    println!(
        "RUNTIME84_EMOJI_SHORTCODE_HASH_INDEX_BENCH_V1 entries={ENTRY_COUNT} hits={HIT_COUNT} samples={SAMPLE_COUNT} ordered_p50_ns={ordered_p50} ordered_p95_ns={ordered_p95} hash_p50_ns={hash_p50} hash_p95_ns={hash_p95} ordered_lookups_before={HIT_COUNT} ordered_lookups_after=0 hash_lookups_after={HIT_COUNT} ordered_ns={} hash_ns={}",
