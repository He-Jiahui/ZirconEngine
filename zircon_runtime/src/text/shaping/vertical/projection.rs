...
use crate::text::{
    BackendShapeRequest, FontFaceId, ShapedGlyph, ShapedGlyphRotation, ShapedGlyphRun,
    TextOrientation,
};
...
fn source_clusters(glyphs: &[ShapedGlyph], request: BackendShapeRequest<'_>) -> Vec<SourceCluster> {
...
fn segment_source_range(segment: &[SourceCluster]) -> TextRange {
    TextRange {
        start: segment
            .iter()
