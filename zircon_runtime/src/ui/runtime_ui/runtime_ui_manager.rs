...
use super::input_router::RuntimeUiInputRouter;
...
pub(crate) struct RuntimeUiManager {
    ...
    input_router: RuntimeUiInputRouter,
...
impl RuntimeUiManager {
    pub(crate) fn new(viewport_size: UVec2) -> Self {
        Self {
            ...
            input_router: RuntimeUiInputRouter::default(),
            active_fixture: None,
        }
    }
    ...
    pub(crate) fn load_builtin_fixture(
        ...
        self.input_router.clear_node_handlers();
    ...
    {
        self.input_router.register_pointer(node_id, kind, handler);
    }
    ...
    {
        self.input_router
            .register_pointer_phase(node_id, kind, phase, handler);
    }
    ...
    {
        self.input_router
            .register_navigation(node_id, kind, handler);
    }
    ...
    pub(crate) fn dispatch_pointer_event(
        ...
        let result = self
            .surface
            .dispatch_pointer_event(self.input_router.pointer(), event)?;
    ...
    pub(crate) fn dispatch_navigation_event(
        ...
        let result = self
            .surface
            .dispatch_navigation_event(self.input_router.navigation(), kind)?;
        self.rebuild_dirty_surface()?;
        Ok(result)
    }
    ...
    pub(crate) fn dispatch_input_event(
        ...
        let result = self.surface.dispatch_input_event(
            self.input_router.pointer(),
            self.input_router.navigation(),
            event,
        )?;
        self.rebuild_dirty_surface()?;
        Ok(result)
    ...
    pub(crate) fn dispatch_window_input_pump_event(
        ...
        let result = self.surface.dispatch_window_input_pump_event(
            self.input_router.pointer(),
            self.input_router.navigation(),
            event,
        )?;
