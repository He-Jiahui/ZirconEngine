...
pub(super) fn convert_source_template_fixture_document(
...
fn convert_template_node(
    ...
    let (kind, widget_type, component, slot_name) = if let Some(component_name) = &node.component {
        ...
        return Err(UiAssetError::InvalidDocument {
            asset_id: "source-template-fixture".to_string(),
            detail: format!(
                "source template fixture node {node_id} missing component/template/slot"
            ),
        });
    };
