...
use super::super::layout_parsed_text_without_tables_with_provider;
use super::{
    axes::TableAxes,
    cell_layout::{
    ...
    source_slice::{layout_cell_range_with_provider, layout_range_with_provider, slice_parsed},
};
    ...
    P: TextShapeRunProvider + ?Sized,
{
    ...
    let top_level_tables: Vec<&RichTable> = parsed
        ...
        .tables
        .iter()
        .filter(|table| table.depth == 0 && !table.cells.is_empty())
        .collect();
    ...
    P: TextShapeRunProvider + ?Sized,
{
    ...
    let preferred_cells = preferred_cell_extents(parsed, &grid, table.depth, style, axes, provider);
    ...
    for placed in &grid.cells {
        ...
        let layout = layout_cell_range_with_provider(
            parsed,
            range,
            table.depth,
            style,
    ...
    UiResolvedTextLayout {
        ...
        measured_height,
        source_range: UiTextRange {
            start: table.byte_range.0 as usize,
            end: table.byte_range.1 as usize,
        },
        lines,
        boxes,
