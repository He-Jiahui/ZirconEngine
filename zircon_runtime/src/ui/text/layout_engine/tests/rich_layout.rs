...
fn styled_rich_layout_without_inline_object_uses_rich_run_metrics() {
    ...
    let frame = UiFrame::new(0.0, 0.0, 300.0, 100.0);

    let rich = layout_text("[size=40]Wide[/size]", &rich_style, frame, None);
    let plain = layout_text("Wide", &plain_style, frame, None);

    assert_eq!(rich.lines.len(), 1);
    assert_eq!(rich.lines[0].text, "Wide");
    assert!(
        rich.measured_width > plain.measured_width,
        "styled run width must not fall through the base-style plain wrapper"
    );
    assert!(
        rich.measured_height > plain.measured_height,
        "styled run height must come from the rich materializer"
    );
    assert!(rich.lines[0].glyph_advances.iter().sum::<f32>() > plain.measured_width);
}

#[test]
fn styled_vertical_rich_layout_without_inline_object_uses_rich_run_metrics() {
    let mut rich_style = test_style(UiTextWrap::None, UiTextOverflow::Clip);
    rich_style.rich_text_format = UiRichTextFormat::BbCodeV1;
    rich_style.text_writing_mode = UiTextWritingMode::VerticalRl;
    let mut plain_style = test_style(UiTextWrap::None, UiTextOverflow::Clip);
    plain_style.text_writing_mode = UiTextWritingMode::VerticalRl;
    let frame = UiFrame::new(0.0, 0.0, 300.0, 300.0);
    ...
    assert!(
        rich.measured_width > plain.measured_width,
        "styled VerticalRl text must not fall through the base-style plain wrapper"
    );
    assert!(
        rich.measured_height > plain.measured_height,
        "styled VerticalRl advances must come from the rich materializer"
    );
    let handle = rich
        .rich_text_artifact
        .as_ref()
...
fn html_inline_image_metrics_reach_resolved_ui_layout() {
    ...
    let handle = layout
        .rich_text_artifact
        .as_ref()
        .expect("rich inline layout keeps a composite artifact");
    let artifact = crate::text::resolve_resolved_text_glyph_artifact(handle)
        .expect("rich inline layout keeps its text glyph artifact");
    let glyph_line = artifact.lines[0]
        .as_ref()
        .expect("text around an inline block remains renderable");
    let inline_run = layout.lines[0]
        .runs
        .iter()
        .find(|run| run.text == "\u{fffc}")
        .expect("retained inline run");
    assert!(
        glyph_line.glyphs.iter().all(|glyph| {
            glyph.source_range.end <= inline_run.source_range.start
                || inline_run.source_range.end <= glyph.source_range.start
        }),
        "the compiled inline source range must not become a font glyph"
    );
    let inline_slice = crate::text::resolve_rich_text_glyph_run_artifact(
        handle,
        0,
        inline_run.source_range,
        inline_run.visual_range,
    )
    .expect("inline run owns an explicit artifact receipt");
    assert!(inline_slice.glyph_range.is_empty());
...
#[test]
fn html_inline_only_line_publishes_an_empty_text_slice() {
    let mut style = test_style(UiTextWrap::None, UiTextOverflow::Clip);
    style.rich_text_format = UiRichTextFormat::HtmlSubsetV1;
    let layout = layout_text(
        "<img src=\"res://icons/star.png\" width=\"16\" height=\"24\">",
        &style,
        UiFrame::new(0.0, 0.0, 40.0, 40.0),
        None,
    );

    let line = &layout.lines[0];
    assert_eq!(line.text, "\u{fffc}");
    let handle = layout
        .rich_text_artifact
        .as_ref()
    ...
    .expect("inline-only run publishes an explicit empty glyph slice");
    assert!(inline_slice.glyph_range.is_empty());
}
...
fn html_inline_image_respects_end_ellipsis_without_placeholder_fallback() {
    ...
    let handle = layout
        .rich_text_artifact
        .as_ref()
        .expect("inline ellipsis keeps a composite artifact");
    let artifact = crate::text::resolve_resolved_text_glyph_artifact(handle)
        .expect("inline ellipsis keeps its text glyph artifact");
    assert!(artifact.lines[0].is_some());
    let inline_run = line
        .runs
        .iter()
        .find(|run| run.text == "\u{fffc}")
    ...
    .expect("inline run owns an explicit artifact receipt");
    assert!(
        inline_slice.glyph_range.is_empty(),
        "external layout blocks must not own a font glyph"
    );
}

#[test]
fn html_text_only_ellipsis_publishes_a_virtual_glyph_artifact() {
    let mut style = test_style(UiTextWrap::None, UiTextOverflow::Ellipsis);
    style.rich_text_format = UiRichTextFormat::HtmlSubsetV1;
    let frame = UiFrame::new(0.0, 0.0, measure_text_size("ab…", &style).width + 0.1, 40.0);

    let layout = layout_text("<b>abcdef</b>", &style, frame, None);
    ...
    let ellipsis_run = line
        ...
        .iter()
        .find(|run| run.text == "…")
        .expect("rich ellipsis run");
    let handle = layout
        .rich_text_artifact
        .as_ref()
        .expect("rich ellipsis artifact handle");
    let mapped = crate::text::resolve_rich_text_glyph_run_artifact(
        handle,
        0,
        ellipsis_run.source_range,
        ellipsis_run.visual_range,
    )
    .expect("rich ellipsis glyph slice");
    assert_eq!(
        mapped.style_source_range,
        Some(UiTextRange { start: 0, end: 6 })
    );
    let artifact = crate::text::resolve_resolved_text_glyph_artifact(handle)
        .expect("rich ellipsis glyph artifact");
    assert!(artifact.lines[0].as_ref().is_some_and(|artifact_line| {
        artifact_line
            .glyphs
            .iter()
            .any(|glyph| glyph.flags.virtual_glyph)
    }));
    let retained_source_end = line
        .runs
        .iter()
    ...
    let omitted_spans = crate::text::resolved_text_glyph_artifact_range_advance_spans(
        artifact.as_ref(),
        0,
        line,
        UiTextRange {
            start: retained_source_end,
            end: line.source_range.end,
        },
    )
    .expect("omitted source resolves to ellipsis geometry");
    assert_eq!(omitted_spans.len(), 1);
    assert!(omitted_spans[0].1 > omitted_spans[0].0);
}
...
fn html_inline_image_vertical_rl_uses_object_height_as_main_axis_advance() {
    ...
    let handle = layout
        .rich_text_artifact
        .as_ref()
        .expect("VerticalRl inline layout keeps a composite artifact");
    let artifact = crate::text::resolve_resolved_text_glyph_artifact(handle)
        .expect("VerticalRl inline layout keeps canonical text glyphs");
    assert!(artifact.lines[0].is_some());
    let inline_run = layout.lines[0]
        .runs
        .iter()
        .find(|run| run.text == "\u{fffc}")
        .expect("first column retains the inline run");
    let inline_slice = crate::text::resolve_rich_text_glyph_run_artifact(
        handle,
        0,
        inline_run.source_range,
    ...
    .expect("VerticalRl inline run owns an artifact receipt");
    assert!(inline_slice.glyph_range.is_empty());
}
...
fn html_inline_image_vertical_rl_marks_clipped_columns_with_end_ellipsis() {
    ...
    let artifact = crate::text::resolve_resolved_text_glyph_artifact(
        layout
            .rich_text_artifact
            .as_ref()
            .expect("VerticalRl ellipsis keeps a composite artifact"),
    )
    .expect("VerticalRl ellipsis keeps canonical vertical glyphs");
    assert!(
        artifact.lines[0]
            .as_ref()
            .is_some_and(|line| { line.glyphs.iter().any(|glyph| glyph.flags.virtual_glyph) }),
        "the generated VerticalRl ellipsis must be a canonical virtual glyph"
    );
}
