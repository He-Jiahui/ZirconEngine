...
fn dynamic_api_export_returns_versioned_function_table() {
    ...
    let api = unsafe { zircon_runtime_get_api_v8(&host) };
    ...
    assert!(api.request_viewport_pick.is_some());
    assert!(api.poll_viewport_pick.is_some());
    assert!(api.cancel_viewport_pick.is_some());
}

#[test]
fn dynamic_api_exports_only_the_v8_runtime_table() {
    ...
    assert!(source.contains("ZrRuntimeApiV8"));
    assert!(source.contains("zircon_runtime_get_api_v8"));
}
...
#[test]
fn dynamic_api_rejects_a_host_table_with_the_wrong_size() {
...
fn runtime_api_table_entries_are_panic_wrapped_at_ffi_boundary() {
    ...
    assert!(source.contains("zircon_runtime_get_api_v8_inner"));
    ...
    for (inner, wrapper) in [
        ...
        ("request_viewport_pick", "request_viewport_pick_ffi"),
        ("poll_viewport_pick", "poll_viewport_pick_ffi"),
        ("cancel_viewport_pick", "cancel_viewport_pick_ffi"),
    ] {
