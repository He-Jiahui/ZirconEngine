...
fn zr_hit_proxy_output(input: ZrVertexOutput) -> ZrHitProxyOutput {
    ...
    output.world_normal = vec4<f32>(input.normal_ws, 0.0);
    return output;
}

@fragment
fn zr_fs_main(input: ZrVertexOutput) -> ZrHitProxyOutput {
    return zr_hit_proxy_output(input);
}

@fragment
fn fs_main(input: ZrVertexOutput) -> ZrHitProxyOutput {
    return zr_hit_proxy_output(input);
}
