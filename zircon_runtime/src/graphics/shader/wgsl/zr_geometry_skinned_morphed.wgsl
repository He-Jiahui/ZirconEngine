...
fn zr_morphed_position(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
    return v.position + zr_morph_vec3_delta(v, instance_index, ZR_MORPH_POSITION_ROW).xyz;
}

fn zr_previous_morphed_position(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
    return v.position + zr_morph_vec3_delta_with_previous_mode(v, instance_index, ZR_MORPH_POSITION_ROW, true).xyz;
}

fn zr_morphed_normal(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
...
fn zr_morphed_tangent(v: ZrVertexInput, instance_index: u32) -> vec4<f32> {
...
fn fetch_position(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
    return (zr_skin_matrix(v) * vec4<f32>(zr_morphed_position(v, instance_index), 1.0)).xyz;
}

fn fetch_prev_position(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
    return (zr_prev_skin_matrix(v) * vec4<f32>(zr_previous_morphed_position(v, instance_index), 1.0)).xyz;
}

fn fetch_normal(v: ZrVertexInput, instance_index: u32) -> vec3<f32> {
    return zr_normalize_or_zero((zr_skin_matrix(v) * vec4<f32>(zr_morphed_normal(v, instance_index), 0.0)).xyz);
}

fn fetch_tangent(v: ZrVertexInput, instance_index: u32) -> vec4<f32> {
    let morphed_tangent = zr_morphed_tangent(v, instance_index);
    return vec4<f32>(zr_normalize_or_zero((zr_skin_matrix(v) * vec4<f32>(morphed_tangent.xyz, 0.0)).xyz), morphed_tangent.w);
}
