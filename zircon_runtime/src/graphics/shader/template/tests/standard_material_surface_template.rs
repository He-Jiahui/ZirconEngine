...
fn render_shader_template_assembles_standard_material_surface_source() {
    ...
    assert_eq!(
        surface_source.source.matches("textureSampleBias(").count(),
        4
    );
...
#[test]
fn render_shader_template_omits_clearcoat_normal_source_without_feature() {
    let material = standard_material_descriptor();
    let surface_source = standard_material_surface_source(&material);
    assert!(
        !surface_source
            .features
            .contains(ShaderFeatureBits::PBR_CLEARCOAT),
        "baseline Standard PBR must exercise the non-clearcoat source path"
    );
    let assembly = assemble_material_shader_template(
        MaterialShaderTemplateRequest::new(
            static_mesh_descriptor(),
    ...
    .expect("baseline Standard PBR template assembly");
    let surface = assembly
        .wgsl_source
        .split("fn zr_material_surface(input: ZrVertexOutput) -> ZrSurfaceOutput {")
        .nth(1)
        .and_then(|source| source.split("return surface;\n}").next())
        .expect("baseline template should retain its material-surface owner");

    let fallback = surface
        .find("surface.clearcoat_normal_ws = surface.normal_ws;")
        .expect("non-clearcoat materials must keep their base normal as the clearcoat fallback");
    assert!(
        fallback > 0,
        "the clearcoat fallback must remain inside the material-surface owner"
    );
    for unreachable in [
        "@group(2) @binding(3) var standard_material_normal_tex",
        "@group(2) @binding(4) var standard_material_normal_sampler",
        ...
        assert!(
            ...
            "baseline Standard PBR must omit unreachable clearcoat source `{unreachable}`"
        );
    }
}

#[test]
fn render_shader_template_omits_tangent_frame_without_anisotropy_feature() {
    let material = standard_material_descriptor();
    let surface_source = standard_material_surface_source(&material);
    assert!(
        !surface_source
            .features
            .contains(ShaderFeatureBits::PBR_ANISOTROPY),
        "baseline Standard PBR must exercise the non-anisotropic source path"
    );
    let assembly = assemble_material_shader_template(
        MaterialShaderTemplateRequest::new(
            static_mesh_descriptor(),
    ...
    let tangent_default = surface
        .find("surface.tangent_ws = vec3<f32>(1.0, 0.0, 0.0);")
        .expect("non-anisotropic materials must keep a stable tangent default");
    let bitangent_default = surface
        .find("surface.bitangent_ws = vec3<f32>(0.0, 1.0, 0.0);")
        .expect("non-anisotropic materials must keep a stable bitangent default");
    assert!(
        tangent_default < bitangent_default,
    ...
    for unreachable in [
        ...
        assert!(
            !surface.contains(unreachable),
            "non-anisotropic source must omit tangent-frame work `{unreachable}`"
        );
    }
}

#[test]
fn render_shader_template_validates_anisotropy_only_surface_specialization() {
    let mut material = standard_material_descriptor();
    ...
    .expect("anisotropy-only Standard PBR template assembly");
    ...
    for required in [
        ...
        assert!(
            assembly.wgsl_source.contains(required),
            "anisotropy-only Standard PBR must retain `{required}`"
        );
    }
    for unreachable in [
        "@group(2) @binding(3) var standard_material_normal_tex",
        ...
        assert!(
            !assembly.wgsl_source.contains(unreachable),
            "anisotropy-only Standard PBR must omit `{unreachable}`"
        );
    }
    validate_material_shader_template_wgsl(&assembly.wgsl_source)
        .expect("anisotropy-only Standard PBR WGSL should validate");
}
