use crate::core::framework::render::{
    builtin_geometry_source_descriptor, RenderMaterialAlphaMode, RenderMaterialDependencySet,
    RenderMaterialFallbackPolicy, RenderMaterialLightingModel, RenderMaterialTextureTransform,
    RenderQueueValue, ShaderFeatureBits, ShaderPassType, StandardMaterialDescriptor,
...
fn render_shader_template_assembles_static_and_skinned_geometry_sources() {
    ...
    let static_assembly = assemble_material_shader_template(MaterialShaderTemplateRequest::new(
        static_mesh,
    ...
    let skinned_assembly = assemble_material_shader_template(MaterialShaderTemplateRequest::new(
        skinned_mesh,
        ShaderPassType::Forward,
        MATERIAL_SURFACE,
        "user_surface",
    ...
    assert!(static_assembly
        .include_tokens
        .contains(&"zr_geometry_static.wgsl".to_string()));
    assert!(static_assembly
        .include_tokens
        .contains(&"zr_scene_runtime.wgsl".to_string()));
    assert!(static_assembly
        .include_tokens
        .contains(&"zr_gpu_scene.wgsl".to_string()));
    assert!(skinned_assembly
        .include_tokens
        .contains(&"zr_geometry_skinned.wgsl".to_string()));
    assert!(static_assembly
        .wgsl_source
...
fn render_shader_template_assembles_standard_material_surface_source() {
    ...
    assert!(assembly
        .wgsl_source
        .contains("@group(2) @binding(10) var<uniform> standard_material_properties"));
    assert!(assembly
        .wgsl_source
    ...
    assert!(assembly
        .include_tokens
        .contains(&"zr_light_grid.wgsl".to_string()));
    assert!(assembly
        .include_tokens
        .contains(&"zr_shadow.wgsl".to_string()));
    assert!(assembly
        .wgsl_source
...
fn render_shader_template_clips_alpha_for_masked_standard_material_passes() {
    ...
    assert!(depth_alpha
        .include_tokens
        .contains(&"zr_template_depth_alpha.wgsl".to_string()));
    assert!(shadow_alpha
        .include_tokens
        .contains(&"zr_template_shadow_alpha.wgsl".to_string()));
}
...
fn render_shader_template_specializes_depth_and_velocity_passes() {
    ...
    let depth_no_alpha = assemble_material_shader_template(MaterialShaderTemplateRequest::new(
        static_mesh.clone(),
        ShaderPassType::DepthPrepass,
        MATERIAL_SURFACE,
        "user_surface",
    ...
    .expect("alpha depth template assembly");
    let velocity = assemble_material_shader_template(MaterialShaderTemplateRequest::new(
        static_mesh.clone(),
    ...
    assert!(depth_no_alpha
        .include_tokens
        .contains(&"zr_template_depth.wgsl".to_string()));
    ...
    assert!(velocity_alpha
        .include_tokens
        .contains(&"zr_template_velocity_alpha.wgsl".to_string()));
