...
impl HybridGiResolveRuntime {
    pub(crate) fn parent_probe_id(&self, probe_id: u32) -> Option<u32> {
        self.probe_parent_probes.get(&probe_id).copied()
    }

    pub(crate) fn has_parent_topology(&self) -> bool {
        !self.probe_parent_probes.is_empty()
    }

    #[cfg(test)]
    pub(crate) fn parent_probe_count(&self) -> usize {
        self.probe_parent_probes.len()
    }

    #[cfg(test)]
    pub(crate) fn replace_probe_parent_probes_for_test(
        &mut self,
    ...
    pub(crate) fn parent_probe_chain(&self, probe_id: u32) -> Vec<(u32, usize)> {
    ...
    pub(crate) fn descendant_probe_chain(&self, probe_id: u32) -> Vec<(u32, usize)> {
        let mut chain = Vec::new();
        let mut stack = self
            .probe_parent_probes
            .iter()
