...
#[test]
fn render_product_post_depth_of_field_split_changes_final_frame() {
    let viewport_size = UVec2::new(160, 120);
    let framework = depth_of_field_product_framework();
    let baseline_viewport =
        create_dof_product_viewport(&framework, viewport_size, "post-dof-baseline");
    let dof_viewport = create_dof_product_viewport(&framework, viewport_size, "post-dof-split");

    let (baseline, _) = submit_and_capture_dof_product(
        &framework,
        baseline_viewport,
        depth_of_field_particle_product_extract(viewport_size, false),
    );
    let (dof_frame, stats) = submit_and_capture_dof_product(
        &framework,
        dof_viewport,
        depth_of_field_particle_product_extract(viewport_size, true),
    );
    ...
    assert_post_process_node_executed(&stats, "depth-of-field");
    assert_post_process_node_executed(&stats, "uber");
    assert_graph_executor_executed(&stats, PARTICLE_TRANSPARENT_EXECUTOR_ID);
    assert_graph_executor_executed(&stats, POST_DOF_PREPARE_EXECUTOR_ID);
    assert_graph_executor_executed(&stats, POST_DOF_EXECUTOR_ID);
    ...
    assert_graph_executor_order(&stats, POST_DOF_EXECUTOR_ID, "post.uber");
    ...
    assert_eq!(
        ...
        vec!["depth-of-field".to_string()]
    );
    let effect_stack_report = &stats.last_post_process_effect_stack_report;
    assert!(
        effect_stack_report.missing_resources.is_empty(),
        "DoF product scene should not miss required resources; report={effect_stack_report:?}"
    );
    assert_texture_backing_exists(&stats, PostProcessGraphResourceNames::DEPTH_OF_FIELD_COC);
    assert_texture_backing_exists(&stats, PostProcessGraphResourceNames::DEPTH_OF_FIELD_BOKEH);
    ...
    assert!(
        ...
        "DoF product baseline should contain visible high-contrast scene content; rgb_sum={baseline_rgb_sum}"
    );

    let frame_delta = frame_rgb_abs_delta(&dof_frame, &baseline);
    let dof_rgb_sum = frame_rgb_sum(&dof_frame);
    assert!(
        frame_delta > 5_000,
        "split DoF should produce a measurable final-frame delta; delta={frame_delta}, baseline_rgb_sum={baseline_rgb_sum}, dof_rgb_sum={dof_rgb_sum}, aliases={:?}",
...
fn create_dof_product_viewport(
    ...
    let viewport = framework
        ...
        .unwrap();
    framework
        .set_quality_profile(viewport, depth_of_field_product_profile(profile_name))
        .unwrap();
    viewport
}

fn depth_of_field_product_profile(profile_name: &str) -> RenderQualityProfile {
...
fn depth_of_field_particle_product_extract(
    viewport_size: UVec2,
    depth_of_field_enabled: bool,
) -> RenderFrameExtract {
    let mut extract = RenderFrameExtract::from_snapshot(
        RenderWorldSnapshotHandle::new(923),
        RenderSceneSnapshot {
    ...
    extract.particles.emitters = vec![923];
    extract.particles.sprites = depth_of_field_particle_sprites();
...
fn depth_of_field_particle_sprites() -> Vec<RenderParticleSpriteSnapshot> {
    let colors = [
        Vec4::new(1.0, 0.02, 0.02, 1.0),
        Vec4::new(0.02, 0.95, 1.0, 1.0),
        Vec4::new(1.0, 0.9, 0.02, 1.0),
        Vec4::new(0.9, 0.02, 1.0, 1.0),
    ];
...
fn submit_and_capture_dof_product(
    framework: &WgpuRenderFramework,
    ...
    let frame = framework
        ...
        .unwrap()
        .expect("DoF product frame should be capturable");
...
fn assert_texture_backing_exists(stats: &RenderStats, logical_name: &str) {
    let _ = texture_backing_name(stats, logical_name);
}
