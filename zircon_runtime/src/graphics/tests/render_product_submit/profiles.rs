...
fn render_product_submit_advanced_profile_accepts_provider_backed_vg_hgi_path() {
    ...
    framework
        .set_quality_profile(
            viewport,
            render_product_advanced::advanced_quality_profile("m10a-advanced-acceptance"),
        )
        .unwrap();
    ...
    framework
        .submit_frame_extract(
            viewport,
            render_product_advanced::advanced_product_extract(),
        )
        .unwrap();
    ...
    assert_eq!(
        render_product_advanced::advanced_provider_report(
            &stats,
...
fn render_product_submit_solari_experimental_reports_gated_provider_status() {
    ...
    framework.override_capabilities_for_tests(render_product_solari::solari_capabilities());
    let viewport = framework
        .create_viewport(RenderViewportDescriptor::new(UVec2::new(320, 240)))
        .unwrap();
    framework
        .set_quality_profile(
            viewport,
            render_product_solari::solari_quality_profile("m10a-solari-acceptance", true)
