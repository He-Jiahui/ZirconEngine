...
const DIRECT_MORPH_VELOCITY_PNG_STATUS: &str =
    "render_plan08_direct_morph_weight_velocity_product_png_passed_renderdoc_deferred";

#[test]
fn render_product_direct_mesh_morph_weight_change_writes_scene_velocity_pixels() {
    let _capture = capture_direct_morph_velocity_product();
}

#[test]
#[ignore = "writes Plan 08 product scene-velocity PNG artifact"]
fn export_direct_morph_weight_velocity_product_png() {
    let capture = capture_direct_morph_velocity_product();
    let output_path = velocity_png::render_test_output_dir()
        .join("runtime_render_plan08_direct_morph_weight_velocity_20260703.png");
...
struct DirectMorphVelocityCapture {
...
fn capture_direct_morph_velocity_product() -> DirectMorphVelocityCapture {
    ...
    let material_uri =
        AssetUri::parse("res://materials/product-direct-morph-velocity.zmaterial").unwrap();
    let material_id = ResourceId::from_locator(&material_uri);
    register_unlit_material_revision(
        &asset_manager,
        material_id,
        material_uri,
        "product-direct-morph-velocity-material-v1",
    );
    let mesh_uri = AssetUri::parse("res://meshes/product-direct-morph-velocity.zmesh").unwrap();
    ...
    framework
        .submit_frame_extract(
            viewport,
            direct_morph_velocity_extract(material_id, mesh_id, 2601, 0.0),
        )
        .unwrap();
    let first = framework.query_stats().unwrap();
    assert_eq!(
        first.last_mesh_gpu_morphed_source_draw_count, 0,
        "the first all-zero morph frame should establish previous morph state without drawing a GPU morph payload"
    );

    framework
        .submit_frame_extract(
            viewport,
            direct_morph_velocity_extract(material_id, mesh_id, 2602, 1.0),
        )
        .unwrap();
    let second = framework.query_stats().unwrap();

    assert_eq!(second.last_mesh_gpu_morphed_source_draw_count, 1);
...
fn direct_morph_velocity_mesh_snapshot(
    ...
    RenderMeshSnapshot {
        node_id: DIRECT_MORPH_VELOCITY_NODE_ID,
        stable_instance_key: DIRECT_MORPH_VELOCITY_NODE_ID << 16,
