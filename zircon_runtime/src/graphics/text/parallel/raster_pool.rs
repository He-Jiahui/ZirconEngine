...
use crate::graphics::text::raster::{
    GlyphBitmap, SwashRasterError, SwashRasterRequest, SwashRasterizer,
};
...
impl TextRasterWorkerPool {
    pub(crate) fn new(options: TextRasterWorkerPoolOptions) -> Result<Self, ZirconError> {
    ...
    pub(crate) fn request(&self, work: TextRasterWorkItem) -> Result<(), ZirconError> {
            ...
            .try_send(work)
        {
            ...
            return Err(match error {
                TrySendError::Full(work) => {
                    ZirconError::ChannelSend(format!("text raster work queue full: {:?}", work.id))
                }
                TrySendError::Disconnected(work) => {
                    ZirconError::ChannelSend(format!("text raster work dropped: {:?}", work.id))
                }
            });
        }
        self.record_in_flight_locked(&in_flight);
