...
pub(crate) fn resolve_rich_run_style(
    ...
    override_style: &crate::core::framework::render::StyleOverride,
) -> UiResolvedStyle {
    let mut style = base.clone();
    if let Some(weight) = override_style.weight {
        style.font_weight = UiResolvedStyle::normalized_font_weight(weight);
    }
...
fn ui_range(range: (u32, u32)) -> Option<UiTextRange> {
    Some(UiTextRange {
