...
use super::super::{
    GlyphAtlasBitmapRetryBackpressurePolicy, GlyphAtlasBitmapSource, GLYPH_BITMAP_ATLAS_PADDING_PX,
};
...
{
    let submission_plan = state.submission_plan_with_backpressure_and_padding(
        frame_sources,
        config.page_size,
        frame_index,
    ...
    let state_report = state.apply_submission_plan(&submission_plan);
