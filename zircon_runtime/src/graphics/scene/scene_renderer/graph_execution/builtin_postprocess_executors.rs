...
use crate::core::framework::render::{
    PostProcessEffectKind, PostProcessGraphResourceNames, RenderExposureMode, RenderPipelinePhase,
};
use crate::render_graph::RenderGraphAttachmentOps;

use super::{RenderPassExecutionContext, RenderPassExecutorId, RenderPassGpuExecutionContext};
...
use self::resource_routing::{
    ...
    taa_input_resource, terminal_anti_alias_input_resource, upscale_input_resource,
};
...
fn with_borrowed_gpu_metadata<'a, T>(
    ...
    let pass_name = mem::take(&mut context.pass_name);
    let executor_id = mem::replace(
        &mut context.executor_id,
        RenderPassExecutorId::from(String::new()),
    );
    let result = match context.gpu_mut() {
        Some(gpu) => operation(&pass_name, executor_id.as_str(), gpu),
        None => Err(format!(
    ...
    context.pass_name = pass_name;
    context.executor_id = executor_id;
    result
}
...
pub(super) fn color_lut_bake_postprocess_executor(
    ...
    with_borrowed_gpu_metadata(context, |pass_name, executor_id, gpu| {
...
pub(super) fn taa_resolve_postprocess_executor(
    ...
    let taa_history_attachment_ops = context
        ...
        .unwrap_or_else(RenderGraphAttachmentOps::clear_store);
    let scene_color_resource = taa_input_resource(context);
    with_borrowed_gpu_metadata(context, |pass_name, _, gpu| {
...
pub(super) fn primary_upscale_postprocess_executor(
    context: &mut RenderPassExecutionContext<'_>,
) -> Result<(), String> {
    spatial_upscale_postprocess_executor(
        context,
...
pub(super) fn secondary_upscale_postprocess_executor(
    context: &mut RenderPassExecutionContext<'_>,
) -> Result<(), String> {
    spatial_upscale_postprocess_executor(
        context,
...
fn spatial_upscale_postprocess_executor(
    context: &mut RenderPassExecutionContext<'_>,
    effect: PostProcessEffectKind,
    output_resource: &'static str,
    phase: RenderPipelinePhase,
) -> Result<(), String> {
    product_postprocess_executor(context, effect)?;
    let attachment_ops = context
        .attachment_ops_for_write(output_resource)
        .unwrap_or_else(RenderGraphAttachmentOps::clear_store);
