...
use crate::core::framework::render::{FontFaceId, InstancedFaceId};
...
impl SdfOfflineSourceCache {
    pub(super) fn load_glyph(
        ...
        let manifest = load_ui_font_manifest_with_asset_manager(font_ref, Some(asset_manager))?;
        ...
        let glyph_id = glyph_id_for_key(&face, key).ok()?.0;
