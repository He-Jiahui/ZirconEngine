...
#[test]
fn compiled_frame_rebuilds_when_text_owner_republishes_artifact() {
    let mut prepared_text = text_batch("A", UiFrame::new(0.0, 0.0, 24.0, 24.0));
    prepared_text.glyph_artifact_line = Some(artifact_line([0.0, 0.0]));

    let mut refreshed_text = prepared_text.clone();
    refreshed_text.glyph_artifact_line = Some(artifact_line([3.0, 0.0]));
...
fn artifact_line(offset: [f32; 2]) -> ScreenSpaceUiGlyphArtifactLine {
