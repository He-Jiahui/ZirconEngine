...
fn render_mesh_draw_processor_depth_prepass_filters_transparent() {
    ...
    DepthPrepassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Opaque, 1),
        &mut context,
        &mut list,
    );
    DepthPrepassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::AlphaMask, 2),
        &mut context,
        &mut list,
    );
    DepthPrepassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Transparent, 3),
        &mut context,
        &mut list,
    );
    ...
    assert_eq!(
        list.commands()
            ...
            .collect::<Vec<_>>(),
        vec![RenderPhase::Prepass, RenderPhase::Prepass]
    );
}
...
fn render_mesh_draw_processor_opaque_preserves_material_slots_for_fallback_shader_selection() {
    ...
    OpaqueBasePassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Opaque, 7),
        &mut context,
        &mut list,
    );
...
fn render_mesh_draw_processor_shadow_excludes_non_casters_and_picks_alpha_mask_variant() {
    ...
    ShadowPassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Opaque, 1),
        &mut context,
        &mut list,
    );
    ShadowPassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Opaque, 2).with_casts_shadow(true),
        &mut context,
        &mut list,
    );
    ShadowPassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::AlphaMask, 3).with_casts_shadow(true),
        &mut context,
        &mut list,
    );
    ShadowPassProcessor.add_mesh_batch(
        &batch(MeshDrawQueuePhase::Transparent, 4).with_casts_shadow(true),
        &mut context,
        &mut list,
    );
    list.sort();
    ...
    assert_eq!(
        list.commands()
            ...
            .collect::<Vec<_>>(),
        vec![
            (RenderPhase::Shadow, MeshPassPipelineKind::ShadowDepth),
            (
                RenderPhase::Shadow,
                MeshPassPipelineKind::ShadowDepthAlphaMask
            ),
