...
use super::super::mesh_pipeline::create_hit_proxy_mesh_pipeline;
use super::shader_source::mesh_pipeline_hit_proxy_template_source_for_geometry_descriptor_with_streamer;
use super::shader_source_validation_admission::CachedMeshShaderModule;
use super::{MeshPipelineCache, PipelineCreationTarget};

const HIT_PROXY_MESH_SHADER_KEY_PREFIX: &str = "zircon.builtin.hit-proxy-mesh@1";
const HIT_PROXY_PIPELINE_TARGET: PipelineCreationTarget =
    PipelineCreationTarget::MeshPass(MeshPassPipelineKind::HitProxy);

impl MeshPipelineCache {
    fn ensure_hit_proxy_pipeline(
        &mut self,
        device: &wgpu::Device,
        ...
        let Some(geometry_source) = self.geometry_source_descriptor_for_variant(shader_variant_key)
            ...
            self.mark_pipeline_failure_for_target(
                HIT_PROXY_PIPELINE_TARGET,
                variant_id,
                PipelineAdmissionReason::GeometrySourceUnavailable,
                "HitProxy pipeline geometry source descriptor is unavailable",
            );
        ...
        let shader_source =
            match mesh_pipeline_hit_proxy_template_source_for_geometry_descriptor_with_streamer(
                streamer,
                key,
                &geometry_source,
            ) {
        ...
        self.record_observed_shader_source(HIT_PROXY_PIPELINE_TARGET, &shader_source.source_hash);
        let shader_key = hit_proxy_mesh_shader_key(shader_variant_key, &shader_source.source_hash);
        ...
        if let Some(source) = validated_source {
            ...
            let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("zircon-hit-proxy-mesh-shader"),
            ...
            self.take_ready_shader_source_validation(&validation_key)
                .expect("installed HitProxy module must consume its validation artifact");
            self.record_shader_module_creation(HIT_PROXY_PIPELINE_TARGET, creation_elapsed);
        }
        if !self.hit_proxy_mesh_pipelines.contains_key(&variant_id) {
            let shader = self
                .shader_modules
                .get(&shader_key)
                .expect("HitProxy mesh shader module cached");
            let creation_started = std::time::Instant::now();
            let pipeline = create_hit_proxy_mesh_pipeline(
                device,
            ...
            self.record_render_pipeline_creation(HIT_PROXY_PIPELINE_TARGET, creation_elapsed);
        }
        ...
        if pipeline_validation_failed {
            ...
            self.mark_pipeline_failure_for_target(
                ...
                "HitProxy pipeline WGPU validation failed",
            );
            return self.unavailable_pipeline_for_target(
                HIT_PROXY_PIPELINE_TARGET,
                variant_id,
        ...
        self.clear_pipeline_unavailable_state_for_target(HIT_PROXY_PIPELINE_TARGET, variant_id);
        PipelineAdmission::Ready(())
    }

    pub(crate) fn ensure_hit_proxy_pipeline_admission_for_variant(
        &mut self,
        device: &wgpu::Device,
        streamer: &ResourceStreamer,
        variant_id: MeshPipelineVariantId,
        ...
        if let Some(reason) =
            self.pipeline_failure_reason_for_target(HIT_PROXY_PIPELINE_TARGET, variant_id)
        {
            return self.unavailable_pipeline_for_target(
                HIT_PROXY_PIPELINE_TARGET,
                variant_id,
        ...
        if kind != MeshPassPipelineKind::HitProxy {
            return self.unavailable_pipeline_for_target(
                HIT_PROXY_PIPELINE_TARGET,
                variant_id,
        ...
        self.ensure_hit_proxy_pipeline(
            device,
            streamer,
            variant_id,
            &pipeline_key,
    ...
    pub(crate) fn hit_proxy_pipeline_for_ready_variant(
        &self,
        variant_id: MeshPipelineVariantId,
    ) -> &wgpu::RenderPipeline {
        self.hit_proxy_mesh_pipelines
            .get(&variant_id)
            .expect("Ready HitProxy pipeline admission must retain its pipeline")
    }
}

fn hit_proxy_mesh_shader_key(variant_key: &ShaderVariantKey, source_hash: &str) -> String {
    format!(
        "{}#{}#{}",
        HIT_PROXY_MESH_SHADER_KEY_PREFIX,
...
mod tests {
    ...
    use super::{HIT_PROXY_MESH_SHADER_KEY_PREFIX, hit_proxy_mesh_shader_key};
