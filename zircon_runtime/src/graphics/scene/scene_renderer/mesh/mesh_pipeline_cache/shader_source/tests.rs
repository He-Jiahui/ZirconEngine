...
use super::{
    ...
    mesh_pipeline_velocity_template_source_for_geometry, MeshPipelineShaderSource,
...
fn mesh_pipeline_virtual_geometry_template_source_declares_page_cluster_fetch_bindings() {
    ...
    assert!(source
        .wgsl_source
        .contains("let cluster_word_count = zr_gpu_scene_virtual_geometry_cluster_word_count()"));
    assert!(!source.wgsl_source.contains(
