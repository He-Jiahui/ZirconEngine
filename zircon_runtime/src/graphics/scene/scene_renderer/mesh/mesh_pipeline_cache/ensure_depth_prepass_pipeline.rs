...
const DEPTH_PREPASS_PIPELINE_TARGET: PipelineCreationTarget =
    PipelineCreationTarget::MeshPass(MeshPassPipelineKind::DepthPrepass);

impl MeshPipelineCache {
    fn ensure_depth_prepass_pipeline(
        &mut self,
        device: &wgpu::Device,
        ...
        let Some(geometry_source) = self.geometry_source_descriptor_for_variant(shader_variant_key)
            ...
            self.mark_pipeline_failure_for_target(
                DEPTH_PREPASS_PIPELINE_TARGET,
                variant_id,
                PipelineAdmissionReason::GeometrySourceUnavailable,
                "DepthPrepass pipeline geometry source descriptor is unavailable",
            );
        ...
        if let Some(source) = validated_source {
            ...
            self.take_ready_shader_source_validation(&validation_key)
                .expect("installed DepthPrepass module must consume its validation artifact");
            self.record_shader_module_creation(DEPTH_PREPASS_PIPELINE_TARGET, creation_elapsed);
        }
        if !self.depth_prepass_mesh_pipelines.contains_key(&variant_id) {
            ...
            self.record_render_pipeline_creation(DEPTH_PREPASS_PIPELINE_TARGET, creation_elapsed);
        }
        ...
        if pipeline_validation_failed {
            ...
            self.mark_pipeline_failure_for_target(
                ...
                "DepthPrepass pipeline WGPU validation failed",
            );
            return self.unavailable_pipeline_for_target(
                DEPTH_PREPASS_PIPELINE_TARGET,
                variant_id,
        ...
        self.clear_pipeline_unavailable_state_for_target(DEPTH_PREPASS_PIPELINE_TARGET, variant_id);
        PipelineAdmission::Ready(())
    }

    pub(crate) fn ensure_depth_prepass_pipeline_admission_for_variant(
        &mut self,
        device: &wgpu::Device,
        streamer: &ResourceStreamer,
        variant_id: MeshPipelineVariantId,
        ...
        if self.depth_prepass_mesh_pipelines.contains_key(&variant_id) {
            self.clear_pipeline_unavailable_state_for_target(
                DEPTH_PREPASS_PIPELINE_TARGET,
                variant_id,
            );
        ...
        if let Some(reason) =
            self.pipeline_failure_reason_for_target(DEPTH_PREPASS_PIPELINE_TARGET, variant_id)
        {
            return self.unavailable_pipeline_for_target(
                DEPTH_PREPASS_PIPELINE_TARGET,
                variant_id,
    ...
    pub(crate) fn depth_prepass_pipeline_for_ready_variant(
        &self,
        variant_id: MeshPipelineVariantId,
    ) -> &wgpu::RenderPipeline {
        self.depth_prepass_mesh_pipelines
            .get(&variant_id)
            .expect("Ready DepthPrepass pipeline admission must retain its pipeline")
    }
}
