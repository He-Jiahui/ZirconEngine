...
@group(0) @binding(0) var scene_depth_tex: texture_depth_multisampled_2d;
...
fn load_depth_range_or_far(coord: vec2<i32>, size: vec2<i32>) -> HzbDepthRange {
    ...
    let first_depth = textureLoad(scene_depth_tex, coord, 0);
    var depth_range = HzbDepthRange(first_depth, first_depth);
    let sample_count = textureNumSamples(scene_depth_tex);
    for (var sample_index = 1u; sample_index < sample_count; sample_index = sample_index + 1u) {
        let sample_depth = textureLoad(scene_depth_tex, coord, sample_index);
        depth_range = combine_depth_ranges(
            depth_range,
            HzbDepthRange(sample_depth, sample_depth),
        );
    }
    return depth_range;
}
