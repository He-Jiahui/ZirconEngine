struct MotionVectorCameraParams {
...
@group(0) @binding(1) var<uniform> params: MotionVectorCameraParams;
...
fn load_motion_vector_scene_depth(coord: vec2<u32>) -> f32 {
...
fn motion_vector_camera_velocity(coord: vec2<u32>, depth: f32) -> vec2<f32> {
...
fn fs_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let depth = load_motion_vector_scene_depth(coord);
    let velocity = motion_vector_camera_velocity(coord, depth);
