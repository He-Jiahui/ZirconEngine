...
mod tests {
    ...
    fn post_process_shader_samples_bound_contact_shadow_occlusion_texture() {
        ...
        assert!(POST_PROCESS_SHADER.contains("color = color * max(contact_shadow"));
    }
    ...
    #[test]
    fn post_process_uber_does_not_apply_ssao_to_terminal_scene_color() {
        assert!(!POST_PROCESS_SHADER.contains("let ao = textureLoad(ambient_occlusion_tex"));
        assert!(!POST_PROCESS_SHADER.contains("max(ao * ao, 0.12)"));
        assert!(!POST_PROCESS_SHADER.contains("color = color * occlusion_factor"));
    }
