...
impl SceneRenderer {
    #[cfg(test)]
    pub(crate) fn read_last_virtual_geometry_node_and_cluster_cull_input_snapshot(
        &self,
    ) -> Result<Option<RenderVirtualGeometryCullInputSnapshot>, GraphicsError> {
        let Some(buffer) = self
            .last_virtual_geometry_node_and_cluster_cull_buffer
            .as_ref()
        else {
            return Ok(None);
        };
        ...
        let staging = self.backend.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("zircon-vg-node-and-cluster-cull-readback"),
        ...
        let mut encoder =
            self.backend
                ...
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("zircon-vg-node-and-cluster-cull-readback-encoder"),
                });
