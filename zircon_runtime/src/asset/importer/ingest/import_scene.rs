...
mod plugins07_scene_source_tests {
    ...
    #[test]
    fn borrowed_document_source_contract_scene_import() {
        let context = AssetImportContext::new(
            PathBuf::from("scenes/plugins07.scene.toml"),
            AssetUri::parse("res://scenes/plugins07.scene.toml").unwrap(),
            br#"
[[entities]]
entity = 7
name = "Plugins07"
...
"#
        ...
        let outcome = import_scene(&context).unwrap();
        let Some(ImportedAsset::Scene(scene)) = outcome.root_entry().map(|entry| &entry.asset)
        else {
            panic!("scene importer must preserve its typed root asset")
        };
        assert_eq!(scene.entities.len(), 1);
        assert_eq!(scene.entities[0].name, "Plugins07");
    }

    #[test]
    #[ignore = "release performance gate"]
    fn borrowed_document_source_performance_release_scene() {
        run_release_gate("plugins07_borrowed_scene_source");
    }
    ...
    fn run_release_gate(marker: &str) {
        ...
        assert!(
            ...
            "borrowed scene source preparation must improve P95 by at least 40%"
        );
    }

    fn benchmark_context() -> AssetImportContext {
        AssetImportContext::new(
            PathBuf::from("scenes/plugins07-large.scene.toml"),
            AssetUri::parse("res://scenes/plugins07-large.scene.toml").unwrap(),
