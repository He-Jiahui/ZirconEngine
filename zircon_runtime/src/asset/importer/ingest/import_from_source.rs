...
impl AssetImporter {
    pub fn import_from_source(
        ...
        if lower_name.ends_with(".font.toml") {
            return self.import_font_asset(source_path);
        }
        if lower_name.ends_with(".model.toml") {
            return self.import_model(source_path);
        }
        ...
        match extension.as_str() {
            ...
            "wav" => self.import_sound(source_path, uri),
