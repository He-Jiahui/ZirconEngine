...
mod plugins07_ui_icon_source_tests {
    ...
    #[test]
    fn borrowed_ui_document_source_contract_icon_import() {
        let context = AssetImportContext::new(
            PathBuf::from("ui/plugins07.icon.toml"),
            AssetUri::parse("res://ui/plugins07.icon.toml").unwrap(),
            br##"
semantic_id = "icons/plugins07"
default_size = 18.0

[source]
kind = "svg_asset"
uri = "res://ui/icons/plugins07.svg"
"##
            .to_vec(),
            toml::Table::new(),
        );

        let outcome = import_ui_icon_asset(&context).unwrap();
        let Some(ImportedAsset::UiIcon(icon)) = outcome.root_entry().map(|entry| &entry.asset)
        else {
            panic!("ui icon importer must preserve its typed root asset")
        };
        assert_eq!(icon.semantic_id, "icons/plugins07");
        assert_eq!(icon.default_size, 18.0);
    }

    #[test]
    #[ignore = "release performance gate"]
    fn borrowed_ui_document_source_performance_release_icon() {
        run_release_gate("plugins07_borrowed_ui_icon_source");
    }
    ...
    fn run_release_gate(marker: &str) {
        ...
        assert!(
            ...
            "borrowed ui icon source preparation must improve P95 by at least 40%"
        );
    }

    fn benchmark_context() -> AssetImportContext {
        AssetImportContext::new(
            PathBuf::from("ui/plugins07-large.icon.toml"),
            AssetUri::parse("res://ui/plugins07-large.icon.toml").unwrap(),
