...
mod tests {
    ...
    #[test]
    fn optimization_batch_ev_runtime454_preserves_fixed_lowercase_hash_hex() {
        for value in [0, 1, 0x0f, 0x10, 0x0123_4567_89ab_cdef, u64::MAX] {
            assert_eq!(fixed_lower_hex(value), format!("{value:016x}"));
        }
        for bytes in [b"".as_slice(), b"zircon", &[0, 1, 2, 254, 255]] {
            let mut legacy_hasher = DefaultHasher::new();
            bytes.hash(&mut legacy_hasher);
            assert_eq!(
                hash_bytes(bytes),
        ...
        let production = include_str!("hash_bytes.rs")
            .split("#[cfg(test)]")
            .next()
    ...
    fn optimization_batch_ev_runtime454_direct_hash_hex_benchmark() {
        for _ in 0..4 {
    ...
    fn measure_hex(mut encode: impl FnMut(u64) -> String) -> u128 {
        ...
        for index in 0..VALUES_PER_SAMPLE {
            let value = black_box((index as u64).wrapping_mul(0x9e37_79b9_7f4a_7c15));
    ...
    fn report_performance(legacy_samples: &[u128], optimized_samples: &[u128]) {
        ...
        println!(
            "RUNTIME454_DIRECT_HASH_HEX_BENCH_V1 sample_pairs={SAMPLE_PAIRS} values_per_sample={VALUES_PER_SAMPLE} legacy_ns={} optimized_ns={} legacy_p95_ns={legacy_p95} optimized_p95_ns={optimized_p95} improvement_percent={improvement_percent} threshold_percent=35",
            csv(legacy_samples),
            csv(optimized_samples),
        );
        assert!(
            optimized_p95 <= legacy_p95.saturating_mul(65) / 100,
            "direct fixed hash hex encoding must reduce P95 by at least 35%"
        );
    }
