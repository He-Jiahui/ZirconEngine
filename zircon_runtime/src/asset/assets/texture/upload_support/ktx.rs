...
#[cfg(test)]
mod plugins07_ktx_header_hotpath_tests {
    ...
    #[test]
    fn header_first_texture_probe_contract_ktx() {
        let texture = fixture_texture();
        assert!(ktx_upload_plan(&texture, " KTX/GL-33777 ", &[0_u8; 16]).is_none());
        assert!(
            ktx2_upload_plan(&texture, " KTX2/VK-133/SUPERCOMPRESSION-0 ", &[0_u8; 16],).is_none()
        );
    }

    #[test]
    #[ignore = "release performance gate"]
    fn header_first_texture_probe_performance_release_ktx() {
        let texture = fixture_texture();
        let bytes = [0_u8; 16];
        ...
        println!(
            "PERF_RESULT plugins07_ktx_header_first_rejection sample_pairs={SAMPLE_PAIRS} legacy_ns={} optimized_ns={} legacy_p95_ns={legacy_p95} optimized_p95_ns={optimized_p95} improvement_percent={improvement_percent} threshold_percent=50 legacy_allocations_per_sample={} optimized_allocations_per_sample=0 order=alternating_legacy_first_even legacy_first_pairs=11 optimized_first_pairs=10",
            csv(&legacy_samples),
        ...
        assert!(
            ...
            "KTX header-first rejection must improve P95 by at least 50%"
        );
    }
    ...
    fn measure_legacy(bytes: &[u8]) -> u128 {
        ...
        for _ in 0..LOOKUPS_PER_SAMPLE {
            let ktx1 = black_box(" KTX/GL-33777 ").trim().to_ascii_lowercase();
            rejected +=
                u64::from(black_box(bytes).get(..KTX1_IDENTIFIER.len()) != Some(KTX1_IDENTIFIER));
            black_box(ktx1);

            let ktx2 = black_box(" KTX2/VK-133/SUPERCOMPRESSION-0 ")
                .trim()
                .to_ascii_lowercase();
            rejected +=
                u64::from(black_box(bytes).get(..KTX2_IDENTIFIER.len()) != Some(KTX2_IDENTIFIER));
            black_box(ktx2);
        }
        black_box(rejected);
    ...
    fn measure_header_first(texture: &TextureAsset, bytes: &[u8]) -> u128 {
        ...
        for _ in 0..LOOKUPS_PER_SAMPLE {
            ...
            rejected += u64::from(
                ktx2_upload_plan(
                    black_box(texture),
                    black_box(" KTX2/VK-133/SUPERCOMPRESSION-0 "),
                    black_box(bytes),
                )
                .is_none(),
            );
    ...
    fn fixture_texture() -> TextureAsset {
        TextureAsset::new_container(
            AssetUri::parse("res://textures/header-probe.ktx2").unwrap(),
            4,
            4,
