use std::sync::Arc;
use std::sync::atomic::Ordering;
use std::time::Instant;

use crate::core::runtime::{TaskDescriptor, TaskId, TaskPoolKind};

use super::contract::{
    RenderArtifactBlockIoDispatchBudget, RenderArtifactBlockIoDispatchError,
    RenderArtifactBlockIoDispatchReport,
};
use super::loader::RenderArtifactBlockLoaderInner;
use super::registry::take_task_id;
use super::worker::run_io_task;

impl RenderArtifactBlockLoaderInner {
    pub(super) fn dispatch_io(
        self: &Arc<Self>,
        budget: RenderArtifactBlockIoDispatchBudget,
    ) -> Result<RenderArtifactBlockIoDispatchReport, RenderArtifactBlockIoDispatchError> {
        if budget.max_tasks() == 0 {
            return Err(RenderArtifactBlockIoDispatchError::ZeroLimit { limit: "max_tasks" });
        }
        if budget.max_encoded_bytes() == 0 {
            return Err(RenderArtifactBlockIoDispatchError::ZeroLimit {
                limit: "max_encoded_bytes",
            });
        }

        self.maintain_deadlines(Instant::now());

        let mut registry = self.lock_registry();
        if !registry.accepting {
            return Err(RenderArtifactBlockIoDispatchError::Closed);
        }
        let mut report = RenderArtifactBlockIoDispatchReport::default();
        let scope = self.scope.census();
        let active_tasks = scope.queued.saturating_add(scope.running);
        let available_task_capacity = scope.task_capacity.saturating_sub(active_tasks);
        let task_limit = budget.max_tasks().min(available_task_capacity);
        while report.submitted_tasks < task_limit {
            let Some((frontier_key, key)) = registry.io_frontier.pop_highest() else {
                break;
            };
            let Some(entry) = registry.entries.get(&key).cloned() else {
                continue;
            };
            let encoded_bytes = entry.descriptor().encoded_bytes();
            let remaining_bytes = budget
                .max_encoded_bytes()
                .saturating_sub(report.submitted_encoded_bytes);
            if encoded_bytes > remaining_bytes {
                registry.io_frontier.restore(frontier_key, key);
                report.budget_exhausted = true;
                break;
            }
            let task_id = match take_task_id(&mut registry) {
                Some(task_id) => task_id,
                None => {
                    registry.io_frontier.restore(frontier_key, key);
                    return Err(RenderArtifactBlockIoDispatchError::TaskIdExhausted);
                }
            };
            let weak_loader = Arc::downgrade(self);
            let entry_for_work = Arc::clone(&entry);
            let store = self.store.clone();
            let store_limits = self.limits.store_limits();
            let metrics = Arc::clone(&self.metrics);
            let task = self.scope.submit(
                TaskDescriptor::new(
                    TaskId::new(task_id),
                    TaskPoolKind::Io,
                    "render-artifact-block-read",
                ),
                move |cancellation| {
                    run_io_task(
                        weak_loader,
                        entry_for_work,
                        store,
                        store_limits,
                        metrics,
                        cancellation,
                    );
                },
            );
            let task = match task {
                Ok(task) => task,
                Err(error) => {
                    registry.io_frontier.restore(frontier_key, key);
                    return Err(error.into());
                }
            };
            entry.install_task(task);
            self.metrics
                .submitted_io_tasks
                .fetch_add(1, Ordering::Relaxed);
            report.submitted_tasks = report.submitted_tasks.saturating_add(1);
            report.submitted_encoded_bytes =
                report.submitted_encoded_bytes.saturating_add(encoded_bytes);
        }
        report.remaining_queued_entries = registry.io_frontier.queued_len();
        if report.remaining_queued_entries > 0
            && (report.submitted_tasks == task_limit
                || report.submitted_encoded_bytes == budget.max_encoded_bytes())
        {
            report.budget_exhausted = true;
        }
        Ok(report)
    }
}
