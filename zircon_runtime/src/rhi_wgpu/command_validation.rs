...
use crate::rhi::{
    BindGroupDesc, BindGroupHandle, BufferDesc, BufferHandle, BufferUsage, CommandListCommand,
    IndexFormat, PipelineDesc, PipelineHandle, PipelineKind, RenderQueueClass, RhiError,
    TextureUsage, VertexBufferLayoutDesc,
};
...
#[derive(Default)]
struct RecordedRenderState<'a> {
    current_pipeline: Option<(PipelineHandle, PipelineKind, &'a PipelineDesc)>,
...
impl<'a> RecordedRenderState<'a> {
    fn set_pipeline(&mut self, pipeline: PipelineHandle, desc: &'a PipelineDesc) {
    ...
    fn set_vertex_buffer(&mut self, slot: u32, size_bytes: u64) {
        self.vertex_buffers
            .insert(slot, VertexBufferBinding { size_bytes });
    }

    fn require_raster_pipeline(&self) -> Result<&PipelineDesc, RhiError> {
    ...
    fn require_compute_pipeline(&self) -> Result<&PipelineDesc, RhiError> {
        match self.current_pipeline {
    ...
    fn ensure_required_bind_groups(
        &self,
        state: &WgpuRenderDeviceState,
    ...
    fn ensure_required_vertex_buffers(&self, pipeline: &PipelineDesc) -> Result<(), RhiError> {
