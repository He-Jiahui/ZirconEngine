...
use super::feature_project_selection_target_modes;
...
#[test]
fn optimization_batch_20260826gi_runtime229_bitset_preserves_first_target_mode_order() {
    let feature = PluginFeatureBundleManifest::new("rendering.path", "Render Path", "rendering")
        .with_runtime_module(
            PluginModuleManifest::runtime("rendering.path.runtime", "rendering_path_runtime")
                .with_target_modes([
        ...
        .with_editor_module(
            PluginModuleManifest::editor("rendering.path.editor", "rendering_path_editor")
                .with_target_modes([
                    RuntimeTargetMode::ServerRuntime,
    ...
    assert_eq!(
        feature_project_selection_target_modes(&feature),
        vec![
            RuntimeTargetMode::ClientRuntime,
            RuntimeTargetMode::ServerRuntime,
            RuntimeTargetMode::EditorHost,
...
#[test]
fn optimization_batch_20260826gi_runtime229_feature_projection_uses_target_mode_bits() {
    let source = include_str!("../target_modes.rs");

    assert!(source.contains("let mut seen_target_modes = 0_u8;"));
    assert!(source.contains("let bit = feature_target_mode_bit(target_mode);"));
...
fn optimization_batch_20260826gi_runtime229_feature_target_mode_bitset_bench() {
    ...
    println!(
        "RUNTIME229_FEATURE_TARGET_MODE_BITSET_BENCH_V1 sample_pairs={SAMPLE_PAIRS} \
