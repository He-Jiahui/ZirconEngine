...
use super::validate_runtime_plugin_display_field;
...
fn measure(value: &str, optimized: bool) -> u128 {
    ...
    for _ in 0..CHECKS_PER_SAMPLE {
        if optimized {
            validate_runtime_plugin_display_field(
                "display_name",
                black_box(value),
                &mut diagnostics,
            );
...
#[test]
fn optimization_batch_20260829bc_runtime330_single_trim_display_preserves_diagnostics() {
    for value in ["Render Plugin", "", " ", " Render Plugin", "Render Plugin "] {
        let mut legacy = Vec::new();
        let mut optimized = Vec::new();
        legacy_validate("display_name", value, &mut legacy);
        validate_runtime_plugin_display_field("display_name", value, &mut optimized);
        assert_eq!(optimized, legacy, "{value:?}");
    }
}

#[test]
fn optimization_batch_20260829bc_runtime330_display_validation_uses_length_check() {
...
fn optimization_batch_20260829bc_runtime330_single_trim_plugin_display_bench() {
    let value = format!(
        "Runtime Plugin {}",
        "x".repeat(DISPLAY_BYTES - "Runtime Plugin ".len())
    );
    ...
    println!(
        "RUNTIME330_SINGLE_TRIM_PLUGIN_DISPLAY_BENCH_V1 sample_pairs={SAMPLE_PAIRS} \
checks_per_sample={CHECKS_PER_SAMPLE} display_bytes={DISPLAY_BYTES} \
legacy_full_display_comparisons=1 optimized_full_display_comparisons=0 \
