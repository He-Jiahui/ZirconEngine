...
#[cfg(test)]
mod tests {
    use super::validate_runtime_plugin_package_root_segments;

    #[test]
    fn optimization_batch_gh_runtime491_root_segments_preserves_path_rules() {
        let valid = ["assets/materials", "plugins/render.v2/shaders"];
        for root in valid {
            let mut diagnostics = Vec::new();
            validate_runtime_plugin_package_root_segments("root", root, &mut diagnostics);
    ...
    fn optimization_batch_gh_runtime491_root_segments_benchmark() {
        const MARKER: &str = "RUNTIME491_ROOT_SEGMENTS_SCAN_BENCH_V1";
        const SAMPLE: &str =
        ...
        for _ in 0..ITERATIONS {
            diagnostics.clear();
            validate_runtime_plugin_package_root_segments("root", SAMPLE, &mut diagnostics);
