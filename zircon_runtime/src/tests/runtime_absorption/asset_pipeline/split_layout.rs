...
fn assert_docs_and_status_mirror_split() {
    for (label, source) in [
        ...
        assert!(
            ...
            "{label} should mirror the asset_pipeline route-owner split status"
        );
    }
    assert!(
        DATE_MAP.contains("Runtime 15 M3 asset-pipeline route-owner split"),
        "date map should mirror the asset_pipeline route-owner split slice"
    );
    assert_contains_all(
