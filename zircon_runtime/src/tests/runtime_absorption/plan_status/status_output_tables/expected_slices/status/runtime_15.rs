...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    foundation::expected_status_for_slice(slice)
        .or_else(|| naming_boundary::expected_status_for_slice(slice))
        .or_else(|| m4_surface_cleanup::expected_status_for_slice(slice))
        .or_else(|| m3_structure_support::expected_status_for_slice(slice))
}
