...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    index_baseline_maps::expected_status_for_slice(slice)
        .or_else(|| runtime_status_anchor_maps::expected_status_for_slice(slice))
        .or_else(|| cargo_attempt_maps::expected_status_for_slice(slice))
        .or_else(|| plan_status_guard_maps::expected_status_for_slice(slice))
        .or_else(|| support_inventory_maps::expected_status_for_slice(slice))
        .or_else(|| status_support_map_rows::expected_status_for_slice(slice))
}
