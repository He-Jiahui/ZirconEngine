...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    top_level_maps::expected_status_for_slice(slice)
        .or_else(|| structure_guard_maps::expected_status_for_slice(slice))
        .or_else(|| structure_assertion_maps::expected_status_for_slice(slice))
        .or_else(|| native_plugin_loader_maps::expected_status_for_slice(slice))
        .or_else(|| moved_guard_absence_maps::expected_status_for_slice(slice))
