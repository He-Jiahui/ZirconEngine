...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    expected_slice_rows::expected_status_for_slice(slice)
        .or_else(|| render_framework_rows::expected_status_for_slice(slice))
        .or_else(|| scene_render_rows::expected_status_for_slice(slice))
        .or_else(|| shader_model_rows::expected_status_for_slice(slice))
        .or_else(|| fixture_fallback_rows::expected_status_for_slice(slice))
        .or_else(|| plugin_texture_rows::expected_status_for_slice(slice))
        .or_else(|| asset_font_rows::expected_status_for_slice(slice))
}
