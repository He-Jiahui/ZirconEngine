...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    route_metadata_rows::expected_status_for_slice(slice)
        .or_else(|| root_route_rows::expected_status_for_slice(slice))
        .or_else(|| foundation_status_rows::expected_status_for_slice(slice))
        .or_else(|| route_children_rows::expected_status_for_slice(slice))
        .or_else(|| source_metadata_rows::expected_status_for_slice(slice))
        .or_else(|| status_map_rows::expected_status_for_slice(slice))
