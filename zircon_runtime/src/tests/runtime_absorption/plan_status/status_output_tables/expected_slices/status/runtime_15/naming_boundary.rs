...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    core_bootstrap::expected_status_for_slice(slice)
        .or_else(|| scene_asset_runtime::expected_status_for_slice(slice))
        .or_else(|| plugin_ui_platform::expected_status_for_slice(slice))
        .or_else(|| render_graphics::expected_status_for_slice(slice))
}
