...
const REVIEW_GUARD_ROW_DATA_FOLDER_BACKED_STATUS_ID: &str =
...
pub(super) fn expected_status_for_slice(slice: &str) -> Option<&'static str> {
    if slice == REVIEW_GUARD_ROW_DATA_FOLDER_BACKED_STATUS_NAME {
        return Some(REVIEW_GUARD_ROW_DATA_FOLDER_BACKED_STATUS_ID);
    }
    ...
    row_data_maps::expected_status_for_slice(slice)
        ...
        .or_else(|| runtime_row_data_maps::expected_status_for_slice(slice))
        .or_else(|| foundation_row_data_maps::expected_status_for_slice(slice))
        .or_else(|| m2_row_data_maps::expected_status_for_slice(slice))
        .or_else(|| hub_editor_maps::expected_status_for_slice(slice))
        .or_else(|| render_shader_maps::expected_status_for_slice(slice))
        .or_else(|| m3_m4_expected_slice_maps::expected_status_for_slice(slice))
        .or_else(|| root_layout_ui_maps::expected_status_for_slice(slice))
        .or_else(|| evidence_maps::expected_status_for_slice(slice))
}
