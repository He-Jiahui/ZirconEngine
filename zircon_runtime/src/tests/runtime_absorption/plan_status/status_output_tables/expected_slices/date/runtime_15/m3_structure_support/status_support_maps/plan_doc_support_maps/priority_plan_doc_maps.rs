...
pub(super) fn expected_date_for_slice(slice: &str) -> Option<&'static str> {
    integrity_guard_maps::expected_date_for_slice(slice)
        .or_else(|| guard_child_owner_maps::expected_date_for_slice(slice))
        .or_else(|| inventory_sync_maps::expected_date_for_slice(slice))
        .or_else(|| row_data_guard_maps::expected_date_for_slice(slice))
        .or_else(|| status_mirror_maps::expected_date_for_slice(slice))
