...
pub(super) fn expected_date_for_slice(slice: &str) -> Option<&'static str> {
    if let Some(date) = foundation_review_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
    if let Some(date) = code_review_guard_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
    if let Some(date) = typed_error_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
    if let Some(date) = typed_error_structure_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
    if let Some(date) = plugin_importer_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
    if let Some(date) = top_row_review_maps::expected_date_for_slice(slice) {
        return Some(date);
    }
