...
pub(super) fn expected_date_for_slice(slice: &str) -> Option<&'static str> {
    structure_route_maps::expected_date_for_slice(slice)
        .or_else(|| review_guard_maps::expected_date_for_slice(slice))
        .or_else(|| naming_guard_maps::expected_date_for_slice(slice))
        .or_else(|| status_support_maps::expected_date_for_slice(slice))
        .or_else(|| row_data_owner_maps::expected_date_for_slice(slice))
        .or_else(|| dead_code_guard_maps::expected_date_for_slice(slice))
        .or_else(|| lock_poison_module_maps::expected_date_for_slice(slice))
        .or_else(|| ui_maps::expected_date_for_slice(slice))
        .or_else(|| asset_budget_maps::expected_date_for_slice(slice))
        .or_else(|| runtime07_script_maps::expected_date_for_slice(slice))
        .or_else(|| plugin_export_gameplay_maps::expected_date_for_slice(slice))
        .or_else(|| scene_ecs_maps::expected_date_for_slice(slice))
}
