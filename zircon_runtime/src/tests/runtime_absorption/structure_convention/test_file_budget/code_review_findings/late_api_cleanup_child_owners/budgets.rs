...
#[test]
fn runtime_15_late_api_cleanup_structure_guard_budgets_are_focused() {
    ...
    for (path, source) in folder_backed_child_sources() {
        ...
        assert!(
            line_count < 220,
            "{path} should stay below the focused late API cleanup structure child budget; got {line_count} lines"
        );
    }
}
