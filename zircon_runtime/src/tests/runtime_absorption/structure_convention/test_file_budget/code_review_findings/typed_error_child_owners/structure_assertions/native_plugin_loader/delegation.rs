...
pub(super) fn assert_typed_error_native_plugin_loader_structure_is_child_owner(
    ...
    assert_contains_all(
        ...
        &sources.native_structure_child,
        &[
    ...
    for (_, child_path, anchor) in TYPED_ERROR_NATIVE_STRUCTURE_GUARD_CHILDREN {
        assert!(
            sources.native_structure_child.contains(child_path),
            "typed-error native plugin loader parent should inventory child path {child_path}"
        );
        assert!(
