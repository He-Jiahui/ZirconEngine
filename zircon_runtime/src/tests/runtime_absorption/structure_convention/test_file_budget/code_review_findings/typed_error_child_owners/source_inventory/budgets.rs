...
pub(super) fn assert_typed_error_source_inventory_children_line_budgets_are_current(
    ...
    for (path, source) in typed_error_source_inventory_delegation_child_sources() {
        let line_count = source.lines().count();
        assert!(
            line_count < TYPED_ERROR_CHILD_OWNER_LINE_BUDGET,
            "{path} should stay below the Runtime 15 test-file budget; got {line_count} lines"
    ...
    for (path, source) in typed_error_source_inventory_delegation_folder_backed_child_sources() {
        let line_count = source.lines().count();
        assert!(
            line_count < TYPED_ERROR_CHILD_OWNER_LINE_BUDGET,
            "{path} should stay below the Runtime 15 test-file budget; got {line_count} lines"
