...
fn runtime_15_typed_error_structure_guard_root_inventory_is_child_owned() {
    ...
    for (module_name, child_path, anchor) in TYPED_ERROR_ROOT_CHILDREN {
        let path_attr = format!("#[path = \"typed_error_child_owners/{module_name}.rs\"]");
        assert_contains_all(
            "typed-error structure guard parent mounts root-inventory child",
            &parent,
        ...
        assert!(
            child_source.lines().count() < TYPED_ERROR_ROOT_CHILD_LINE_BUDGET,
    ...
    let status_anchors = [
        TYPED_ERROR_ROOT_INVENTORY_SLICE,
    ...
    assert_contains_all(
        "M3 review status map records typed-error structure guard root inventory split",
        &status_map,
        &[
            TYPED_ERROR_ROOT_INVENTORY_SLICE,
            TYPED_ERROR_ROOT_INVENTORY_STATUS,
    ...
    assert_contains_all(
        "M3 review date map records typed-error structure guard root inventory split",
        &date_map,
        &[
            TYPED_ERROR_ROOT_INVENTORY_SLICE,
            TYPED_ERROR_ROOT_INVENTORY_DATE,
        ],
    );
}
