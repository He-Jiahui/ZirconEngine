...
pub(super) fn assert_typed_error_source_inventory_child_inventory_status_current_is_child_backed() {
    ...
    assert_contains_all(
        ...
        &[
            "#[path = \"child_inventory/delegation_children.rs\"]",
            "#[path = \"child_inventory/folder_backed_children.rs\"]",
            "#[path = \"child_inventory/folder_backed_ownership_children.rs\"]",
            "#[path = \"child_inventory/root_children.rs\"]",
            "#[path = \"child_inventory/source_helper_children.rs\"]",
            "#[path = \"child_inventory/status_current.rs\"]",
            "pub(super) use delegation_children::*;",
            "pub(super) use folder_backed_children::*;",
            "pub(super) use folder_backed_ownership_children::*;",
            "pub(super) use root_children::*;",
            "pub(super) use source_helper_children::*;",
        ],
    );
    for moved_anchor in [
        "pub(super) const TYPED_ERROR_SOURCE_INVENTORY_CHILDREN: &[",
        "pub(super) const TYPED_ERROR_SOURCE_INVENTORY_SOURCE_HELPER_CHILDREN: &[",
        "pub(super) const TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_CHILDREN: &[",
        "pub(super) const TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_FOLDER_BACKED_CHILDREN: &[",
        "pub(super) const TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_FOLDER_BACKED_OWNERSHIP_CHILDREN: &[",
    ] {
        assert!(
            !child_inventory_parent.contains(moved_anchor),
            "source_inventory/child_inventory.rs should delegate `{moved_anchor}` to focused children"
        );
    }
    for (_, child_path, anchor) in TYPED_ERROR_SOURCE_INVENTORY_CHILD_INVENTORY_CHILDREN {
        assert!(
            child_inventory_parent.contains(child_path),
            "typed-error source inventory child inventory parent should mount child path {child_path}"
        );
        assert!(
    ...
    {
        assert!(
            status_current_parent.contains(child_path),
            "typed-error source inventory child inventory status-current parent should mount child path {child_path}"
        );
        assert!(
