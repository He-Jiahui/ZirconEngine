...
fn runtime_15_code_review_findings_status_docs_folder_backed_status_is_current() {
    let parent = status_doc_route_inventory_source();
    let parent_file = read_runtime_src(STATUS_DOC_PARENT_PATH);
    ...
    for moved_anchor in [
        ...
        assert!(
            !parent_file.contains(moved_anchor),
    ...
    for (path, source) in status_doc_child_sources()
        .into_iter()
        .chain([(STATUS_DOC_PARENT_PATH, parent_file)])
    {
