...
#[test]
fn runtime_15_late_api_cleanup_structure_guard_folder_backed_status_is_current() {
    let runtime_15_plan =
    ...
    let status_map = format!(
        ...
        read_runtime_src(REVIEW_GUARD_LATE_API_STATUS_MAP)
    );
    let date_map = format!(
        "{}\n{}",
        read_runtime_src(REVIEW_GUARD_DATE_MAP),
        read_runtime_src(REVIEW_GUARD_LATE_API_DATE_MAP)
    );
