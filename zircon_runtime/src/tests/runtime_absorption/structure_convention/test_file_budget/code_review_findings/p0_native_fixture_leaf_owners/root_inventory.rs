...
fn runtime_15_p0_native_fixture_leaf_owner_root_inventory_is_child_owned() {
    ...
    for (module_name, child_path, anchor) in P0_NATIVE_FIXTURE_ROOT_CHILDREN {
        let path_attr = format!("#[path = \"p0_native_fixture_leaf_owners/{module_name}.rs\"]");
        assert_contains_all(
            "P0 native fixture leaf-owner parent mounts root-inventory child",
            &parent,
        ...
        assert!(
            child_source.lines().count() < P0_NATIVE_FIXTURE_ROOT_CHILD_LINE_BUDGET,
    ...
    let status_anchors = [
        P0_NATIVE_FIXTURE_ROOT_INVENTORY_SLICE,
    ...
    for (label, source) in [
        ("P0 native fixture row data", status_rows.as_str()),
    ...
    assert_contains_all(
        "M3 review status map records P0 native fixture root inventory split",
        &status_map,
        &[
            P0_NATIVE_FIXTURE_ROOT_INVENTORY_SLICE,
            P0_NATIVE_FIXTURE_ROOT_INVENTORY_STATUS,
    ...
    assert_contains_all(
        "M3 review date map records P0 native fixture root inventory split",
        &date_map,
        &[
            P0_NATIVE_FIXTURE_ROOT_INVENTORY_SLICE,
            P0_NATIVE_FIXTURE_ROOT_INVENTORY_DATE,
        ],
    );
}
