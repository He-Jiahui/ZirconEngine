...
pub(super) fn typed_error_status_maps_status_current_sources_guard_blob() -> String {
    let mut blob = read_runtime_src(
        TYPED_ERROR_STATUS_DOCS_STATUS_MAPS_STATUS_CURRENT_SOURCES_SOURCE_SPLIT_CHILD,
    );
    blob.push('\n');
    for child_path in typed_error_status_maps_status_current_sources_guard_children() {
...
pub(super) fn typed_error_status_maps_status_current_sources_guard_children(
) -> &'static [&'static str] {
    TYPED_ERROR_STATUS_DOCS_STATUS_MAPS_STATUS_CURRENT_SOURCES_SOURCE_SPLIT_CHILDREN
}
