...
pub(super) fn typed_error_native_plugin_loader_structure_source_tree(
    sources: &TypedErrorNativePluginLoaderSources,
) -> String {
    [
        sources.native_structure_child.as_str(),
        typed_error_native_plugin_loader_source_helper_child_source_blob().as_str(),
        typed_error_native_plugin_loader_child_source_blob().as_str(),
        sources.native_plugin_loader_parent.as_str(),
        sources.native_abi_surfaces_parent.as_str(),
        sources.native_plugin_descriptor_parent.as_str(),
        sources.native_live_host_parent.as_str(),
        sources.native_live_host_lifecycle_paths_parent.as_str(),
        sources.native_live_host_replay_and_runtime_parent.as_str(),
        sources.native_manifest_sources_parent.as_str(),
    ]
    .join("\n")
}
