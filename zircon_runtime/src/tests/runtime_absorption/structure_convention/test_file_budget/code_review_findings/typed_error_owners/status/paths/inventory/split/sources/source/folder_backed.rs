...
use super::source_tree::{
    typed_error_paths_child_inventory_split_layout_sources_guard_blob,
    typed_error_paths_child_inventory_split_layout_sources_guard_children,
};
...
#[test]
fn runtime_15_typed_error_status_doc_paths_child_inventory_split_layout_sources_guard_is_folder_backed(
) {
    let parent = read_runtime_src(
        TYPED_ERROR_STATUS_DOCS_PATHS_CHILD_INVENTORY_SPLIT_LAYOUT_SOURCES_SOURCE_SPLIT_CHILD,
    );
    let child_tree = typed_error_paths_child_inventory_split_layout_sources_guard_blob();

    assert_contains_all(
        "typed-error status-doc paths child-inventory split-layout source-split route mounts child owners",
        &parent,
        &[
    ...
    for moved_anchor in [
        "fn runtime_15_typed_error_status_doc_paths_child_inventory_split_layout_sources_are_child_backed",
        "fn assert_sources_child_split_status_is_current",
        "TYPED_ERROR_STATUS_DOCS_PATHS_CHILD_INVENTORY_CHILDREN: &[",
        "typed_error_status_doc_paths_child_inventory_split_layout_child_source_blob",
    ] {
        assert!(
            !parent.contains(moved_anchor),
            "typed-error paths child-inventory split-layout source-split route should not own moved anchor {moved_anchor}"
        );
        assert!(
            child_tree.contains(moved_anchor),
            "typed-error paths child-inventory split-layout source-split child tree should own moved anchor {moved_anchor}"
        );
    }
