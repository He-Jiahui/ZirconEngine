...
#[test]
fn runtime_15_typed_error_status_doc_status_mirrors_status_is_current() {
    assert_typed_error_status_mirrors_status_is_current();
}

pub(in super::super) fn assert_typed_error_status_mirrors_status_is_current() {
    ...
    for (label, source) in [
        ...
        assert_contains_all(
            ...
            source,
            &[
                TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_SPLIT_NAME,
                TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_SPLIT_ID,
                TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_CHILD,
                TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_CHILD_INVENTORY_CHILD,
    ...
    assert_contains_all(
        "M3 review date map records typed-error status-doc status mirrors split",
        &date_map,
        &[
            TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_SPLIT_NAME,
            TYPED_ERROR_STATUS_DOCS_STATUS_MIRRORS_SPLIT_DATE,
        ],
    );
}
