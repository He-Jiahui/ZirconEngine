...
pub(super) fn assert_typed_error_status_docs_folder_backed_status_is_current() {
    ...
    for (label, source) in [
        ...
        assert_contains_all(
            ...
            source,
            &[
                TYPED_ERROR_STATUS_DOCS_GUARD_SPLIT_NAME,
                TYPED_ERROR_STATUS_DOCS_GUARD_SPLIT_ID,
    ...
    assert_contains_all(
        "M3 review date map records typed-error status-doc folder-backed split",
        &date_map,
        &[TYPED_ERROR_STATUS_DOCS_GUARD_SPLIT_NAME, "2026-07-02"],
    );
}
