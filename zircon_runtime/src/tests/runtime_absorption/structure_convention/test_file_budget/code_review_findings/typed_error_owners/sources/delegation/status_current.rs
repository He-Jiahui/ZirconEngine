...
#[test]
fn runtime_15_typed_error_source_inventory_delegation_status_is_current() {
    ...
    for (label, source) in [
        ...
        assert_contains_all(
            ...
            source,
            &[
                TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_SPLIT,
                TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_STATUS,
    ...
    assert_contains_all(
        "M3 review status map records typed-error source inventory delegation split",
        &status_map,
        &[
            TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_SPLIT,
            TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_STATUS,
    ...
    assert_contains_all(
        "M3 review date map records typed-error source inventory delegation split",
        &date_map,
        &[
            TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_SPLIT,
            TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_DATE,
        ],
    );
}
