...
use super::super::super::super::sources::typed_error_status_row_source;
use super::super::super::review_guard_paths::{
...
use super::super::super::status_slices::{
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD,
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD_SPLIT_DATE,
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD_SPLIT_ID,
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD_SPLIT_NAME,
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_GUARD, TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_DATE,
    TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_ID, TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_NAME,
};
use super::super::path_children::TYPED_ERROR_STATUS_DOCS_PATHS_ROOT_PATHS_CHILD;
use super::super::source_tree::typed_error_status_doc_root_paths_child_source_paths;
...
pub(super) fn assert_root_paths_folder_backed_status_is_current() {
    let anchors = [
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_NAME,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_ID,
        TYPED_ERROR_STATUS_DOCS_PATHS_ROOT_PATHS_CHILD,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_GUARD,
        "Cargo gate deferred",
    ];

    assert_status_documents_contain("typed-error root paths folder-backed split", &anchors);
    ...
    for path in typed_error_status_doc_root_paths_child_source_paths() {
        assert!(
            ...
            "typed-error root paths status row should include child source {path}"
        );
    }
    assert_status_maps_contain(
        "typed-error root paths folder-backed split",
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_NAME,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_ID,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_SPLIT_DATE,
    );
}

pub(super) fn assert_root_paths_folder_backed_guard_status_is_current() {
    let anchors = [
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD_SPLIT_NAME,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_GUARD_SPLIT_ID,
        TYPED_ERROR_STATUS_DOCS_ROOT_PATHS_FOLDER_BACKED_CHILD,
    ...
    assert_status_documents_contain(
        "typed-error root paths folder-backed guard folder-backed split",
        &anchors,
    );
    let status_rows = typed_error_status_row_source();
    for path in typed_error_status_doc_root_paths_folder_backed_guard_children() {
        assert!(
            status_rows.contains(path),
            "typed-error root paths folder-backed guard row should include child source {path}"
        );
    }
    assert_status_maps_contain(
        "typed-error root paths folder-backed guard folder-backed split",
