...
use super::source_tree::{
    ...
    typed_error_status_doc_paths_status_current_split_layout_status_mirrors_child_source_blob,
};
use super::status_current::{
    assert_typed_error_status_doc_paths_status_current_split_layout_guard_status_is_current,
    assert_typed_error_status_doc_paths_status_current_split_layout_status_is_current,
    assert_typed_error_status_doc_paths_status_current_split_layout_status_mirrors_guard_status_is_current,
    assert_typed_error_status_doc_paths_status_current_status_is_current,
};

#[test]
fn runtime_15_typed_error_status_doc_paths_status_current_split_layout_status_mirrors_guard_is_folder_backed(
) {
    ...
    assert_contains_all(
        "typed-error status-doc paths status-current split-layout status-mirrors route mounts child owners",
        &parent,
        &[
    ...
    for moved_anchor in [
        ...
        "fn assert_typed_error_status_doc_paths_status_current_split_layout_status_is_current",
        "fn assert_typed_error_status_doc_paths_status_current_split_layout_guard_status_is_current",
        "fn assert_status_documents_contain",
        "REVIEW_GUARD_STATUS_MAP_PATH",
    ] {
        assert!(
            !parent.contains(moved_anchor),
            "typed-error paths status-current split-layout status-mirrors route should not own moved anchor {moved_anchor}"
        );
        assert!(
            child_tree.contains(moved_anchor),
            "typed-error paths status-current split-layout status-mirrors child tree should own moved anchor {moved_anchor}"
        );
    }
