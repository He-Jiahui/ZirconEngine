...
pub(in super::super) fn assert_typed_error_source_inventory_metadata_status_current_is_child_backed(
) {
    ...
    for moved_anchor in [
        "pub(in super::super) const TYPED_ERROR_STRUCTURE_CHILD",
        "pub(in super::super) const TYPED_ERROR_SOURCE_INVENTORY_CHILD_INVENTORY_STATUS_CURRENT_ROUTE_CHILD",
        "pub(in super::super) const TYPED_ERROR_SOURCE_INVENTORY_DELEGATION_FOLDER_BACKED_OWNERSHIP_CHILD_BACKED_GUARD",
        "pub(in super::super) const TYPED_ERROR_CHILD_OWNER_LINE_BUDGET",
        "pub(in super::super) const REVIEW_GUARD_STATUS_ROWS_PATH",
    ] {
        assert!(
            !metadata_parent.contains(moved_anchor),
            "sources/metadata.rs should delegate `{moved_anchor}` to focused children"
        );
    }
