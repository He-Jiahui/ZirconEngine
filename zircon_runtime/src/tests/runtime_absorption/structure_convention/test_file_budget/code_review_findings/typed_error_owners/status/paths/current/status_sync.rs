...
#[test]
fn runtime_15_typed_error_status_doc_paths_status_is_current() {
    assert_typed_error_status_doc_paths_status_is_current();
}

pub(in super::super) fn assert_typed_error_status_doc_paths_status_is_current() {
    ...
    for (label, source) in [
        ...
        assert_contains_all(
            ...
            source,
            &[
                TYPED_ERROR_STATUS_DOCS_PATHS_SPLIT_NAME,
                TYPED_ERROR_STATUS_DOCS_PATHS_SPLIT_ID,
    ...
    assert_contains_all(
        "M3 review date map records typed-error status-doc paths split",
        &date_map,
        &[
            TYPED_ERROR_STATUS_DOCS_PATHS_SPLIT_NAME,
            TYPED_ERROR_STATUS_DOCS_PATHS_SPLIT_DATE,
        ],
    );
}
