...
#[test]
fn runtime_15_m2_row_data_children_guard_folder_backed_status_mirrors_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 naming-boundary expected status map owns M2 row-data children guard folder-backed split",
        &status_map,
        &[FOLDER_BACKED_STATUS_NAME, FOLDER_BACKED_STATUS_ID],
    );
    assert_contains_all(
        "Runtime 15 naming-boundary expected date map owns M2 row-data children guard folder-backed split",
        &date_map,
