...
fn runtime_15_status_output_row_data_module_layout_status_docs_are_child_owner() {
    let production_guard_support = format!(
        "{}\n{}",
        read_runtime_src(PRODUCTION_GUARD_SUPPORT_MODULE_LAYOUT_BASE_ROWS_PATH),
        read_runtime_src(PRODUCTION_GUARD_SUPPORT_MODULE_LAYOUT_ROWS_PATH),
    );
    let expected_status_map = format!(
        "{}\n{}",
        read_runtime_src(STATUS_SUPPORT_ROOT_RUNTIME_STATUS_MAP_PATH),
        read_runtime_src(EXPECTED_STATUS_MAP_PATH),
    );
    let expected_date_map = format!(
        "{}\n{}",
        read_runtime_src(STATUS_SUPPORT_ROOT_RUNTIME_DATE_MAP_PATH),
        read_runtime_src(EXPECTED_DATE_MAP_PATH),
    );
    let runtime_15_plan =
