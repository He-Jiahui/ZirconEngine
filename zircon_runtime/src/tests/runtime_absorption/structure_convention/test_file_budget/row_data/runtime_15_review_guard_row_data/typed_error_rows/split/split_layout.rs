...
#[test]
fn runtime_15_review_guard_typed_error_rows_split_layout_guard_is_folder_backed() {
    let parent = read_runtime_src(TYPED_ERROR_ROWS_SPLIT_LAYOUT_PATH);
    let route_mounts = read_runtime_src(TYPED_ERROR_ROWS_SPLIT_LAYOUT_ROUTE_MOUNTS_CHILD_PATH);
    ...
    assert_contains_all(
        "review-guard typed-error rows split-layout mounts focused children",
        &parent,
        &[
    ...
    for moved_anchor in [
        ...
        "should stay under its focused typed-error rows guard budget",
    ] {
        assert!(
            !parent.contains(moved_anchor),
            "typed_error_rows/split_layout.rs should stay a route owner and delegate {moved_anchor}"
        );
    }
    assert_contains_all(
        "review-guard typed-error rows split-layout children retain moved checks",
        &format!("{route_mounts}\n{status_current}\n{budgets}\n{split_layout}"),
        &[
            TYPED_ERROR_ROWS_GUARD_FOLDER_BACKED_GUARD_NAME,
            "runtime_15_review_guard_typed_error_rows_guard_status_is_current",
            "runtime_15_review_guard_typed_error_rows_guard_budgets_are_current",
            TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_GUARD_NAME,
        ],
    );

    let status_anchors = [
        TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_STATUS_NAME,
        TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_STATUS_ID,
    ...
    assert_contains_all(
        "review-guard status-support rows record typed-error rows split-layout folder-backed split",
        &status_rows,
        &status_anchors,
    );
    assert_contains_all(
        "review status map records typed-error rows split-layout folder-backed split",
        &status_map,
        &[
            TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_STATUS_NAME,
            TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_STATUS_ID,
    ...
    assert_contains_all(
        "review date map records typed-error rows split-layout folder-backed split",
        &date_map,
        &[
            TYPED_ERROR_ROWS_SPLIT_LAYOUT_GUARD_FOLDER_BACKED_STATUS_NAME,
            "2026-07-06",
