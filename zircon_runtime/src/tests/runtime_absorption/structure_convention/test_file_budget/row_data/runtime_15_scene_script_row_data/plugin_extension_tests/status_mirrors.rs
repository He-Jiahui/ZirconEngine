...
pub(super) fn assert_plugin_extension_row_data_status_is_current() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            PLUGIN_EXTENSION_ROW_DATA_CHILD_SPLIT_STATUS_ID,
        ],
    );
    assert_contains_all(
        "M3 date map records plugin-extension row-data child split",
        &date_map,
        &[
            PLUGIN_EXTENSION_ROW_DATA_CHILD_SPLIT_STATUS_NAME,
            "2026-07-07",
...
pub(super) fn assert_plugin_extension_guard_status_is_current() {
    let status_map = read_runtime_src(STATUS_ROW_DATA_STATUS_MAP_PATH);
    let date_map = read_runtime_src(STATUS_ROW_DATA_DATE_MAP_PATH);
    let row_data_owner = read_runtime_src(SCENE_SCRIPT_PLUGIN_EXTENSION_ROW_DATA_OWNER_PATH);

    assert_contains_all(
        "plugin-extension row-data owner records guard folder-backed split",
        &row_data_owner,
        &[
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_STATUS_NAME,
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_STATUS_ID,
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_GUARD_NAME,
        ],
    );
    assert_contains_all(
        "M3 status map records plugin-extension guard folder-backed split",
        &status_map,
        &[
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_STATUS_NAME,
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records plugin-extension guard folder-backed split",
        &date_map,
        &[
            PLUGIN_EXTENSION_GUARD_FOLDER_BACKED_STATUS_NAME,
            "2026-07-07",
