...
#[test]
fn runtime_15_scene_script_row_data_guard_folder_backed_status_mirrors_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map owns scene-script row-data guard folder-backed split",
        &status_map,
        &[FOLDER_BACKED_STATUS_NAME, FOLDER_BACKED_STATUS_ID],
    );
    assert_contains_all(
        "Runtime 15 status-support date map owns scene-script row-data guard folder-backed split",
        &date_map,
