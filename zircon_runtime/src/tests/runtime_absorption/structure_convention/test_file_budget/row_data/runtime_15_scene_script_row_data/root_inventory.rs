...
#[test]
fn runtime_15_scene_script_row_data_root_inventory_is_child_owned() {
    let parent = read_runtime_src(SCENE_SCRIPT_ROW_DATA_GUARD_PATH);
    ...
    assert_contains_all(
        "scene-script row-data root mounts inventory children",
        &parent,
        &[
    ...
    assert_contains_all(
        "M3 status map records scene-script root inventory split",
        &status_map,
        &[
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME,
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records scene-script root inventory split",
        &date_map,
