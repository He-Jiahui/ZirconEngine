...
fn runtime_15_asset_budget_row_data_owner_is_child_backed() {
    ...
    let asset_tests_project_rows = read_runtime_src(ASSET_BUDGET_ASSET_TESTS_PROJECT_ROWS_PATH);
    let asset_tests_asset_resource_rows =
        read_runtime_src(ASSET_BUDGET_ASSET_TESTS_ASSET_RESOURCE_ROWS_PATH);
    let asset_tests_ui_pipeline_rows =
        read_runtime_src(ASSET_BUDGET_ASSET_TESTS_UI_PIPELINE_ROWS_PATH);
    ...
    let naming_graphics_misc_root_route_rows =
        read_runtime_src(ASSET_BUDGET_NAMING_GRAPHICS_MISC_ROOT_ROUTE_ROWS_PATH);
    let naming_graphics_misc_graphics_asset_rows =
        read_runtime_src(ASSET_BUDGET_NAMING_GRAPHICS_MISC_GRAPHICS_ASSET_ROWS_PATH);
    let naming_graphics_misc_scene_platform_rows =
        read_runtime_src(ASSET_BUDGET_NAMING_GRAPHICS_MISC_SCENE_PLATFORM_ROWS_PATH);
    let naming_graphics_misc_plugin_banned_rows =
        read_runtime_src(ASSET_BUDGET_NAMING_GRAPHICS_MISC_PLUGIN_BANNED_ROWS_PATH);
    let row_data_owner = read_runtime_src(ASSET_BUDGET_ROW_DATA_OWNER_PATH);
    let row_children = [
        runtime_rhi.as_str(),
        asset_tests.as_str(),
        asset_tests_project_rows.as_str(),
        asset_tests_asset_resource_rows.as_str(),
        asset_tests_ui_pipeline_rows.as_str(),
        budget_render_ui.as_str(),
        naming_core_asset_dynamic.as_str(),
        naming_graphics_misc.as_str(),
        naming_graphics_misc_root_route_rows.as_str(),
        naming_graphics_misc_graphics_asset_rows.as_str(),
        naming_graphics_misc_scene_platform_rows.as_str(),
        naming_graphics_misc_plugin_banned_rows.as_str(),
