...
#[test]
fn runtime_15_asset_budget_row_data_folder_backed_status_mirrors_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map owns asset-budget row-data guard folder-backed split",
        &status_map,
        &[FOLDER_BACKED_STATUS_NAME, FOLDER_BACKED_STATUS_ID],
    );
    assert_contains_all(
        "Runtime 15 status-support date map owns asset-budget row-data guard folder-backed split",
        &date_map,
