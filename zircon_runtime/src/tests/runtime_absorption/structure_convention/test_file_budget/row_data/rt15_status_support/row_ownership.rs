...
fn runtime_15_status_support_row_data_owner_is_child_backed() {
    ...
    let expected_slice_map_children = EXPECTED_SLICE_MAP_CHILDREN
        .iter()
    ...
    let row_data_and_budget_child_rows = row_data_and_budget_children
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let expected_slice_map_child_rows = expected_slice_map_children
        .iter()
        .map(String::as_str)
        .collect::<Vec<_>>()
        .join("\n");
    let runtime_index_anchor_child_rows = RUNTIME_INDEX_ANCHOR_CHILDREN
        .iter()
        .map(|(_, path, _)| read_runtime_src(path))
        .collect::<Vec<_>>()
        .join("\n");
    let row_children = [
        ...
        expected_slice_map_child_rows.as_str(),
        runtime_index_anchors.as_str(),
        runtime_index_anchor_child_rows.as_str(),
    ...
    assert_contains_all(
        ...
        &[
            ...
            "expected_slice_maps::BASE_MAPS_EXPECTED_STATUS_OUTPUT_SLICES",
