pub(super) const CHILD_OWNER_STATUS_NAME: &str =
    "Runtime 15 M3 foundation-guards row-data owner child split";
pub(super) const CHILD_OWNER_STATUS_ID: &str =
    "runtime_15_foundation_guards_row_data_owner_child_split_static_passed_cargo_deferred";
pub(super) const CHILD_OWNER_GUARD_NAME: &str =
    "runtime_15_foundation_guards_row_data_owner_is_child_backed";
pub(super) const FOLDER_BACKED_STATUS_NAME: &str =
    "Runtime 15 M3 foundation-guards row-data guard folder-backed split";
pub(super) const FOLDER_BACKED_STATUS_ID: &str =
    "runtime_15_foundation_guards_row_data_guard_folder_backed_static_passed_cargo_deferred";
pub(super) const FOLDER_BACKED_GUARD_NAME: &str =
    "runtime_15_foundation_guards_row_data_guard_is_folder_backed";
pub(super) const ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME: &str =
    "Runtime 15 M3 foundation-guards row-data root inventory child split";
pub(super) const ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID: &str =
    "runtime_15_foundation_guards_row_data_root_inventory_child_split_static_passed_cargo_deferred";
pub(super) const ROOT_INVENTORY_CHILD_SPLIT_GUARD_NAME: &str =
    "runtime_15_foundation_guards_row_data_root_inventory_is_child_owned";
pub(super) const RUNTIME_STRUCTURE_ROW_DATA_CHILD_SPLIT_STATUS_NAME: &str =
    "Runtime 15 M3 foundation-guards runtime-structure row-data child split";
pub(super) const RUNTIME_STRUCTURE_ROW_DATA_CHILD_SPLIT_STATUS_ID: &str =
    "runtime_15_foundation_guards_runtime_structure_row_data_child_split_static_passed_cargo_deferred";
pub(super) const RUNTIME_STRUCTURE_ROW_DATA_CHILD_SPLIT_GUARD_NAME: &str =
    "runtime_15_foundation_guards_runtime_structure_row_data_is_child_backed";
pub(super) const RUNTIME_STRUCTURE_GUARD_FOLDER_BACKED_STATUS_NAME: &str =
    "Runtime 15 M3 foundation-guards runtime-structure guard folder-backed split";
pub(super) const RUNTIME_STRUCTURE_GUARD_FOLDER_BACKED_STATUS_ID: &str =
    "runtime_15_foundation_guards_runtime_structure_guard_folder_backed_static_passed_cargo_deferred";
pub(super) const RUNTIME_STRUCTURE_GUARD_FOLDER_BACKED_GUARD_NAME: &str =
    "runtime_15_foundation_guards_runtime_structure_guard_is_folder_backed";
