...
#[test]
fn runtime_15_evidence_anchors_root_inventory_is_child_owned() {
    let parent = read_runtime_src(EVIDENCE_ANCHORS_GUARD_PATH);
    let status_rows = read_runtime_src(RUNTIME_15_M3_PRODUCTION_GUARD_SUPPORT_ROW_DATA_PATH);
    ...
    assert_contains_all(
        "Runtime 15 evidence anchors root mounts inventory children",
        &parent,
        &[
    ...
    assert_contains_all(
        "M3 status map records evidence anchors root inventory split",
        &status_map,
        &[
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME,
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records evidence anchors root inventory split",
        &date_map,
