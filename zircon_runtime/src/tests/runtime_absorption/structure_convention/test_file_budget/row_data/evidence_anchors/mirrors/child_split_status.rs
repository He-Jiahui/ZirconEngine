...
#[test]
fn runtime_15_evidence_anchors_status_mirror_status_rows_are_current() {
    ...
    for (label, source) in [
        ...
        (
            "Runtime 15 M3 production guard support row data",
    ...
    assert_contains_all(
        "Runtime 15 status-support map records evidence anchors status-mirror split",
        &status_map,
        &[STATUS_MIRROR_CHILD_SPLIT_NAME, STATUS_MIRROR_CHILD_SPLIT_ID],
    );
    assert_contains_all(
        "Runtime 15 date map records evidence anchors status-mirror split",
        &date_map,
