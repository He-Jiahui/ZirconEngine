...
fn runtime_15_m4_row_data_children_guard_is_folder_backed() {
    ...
    assert_contains_all(
        "Runtime 15 M4 row-data children root statuses preserve historical status anchors",
        &root_statuses,
        &[
    ...
    for (_, child_path, guard_name) in M4_ROW_DATA_CHILDREN_GUARD_CHILDREN {
        assert!(
            ...
            "M4 row-data children root child inventory should mount child path {child_path}"
        );
        assert!(
