...
#[test]
fn runtime_15_runtime_15_row_data_root_inventory_is_child_owned() {
    let parent = read_runtime_src(RUNTIME_15_ROW_DATA_GUARD_PATH);
    let status_rows = read_runtime_src(RUNTIME_15_ROW_DATA_STATUS_ROWS_PATH);
    ...
    assert_contains_all(
        "Runtime 15 row-data root mounts inventory children",
        &parent,
        &[
            "#[path = \"runtime_15_row_data/root_paths.rs\"]",
            "#[path = \"runtime_15_row_data/root_statuses.rs\"]",
            "#[path = \"runtime_15_row_data/root_child_rows.rs\"]",
            "#[path = \"runtime_15_row_data/root_owner_paths.rs\"]",
            "#[path = \"runtime_15_row_data/root_inventory.rs\"]",
        ],
    );
    ...
    for (label, source) in [
        ...
        ("Runtime 15 row-data rows", status_rows.as_str()),
    ] {
        assert_contains_all(label, source, &status_anchors);
    }
    assert_contains_all(
        "M3 status map records Runtime 15 row-data root inventory split",
        &status_map,
        &[
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME,
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records Runtime 15 row-data root inventory split",
        &date_map,
