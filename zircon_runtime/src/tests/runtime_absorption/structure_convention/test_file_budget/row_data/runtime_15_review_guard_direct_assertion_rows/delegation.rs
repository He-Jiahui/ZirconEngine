...
fn runtime_15_review_guard_direct_assertion_rows_guard_is_folder_backed() {
    ...
    assert_contains_all(
        "review-guard direct-assertion guard records folder-backed split anchors",
    ...
    for (_, child_path, guard_name) in DIRECT_ASSERTION_GUARD_CHILDREN {
        assert!(
            ...
            "review-guard direct-assertion child inventory should list child path {child_path}"
        );
        assert!(
