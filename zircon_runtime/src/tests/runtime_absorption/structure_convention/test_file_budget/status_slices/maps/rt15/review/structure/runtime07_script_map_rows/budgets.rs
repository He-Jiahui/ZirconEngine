...
#[test]
fn runtime_15_runtime_07_script_expected_slice_maps_guard_children_stay_budgeted() {
    for (path, limit) in [
        ...
        assert!(
            line_count <= limit,
            "{path} should stay below the Runtime 07/script map guard budget {limit}; got {line_count} lines"
        );
    }
}
