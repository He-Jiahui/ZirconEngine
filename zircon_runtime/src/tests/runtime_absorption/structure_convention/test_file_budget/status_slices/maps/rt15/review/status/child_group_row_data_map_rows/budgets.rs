...
#[test]
fn runtime_15_status_support_child_group_row_data_maps_guard_children_stay_budgeted() {
    for (path, limit) in [
        ...
        assert!(
            line_count <= limit,
            "{path} should stay below the child-group row-data map guard budget {limit}; got {line_count} lines"
        );
    }
}
