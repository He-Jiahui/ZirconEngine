...
fn runtime_15_status_output_runtime_15_expected_slice_child_owner_guard_body_route_mounts_status_is_synced(
    ...
    assert_contains_all(
        "Runtime 15 child-owner guard-body route-mount row data",
        &row_data,
        &[
            ROUTE_MOUNTS_SLICE,
            ROUTE_MOUNTS_STATUS,
            ...
            GUARD_BODY_ROUTE_MOUNTS_CHILDREN[5],
            ROUTE_MOUNTS_GUARD,
            "Cargo gate deferred",
        ],
    );
    assert_contains_all(
        "Runtime 15 child-owner guard-body route-mount status/date maps",
        &format!("{status_map}\n{date_map}"),
    ...
    for (label, source) in [
        ...
        assert_contains_all(
            ...
            &[
                ...
                ROUTE_MOUNTS_STATUS,
                ROUTE_MOUNTS_GUARD,
                "owners/split/body/mounts/status_mirrors.rs",
            ],
        );
    }
    ...
    assert_contains_all(
        "Frameworks 02 child-owner route-mount mirror",
        &frameworks,
        &[
            ROUTE_MOUNTS_SLICE,
            ROUTE_MOUNTS_STATUS,
