...
#[test]
fn runtime_15_runtime_15_row_data_status_mirror_status_rows_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map records Runtime 15 row-data status-mirror split",
        &status_map,
        &[STATUS_MIRROR_CHILD_SPLIT_NAME, STATUS_MIRROR_CHILD_SPLIT_ID],
    );
    assert_contains_all(
        "Runtime 15 date map records Runtime 15 row-data status-mirror split",
        &date_map,
