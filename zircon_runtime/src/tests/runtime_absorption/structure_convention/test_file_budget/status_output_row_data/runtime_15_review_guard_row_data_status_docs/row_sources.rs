...
fn runtime_15_status_output_review_guard_row_data_status_docs_are_child_owner() {
    ...
    let status_maps_guard = read_runtime_src(
    ...
    let status_mirrors_guard = read_runtime_src(
    ...
    let status_doc_source_children = [
        status_docs_guard.as_str(),
        status_maps_guard.as_str(),
        status_mirrors_guard.as_str(),
    ]
    .join("\n");
