...
#[test]
fn runtime_15_asset_budget_row_data_root_inventory_is_child_owned() {
    let parent = read_runtime_src(ASSET_BUDGET_ROW_DATA_GUARD_PATH);
    ...
    assert_contains_all(
        "asset-budget row-data root mounts inventory children",
        &parent,
        &[
    ...
    assert_contains_all(
        "M3 status map records asset-budget root inventory split",
        &status_map,
        &[
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME,
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records asset-budget root inventory split",
        &date_map,
