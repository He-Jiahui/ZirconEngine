...
#[test]
fn runtime_15_m3_child_group_status_doc_status_mirror_status_rows_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map records child-groups status-doc status-mirror split",
        &status_map,
        &[STATUS_MIRROR_CHILD_SPLIT_NAME, STATUS_MIRROR_CHILD_SPLIT_ID],
    );
    assert_contains_all(
        "Runtime 15 date map records child-groups status-doc status-mirror split",
        &date_map,
