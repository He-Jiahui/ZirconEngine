...
#[test]
fn runtime_15_m2_row_data_children_status_mirror_status_rows_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 naming-boundary status map records M2 row-data children status-mirror split",
        &status_map,
        &[STATUS_MIRROR_CHILD_SPLIT_NAME, STATUS_MIRROR_CHILD_SPLIT_ID],
    );
    assert_contains_all(
        "Runtime 15 naming-boundary date map records M2 row-data children status-mirror split",
        &date_map,
