...
#[test]
fn runtime_15_m3_row_data_guard_folder_backed_status_mirrors_are_current() {
    let status_map = read_runtime_src(STATUS_SUPPORT_STATUS_MAP_PATH);
    let date_map = read_runtime_src(STATUS_SUPPORT_DATE_MAP_PATH);
    let status_support_row_data =
        read_runtime_src(RUNTIME_15_M3_STATUS_SUPPORT_ROW_DATA_AND_BUDGET_PATH);
    let runtime_15_plan =
    ...
    assert_contains_all(
        "Runtime 15 status-support map owns Runtime 15 M3 row-data guard folder-backed split",
        &status_map,
        &[FOLDER_BACKED_STATUS_NAME, FOLDER_BACKED_STATUS_ID],
    );
    assert_contains_all(
        "Runtime 15 status-support date map owns Runtime 15 M3 row-data guard folder-backed split",
        &date_map,
