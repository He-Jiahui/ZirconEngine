...
#[test]
fn runtime_15_scene_script_row_data_status_mirror_status_rows_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map records scene-script row-data status-mirror split",
        &status_map,
        &[STATUS_MIRROR_CHILD_SPLIT_NAME, STATUS_MIRROR_CHILD_SPLIT_ID],
    );
    assert_contains_all(
        "Runtime 15 date map records scene-script row-data status-mirror split",
        &date_map,
