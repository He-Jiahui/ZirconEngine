...
#[test]
fn runtime_15_m3_child_groups_root_inventory_is_child_owned() {
    let parent = read_runtime_src(M3_CHILD_GROUPS_GUARD_PATH);
    let status_rows = read_runtime_src(PRODUCTION_GUARD_SUPPORT_CORE_AND_EVIDENCE_ROWS_PATH);
    ...
    assert_contains_all(
        "M3 child-groups root mounts inventory children",
        &parent,
        &[
    ...
    assert_contains_all(
        "M3 status map records child-groups root inventory split",
        &status_map,
        &[
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_NAME,
            ROOT_INVENTORY_CHILD_SPLIT_STATUS_ID,
    ...
    assert_contains_all(
        "M3 date map records child-groups root inventory split",
        &date_map,
