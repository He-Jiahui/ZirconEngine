...
#[test]
fn runtime_15_m3_child_groups_row_data_guard_folder_backed_status_mirrors_are_current() {
    ...
    assert_contains_all(
        "Runtime 15 status-support map owns M3 child-groups row-data guard folder-backed split",
        &status_map,
        &[FOLDER_BACKED_STATUS_NAME, FOLDER_BACKED_STATUS_ID],
    );
    assert_contains_all(
        "Runtime 15 status-support date map owns M3 child-groups row-data guard folder-backed split",
        &date_map,
