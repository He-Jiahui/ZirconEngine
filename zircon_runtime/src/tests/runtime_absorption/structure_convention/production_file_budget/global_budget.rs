...
fn runtime_15_no_oversized_production_files() {
    ...
    let production_files = production_file_line_counts(&runtime_src_root);
    assert!(
        !production_files.is_empty(),
        "Runtime 15 production-file budget guard should scan at least one runtime production file"
    );

    let oversized: Vec<_> = production_files
        .iter()
        .filter_map(|(path, line_count)| {
            (*line_count >= PRODUCTION_FILE_LINE_BUDGET).then(|| (path.clone(), *line_count))
        })
        .collect();
    assert!(
        oversized.is_empty(),
        "Runtime 15 production files should stay below {PRODUCTION_FILE_LINE_BUDGET} lines; oversized files: {}",
    ...
    assert_contains_all(
        "production-file budget parent mounts global budget guard",
        &parent,
        &["mod global_budget;"],
    );
...
fn production_file_line_counts(src_root: &Path) -> Vec<(String, usize)> {
    let mut files = Vec::new();
    collect_production_file_line_counts(src_root, src_root, &mut files);
    files.sort_by(|left, right| left.0.cmp(&right.0));
    files
}

fn collect_production_file_line_counts(
    root: &Path,
    directory: &Path,
    ...
    for path in paths {
        if path.is_dir() {
            collect_production_file_line_counts(root, &path, files);
        } else if is_production_rust_file(&path) {
            let source = std::fs::read_to_string(&path)
                .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
            files.push((
                relative_production_path(root, &path),
                source.lines().count(),
            ));
        }
    }
...
fn relative_production_path(root: &Path, path: &Path) -> String {
