...
fn resource_management_generation_is_owned_without_framework_backflow() {
    const ASSET_CONTRACT: &str = include_str!("../../../../core/framework/asset.rs");
    const FRAMEWORK_ROOT: &str = include_str!("../../../../core/framework/mod.rs");
    const MANAGEMENT_PROJECTION: &str =
        include_str!("../../../../../crates/zr_resource/src/manager/management_projection.rs");
    const RESOURCE_MANAGER: &str =
        include_str!("../../../../../crates/zr_resource/src/manager/resource_manager.rs");
    ...
    let owner_path =
        repo_root.join("zircon_runtime/crates/zr_resource/src/management_generation.rs");
...
fn resource_owner_has_no_higher_layer_runtime_dependency() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("zircon_runtime must be located under the repository root");
    let resource_root = repo_root.join("zircon_runtime/crates/zr_resource/src");
...
fn resource_owner_dependency_guard_rejects_higher_layer_aliases() {
    let module_path = ["crate", "management_generation"].map(str::to_owned);
    ...
    assert!(references_higher_layer(
        "use zircon_runtime as runtime_root;"
    ));
    assert!(!references_higher_layer(
        "use crate::{ResourceId, ResourceRecord};"
    ));
    assert!(!references_higher_layer(
...
fn resource_management_owner_guard_rejects_alias_and_multiline_consumer_forms() {
    ...
    assert!(!reexports_resource_owner(
        "pub use crate::core::runtime::tasks::TaskPoolKind;",
        &nested_framework_module,
        &framework_aliases,
    ));
    assert!(!reexports_resource_owner(
