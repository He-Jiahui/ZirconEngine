...
fn assert_docs_and_status_mirror_split() {
    for (label, source) in [
        ...
        assert!(
            ...
            "{label} should mirror the compatibility_shells route-owner split status"
        );
    }
    assert!(
        DATE_MAP.contains("Runtime 15 M3 compatibility-shells route-owner split"),
        "date map should mirror the compatibility_shells route-owner split slice"
    );
    assert_contains_all(
