...
fn assert_docs_and_status_mirror_split() {
    for (label, source) in [
        ...
        assert!(
            ...
            "{label} should mirror the schedule_frame_loop route-owner split status"
        );
    }
    assert!(
        DATE_MAP.contains("Runtime 15 M3 schedule-frame-loop route-owner split"),
        "date map should mirror the schedule_frame_loop route-owner split slice"
    );
    assert_contains_all(
