...
// Sibling VM hosts consume scene-system helper results without reaching into
// the private error implementation module.
pub(super) use self::error::{ScriptSceneSystemError, ScriptSceneSystemResult};

use crate::core::framework::script::{ScriptHostHotPathMetrics, ScriptHostValue};
...
pub const SCRIPT_SCENE_FIXED_UPDATE_SYSTEM: &str = "zr_vm_language.script.scene.fixed_update";
pub const SCRIPT_SCENE_UPDATE_SYSTEM: &str = "zr_vm_language.script.scene.update";
pub const SCRIPT_SCENE_RUNTIME_SYSTEM_SET: &str = "zr_vm_language.main";
...
#[derive(Clone, Debug)]
pub struct ScriptSceneRuntimeSystem {
...
impl ScriptSceneRuntimeSystem {
    ...
    pub fn id(&self) -> &'static str {
        match self.phase {
            ScriptSceneLifecyclePhase::FixedUpdate => SCRIPT_SCENE_FIXED_UPDATE_SYSTEM,
            ScriptSceneLifecyclePhase::Update => SCRIPT_SCENE_UPDATE_SYSTEM,
        }
    }

    pub fn stage(&self) -> SystemStage {
        match self.phase {
            ScriptSceneLifecyclePhase::FixedUpdate => SystemStage::FixedUpdate,
            ScriptSceneLifecyclePhase::Update => SystemStage::Update,
        }
    }

    pub fn run(&self, context: RuntimeSceneSystemContext<'_>) -> Result<(), CoreError> {
...
fn tick_script_bindings(
    ...
    let manager = core
        ...
        .map_err(ScriptSceneSystemError::from)?;
...
fn call_export_for_binding(
    ...
    let mut callback = match binding.callbacks.borrow_mut().take(export_name) {
        ...
        None => manager
            ...
            .map_err(|source| {
                ScriptSceneSystemError::export_call(
...
fn active_script_binding_projection(
    level: &LevelSystem,
) -> ScriptSceneSystemResult<Rc<ActiveScriptBindingProjection>> {
...
pub(super) fn script_binding_number_for_entity(
    ...
    entity: EntityId,
    property: &str,
) -> ScriptSceneSystemResult<Option<f64>> {
    let projection = active_script_binding_projection(level)?;
...
fn collect_active_script_binding_projection(
    world: &World,
) -> ScriptSceneSystemResult<ActiveScriptBindingProjection> {
    ...
    for (entity, value) in rows {
        let bindings = serde_json::from_value::<Vec<RuntimeSceneScriptBinding>>(value.clone())
            .map_err(|source| ScriptSceneSystemError::invalid_binding_component(entity, source))?;
