...
impl ResourceManagementScan {
    pub fn next_row(&mut self) -> Option<Arc<ResourceManagementRow>> {
        ...
        #[cfg(feature = "profiling")]
        {
            self.profile_metrics.rows_emitted = self.profile_metrics.rows_emitted.saturating_add(1);
        }
        Some(row)
    }
...
impl ResourceManagementGeneration {
    ...
    fn page_with_profile_metrics<const PROFILE: bool>(
        ...
        let mut profile_metrics = ResourceManagementPageProfileMetrics::default();
        ...
        while ordered.len() < take_count {
            ...
            for (shard_index, shard) in self.shards.iter().enumerate() {
                if PROFILE {
                    profile_metrics.shard_candidate_checks =
                        profile_metrics.shard_candidate_checks.saturating_add(1);
                }
                let rows = shard.rows();
                ...
                {
                    if PROFILE {
                        profile_metrics.filtered_rows_skipped =
                            profile_metrics.filtered_rows_skipped.saturating_add(1);
                    }
            ...
            ordered.push(row);
            if PROFILE {
                profile_metrics.candidate_rows = profile_metrics.candidate_rows.saturating_add(1);
            }
        }
        let rows: Arc<[Arc<ResourceManagementRow>]> = ordered
            .into_iter()
            .skip(offset)
            .take(limit)
            .collect::<Vec<_>>()
            .into();
        if PROFILE {
            profile_metrics.rows_returned = rows.len() as u64;
        }
        (
            ResourceManagementPage {
