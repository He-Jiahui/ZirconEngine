...
#[test]
fn optimization_batch_20260826hp_runtime262_sorts_owned_overview_storage_in_place() {
    let values = vec![
        "charlie".to_string(),
        "alpha".to_string(),
        "bravo".to_string(),
...
#[test]
fn optimization_batch_20260826hp_runtime262_rebuilds_owned_overview_indexes() {
    let source = include_str!("../record_set.rs");
    let start = source
        .find("pub fn overview_sorted(")
        .expect("record-set overview sort function");
    let end = source[start..]
        .find("\n    }")
        .map(|offset| start + offset)
        .expect("record-set overview sort boundary");
    let body = &source[start..end];

    assert!(body.contains("sort_owned_overview("));
    assert!(!body.contains(".sorted(sort_order)"));
    assert!(source.contains("RenderMaterialManagementStatusIndex::from_overview_records"));
    assert!(source.contains("RenderMaterialManagementIssueIndex::from_overview_records"));
}

#[test]
#[ignore = "managed release performance evidence"]
fn optimization_batch_20260826hp_runtime262_owned_overview_sort_release_benchmark() {
    let source = (0..RECORD_COUNT)
        .map(|index| format!("overview-{index:08}-{}", "x".repeat(64)))
        .collect::<Vec<_>>();
    ...
    for sample_index in 0..SAMPLE_PAIRS {
        let mut measure_legacy = || {
            ...
            for _ in 0..OPERATIONS_PER_SAMPLE {
                black_box(legacy_sorted_overview(black_box(&source)));
            }
        ...
        let mut measure_optimized = || {
            let started = Instant::now();
            for _ in 0..OPERATIONS_PER_SAMPLE {
                black_box(owned_sorted_overview(black_box(&source)));
            }
    ...
    println!(
        "RUNTIME262_OWNED_OVERVIEW_SORT_BENCH_V1 \
...
fn legacy_sorted_overview(source: &[String]) -> Vec<String> {
...
fn owned_sorted_overview(source: &[String]) -> Vec<String> {
    let owned = source.to_vec();
