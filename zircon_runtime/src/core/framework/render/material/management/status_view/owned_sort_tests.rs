...
#[test]
fn optimization_batch_20260826hm_runtime259_sorts_owned_storage_in_place() {
    let values = vec![
        "charlie".to_string(),
        "alpha".to_string(),
        "bravo".to_string(),
...
#[test]
fn optimization_batch_20260826hm_runtime259_owned_constructors_skip_second_clone() {
    let source = include_str!("../status_view.rs");
...
fn optimization_batch_20260826hm_runtime259_owned_status_view_sort_release_benchmark() {
    let source = (0..RECORD_COUNT)
        .map(|index| format!("material-{index:08}-{}", "x".repeat(64)))
        .collect::<Vec<_>>();
    ...
    println!(
        "RUNTIME259_OWNED_STATUS_VIEW_SORT_BENCH_V1 \
