...
pub fn source_cubemap_mip_chain_with_bake_artifact(
    ...
    if descriptor.face_size() != source.face_size() || descriptor.mip_count() != source.mip_count()
    {
        return Err(SourceCubemapBakeArtifactError::LayoutMismatch {
            expected_face_size: source.face_size(),
            actual_face_size: descriptor.face_size(),
            expected_mip_count: source.mip_count(),
    ...
    let expected = source_cubemap_sample_count(source.face_size(), source.mip_count());
