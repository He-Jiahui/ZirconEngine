...
#[test]
fn node_record_uses_direct_default_lookup_branches() {
    ...
    assert!(
        node_record.contains("let parent = match self.hierarchy.get(&entity)")
    ...
    assert!(
        node_record.contains("let transform = match self.local_transforms.get(&entity)")
    ...
    assert!(
        node_record.contains("let active = match self.active_self.get(&entity)")
    ...
    assert!(
        node_record.contains("let render_layer_mask = match self.render_layer_masks.get(&entity)")
    ...
    assert!(
        node_record.contains("let mobility = match self.mobility.get(&entity)")
