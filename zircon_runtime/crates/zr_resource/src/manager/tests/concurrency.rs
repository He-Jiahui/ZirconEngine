...
use crate::{
    ...
    ResourceManager, ResourceMutationBatch, ResourceRecord, ResourceState,
};
...
fn prepared_commit_stays_private_and_holds_the_shared_publication_gate() {
    ...
    assert_eq!(committed.state, crate::ResourceState::Ready);
...
fn dropping_a_prepared_commit_releases_the_gate_without_publishing_state() {
    ...
    let stored = manager
        ...
        .get(committed.id)
        .cloned()
        .expect("committed follower record");
    assert_eq!(stored.id, committed.id);
    assert_eq!(stored.kind, committed.kind);
    assert_eq!(stored.primary_locator, committed.primary_locator);
    assert_eq!(stored.state, ResourceState::Ready);
