...
use super::{encode_nul_terminated_path, windows_api_path};

const MARKER: &str = "RUNTIME238_WINDOWS_PATH_SINGLE_ALLOCATION_ENCODING_BENCH_V1";
const SAMPLE_PAIRS: usize = 17;
const REPEATS: usize = 8_192;

#[test]
fn optimization_batch_20260826gr_runtime238_windows_path_preserves_utf16_and_single_nul() {
    let path = Path::new("C:\\Zircon\\caf\u{e9}\\scene.zscene");
    let encoded = windows_api_path(path).expect("absolute fixture path should encode");
    let expected = path
        .as_os_str()
        .encode_wide()
...
#[test]
fn optimization_batch_20260826gr_runtime238_windows_path_collects_terminator_once() {
    ...
    assert!(!source.contains("let mut wide = encoded;\n        wide.push(0);"));
}

#[test]
#[ignore = "managed Windows release performance evidence"]
fn optimization_batch_20260826gr_runtime238_windows_path_single_allocation_encoding_bench() {
