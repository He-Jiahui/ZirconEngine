...
#[test]
fn batch_plan_scale_matrix_batches_same_generation_images_once() {
    for item_count in [1_u32, 1_000, 10_000] {
        let commands = (0..item_count)
            .map(|item| {
                image_with_generation(
                    item as i32,
                    UiSurfaceRect::new(item as f32 * 2.0, 0.0, 1.0, 1.0),
                    "atlas://editor/icons",
                    23,
                )
            })
            .collect();
        let draw_list = UiSurfaceDrawList::new((item_count.saturating_mul(2), 1), None, commands);
        ...
        assert_eq!(plan.image_upload_sources[0].resource_generation, 23);
        assert_eq!(
            plan.image_upload_sources[0].command_indices.len(),
            item_count as usize
        );
        let [DrawOp::Image(draw)] = plan.ops.as_slice() else {
            panic!("same-generation images must share one draw");
        };
        assert_eq!(draw.resource_generation, 23);
        assert_eq!(
            draw.vertex_end.saturating_sub(draw.vertex_start),
            item_count.saturating_mul(UI_QUAD_VERTEX_COUNT)
        );
    }
}
