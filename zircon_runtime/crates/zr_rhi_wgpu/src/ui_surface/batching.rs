...
fn batch_draw_plan_from_items(mut items: Vec<DrawItem>) -> BatchDrawPlan {
    ...
    let mut image_source_indices_by_resource = BTreeMap::<(String, u64), Vec<usize>>::new();
...
fn image_upload_sources(
    source_indices_by_resource: BTreeMap<(String, u64), Vec<usize>>,
) -> Vec<ImageUploadSource> {
    source_indices_by_resource
        .into_iter()
        .map(
            |((resource_key, resource_generation), mut command_indices)| {
...
fn push_layer_image_draws(
    ...
    source_indices_by_resource: &mut BTreeMap<(String, u64), Vec<usize>>,
) {
    let mut item_indices_by_resource = BTreeMap::<(&str, u64), (Vec<usize>, UiSurfaceRect)>::new();
    for item_index in layer {
        let DrawItem::Image(item) = &items[*item_index] else {
            continue;
        };
        // Keep the per-item identity borrowed; the compiled draw owns one identity per batch.
        let entry = item_indices_by_resource
            .entry((item.resource_key.as_str(), item.resource_generation))
            .or_insert_with(|| (Vec::new(), item.rect));
        entry.0.push(*item_index);
        entry.1 = union_rects(entry.1, item.rect);
    }
    for ((resource_key, resource_generation), (item_indices, bounds)) in item_indices_by_resource {
        ...
        source_indices_by_resource
            .entry((resource_key.to_owned(), resource_generation))
            .or_default()
