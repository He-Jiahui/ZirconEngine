...
use zircon_runtime_interface::{
    ...
    ZrOwnedResultV2, ZrRuntimeAllocationId, ZrRuntimeApiV8, ZrRuntimeSessionConfigV3,
...
fn create_session(api: &ZrRuntimeApiV8) -> ZrRuntimeSessionHandle {
...
fn destroy_session(api: &ZrRuntimeApiV8, session: ZrRuntimeSessionHandle) -> ZrStatusCode {
    let destroy = api.destroy_session.expect("destroy_session");
    unsafe { destroy(session) }.status_code()
}

fn snapshot_output(api: &ZrRuntimeApiV8, session: ZrRuntimeSessionHandle) -> ZrOwnedResultV2 {
...
fn record_performance_metric(metric: &str) {
    println!("{metric}");
    let Some(path) = std::env::var_os("ZIRCON_RUNTIME_V8_PERF_OUTPUT") else {
        return;
    };
    let mut output = OpenOptions::new()
        ...
        .append(true)
        .open(path)
        .expect("open V8 performance output");
    writeln!(output, "{metric}").expect("write V8 performance output");
}

#[test]
fn runtime_v8_owned_results_require_opaque_exactly_once_release() {
...
#[test]
fn runtime_v8_release_is_concurrent_and_exactly_once() {
    let _guard = runtime_test_guard();
    let api = runtime_api();
    let session = create_session(api);
    let output = snapshot_output(api, session);
...
#[test]
fn runtime_v8_destroy_is_retryable_after_outstanding_result_release() {
    let _guard = runtime_test_guard();
    let api = runtime_api();
    let session = create_session(api);
    let output = snapshot_output(api, session);
...
#[test]
fn runtime_v8_release_rejects_a_different_session_without_changing_owner_census() {
    let _guard = runtime_test_guard();
    let api = runtime_api();
...
#[test]
fn runtime_v8_release_performance_acceptance() {
    let _guard = runtime_test_guard();
    let api = runtime_api();
    let session = create_session(api);
    ...
    record_performance_metric(&format!(
        "RUNTIME_INTERFACE01_V8_RELEASE_PERF iterations={MEASURED_ITERATIONS} p50_ns={p50_ns} p95_ns={p95_ns} p99_ns={p99_ns} throughput_releases_per_second={throughput:.2}"
    ));
