...
use zircon_manager::{AssetChangeKind as FacadeAssetChangeKind, AssetChangeRecord};
...
impl ProjectAssetManager {
    pub(super) fn project_read(&self) -> RwLockReadGuard<'_, Option<ProjectManager>> {
        self.project.read().expect("asset project lock poisoned")
    }

    pub(super) fn project_write(&self) -> RwLockWriteGuard<'_, Option<ProjectManager>> {
        self.project.write().expect("asset project lock poisoned")
    }

    pub(super) fn broadcast(&self, changes: Vec<AssetChangeRecord>) {
        if changes.is_empty() {
            return;
        }
    ...
    pub(super) fn restart_watcher(&self) -> Result<(), CoreError> {
        let assets_root = {
    ...
    pub(super) fn sync_project_resources(&self, project: &ProjectManager) -> Result<(), CoreError> {
