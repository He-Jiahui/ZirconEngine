...
use crate::{
    AssetCatalogRecord, AssetImportError, AssetMetaDocument, AssetUri, PreviewArtifactKey,
    PreviewCache, PreviewScheduler, PreviewState, ProjectManager,
};

use super::default_editor_asset_manager::DefaultEditorAssetManager;
use super::reference_analysis::direct_references;
use super::super::{EditorAssetChangeKind, EditorAssetChangeRecord};
...
impl DefaultEditorAssetManager {
    pub fn sync_from_project(&self, project: ProjectManager) -> Result<(), AssetImportError> {
        ...
        let reference_graph = crate::ReferenceGraph::rebuild(catalog_by_uuid.values());
        let change = {
            let mut state = self
                .state
                .write()
